...
/*
    * NkUIRHIBackend.cpp
    *
    * Implementation du backend NKRHI reutilisable pour les draw lists NKUI.
    *
    * Ce fichier fait partie de Nkentseu.
    *
    * Portage fonctionnellement identique de la demo Base04 NkUINKRHIBackend.cpp
    * (meme pipeline, memes buffers, meme upload texture/atlas, meme scissor/clip,
    * meme conversion de vertices). Seules differences :
    *   - allocations CPU temporaires via NKMemory (NkAlloc/NkFree) au lieu de
    *     new[]/delete[] (regle dure du projet : zero new/delete brut) ;
    *   - noms de debug neutres (plus de suffixe "_Demo").
*/

/*
 * NKUI_MAINTENANCE_GUIDE
 * Responsibility: Reusable NKRHI adapter implementation for NKUI rendering.
...
float4 PSMain(PSIn i) : SV_Target {
    ...
    return i.col * tc;
        ...
        bool NkUIRHIBackend::Init(NkIDevice* device, NkRenderPassHandle renderPass, NkGraphicsApi api) {
            if (!device) {
                return false;
            }
            mDevice = device;
            ...
            if (!CreatePipeline()) {
                logger.Errorf("[NkUIRHIBackend] Failed to create pipeline\n");
                Destroy();
                return false;
            }
            if (!CreateResources()) {
                logger.Errorf("[NkUIRHIBackend] Failed to create resources\n");
                Destroy();
                return false;
            }
            return true;
        ...
        bool NkUIRHIBackend::CreatePipeline() {
            NkShaderDesc shaderDesc;
            shaderDesc.debugName = "NkUI_Backend";
            ...
            if (mApi == NkGraphicsApi::NK_GFX_API_OPENGL || mApi == NkGraphicsApi::NK_GFX_API_SOFTWARE) {
                ...
                if (!vert.success || !frag.success) {
                    logger.Errorf("[NkUIRHIBackend] Vulkan shader compilation failed\n");
                    return false;
                }
            ...
            pd.debugName = "NkUI_Backend_Pipeline";
        ...
        bool NkUIRHIBackend::CreateResources() {
            ...
            whiteDesc.debugName = "NkUI_Backend_White";
        ...
        bool NkUIRHIBackend::CreateDescriptorSetForTexture(NkTextureHandle texture, NkDescSetHandle& outSet) {
        ...
        bool NkUIRHIBackend::EnsureGeometryBuffers(uint64 requiredVtxBytes, uint64 requiredIdxBytes, bool allowShrink) {
            if (!mDevice) {
                return false;
            }
        ...
        void NkUIRHIBackend::BindTexture(NkICommandBuffer* cmd, uint32 texId) {
        ...
        void NkUIRHIBackend::RetireTextureEntry(const TextureEntry& entry) {
        ...
        void NkUIRHIBackend::CollectRetiredTextures() {
        ...
        void NkUIRHIBackend::Submit(NkICommandBuffer* cmd, const NkUIContext& ctx, uint32 fbW, uint32 fbH) {
            ...
            if (totalVtxCount > 0xFFFFFFFFull || totalIdxCount > 0xFFFFFFFFull) {
                logger.Warn("[NkUIRHIBackend] UI geometry overflow: vtx=%llu idx=%llu\n",
        ...
        bool NkUIRHIBackend::UploadTextureInternal(uint32 texId, const uint8* data, int32 width, int32 height, bool rgba8) {
            ...
            usize convertedBytes = 0;
            if (!rgba8) {
                const int32 count = width * height;
                convertedBytes = static_cast<usize>(count) * 4u;
                converted = static_cast<uint8*>(nkentseu::memory::NkAlloc(convertedBytes));
                if (!converted) {
                    return false;
                }
            ...
            if (canReuse) {
                ...
                if (converted) {
                    nkentseu::memory::NkFree(converted);
                }
                return ok;
            }
            ...
            desc.debugName = "NkUI_Backend_Texture";
        ...
        bool NkUIRHIBackend::UploadTextureGray8(uint32 texId, const uint8* data, int32 width, int32 height) {
            return UploadTextureInternal(texId, data, width, height, false);
        }

        bool NkUIRHIBackend::HasTexture(uint32 texId) const noexcept {
            if (texId == 0) {
                return true;
            }

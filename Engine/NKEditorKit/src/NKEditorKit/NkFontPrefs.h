...
namespace nkentseu {
	namespace editorkit {
		...
		inline const char *const *NkUiFontNames(int32 *n) {
			static const char *a[] = {"Inter",	  "Karla", "DroidSans", "Roboto",
									  "Segoe UI", "Arial", "Tahoma",	"Verdana"};
			*n = 8;
			return a;
		}

		inline const char *const *NkCodeFontNames(int32 *n) {
			static const char *a[] = {"DejaVuSansMono", "Cousine",		 "SourceCodePro", "ProggyClean",   "Consolas",
									  "Courier New",	"Cascadia Code", "Cascadia Mono", "Lucida Console"};
			*n = 9;
			return a;
		}
		...
		inline bool NkEmbeddedIdFromName(const NkString &name, NkEmbeddedFontId &out) {
			...
			static const E t[] = {
				{"Inter", NkEmbeddedFontId::Inter},
				{"Karla", NkEmbeddedFontId::Karla},
				{"DroidSans", NkEmbeddedFontId::DroidSans},
				{"Roboto", NkEmbeddedFontId::Roboto},
				{"DroidSerif", NkEmbeddedFontId::DroidSerif},
				{"Cousine", NkEmbeddedFontId::Cousine},
				{"SourceCodePro", NkEmbeddedFontId::SourceCodePro},
				{"ProggyClean", NkEmbeddedFontId::ProggyClean},
				{"ProggyTiny", NkEmbeddedFontId::ProggyTiny},
				{"DejaVuSansMono", NkEmbeddedFontId::DejaVuSansMono},
			};
			for (const E &e : t)
				if (name == e.n) {
					out = e.id;
...
#endif
			if (home && *home)
				return NkString(home) + "/.nkcode_fonts.cfg";
		...
		inline void NkLoadFontPrefs(NkFontPrefs &p) {
			...
			while (std::fgets(line, sizeof(line), f)) {
				...
				*eq = 0;
				char *key = line;
				char *val = eq + 1;
				usize n = std::strlen(val);
				while (n > 0 && (val[n - 1] == '\n' || val[n - 1] == '\r' || val[n - 1] == ' '))
					val[--n] = 0;
				if (std::strcmp(key, "ui.font") == 0)
					p.uiFont = val;
				else if (std::strcmp(key, "code.font") == 0)
					p.codeFont = val;
				else if (std::strcmp(key, "ui.size") == 0)
					p.uiSize = static_cast<float32>(std::atof(val));
				else if (std::strcmp(key, "code.size") == 0)
					p.codeSize = static_cast<float32>(std::atof(val));
			}
			std::fclose(f);
			if (p.uiSize < 8.f)
				p.uiSize = 8.f;
		...
		inline void NkThemeFieldList(nkgui::NkGuiTheme &t, NkThemeField *out, int32 *n) {
			...
			auto OFF = [&](nkgui::NkColor *p) {
				return static_cast<nkft_uint32>(reinterpret_cast<char *>(p) - reinterpret_cast<char *>(base));
			};
			NkThemeField f[] = {
				{"bgPrimary", OFF(&t.bgPrimary)},
				{"panel", OFF(&t.panel)},
				{"header", OFF(&t.header)},
				{"button", OFF(&t.button)},
				{"buttonHover", OFF(&t.buttonHover)},
				{"buttonActive", OFF(&t.buttonActive)},
				{"border", OFF(&t.border)},
				{"text", OFF(&t.text)},
				{"textDisabled", OFF(&t.textDisabled)},
				{"selection", OFF(&t.selection)},
				{"accent", OFF(&t.accent)},
				{"track", OFF(&t.track)},
				{"tabBar", OFF(&t.tabBar)},
				{"tab", OFF(&t.tab)},
				{"tabHover", OFF(&t.tabHover)},
				{"tabActive", OFF(&t.tabActive)},
			};
			*n = 16;
...
#endif
			if (home && *home)
				return NkString(home) + "/.nkcode_theme.cfg";
		...
		inline bool NkLoadTheme(nkgui::NkGuiTheme &t, const char *path = nullptr) {
			...
			while (std::fgets(line, sizeof(line), f)) {
				...
				*eq = 0;
				...
				if (std::sscanf(eq + 1, "%d,%d,%d,%d", &r, &g, &b, &a) < 3)
					continue;
				for (int32 i = 0; i < n; ++i)
					if (std::strcmp(line, fl[i].name) == 0) {
						nkgui::NkColor *c = reinterpret_cast<nkgui::NkColor *>(base + fl[i].off);
						c->r = (nkft_uint8)r;
						c->g = (nkft_uint8)g;
						c->b = (nkft_uint8)b;
...
#endif
			if (home && *home)
				return NkString(home) + "/.nkcode_syntax.cfg";

...
namespace nkentseu {
	...
	struct NkPrefabComponentData {
			...
			NkPrefabComponentData(const char *type, const char *json) noexcept : typeName(type), jsonValue(json) {
			}
	};
	...
	struct NkPrefabChild {
			...
			NkPrefabChild(const char *n, const char *path = nullptr) noexcept : name(n), prefabPath(path ? path : "") {
			}
	};
	...
	class NkPrefab {
		public:
			...
			explicit NkPrefab(const char *prefabName) noexcept : name(prefabName) {
				guid = ecs::detail::FNV1a(prefabName);
			}

			// ── Builder fluide ─────────────────────────────────────────────
			template <typename T, typename... Args> NkPrefab &WithComponent(Args &&...args) noexcept {
			 ...
			 */
			[[nodiscard]] ecs::NkEntityId Instantiate(ecs::NkWorld &world,
			 ...
			 */
			[[nodiscard]] ecs::NkGameObject InstantiateGameObject(ecs::NkWorld &world,
																  const char *instanceName = nullptr) const noexcept {
			 ...
			 */
			void InstantiateBatch(ecs::NkWorld &world, uint32 count, NkVector<ecs::NkEntityId> &out) const noexcept;
			...
			[[nodiscard]] const NkPrefabComponentData *GetComponentData(const char *typeName) const noexcept {
				return components.Find(NkString(typeName));
			}

		private:
			static bool SerializeComponent(void *data, ecs::NkComponentId cid, char *outJson, uint32 bufSize) noexcept;
	};
	...
	struct NkPrefabInstance {
			...
			NkPrefabInstance(ecs::NkEntityId root, const char *path, const char *name) noexcept
	...
	class NkPrefabRegistry {
		public:
			...
			[[nodiscard]] const NkPrefab *Get(const char *path) const noexcept {
				return mPrefabs.Find(NkString(path));
			}

			[[nodiscard]] const NkPrefab *Get(const NkString &path) const noexcept {
				return mPrefabs.Find(path);
			}

			template <typename Fn> void ForEach(Fn &&fn) const noexcept {
				mPrefabs.ForEach([&](const NkString &path, const NkPrefab &prefab) { fn(path.CStr(), prefab); });
			}

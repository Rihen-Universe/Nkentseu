		...
		class NkComponentMetaRegistry {
			public:
				...
				const NkComponentMeta *Find(const char *typeName) const noexcept {
					for (nk_usize i = 0; i < mMetas.Size(); ++i)
						if (NkStrEqual(mMetas[i].typeName, typeName))
							return &mMetas[i];
					return nullptr;
				}

				const NkVector<NkComponentMeta> &All() const noexcept {
					return mMetas;
				}
...
#define NK_COMPONENT_BEGIN(Type, DisplayName)                                                                          \
	static struct _NkMetaAutoReg_##Type {                                                                              \
			_NkMetaAutoReg_##Type() noexcept {                                                                         \
				...
				_meta.addFn = [](nkentseu::ecs::NkWorld &w, nkentseu::ecs::NkEntityId id) { w.Add<Type>(id); };
...
#define NK_FIELD_RANGE(field, display, ftype, mn, mx, st)                                                              \
	{                                                                                                                  \
		...
		_f.offset = offsetof(Type, field);                                                                             \
		_f.size = sizeof(((Type *)0)->field);                                                                          \
		_f.rangeMin = (float)(mn);                                                                                     \
		_f.rangeMax = (float)(mx);                                                                                     \
		_f.step = (float)(st);                                                                                         \
		_meta.fields.PushBack(_f);                                                                                     \
	}

#define NK_FIELD_VEC3(field, display) NK_FIELD(field, display, Vec3f)

#define NK_FIELD_QUAT(field, display) NK_FIELD(field, display, Quatf)

#define NK_FIELD_STR_FIXED(field, display, maxLen)                                                                     \
	{                                                                                                                  \
		nkentseu::ecs::NkFieldMeta _f;                                                                                 \
		_f.name = display;                                                                                             \
		_f.type = nkentseu::ecs::NkFieldType::StringFixed;                                                             \
		_f.offset = offsetof(Type, field);                                                                             \
		_f.size = sizeof(((Type *)0)->field);                                                                          \
		_f.strMaxLen = maxLen;                                                                                         \
		_meta.fields.PushBack(_f);                                                                                     \
...
#define NK_FIELD_BOOL(field, display) NK_FIELD(field, display, Bool)

#define NK_FIELD_ASSET(field, display) NK_FIELD(field, display, AssetPath)

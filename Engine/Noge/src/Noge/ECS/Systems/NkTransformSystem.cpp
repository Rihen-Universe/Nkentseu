...
namespace nkentseu {
	...
	void NkTransformSystem::Execute(NkWorld &world, float32 /*dt*/) noexcept {
		...
		world.Query<NkTransform>().ForEach([&](NkEntityId id, const NkTransform &tf) {
			// Vérifier si c'est une racine
			bool isRoot = true;
			if (world.Has<NkParent>(id)) {
				const auto *parent = world.Get<NkParent>(id);
				if (parent && parent->entity.IsValid() && world.IsAlive(parent->entity)) {
	...
	// =========================================================================
	void NkTransformSystem::ProcessEntity(NkWorld &world, NkEntityId id, const NkMat4f &parentWorld,
		...
		if (needsUpdate) {
			...
			tf->worldPosition = {tf->worldMatrix[3][0], tf->worldMatrix[3][1], tf->worldMatrix[3][2]};
		...
		for (nk_uint32 i = 0; i < children->count; ++i) {
			...
			if (!childId.IsValid() || !world.IsAlive(childId))
				continue;

			mStack.PushBack({
				childId, tf->worldMatrix,

...
namespace nkentseu {
	namespace ecs {
		...
		class NkDLLScriptAdapter final : public NkScriptComponent {
			public:
				NkDLLScriptAdapter(const NkScriptDLLFactory &factory) noexcept : mFactory(factory) {
		...
		class NkScriptLoader {
			public:
				...
				bool LoadDLL(const char *path) noexcept {
					...
					// Récupère la factory
					auto getFactory =
						reinterpret_cast<NkScript_GetFactoryFn>(NKECS_FIND_SYMBOL(dll.handle, "nkecs_get_factory"));
					if (!getFactory) {
						dll.Unload();
						return false;
					}
			...
			private:
				...
				void ReloadDLL(NkWorld &world, uint32 idx) noexcept {
					...
					if (!LoadDLL(path))
						return; // échec → garder l'ancienne (déjà déchargée)
		...
		class NkScriptDLLBase {
			public:
				...
				[[nodiscard]] NkEntityId GetEntityId() const noexcept {
					return mEntity;
				}

				[[nodiscard]] float32 GetDt() const noexcept {
					return mDt;
				}
				...
				template <typename T> T &AddComponent(const T &value = T{}) noexcept {
					return mWorld->Add<T>(mEntity, value);
				}

				template <typename T> void RemoveComponent() noexcept {
					if (mWorld)
						mWorld->Remove<T>(mEntity);
				}
...
#define NK_EXPORT_DLL_SCRIPT(ClassName)                                                                                \
	static void _nkecs_dispatch(void *self, nkentseu::ecs::NkScriptContext *ctx, void (ClassName::*fn)()) {            \
		auto *s = static_cast<ClassName *>(self);                                                                      \
		s->_SetContext(static_cast<nkentseu::ecs::NkWorld *>(ctx->world),                                              \
					   nkentseu::ecs::NkEntityId::Unpack(ctx->entityPack), ctx->dt);                                   \
		(s->*fn)();                                                                                                    \
	}                                                                                                                  \
	extern "C" {                                                                                                       \

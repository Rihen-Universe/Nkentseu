	...
	namespace ecs {
		...
		namespace CSharpInternals {
			...
			static uint64 NkCS_CreateEntity(void *worldPtr, const char *name) {
				...
				if (name)
					world->Add<NkName>(id, NkName(name));
			...
			// Transform
			static void NkCS_GetTransform(void *worldPtr, uint64 entityPack, float32 *outX, float32 *outY,
										  float32 *outZ) {
				auto *world = static_cast<NkWorld *>(worldPtr);
				if (!world || !outX)
					return;
			...
			static void NkCS_SetPosition(void *worldPtr, uint64 entityPack, float32 x, float32 y, float32 z) {
				auto *world = static_cast<NkWorld *>(worldPtr);
				if (!world)
					return;
				auto id = NkEntityId::Unpack(entityPack);
			...
			static void NkCS_Translate(void *worldPtr, uint64 entityPack, float32 dx, float32 dy, float32 dz) {
				auto *world = static_cast<NkWorld *>(worldPtr);
				if (!world)
					return;
				auto id = NkEntityId::Unpack(entityPack);
			...
			static void NkCS_Rotate(void *worldPtr, uint64 entityPack, float32 axX, float32 axY, float32 axZ,
									float32 angleDeg) {
				auto *world = static_cast<NkWorld *>(worldPtr);
				if (!world)
					return;
			...
			static void NkCS_LookAt(void *worldPtr, uint64 entityPack, float32 tx, float32 ty, float32 tz) {
				auto *world = static_cast<NkWorld *>(worldPtr);
				if (!world)
					return;
				auto id = NkEntityId::Unpack(entityPack);
...
#ifdef NKECS_MONO_AVAILABLE
					if (!mInitialized || mAssemblyCount >= kMaxAssemblies)
						return false;
...
#undef NKCS_REG
		...
		class NkCSharpScriptAdapter final : public NkScriptComponent {
			public:
				...
				void OnCollisionEnter(NkWorld &world, NkEntityId self, NkEntityId other) noexcept override {
			...
			private:
...
#else
				NkMonoMethod *FindMethod(const char *) noexcept {
					return nullptr;
				}
#endif

				void InvokeVoid(NkMonoMethod *method, NkWorld &world, NkEntityId self, float32 dt) noexcept {
#ifdef NKECS_MONO_AVAILABLE
					if (!method || !mObject)
						return;
				...
				void InvokeCollision(NkMonoMethod *method, NkWorld &world, NkEntityId self, NkEntityId other) noexcept {
					(void)method;
					(void)world;
					(void)self;
					(void)other;

...
print("[NkECS] Module Python 'nkecs' initialisé")
		...
		class NkPythonScriptAdapter final : public NkScriptComponent {
			...
			private:
				...
				void CallMethod(const char *method, NkWorld &world, NkEntityId self, float32 dt) noexcept {
#ifdef NKECS_PYTHON_AVAILABLE
					if (!mInstance)
						return;
				...
				void CallCollisionMethod(const char *method, NkWorld &world, NkEntityId self,
										 NkEntityId other) noexcept {
#ifdef NKECS_PYTHON_AVAILABLE
					if (!mInstance)
						return;
					PyObject *ctx = PyDict_New();
					...
					PyDict_SetItemString(ctx, "entity_id", PyLong_FromUnsignedLongLong(self.Pack()));
					PyObject *result =
						PyObject_CallMethod(mInstance, method, "OK", ctx, (unsigned long long)other.Pack());
					if (!result && PyErr_Occurred())
						PyErr_Print();
					else if (result)
						Py_DECREF(result);

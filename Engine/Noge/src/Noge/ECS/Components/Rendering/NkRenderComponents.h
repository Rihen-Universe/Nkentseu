...
namespace nkentseu {
	namespace ecs {
		...
		struct NkColor4 {
				...
				static NkColor4 White() noexcept {
					return {1, 1, 1, 1};
				}

				static NkColor4 Black() noexcept {
					return {0, 0, 0, 1};
				}

				static NkColor4 Red() noexcept {
					return {1, 0, 0, 1};
				}

				static NkColor4 Green() noexcept {
					return {0, 1, 0, 1};
				}

				static NkColor4 Blue() noexcept {
					return {0, 0, 1, 1};
				}

				static NkColor4 Clear() noexcept {
					return {0, 0, 0, 0};
				}

				static NkColor4 Yellow() noexcept {
					return {1, 1, 0, 1};
				}

				static NkColor4 Cyan() noexcept {
					return {0, 1, 1, 1};
				}

				static NkColor4 Magenta() noexcept {
					return {1, 0, 1, 1};
				}
		 ...
		 */
		enum class NkLightType : uint8 { Directional = 0, Point, Spot, Area, Ambient };

		enum class NkBlendMode : uint8 { Opaque = 0, AlphaBlend, Additive, Multiply, PremultAlpha };

		// Modes de rendu 2D
		enum class NkRenderer2DType : uint8 { None = 0, Quad, Sprite, Text, Shape, Tilemap, Custom };

		struct NkMeshComponent {
				nk_uint64 meshHandle = 0;
				NkString meshPath;
		...
		struct NkSkinnedMeshComponent {
				nk_uint64 meshHandle = 0;
				NkString meshPath;
				nk_uint64 skeletonHandle = 0;
				NkString skeletonPath;

...
namespace nkentseu {
	 ...
	 */
	template <typename T> class NkComponentHandle {
		public:
			// ── Constructeurs ──────────────────────────────────────────────
			...
			NkComponentHandle(ecs::NkWorld *world, ecs::NkEntityId id) noexcept : mWorld(world), mId(id) {
			}
			...
			// ── Accès au contexte ──────────────────────────────────────────
			[[nodiscard]] ecs::NkEntityId EntityId() const noexcept {
				return mId;
			}

			[[nodiscard]] ecs::NkWorld *World() const noexcept {
				return mWorld;
			}
	 ...
	 */
	template <typename T> class NkOptionalComponent {
		public:
			NkOptionalComponent() noexcept = default;

			NkOptionalComponent(ecs::NkWorld *world, ecs::NkEntityId id) noexcept : mWorld(world), mId(id) {
			}
	 ...
	 * @endcode
	 */
	template <typename T> class NkRequiredComponent {
		public:
			NkRequiredComponent() noexcept = default;

			NkRequiredComponent(ecs::NkWorld *world, ecs::NkEntityId id) noexcept : mWorld(world), mId(id) {
			}
		...
		private:
			T &Resolve() const noexcept {
				...
				NKECS_ASSERT(ptr != nullptr && "NkRequiredComponent: composant invariant absent — "

...
namespace nkentseu {
	namespace ecs {
		...
		struct NkRigidbody2D {
				...
				// ── API runtime ───────────────────────────────────────────────────────────
				void AddForce(NkVec2 f) noexcept {
					force += f;
				}

				void AddImpulse(NkVec2 imp) noexcept {
					velocity += imp * (mass > 0.f ? 1.f / mass : 0.f);
				}

				void AddTorque(float32 t) noexcept {
					torque += t;
				}

				void SetVelocity(NkVec2 v) noexcept {
					velocity = v;
				}
		...
		struct NkRigidbody3D {
				...
				void AddForce(NkVec3 f) noexcept {
					force += f;
				}

				void AddImpulse(NkVec3 imp) noexcept {
					velocity += imp * (mass > 0.f ? 1.f / mass : 0.f);
				}

				void AddTorque(NkVec3 t) noexcept {
					torque += t;
				}
				...
				void SetVelocity(NkVec3 v) noexcept {
					velocity = v;
				}
		...
		struct NkCharacterController {
				...
				// ── API ───────────────────────────────────────────────────────────────────
				void Move(NkVec3 motion) noexcept {
					velocity = motion;
				}
		};

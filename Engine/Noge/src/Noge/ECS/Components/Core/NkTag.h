...
namespace nkentseu {
	namespace ecs {
		...
		struct NkName {
				...
				void Set(const char *name) noexcept {
					std::strncpy(value, name, kMaxLen - 1);
				}
		...
		struct NkTag {
				...
				void Add(NkTagBit tag) noexcept {
					bits |= static_cast<uint64>(tag);
				}

				void Remove(NkTagBit tag) noexcept {
					bits &= ~static_cast<uint64>(tag);
				}
				...
				bool HasNone(NkTagBit tags) const noexcept {
					return !HasAny(tags);
				}
		...
		struct NkLayer {
				...
				explicit NkLayer(uint32 layer, uint32 mask = 0xFFFFFFFFu) noexcept : layer(layer), mask(mask) {
				}

				[[nodiscard]] bool CanCollideWith(const NkLayer &other) const noexcept {
					return (mask & (1u << other.layer)) != 0 && (other.mask & (1u << layer)) != 0;
				}

				[[nodiscard]] uint32 LayerBit() const noexcept {
					return 1u << layer;
				}

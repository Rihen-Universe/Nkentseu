...
namespace nkentseu {
	namespace ecs {
		...
		namespace NkAnchors {
			inline NkAnchor TopLeft() {
				return {0, 0, 0, 0};
			}

			inline NkAnchor TopCenter() {
				return {0.5f, 0, 0.5f, 0};
			}

			inline NkAnchor TopRight() {
				return {1, 0, 1, 0};
			}

			inline NkAnchor MiddleLeft() {
				return {0, 0.5f, 0, 0.5f};
			}

			inline NkAnchor MiddleCenter() {
				return {0.5f, 0.5f, 0.5f, 0.5f};
			}

			inline NkAnchor MiddleRight() {
				return {1, 0.5f, 1, 0.5f};
			}

			inline NkAnchor BottomLeft() {
				return {0, 1, 0, 1};
			}

			inline NkAnchor BottomCenter() {
				return {0.5f, 1, 0.5f, 1};
			}

			inline NkAnchor BottomRight() {
				return {1, 1, 1, 1};
			}

			inline NkAnchor StretchFull() {
				return {0, 0, 1, 1};
			}

			inline NkAnchor StretchH() {
				return {0, 0.5f, 1, 0.5f};
			}

			inline NkAnchor StretchV() {
				return {0.5f, 0, 0.5f, 1};
			}
		} // namespace NkAnchors

		// Alignement horizontal/vertical du texte
		enum class NkTextAlign : uint8 { Left = 0, Center = 1, Right = 2, Top = 0, Middle = 1, Bottom = 2 };
		...
		struct NkEdges {
				...
				constexpr NkEdges(float32 l, float32 r, float32 t, float32 b) : left(l), right(r), top(t), bottom(b) {
				}
		};
		...
		enum class NkUIAlignment : uint8 { Start = 0, Center = 1, End = 2, SpaceBetween = 3, SpaceAround = 4 };
		...
		struct NkOnButtonClicked {
				NkEntityId button;
				uint32 buttonId;
				char tag[64];
		};

		struct NkOnSliderChanged {
				NkEntityId slider;
				float32 value;
				float32 normalized;
		};

		struct NkOnToggleChanged {
				NkEntityId toggle;
				bool isOn;
		};

		struct NkOnInputChanged {
				NkEntityId field;
				char text[512];
		};

		struct NkOnInputSubmitted {
				NkEntityId field;
				char text[512];
		};

		struct NkOnDropdownChanged {
				NkEntityId dropdown;
				int32 index;
				char option[128];
		};

		struct NkOnScrollChanged {
				NkEntityId scroll;
				NkVec2 position;
		};

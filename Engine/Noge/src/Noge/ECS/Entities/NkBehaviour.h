	...
	namespace ecs {
		...
		class NkBehaviour {
			public:
				...
				} // immédiatement après l'ajout du behaviour

				virtual void OnStart() {
				} // avant la première frame

				virtual void OnEnable() {
				} // quand le GameObject devient actif

				virtual void OnDisable() {
				} // quand le GameObject devient inactif
				...
				} // avant la destruction de l'entité

				// Accès au GameObject propriétaire (rempli automatiquement)
				NkGameObject *GetGameObject() const {
					return mGameObject;
				}
			...
			private:
				...
				void SetGameObject(NkGameObject *go) {
					mGameObject = go;
				}

	...
	namespace ecs {
		...
		class NkActor : public NkGameObject {
			public:
				 ...
				 */
				virtual void Tick(float32 dt) noexcept {
					(void)dt;
				}
				 ...
				 */
				virtual void LateTick(float32 dt) noexcept {
					(void)dt;
				}
				 ...
				 */
				virtual void FixedTick(float32 fixedDt) noexcept {
					(void)fixedDt;
				}
				...
				virtual void SpawnDefaultComponents() noexcept {
					if (!HasComponent<NkTransform>())
						AddComponent<NkTransform>();
					if (!HasComponent<NkTag>())
						AddComponent<NkTag>();
					if (!HasComponent<NkName>())
						AddComponent<NkName>("UnnamedActor");
				}
		};
		...
		class NkPawn : public NkActor {
			public:
				...
				[[nodiscard]] uint32 GetControllerId() const noexcept {

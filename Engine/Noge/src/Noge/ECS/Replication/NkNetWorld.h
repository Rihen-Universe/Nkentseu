...
#define NK_NET_COMPONENT(T) /* impl requis : NetSerialize, NetDeserialize, NetEquals */
		...
		class NkNetWorld {
			public:
				 ...
				 */
				void Init(NkWorld *world, NkConnectionManager *connMgr, bool isServer) noexcept;
				 ...
				 */
				NkEntityId SpawnNetEntity(const char *typeName, NkNetAuthority auth = NkNetAuthority::Server) noexcept;
				 ...
				 */
				void SubmitInput(NkEntityId playerEntity, const NkNetInput &input) noexcept;
				 ...
				 */
				void ApplySnapshot(const NkNetSnapshot &snap, NkEntityId localEntity) noexcept;
				...
				[[nodiscard]] uint32 GetTick() const noexcept {
					return mTick;
				}
			...
			private:
				...
				uint32 SerializeDelta(NkEntityId id, const NkNetSnapshot &prev, uint8 *out,
									  uint32 outSize) const noexcept;
		...
		class NkNetSystem final : public NkSystem {
			public:
				explicit NkNetSystem(NkNetWorld *netWorld) noexcept : mNetWorld(netWorld) {
				}
		...
		class NkNetInterpolator {
			public:
				explicit NkNetInterpolator(float32 delayS = 0.1f) noexcept : mDelay(delayS) {
				}
				...
				void SetDelay(float32 s) noexcept {
					mDelay = s;
				}

				[[nodiscard]] float32 GetDelay() const noexcept {
					return mDelay;
				}

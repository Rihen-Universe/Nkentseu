...
namespace nkentseu {
	namespace ecs {
		 ...
		 */
		bool NkSceneManager::LoadScene(const NkString &name, const NkSceneTransition &transition) noexcept {
			// ── Étape 0 : Validation de l'entrée ──
			NkSceneFactory *factory = FindFactory(name);
			if (!factory || !(*factory)) {
				logger.Errorf("[NkSceneManager] Scène inconnue ou factory nulle : {}\n", name.CStr());
				return false;
			}
			...
			if (transition.type != NkTransitionType::Instant && !transition.loadingScene.Empty()) {
			...
			if (!mCurrentScene) {
				logger.Errorf("[NkSceneManager] Factory a retourné nullptr pour la scène : {}\n", name.CStr());
		...
		NkSceneManager::NkSceneManager(NkWorld &world) noexcept
			: mWorld(world), mEntries(), mCurrentScene(nullptr), mCurrentName(), mLoadingScene(nullptr),
			  mLoadingSceneName() {
		}
		...
		// ── Enregistrement ─────────────────────────────────────────────────
		void NkSceneManager::Register(const NkString &name, NkSceneFactory factory) noexcept {

...
namespace nkentseu {
	namespace ecs {
		 ...
		 */
		NkSceneFixedTickSystem::NkSceneFixedTickSystem(NkSceneGraph *scene) noexcept : mScene(scene) {
		}
		...
		NkSystemDesc NkSceneFixedTickSystem::Describe() const {
			return NkSystemDesc{}.InGroup(NkSystemGroup::FixedUpdate).Named("NkSceneFixedTick").WithPriority(100);
		}
		 ...
		 * @param scene Pointeur vers la NkSceneGraph à mettre à jour.
		 */
		NkSceneTickSystem::NkSceneTickSystem(NkSceneGraph *scene) noexcept : mScene(scene) {
		}
		...
		NkSystemDesc NkSceneTickSystem::Describe() const {
			return NkSystemDesc{}.InGroup(NkSystemGroup::Update).Named("NkSceneTick").WithPriority(100);
		}
		 ...
		 * @param scene Pointeur vers la NkSceneGraph à mettre à jour.
		 */
		NkSceneLateTickSystem::NkSceneLateTickSystem(NkSceneGraph *scene) noexcept : mScene(scene) {
		}
		...
		NkSystemDesc NkSceneLateTickSystem::Describe() const {
			return NkSystemDesc{}.InGroup(NkSystemGroup::PostUpdate).Named("NkSceneLateTick").WithPriority(200);
		}
...
namespace nkentseu {
	namespace ecs {
		...
		bool SceneLifecycleManager::RegisterScene(NkScheduler &scheduler, NkSceneGraph *scene) noexcept {
		...
		void SceneLifecycleManager::DumpRegisteredScenes() const noexcept {
			...
			for (nk_usize i = 0; i < mRegisteredScenes.Size(); ++i) {
				...
				if (scene) {
					...
					printf("  [%u] %s (paused:%s, script:%s)\n", static_cast<unsigned>(i), name, paused ? "yes" : "no",
						   hasScript ? "yes" : "no");
				} else {

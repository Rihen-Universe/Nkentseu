...
namespace nkentseu {
	namespace ecs {
		 ...
		 */
		void NkSceneScript::SetContext(NkWorld *world, NkSceneGraph *scene) noexcept {
			mWorld = world;
			mScene = scene;
		}
		...
		NkVector<NkGameObject> NkSceneScript::FindGameObjectsWithTag(const char *tag) const {
			...
			const_cast<NkWorld *>(mWorld)->Query<NkName>().ForEach([&](NkEntityId id, NkName &n) {
		 ...
		 */
		NkGameObject NkSceneScript::SpawnPrefab(const NkPrefab &prefab, const math::NkVec3f &position,
												const char *name) {

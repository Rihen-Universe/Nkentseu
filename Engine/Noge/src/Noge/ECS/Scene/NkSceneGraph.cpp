...
namespace nkentseu {
	namespace ecs {
		 ...
		 */
		NkSceneGraph::NkSceneGraph(NkWorld &world, const NkString &name) noexcept
			: mWorld(world), mName(name), mScript(nullptr), mStarted(false), mPaused(false) {
		 ...
		 */
		void NkSceneGraph::SetParent(NkEntityId child, NkEntityId parent) noexcept {
		...
		NkEntityId NkSceneGraph::FindByName(const char *name) const noexcept {
			...
			const_cast<NkWorld &>(mWorld).Query<const NkSceneNode>().ForEach(
				[&](NkEntityId id, const NkSceneNode &node) {

...
namespace nkentseu {
	...
	ecs::NkGameObject NkGameObjectFactory::InstantiatePrefab(ecs::NkWorld &world, const char *prefabPath,
															 const char *instanceName) noexcept {
		if (!prefabPath)
			return ecs::NkGameObject{};
	...
	void NkGameObjectFactory::InstantiatePrefabBatch(ecs::NkWorld &world, const char *prefabPath, uint32 count,

	...
	namespace ecs {
		namespace blueprint {
			...
			struct NkTypeId {
					...
					bool operator==(const NkTypeId &o) const noexcept {
						return Hash == o.Hash && Name == o.Name;
					}
			};
			...
			struct NkPinType {
					...
					[[nodiscard]] bool IsExec() const noexcept {
						return Primitive == NkPinPrimitiveType::Exec;
					}
					...
					[[nodiscard]] bool IsCustom() const noexcept {
						return Primitive == NkPinPrimitiveType::None && Custom.Hash != 0;
					}
			};
			...
			struct NkValue {
					...
					static NkValue Exec() noexcept {
						return {};
					} // Exec n'a pas de data

					static NkValue Bool(bool v) noexcept {
						NkValue val;
						val.Type.Primitive = NkPinPrimitiveType::Bool;
						val.data.b = v;
						return val;
					}

					static NkValue Int(int32 v) noexcept {
						NkValue val;
						val.Type.Primitive = NkPinPrimitiveType::Int;
						val.data.i = v;
						return val;
					}

					static NkValue Float(float v) noexcept {
						NkValue val;
						val.Type.Primitive = NkPinPrimitiveType::Float;
						val.data.f = v;
						return val;
					}

					static NkValue Vec3(NkVec3 v) noexcept {
						NkValue val;
						val.Type.Primitive = NkPinPrimitiveType::Vec3;
						val.data.v3 = v;
						return val;
					}

					static NkValue Entity(NkEntityId id) noexcept {
						NkValue val;
						val.Type.Primitive = NkPinPrimitiveType::EntityId;
						val.data.entityId = id.Pack();
						return val;
					}

					static NkValue Ptr(void *p, const NkTypeId &type) noexcept {
						NkValue val;
					...
					static NkValue String(
						const char *s) noexcept { /* Simplifié: stocke pointeur const char* dans union ou utilise ptr */
						NkValue val;
						val.Type.Primitive = NkPinPrimitiveType::String;
						val.data.ptr = (void *)s;
						return val;
					}
					...
					[[nodiscard]] int32 AsInt() const noexcept {
						return data.i;
					}
					...
					[[nodiscard]] NkVec3 AsVec3() const noexcept {
						return data.v3;
					}

					[[nodiscard]] NkEntityId AsEntityId() const noexcept {
						return NkEntityId::Unpack(data.entityId);
					}

					template <typename T> [[nodiscard]] T *AsPtr() const noexcept {
				...
				public:
					...
					const NkValue &GetInput(uint32 index) const noexcept {
						return Inputs[index].IsConnected
								   ...
								   : Inputs[index].DefaultValue; // Simplifié: DefaultValue est écrasé par la connexion
					}

					// Helper pour définir une valeur de sortie
					void SetOutput(uint32 index, const NkValue &value) noexcept {
						if (index < Outputs.size())
							Outputs[index].DefaultValue = value;
					}

					[[nodiscard]] virtual const char *GetCategory() const noexcept {
						return "Base";
					}
			};
			...
			class NkNodeRegistry {
				public:
					...
					template <typename T> static void Register(const char *name) noexcept {
						auto &reg = Global();
						if (reg.mCount >= 512)
							return;
...
#define NK_REGISTER_BLUEPRINT_NODE(Type)                                                                               \
	namespace {                                                                                                        \
		struct _NkNodeReg_##Type {                                                                                     \
				_NkNodeReg_##Type() noexcept {                                                                         \
					NkNodeRegistry::Global().Register<Type>(#Type);                                                    \
				}                                                                                                      \
		} _sNkNodeReg_##Type;                                                                                          \
	}
			...
			class NkNodeEventBeginPlay : public NkBlueprintNode {
				public:
					...
					[[nodiscard]] const char *GetCategory() const noexcept override {
						return "Events";
					}
			};
			...
			class NkNodeSwitchInt : public NkBlueprintNode {
				public:
					...
					void Execute(NkWorld &, NkEntityId, float) noexcept override {
						...
						if (sel == 0)
							SetOutput(0, NkValue::Exec()); // Active la pin 0
						else if (sel == 1)
							SetOutput(1, NkValue::Exec());
						else
							SetOutput(2, NkValue::Exec()); // Default
					}
						...
						while (!execStack.empty()) {
							...
							for (uint32 i = 0; i < node->Outputs.size(); ++i) {
								if (node->Outputs[i].Type.IsExec()) {
									...
									// (Simplification: dans un vrai moteur, on vérifie si Output[i] contient un Exec
									// token)
					...
					void ResolveInputs(uint32 nodeIdx) noexcept {
						...
						for (uint32 i = 0; i < node->Inputs.size(); ++i) {
							...
							if (found) {
								...
								// Pour un système complet, il faut évaluer les dépendances de données avant
								// l'exécution.
							}
						}
					}
					...
					void Execute(NkWorld &world, NkEntityId, float) noexcept override {
						...
						if (actorId.IsValid() && world.IsAlive(actorId)) {
							...
							if (auto *tag = world.Get<NkTag>(actorId)) {
								...
								// if (tag->Has(NkTagBit::Character) && TargetClassType.Hash ==
								// NkTypeId("NkCharacter").Hash)

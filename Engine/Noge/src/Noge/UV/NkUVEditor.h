...
namespace nkentseu {
	...
	class NkUVEditor {
		public:
			...
			void Pan(NkVec2f delta) noexcept {
				viewOffset = viewOffset + delta;
			}

			void Zoom(float32 delta) noexcept {
				zoom = NkClamp(zoom + delta, 0.1f, 50.f);
			}
			...
			void Draw(renderer::NkRender2D &r2d, const NkRectF &viewport, nk_uint64 texHandle = 0) const noexcept;

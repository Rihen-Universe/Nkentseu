...
namespace nkentseu {
	...
	class NkHalfEdgeMesh {
		public:
			 ...
			 */
			void BevelEdge(uint32 halfEdgeId, float32 width, uint32 segments = 1) noexcept;
			 ...
			 */
			void VertexStar(uint32 vertexId, NkVector<uint32> &outHalfEdges) const noexcept;

			/**
			 * @brief Retourne les faces adjacentes à un vertex.
			 */
			void VertexFaces(uint32 vertexId, NkVector<uint32> &outFaces) const noexcept;

			/**
			 * @brief Retourne les vertices des faces adjacentes à un vertex.
			 */
			void VertexNeighbors(uint32 vertexId, NkVector<uint32> &outVerts) const noexcept;
			 ...
			 */
			void EdgeLoop(uint32 halfEdgeId, NkVector<uint32> &outLoop) const noexcept;

			/**
			 * @brief Retourne les half-edges formant un edge ring.
			 */
			void EdgeRing(uint32 halfEdgeId, NkVector<uint32> &outRing) const noexcept;
			...
			// ── Accès ─────────────────────────────────────────────────────────
			[[nodiscard]] NkVector<NkHEVertex> &Vertices() noexcept {
				return mVerts;
			}

			[[nodiscard]] NkVector<NkHEHalfEdge> &HalfEdges() noexcept {
				return mHEdges;
			}

			[[nodiscard]] NkVector<NkHEFace> &Faces() noexcept {
				return mFaces;
			}

			[[nodiscard]] const NkHEVertex &V(uint32 i) const noexcept {
				return mVerts[i];
			}

			[[nodiscard]] const NkHEHalfEdge &H(uint32 i) const noexcept {
				return mHEdges[i];
			}

			[[nodiscard]] const NkHEFace &F(uint32 i) const noexcept {
				return mFaces[i];
			}
			...
			[[nodiscard]] uint32 HalfEdgeCount() const noexcept {
				return static_cast<uint32>(mHEdges.Size());
			}

			[[nodiscard]] uint32 EdgeCount() const noexcept {
				return static_cast<uint32>(mHEdges.Size()) / 2;
			}
		...
		private:
			// ── Helpers ───────────────────────────────────────────────────────
			uint32 NewVertex() noexcept {
				mVerts.PushBack({});
				return VertexCount() - 1;
			}

			uint32 NewHalfEdge() noexcept {
				mHEdges.PushBack({});
				return HalfEdgeCount() - 1;
			}

			uint32 NewFace() noexcept {
				mFaces.PushBack({});
				return FaceCount() - 1;
			}

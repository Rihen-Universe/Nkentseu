...
namespace nkentseu {
	...
	class NkSculptSession {
		public:
			 ...
			 */
			bool Init(NkEditableMesh *mesh, NkUndoStack *undoStack = nullptr) noexcept;
			...
			void SetCamera(const NkViewportCamera *cam) noexcept {
				mCamera = cam;
			}

			void SetDynamic(bool v) noexcept {
				mDynamic = v;
			} ///< Remesh dynamique
			 ...
			 */
			void PointerDown(NkVec2f screenPos, float32 vpW, float32 vpH, float32 pressure = 1.f) noexcept;

			void PointerMove(NkVec2f screenPos, float32 vpW, float32 vpH, float32 pressure = 1.f) noexcept;
		...
		private:
			...
			bool Raycast(NkVec2f screenPos, float32 vpW, float32 vpH, NkVec3f &outHitPos,
						 NkVec3f &outHitNormal) const noexcept;

			void ApplyBrush(NkVec3f hitPos, NkVec3f hitNormal, float32 pressure) noexcept;
			...
			void CollectVerticesInRadius(NkVec3f center, float32 radius, NkVector<uint32> &out) const noexcept;

			// ── BVH simple pour accélérer le raycast ─────────────────────────
			void RebuildBVH() noexcept;
			bool RaycastBVH(const NkRay &ray, NkVec3f &hitPos, NkVec3f &hitNormal) const noexcept;

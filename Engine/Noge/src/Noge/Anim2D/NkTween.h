...
namespace nkentseu {
	...
	class NkTween {
		public:
			...
			NkTween &SetEase(NkEase e) noexcept {
				mEase = e;
				return *this;
			}

			NkTween &SetDelay(float32 d) noexcept {
				mDelay = d;
				return *this;
			}

			NkTween &SetLoops(int32 n) noexcept {
				mLoops = n;
				return *this;
			} ///< -1 = infini

			NkTween &SetPingPong(bool v) noexcept {
				mPingPong = v;
				return *this;
			}

			NkTween &SetSpeed(float32 s) noexcept {
				mSpeed = s;
				return *this;
			}

			NkTween &SetRelative(bool v) noexcept {
				mRelative = v;
				return *this;
			}

			NkTween &OnStart(Callback cb) noexcept {
				mOnStart = static_cast<Callback &&>(cb);
				return *this;
			}

			NkTween &OnComplete(Callback cb) noexcept {
				mOnComplete = static_cast<Callback &&>(cb);
				return *this;
			}

			NkTween &OnUpdate(UpdateCallback cb) noexcept {
				mOnUpdate = static_cast<UpdateCallback &&>(cb);
				return *this;
			}
	...
	class NkTweenFloat : public NkTween {
		public:
			NkTweenFloat(float32 *target, float32 from, float32 to, float32 duration) noexcept
				: mTarget(target), mFrom(from), mTo(to) {
				mDuration = duration;
			}
	...
	class NkTweenColor : public NkTween {
		...
		protected:
			void ApplyValue(float32 t) noexcept override {
				if (mTarget)
					*mTarget = {mFrom.x + (mTo.x - mFrom.x) * t, mFrom.y + (mTo.y - mFrom.y) * t,
								mFrom.z + (mTo.z - mFrom.z) * t, mFrom.w + (mTo.w - mFrom.w) * t};
			}

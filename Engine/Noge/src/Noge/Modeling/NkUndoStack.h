...
namespace nkentseu {
	...
	class NkCommandGroup : public NkEditCommand {
		public:
			...
			void Add(NkEditCommand *cmd) noexcept {
				mCmds.PushBack(cmd);
			}
			...
			void Undo() noexcept override {
				...
				for (nk_usize i = mCmds.Size(); i > 0; --i)
					mCmds[i - 1]->Undo();
			}
	...
	class NkUndoStack {
		public:
			explicit NkUndoStack(uint32 maxSteps = 100, nk_usize memBudget = 256 * 1024 * 1024) noexcept
				: mMaxSteps(maxSteps), mMemBudget(memBudget) {
			}

			~NkUndoStack() noexcept {
				Clear();
			}
			...
			[[nodiscard]] int32 CurrentIndex() const noexcept {
				return mCurrent;
			}
			 ...
			 */
			void SetCleanState() noexcept {
				mCleanIndex = mCurrent;
			}
	...
	class NkLambdaCommand : public NkEditCommand {
		public:
			...
			NkLambdaCommand(const char *n, Fn execFn, Fn undoFn) noexcept
				: mExec(static_cast<Fn &&>(execFn)), mUndo(static_cast<Fn &&>(undoFn)) {

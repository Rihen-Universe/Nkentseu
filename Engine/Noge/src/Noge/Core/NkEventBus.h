...
#define NK_EVENTBUS_MAX_EVENT_TYPES 64
		...
		struct NkBusRegistry {
				...
				NkEventTable *FindOrCreate(nk_uint32 typeId) {
					for (nk_uint32 i = 0; i < tableCount; ++i) {
						if (tables[i].typeId == typeId)
							return &tables[i];
					}
					if (tableCount >= NK_EVENTBUS_MAX_EVENT_TYPES)
						return nullptr;
		...
		template <typename TClass, typename TEvent> bool NkDispatchMember(void *ctx, void *lam, NkEvent *event) {
			using MemberFn = bool (TClass::*)(TEvent *);
			MemberFn fn;
			static_assert(sizeof(fn) <= kLambdaStorage, "Pointeur de méthode trop grand pour le stockage inline");
	...
	class NkEventBus {
		public:
			...
			template <typename TEvent, typename TFunctor> static NkEventHandlerId Subscribe(TFunctor &&functor) {
			...
			static NkEventHandlerId Subscribe(TClass *obj, bool (TClass::*fn)(TEvent *)) {
				using MemberFn = bool (TClass::*)(TEvent *);
			...
			// ── Unsubscribe par ID ────────────────────────────────────────────────────
			template <typename TEvent> static void Unsubscribe(NkEventHandlerId id) {
				if (id == NK_INVALID_HANDLER_ID)
					return;
				auto &reg = internal::NkGetRegistry();
			...
			template <typename TEvent> static bool Dispatch(TEvent *event) {
			...
			static bool DispatchRaw(nk_uint32 typeId, NkEvent *event) {
				...
				for (nk_uint32 i = 0; i < NK_EVENTBUS_MAX_HANDLERS; ++i) {
					auto &slot = table->slots[i];
					if (!slot.active || !slot.dispatch)
						continue;
					if (slot.dispatch(slot.ctx, reinterpret_cast<void *>(slot.storage), event)) {

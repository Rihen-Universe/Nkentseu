...
namespace nkentseu {
	...
	class NkSelectionMask {
		public:
			 ...
			 */
			void SelectByColor(const NkRasterCanvas &canvas, uint32 x, uint32 y, uint32 tolerance = 32,
			 ...
			 */
			void DrawOverlay(renderer::NkRender2D &r2d, float32 animTime, const NkRectF &dst) const noexcept;
		...
		private:
			...
			void FloodFillRecursive(uint32 x, uint32 y, const NkRasterCanvas &canvas, uint8 refR, uint8 refG,
									uint8 refB, uint32 tol, NkVector<bool> &visited) noexcept;
	};
	...
	struct NkSelectionTool {
			...
			void PointerDown(NkVec2f pos, NkSelectionMask &mask, const NkRasterCanvas &canvas) noexcept;
			void PointerMove(NkVec2f pos, NkSelectionMask &mask) noexcept;
			void PointerUp(NkVec2f pos, NkSelectionMask &mask, const NkRasterCanvas &canvas) noexcept;
	};
	...
	struct NkRasterFilter {
			 ...
			 */
			void Apply(NkRasterCanvas &canvas, const NkSelectionMask *mask = nullptr,
					   const NkIRect &region = {}) const noexcept;

			/**
			 * @brief Prévisualisation — applique sur une copie sans modifier l'original.
			 */
			void Preview(const NkRasterCanvas &src, NkRasterCanvas &dst, const NkIRect &region = {}) const noexcept;
	};
	...
	class NkRasterIO {
		public:
			 ...
			 */
			[[nodiscard]] static bool Import(const char *path, NkRasterCanvas &out,
											 const NkRasterImportOptions &opts = {}) noexcept;

			/**
			 * @brief Exporte un NkRasterCanvas vers un fichier.
			 */
			[[nodiscard]] static bool Export(const NkRasterCanvas &canvas, const char *path,
											 const NkRasterExportOptions &opts = {}) noexcept;

			/**
			 * @brief Charge depuis un buffer mémoire (sans passer par un fichier).
			 */
			[[nodiscard]] static bool ImportFromMemory(const uint8 *data, nk_usize size, NkRasterCanvas &out,
													   const NkRasterImportOptions &opts = {}) noexcept;

...
namespace nkentseu {
	...
	struct NkSkinRegion {
			...
			bool AddRegion(const char *name, float32 sssR = 1.f, float32 sssStr = 0.5f) noexcept {
				if (regionCount >= kMaxRegions)
					return false;
	...
	class NkEyeSystem final : public NkSystem {
		...
		private:
			...
			void UpdatePupil(NkEyeRig &eye, float32 dt, float32 ambientLuminance) noexcept;
			void UpdateLook(NkEyeRig &eye, NkWorld &world, const NkTransform &tf, float32 dt) noexcept;

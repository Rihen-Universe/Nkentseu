...
namespace nkentseu {
	...
	struct NkFacialRig {
			...
			void SetAU(NkFACS au, float32 weight) noexcept {
				...
				if (idx < kFACSCount)
					auWeights[idx] = NkClamp(weight, 0.f, 1.f);
			}
			 ...
			 */
			void BlendExpressions(const char *fromExpr, const char *toExpr, float32 t) noexcept;
	...
	struct NkWrinkleMap {
			 ...
			 */
			void Update(NkICommandBuffer *cmd, nk_uint64 restPosBuffer, nk_uint64 curPosBuffer, nk_uint64 uvBuffer,
	...
	class NkFacialSystem final : public NkSystem {
		...
		private:
			void ProcessFacialRig(NkWorld &world, NkEntityId id, NkFacialRig &rig, float32 dt) noexcept;
	};

...
namespace nkentseu {
	...
	class NkViewportCamera {
		public:
			 ...
			 * @param dt       Delta-time.
			 */
			void Fly(float32 forward, float32 right, float32 up, float32 dt) noexcept;
			...
			[[nodiscard]] bool IsOrtho() const noexcept {
				return mOrthoView != OrthoView::None;
			}
			...
			[[nodiscard]] OrthoView GetOrthoView() const noexcept {
				return mOrthoView;
			}
			 ...
			 */
			[[nodiscard]] NkRay ScreenToRay(float32 pixelX, float32 pixelY, float32 vpWidth,
											float32 vpHeight) const noexcept;

			/**
			 * @brief Projette un point 3D monde vers coordonnées écran [0..1].
			 */
			[[nodiscard]] NkVec3f WorldToScreen(const NkVec3f &worldPos, float32 vpWidth, float32 vpHeight,
												float32 aspect) const noexcept;

			// ── Accès à l'état ────────────────────────────────────────────
			[[nodiscard]] const NkVec3f &GetPosition() const noexcept {
				return mPosition;
			}

			[[nodiscard]] const NkVec3f &GetTarget() const noexcept {
				return mTarget;
			}

			[[nodiscard]] const NkVec3f &GetUp() const noexcept {
				return mUp;
			}

			[[nodiscard]] float32 GetDistance() const noexcept {
				return mDistance;
			}

			[[nodiscard]] float32 GetAzimuth() const noexcept {
				return mAzimuth;
			}

			[[nodiscard]] float32 GetElevation() const noexcept {
				return mElevation;
			}

			void SetTarget(const NkVec3f &t) noexcept {
				mTarget = t;
				UpdatePosition();
			}

			void SetDistance(float32 d) noexcept {
				mDistance = NkMax(0.01f, d);
				UpdatePosition();
			}
	...
	class NkMultiViewport {
		public:
			...
			[[nodiscard]] bool IsMaximized() const noexcept {
				return mMaximizedVp >= 0;
			}

			[[nodiscard]] uint32 ActiveViewport() const noexcept {
				return mActiveVp;
			}

			void SetActive(uint32 idx) noexcept {
				mActiveVp = idx;
			}

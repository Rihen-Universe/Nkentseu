...
namespace nkentseu {
	...
	class NkGizmo {
		public:
			...
			bool Update(const NkViewportCamera &cam, const NkMat4f &objectTf, NkVec2f mousePos, NkVec2f vpSize,
			...
			[[nodiscard]] Axis GetActiveAxis() const noexcept {
				return mActiveAxis;
			}

			// ── Rendu ─────────────────────────────────────────────────────────
			void Draw(renderer::NkRender3D &r3d, const NkMat4f &objectTf, const NkViewportCamera &cam) const noexcept;

...
namespace nkentseu {
	...
	class NkIKSolver {
		...
		private:
			// ── FABRIK internal ───────────────────────────────────────────
			void FABRIKForward(NkVector<NkVec3f> &positions, const NkVec3f &target) const noexcept;
			void FABRIKBackward(NkVector<NkVec3f> &positions, const NkVec3f &root) const noexcept;
			void ApplyPositionsToSkeleton(ecs::NkSkeleton &sk, const NkVector<uint32> &boneIds,
										  const NkVector<NkVec3f> &worldPos) const noexcept;

			// ── TwoBone internal ──────────────────────────────────────────
			[[nodiscard]] float32 ComputeBendAngle(float32 a, float32 b, float32 c) const noexcept;

			// ── Spline internal ───────────────────────────────────────────
			[[nodiscard]] NkVec3f EvalCubicBezier(const NkVector<NkVec3f> &pts, float32 t) const noexcept;
			[[nodiscard]] NkVec3f EvalCubicBezierTangent(const NkVector<NkVec3f> &pts, float32 t) const noexcept;

			// ── Constraints ───────────────────────────────────────────────
			[[nodiscard]] NkQuatf ApplyAngleConstraint(const NkQuatf &rotation,

...
namespace nkentseu {
	...
	class NkRasterCanvas {
		public:
			 ...
			 */
			bool Create(uint32 width, uint32 height, NkPixelDepth depth = NkPixelDepth::Depth8) noexcept;
			 ...
			 */
			void Blit(const NkRasterCanvas &src, NkIRect srcRect, NkIVec2 dstPos, float32 opacity = 1.f,
			 ...
			 */
			[[nodiscard]] NkSpan<uint8> GetTilePixels(uint32 tileX, uint32 tileY) noexcept;
			 ...
			 */
			void DrawToRender2D(renderer::NkRender2D &r2d, const NkRectF &dst, float32 opacity = 1.f) const noexcept;
			...
			[[nodiscard]] NkPixelDepth GetDepth() const noexcept {
				return mDepth;
			}
			...
			[[nodiscard]] uint32 TileCountX() const noexcept {
				return mTilesX;
			}

			[[nodiscard]] uint32 TileCountY() const noexcept {
				return mTilesY;
			}

			[[nodiscard]] uint32 DirtyTileCount() const noexcept {
				return mDirtyCount;
			}
		...
		private:
			...
			void BlendPixel(uint8 *dst, const uint8 *src, float32 alpha, NkBrushDab::BlendMode mode) const noexcept;

			// ── Dab painting par tile ─────────────────────────────────────
			void PaintDabOnTile(NkCanvasTile &tile, uint32 tileX, uint32 tileY, const NkBrushDab &dab,
	...
	class NkBrushEngine {
		public:
			...
			// ── Configuration ─────────────────────────────────────────────
			void SetPreset(const NkBrushPreset &p) noexcept {
				mPreset = p;
			}
			...
			void SetCanvas(NkRasterCanvas *canvas) noexcept {
				mCanvas = canvas;
			}
			 ...
			 */
			void PointerDown(NkVec2f pos, float32 pressure = 1.f, float32 tiltDeg = 0.f,
							 float32 rotation = 0.f) noexcept;

			void PointerMove(NkVec2f pos, float32 pressure = 1.f, float32 tiltDeg = 0.f,
							 float32 rotation = 0.f) noexcept;

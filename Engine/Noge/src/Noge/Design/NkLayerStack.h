...
namespace nkentseu {
	...
	struct NkLayer {
			...
			// ── Type de couche ────────────────────────────────────────────────
			enum class Type : uint8 { Raster, Vector, Text, Adjustment, Group, Reference };
	...
	class NkLayerStack {
		public:
			 ...
			 */
			NkLayer *AddAdjustmentLayer(NkAdjustmentType type, const char *name = nullptr) noexcept;
		...
		private:
			void CompositeGroup(NkLayer *group, renderer::NkRender2D &r2d, NkICommandBuffer *cmd) noexcept;

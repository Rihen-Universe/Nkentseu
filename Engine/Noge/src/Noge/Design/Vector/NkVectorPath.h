...
namespace nkentseu {
	...
	struct NkPaint {
			...
			static NkPaint Black() noexcept {
				return Solid({0, 0, 0, 1});
			}

			static NkPaint White() noexcept {
				return Solid({1, 1, 1, 1});
			}

			static NkPaint Transparent() noexcept {
				return Solid({0, 0, 0, 0});
			}

			static NkPaint LinearGrad(NkVec2f from, NkVec2f to, NkVec4f c0, NkVec4f c1) noexcept {
				NkPaint p;
				p.type = Type::LinearGradient;
				p.gradientStart = from;
				p.gradientEnd = to;
				p.stops[0] = {0.f, c0};
				p.stops[1] = {1.f, c1};
				p.stopCount = 2;
	...
	struct NkPathCmd {
			...
			static NkPathCmd Line(float32 x, float32 y) noexcept {
				NkPathCmd c;
				c.type = NkPathCmdType::LineTo;
				c.pts[0] = x;
				c.pts[1] = y;
				return c;
			}

			static NkPathCmd Cubic(float32 cx1, float32 cy1, float32 cx2, float32 cy2, float32 x, float32 y) noexcept {
				NkPathCmd c;
				c.type = NkPathCmdType::CubicTo;
				c.pts[0] = cx1;
				c.pts[1] = cy1;
				c.pts[2] = cx2;
				c.pts[3] = cy2;
	...
	class NkVectorPath {
		public:
			...
			NkVectorPath &CubicTo(float32 cx1, float32 cy1, float32 cx2, float32 cy2, float32 x, float32 y) noexcept;
			NkVectorPath &QuadTo(float32 cx, float32 cy, float32 x, float32 y) noexcept;
			NkVectorPath &ArcTo(float32 rx, float32 ry, float32 xRotDeg, bool largeArc, bool sweep, float32 x,
								float32 y) noexcept;
			NkVectorPath &Close() noexcept;

			// ── Formes de haut niveau ─────────────────────────────────────
			NkVectorPath &AddRect(float32 x, float32 y, float32 w, float32 h, float32 radius = 0.f) noexcept;
			NkVectorPath &AddCircle(float32 cx, float32 cy, float32 r) noexcept;
			NkVectorPath &AddEllipse(float32 cx, float32 cy, float32 rx, float32 ry) noexcept;
			NkVectorPath &AddStar(float32 cx, float32 cy, float32 outerR, float32 innerR, uint32 points) noexcept;
			NkVectorPath &AddRoundedRect(float32 x, float32 y, float32 w, float32 h, float32 tl, float32 tr, float32 br,
										 float32 bl) noexcept;
			 ...
			 */
			void DrawFill(renderer::NkRender2D &r, const NkPaint &fill, NkFillRule rule = NkFillRule::NonZero,
						  float32 tolerance = 0.5f) const noexcept;

			/**
			 * @brief Tesselle le stroke et le dessine via NkRender2D.
			 */
			void DrawStroke(renderer::NkRender2D &r, const NkPaint &paint, const NkStrokeStyle &stroke) const noexcept;

			/**
			 * @brief Dessine fill + stroke en un appel.
			 */
			void Draw(renderer::NkRender2D &r, const NkPaint &fill, const NkPaint &strokePaint,
					  const NkStrokeStyle &stroke, NkFillRule rule = NkFillRule::NonZero) const noexcept;
			...
			[[nodiscard]] bool IsEmpty() const noexcept {
				return mCmds.IsEmpty();
			}

			[[nodiscard]] uint32 CmdCount() const noexcept {
		...
		private:
			...
			void TesselateFill(NkVector<NkVec2f> &verts, NkVector<uint32> &idx, NkFillRule rule,
							   float32 tol) const noexcept;
			void TessellateStroke(NkVector<NkVec2f> &verts, NkVector<uint32> &idx,
								  const NkStrokeStyle &s) const noexcept;
			void SubdivideBezier(float32 x0, float32 y0, float32 cx1, float32 cy1, float32 cx2, float32 cy2, float32 x1,
								 float32 y1, NkVector<NkVec2f> &out, float32 tol) const noexcept;
	};

...
namespace nkentseu {
	...
	class NkColor {
		public:
			...
			NkColor(float32 r, float32 g, float32 b, float32 a = 1.f) noexcept : mR(r), mG(g), mB(b), mA(a) {
			}
			...
			[[nodiscard]] NkVec4f ToLinearRGB() const noexcept {
				return {mR, mG, mB, mA};
			}
			...
			// ── Accès direct ─────────────────────────────────────────────────
			[[nodiscard]] float32 R() const noexcept {
				return mR;
			}

			[[nodiscard]] float32 G() const noexcept {
				return mG;
			}

			[[nodiscard]] float32 B() const noexcept {
				return mB;
			}

			[[nodiscard]] float32 A() const noexcept {
				return mA;
			}

			void SetAlpha(float32 a) noexcept {
				mA = NkClamp(a, 0.f, 1.f);
			}
			...
			// ── Couleurs nommées ──────────────────────────────────────────────
			[[nodiscard]] static NkColor Black() noexcept {
				return {0, 0, 0, 1};
			}

			[[nodiscard]] static NkColor White() noexcept {
				return {1, 1, 1, 1};
			}

			[[nodiscard]] static NkColor Transparent() noexcept {
				return {0, 0, 0, 0};
			}
	...
	class NkHarmony {
		public:
			 ...
			 */
			[[nodiscard]] static NkVector<NkColor> Complementary(const NkColor &base) noexcept;

			/**
			 * @brief Triade (3 couleurs espacées de 120°).
			 */
			[[nodiscard]] static NkVector<NkColor> Triadic(const NkColor &base) noexcept;

			/**
			 * @brief Tétrade (4 couleurs espacées de 90°).
			 */
			[[nodiscard]] static NkVector<NkColor> Tetradic(const NkColor &base) noexcept;

			/**
			 * @brief Analogues (3 couleurs adjacentes ±30°).
			 */
			[[nodiscard]] static NkVector<NkColor> Analogous(const NkColor &base, float32 angle = 30.f) noexcept;

			/**
			 * @brief Split-complémentaires.
			 */
			[[nodiscard]] static NkVector<NkColor> SplitComplementary(const NkColor &base,
																	  float32 split = 30.f) noexcept;

			/**
			 * @brief Génère N nuances monochromatiques (same hue, varying L).
			 */
			[[nodiscard]] static NkVector<NkColor> Monochromatic(const NkColor &base, uint32 count = 5) noexcept;

			/**
			 * @brief Gradient perceptuellement uniforme entre deux couleurs (via OKLab).
			 */
			[[nodiscard]] static NkVector<NkColor> GradientOKLab(const NkColor &from, const NkColor &to,
	...
	class NkColorManager {
		public:
			...
			void ResetDefault() noexcept {
				foreground = NkColor::Black();
				background = NkColor::White();
			}

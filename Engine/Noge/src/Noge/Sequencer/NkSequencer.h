...
namespace nkentseu {
	...
	struct NkAnimChannel {
			...
			void AddKey(float32 time, float32 value, NkInterpolation interp = NkInterpolation::Bezier) noexcept;
	...
	struct NkClipOnTrack {
			...
			[[nodiscard]] float32 EndTime() const noexcept {
	...
	struct NkTrack {
			...
			// ── API ───────────────────────────────────────────────────────────
			NkClipOnTrack &AddClip(nk_uint64 clipHandle, float32 startTime, float32 duration) noexcept {
				NkClipOnTrack clip;
	...
	struct NkCameraTrack {
			...
			NkCameraShot &AddShot(NkEntityId cam, float32 start, float32 dur, NkCutType cut = NkCutType::Cut) noexcept {
				NkCameraShot s;
				s.cameraEntity = cam;
				s.startTime = start;
				s.duration = dur;
	...
	struct NkPlaybackCtrl {
			...
			void JumpTo(float32 t) noexcept {
				time = t;
			}
	...
	class NkSequence {
		public:
			...
			// ── API pistes ────────────────────────────────────────────────────
			NkTrack &AddTrack(NkEntityId entity, NkTrackType type, const char *name = nullptr) noexcept {
				NkTrack t;
				t.entity = entity;
				t.type = type;
			...
			NkNLATrack &AddNLATrack(NkEntityId entity, const char *name = nullptr) noexcept {
				NkNLATrack t;
				t.entity = entity;
			...
			NkMarker &AddMarker(float32 time, const char *label, NkMarker::Type type = NkMarker::Type::Note) noexcept {
				NkMarker m;
				m.time = time;
				m.type = type;

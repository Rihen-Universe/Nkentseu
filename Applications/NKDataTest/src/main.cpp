...
int main() {
    ...
    int expected[16] = { 0 };
    for (uint32 i = 0; i < ds.Size(); ++i) expected[ds.Labels()[i]]++;
    ...
    for (uint32 b = 0; b < loader.NumBatches(); ++b) {
        ...
        seen += batch.size;
        if ((uint32)batch.inputs.Shape()[0] != batch.size ||
            (uint32)batch.inputs.Shape()[1] != ds.FeatureDim() ||
        ...
        for (uint32 k = 0; k < batch.size; ++k) {
            ...
            double sum = 0.0; int hot = -1;
            for (uint32 c = 0; c < ds.NumClasses(); ++c) {
                float v = tgt[k * ds.NumClasses() + c];
                sum += v; if (v > 0.5f) hot = (int)c;
            }
            if (hot != batch.labels[k] || sum < 0.999 || sum > 1.001) oneHotOk = false;
        }
    }
    Check(seen == N, "Couverture : tous les exemples exactement une fois (60)");
    bool histOk = true;
    for (uint32 c = 0; c < NC; ++c) if (actual[c] != expected[c]) histOk = false;
    ...
    for (uint32 k = 0; k < m; ++k) if (b0.labels[k] != b0b.labels[k]) { changed = true; break; }
    ...
    if (!dir || !*dir) {
        ...
        if (!mnist.IsValid()) {
            ...
            printf("  MNIST chargé : %u exemples, %u features (28x28), %u classes\n",
                   mnist.Size(), mnist.FeatureDim(), mnist.NumClasses());
            data::NkDataLoader ml(mnist, 64, true, 1u);
            printf("  DataLoader MNIST : %u lots de 64\n", ml.NumBatches());
            Check(mnist.FeatureDim() == 784 && mnist.NumClasses() == 10,

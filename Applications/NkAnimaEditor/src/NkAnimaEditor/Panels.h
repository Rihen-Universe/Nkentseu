	...
	class PreviewPanel : public NkEditorPanel {
		public:
			...
			void OnUI(NkEditorFrameContext &ec) override {
				...
				for (uint32 j = 0; j < jc; ++j) {
					NkVec2 sp = proj(pos[j]);
					int32 p = par[j];
					if (p >= 0 && p < (int32)jc)
						dl.AddLine(proj(pos[(uint32)p]), sp, NkColor{105, 135, 160, 255}, 2.f);
					bool sel = ((int32)j == mSel);
					dl.AddCircleFilled(sp, sel ? 6.f : 3.f,
									   sel ? NkColor{0, 212, 255, 255} : NkColor{0, 175, 215, 255});
				}
				...
				if (gizmoOn && mTool == 1) { // Rotation FK : anneau cyan
					...
					for (int32 s = 0; s <= SEG; ++s) {
						float32 a = (float32)s / (float32)SEG * 6.2831853f;
						NkVec2 q{gc.x + cosf(a) * gizR, gc.y + sinf(a) * gizR};
						if (s > 0)
							dl.AddLine(prev, q, NkColor{0, 212, 255, 200}, 1.5f);
						prev = q;
					}
				...
				auto nearSeg = [&](const NkVec2 &a, const NkVec2 &b, float32 tol) {
					...
					float32 L = vx * vx + vy * vy, t = (L > 1e-6f) ? ((wx * vx + wy * vy) / L) : 0.f;
					if (t < 0)
						t = 0;
					if (t > 1)
						t = 1;
					float32 dx = m.x - (a.x + vx * t), dy = m.y - (a.y + vy * t);
				...
				if (ctx.IsHovered(area) && ctx.input.mouseClicked[0]) {
					...
					if (mSel >= 0 && mSel < (int32)jc) {
						...
						if (mTool == 1) {
							float32 dx = c.x - m.x, dy = c.y - m.y, d = sqrtf(dx * dx + dy * dy);
							if (d <= gizR + 8.f && d >= gizR - 14.f) {
								mRotating = true;
								mPrevAng = angAt(c);
								grabbed = true;
							}
							...
							if (nearSeg(c, xe, axHit)) {
								mTranslating = true;
								mTransAxis = 1;
								grabbed = true;
							} else if (nearSeg(c, ye, axHit)) {
								mTranslating = true;
								mTransAxis = 2;
								grabbed = true;
							} else {
								float32 dx = c.x - m.x, dy = c.y - m.y;
								if (sqrtf(dx * dx + dy * dy) <= 8.f) {
									mTranslating = true;
									mTransAxis = 0;
									grabbed = true;
								}
					...
					if (!grabbed) {
						...
						for (uint32 j = 0; j < jc; ++j) {
							NkVec2 sp = proj(pos[j]);
							float32 dx = sp.x - m.x, dy = sp.y - m.y;
							float32 d = sqrtf(dx * dx + dy * dy);
							if (d < best) {
								best = d;
								hit = (int32)j;
							}
						...
						if (hit >= 0) {
							mSel = hit;
							mDragZ = pos[(uint32)hit].z;
							if (mTool == 0)
								mDragging = true;
							else if (mTool == 1) {
								mRotating = true;
								mPrevAng = angAt(proj(pos[(uint32)hit]));
							} else if (mTool == 2) {
								mTranslating = true;
								mTransAxis = 0;
							}

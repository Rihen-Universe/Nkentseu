...
namespace nkentseu {
	namespace cameradem {
		...
		static NdcRect AspectFitInQuadrant(uint32 srcW, uint32 srcH, const NdcRect &q, float padding) {
			...
			if (srcAR > dstAR)
				...
				outW = availH * srcAR;
			return {q.x + padding + (availW - outW) * 0.5f, q.y + padding + (availH - outH) * 0.5f, outW, outH};
		}
		...
		int RunCameraMultiDemo(const NkEntryState & /*state*/) {
			...
			auto &cam = NkCamera();
			if (!cam.Init()) {
				logger.Warnf("[Multi] NkCamera::Init() echoue : %s", cam.GetLastError().CStr());
			}

			NkVector<NkCameraDevice> devices = cam.EnumerateDevices();
			logger.Infof("[Multi] %u camera(s) detectee(s)", (uint32)devices.Size());

			NkMultiCamera multi;
			CameraSlot slots[NKENTSEU_CAM_MULTI_MAX];

			const uint32 toOpen =
				(uint32)devices.Size() < NKENTSEU_CAM_MULTI_MAX ? (uint32)devices.Size() : NKENTSEU_CAM_MULTI_MAX;
			for (uint32 i = 0; i < toOpen; ++i) {
				...
				logger.Infof("[Multi] Ouverture device[%u] : %s", devices[i].index, devices[i].name.CStr());
			}
			...
			while (running && window.IsOpen()) {
				...
				for (uint32 i = 0; i < NKENTSEU_CAM_MULTI_MAX; ++i) {
					if (!slots[i].stream)
						continue;
					NkCameraFrame frame;
					if (!slots[i].stream->GetLastFrame(frame))
						continue;
					...
					if (!slots[i].texture.IsValid() || slots[i].texture.Width() != frame.width ||
				...
				for (uint32 i = 0; i < NKENTSEU_CAM_MULTI_MAX; ++i) {
					if (!slots[i].stream || !slots[i].everValid)
						continue;
					...
					const NdcRect fit =
						AspectFitInQuadrant(slots[i].texture.Width(), slots[i].texture.Height(), q, 0.02f);
					blit.Draw(slots[i].texture.Id(), fit.x, fit.y, fit.w, fit.h);
				}

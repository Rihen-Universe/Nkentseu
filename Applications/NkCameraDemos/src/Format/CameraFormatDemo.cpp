	...
	namespace cameradem {
		...
		static void RunSlotConversion(FormatSlot &slot, const char *baseName) {
			...
			if (NkCameraSystem::SaveFrameToFile(slot.converted, path, 90)) {
				logger.Infof("[Format] %s : converti et sauve dans %s", baseName, path.CStr());
			} else {
				logger.Warnf("[Format] %s : conversion OK mais Save a echoue", baseName);
			}
		}
		...
		int RunCameraFormatDemo(const NkEntryState & /*state*/) {
			...
			if (slots[3].frame.data.Size() > 0) {
				...
				if (fjpg) {
					std::fwrite(slots[3].frame.data.Data(), 1, slots[3].frame.data.Size(), fjpg);
					std::fclose(fjpg);
			...
			for (uint32 i = 0; i < NK_FORMAT_TEST_COUNT; ++i) {
				...
				if (slots[i].convertOk && slots[i].converted.IsValid()) {
					if (slots[i].texture.Create(slots[i].converted.width, slots[i].converted.height)) {
						slots[i].texture.Update(slots[i].converted.data.Data());
					}
				}
				logger.Infof("[Format] slot[%u] %s : %s", i, slots[i].label.CStr(),
			...
			(void)winW;
			(void)winH; // pas utilises (grille fixe en NDC)

...
int main() {
	...
	printf("x=[%lld,%lld,%lld,%lld]  w=[%lld,%lld,%lld,%lld]  stride=%d pad=%d  -> out=[%lld,%lld,%lld,%lld]\n",
		   (long long)B, (long long)Cin, (long long)H, (long long)W, (long long)Cout, (long long)Cin, (long long)k,
		   (long long)k, stride, pad, (long long)B, (long long)Cout, (long long)outH, (long long)outW);

	// ---- Données déterministes -----------------------------------------------
	const int64 nX = B * Cin * H * W, nW = Cout * Cin * k * k;
	float *xd = new float[nX];
	for (int64 i = 0; i < nX; i++)
		xd[i] = std::sin(0.01 * (double)i);
	float *wd = new float[nW];
	for (int64 i = 0; i < nW; i++)
		wd[i] = 0.1 * std::cos(0.02 * (double)i);
	NkShape xs;
	xs.PushBack(B);
	xs.PushBack(Cin);
	xs.PushBack(H);

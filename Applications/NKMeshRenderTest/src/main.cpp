...
int main() {
	...
	rp.Resize(vc * 3);
	for (uint32 i = 0; i < vc; ++i)
		xform(&mesh.positions[i * 3], &rp[i * 3]);
	...
	for (uint32 t = 0; t < tc; ++t) {
		...
		float ux = b[0] - a[0], uy = b[1] - a[1], uz = b[2] - a[2], wx = c[0] - a[0], wy = c[1] - a[1],
			  wz = c[2] - a[2];
		...
		project(a, ax_, ay_, ad);
		project(b, bx_, by_, bd);
		project(c, cx_, cy_, cd);
		...
		if (maxy >= H)
			maxy = H - 1;
		float area = (bx_ - ax_) * (cy_ - ay_) - (by_ - ay_) * (cx_ - ax_);
		if (fabsf(area) < 1e-6f)
			continue;
	...
	if (f) {
		...
		for (int i = 0; i < W * H; ++i) {
			...
			for (int k = 0; k < 3; ++k) {
				...
				if (v > 1)
					v = 1;
				rgb[k] = (unsigned char)(v * 255.0f + 0.5f);
			}
			fwrite(rgb, 1, 3, f);
			if (zb[i] < 1e29f)
				++shaded;

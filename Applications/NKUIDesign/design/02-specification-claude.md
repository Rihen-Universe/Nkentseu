...
| 1k | 🟡 **porté au canal** : `nkgui::Selectable` ne publie un `lastItemRect` utilisable **que** dans une rangée explicite — 120×28 en `BeginRow`, **1×28** en largeur auto, **0×28** en `BeginFlow`. Un essai à la souris ne peut donc viser que les contrôles posés en rangée. Contourné (la palette est devenue un contrôle segmenté, ce qui se défend par ailleurs), pas corrigé | — |

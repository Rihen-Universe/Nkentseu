...
| 1m | ✅ **l'agencement à la souris — prouvé, et il n'écrit pas de coordonnée.** L'aperçu publie le rectangle de chaque nœud ; glisser du bord droit de « Arbre » de +80 px puis `Ctrl+S` donne `largeur = fixed 260` → **`fixed 340`**. Le geste écrit une **taille**, jamais une position. ⚠️ La position d'un nœud est un **résultat** : la recalculer dans l'essai reviendrait à réimplémenter le solveur qu'on éprouve | 2 |
| 1n | ✅ **le rapport de sonde porte l'identité du binaire qui l'a écrit** (`__DATE__`/`__TIME__`). Un rapport « 78/78 » daté de la veille avait fait croire à un binaire périmé — **le binaire rendait 103/103** ; la sonde écrit dans le répertoire courant et la copie posée à côté de l'exécutable n'était jamais rafraîchie. Un rapport périmé ressemble exactement à un rapport frais | — |

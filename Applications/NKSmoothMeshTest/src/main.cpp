...
int main() {
    ...
    printf("  export OBJ lisse : %s -> Build/nkgen_smooth.obj (normales: %s)\n",
           savedSmooth ? "OK" : "KO", smooth.HasNormals() ? "oui" : "non");
    ...
    printf("  décimé : %u sommets, %u triangles  (réduction : %.0f%% des triangles)\n",
           retopo.VertexCount(), retopo.TriangleCount(),
           smooth.TriangleCount() ? 100.0 * (1.0 - (double)retopo.TriangleCount() / (double)smooth.TriangleCount()) : 0.0);
    ...
    auto idxValid = [](const gen::NkMesh& m) {
        for (uint32 i = 0; i < m.triangles.Size(); ++i) if (m.triangles[i] >= m.VertexCount()) return false;
        return true;
    };
    ...
    check(retopo.TriangleCount() > 0 &&
          retopo.TriangleCount() < smooth.TriangleCount(),
    ...
    for (uint32 i = 0; i < quadMesh.quads.Size(); ++i)
        if (quadMesh.quads[i] >= quadMesh.VertexCount()) { quadIdxOk = false; break; }

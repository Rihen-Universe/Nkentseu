...
static void Proto(uint32 kind, float* out) {
    for (uint32 i = 0; i < VOX; ++i) out[i] = 0.f;
    for (uint32 z=0; z<G; ++z) for (uint32 y=0; y<G; ++y) for (uint32 x=0; x<G; ++x) {
        float fx=(float)x, fy=(float)y, fz=(float)z; bool on=false;
        if (kind == 0) {
            ...
            for (int i=0;i<4;++i) {
                float d=(fx-cs[i][0])*(fx-cs[i][0])+(fy-cs[i][1])*(fy-cs[i][1])+(fz-cs[i][2])*(fz-cs[i][2]);
                if (d <= cs[i][3]*cs[i][3]) { on=true; break; }
            }
        } else {
...
int main() {
    ...
    {
        float* xp=X.DataAs<float>(); float proto[VOX]; uint32 s=1234u;
        for (uint32 c=0;c<NCLS;++c){ Proto(c, proto);
    ...
    for (int e=0;e<=600;++e){
        ...
        if (e%100==0) printf("  époque %3d : recon MSE = %.5f\n", e, reconMse);
    }

    // Centroïde latent par classe (= "conditionnement" : où vit chaque objet).
    NkVar muV, lvV; vae.Encode(xin, muV, lvV);
    ...
    for (uint32 c=0;c<NCLS;++c) {
        ...
        for (uint32 l=0; l<LAT; ++l) {
            double m=0; for (uint32 k=0;k<PER;++k) m += mp[(c*PER+k)*LAT + l];
        ...
        uint32 occ=0; for (uint32 j=0;j<VOX;++j) if (gp[j]>0.5f) ++occ;
        gen::NkMesh mesh = gen::VoxelsToMesh(gp, G, 0.5f, 1.0f);
        char path[64]; snprintf(path, sizeof(path), "Build/nkgen_%s.obj", names[c]);

...
int main() {
    ...
    if (!f) { printf("  MNIST introuvable (%s). Définis NK_MNIST_DIR.\n", path); return 1; }
    ...
    NkVector<int32> dummy; for (uint32 i=0;i<N;++i) dummy.PushBack(0);
    ...
    for (int e=0;e<=60;++e){
        ...
        for (uint32 b=0;b<loader.NumBatches();++b){
            ...
            NkTensor epsT=NkTensor::Zeros(NkShape{ B, (int64)LAT }); FillRandn(epsT, rng);
        ...
        if (e%10==0) printf("  époque %2d : recon BCE = %.5f\n", e, reconMse);
    }

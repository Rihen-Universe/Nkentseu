...
namespace nkedemo {
	...
	struct EditorScene {
			...
			EditorScene() {
				entities[0].name = NkString("Heros");
				entities[0].kind = EntityKind::Player;
				entities[1].name = NkString("Gobelin");
				entities[1].kind = EntityKind::Enemy;
				entities[1].health = 30;
				entities[2].name = NkString("Coffre");
				entities[2].kind = EntityKind::Prop;
				entities[2].health = 0;
			}
	};
		...
		public:
			...
			void OnUI(NkEditorFrameContext &ec) override {
				...
				for (int32 i = 0; i < EditorScene::COUNT; ++i) {
					...
					if (Selectable(ctx, n && *n ? n : "(sans nom)", i == mScene->selected))
						mScene->selected = i;
				}
			}
		...
		public:
			...
			void OnUI(NkEditorFrameContext &ec) override {
				...
				if (mSelected >= 0) {
					...
					int32 k = 0;
					const char *pre = "Selection : ";
					for (; pre[k]; ++k)
						buf[k] = pre[k];
					for (int32 j = 0; s[j] && k + 1 < 64; ++j, ++k)
						buf[k] = s[j];
					buf[k] = '\0';
				}
				ec.Text(buf);

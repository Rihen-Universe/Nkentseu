...
namespace nkentseu {
	namespace songoo {
		...
		AudioSample AudioManager::MakePickup() const {
			...
			AudioSample s = AudioGenerator::GenerateTone(440.0f, 0.08f, WaveformType::SINE, 44100, 0.7f);
			AdsrEnvelope env;
		...
		AudioSample AudioManager::MakeDeposit() const {
			...
			AudioSample s = AudioGenerator::GenerateTone(660.0f, 0.05f, WaveformType::SQUARE, 44100, 0.5f);
			AdsrEnvelope env;
		...
		AudioSample AudioManager::MakeDrum() const {
			...
			AudioSample s = AudioGenerator::GenerateChirp(200.0f, 80.0f, 0.30f, 44100);
			AdsrEnvelope env;
		...
		AudioSample AudioManager::MakeScore() const {
			...
			AudioSample s = AudioGenerator::GenerateChirp(220.0f, 880.0f, 0.40f, 44100);
			AdsrEnvelope env;
		...
		bool AudioManager::Initialize() {
			...
			for (auto &sf : files) {
				if (!LoadSample(sf.id, sf.path)) {
					...
					switch (sf.id) {
						case SoundId::Pickup:
							mSamples[(int)sf.id] = MakePickup();
							break;
						case SoundId::Deposit:
							mSamples[(int)sf.id] = MakeDeposit();
							break;
						case SoundId::Drum:
							mSamples[(int)sf.id] = MakeDrum();
							break;
						case SoundId::Score:
							mSamples[(int)sf.id] = MakeScore();
							break;
						default:
							break;
					}
		...
		bool AudioManager::LoadSample(SoundId id, const char *path) noexcept {
			...
			if (!ok)
				logger.Warn("[AudioManager] Cannot load '{}' — using procedural", path);
			return ok;
		}
		...
		void AudioManager::ResumeBgMusic() {
			if (mInitialized)
				mBgMusic.Resume();
		}
		...
		void AudioManager::StopCreditMusic() {
			if (mInitialized)
				mCreditMusic.Stop();
		}
		...
		void AudioManager::SetMusicVolume(float v) noexcept {
			...
			if (mInitialized) {
				mBgMusic.SetVolume(eff);
				mCreditMusic.SetVolume(eff);
			}
		}

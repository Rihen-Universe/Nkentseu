...
namespace nkentseu {
	namespace songoo {
		...
		void SceneManager::Push(Scene *s) {
			mPending.push_back({CmdType::Push, s});
		}

		void SceneManager::Pop() {
			mPending.push_back({CmdType::Pop, nullptr});
		}

		void SceneManager::Replace(Scene *s) {
			mPending.push_back({CmdType::Replace, s});
		}

		void SceneManager::PopToRoot() {
			mPending.push_back({CmdType::PopToRoot, nullptr});
		}

...
namespace nkentseu {
	namespace songoo {
		...
		static math::NkColor VF() {
			return {30, 100, 40, 255};
		}
		...
		void OptionsScene::OnUpdate(AppContext &ctx, float dt) {
			...
			mBtnBackX = cx - bW * 0.5f;
			mBtnBackY = cy + 130.f * scale;
			mBtnBackW = bW;
			mBtnBackH = bH;
		}
		...
		void OptionsScene::OnEvent(AppContext &ctx, NkEvent &ev) {
			...
			auto handlePos = [&](float px, float py, bool release) {
				if (!release) {
					if (HitSlider(mMusicSlider, px, py)) {
						mDraggingMusic = true;
					}
					if (HitSlider(mSfxSlider, px, py)) {
						mDraggingSfx = true;
					}
				}
				if (release) {
					mDraggingMusic = mDraggingSfx = false;
				}
				if (mDraggingMusic) {
					gs.musicVolume = ComputeSliderVal(mMusicSlider, px);
					if (ctx.audio)
						ctx.audio->SetMusicVolume(gs.musicVolume);
				}
				if (mDraggingSfx) {
					gs.sfxVolume = ComputeSliderVal(mSfxSlider, px);
					if (ctx.audio)
						ctx.audio->SetSfxVolume(gs.sfxVolume);
				}
				if (release) {
		...
		void OptionsScene::DrawSlider(AppContext &ctx, const Slider &s, float val, const char *label,
									  math::NkColor col) {
			...
			f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, 0.85f * scale, s.x + s.w * 0.5f, s.y - 18.f * scale,
									   buf, {230, 200, 140, 220});
		}

		void OptionsScene::DrawToggle(AppContext &ctx, float x, float y, float w, float h, bool on, const char *label,
									  math::NkColor col) {
			GLRenderer2D &r = *ctx.renderer;
			FontAtlas &f = *ctx.font;
			const float scale = GetUIScale(ctx.viewportW, ctx.viewportH);
			...
			f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, 0.75f * scale, x - 70.f * scale, y + h * 0.15f, label,
									   {220, 190, 130, 200});
		}
		...
		void OptionsScene::OnRender(AppContext &ctx) {
			...
			f.DrawStringShadowCenteredScaled(r, FontAtlas::HeadlineSlot, 1.1f * scale, cx, panY + 20.f * scale,
											 "OPTIONS", {242, 184, 46, 255}, {80, 30, 5, 120}, 2);

			r.DrawQuad(panX + 20.f * scale, panY + 78.f * scale, panW - 40.f * scale, 2.f, AlphaF(OR_(), 0.4f));

			// Section Audio
			f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, 0.9f * scale, cx, panY + 95.f * scale, "AUDIO",
									   {242, 184, 46, 200});

			DrawSlider(ctx, mMusicSlider, gs.musicVolume, "Musique", OR_());
			DrawSlider(ctx, mSfxSlider, gs.sfxVolume, "Effets", TC());

			DrawToggle(ctx, mTgMusic.x, mTgMusic.y, mTgMusic.w, mTgMusic.h, gs.musicEnabled, "Musique", VF());
			DrawToggle(ctx, mTgSfx.x, mTgSfx.y, mTgSfx.w, mTgSfx.h, gs.soundEnabled, "Effets", TC());
			DrawToggle(ctx, mTgDrum.x, mTgDrum.y, mTgDrum.w, mTgDrum.h, gs.drumEnabled, "Tambours", OR_());

			// Bouton Retour
			r.DrawQuad(mBtnBackX, mBtnBackY, mBtnBackW, mBtnBackH, {70, 30, 6, 200});
			r.DrawQuadOutline(mBtnBackX, mBtnBackY, mBtnBackW, mBtnBackH, OR_(), 2.f);
			f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, mBtnBackX + mBtnBackW * 0.5f,
									   mBtnBackY + mBtnBackH * 0.18f, "RETOUR", {255, 235, 180, 255});

...
namespace nkentseu {
	namespace songoo {
		...
		void MainMenuScene::OnRender(AppContext &ctx) {
			...
			f.DrawStringShadowCenteredScaled(r, FontAtlas::DisplaySlot, 1.5f * scale * pulse, sa.SafeCX(), titleY,
											 "SONGO'O", {(uint8_t)(210 * pulse), (uint8_t)(160 * pulse), 30, 255},
											 {80, 30, 5, 160}, 3);

			f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, 0.75f * scale, sa.SafeCX(), titleY + 68.f * scale,
									   "~ Jeu Traditionnel Camerounais ~", {184, 133, 56, (uint8_t)(200 * mEnterAnim)});

			// Séparateur or
			r.DrawQuad(mCardListX, titleY + 95.f * scale, mCardListW, 2.f, AlphaF(OR_(), 0.5f));
			...
			for (int i = 0; i < kItemCount; ++i) {
				...
				// Fond card
				math::NkColor bgC = focused ? AlphaF(TC(), 0.80f) : AlphaF(BC(), 0.55f);
				math::NkColor brdC = focused ? AlphaF(OR_(), 0.90f) : AlphaF(TC(), 0.40f);
				...
				f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, tscale, cardX + cardW * 0.5f,
										   cardY + cardH * 0.22f, kItems[i].label, txtC);

				// Description
				if (focused) {
					f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, 0.75f * scale, cardX + cardW * 0.5f,
											   cardY + cardH * 0.62f, kItems[i].desc, {220, 190, 120, 180});
				}
			}
			...
			f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, 0.65f * scale, sa.SafeCX(), botY + 6.f * scale,

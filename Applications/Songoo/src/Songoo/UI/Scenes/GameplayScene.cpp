...
namespace nkentseu {
	namespace songoo {
		...
		static math::NkColor TC() {
			return {180, 70, 15, 255};
		}

		static math::NkColor OR_() {
			return {210, 160, 30, 255};
		}

		static math::NkColor CY() {
			return {0, 200, 255, 255};
		}

		static math::NkColor PAR() {
			return {255, 235, 184, 255};
		}
		...
		void GameplayScene::OnEnter(AppContext &ctx) {
			...
			for (int i = 0; i < 14; i++)
				mVisualPits[i] = mBoard.GetPitGrains(i);
		...
		void GameplayScene::ComputeLayout(AppContext &ctx) {
			...
			for (int id = 1; id <= 7; id++) {
				int i = id - 1;
				float x = (id == 1) ? (DX + (id - 1) * w - 10.f * mScale) : (DX + (id - 1) * steep);
				mPitGeo[i] = {x + w * 0.5f, DY + h * 0.5f, w * 0.45f};
			}
			for (int id = 8; id <= 14; id++) {
				int i = id - 1;
				float x = (id == 8) ? (DX + (id - 8) * w - 10.f * mScale) : (DX + (id - 8) * steep);
		...
		void GameplayScene::OnEvent(AppContext &ctx, NkEvent &ev) {
			...
			auto handleTouch = [&](float px, float py) {
				// Bouton retour
				if (px >= mRetourBtnX && px <= mRetourBtnX + mRetourBtnW && py >= mRetourBtnY &&
					py <= mRetourBtnY + mRetourBtnH) {
					ctx.scenes->Pop();
					return;
				}
		...
		void GameplayScene::TryMove(AppContext &ctx, int pitIdx) {
			if (mGameOver || mAnim.active)
				return;

			// IA bloquée au clic si c'est son tour (mode IA)
			if (ctx.settings && ctx.settings->mode == GameMode::VsAI && mCurrentPlayer == 1)
				return;

			if (!mBoard.CanPlay(mCurrentPlayer, pitIdx)) {
				bool wrongCamp = (mCurrentPlayer == 0 && pitIdx >= 7) || (mCurrentPlayer == 1 && pitIdx < 7);
				std::strncpy(mStatusMsg, wrongCamp ? "Ce n'est pas votre camp !" : "Ce trou est vide !",
							 sizeof(mStatusMsg));
				mShowInvalidFb = true;
				mInvalidFbT = 0.f;
				return;
		...
		void GameplayScene::SwitchPlayer(AppContext &ctx) {
			...
			std::snprintf(mStatusMsg, sizeof(mStatusMsg), "Au tour du Joueur %d", mCurrentPlayer + 1);
			if (ctx.audio)
				ctx.audio->PlayDrum();
		...
		void GameplayScene::OnRender(AppContext &ctx) {
			...
			if (mShowInvalidFb) {
				...
				r.DrawQuad(DX - 10.f, DY - 10.f, 7.f * (w + 30.f * mScale) + 20.f, 2.f * h + gap + 20.f,
		...
		void GameplayScene::DrawBackground(AppContext &ctx) {
			...
			if (mBgTex.IsValid()) {
				...
				r.DrawTexturedQuadRGBA(DX - 20.f, DY - 20.f, bW + 40.f, bH + 40.f, 0.f, 0.f, 1.f, 1.f,
									   {255, 255, 255, 255});
			} else {
		...
		void GameplayScene::DrawPlayerGlow(AppContext &ctx) {
			...
			if (mCurrentPlayer == 0) {
				...
				r.DrawQuad(DX - 10.f, 215.f * mScale, rowW + 20.f, 270.f * mScale, {255, 185, 30, a});
				r.DrawQuadOutline(DX - 10.f, 215.f * mScale, rowW + 20.f, 270.f * mScale, {255, 200, 50, ab}, 2.f);
			} else {
				float yBot = 215.f * mScale + h + gap;
				uint8_t a = (uint8_t)(60.f * pulse);
				uint8_t ab = (uint8_t)(120.f * pulse);
				r.DrawQuad(DX - 10.f, yBot - 10.f, rowW + 20.f, 270.f * mScale, {60, 160, 255, a});
				r.DrawQuadOutline(DX - 10.f, yBot - 10.f, rowW + 20.f, 270.f * mScale, {100, 180, 255, ab}, 2.f);
			}
		}
		...
		void GameplayScene::DrawPit(AppContext &ctx, int i) {
			...
			if (mTrouTex[texIdx].IsValid()) {
				...
				r.DrawTexturedQuadRGBA(g.cx - hw, g.cy - hh, hw * 2.f, hh * 2.f, 0.f, 0.f, 1.f, 1.f,
									   {255, 255, 255, 255});
			} else {
				bool isP1 = (i < 7);
				math::NkColor col = isP1 ? TC() : CY();
				r.DrawCircle(g.cx, g.cy, g.radius, AlphaF(col, 0.35f), 24);
				r.DrawCircleOutline(g.cx, g.cy, g.radius + 1.5f, AlphaF(col, mHoveredPit == i ? 0.9f : 0.6f), 2.f, 24);
				char buf[8];
				std::snprintf(buf, sizeof(buf), "%d", grains);
				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, mScale, g.cx, g.cy - 8.f * mScale, buf, PAR());
			}

			if (mHoveredPit == i) {
				float p = 0.5f + 0.5f * math::NkSin(mTime * 3.5f);
				r.DrawCircleOutline(g.cx, g.cy, g.radius + 4.f, {200, 150, 50, (uint8_t)(150 * p)}, 3.f, 24);
			}
		}
		...
		void GameplayScene::DrawHand(AppContext &ctx) {
			...
			if (mHandTex.IsValid()) {
				...
				r.DrawTexturedQuadRGBA(mAnim.handX - hw, mAnim.handY - hh, hw * 2.f, hh * 2.f, 0.f, 0.f, 1.f, 1.f,
									   {255, 255, 255, 255});
			} else {
				r.DrawCircle(mAnim.handX, mAnim.handY, 20.f * mScale, {230, 120, 20, 220}, 16);
			}

			// CORRECTION BUG 2 : plus de TTF_OpenFont ici — FontAtlas natif
			if (mAnim.grainesLeft > 0) {
				char buf[8];
				std::snprintf(buf, sizeof(buf), "%d", mAnim.grainesLeft);
				f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, mScale, mAnim.handX, mAnim.handY - 52.f * mScale,
										   buf, {255, 255, 200, 255});
			}
		}
		...
		void GameplayScene::DrawUI(AppContext &ctx) {
			...
			r.DrawQuad(sa.LeftX(), sa.TopY(), sa.SafeW(), headerH, {20, 12, 4, 210});

			// Bouton Retour
			r.DrawQuad(mRetourBtnX, mRetourBtnY, mRetourBtnW, mRetourBtnH, {80, 35, 5, (uint8_t)(200 * mEnterAnim)});
			r.DrawQuadOutline(mRetourBtnX, mRetourBtnY, mRetourBtnW, mRetourBtnH, OR_(), 1.5f);
			f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, 0.85f * mScale, mRetourBtnX + mRetourBtnW * 0.5f,
									   mRetourBtnY + mRetourBtnH * 0.18f, "RETOUR",
									   {255, 235, 180, (uint8_t)(200 * mEnterAnim)});
			...
			f.DrawStringShadowCenteredScaled(
				r, FontAtlas::SubtitleSlot, 1.1f * mScale, cx - 220.f * mScale, sa.TopY(10.f * mScale), s1,
				p1Active ? math::NkColor{250, 204, 38, 255} : math::NkColor{160, 120, 60, 180}, {40, 15, 2, 120}, 2);
			f.DrawStringShadowCenteredScaled(
				r, FontAtlas::SubtitleSlot, 1.1f * mScale, cx + 220.f * mScale, sa.TopY(10.f * mScale), s2,
				!p1Active ? math::NkColor{140, 214, 255, 255} : math::NkColor{80, 120, 160, 180}, {5, 20, 40, 120}, 2);

			// ── Statut bas ────────────────────────────────────────────────────────
			float statY = sa.BottomY(48.f * mScale);
			r.DrawQuad(sa.LeftX(), statY, sa.SafeW(), 44.f * mScale, {10, 6, 2, 180});
			f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, 0.9f * mScale, cx, statY + 8.f * mScale, mStatusMsg,
									   p1Active ? math::NkColor{250, 214, 76, 255} : math::NkColor{153, 219, 255, 255});

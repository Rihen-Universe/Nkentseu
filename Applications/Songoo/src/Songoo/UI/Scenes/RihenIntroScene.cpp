	...
	namespace songoo {
		...
		void RihenIntroScene::WorkerProc() {
			...
			for (int i = 0; i < kFrameCount; ++i) {
				...
				std::snprintf(path, sizeof(path), "Resources/Songoo/assets/animrihen/rihen_%05d.png", i);
			...
			while (uploaded < maxUploads && mPendingNext < kFrameCount) {
				...
				if (img) {
					...
					if (ok && mPendingNext == 0)
						mAspect =
							(float)mFrames[0].Width() / (float)(mFrames[0].Height() > 0 ? mFrames[0].Height() : 1);
					if (ok) {
						++mFramesLoaded;
						++uploaded;
					}
		...
		void RihenIntroScene::OnRender(AppContext &ctx) {
			...
			if (mState == State::Loading) {
				...
				pitCX[12] = boardX + boardW * 0.95f;
				pitCY[12] = row1Y;
				pitCX[13] = boardX + boardW * 0.95f;
				pitCY[13] = row2Y;
				...
				if (mLoadingLogo.IsValid()) {
					...
					r.DrawTexturedQuadRGBA(cx - lw * 0.5f, boardY - lh * 1.2f, lw, lh, 0.f, 0.f, 1.f, 1.f,
										   theme::White());
				}
			...
			if (mFramesLoaded > 0) {
				...
				r.DrawTexturedQuadRGBA(drawX, drawY, drawW, drawH, 0.f, 0.f, 1.f, 1.f, theme::White());
			}

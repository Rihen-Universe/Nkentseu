...
namespace nkentseu {
	namespace songoo {
		...
		static const CredLine kLines[] = {
			...
			{"", 0.8f, false, 0, 0, 0},
			{"\xE2\x94\x80\xE2\x94\x80\xE2\x94\x80\xE2\x94\x80\xE2\x94\x80\xE2\x94\x80\xE2\x94\x80\xE2\x94\x80\xE2\x94"
		...
		void CreditsScene::OnRender(AppContext &ctx) {
			...
			for (int i = 0; i < kLineCount; i++) {
				...
				if (y + lineH > 0.f && y < (float)H && kLines[i].text && kLines[i].text[0] != '\0') {
					// Fond doré pour les titres
					if (kLines[i].isTitle) {
						r.DrawQuad(0.f, y - 4.f, (float)W, lineH + 8.f,
								   {(uint8_t)(kLines[i].r / 2), (uint8_t)(kLines[i].g / 2), 0, 60});
					}

					// Choisir le slot de police selon la taille
					int slot = FontAtlas::BodySlot;
					if (kLines[i].sizeRel >= 2.5f)
						slot = FontAtlas::DisplaySlot;
					else if (kLines[i].sizeRel >= 1.5f)
						slot = FontAtlas::HeadlineSlot;
					else if (kLines[i].sizeRel >= 1.1f)
						slot = FontAtlas::SubtitleSlot;
					else if (kLines[i].sizeRel >= 0.85f)
						slot = FontAtlas::BodySlot;
					else
						slot = FontAtlas::SmallSlot;

					float textScale = kLines[i].sizeRel * 0.7f * scale;

					f.DrawStringCenteredScaled(r, slot, textScale, cx, y + lineH * 0.18f, kLines[i].text,
			...
			for (int k = 0; k < 14; k++) {
				r.DrawQuad(k * segW2, (float)H - 12.f * scale, segW2 + 1.f, 12.f * scale, kente[k % 3]);
			}
			...
			f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, 0.85f * scale, bX + bW * 0.5f, bY + bH * 0.18f,

...
namespace nkentseu {
	namespace songoo {
		...
		void GameOverScene::OnUpdate(AppContext &ctx, float dt) {
			...
			mBtnReplayX = cx - bW - 10.f * scale;
			mBtnReplayY = cy + 80.f * scale;
			mBtnReplayW = bW;
			mBtnReplayH = bH;
			mBtnMenuX = cx + 10.f * scale;
			mBtnMenuY = cy + 80.f * scale;
			mBtnMenuW = bW;
			mBtnMenuH = bH;
		}
		...
		void GameOverScene::OnEvent(AppContext &ctx, NkEvent &ev) {
			auto doHit = [&](float px, float py) {
				if (px >= mBtnReplayX && px <= mBtnReplayX + mBtnReplayW && py >= mBtnReplayY &&
					py <= mBtnReplayY + mBtnReplayH) {
					ctx.scenes->Replace(new GameplayScene());
					return;
				}
				if (px >= mBtnMenuX && px <= mBtnMenuX + mBtnMenuW && py >= mBtnMenuY && py <= mBtnMenuY + mBtnMenuH) {
		...
		void GameOverScene::OnRender(AppContext &ctx) {
			...
			f.DrawStringShadowCenteredScaled(
				r, FontAtlas::DisplaySlot, 1.3f * scale * pulse, cx, panY + 30.f * scale, "FIN DE PARTIE",
				{(uint8_t)(247 * pulse), (uint8_t)(184 * pulse), 46, (uint8_t)(255 * alpha)}, {80, 30, 5, 120}, 3);

			// Séparateur
			r.DrawQuad(panX + 30.f * scale, panY + 100.f * scale, panW - 60.f * scale, 2.f,
					   AlphaF(OR_(), alpha * 0.6f));

			// Scores
			char b1[32], b2[32];
			snprintf(b1, sizeof(b1), "Joueur 1 : %d graines", mS0);
			snprintf(b2, sizeof(b2), "Joueur 2 : %d graines", mS1);
			f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, cx, panY + 120.f * scale, b1,
									   {235, 204, 133, (uint8_t)(255 * alpha)});
			f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, cx, panY + 160.f * scale, b2,
			...
			math::NkColor rcol;
			if (mWinner == 0) {
				result = "VICTOIRE JOUEUR 1 !";
				rcol = {247, 204, 51, (uint8_t)(255 * alpha)};
			} else if (mWinner == 1) {
				result = "VICTOIRE JOUEUR 2 !";
				rcol = {140, 214, 255, (uint8_t)(255 * alpha)};
			} else {
				result = "EGALITE !";
				rcol = {247, 224, 92, (uint8_t)(255 * alpha)};
			}

			f.DrawStringShadowCenteredScaled(r, FontAtlas::HeadlineSlot, 1.1f * scale, cx, panY + 200.f * scale, result,
											 rcol, {30, 15, 3, 100}, 2);
			...
			r.DrawQuadOutline(mBtnReplayX, mBtnReplayY, mBtnReplayW, mBtnReplayH, AlphaF(OR_(), alpha), 2.f);
			f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, mBtnReplayX + mBtnReplayW * 0.5f,
									   mBtnReplayY + mBtnReplayH * 0.18f, "REJOUER", AlphaF(PAR(), alpha));

			// Bouton Menu
			r.DrawQuad(mBtnMenuX, mBtnMenuY, mBtnMenuW, mBtnMenuH,
					   {(uint8_t)(30 * alpha), (uint8_t)(20 * alpha), (uint8_t)(10 * alpha), (uint8_t)(200 * alpha)});
			r.DrawQuadOutline(mBtnMenuX, mBtnMenuY, mBtnMenuW, mBtnMenuH, AlphaF(TC(), alpha), 2.f);
			f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, mBtnMenuX + mBtnMenuW * 0.5f,
									   mBtnMenuY + mBtnMenuH * 0.18f, "MENU PRINCIPAL", AlphaF(PAR(), alpha));

...
namespace nkentseu {
	namespace songoo {
		...
		void StoryScene::OnUpdate(AppContext &ctx, float dt) {
			...
			while (mCurrentFrame < kFrameCount && mFrameTimer >= kFrames[mCurrentFrame].displayTime) {
		...
		void StoryScene::OnRender(AppContext &ctx) {
			...
			if (mCurrentFrame < kFrameCount && mTextures[mCurrentFrame].IsValid()) {
				...
				r.DrawTexturedQuadRGBA(0.f, 0.f, (float)W, (float)H, 0.f, 0.f, 1.f, 1.f, {255, 255, 255, a});
				...
				f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, 0.9f * scale, cx, (float)H - bandH + 14.f * scale,
										   kFrames[si].subtitle, {255, 240, 200, a});

				// Indication "Toucher pour passer"
				if (mFrameTimer > 2.f) {
					uint8_t ha = (uint8_t)(100.f * alpha);
					f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, 0.7f * scale, cx, 16.f * scale,
											   "Appuyer pour passer", {200, 200, 200, ha});
				}
			}

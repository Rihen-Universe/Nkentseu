	...
	namespace noge {
		...
		class NkUIInputBridge {
			public:
				...
				void Attach() {
					...
					mIds[2] =
						NkEventBus::Subscribe<NkMouseButtonReleaseEvent>([this](NkMouseButtonReleaseEvent *e) -> bool {
							...
							int btn = ToUIButton(e->GetButton());
							if (btn >= 0)
								mState.SetMouseButton(btn, false);
							return false;
						});
				...
				// ── Accès ─────────────────────────────────────────────────────────
				nkui::NkUIInputState &GetState() {
					return mState;
				}

				const nkui::NkUIInputState &GetState() const {
					return mState;
				}
				...
				void AddUtf8ToInput(const char *utf8) {
					...
					for (int i = 0; utf8[i];) {
						...
						if (c0 < 0x80u) {
							...
							cp = ((c0 & 0x1Fu) << 6) | (static_cast<nk_uint8>(utf8[i + 1]) & 0x3Fu);
							n = 2;
						} else if ((c0 & 0xF0u) == 0xE0u && utf8[i + 1] && utf8[i + 2]) {
							cp = ((c0 & 0x0Fu) << 12) | ((static_cast<nk_uint8>(utf8[i + 1]) & 0x3Fu) << 6) |
								 (static_cast<nk_uint8>(utf8[i + 2]) & 0x3Fu);
							n = 3;
						} else if ((c0 & 0xF8u) == 0xF0u && utf8[i + 1] && utf8[i + 2] && utf8[i + 3]) {
							cp = ((c0 & 0x07u) << 18) | ((static_cast<nk_uint8>(utf8[i + 1]) & 0x3Fu) << 12) |
								 ((static_cast<nk_uint8>(utf8[i + 2]) & 0x3Fu) << 6) |
								 (static_cast<nk_uint8>(utf8[i + 3]) & 0x3Fu);
							n = 4;
						}

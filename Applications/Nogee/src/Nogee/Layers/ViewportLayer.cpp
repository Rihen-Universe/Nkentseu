...
namespace nkentseu {
	namespace noge {
		...
		void ViewportLayer::OnUpdate(float dt) {
			...
			mCamera->OnMouseMove(mMouse.dx, mMouse.dy, mMouse.altDown, mMouse.rightDown);
			...
			if (mGizmos && mWorld && mSel && mVpWidth > 0 && mVpHeight > 0) {
				mGizmos->Update(*mWorld, *mSel, *mCamera, mMouse.x, mMouse.y, (float32)mVpWidth, (float32)mVpHeight,
		...
		void ViewportLayer::OnRender() {
			if (!mDevice || !mCmd)
				return;
			if (!mRenderPass.IsValid() || !mFramebuffer.IsValid())
				return;
			if (mVpWidth == 0 || mVpHeight == 0)
				return;

			const NkRect2D area{0, 0, static_cast<nk_int32>(mVpWidth), static_cast<nk_int32>(mVpHeight)};
		...
		void ViewportLayer::DestroyFBO() noexcept {
			...
			if (mDepthTarget.IsValid())
				mDevice->DestroyTexture(mDepthTarget);
			if (mColorTarget.IsValid())
				mDevice->DestroyTexture(mColorTarget);
			mFramebuffer = {};
			mRenderPass = {};
			mDepthTarget = {};
			mColorTarget = {};

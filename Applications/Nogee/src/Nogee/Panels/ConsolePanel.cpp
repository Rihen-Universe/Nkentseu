...
namespace nkentseu {
	namespace noge {
		...
		void ConsolePanel::Render(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDrawList &dl, NkUIFont &font,
			...
			bool open = true;
			if (!NkUIWindow::Begin(ctx, wm, dl, font, ls, "Console", &open,
			...
			if (NkUI::ButtonSmall(ctx, ls, dl, font, "Effacer"))
				Clear();
			...
			if (mErrorCount > 0) {
				char buf[32];
				snprintf(buf, sizeof(buf), "Err:%u", mErrorCount);
				NkUI::TextColored(ctx, ls, dl, font, {255, 80, 80, 255}, buf);
				NkUI::SameLine(ctx, ls);
			}
			if (mWarnCount > 0) {
				char buf[32];
				snprintf(buf, sizeof(buf), "Avert:%u", mWarnCount);
			...
			if (NkUI::BeginScrollRegion(ctx, ls, "console_scroll", contentRect, nullptr, &scrollY)) {
				for (nk_usize i = 0; i < mLines.Size(); ++i) {
					...
					// Filtre texte
					if (mFilterBuf[0] != '\0') {
						if (!line.text.Contains(mFilterBuf))
							continue;
					}

					// Ligne
					char prefix[8];
					snprintf(prefix, sizeof(prefix), "%s", LevelPrefix(line.level));
					NkString display = NkString(prefix) + line.text;
					if (line.count > 1) {
						char cnt[16];
						snprintf(cnt, sizeof(cnt), " (%u)", line.count);
						display += NkString(cnt);
					}

					NkUI::TextColored(ctx, ls, dl, font, LevelColor(line.level), display.CStr());
				}

...
namespace nkentseu {
	namespace noge {
		...
		void InspectorPanel::Render(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDrawList &dl, NkUIFont &font,
			...
			if (!NkUIWindow::Begin(ctx, wm, dl, font, ls, "Inspector##insp", nullptr,
			...
			if (!id.IsValid() || !world.IsAlive(id)) {
				NkUI::Text(ctx, ls, dl, font, "Aucune entité sélectionnée", NkColor{130, 130, 130, 255});
				NkUIWindow::End(ctx, wm, dl, ls);
				return;
			}
		...
		void InspectorPanel::RenderComponent(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkUILayoutStack &ls,
											 const ecs::NkComponentMeta &meta, void *compPtr, ecs::NkEntityId /*id*/,
			...
			for (nk_usize i = 0; i < meta.fields.Size(); ++i) {
				...
				NkUI::Text(ctx, ls, dl, font, field.name, NkColor{180, 180, 180, 255});
				...
				if (!field.readOnly)
					...
					char buf[64] = "?";
					NkUI::Text(ctx, ls, dl, font, buf, NkColor{150, 150, 150, 255});
				}
		...
		bool InspectorPanel::RenderField(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkUILayoutStack &ls,
										 const ecs::NkFieldMeta &f, void *ptr) noexcept {
			...
			switch (f.type) {
				...
				case FT::Float32: {
					...
					changed = NkUI::DragFloat(ctx, ls, dl, font, "##f32", v, st > 0.f ? st : 0.01f, mn, mx);
					break;
				}
				case FT::Int32: {
				...
				case FT::Vec4f:
				case FT::Color: {
					NkColor col{(nk_uint8)(static_cast<float32 *>(ptr)[0] * 255),
				...
				case FT::StringFixed: {
					...
					changed = NkUI::InputText(ctx, ls, dl, font, "##str", buf, maxLen);
					break;
				}
				case FT::AssetPath: {
				...
				default:
					NkUI::Text(ctx, ls, dl, font, "(non éditable)", NkColor{100, 100, 100, 255});
					break;
			}
			return changed;
		}

		void InspectorPanel::RenderAddComponentMenu(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font,
													NkUILayoutStack &ls, ecs::NkWorld &world,

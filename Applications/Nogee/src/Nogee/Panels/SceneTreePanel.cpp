...
namespace nkentseu {
	namespace noge {
		...
		void SceneTreePanel::Render(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDrawList &dl, NkUIFont &font,
			...
			if (!NkUIWindow::Begin(ctx, wm, dl, font, ls, "Scene##tree", nullptr,
			...
			NkUI::Text(ctx, ls, dl, font, "Scène", NkColor{180, 200, 255, 255});
			...
			if (mContextMenuEntity.IsValid()) {
				RenderContextMenu(ctx, dl, font, world, mContextMenuEntity, scene, sel);
			}

			NkUIWindow::End(ctx, wm, dl, ls);
		}

		void SceneTreePanel::RenderEntity(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkUILayoutStack &ls,
										  ecs::NkWorld &world, ecs::NkEntityId id, NkSelectionManager &sel,
										  CommandHistory *hist, nk_uint32 depth) noexcept {
			...
			if (mRenamingEntity == id) {
				...
				char label[96];
				snprintf(label, sizeof(label), "%s##%u_%u", node->name[0] ? node->name : "Entity", id.index, id.gen);
				...
				if (hasChildren) {
					...
					clicked = NkUI::InvisibleButton(ctx, ls, label, {-1.f, ctx.GetItemHeight()});
					if (!clicked) {
						// Dessiner le texte manuellement dans la zone
						NkVec2 cursor = ctx.GetCursor();
						dl.AddText({cursor.x + 16.f, cursor.y + 2.f}, node->name, NkColor{210, 210, 210, 255});
					}
					NkUI::EndRow(ctx, ls);
				}
			...
			if (hasChildren && IsOpen(id)) {
				for (nk_uint32 i = 0; i < children->count; ++i) {
					RenderEntity(ctx, dl, font, ls, world, children->children[i], sel, hist, depth + 1);
				}
				if (hasChildren)
					NkUI::TreePop(ctx, ls);
			}
		...
		void SceneTreePanel::RenderContextMenu(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, ecs::NkWorld &world,
											   ecs::NkEntityId id, ecs::NkSceneGraph *scene,
			...
			if (NkUIMenu::MenuItem(ctx, dl, font, "Renommer", "F2")) {
				...
				if (n)
					NkStrNCpy(mRenameBuffer, n->name, 63);
				else
					mRenameBuffer[0] = '\0';
				mContextMenuEntity = ecs::NkEntityId::Invalid();
			}

	...
	namespace noge {
		...
		bool ProjectManager::Load(const char *path) noexcept {
			...
			if (archive.GetString("name", s))
				mConfig.name = s;
			if (archive.GetString("version", s))
				mConfig.version = s;
			if (archive.GetString("assetsDir", s))
				mConfig.assetsDir = s;
			if (archive.GetString("buildsDir", s))
				mConfig.buildsDir = s;
			if (archive.GetString("startupScene", s))
				mConfig.startupScene = s;
			if (archive.GetString("defaultBackend", s))
			...
			for (nk_int64 i = 0; i < sceneCount; ++i) {
				NkString key = NkFormat("scene_{}", i);
				NkString sc;
				if (archive.GetString(key.CStr(), sc))
					mConfig.scenes.PushBack(sc);
			}

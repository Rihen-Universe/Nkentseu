	...
	namespace Noge {
		...
		void NkEditorCamera::Update(float32 dt, bool hovered) noexcept {
			if (mode == NkEditorCameraMode::Orbit)
				UpdateOrbit(dt, hovered);
			else
				UpdateFly(dt, hovered);
			RecalcMatrices();
		}
		...
		void NkEditorCamera::UpdateFly(float32 dt, bool hovered) noexcept {
			...
			NkVec3f fwd = {NkCos(pyRad) * NkSin(yyRad), NkSin(pyRad), -NkCos(pyRad) * NkCos(yyRad)};
			NkVec3f right = NkVec3f(NkCos(yyRad), 0.f, NkSin(yyRad));
			NkVec3f up = {0.f, 1.f, 0.f};

			if (mKeys[static_cast<int>(NkKey::NK_W)])
				flyPos = flyPos + fwd * speed;
			if (mKeys[static_cast<int>(NkKey::NK_S)])
				flyPos = flyPos + fwd * -speed;
			if (mKeys[static_cast<int>(NkKey::NK_A)])
				flyPos = flyPos + right * -speed;
			if (mKeys[static_cast<int>(NkKey::NK_D)])
				flyPos = flyPos + right * speed;
			if (mKeys[static_cast<int>(NkKey::NK_Q)])
				flyPos = flyPos + up * -speed;
			if (mKeys[static_cast<int>(NkKey::NK_E)])
				flyPos = flyPos + up * speed;

			position = flyPos;
		}

		void NkEditorCamera::OnMouseMove(float32 dx, float32 dy, bool altDown, bool rightDown) noexcept {
			mAltDown = altDown;
		...
		NkEditorCamera::Ray NkEditorCamera::ScreenRay(float32 vpX, float32 vpY, float32 vpW,
													  float32 vpH) const noexcept {
			// NDC

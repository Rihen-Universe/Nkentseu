...
namespace nkentseu {
	namespace Noge {
		...
		void NkGizmoSystem::Update(ecs::NkWorld &world, const NkSelectionManager &sel, const NkEditorCamera &cam,
								   float32 vpMouseX, float32 vpMouseY, float32 vpW, float32 vpH, bool leftDown,
			...
			float32 screenScale = (clipPos.w > 0.f) ? size * clipPos.w / (vpH * 0.5f) : 1.f;
		...
		void NkGizmoSystem::BuildTranslateGizmo(const NkVec3f &o, const NkMat4f & /*vp*/, float32 s) noexcept {
			float32 L = s;
		...
		void NkGizmoSystem::BuildRotateGizmo(const NkVec3f &o, const NkMat4f & /*vp*/, float32 s) noexcept {
				...
				for (int i = 0; i < N; ++i) {
					...
					switch (a) {
						case NkGizmoAxis::X:
							...
							p1 = o + NkVec3f(0, NkCos(a1) * s, NkSin(a1) * s);
							break;
						case NkGizmoAxis::Y:
							p0 = o + NkVec3f(NkCos(a0) * s, 0, NkSin(a0) * s);
							p1 = o + NkVec3f(NkCos(a1) * s, 0, NkSin(a1) * s);
							break;
						default:
							p0 = o + NkVec3f(NkCos(a0) * s, NkSin(a0) * s, 0);
							p1 = o + NkVec3f(NkCos(a1) * s, NkSin(a1) * s, 0);
							break;
					}
		...
		void NkGizmoSystem::BuildScaleGizmo(const NkVec3f &o, const NkMat4f &vp, float32 s) noexcept {
		...
		NkGizmoAxis NkGizmoSystem::HitTest(const NkEditorCamera::Ray &ray, const NkVec3f &o, float32 s) const noexcept {
			...
			if (dX < hr && dX < dY && dX < dZ)
				return NkGizmoAxis::X;
			if (dY < hr && dY < dX && dY < dZ)
				return NkGizmoAxis::Y;
			if (dZ < hr && dZ < dX && dZ < dY)
				return NkGizmoAxis::Z;

    ...
    for (uint32 z = 0; z < G; ++z)
        for (uint32 y = 0; y < G; ++y)
            for (uint32 x = 0; x < G; ++x) {
                ...
                bool on = false;
                switch (kind) {
                    case 0: on = (fx*fx + fy*fy + fz*fz) <= 6.3f; break;               // sphère
                    case 1: on = (x>=1 && x<=4 && y>=1 && y<=4 && z>=1 && z<=4); break; // cube
                    case 2: on = (fx*fx + fy*fy) <= 4.2f; break;                        // cylindre
                    default: { float r = 2.5f - (float)z * 0.45f; on = (std::fabs(fx) <= r && std::fabs(fy) <= r); }
                }
...
static void PrintSlices(const float* v, const char* title) {
    ...
    for (uint32 y = 0; y < G; ++y) {
        ...
        for (uint32 z = 0; z < G; ++z) {
            for (uint32 x = 0; x < G; ++x) {
                float val = v[Idx(x, y, z)]; if (val<0.f) val=0.f; if (val>1.f) val=1.f;
...
int main() {
    ...
    {
        float* xp = X.DataAs<float>(); float proto[D]; uint32 s = 7777u;
    ...
    for (int e = 1; e <= 400; ++e) {
        ...
        for (uint32 b = 0; b < loader.NumBatches(); ++b) {
            ...
            sum += loss.Value().GetItem(NkShape{ (int64)0 }); ++nb;
        }
        loader.Shuffle();
        mse = nb ? sum / nb : 0.0;
        if (e % 200 == 0) printf("  époque %3d : MSE = %.5f\n", e, mse);
    }
    ...
    for (uint32 i = 0; i < mesh.triangles.Size(); ++i)
        if (mesh.triangles[i] >= vc) { idxOk = false; break; }

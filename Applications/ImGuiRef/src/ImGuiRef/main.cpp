...
int nkmain(const NkEntryState &state) {
    ...
    events.AddEventCallback<NkMouseButtonPressEvent>(
        [&](NkMouseButtonPressEvent *e) { mouseBtn(e->GetButton(), true); });
    events.AddEventCallback<NkMouseButtonReleaseEvent>(
        [&](NkMouseButtonReleaseEvent *e) { mouseBtn(e->GetButton(), false); });
    events.AddEventCallback<NkMouseWheelVerticalEvent>(
        [&](NkMouseWheelVerticalEvent *e) { io.AddMouseWheelEvent(0.f, static_cast<float>(e->GetDeltaY())); });
    events.AddEventCallback<NkMouseWheelHorizontalEvent>(
        [&](NkMouseWheelHorizontalEvent *e) { io.AddMouseWheelEvent(static_cast<float>(e->GetDeltaX()), 0.f); });
    events.AddEventCallback<NkTextInputEvent>([&](NkTextInputEvent *e) {
        if (const char *s = e->GetUtf8())

...
static void DrawInspector(NkGuiContext &ctx, void *obj, const reflection::NkClass *cls) {
	...
	for (usize i = 0; i < props.Size(); ++i) {
		...
		if (p.category == NkTypeCategory::NK_CLASS) { // sous-objet -> section repliable RECURSIVE
			...
			if (sub && subCls && CollapsingHeader(ctx, lbl))
				DrawInspector(ctx, sub, subCls);
			continue;
		}
		if (p.readOnly)
			ctx.BeginDisabled(true); // grise + non-editable
		...
		switch (p.category) { // colonne 2 : champ ("" = pas de label affiché)
			case NkTypeCategory::NK_BOOL: {
				...
				if (Checkbox(ctx, "", v))
					SetPropertyByName(cls, obj, p.name, NkReflectVariant::From<bool>(v));
				break;
			}
			...
			case NkTypeCategory::NK_FLOAT64: {
				...
				const bool ch =
					p.hasRange ? SliderFloat(ctx, "", v, p.rangeMin, p.rangeMax) : DragFloat(ctx, "", v, 0.05f);
				if (ch)
					SetPropertyByName(cls, obj, p.name, NkReflectVariant::From<float32>(v));
				break;
			}
			...
			case NkTypeCategory::NK_UINT64: {
				...
				const bool ch = p.hasRange ? DragInt(ctx, "", v, 0.25f, static_cast<int32>(p.rangeMin),
													 static_cast<int32>(p.rangeMax))
										   : DragInt(ctx, "", v);
				if (ch)
					SetPropertyByName(cls, obj, p.name, NkReflectVariant::From<int32>(v));
				break;
			}
			case NkTypeCategory::NK_STRING: {
				char buf[256] = {};
				NkString s;
				p.value.Get<NkString>(s);
				const char *csr = s.CStr();
				int32 n = 0;
				while (csr && csr[n] && n < 255) {
					buf[n] = csr[n];
					++n;
				}
				buf[n] = '\0';
				if (InputText(ctx, "", buf, 256))
					SetPropertyByName(cls, obj, p.name, NkReflectVariant::From<NkString>(NkString(buf)));
				break;
			}
...
static bool DemoStyle(NkGuiContext &ctx, const NkGuiStyleItem &it, void *) {
	...
	if (it.kind == NkGuiStyleKind::Button && it.label && it.label[0] == '*') {
		const NkColor a = it.active	   ? NkColor{255, 200, 90, 255}
						  : it.hovered ? NkColor{255, 160, 60, 255}
									   : NkColor{235, 120, 40, 255};
		const NkColor b = it.active	   ? NkColor{255, 120, 190, 255}
						  : it.hovered ? NkColor{235, 80, 165, 255}
									   : NkColor{205, 55, 140, 255};
	...
	if (it.kind == NkGuiStyleKind::DockTarget) {
		if (it.value == 5) {
			ctx.DL().AddRectFilled(it.rect, NkColor{255, 130, 50, 255}, 0.f);
			return true;
		}
		...
		if (it.value == 0)
			ctx.DL().AddCircleFilled(c, 3.f, ic); // onglet
		else if (it.value == 1)
			ctx.DL().AddTriangleFilled({cx - a, cy}, {cx + a * 0.5f, cy - a}, {cx + a * 0.5f, cy + a}, ic);
		else if (it.value == 2)
			ctx.DL().AddTriangleFilled({cx + a, cy}, {cx - a * 0.5f, cy - a}, {cx - a * 0.5f, cy + a}, ic);
		else if (it.value == 3)
			ctx.DL().AddTriangleFilled({cx, cy - a}, {cx - a, cy + a * 0.5f}, {cx + a, cy + a * 0.5f}, ic);
		else
			ctx.DL().AddTriangleFilled({cx, cy + a}, {cx - a, cy - a * 0.5f}, {cx + a, cy - a * 0.5f}, ic);
		return true;
	}
...
int nkmain(const NkEntryState &state) {
	...
	events.AddEventCallback<NkMouseWheelVerticalEvent>([&](NkMouseWheelVerticalEvent *e) {
		...
		ctx.input.ctrlDown = m.ctrl;
		ctx.input.shiftDown = m.shift;
		ctx.input.altDown = m.alt;
	});
	...
	auto setKey = [&](NkKey k, bool down) {
		switch (k) {
			case NkKey::NK_LEFT:
				ctx.input.SetKey(NkGuiKey::Left, down);
				break;
			case NkKey::NK_RIGHT:
				ctx.input.SetKey(NkGuiKey::Right, down);
				break;
			case NkKey::NK_UP:
				ctx.input.SetKey(NkGuiKey::Up, down);
				break;
			case NkKey::NK_DOWN:
				ctx.input.SetKey(NkGuiKey::Down, down);
				break;
			case NkKey::NK_HOME:
				ctx.input.SetKey(NkGuiKey::Home, down);
				break;
			case NkKey::NK_END:
				ctx.input.SetKey(NkGuiKey::End, down);
				break;
			case NkKey::NK_BACK:
				ctx.input.SetKey(NkGuiKey::Backspace, down);
				break;
			case NkKey::NK_DELETE:
				ctx.input.SetKey(NkGuiKey::Delete, down);
				break;
			case NkKey::NK_ENTER:
				ctx.input.SetKey(NkGuiKey::Enter, down);
				break;
			case NkKey::NK_ESCAPE:
				ctx.input.SetKey(NkGuiKey::Escape, down);
				break;
			default:
				break;
		}
	...
	while (running && window.IsOpen()) {
		...
		static bool showWidgets = true, showStructure = true, showDragBox = true, showListBox = true, showMulti = true,
					showHScroll = true, showTable = true;
		{
		...
		if (showWidgets && BeginPanel(ctx, "Widgets", panel)) {
			Text(ctx, "Boutons :");
			if (Button(ctx, "Bouton normal"))
				++clicks[0];
			if (ctx.IsItemHovered())
				SetTooltip(ctx, "Incremente le compteur de clics (infobulle overlay)");
			ctx.SameLine();
			{
				const NkRect rb = ctx.NextItemRect(180.f, ctx.ItemHeight());
				if (RepeatButton(ctx, "Rafale (maintenir)", rb, 0.20f, 0.03f))
					++clicks[1];
			}
			char buf[64];
			...
			if (Button(ctx, "*Bouton stylise (hook)"))
				++clicks[0];
			...
			NkGuiCheck all = (onCount == 0) ? NkGuiCheck::Off : (onCount == 3) ? NkGuiCheck::On : NkGuiCheck::Mixed;
			...
			InputTextEx(ctx, "Code MAJ (max 6)", codeBuf, 32, NkGuiInputFlags::Uppercase | NkGuiInputFlags::NoBlank, 6);
			...
			if (BeginCombo(ctx, "Mode", comboOpts[comboSel], 4)) {
				for (int32 i = 0; i < 4; ++i)
					if (Selectable(ctx, comboOpts[i], i == comboSel)) {
						comboSel = i;
						ctx.ClosePopup();
					}
				EndCombo(ctx);
		...
		if (showStructure && BeginPanel(ctx, "Structure", panel2)) {
			...
			if (TreeNodeEditable(ctx, "root", rootName, 64)) {
				if (SelectableEditable(ctx, "lA", leafA, 64, selLeaf == 0))
					selLeaf = 0;
				if (TreeNodeEditable(ctx, "sub", subName, 64)) {
					if (SelectableEditable(ctx, "l1", leaf1, 64, selLeaf == 1))
						selLeaf = 1;
					// allowRename=false → cette feuille refuse le renommage (mais reste sélectionnable).
					if (SelectableEditable(ctx, "l2", leaf2, 64, selLeaf == 2, /*allowRename=*/false))
						selLeaf = 2;
					TreePop(ctx);
				}
			...
			if (tsel == 0)
				Text(ctx, "Contenu : Infos");
			else if (tsel == 1)
				Text(ctx, "Contenu : Style");
			else
				Text(ctx, "Contenu : Avance");
			...
			for (int32 i = 0; i < 6; ++i)
				SelectItemEditable(ctx, listIds[i], listItems[i], 64);
			ctx.EndSelectList();
			...
			static char nAssets[32] = "Assets", nTex[32] = "Textures", nWood[32] = "wood.png", nRock[32] = "rock.png",
						nSnd[32] = "Sounds", nBeep[32] = "beep.wav";
			...
			crow("c0", 0);
			if (TreeNodeEditable(ctx, "h0", nAssets, 32)) {
				crow("c1", 1);
				if (TreeNodeEditable(ctx, "h1", nTex, 32)) {
					crow("c2", 2);
					SelectableEditable(ctx, "h2", nWood, 32, false);
					crow("c3", 3);
					SelectableEditable(ctx, "h3", nRock, 32, false);
					TreePop(ctx);
				}
				crow("c4", 4);
				if (TreeNodeEditable(ctx, "h4", nSnd, 32)) {
					crow("c5", 5);
					SelectableEditable(ctx, "h5", nBeep, 32, false);
					TreePop(ctx);
				}
				TreePop(ctx);
			}
			...
			if (BeginTable(ctx, "files", 3,
				...
				if (TableGetSortColumn(ctx, &scol, &sasc)) {
					auto cmp = [&](int32 a, int32 b) -> int32 { // <0 si a avant b
						if (scol == 2)
							return (tSizes[a] < tSizes[b]) ? -1 : (tSizes[a] > tSizes[b] ? 1 : 0);
						const char *sa = (scol == 0) ? tNames[a] : tTypes[a];
						const char *sb = (scol == 0) ? tNames[b] : tTypes[b];
						int32 k = 0;
						while (sa[k] && sa[k] == sb[k])
							++k;
						return static_cast<int32>(static_cast<uint8>(sa[k])) -
							   static_cast<int32>(static_cast<uint8>(sb[k]));
					};
					for (int32 a = 1; a < 6; ++a) { // tri par insertion (stable, 0 stdlib)
						const int32 key = order[a];
						int32 j = a - 1;
						while (j >= 0) {
				...
				for (int32 r = 0; r < 6; ++r) {
					...
					TableNextColumn(ctx);
					TableCellText(ctx, nameIds[i], tNames[i], 32, /*editable=*/true);
					TableNextColumn(ctx);
					Text(ctx, tTypes[i]);
					TableNextColumn(ctx);
					char sz[24];
					std::snprintf(sz, sizeof(sz), "%d o", tSizes[i]);
					Text(ctx, sz);
				}
				EndTable(ctx);
			}
		...
		if (showListBox) {
			...
			if (BeginListBox(ctx, "lbDemo", lbR)) {
				for (int32 i = 0; i < 15; ++i) {
					char s[20];
					std::snprintf(s, sizeof(s), "Element %02d", i + 1);
					if (Selectable(ctx, s, lbSel == i))
						lbSel = i;
				}
				EndListBox(ctx);
			}
		...
		if (showMulti) {
			...
			static char mlBuf[512] = "Editeur multi-ligne.\nEntree = nouvelle ligne.\nFleches haut/bas "
		...
		if (showHScroll) {
			...
			if (BeginChild(ctx, "hsBox", hsR, true, /*horizontal=*/true)) {
				for (int32 i = 0; i < 7; ++i) {
					char b[18];
					std::snprintf(b, sizeof(b), "Colonne %d", i + 1);
					if (i > 0)
						ctx.SameLine();
					Button(ctx, b);
				}
		...
		for (int32 i = 0; i < extraCount; ++i)
			if (Begin(ctx, extraStore[i])) {
				Text(ctx, "Onglet cree via +.");
				EndWindow(ctx);
			}
		...
		if (Begin(ctx, "Inspecteur")) {
			...
			for (int32 i = 0; i < 8; ++i) { // contenu long -> scroll
				char ln[32];
				std::snprintf(ln, sizeof(ln), "Propriete %02d", i + 1);
				Text(ctx, ln);
			}
			EndWindow(ctx);
		}
		...
		if (Begin(ctx, "Calque B")) {
			...
			Checkbox(ctx, "Option", cb);
			EndWindow(ctx);
		}
		if (Begin(ctx, "Calque C")) {
			Text(ctx, "3e fenetre.");
			EndWindow(ctx);
		}
		...
		if (captureRequested) {
			...
			if (ok)
				logger.Info("Capture ecran -> {0}", path);
			else

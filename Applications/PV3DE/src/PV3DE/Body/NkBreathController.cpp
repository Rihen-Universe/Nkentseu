...
namespace nkentseu {
	namespace pv3de {
		...
		void NkBreathController::SetFromBreathingDifficulty(nk_float32 d) noexcept {
			d = NkClamp(d, 0.f, 1.f);
			if (d < 0.1f)
				SetPattern(NkBreathPattern::Normal);
			else if (d < 0.4f)
				SetPattern(NkBreathPattern::Dyspnea);
			else if (d < 0.7f) {
				SetPattern(NkBreathPattern::Dyspnea);
				mTarget.rate = 35.f;
			} else
				SetPattern(NkBreathPattern::Paradoxical);
		}
		...
		void NkBreathController::Update(nk_float32 dt) noexcept {
			...
			switch (mCurrent.pattern) {
				case NkBreathPattern::CheyneStokes:
					_UpdateCheyneStokes(dt);
					break;
				case NkBreathPattern::Biot:
					_UpdateBiot(dt);
					break;
				case NkBreathPattern::Paradoxical:
					_UpdateParadoxical(dt);
					break;
				default:
					_UpdateNormal(dt);
					break;
		...
		void NkBreathController::_UpdateCheyneStokes(nk_float32 dt) noexcept {
			...
			mCSEnvPhase += dt;
			if (mCSEnvPhase >= kCycleLen)
				mCSEnvPhase -= kCycleLen;
		...
		void NkBreathController::_Solve() noexcept {
			...
			nk_float32 wave =
				NkSin(mPhase) * 0.65f + NkSin(mPhase * 2.f + 0.3f) * 0.25f + NkSin(mPhase * 3.f + 0.7f) * 0.10f;

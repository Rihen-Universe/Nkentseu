	...
	namespace pv3de {
		...
		class NkBodyController {
			public:
				...
				void SetBreathParams(const NkBreathParams &p) {
					mBreathTarget = p;
				}
				...
				nk_float32 GetChestDisplacement() const {
					return mChestDisp;
				}

				// Déplacement abdominal normalisé
				nk_float32 GetAbdomenDisplacement() const {
					return mChestDisp * 0.6f;
				}

				// Amplitude tremblement membres (0–1)
				nk_float32 GetTremorAmplitude() const {
					return mTremorAmp;
				}

				// Offset de position tremblement (x, y, z) — à additionner au mesh
				NkVec3f GetTremorOffset() const {
					return mTremorOffset;
				}

				// Poids de blend de la pose courante (0=Neutral, 1=cible)
				nk_float32 GetPoseBlend() const {
					return mPoseBlend;
				}

				NkBodyPose GetCurrentPose() const {
					return mCurrentPose;
				}

				// Paramètres de respiration courants (interpolés)
				const NkBreathParams &GetBreathParams() const {
					return mBreath;
				}

				// Rotation tête par balancement (degrés)
				nk_float32 GetHeadSwayX() const {
					return mHeadSwayX;
				}

				nk_float32 GetHeadSwayZ() const {
					return mHeadSwayZ;
				}

	...
	namespace pv3de {
		...
		class NkBreathController {
			public:
				...
				// ── Sorties ───────────────────────────────────────────────────────
				nk_float32 GetChestDisplacement() const noexcept {
					return mChest;
				}

				nk_float32 GetAbdomenDisplacement() const noexcept {
					return mAbdomen;
				}

				nk_float32 GetNasalFlareFactor() const noexcept {
					return mNasalFlare;
				}

				nk_float32 GetAccessoryUsage() const noexcept {
					return mAccessory;
				}

				bool IsInApnea() const noexcept {
					return mInApnea;
				}

				const NkBreathParams &GetParams() const noexcept {
					return mCurrent;
				}

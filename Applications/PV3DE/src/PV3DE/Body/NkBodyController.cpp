...
namespace nkentseu {
	namespace pv3de {
		...
		void NkBodyController::UpdateTremor(nk_float32 dt) {
			...
			nk_float32 nx = NkSin(mTremorTime * 17.3f) * 0.5f + NkSin(mTremorTime * 31.7f) * 0.3f +
							NkSin(mTremorTime * 47.1f) * 0.2f;
			nk_float32 ny = NkSin(mTremorTime * 13.1f + 1.2f) * 0.5f + NkSin(mTremorTime * 27.9f + 0.8f) * 0.5f;
			nk_float32 nz = NkSin(mTremorTime * 11.7f + 2.1f) * 0.6f + NkSin(mTremorTime * 23.3f + 1.5f) * 0.4f;

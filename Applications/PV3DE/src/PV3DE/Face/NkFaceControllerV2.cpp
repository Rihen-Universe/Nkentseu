...
namespace nkentseu {
	namespace humanoid {
		...
		void NkFaceControllerV2::AddFlash(nk_uint8 auId, nk_float32 intensity, nk_float32 dur) noexcept {
		...
		void NkFaceControllerV2::UpdateFlashes(nk_float32 dt) noexcept {
			...
			for (nk_uint32 i = 0; i < mFlashCount; ++i) {
				...
				if (mFlashes[i].timer > 0.f) {
					...
					float env = (norm < 0.5f) ? norm * 2.f : (1.f - norm) * 2.f;

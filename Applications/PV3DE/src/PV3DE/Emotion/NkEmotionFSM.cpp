...
namespace nkentseu {
	namespace pv3de {
		...
		void NkEmotionFSM::Update(const NkClinicalState &clinical, nk_float32 dt) {
			...
			if (targetState != mCurrentState) {
				...
				for (const auto &t : mTransitions) {
					if (t.from == mCurrentState && t.to == targetState && targetIntensity >= t.minIntensity) {
		...
		void NkEmotionFSM::ComputeOutput() {
			nk_float32 blendT = (mBlendDuration > 0.f) ? NkClamp(mBlendTimer / mBlendDuration, 0.f, 1.f) : 1.f;

	...
	namespace humanoid {
		...
		void NkHumanoidBehavior::Init(const NkPersonality &p) noexcept {
			...
			logger.Infof("[NkHumanoidBehavior] Init — rôle {} extraversion={:.2f} N={:.2f}\n", (int)p.role,
						 p.traits.extraversion, p.traits.neuroticism);
		}
		...
		// =====================================================================
		void NkHumanoidBehavior::Update(nk_float32 dt, nk_float32 pain, nk_float32 anxiety,
		...
		// =====================================================================
		void NkHumanoidBehavior::EvaluateIntentions(nk_float32 pain, nk_float32 anxiety, nk_float32 fatigue) noexcept {
		...
		void NkHumanoidBehavior::GenerateMicroexpressions(nk_float32 timer) noexcept {
			...
			if (mResidualPain > 0.3f)
				micro.data = "pain_flash";
			else if (mResidualAnxiety > 0.3f)
				micro.data = "worry_flash";
			else
				micro.data = "neutral_twitch";
		...
		void NkHumanoidBehavior::FilterByPersonality() noexcept {
			...
			for (nk_usize i = 0; i < mIntentions.Size(); ++i) {
				...
				if (intent.isInvoluntary)
					continue;

				// Stoïcisme : atténue l'expression de la douleur
				if (intent.type == NkIntentType::Wince || intent.type == NkIntentType::ExpressEmotion) {
				...
				switch (intent.type) {
					...
					case NkIntentType::Microexpression: {
						...
						if (intent.data == "shock") {
							ft.auId = 1;
							ft.intensity = 0.8f * intent.intensity;
							mOutput.faceTargets.PushBack(ft);
							ft.auId = 5;
							ft.intensity = 0.7f * intent.intensity;
							mOutput.faceTargets.PushBack(ft);
							...
							ft.auId = 4;
							ft.intensity = 0.5f * intent.intensity;
							mOutput.faceTargets.PushBack(ft);
						} else if (intent.data == "relief") {
							ft.auId = 12;
							ft.intensity = 0.3f * intent.intensity;
							mOutput.faceTargets.PushBack(ft);
						}
						break;
					}
					...
					case NkIntentType::Cry:
						// Les pleurs modifient aussi le visage (AU1+4+17)
						{
							...
							mOutput.faceTargets.PushBack(ft);
							ft.auId = 17;
							ft.intensity = 0.6f * intent.intensity;
							mOutput.faceTargets.PushBack(ft);
						}
						break;
					default:

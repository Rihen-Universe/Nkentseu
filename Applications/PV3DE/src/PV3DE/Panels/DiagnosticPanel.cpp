...
namespace nkentseu {
	namespace pv3de {
		...
		void DiagnosticPanel::Render(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDrawList &dl, NkUIFont &font,
			...
			if (!NkUIWindow::Begin(ctx, wm, dl, font, ls, "Diagnostic différentiel##diag", nullptr,
			...
			if (ranking.IsEmpty()) {
				NkUI::BeginRow(ctx, ls, 20.f);
				NkUI::Text(ctx, ls, dl, font, "Aucun symptôme sélectionné", {140, 140, 140, 255});
				NkUI::EndRow(ctx, ls);
				NkUIWindow::End(ctx, wm, dl, ls);
				return;
			}
			...
			if (NkUI::BeginScrollRegion(ctx, ls, "diag_scroll", scrollRect, nullptr, &scrollY)) {
				for (nk_usize i = 0; i < maxShow; ++i) {
					...
					NkUI::TextColored(ctx, ls, dl, font, ProbabilityColor(entry.probability), entry.diseaseName.CStr());
					NkUI::EndRow(ctx, ls);
					...
					NkUI::Text(ctx, ls, dl, font, "Sévérité:", {140, 140, 140, 255});
					char sevBuf[32];
					...
					NkUI::TextColored(ctx, ls, dl, font, SeverityColor(entry.severity), sevBuf);
					NkUI::EndRow(ctx, ls);

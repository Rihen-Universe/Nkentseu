...
namespace nkentseu {
	namespace pv3de {
		...
		void ReportPanel::Render(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDrawList &dl, NkUIFont &font,
			...
			if (!NkUIWindow::Begin(ctx, wm, dl, font, ls, "Rapport clinique##report", nullptr,
			...
			NkUI::Text(ctx, ls, dl, font, "Rapport clinique", {220, 220, 100, 255});
			NkUI::EndRow(ctx, ls);
		...
		// =====================================================================
		void ReportPanel::RenderPatientInfo(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font,
		...
		// =====================================================================
		void ReportPanel::RenderSummary(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkUILayoutStack &ls,
		...
		// =====================================================================
		void ReportPanel::RenderExportButtons(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkUILayoutStack &ls,
			...
			if (NkUI::Button(ctx, ls, dl, font, "Export FHIR JSON")) {
				...
				char fname[64];
				snprintf(fname, sizeof(fname), "report_%s_%s_%ld.fhir.json", mLastName, mFirstName, (long)now);
			...
			if (NkUI::Button(ctx, ls, dl, font, "Export PDF")) {
				time_t now = time(nullptr);
				char fname[64];
				snprintf(fname, sizeof(fname), "report_%s_%s_%ld.pdf", mLastName, mFirstName, (long)now);
		...
		bool ReportPanel::ExportPDF(const PatientLayer &patient, const char *path) noexcept {

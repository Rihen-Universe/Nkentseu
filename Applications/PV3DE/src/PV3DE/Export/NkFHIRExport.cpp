...
namespace nkentseu {
	namespace pv3de {
		...
		NkString NkFHIRExport::BuildObservations(const NkClinicalState &s) const {
			...
			auto AddObs = [&](const char *code, const char *display, const char *unit, nk_float32 val) {
				char buf[512];
		...
		NkString NkFHIRExport::BuildDiagnosticReport(const NkClinicalState &s) const {
			...
			if (!s.differentialRanking.IsEmpty()) {
				...
				char buf[256];
				snprintf(buf, sizeof(buf), "Hypothèse principale: %s (%.0f%%). ", top.diseaseName.CStr(),
						 (double)(top.probability * 100.f));
				r += EscapeJson(NkString(buf));
			}

			char pain[64];
			snprintf(pain, sizeof(pain), "Douleur EVA: %.1f/10. FC: %.0f bpm. SpO2: %.0f%%.", (double)s.painLevel,
					 (double)s.heartRate, (double)s.spo2);
		...
		NkString NkFHIRExport::GenerateSummary(const NkClinicalState &s) const {
			...
			snprintf(vitals, sizeof(vitals),
					 ...
					 "  Douleur (EVA): %.1f/10\n\n",
					 (double)s.heartRate, (double)s.temperature, (double)s.spo2, (double)s.painLevel);
			out += NkString(vitals);

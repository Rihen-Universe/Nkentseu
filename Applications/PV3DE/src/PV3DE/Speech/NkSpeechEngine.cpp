...
namespace nkentseu {
	namespace pv3de {
		...
		const NkVisemeMapper::PhonemeRule NkVisemeMapper::kRules[] = {
			{"ou", NkViseme::OO}, {"oi", NkViseme::OO}, {"au", NkViseme::AO},		 {"eau", NkViseme::OH},
			{"ch", NkViseme::SH}, {"ph", NkViseme::FF}, {"th", NkViseme::TH},		 {"gn", NkViseme::NN},
			{"eu", NkViseme::UH}, {"ai", NkViseme::AE}, {nullptr, NkViseme::Silence}};
		...
		NkVector<NkVisemeFrame> NkVisemeMapper::Map(const NkString &text, nk_float32 durationMs) {
			...
			for (nk_usize i = 0; i < len;) {
				...
				for (nk_uint32 r = 0; kRules[r].grapheme; ++r) {
					...
					if (i + rlen <= len && strncasecmp(s + i, kRules[r].grapheme, rlen) == 0) {
						NkVisemeFrame f;
						f.timeMs = t;
		...
		void NkLipSyncController::Play(const NkVector<NkVisemeFrame> &frames, nk_float32 startMs) {
			mFrames = frames;
			mCurrentMs = startMs;
			mFrameIdx = 0;
		...
		void NkLipSyncController::Update(nk_float32 dt, NkFaceController &fc) {
			...
			while (mFrameIdx + 1 < mFrames.Size() && mCurrentMs >= mFrames[mFrameIdx + 1].timeMs) {
				++mFrameIdx;
		...
		void NkLipSyncController::ApplyViseme(NkViseme v, nk_float32 intensity, NkFaceController &fc) {
			// Reset AUs bouche
		...
		NkString NkSpeechEngine::GetResponse(const NkString &question, nk_float32 pain, nk_float32 breath) {

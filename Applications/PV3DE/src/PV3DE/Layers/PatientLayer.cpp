...
namespace nkentseu {
	namespace pv3de {
		...
		void PatientLayer::OnUpdate(nk_float32 dt) {
			...
			switch (em.state) {
				case EmotionState::PainSevere:
					mFaceController.ApplyPreset(kAU_PainSevere, em.intensity);
					break;
				case EmotionState::PainMild:
				case EmotionState::Discomfort:
					mFaceController.ApplyPreset(kAU_PainMild, em.intensity);
					break;
				case EmotionState::Anxious:
				case EmotionState::Panic:
					mFaceController.ApplyPreset(kAU_Anxious, em.intensity);
					break;
				case EmotionState::Nauseous:
					mFaceController.ApplyPreset(kAU_Nauseous, em.intensity);
					break;
				case EmotionState::Exhausted:
					mFaceController.ApplyPreset(kAU_Exhausted, em.intensity);
					break;
				default:
					mFaceController.ApplyNeutral();
					break;
			}
		...
		bool PatientLayer::OnEvent(nkentseu::NkEvent * /*e*/) {
			return false;
		}
		...
		void PatientLayer::AnswerQuestion(const char *question) noexcept {
			if (!mCaseLoaded) {
				...
				NkString answer = NkSpeechEngine::GetResponse(NkString(question), mClinicalState.painLevel,
			...
			if (qa)
				Speak(qa->answer.CStr());
			else
				Speak("Je... je ne sais pas.");
		}

		// =====================================================================
		bool PatientLayer::LoadCase(const char *path) noexcept {
			if (!mCaseLoader.Load(path, mCurrentCase))
				return false;
			mCaseLoaded = true;
		...
		void PatientLayer::SetCamera(const NkMat4f &vp, const NkMat4f &m, const NkVec3f &pos) noexcept {
			mViewProj = vp;
			mModel = m;
			mCamPos = pos;
		}
		...
		void PatientLayer::DispatchCaseEvent(const NkCaseEvent &ev) noexcept {
			switch (ev.type) {
				...
				case NkCaseEventType::ForceEmotion: {
					...
					if (name == "PainMild")
						state = EmotionState::PainMild;
					if (name == "PainSevere")
						state = EmotionState::PainSevere;
					if (name == "Anxious")
						state = EmotionState::Anxious;
					if (name == "Panic")
						state = EmotionState::Panic;
					if (name == "Nauseous")
						state = EmotionState::Nauseous;
					if (name == "Exhausted")
				...
				case NkCaseEventType::SetBreathPattern: {
					...
					if (ev.stringValue == "Dyspnea")
						pat = NkBreathPattern::Dyspnea;
					if (ev.stringValue == "CheyneStokes")
						pat = NkBreathPattern::CheyneStokes;
					if (ev.stringValue == "Kussmaul")
						pat = NkBreathPattern::Kussmaul;

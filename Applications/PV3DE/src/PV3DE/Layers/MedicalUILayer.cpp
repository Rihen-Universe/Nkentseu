...
namespace nkentseu {
	namespace pv3de {
		...
		void MedicalUILayer::OnUIRender() {
			...
			mDiagPanel.Render(mCtx, mWM, mDL, font, mLS, *mPatient, mLayout.diagnostic);

			mStatePanel.Render(mCtx, mWM, mDL, font, mLS, *mPatient, mLayout.state);

			mReportPanel.Render(mCtx, mWM, mDL, font, mLS, *mPatient, mLayout.report);
		...
		void MedicalUILayer::RenderMenuBar() noexcept {
			...
			if (NkUIMenu::BeginMenu(mCtx, mDL, font, "Patient")) {
				if (NkUIMenu::MenuItem(mCtx, mDL, font, "Neutre", "F1")) {
					if (mPatient)
		...
		void MedicalUILayer::RenderViewport() noexcept {
			...
			if (!NkUIWindow::Begin(mCtx, mWM, mDL, font, mLS, "PatientView##pv", nullptr,
								   NkUIWindowFlags::NK_NO_TITLE_BAR | NkUIWindowFlags::NK_NO_RESIZE |
									   NkUIWindowFlags::NK_NO_MOVE | NkUIWindowFlags::NK_NO_BACKGROUND)) {
				NkUIWindow::End(mCtx, mWM, mDL, mLS);
				return;
			}

			if (mPatientFBO.IsValid()) {
				mDL.AddImage((nk_uint32)mPatientFBO.id, {r.x, r.y, r.w, r.h}, {0, 0}, {1, 1}, NkColor::White);
			} else {
				// Placeholder
				mDL.AddRectFilled({r.x, r.y, r.w, r.h}, {20, 20, 30, 255});
				mDL.AddText({r.x + r.w * 0.5f - 100.f, r.y + r.h * 0.5f}, "Patient 3D — en attente du rendu GPU",
							{80, 80, 80, 255});
			}
			...
			if (mPatient) {
				...
				mDL.AddText({r.x + r.w - 120.f, r.y + 8.f}, breathBuf, {180, 220, 180, 200});
			}

...
namespace nkentseu {
	namespace pv3de {
		...
		void PatientLayerV3::OnAttach() {
			...
			mAIDriver.SetResponseCallback([this](const NkString &text, nk_float32 emotionIntensity) {
		...
		void PatientLayerV3::OnUpdate(nk_float32 dt) {
			...
			mAIDriver.Update(dt, mClinicalState.painLevel, mClinicalState.anxietyLevel, mClinicalState.fatigueLevel);
			...
			switch (em.state) {
				case EmotionState::PainSevere:
					mFaceController.ApplyPreset(kAU_PainSevere, em.intensity * bgScale);
					break;
				case EmotionState::PainMild:
					mFaceController.ApplyPreset(kAU_PainMild, em.intensity * bgScale);
					break;
				case EmotionState::Anxious:
				case EmotionState::Panic:
					mFaceController.ApplyPreset(kAU_Anxious, em.intensity * bgScale);
					break;
				case EmotionState::Nauseous:
					mFaceController.ApplyPreset(kAU_Nauseous, em.intensity * bgScale);
					break;
				case EmotionState::Exhausted:
					mFaceController.ApplyPreset(kAU_Exhausted, em.intensity * bgScale);
					break;
				default:
					break;
			}
			...
			mFaceController.SetGazeDirection(behOutput.gazeYaw,
		...
		bool PatientLayerV3::OnEvent(nkentseu::NkEvent * /*e*/) {
			return false;
		}

		// =====================================================================
		void PatientLayerV3::AskQuestion(const char *question, NkAIResponseCallback callback) noexcept {
			if (!question)
				return;

			const auto &cs = mClinicalState;
			mAIDriver.AskQuestion(question, cs.painLevel, cs.anxietyLevel, cs.fatigueLevel, cs.breathingDifficulty,
								  cs.heartRate, cs.temperature, cs.spo2, callback);
		}

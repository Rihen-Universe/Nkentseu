	...
	namespace pv3de {
		...
		class NkCaseLoader {
			public:
				...
				void ScanDirectory(const char *dir, NkVector<NkString> &outPaths) const noexcept;
				...
				void GetPendingEvents(const NkCaseData &caseData, nk_float32 prevTimeS, nk_float32 currTimeS,
									  NkVector<const NkCaseEvent *> &out) const noexcept;

				// ── Dialogue ──────────────────────────────────────────────────────
				// Cherche une réponse aux mots-clés dans la question
				const NkQAPair *FindAnswer(const NkCaseData &caseData, const char *question) const noexcept;
		...
		class NkCaseRunner {
			public:
				...
				nk_float32 GetElapsedSeconds() const noexcept {
					return mElapsed;
				}

				// Retourne les événements déclenchés ce tick
				const NkVector<const NkCaseEvent *> &Update(nk_float32 dt, const NkCaseLoader &loader) noexcept;

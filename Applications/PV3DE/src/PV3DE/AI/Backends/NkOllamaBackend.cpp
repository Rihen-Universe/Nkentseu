...
namespace nkentseu {
	namespace humanoid {
		...
		// =====================================================================
		bool NkOllamaBackend::Complete(const NkVector<NkConvMessage> &messages, nk_float32 temperature,
		...
		NkString NkOllamaBackend::BuildRequestJSON(const NkVector<NkConvMessage> &msgs, nk_float32 temp,
			...
			for (nk_usize i = 0; i < msgs.Size(); ++i) {
				...
				json += "{\"role\":\"";
				switch (msgs[i].role) {
					case NkConvRole::System:
						json += "system";
						break;
					case NkConvRole::User:
						json += "user";
						break;
					case NkConvRole::Assistant:
						json += "assistant";
						break;
				}
				json += "\",\"content\":\"";
			...
			// Options
			char optBuf[128];
			snprintf(optBuf, sizeof(optBuf), "\"options\":{\"temperature\":%.2f,\"num_predict\":%u}", (double)temp,
					 maxTok);
			json += optBuf;
			json += "}";
		...
		bool NkOllamaBackend::ParseResponse(const NkString &json, NkConvResponse &out) const noexcept {
		...
		bool NkOllamaBackend::HttpPost(const char *url, const NkString &body, NkString &responseOut) const noexcept {

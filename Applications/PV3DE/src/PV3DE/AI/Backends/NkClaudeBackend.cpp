...
namespace nkentseu {
	namespace humanoid {
		...
		bool NkClaudeBackend::Init(const char * /*endpoint*/, const char *modelName, const char *apiKey) noexcept {
			if (modelName)
				mModel = NkString(modelName);
		...
		NkString NkClaudeBackend::BuildRequestJSON(const NkVector<NkConvMessage> &msgs, nk_float32 temp,
		...
		bool NkClaudeBackend::Complete(const NkVector<NkConvMessage> &messages, nk_float32 temperature,
									   nk_uint32 maxTokens, NkConvResponse &out) noexcept {
			if (!mApiKeySet) {
		...
		bool NkClaudeBackend::ParseResponse(const NkString &json, NkConvResponse &out) const noexcept {
			// Réponse Claude : {"content":[{"type":"text","text":"..."}],...}
			const char *textKey = "\"text\":\"";
			const char *found = strstr(json.CStr(), textKey);
			if (!found) {
				out.error = "No text in response";
				return false;
			}
		...
		bool NkClaudeBackend::HttpsPost(const NkString &body, NkString &out) const noexcept {
#if defined(NK_HAVE_CURL)
			...
			curl_easy_setopt(
				curl, CURLOPT_WRITEFUNCTION, +[](char *data, size_t, size_t nmemb, void *userdata) -> size_t {

...
namespace nkentseu {
	namespace humanoid {
		...
		bool NkConversationEngine::Init(NkConvBackendType type, const char *modelName, const char *endpoint,
		...
		NkConvResponse NkConversationEngine::Ask(const NkConvRequest &req) noexcept {
			...
			nk_usize histStart = mHistory.Size() > kMaxHistory ? mHistory.Size() - kMaxHistory : 0;
		...
		// =====================================================================
		NkString NkConversationEngine::BuildSystemPrompt(const NkConvRequest &req) const noexcept {
			NkString prompt;
			...
			if (req.personality) {
				...
				snprintf(buf, sizeof(buf),
						 ...
						 "Nom: %s, %u ans (%s), %s.\n"
						 "Situation: %s\n\n",
						 life.name.CStr(), life.ageYears, life.gender.CStr(), life.profession.CStr(),
						 life.currentSituation.CStr());
				prompt += buf;
				...
				// Confiance
				snprintf(buf, sizeof(buf), "- Ton niveau de confiance envers ce médecin: %.0f%% (%s).\n",
			...
			snprintf(stateBuf, sizeof(stateBuf),
					 ...
					 : req.painLevel >= 1.f ? "légère"
											: "absente",
					 req.anxietyLevel * 100.f,
					 req.anxietyLevel > 0.7f   ? "très anxieux"
					 : req.anxietyLevel > 0.3f ? "un peu anxieux"
											   : "calme",
					 req.fatigueLevel * 100.f, req.breathingDifficulty * 100.f, req.heartRate, req.temperature,
					 req.spo2);
			prompt += stateBuf;

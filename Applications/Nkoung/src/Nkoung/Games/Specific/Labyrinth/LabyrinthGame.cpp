...
namespace nkoung {
	...
	void LabyrinthGame::Move(int32 dx, int32 dy) noexcept {
		...
		mPY = ny;
		++mSteps;
		if (mPX == mExitX && mPY == mExitY)
			mWon = true;
	}

	void LabyrinthGame::NextLevel() noexcept {
		LoadLevel((mLevelIndex + 1) % kMazeCount);
	}

	void LabyrinthGame::Update(float32 dt) noexcept {
		mAnim += dt;
	}
	...
	void LabyrinthGame::Render(const NkoungFrame &frame) noexcept {
		...
		if (mW <= 0 || mH <= 0 || areaW <= 0.f || areaH <= 0.f)
			return;
		...
		char buf[128];
		snprintf(buf, sizeof(buf), "Gardien du Labyrinthe  -  Niveau %d : %s", mLevelIndex + 1,
				 kMazes[mLevelIndex].name);
		frame.Text(frame.font, SX + 16.f, SY + 10.f, buf, C::CYAN_BRIGHT());
		frame.Text(frame.font, SX + 16.f, SY + 38.f, "Atteins le portail vert. Fleches/WASD ou les boutons.",
				   C::TEXT_TERTIARY());
		...
		if (frame.Button(SX + 12.f, fy, 84.f, bh, "Menu", bg, bgh, fg))
			mWantExit = true;

		const float32 dpx = SX + 110.f, db = bh;
		if (frame.Button(dpx, fy, db, bh, "<", bg, bgh, fg))
			Move(-1, 0);
		if (frame.Button(dpx + db + 6.f, fy, db, bh, "^", bg, bgh, fg))
			Move(0, -1);
		if (frame.Button(dpx + 2 * (db + 6.f), fy, db, bh, "v", bg, bgh, fg))
			Move(0, 1);
		if (frame.Button(dpx + 3 * (db + 6.f), fy, db, bh, ">", bg, bgh, fg))
			Move(1, 0);
	...
	const char *LabyrinthGame::GetCurrentLevelTitle() const noexcept {
		return kMazes[mLevelIndex].name;
	}

...
namespace nkoung {
	...
	enum class LaserTile : nkentseu::nk_uint8 { Empty = 0, Source, Mirror, Target, Wall };
	...
	class LaserPuzzleGame : public NkoungGame {
		...
		private:
			...
			LaserCell &At(nkentseu::int32 x, nkentseu::int32 y) noexcept {
				return mGrid[y * mGridW + x];
			}

			const LaserCell &At(nkentseu::int32 x, nkentseu::int32 y) const noexcept {
				return mGrid[y * mGridW + x];
			}

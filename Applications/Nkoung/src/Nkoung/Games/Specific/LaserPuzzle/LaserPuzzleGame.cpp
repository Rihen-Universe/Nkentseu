...
namespace nkoung {
	...
	bool LaserPuzzleGame::LoadLevel(int32 index) noexcept {
		if (index < 0 || index >= kLevelCount)
			index = 0; // boucle sur les niveaux
		mLevelIndex = index;
			...
			for (int32 x = 0; x < mGridW; ++x) {
				...
				switch (CharAt(row, x)) {
					...
					case '/':
						...
						break;
					case 'T':
						c.type = LaserTile::Target;
						++mTargetsTotal;
						break;
					case '#':
						c.type = LaserTile::Wall;
						break;
					default:
						c.type = LaserTile::Empty;
						break;
				}
	...
	void LaserPuzzleGame::Simulate() noexcept {
		...
		for (usize i = 0; i < mGrid.Size(); ++i)
			if (mGrid[i].type == LaserTile::Target)
				mGrid[i].hit = false;
		...
		for (int32 y = 0; y < mGridH; ++y)
			for (int32 x = 0; x < mGridW; ++x)
				if (At(x, y).type == LaserTile::Source) {
					sx = x;
					sy = y;
					sdir = At(x, y).dir;
				}
		...
		while (guard-- > 0) {
			x += dx;
			y += dy;
			if (x < 0 || x >= mGridW || y < 0 || y >= mGridH)
				break;
			LaserCell &c = At(x, y);
			if (c.type == LaserTile::Wall)
				break;
			mRay.PushBack(math::NkVec2f{static_cast<float32>(x), static_cast<float32>(y)});
	...
	void LaserPuzzleGame::RotateMirror(int32 idx) noexcept {
		if (idx < 0 || idx >= static_cast<int32>(mGrid.Size()))
			return;
		if (mGrid[idx].type != LaserTile::Mirror)
			return;
		mGrid[idx].orient ^= 1;
		++mMoves;
		Simulate();
	...
	void LaserPuzzleGame::NextLevel() noexcept {
		LoadLevel((mLevelIndex + 1) % kLevelCount);
	}

	void LaserPuzzleGame::Update(float32 dt) noexcept {
		mAnim += dt;
	}
	...
	void LaserPuzzleGame::Render(const NkoungFrame &frame) noexcept {
		...
		if (mGridW <= 0 || mGridH <= 0 || areaW <= 0.f || areaH <= 0.f)
			return;
		...
		frame.Text(frame.font, SX + 16.f, SY + 38.f, "Oriente les miroirs pour guider le rayon vers la cible doree.",
				   C::TEXT_TERTIARY());
				...
				switch (c.type) {
					case LaserTile::Source:
						...
						frame.Line(math::NkVec2f{cx, cy},
								   math::NkVec2f{cx + DX[c.dir] * rr * 1.4f, cy + DY[c.dir] * rr * 1.4f}, C::BG_DARK(),
								   cell * 0.06f);
						break;
					case LaserTile::Target:
						if (c.hit)
							...
							frame.CircleOutline(math::NkVec2f{cx, cy}, rr, math::NkColor{255, 215, 0, 255}, 3.f, 0);
						break;
					case LaserTile::Mirror: {
						...
						const math::NkColor mc =
							(idx == mSelected) ? C::CYAN_BRIGHT() : math::NkColor{185, 205, 235, 255};
		...
		if (mRay.Size() >= 2 && cell > 0.f) {
			float32 t = mAnim * 1.5f;
			t -= static_cast<float32>(static_cast<int32>(t));
		...
		if (frame.Button(SX + 16.f, fy, 110.f, bh, "Menu", C::BG_TERTIARY(), C::CYAN_MID(), C::TEXT_PRIMARY()))
			mWantExit = true;
		if (frame.Button(SX + 16.f + 122.f, fy, 150.f, bh, "Recommencer", C::BG_TERTIARY(), C::CYAN_MID(),
						 C::TEXT_PRIMARY()))
			LoadLevel(mLevelIndex);
		if (mWon) {
			const math::NkColor brd = C::GREEN_SUCCESS();
			if (frame.Button(SX + SW - 16.f - 170.f, fy, 170.f, bh, "Niveau suivant >", C::GREEN_SUCCESS(),
							 C::GRADIENT_GREEN_END(), C::BG_DARK(), &brd))
				NextLevel();
		}
	}

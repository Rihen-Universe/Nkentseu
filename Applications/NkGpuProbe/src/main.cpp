...
static bool TryApi(NkGraphicsApi api) {
	...
	const bool ok =
		(fabs(c[0] - 58) < 0.5 && fabs(c[1] - 64) < 0.5 && fabs(c[2] - 139) < 0.5 && fabs(c[3] - 154) < 0.5);
...
int main() {
	...
	bool ok = false;
	if (TryApi(NkGraphicsApi::NK_GFX_API_DX11)) {
		printf("\n>>> DX11 compute headless : FONCTIONNEL\n");
		ok = true;
	} else if (TryApi(NkGraphicsApi::NK_GFX_API_DX12)) {

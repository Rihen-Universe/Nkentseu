	...
	namespace game {
		namespace ui {
			...
			NkGemHudButtons NkGemHud::DrawOverlay(NkGuiDrawList &dl, const NkGemFonts &fonts, const NkGemLayout &layout,
				...
				if (!state.paused) {
					// ⚠️ LE PANNEAU DESSINE EXACTEMENT CE QUE LE COMPTEUR MONTRAIT.
					//
					// Defaut mesure a l ecran le 2026-08-28 : le compteur en jeu
					// affichait une etoile a MOITIE pleine, et le panneau de fin la
					// dessinait PLEINE. Les deux comptaient pourtant pareil (2) —
					// c est le DESSIN qui embellissait. Le joueur, lui, voit deux
					// images differentes du meme resultat.
					//
					// On reutilise donc la meme fonction de remplissage partiel que
					// le HUD, avec la meme part consommee. Ce qu il a vu fondre est
					// ce qu on lui montre.
					const float32 spentAtEnd = NkGemSpentFraction(state);
					const int32 earned = NkStarsFromSpend(state.victory, spentAtEnd);
					...
					for (int32 i = 0; i < 3; ++i) {
						...
						// Perdu : toutes eteintes. Gagne : le remplissage reel.
						if (state.victory) {
							ProgressStarPartial(dl, NkVec2f(c.x, c.y + lift), starR, NkStarFill(2 - i, spentAtEnd), th);
						} else {
							ProgressStar(dl, NkVec2f(c.x, c.y + lift), starR, false, th);
						}
					}
					y += starR * 2.4f;

...
namespace nkentseu {
	namespace game {
		...
		namespace {
			...
			/// Un dossier n'est inscriptible que si on y a REELLEMENT ecrit.
			///
			/// ⚠️ C'est tout le point de cette fonction : `GetAppDataDirectory()`
			/// rend un chemin PLAUSIBLE sur toutes les plateformes, et un chemin
			/// NON INSCRIPTIBLE sur Android. Les deux se ressemblent — seule
			/// l'ecriture les distingue.
			bool DossierInscriptible(const NkPath &dir) {
				if (dir.ToString().Empty()) {
					return false;
				}
				NkDirectory::CreateRecursive(dir);
				const NkPath temoin = dir / ".gemcrush_ecriture";
				if (!NkFile::WriteAllText(temoin, "1")) {
					return false;
				}
				const bool relu = NkFile::ReadAllText(temoin).Size() > 0;
				NkFile::Delete(temoin);
				return relu;
			}

			/// Dossier de sauvegarde, RESOLU UNE FOIS et memorise.
			///
			/// La liste va du plus specifique au plus general. Sur Android, le
			/// dossier interne de l'application est le seul endroit ou elle a le
			/// droit d'ecrire sans permission ; son chemin se deduit de
			/// l'identifiant declare dans GemCrush.jenga (androidapplicationid).
			/// ⚠️ Si cet identifiant change la-bas, il doit changer ici.
			const NkPath &DossierSauvegarde() {
				static NkPath resolu;
				static bool cherche = false;
				if (cherche) {
					return resolu;
				}
				cherche = true;
				...
				candidats[n++] = NkPath("/data/data/com.nkentseu.gemcrush/files");
				candidats[n++] = NkPath("/data/user/0/com.nkentseu.gemcrush/files");
#elif defined(NKENTSEU_PLATFORM_HARMONYOS)
				candidats[n++] = NkPath("/data/storage/el2/base/haps/entry/files");
				candidats[n++] = NkPath("/data/storage/el2/base/files");
#endif
				candidats[n++] = NkDirectory::GetAppDataDirectory() / "Rihen" / "GemCrush";
				candidats[n++] = NkDirectory::GetHomeDirectory() / ".rihen" / "gemcrush";
				candidats[n++] = NkDirectory::GetCurrentDirectory() / "sauvegarde";

				for (int32 i = 0; i < n; ++i) {
					if (DossierInscriptible(candidats[i])) {
						resolu = candidats[i];
						logger.Info("[gemcrush] progression ecrite dans : {0}", resolu.ToString().CStr());
						return resolu;
					}
				}
				// Aucun candidat n'accepte l'ecriture : on le DIT. Le jeu tourne,
				// mais le joueur doit savoir que rien ne sera conserve.
				logger.Error("[gemcrush] AUCUN dossier inscriptible trouve ({0} essayes) : la progression NE SERA PAS conservee", n);
				return resolu;
			}

			NkPath SaveFilePath() {
				return DossierSauvegarde() / "progression.txt";
			}
		...
		bool NkGemProgress::Save() const {
			if (DossierSauvegarde().ToString().Empty()) {
				return false; // deja journalise a la resolution ; on ne repete pas a chaque partie
			}
			const NkPath path = SaveFilePath();

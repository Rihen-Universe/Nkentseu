...
static void DrawScene3D(NkGuiContext &ctx, const NkRect &area, float32 yaw, float32 pitch, float32 dist) {
	...
	auto norm3 = [](float32 &a, float32 &b, float32 &c) {
		const float32 l = mth::NkSqrt(a * a + b * b + c * c);
		if (l > 1e-6f) {
			a /= l;
			b /= l;
			c /= l;
	...
	float32 fx = tx - ex, fy = ty - ey, fz = tz - ez;
	norm3(fx, fy, fz); // forward
	float32 rx = fz, ry = 0.f, rz = -fx;
	norm3(rx, ry, rz);																	  // right = up(0,1,0) x fwd
	...
	auto drawLine3D = [&](float32 ax, float32 ay, float32 az, float32 bx, float32 by, float32 bz, const NkColor &col,
						  float32 th) {
		float32 avx, avy, avz, bvx, bvy, bvz;
		viewOf(ax, ay, az, avx, avy, avz);
		viewOf(bx, by, bz, bvx, bvy, bvz);
		if (avz <= kNear && bvz <= kNear)
			return; // entierement derriere
		if (avz <= kNear) {
			const float32 t = (kNear - avz) / (bvz - avz);
			avx += (bvx - avx) * t;
			avy += (bvy - avy) * t;
			avz = kNear;
		} else if (bvz <= kNear) {
			const float32 t = (kNear - bvz) / (avz - bvz);
			bvx += (avx - bvx) * t;
			bvy += (avy - bvy) * t;
			bvz = kNear;
		}
		NkVec2 pa, pb;
		projVS(avx, avy, avz, pa);
		projVS(bvx, bvy, bvz, pb);
	...
	const float32 V[8][3] = {{-1, 0, -1}, {1, 0, -1}, {1, 2, -1}, {-1, 2, -1},
	...
	{
		const float32 il = 1.f / mth::NkSqrt(L[0] * L[0] + L[1] * L[1] + L[2] * L[2]);
		L[0] *= il;
		L[1] *= il;
		L[2] *= il;
	}
	...
	for (int32 f = 0; f < 6; ++f) {
		...
		if (N[f][0] * vex + N[f][1] * vey + N[f][2] * vez <= 0.f)
			continue; // back-face cull
		float32 nd = N[f][0] * L[0] + N[f][1] * L[1] + N[f][2] * L[2];
		if (nd < 0.f)
			nd = 0.f;
		...
		bool ok = true;
		for (int32 k = 0; k < 4; ++k)
			if (!project(V[F[f][k]][0], V[F[f][k]][1], V[F[f][k]][2], p[k])) {
				ok = false;
				break;
			}
		if (!ok)
		...
		for (int32 k = 0; k < 4; ++k)
			dl.AddLine(p[k], p[(k + 1) % 4], NkColor{36, 40, 56, 255}, 1.f);
	}
...
int nkmain(const NkEntryState &state) {
	...
	while (running && window.IsOpen()) {
		...
		if (ctx.input.mouseDown[0]) {
			if (dragging) {
				yaw += (mp.x - prevMouse.x) * 0.01f;
				pitch -= (mp.y - prevMouse.y) * 0.01f;
			}
			dragging = true;
		} else
			dragging = false;
		prevMouse = mp;
		const float32 lim = 1.5f;
		if (pitch > lim)
			pitch = lim;
		if (pitch < -lim)
			pitch = -lim;

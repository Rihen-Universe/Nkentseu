...
static NkTensor RandMat(uint32 r, uint32 c, uint32& s) {
    ...
    for (int64 i = 0; i < (int64)r * c; ++i) { s = s*1664525u+1013904223u; p[i] = (float)((s>>9)&0x3FF)/1024.f - 0.5f; }
    return t;
}
static double MaxAbsDiff(const NkTensor& a, const NkTensor& b) {
    NkTensor ac = a.Contiguous(), bc = b.Contiguous();
    const float* ap=ac.DataAs<float>(); const float* bp=bc.DataAs<float>();
    int64 n = NkShapeNumel(ac.Shape()); double m=0;
    for (int64 i=0;i<n;++i){ double d=std::fabs((double)ap[i]-(double)bp[i]); if(d>m)m=d; }
    return m;
}
...
int main() {
    ...
    { uint32 s0=1u; NkTensor probe = RandMat(4,4,s0).ToGPU();
    ...
    for (uint32 si=0; si<4; ++si) {
        ...
        double gpuMs = -1.0; double err = -1.0;

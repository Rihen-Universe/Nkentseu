...
static NkTensor RandT(const NkShape &s, uint32 &seed) {
	...
	int64 n = NkShapeNumel(s);
	for (int64 i = 0; i < n; ++i) {
		seed = seed * 1664525u + 1013904223u;
		p[i] = (float)((seed >> 9) & 0x3FF) / 1024.f - 0.5f;
	}
	return t;
}
					...
					for (int64 ic = 0; ic < Cin; ++ic)
						for (int64 ky = 0; ky < kH; ++ky)
							for (int64 kx = 0; kx < kW; ++kx) {
								int64 iy = oy - pad + ky, ix = ox - pad + kx;
								if (iy < 0 || iy >= H || ix < 0 || ix >= W)
									continue;
								s += (double)xp[((b * Cin + ic) * H + iy) * W + ix] *
									 (double)wp[((oc * Cin + ic) * kH + ky) * kW + kx];
							}
...
int main() {
	...
	for (int r = 0; r < R; ++r)
		gpu = autograd::Conv2D(NkVar::Leaf(x, false), NkVar::Leaf(w, false), 1, 1).Value();

...
namespace nkentseu {
	namespace nkcode {
		...
		struct NkCloneGitState {
				...
				static NkString HistFile() {
					const NkString home = NkOpenWsState::Home();
					return (NkPath(home.CStr()) / ".nkcode" / "clone_history.txt").ToString();
				}
					...
					for (const char *s = content.CStr();; ++s) {
						if (*s == '\n' || *s == '\0') {
							if (!line.Empty()) {
								...
								if (!url.Empty())
									recents.PushBack({url, age.Empty() ? NkString(NkT("cg.recently")) : age});
							}
							line.Clear();
							if (*s == '\0')
								break;
				...
				NkString RepoName() const {
					...
					// retire .git final
					if (n.Size() >= 4) {
						const char *e = n.CStr() + n.Size() - 4;
						if (e[0] == '.' && e[1] == 'g' && e[2] == 'i' && e[3] == 't') {
							NkString m;
							for (usize i = 0; i + 4 <= n.Size(); ++i)
								m += n.CStr()[i];
							n = m;
						}
					}
					(void)s;
					return n.Empty() ? NkString("repo") : n;
				}

				NkString FinalDest() const {
					if (!subFolder)
						return NkString(dest);
					NkString d = dest;
					if (!d.Empty() && d.CStr()[d.Size() - 1] != '/' && d.CStr()[d.Size() - 1] != '\\')
						d += "/";
					d += RepoName();
					return d;
				}
		...
		inline const char *const *NkCgBranches(int32 *n) {
			static const char *k[] = {"main", "master", "develop", "(par defaut)"};
			if (n)
				*n = 4;
			return k;
		}

		inline const char *const *NkCgDepth(int32 *n) {
			static const char *k[] = {"Complete", "Shallow (--depth 1)"};
			if (n)
				*n = 2;
			return k;
		}
		...
		inline void NkCgField(const NkUi &u, const NkRect &r, char *buf, int32 cap, int32 id, NkCloneGitState *g,
							  float32 dt, bool blockBg, float32 leftPad, bool password = false) {
			...
			if (foc) {
				...
				for (int32 i = 0; i < len; ++i)
					dots += "*";
				if (len)
					u.TextEllipsis(r.x + leftPad, ty, r.w - leftPad - u.s(8), dots.CStr(), NkCol::foreground);
			} else {
				u.TextEllipsis(r.x + leftPad, ty, r.w - leftPad - u.s(8), buf,
			...
			if (!blockBg && u.Hit(r) && u.click) {
				...
				if (masked) {
					...
					g->caret = len;
					return;
				} // mot de passe : caret en fin
				const float32 viewW = r.w - leftPad - u.s(6);
				float32 off = 0.f;
				if (foc) {
					NkString pre;
					const int32 cc = (g->caret < len ? g->caret : len);
		...
		inline void NkCgCombo(const NkUi &u, const NkRect &r, int32 id, NkCloneGitState *g, const char *const *opts,
		...
		inline bool NkCgRadio(const NkUi &u, float32 x, float32 cy, const char *label, bool on, bool blockBg) {
			...
			u.Panel(c, on ? NkCol::secondary : NkCol::input, on ? NkCol::primary : NkCol::border,
					rd * 0.5f); // cercle (carre tres arrondi)
			if (on)
				u.dl->AddRectFilled({c.x + rd * 0.28f, c.y + rd * 0.28f, rd * 0.44f, rd * 0.44f}, NkCol::primary,
									rd * 0.22f);
		...
		inline int32 NkCloneGitPanel(const NkUi &u, const NkRect &r, NkCloneGitState *g, NkCodeState *st,
			...
			NkOwIco(u, ic.cloner, "git-branch", {r.x + u.s(28), r.y + (hH - u.s(18)) * 0.5f, u.s(18), u.s(18)},
					NkCol::primary);
			...
			// URL
			NkWizLabel(u, cx, y, NkT("cg.url"));
			y += u.s(22);
			NkCgField(u, {cx, y, cw, fH}, g->url, (int32)sizeof(g->url), 1, g, dt, blockBg, u.s(34));
			NkOwIco(u, ic.github, "git-branch", {cx + u.s(11), y + (fH - u.s(14)) * 0.5f, u.s(14), u.s(14)},
					NkCol::mutedFg);
			y += fH + u.s(10);
			{
				...
				float32 bx = cx;
				for (int32 i = 0; i < 6; ++i) {
					const float32 bw = u.TextW(badges[i]) + u.s(16);
					u.Panel({bx, y, bw, u.s(20)}, NkCol::muted, NkCol::border, NkR::sm * u.S);
					u.TextV(bx + u.s(8), y - u.s(3), u.s(20), badges[i], NkCol::mutedFg);
					bx += bw + u.s(6);
				}
			}
			y += u.s(34);
			// DESTINATION
			NkWizLabel(u, cx, y, NkT("cg.dest"));
			y += u.s(22);
			{
				const float32 browseW = u.s(120);
				NkCgField(u, {cx, y, cw - browseW - u.s(8), fH}, g->dest, (int32)sizeof(g->dest), 2, g, dt, blockBg,
						  u.s(10));
				...
				if (hv && u.click && dlg) {
					g->focus = -1;
					dlg->BrowseInto(g->dest, (int32)sizeof(g->dest), "Dossier de destination");
				}
			}
			y += fH + u.s(10);
			if (NkWizCheck(u, cx, y + u.s(8), NkT("cg.subfolder"), g->subFolder, blockBg))
				g->subFolder = !g->subFolder;
			y += u.s(34);
			// OPTIONS
			NkWizLabel(u, cx, y, NkT("tc.options"));
			y += u.s(22);
			{
				int32 nB = 0, nD = 0;
				const char *const *br = NkCgBranches(&nB);
				const char *const *dp = NkCgDepth(&nD);
				const float32 colW = (cw - u.s(16)) * 0.5f;
				u.Text(cx, y, NkT("cg.branch"), NkCol::mutedFg);
				u.Text(cx + colW + u.s(16), y, NkT("cg.depth"), NkCol::mutedFg);
			...
			if (NkWizCheck(u, cx, y + u.s(8), NkT("cg.recursive"), g->recursive, blockBg))
				g->recursive = !g->recursive;
			y += u.s(36);
			// AUTHENTIFICATION
			NkWizLabel(u, cx, y, NkT("cg.auth"));
			y += u.s(22);
			if (NkWizCheck(u, cx, y + u.s(8), NkT("cg.syscreds"), g->useSysCreds, blockBg))
				g->useSysCreds = !g->useSysCreds;
			y += u.s(30);
			if (NkWizCheck(u, cx, y + u.s(8), NkT("cg.token"), g->useToken, blockBg))
				g->useToken = !g->useToken;
			y += u.s(30);
			{
				const bool en = g->useToken;
				const bool dis = blockBg || !en;
				NkCgField(u, {cx + u.s(14), y, cw - u.s(60), u.s(30)}, g->token, (int32)sizeof(g->token), 3, g, dt, dis,
						  u.s(10), true);
				const NkRect er = {cx + cw - u.s(40), y, u.s(40), u.s(30)};
				const bool hv = !dis && u.Hit(er);
				u.Rect(er, hv ? NkCol::hover : NkCol::muted, NkR::md * u.S);
				NkOwIco(u, g->showToken ? ic.oeilOuvert : ic.oeilFermer, g->showToken ? "eye-off" : "eye",
						{er.x + u.s(13), er.y + u.s(8), u.s(14), u.s(14)}, en ? NkCol::foreground : NkCol::mutedFg);
				if (hv && u.click)
					g->showToken = !g->showToken;
			}
			y += u.s(40);
			if (NkWizCheck(u, cx, y + u.s(8), NkT("cg.ssh"), g->useSsh, blockBg))
				g->useSsh = !g->useSsh;
			y += u.s(36);
			// APRES LE CLONAGE
			NkWizLabel(u, cx, y, NkT("cg.afterclone"));
			y += u.s(24);
			if (NkCgRadio(u, cx, y, NkT("cg.after0"), g->afterAction == 0, blockBg))
				g->afterAction = 0;
			y += u.s(28);
			if (NkCgRadio(u, cx, y, NkT("cg.after1"), g->afterAction == 1, blockBg))
				g->afterAction = 1;
			y += u.s(28);
			if (NkCgRadio(u, cx, y, NkT("cg.after2"), g->afterAction == 2, blockBg))
				g->afterAction = 2;
			y += u.s(28);

			// ===== COLONNE DROITE =====
			float32 ry = r.y + hH + u.s(22);
			NkWizLabel(u, rx, ry, NkT("cg.recent"));
			ry += u.s(22);
			if (g->recents.Empty()) {
				const NkRect box = {rx, ry, rightW, u.s(40)};
				u.Panel(box, NkCol::surface, NkCol::border, NkR::md * u.S);
				u.TextV(box.x + u.s(12), box.y, u.s(40), NkT("cg.norepo"), NkCol::mutedFg);
				ry += u.s(48);
			}
			for (usize i = 0; i < g->recents.Size(); ++i) {
				...
				NkOwIco(u, isGh ? ic.github : ic.cloner, "git-branch",
						{box.x + u.s(12), box.y + u.s(10), u.s(14), u.s(14)}, NkCol::accent);
				u.TextEllipsis(box.x + u.s(32), box.y + u.s(8), rightW - u.s(120), g->recents[i].url.CStr(),
							   NkCol::foreground);
				u.Text(box.x + u.s(32), box.y + u.s(26), g->recents[i].age.CStr(), NkCol::mutedFg);
				const NkRect rb = {box.x + rightW - u.s(80), box.y + u.s(12), u.s(70), u.s(24)};
				const bool hv = !blockBg && u.Hit(rb);
				u.Rect(rb, hv ? NkCol::hover : NkCol::muted, NkR::sm * u.S);
				NkOwIco(u, ic.clonerTel, "refresh", {rb.x + u.s(6), rb.y + u.s(6), u.s(11), u.s(11)},
			...
			ry += u.s(6);
			// APERCU
			NkWizLabel(u, rx, ry, NkT("cg.preview"));
			ry += u.s(22);
			{
				const NkRect box = {rx, ry, rightW, u.s(78)};
				u.Panel(box, NkCol::surface, NkCol::border, NkR::md * u.S);
				int32 nB = 0;
				const char *const *brs = NkCgBranches(&nB);
				NkOwIco(u, ic.github, "git-branch", {box.x + u.s(12), box.y + u.s(10), u.s(13), u.s(13)},
						NkCol::primary);
			...
			{
				const NkRect ca = {rx, ry, rightW, fH};
				if (u.Button(ca, NkT("btn.cancel"), NkCol::muted, NkCol::hover, NkCol::foreground, NkR::md * u.S) &&
					!blockBg)
					result = 1;
				ry += fH + u.s(10);
			}
			{
				const bool valid = g->url[0] != 0;
				const NkRect cl = {rx, ry, rightW, u.s(40)};
				...
				NkOwIco(u, ic.clonerTel, "download",
						{cl.x + (rightW - u.TextW(NkT("cg.clone")) - u.s(20)) * 0.5f, cl.y + u.s(13), u.s(14), u.s(14)},
						fg);
				u.TextV(cl.x + (rightW - u.TextW(NkT("cg.clone")) - u.s(20)) * 0.5f + u.s(20), cl.y, u.s(40),
						NkT("cg.clone"), fg);
			...
			if (u.click && !blockBg && !g->focusClaimed && u.Hit(r))
				g->focus = -1;
			...
			if (g->comboOpen >= 0 && g->comboOpts && g->comboSel) {
				...
				for (int32 k = 0; k < g->comboN; ++k) {
					const float32 tw = u.TextW(g->comboOpts[k]) + u.s(30);
					if (tw > ddw)
						ddw = tw;
				}
				float32 ddx = g->comboR.x;
				...
				for (int32 k = 0; k < g->comboN; ++k) {
					...
					u.TextEllipsis(ir.x + u.s(8), ir.y + (ih - u.Lh()) * 0.5f, ir.w - u.s(12), g->comboOpts[k],
								   NkCol::foreground);
					if (hv && u.click) {
						*g->comboSel = k;
						chose = true;
				...
				if (chose || (u.click && !u.Hit(dd) && !g->comboJustOpened))
					g->comboOpen = -1;

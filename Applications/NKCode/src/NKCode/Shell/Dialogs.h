	...
	namespace nkcode {
		...
		struct NkCodeDialogs {
				...
				bool PickerAllowed(const char *p) const {
					return pickerConfine.Empty() || NkPathIsAncestor(pickerConfine.CStr(), p);
				}
...
// ── Disques ──
				...
				void OpenPicker(int32 purpose, const char *startDir, char *buf = nullptr, int32 cap = 0,
					...
					if (!PickerAllowed(pickerPath))
						CopyTo(pickerPath, pickerConfine.CStr(),
							   (int32)sizeof(pickerPath)); // départ dans la zone autorisée
					BuildPickerTree();
					if (purpose == PK_File)
				...
				void PickerUp() {
					PickerGoto(NkPath(pickerPath).GetParent());
				}

				void PickerEnter(const char *sub) {
					PickerGoto(NkPath(pickerPath) / sub);
				}

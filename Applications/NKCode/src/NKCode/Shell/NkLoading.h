...
namespace nkcode {
    ...
    struct NkLoadingState {
        ...
        static const char* StepKey(int32 i) {
            static const char* K[] = { "load.s0", "load.s1", "load.s2", "load.s3", "load.s4", "load.s5", "load.s6" };
        ...
        NkString StepLabel(int32 i) const {
            if (i == 0) { char b[176]; std::snprintf(b, sizeof(b), NkT("load.s0"), wsName.CStr()); return NkString(b); }
            if (i == 1 && projCount > 0) { char b[80]; std::snprintf(b, sizeof(b), "%s (%d %s)", NkT("load.s1"), projCount, NkT("load.projword")); return NkString(b); }
        ...
        void Start(const NkPath& f, NkCodeState* st, const char* openJenga = nullptr) {
            active = true; finished = false; error = false; step = 0; stepTimer = 0.f; anim = 0.f; projCount = 0;
            errLine = NkString(); errHint = NkString();
            folder = f;
            // Étape 0 : lecture réelle du .jenga.
            if (!st->LoadFolder(f)) { active = true; error = true; errLine = NkT("load.err.nows"); return; }
            if (openJenga && *openJenga) {   // sélectionne le workspace précis puis relance `jenga info`
                for (usize i = 0; i < st->wsPaths.Size(); ++i) if (st->wsPaths[i] == NkString(openJenga)) { st->wsIdx = (int32)i; break; }
                st->OpenPath(NkPath(openJenga)); st->RequestReload();
            }
            const NkString jp = (!st->wsPaths.Empty()) ? st->wsPaths[st->wsIdx < (int32)st->wsPaths.Size() ? st->wsIdx : 0] : NkString();
        ...
        void Cancel() { active = false; finished = false; error = false; }

        void Tick(NkCodeState* st, float32 dt) {
            if (!active || error || finished) return;
            anim += dt; stepTimer += dt;
            st->LoadProjects();   // DEMARRE `jenga info` (idempotent) — sinon rien ne le lance pendant le chargement
            st->PollProjects();   // puis draine chaque frame
            auto adv = [&]() { ++step; stepTimer = 0.f; };
            switch (step) {
                case 0: if (stepTimer > 0.20f) adv(); break;                 // lecture du .jenga (faite dans Start)
                case 1:                                                      // analyse des projets (réel)
                    if (st->InfoParsed()) {
                        projCount = (int32)st->projects.Size();
                        if (st->InfoHasError() && projCount == 0) { error = true; errLine = st->InfoErrorLine(); errHint = HintOf(errLine); return; }
                        if (stepTimer > 0.25f) adv();
                    } else if (stepTimer > 45.f) {                           // garde-fou : jenga info bloqué (gros workspaces = marge)
                        error = true; errLine = NkT("load.err.timeout"); errHint = NkString();
                    }
                    break;
                case 2: if (stepTimer > 0.30f) adv(); break;                 // toolchains (déclarées dans le .jenga)
                case 3: if (stepTimer > 0.25f) adv(); break;                 // IntelliSense (roadmap)
                case 4: if (stepTimer > 0.25f) adv(); break;                 // indexation (roadmap)
                case 5: if (stepTimer > 0.20f) adv(); break;                 // extensions (roadmap)
                case 6: if (stepTimer > 0.20f) { finished = true; } break;   // session -> fin (bascule geree par DrawHome)
            }
        }
        // Fraction de progression (0..1), l'étape active comptant à moitié.
        float32 Progress() const {
            if (error) return (float32)step / (float32)STEPS;
            const float32 base = (float32)step + 0.5f;
            float32 p = base / (float32)STEPS; if (p > 1.f) p = 1.f; return p;
        }

        // Cause probable, best-effort, à partir du message d'erreur Python.
        static NkString HintOf(const NkString& e) {
            if (e.Contains("is not defined") || e.Contains("NameError")) return NkT("load.hint.import");
            if (e.Contains("SyntaxError"))    return NkT("load.hint.syntax");
            if (e.Contains("No module") || e.Contains("ModuleNotFound")) return NkT("load.hint.module");
            if (e.Contains("IndentationError")) return NkT("load.hint.indent");
            return NkString();
        }
    };

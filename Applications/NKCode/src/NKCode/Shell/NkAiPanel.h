			...
			public:
				...
				void OnUI(NkEditorFrameContext &ec) override {
					...
					if (ctx.font && ctx.font->Valid()) {
						...
						dl.AddText(ctx.font->Face(), ctx.font->TexId(), {hdr.x + pad, by}, NkT("ai.title"),
								   ctx.theme.text);
						...
						for (int32 i = 2; i >= 0; --i) {
							...
							dl.AddRectFilled(
								pb, sel ? ctx.theme.buttonActive : (hov ? ctx.theme.buttonHover : ctx.theme.button),
								ctx.theme.rounding);
							const NkColor fg =
								dis ? ctx.theme.textDisabled : (sel ? NkColor{255, 255, 255, 255} : ctx.theme.text);
					...
					dl.AddRectFilled(send,
									 canSend ? (sHov ? ctx.theme.buttonHover : ctx.theme.accent) : ctx.theme.button,
									 ctx.theme.rounding);
					if (ctx.font && ctx.font->Valid()) {
						const char *sl = mBusy ? "…" : NkT("ai.send");
						const float32 tw = ctx.font->MeasureWidth(sl);
						dl.AddText(ctx.font->Face(), ctx.font->TexId(),
								   {send.x + (send.w - tw) * 0.5f,
									send.y + (send.h - ctx.font->LineHeight()) * 0.5f + ctx.font->Ascent()},
			...
			private:
				...
				void DrawMessages(NkGuiContext &ctx, const NkRect &body) {
					...
					for (usize i = 0; i < mMsgs.Size(); ++i) {
						...
						WrapLines(ctx, mMsgs[i].text.CStr(), textMaxW, ls);
					...
					if (NkGuiRectContains(body, ctx.input.mousePos) && ctx.input.wheel != 0.f) {
						mScroll -= ctx.input.wheel * lineH * 3.f;
						ctx.input.wheel = 0.f;
						mStick = false;
					}
					if (mStick)
					...
					for (usize i = 0; i < mMsgs.Size(); ++i) {
						...
						for (usize k = 0; k < ls.Size(); ++k) {
							const float32 w = ctx.font ? ctx.font->MeasureWidth(ls[k].CStr()) : 0.f;
							if (w > tw)
								tw = w;
						}
						...
						if (y + bh >= body.y && y <= body.y + body.h) {
							NkColor bg =
								user ? ctx.theme.buttonActive : (m.role == 2 ? ctx.theme.header : ctx.theme.panel);
							dl.AddRectFilled({bx, y, bw, bh}, bg, ctx.theme.rounding);
							if (m.role != 0)
								dl.AddRect({bx, y, bw, bh}, ctx.theme.border, 1.f);
							const NkColor fg = user ? NkColor{255, 255, 255, 255}
													: (m.role == 2 ? ctx.theme.textDisabled : ctx.theme.text);
							if (ctx.font && ctx.font->Valid())
								for (usize k = 0; k < ls.Size(); ++k)
									dl.AddText(ctx.font->Face(), ctx.font->TexId(),
											   {bx + bubPad, y + bubPad + k * lineH + asc}, ls[k].CStr(), fg);
						}
						y += bh + gap;
					}
					dl.PopClipRect();
				...
				NkString BuildJson() const {
					...
					if (ollama) {
						...
						j += "\"model\":\"claude-sonnet-5\",\"max_tokens\":1024,";
					}
					j += "\"messages\":[";
					bool first = true;
					for (usize i = 0; i < mMsgs.Size(); ++i) {
						if (mMsgs[i].role != 0 && mMsgs[i].role != 1)
							continue; // system/erreur exclus
						if (!first)
							j += ",";
						first = false;
						j += "{\"role\":\"";
						j += (mMsgs[i].role == 0 ? "user" : "assistant");
						j += "\",\"content\":\"";
						j += JsonEsc(mMsgs[i].text.CStr());
						j += "\"}";
					}
					j += "]}";
					return j;
				...
				void Send() {
					if (mBusy || mInput[0] == 0)
						return;
					if (mProvider == 2) {
						mMsgs.PushBack({2, NkString(NkT("ai.homesoon"))});
						return;
					}
					...
					mRespLines.Clear();
					if (!mProc.Start(cmd)) {
						mMsgs.PushBack({2, NkString(NkT("ai.errbusy"))});
						return;
					}
					mBusy = true;
				}
				...
				void Poll() {
					...
					mRespLines.Clear();
					if (raw.Empty()) {
						mMsgs.PushBack({2, NkString(NkT("ai.errnet"))});
						mStick = true;
						return;
					}

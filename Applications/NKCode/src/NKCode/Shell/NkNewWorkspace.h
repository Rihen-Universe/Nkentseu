...
namespace nkentseu {
	namespace nkcode {
		...
		inline int32 NkWizOsCount() {
			int32 n = 0;
			(void)NkWizOsTable(&n);
			return n;
		}

		inline const char *NkWizOsName(int32 i) {
			int32 n = 0;
			const NkOsDef *T = NkWizOsTable(&n);
			return (i >= 0 && i < n) ? T[i].name : "";
		}

		inline const char *NkWizOsFilt(int32 i) {
			int32 n = 0;
			const NkOsDef *T = NkWizOsTable(&n);
			return (i >= 0 && i < n) ? T[i].filt : "";
		}

		inline int32 NkWizOsSpecial(int32 i) {
			int32 n = 0;
			const NkOsDef *T = NkWizOsTable(&n);
			return (i >= 0 && i < n) ? T[i].special : 0;
		}
		...
		struct NkWizProject {
				...
				struct OsFlt {
						bool on = false;
						int32 toolchain = -1;
						NkVector<NkString> links, defines;
						int32 androidKind = 1;
						int32 webMem = 1;
				};
				...
				struct CustFlt {
						char expr[120] = {};
						int32 toolchain = -1;
						NkVector<NkString> defines, links;
						bool ok = false;
				};
		...
		struct NkNewWsState {
				...
				float32 scroll = 0.f, scrollMax = 0.f, barOff = 0.f;
				bool barDrag = false;
				float32 hscroll = 0.f, hscrollMax = 0.f, hbarOff = 0.f;
				bool hbarDrag = false;
				// — Etape 1 : Workspace —
				...
				int32 projEditIdx = -1;
				int32 projStep = 0;
				NkWizProject projDraft;
				char projSrcAdd[160] = {}, projIncAdd[160] = {};
				char projFltAdd[160] = {};
				int32 projFltTarget = -1; // ajout dans une liste de filtre (links/defines)
				...
				float32 codeSY = 0.f, codeSX = 0.f, codeBarOff = 0.f;
				int32 codeBar = 0;
				...
				NkString extractBuf;
				bool extractEditing = false, extractEdited = false;
				int32 extractCaret = 0;
				...
				void OpenProjDlg(int32 editIdx) {
					...
					comboOpen = -1;
					projScroll = 0.f;
					projSrcAdd[0] = projIncAdd[0] = projFltAdd[0] = '\0';
					projFltTarget = -1;
					fltLSY = fltLSX = fltRSY = fltRSX = 0.f;
					codeSX = codeSY = 0.f;
					cfgSY = cfgSX = 0.f;
					...
					extractCaret = 0;
					extractBuf.Clear();
					if (editIdx >= 0 && editIdx < (int32)projects.Size())
						projDraft = projects[editIdx];
					else {
						NkWizProject p;
						p.sources.PushBack(NkString("src/**.cpp"));
						p.sources.PushBack(NkString("src/**.c"));
						p.includes.PushBack(NkString("include"));
						projDraft = p;
					}
					// Filtres systeme presents par defaut = plateformes cochees au workspace (modifiable ensuite).
					{
						int32 nOs = NkWizOsCount();
						bool anyOn = false;
						for (int32 i = 0; i < nOs; ++i)
							if (projDraft.os[i].on) {
								anyOn = true;
								break;
							}
						if (!anyOn) {
							const bool en[7] = {osWin, osLinux, osMac, osAndroid, osIos, osWeb, osHarmony};
							for (int32 i = 0; i < nOs; ++i)
								projDraft.os[i].on = (i < 7) ? en[i] : false;
						}
					}
					// Reprendre l'edition manuelle existante.
					if (projDraft.hasCustomCode) {
						extractBuf = projDraft.customCode;
						extractEdited = true;
					}
					...
					if (projDraft.cfgFlt.Empty()) {
						auto addCfg = [&](const char *name, bool isDbg) {
							NkWizProject::CfgFlt c;
							c.name = name;
							if (isDbg) {
								c.opt = 0;
								c.sym = true;
								c.defines.PushBack(NkString("_DEBUG"));
								c.defines.PushBack(NkString("DEBUG"));
							} else {
								c.opt = 2;
								c.sym = false;
								c.defines.PushBack(NkString("NDEBUG"));
							}
						...
						if (cfgDebug)
							addCfg("Debug", true);
						if (cfgRelease)
							addCfg("Release", false);
						for (usize i = 0; i < customCfgs.Size(); ++i)
							if (customCfgs[i].on) {
								NkWizProject::CfgFlt c;
								c.name = customCfgs[i].name;
								projDraft.cfgFlt.PushBack(c);
							}
					}
				}
				...
				void CommitProjDlg() {
					if (!projDraft.name[0] || !ValidName(projDraft.name))
						return;
					// On ne depend QUE d'une bibliotheque existante : on purge tout residu (app cochee, projet
					// supprime...).
					for (int32 k = (int32)projDraft.dependsOn.Size() - 1; k >= 0; --k) {
						bool isLib = false;
						for (usize i = 0; i < projects.Size(); ++i)
							if ((int32)i != projEditIdx && StrEq(projects[i].name, projDraft.dependsOn[k].CStr()) &&
								(projects[i].kind == 2 || projects[i].kind == 3)) {
								isLib = true;
								break;
							}
						if (!isLib)
							projDraft.dependsOn.Erase(projDraft.dependsOn.Begin() + k);
					}
					// L'extrait edite manuellement est CONSERVE a la creation (et reste re-editable).
					if (extractEdited) {
						projDraft.customCode = extractBuf;
						projDraft.hasCustomCode = true;
					}
					if (projEditIdx >= 0 && projEditIdx < (int32)projects.Size())
						projects[projEditIdx] = projDraft;
					else
						projects.PushBack(projDraft);
					projDlg = false;
					extractEditing = false;
					extractEdited = false;
					extractBuf.Clear();
				...
				struct TcDet {
						NkString name, version, target, path;
						bool found = false;
				};
				...
				const char *tcPlatPtrs[7][16] = {};
				int32 tcPlatN[7] = {};
				int32 tcPlatSel[7] = {};

				// Toolchain personnalise (dialog « Ajouter manuel »)
				struct CustomTc {
						char name[64] = "mon-gcc-12";
						int32 family = 1;
						int32 os = 1;
						int32 arch = 0;
						int32 env = 0;
						...
						NkVector<NkString> cflags, cxxflags, ldflags;
						bool regGlobal = true;
				};
				...
				bool osWin = true, osLinux = true, osMac = false, osAndroid = true, osWeb = true, osIos = false,
					 osHarmony = false;
				bool osFreeBSD = false, osTvos = false, osWatchos = false, osXboxOne = false, osXboxSeries = false,
					 osPs4 = false, osPs5 = false, osSwitch = false;
				...
				// — Etape 5 : edition manuelle de l'apercu workspace (variables DEDIEES, distinctes de la modale
				// projet) —
				NkString wsExtractBuf;
				bool wsExtractEditing = false, wsExtractEdited = false;
				int32 wsExtractCaret = 0;
				...
				NkString FinalFolder() const {
					return (NkPath(location) / wsName).ToString();
				}
				...
				// Execute une commande et capture sa sortie standard (detection unique ; bref flash console sous
				// Windows).
				...
				static bool IsJengaTc(const char *n) {
					static const char *T[] = {"android-ndk", "clang-cross-linux", "clang-mingw",
											  "emscripten",	 "gcc-cross-linux",	  "host-apple-clang",
											  "host-clang",	 "host-gcc",		  "mingw",
											  "msvc",		 "ohos-ndk"};
					for (const char *t : T)
						if (StrEq(n, t))
							return true;
				...
				void DetectToolchains() {
...
#if defined(_WIN32)
					{
						NkString p = TcWhich("gcc");
						if (p.Empty() && NkFile::Exists("C:\\msys64\\ucrt64\\bin\\gcc.exe"))
							p = NkString("C:\\msys64\\ucrt64\\bin\\gcc.exe");
						add("mingw", "GCC (MinGW/UCRT64)", "Windows x64", p, !p.Empty());
					}
#else
					{
						NkString p = TcWhich("gcc");
						add("host-gcc", "GCC (PATH)", hostOs, p, !p.Empty());
					}
#endif
					{
						NkString p = TcEnv("ANDROID_NDK_ROOT");
						if (p.Empty())
							p = TcEnv("ANDROID_NDK_HOME");
						if (p.Empty()) {
							NkString ah = TcEnv("ANDROID_HOME");
							if (!ah.Empty()) {
								NkString c = ah;
								c += "/ndk";
								if (NkDirectory::Exists(c.CStr()))
									p = c;
							}
						}
					...
					{
						NkString p = TcEnv("EMSDK");
						if (p.Empty())
							p = TcWhich("emcc");
						add("emscripten", "Emscripten", "Web WASM32", p, !p.Empty());
					}
					{
						NkString p = TcWhich("zig");
						add("zig", "Zig (cross-compile)", "Linux / Win / macOS", p, !p.Empty());
					}
					{
						NkString p = TcWhich("clang-cl");
						add("msvc", "MSVC clang-cl", "Windows MSVC", p, !p.Empty());
					}
					{
						NkString p = TcEnv("OHOS_SDK");
						if (p.Empty())
							p = TcEnv("OHOS_NDK_HOME");
						if (p.Empty())
							p = TcEnv("DEVECO_SDK_HOME");
						if (p.Empty())
							p = TcEnv("HARMONY_HOME");
						add("ohos-ndk", "OHOS / HarmonyOS NDK", "HarmonyOS ARM64", p, !p.Empty());
					}
#if defined(_WIN32)
					// WSL est 100% OPTIONNEL : si wsl.exe est absent, on n'ajoute rien et on n'execute rien (aucun
					// risque).
					if (NkFile::Exists("C:\\Windows\\System32\\wsl.exe")) {
						{
							...
							// NB2: wsl.exe sort en UTF-16LE par defaut (-> caracteres « chinois ») : WSL_UTF8=1 force
							// l'UTF-8.
							_putenv("WSL_UTF8=1");
							NkString oRaw = TcRun(
								"wsl.exe -e sh -c \"grep -h '^PRETTY_NAME=' /etc/os-release 2>/dev/null; uname -m; "
								"echo @@; for c in gcc clang g++ clang++ cc; do command -v $c 2>/dev/null; done\"");
							// Securite : on retire tout octet non-ASCII imprimable (residu d'encodage) sauf les sauts
							// de ligne.
							NkString o;
							...
							NkString distro, arch, comps;
							bool afterMark = false, gotArch = false;
							NkString line;
							for (const char *s = o.CStr();; ++s) {
								if (*s == '\n' || *s == '\0') {
									NkString ln;
									for (const char *c = line.CStr(); *c && *c != '\r'; ++c)
										ln += *c;
									if (ln == "@@")
										afterMark = true;
									else if (!afterMark) {
							...
							if (hasGcc)
								add("wsl2-gcc", "GCC via WSL2 (gcc/g++)", tgt.CStr(), NkString("wsl.exe -e g++"), true);
							if (hasClang)
								add("wsl2-clang", "Clang via WSL2 (clang/clang++)", tgt.CStr(),
									NkString("wsl.exe -e clang++"), true);
							if (!hasGcc && !hasClang)
								add("wsl2-linux", "WSL2 - aucun compilateur installe", tgt.CStr(),
									NkString("C:\\Windows\\System32\\wsl.exe"), false);
						}
					}
#endif
				...
				void RefreshTcPtrs() {
					tcNameN = (int32)tcDet.Size();
					if (tcNameN > 16)
						tcNameN = 16;
					for (int32 i = 0; i < tcNameN; ++i)
						tcNamePtrs[i] = tcDet[i].name.CStr();
					...
					for (int32 pp = 0; pp < 7; ++pp) {
						...
						for (int32 i = 0; i < (int32)tcDet.Size() && n < 16; ++i)
							if (tcDet[i].found && tcDet[i].target.Contains(pkeys[pp]))
								tcPlatPtrs[pp][n++] = tcDet[i].name.CStr();
						tcPlatN[pp] = n;
						if (tcPlatSel[pp] >= n)
							tcPlatSel[pp] = 0;
					}
				...
				bool Step1Valid() const {
					if (!ValidName(wsName))
						return false;
					if (!jengaFile[0] || !ValidName(jengaFile) || !NkCodeState::EndsWithI(jengaFile, ".jenga"))
						return false;
					if (!location[0])
						return false;
					return true;
				...
				void AddCustomCfg() {
					if (!newCfgName[0] || !ValidName(newCfgName))
						return;
					if (StrEq(newCfgName, "Debug") || StrEq(newCfgName, "Release"))
						return; // doublon
					for (usize i = 0; i < customCfgs.Size(); ++i)
						if (StrEq(customCfgs[i].name.CStr(), newCfgName)) {
							customCfgs[i].on = true;
							return;
						}
					CustomCfg c;
					c.name = newCfgName;
					c.on = true;
					customCfgs.PushBack(c);
				}
				...
				NkString BuildJenga() const {
					...
					s += "\"\"\"";
					s += wsName;
					s += " - workspace Jenga (genere par NKCode).\"\"\"\n\n";
					if (osAndroid)
					...
					// configurations
					{
						...
						for (usize i = 0; i < customCfgs.Size(); ++i)
							if (customCfgs[i].on)
								addc(customCfgs[i].name.CStr());
						s += "])\n";
					}
					// targetoses : UNIQUEMENT les systemes coches a l'etape 4.
					s += "    targetoses([\n";
					{
						auto addos = [&](bool on, const char *e) {
							if (on) {
								s += "        TargetOS.";
								s += e;
								s += ",\n";
						...
						addos(osLinux, "LINUX");
						addos(osMac, "MACOS");
						addos(osFreeBSD, "FREEBSD");
						addos(osAndroid, "ANDROID");
						addos(osIos, "IOS");
						addos(osTvos, "TVOS");
						addos(osWatchos, "WATCHOS");
						addos(osHarmony, "HARMONYOS");
						addos(osWeb, "WEB");
						addos(osXboxOne, "XBOX_ONE");
						addos(osXboxSeries, "XBOX_SERIES");
						addos(osPs4, "PS4");
						addos(osPs5, "PS5");
						addos(osSwitch, "SWITCH");
					}
					s += "    ])\n";
					// targetarchs : UNIQUEMENT les architectures cochees.
					s += "    targetarchs([\n";
					{
						auto adda = [&](bool on, const char *e) {
							if (on) {
								s += "        TargetArch.";
								s += e;
								s += ",\n";
						...
						adda(archX64, "X86_64");
						adda(archArm64, "ARM64");
						adda(archWasm32, "WASM32");
						adda(archX86, "X86");
						adda(archArm, "ARM");
						adda(archRiscv64, "RISCV64");
						adda(archWasm64, "WASM64");
						adda(archMips, "MIPS");
					}
					s += "    ])\n";
					if (startProj >= 0 && startProj < (int32)projects.Size()) {
						s += "    startproject(\"";
						s += projects[startProj].name;
						s += "\")\n";
					}
					...
					// Android : chemins SDK/NDK au niveau WORKSPACE (les ABIs/API/appId sont au niveau PROJET, cf.
					// BuildProjectJenga).
					if (osAndroid) {
					...
					// Framework de tests : un bloc unitest() est requis des qu'un projet genere des tests.
					{
						bool anyTest = false;
						for (usize i = 0; i < projects.Size(); ++i)
							if (projects[i].genTest && projects[i].kind != 4) {
								anyTest = true;
								break;
							}
						if (anyTest)
					...
					// Projets INTEGRES : bloc complet (filtres compris), edition manuelle conservee, reindente sous le
					// workspace.
					for (usize i = 0; i < projects.Size(); ++i) {
						const NkWizProject &p = projects[i];
						if (p.separateFile)
				...
				NkString BuildProjectJenga(const NkWizProject &p) const {
					...
					static const char *TCS[] = {"(auto)",	  "clang-mingw",  "zig-linux-x64", "android-ndk",
												"emscripten", "clang-native", "clang-cl"};
					...
					s += "\"):\n";
					s += "    ";
					s += (p.kind >= 0 && p.kind < 5 ? KINDS[p.kind] : "consoleapp");
					s += "()\n";
					s += "    language(\"";
					s += (p.lang >= 0 && p.lang < 6 ? LANGS[p.lang] : "C++");
					s += "\")\n";
					if (p.lang == 0)
						s += "    cdialect(\"C17\")\n"; // C
					else {
						s += "    cppdialect(\"";
						s += (p.dialect >= 0 && p.dialect < 5 ? DIALS[p.dialect] : "C++20");
						s += "\")\n";
					}
					// Fichier .jenga separe -> il est DEJA dans le dossier du projet : location relative = "."
					s += "    location(\"";
					s += (p.separateFile ? "." : p.location);
					s += "\")\n";
					// files/includedirs sont RELATIFS a location -> pas de prefixe (sinon double chemin = 0 source
					// trouvee).
					s += "    files(";
					lst(s, p.sources, "");
					s += ")\n";
					...
					if (p.binDir[0] && !StrEq(p.binDir, p.tgtDir)) {
						s += "    bindir(\"";
						s += p.binDir;
						s += "\")\n";
					}
					...
					for (int32 i = 0; i < nOs; ++i) {
						...
						if (andOv) {
							s += "        ";
							s += (o.androidKind >= 0 && o.androidKind < 5 ? KINDS[o.androidKind] : "windowedapp");
							s += "()\n";
							any = true;
						}
						if (o.toolchain > 0) {
					...
					for (usize c = 0; c < p.cfgFlt.Size(); ++c) {
						...
						s += "        optimize(\"";
						s += (cf.opt >= 0 && cf.opt < 4 ? OPT[cf.opt] : "Off");
						s += "\")\n";
						s += "        symbols(";
						s += (cf.sym ? "True" : "False");
						s += ")\n";
					...
					// Assignation toolchain par plateforme (etape 3) : niveau PROJET (filtre respecte), toolchains
					// Jenga valides.
					{
						const char *sysTok[7] = {"system:Windows", "system:Linux", "system:macOS",	  "system:Android",
												 "system:iOS",	   "system:Web",   "system:HarmonyOS"};
						const bool osOn[7] = {osWin, osLinux, osMac, osAndroid, osIos, osWeb, osHarmony};
						for (int32 pp = 0; pp < 7; ++pp) {
							if (!osOn[pp] || tcPlatN[pp] <= 0)
								continue;
							if (p.os[pp].on && p.os[pp].toolchain > 0)
								continue; // le projet a deja son propre toolchain pour cette plateforme
					...
					if ((p.kind == 0 || p.kind == 1) && osAndroid) {
						...
						s += ")\n";
						s += "        androidtargetsdk(";
						s += api(androidApiTarget);
						s += ")\n";
						s += "        androidcompilesdk(";
						s += api(androidCompileSdk);
						s += ")\n";
						s += "        androidabis([";
						{
							bool f = true;
							auto ab = [&](bool on, const char *a) {
								if (on) {
									if (!f)
										s += ", ";
									s += "\"";
							...
							ab(abiArm64v8a, "arm64-v8a");
							ab(abiX86_64, "x86_64");
							ab(abiArmeabi, "armeabi-v7a");
							ab(abiX86, "x86");
						}
						s += "])\n";
					}
				...
				NkString BuildProjectFile(const NkWizProject &p) const {
					...
					s += "\"\"\"";
					s += p.name;
					s += " - fichier projet Jenga (genere par NKCode).\"\"\"\n\n";
					s += "from Jenga import *\n";
					s += "from Jenga.GlobalToolchains import RegisterJengaGlobalToolchains\n\n";
					s += "RegisterJengaGlobalToolchains()\n\n";
				...
				bool Generate(NkCodeDialogs *dlg) {
					...
					if (!NkDirectory::CreateRecursive(folder.CStr()))
						return false;
					...
					if (!NkFile::WriteAllText((NkPath(folder.CStr()) / jengaFile), wsContent))
						return false;
					// 2) Pour CHAQUE projet : dossiers + fichiers sources + headers + README + tests + .jenga separe.
					for (usize i = 0; i < projects.Size(); ++i) {
						const NkWizProject &p = projects[i];
						NkString loc = p.location;
						if (!loc.Empty() && loc.CStr()[loc.Size() - 1] != '/')
							loc += "/"; // normalise
						...
						if (p.separateFile) {
							...
							f += p.name;
							f += ".jenga";
							WriteRel(folder, f, BuildProjectFile(p));
						}
						// main.cpp (apps)
						if (isApp && p.genMain) {
						...
						if (isLib) {
							...
							hf += p.name;
							hf += ".h";
							WriteRel(folder, hf, h);
							NkString cpp = "#include \"";
							cpp += p.name;
							cpp += ".h\"\n#include <iostream>\n\nnamespace ";
							cpp += p.name;
							cpp += "\n{\n    void Hello()\n    {\n        std::cout << \"";
						...
						if (p.genReadme) {
							...
							r += p.name;
							r += "\n\nProjet `";
							r += (p.kind >= 0 && p.kind < 5 ? KN[p.kind] : "consoleapp");
							r += "` genere par NKCode.\n";
							NkString f = loc;
							f += "README.md";
							WriteRel(folder, f, r);
					...
					// 4) .gitignore.
					if (gitIgnore)
						NkFile::WriteAllText((NkPath(folder.CStr()) / ".gitignore"),
											 NkString("Build/\n.jenga/\n*.o\n*.obj\n*.exe\n*.dll\n*.so\n*.a\n*.lib\n"));
					// 5) Depot Git (init).
					if (gitInit) {
						NkString cmd = "git -C \"";
					...
					if (dlg)
						dlg->DoLoad(NkPath(folder.CStr())); // ouvre dans l'editeur
					return true;
				}
		};

		// ── Combo deroulant fonctionnel (le menu est rendu en fin de panneau, par-dessus). ──
		inline void NkWizCombo(const NkUi &u, const NkRect &r, int32 id, NkNewWsState *w, const char *const *opts,
							   int32 n, int32 *sel, bool blockBg) {
			...
			u.Icon("chevron-down", {r.x + r.w - u.s(18), r.y + (r.h - u.s(10)) * 0.5f, u.s(10), u.s(10)},
				   NkCol::mutedFg);
		...
		inline void NkWizSteps(const NkUi &u, const NkRect &r, int32 current, const char *const *labels, int32 count,
			...
			for (int32 i = 0; i < count; ++i) {
				...
				if (i > 0) {
					...
					u.dl->AddRectFilled({sx + u.s(2), cy + dotD * 0.5f - u.s(1.5f), segW - u.s(4), u.s(3)},
										(done || active) ? NkCol::accent : NkCol::border, u.s(1.5f));
				}
				const NkRect dot = {x + (cellW - dotD) * 0.5f, cy, dotD, dotD};
				if (active)
					u.dl->AddRectFilled({dot.x - u.s(3), dot.y - u.s(3), dotD + u.s(6), dotD + u.s(6)},
										NkColor{15, 115, 213, 60}, (dotD + u.s(6)) * 0.5f);
				...
				if (done) { // etape validee -> icone de validation simple
					...
					if (doneTex)
						NkDrawIcon(u, doneTex, ir, NkCol::primaryFg);
					else
		...
		inline bool NkWizCheck(const NkUi &u, float32 x, float32 cy, const char *label, bool val, bool blockBg) {
			...
			u.Panel(b, val ? NkCol::accent : NkCol::input, val ? NkCol::accent : NkCol::border,
					NkR::md * u.S); // coins lisses
			if (val) {
				const float32 s = bs;
				u.dl->AddLine({b.x + s * 0.26f, b.y + s * 0.52f}, {b.x + s * 0.44f, b.y + s * 0.70f}, NkCol::primaryFg,
							  u.s(2.1f));
				u.dl->AddLine({b.x + s * 0.44f, b.y + s * 0.70f}, {b.x + s * 0.76f, b.y + s * 0.30f}, NkCol::primaryFg,
							  u.s(2.1f));
			}
		...
		inline void NkWizField(const NkUi &u, const NkRect &r, char *buf, int32 cap, int32 id, NkNewWsState *w,
							   float32 dt, bool blockBg, float32 leftPad, bool readOnly = false) {
			const bool foc = (w->focus == id) && !readOnly;
			u.Panel(r, readOnly ? NkCol::muted : NkCol::input, foc ? NkCol::primary : NkCol::border, NkR::md * u.S);
			if (foc)
				NkOwEdit(u, r, buf, cap, w->caret, w->blink, dt, leftPad);
			else
				u.TextEllipsis(r.x + leftPad, r.y + (r.h - u.Lh()) * 0.5f, r.w - leftPad - u.s(8), buf,
							   (readOnly ? NkCol::mutedFg : (buf[0] ? NkCol::foreground : NkCol::mutedFg)));
			if (!readOnly && !blockBg && u.Hit(r) && u.click) {
				...
				if (foc) {
					NkString pre;
					const int32 cc = (w->caret < len ? w->caret : len);
					for (int32 k = 0; k < cc; ++k)
						pre += buf[k];
					const float32 pw = u.TextW(pre.CStr());
					off = (pw > viewW) ? (pw - viewW) : 0.f;
				}
				...
				NkString acc;
				for (int32 i = 0;; ++i) {
					const float32 wv = u.TextW(acc.CStr());
		...
		inline int32 NkWizFooter(const NkUi &u, const NkRect &r, int32 step, int32 lastStep, bool blockBg,
								 bool nextEnabled, const char *lastLabel = "Creer le Workspace",
								 bool showCreateNow = true, uint32 validTex = 0) {
			u.Rect(r, NkCol::sidebar);
			...
			auto createIcon = [&](float32 ix, float32 iy, const NkColor &fg) {
				if (nextEnabled)
					NkOwIco(u, validTex, "check", {ix, iy, u.s(14), u.s(14)}, fg);
				else
					u.Icon("x", {ix, iy, u.s(14), u.s(14)}, NkCol::danger);
			};
			...
			const NkRect nextR = {rx - nextW, by, nextW, bh};
			rx -= nextW + u.s(8);
			{
				...
				const float32 ix = nextR.x + (nextW - tw - u.s(18)) * 0.5f + tw + u.s(6),
							  iy = nextR.y + (bh - u.s(14)) * 0.5f;
				if (last)
					createIcon(ix, iy, nfg);
				else
					u.Icon("arrow-right", {ix, iy, u.s(13), u.s(13)}, nfg);
				if (hv && u.click)
					act = 2;
			}
			// Creer maintenant (sur les etapes NON finales : cree avec les valeurs par defaut)
			if (!last && showCreateNow) {
				const float32 crw = u.TextW(lastLabel) + u.s(50);
				const NkRect crR = {rx - crw, by, crw, bh};
				rx -= crw + u.s(8);
				const bool hv = nextEnabled && !blockBg && u.Hit(crR);
				u.Rect(crR, !nextEnabled ? NkCol::muted : (hv ? NkColHover(NkCol::success) : NkCol::success),
					   NkR::md * u.S);
			...
			// Annuler
			const float32 cw = u.s(96);
			const NkRect cancelR = {rx - cw, by, cw, bh};
			rx -= cw + u.s(8);
			...
			if (step > 0) {
				...
				u.Icon("arrow-left", {prevR.x + u.s(14), prevR.y + (bh - u.s(13)) * 0.5f, u.s(13), u.s(13)},
					   NkCol::foreground);
		...
		inline float32 NkWizStep1(const NkUi &u, const NkRect &body, NkNewWsState *w, NkCodeDialogs *dlg, float32 dt,
								  bool blockBg, const NkIcons &ic) {
			...
			// NOM DU WORKSPACE
			NkWizLabel(u, cx, y, NkT("nws.wsname"));
			y += labGap;
			...
			// EMPLACEMENT
			NkWizLabel(u, cx, y, NkT("nws.location"));
			y += labGap;
			const float32 browseW = u.s(110);
			NkWizField(u, {cx, y, cw - browseW - u.s(8), fH}, w->location, (int32)sizeof(w->location), 2, w, dt,
					   blockBg, u.s(10));
			{
				...
				NkOwIco(u, ic.ouvrirDossier, "folder-open",
						{br.x + u.s(14), br.y + (fH - u.s(14)) * 0.5f, u.s(14), u.s(14)}, NkCol::mutedFg);
				u.TextV(br.x + u.s(34), br.y, fH, NkT("btn.browse"), NkCol::foreground);
				if (hv && u.click && !blockBg && dlg) {
					w->focus = -1;
					dlg->BrowseInto(w->location, (int32)sizeof(w->location), "Emplacement du workspace");
				}
			}
			y += fH + hintGap;
			{
			...
			NkWizLabel(u, cx, y, NkT("nws.jengafile"));
			y += labGap;
			NkWizField(u, {cx, y, cw - u.s(96), fH}, w->jengaFile, (int32)sizeof(w->jengaFile), 3, w, dt, blockBg,
					   u.s(10));
			...
			NkWizLabel(u, cx, y, NkT("nws.buildcfg"));
			y += labGap;
			const float32 cfgBoxH =
				u.s(80) + (w->customCfgs.Empty() ? 0.f : u.s(26) * ((w->customCfgs.Size() + 2) / 3));
			{
				const NkRect box = {cx, y, cw, cfgBoxH};
				u.Panel(box, NkCol::input, NkCol::border, NkR::md * u.S);
				float32 bx = box.x + u.s(16);
				const float32 bcy = box.y + u.s(24);
				if (NkWizCheck(u, bx, bcy, "Debug", w->cfgDebug, blockBg))
					w->cfgDebug = !w->cfgDebug;
				bx += u.s(100);
				if (NkWizCheck(u, bx, bcy, "Release", w->cfgRelease, blockBg))
					w->cfgRelease = !w->cfgRelease;
				bx += u.s(108);
				for (usize i = 0; i < w->customCfgs.Size(); ++i) { // config custom : clic = (de)cocher
					if (NkWizCheck(u, bx, bcy, w->customCfgs[i].name.CStr(), w->customCfgs[i].on, blockBg))
						w->customCfgs[i].on = !w->customCfgs[i].on;
				...
				u.TextV(box.x + u.s(16), box.y + cfgBoxH - u.s(28), u.s(22),
						"Ajoutez-en via « Nouvelle configuration » -->", NkCol::mutedFg);
			}
			y += cfgBoxH + hintGap;
			{
				NkString cfgs;
			...
			// PROJET DE DEMARRAGE
			NkWizLabel(u, cx, y, NkT("nws.startproj"));
			y += labGap;
			{
				...
				const char *txt = (w->startProj >= 0 && w->startProj < (int32)w->projects.Size())
									  ? w->projects[w->startProj].name
									  : "(sera defini apres creation des projets)";
			...
			// GIT (dans un bloc)
			NkWizLabel(u, rx, ry, NkT("nws.git"));
			ry += labGap;
			{
				const NkRect box = {rx, ry, rightW, u.s(108)};
				u.Panel(box, NkCol::surface, NkCol::border, NkR::md * u.S);
				float32 gy = box.y + u.s(20);
				if (NkWizCheck(u, box.x + u.s(14), gy, NkT("nws.gitinit"), w->gitInit, blockBg))
					w->gitInit = !w->gitInit;
				gy += u.s(30);
				if (NkWizCheck(u, box.x + u.s(14), gy, NkT("nws.gitignore"), w->gitIgnore, blockBg))
					w->gitIgnore = !w->gitIgnore;
				gy += u.s(30);
				if (NkWizCheck(u, box.x + u.s(14), gy, NkT("nws.gitcommit"), w->gitCommit, blockBg))
					w->gitCommit = !w->gitCommit;
			}
			ry += u.s(108) + secGap;

			// NOUVELLE CONFIGURATION (formulaire fonctionnel : ajoute a CONFIGURATIONS)
			NkWizLabel(u, rx, ry, NkT("nws.newcfg"));
			ry += labGap;
			{
				const NkRect box = {rx, ry, rightW, u.s(188)};
				u.Panel(box, NkCol::surface, NkCol::border, NkR::md * u.S);
				float32 by = box.y + u.s(14);
				u.Text(box.x + u.s(14), by, NkT("ow.col.name"), NkCol::mutedFg);
				by += u.s(20);
				NkWizField(u, {box.x + u.s(14), by, rightW - u.s(28), fH}, w->newCfgName, (int32)sizeof(w->newCfgName),
						   10, w, dt, blockBg, u.s(10));
				by += fH + u.s(14);
				u.Text(box.x + u.s(14), by, NkT("nws.basedon"), NkCol::mutedFg);
				by += u.s(20);
				{
					static const char *baseOpts[] = {"Debug", "Release"};
					NkWizCombo(u, {box.x + u.s(14), by, rightW - u.s(28), fH}, 20, w, baseOpts, 2, &w->newCfgBase,
							   blockBg);
				}
				by += fH + u.s(16);
				const float32 bw = u.s(86), bh = u.s(30);
				if (u.Button({box.x + rightW - u.s(14) - bw * 2 - u.s(8), by, bw, bh}, NkT("btn.cancel"), NkCol::muted,
		...
		inline const char *const *NkWizKinds(int32 *n) {
			static const char *k[] = {"consoleapp", "windowedapp", "staticlib", "sharedlib", "test"};
			if (n)
				*n = 5;
			return k;
		}

		inline const char *const *NkWizLangs(int32 *n) {
			static const char *k[] = {"C", "C++", "ObjC", "ObjC++", "Zig", "Rust"};
			if (n)
				*n = 6;
			return k;
		}

		inline const char *const *NkWizDialects(int32 *n) {
			static const char *k[] = {"C++11", "C++14", "C++17", "C++20", "C++23"};
			if (n)
				*n = 5;
			return k;
		}

		inline const char *NkWizKindIcon(int32 k) {
			const char *ic_[] = {"terminal", "app-window", "archive", "link", "flask"};
			return (k >= 0 && k < 5) ? ic_[k] : "file";
		}
		...
		inline int32 NkWizTcOs(int32 tc) {
			switch (tc) {
				case 1:
					return 0;
				case 6:
		...
		inline float32 NkWizStep2(const NkUi &u, const NkRect &body, NkNewWsState *w, float32 dt, bool blockBg,
								  const NkIcons &ic) {
			(void)dt;
			(void)ic;
			const float32 lh = u.Lh();
			...
			{
				const float32 aw = u.s(160);
				const NkRect addR = {x + wkW - aw, y, aw, u.s(30)};
				...
				NkOwIco(u, ic.plus, "plus",
						{addR.x + (aw - tw - u.s(20)) * 0.5f, addR.y + (u.s(30) - u.s(13)) * 0.5f, u.s(13), u.s(13)},
						NkCol::primaryFg);
				u.TextV(addR.x + (aw - tw - u.s(20)) * 0.5f + u.s(20), addR.y, u.s(30), NkT("nws.addproj"),
						NkCol::primaryFg);
				if (hv && u.click)
					w->OpenProjDlg(-1);
			}
			...
			if (w->projects.Empty()) {
				...
				for (usize i = 0; i < w->projects.Size(); ++i) {
					...
					NkOwIco(u, NkWizKindTex(ic, p.kind), NkWizKindIcon(p.kind),
							{card.x + u.s(14), card.y + u.s(14), u.s(18), u.s(18)}, NkCol::accent);
					...
					{
						const float32 tw = u.s(28);
						const NkRect r = {bx - tw, card.y + u.s(12), tw, u.s(24)};
						bx -= tw + u.s(6);
						const bool hv = !blockBg && u.Hit(r);
						u.Rect(r, hv ? NkCol::hover : NkCol::muted, NkR::sm * u.S);
						NkOwIco(u, ic.corbeille, "trash", {r.x + u.s(7), r.y + u.s(5), u.s(14), u.s(14)},
								NkCol::danger);
						if (hv && u.click)
							doDel = (int32)i;
					}
					{
						...
						const bool hv = !blockBg && u.Hit(r);
						u.Rect(r, hv ? NkCol::hover : NkCol::muted, NkR::sm * u.S);
						NkOwIco(u, ic.editer, "pencil", {r.x + u.s(7), r.y + u.s(5), u.s(14), u.s(14)},
								NkCol::foreground);
						if (hv && u.click)
							doEdit = (int32)i;
					}
					{
						...
						const float32 tw = u.s(70);
						const NkRect r = {bx - tw, card.y + u.s(12), tw, u.s(24)};
						bx -= tw + u.s(6);
						const bool hv = !blockBg && u.Hit(r);
						u.Panel(r, isStart ? NkCol::secondary : (hv ? NkCol::hover : NkCol::muted),
								isStart ? NkCol::accent : NkCol::border, NkR::sm * u.S);
						NkOwIco(u, ic.star, "star", {r.x + u.s(8), r.y + u.s(6), u.s(12), u.s(12)},
								isStart ? NkCol::accent : NkCol::mutedFg);
						u.TextV(r.x + u.s(24), r.y, u.s(24), NkT("nws.start"),
								isStart ? NkCol::accent : NkCol::mutedFg);
						if (hv && u.click)
							doStar = (int32)i;
					}
					...
					const char *const *langs = NkWizLangs(&nL);
					const char *const *dials = NkWizDialects(&nD);
					const char *kind = (p.kind >= 0 && p.kind < nK) ? kinds[p.kind] : "consoleapp";
					const bool isApp = (p.kind == 0 || p.kind == 1);
					{
						const float32 pw = u.TextW(kind) + u.s(16);
						const NkRect pill = {card.x + u.s(40), card.y + u.s(40), pw, u.s(18)};
						u.Panel(pill, isApp ? NkCol::secondary : NkCol::muted, isApp ? NkCol::accent : NkCol::border,
								NkR::sm * u.S);
						u.TextV(pill.x + u.s(8), pill.y - u.s(3), u.s(18), kind,
					...
					// ligne sources/includes/depend
					{
						...
						for (usize k = 0; k < p.sources.Size(); ++k) {
							if (k)
								line += ", ";
							line += p.sources[k].CStr();
						}
						line += "   Includes: ";
						for (usize k = 0; k < p.includes.Size(); ++k) {
							if (k)
								line += ", ";
							line += p.includes[k].CStr();
						}
						if (!p.dependsOn.Empty()) {
							line += "   Depend de: ";
							for (usize k = 0; k < p.dependsOn.Size(); ++k) {
								if (k)
									line += ", ";
								line += p.dependsOn[k].CStr();
							}
						}
						u.TextEllipsis(card.x + u.s(40), card.y + u.s(64), card.w - u.s(54), line.CStr(),
									   NkCol::mutedFg);
					}
					y += ch + u.s(12);
				}
				if (doStar >= 0)
					w->startProj = (w->startProj == doStar) ? -1 : doStar;
				if (doEdit >= 0)
					w->OpenProjDlg(doEdit);
				if (doDel >= 0) {
					w->projects.Erase(w->projects.Begin() + doDel);
					if (w->startProj == doDel)
						w->startProj = -1;
					else if (w->startProj > doDel)
						w->startProj--;
				}

				// ORDRE DE BUILD
				y += u.s(10);
				NkWizLabel(u, x, y, NkT("nws.buildorder"));
				u.Text(x + u.TextW(NkT("nws.buildorder")) + u.s(8), y, NkT("nws.buildorderhint"), NkCol::mutedFg);
				y += u.s(24);
				float32 px = x;
				for (usize i = 0; i < w->projects.Size(); ++i) {
					const NkWizProject &p = w->projects[i];
					...
					u.Panel(pill, (w->dragProj == (int32)i) ? NkCol::secondary : (hv ? NkCol::hover : NkCol::muted),
							NkCol::border, NkR::md * u.S);
					NkOwIco(u, NkWizKindTex(ic, p.kind), NkWizKindIcon(p.kind),
							{pill.x + u.s(8), pill.y + u.s(8), u.s(14), u.s(14)}, NkCol::accent);
					...
					if (i + 1 < w->projects.Size()) {
						// Fleche ACCENT seulement si le projet suivant depend reellement de celui-ci ; sinon gris
						// (simple ordre).
						const NkWizProject &nxt = w->projects[i + 1];
						bool linked = false;
						for (usize k = 0; k < nxt.dependsOn.Size(); ++k)
							if (StrEq(nxt.dependsOn[k].CStr(), p.name)) {
								linked = true;
								break;
							}
						u.Icon("arrow-right", {px + u.s(6), y + u.s(9), u.s(12), u.s(12)},
							   linked ? NkCol::accent : NkCol::muted);
						px += u.s(24);
					}
				}
					...
					if (!u.down) {
						...
						for (usize i = 0; i < w->projects.Size(); ++i) {
							...
							if (u.Hit(pill) && (int32)i != w->dragProj) {
								NkWizProject t = w->projects[w->dragProj];
								w->projects.Erase(w->projects.Begin() + w->dragProj);
								w->projects.Insert(w->projects.Begin() + i, t);
								break;
							}
							qx += pw + u.s(24);
						}
				...
				y += u.s(40);
				NkWizHint(u, x, y, wkW, "Correspond a l'ordre de declaration with project(\"...\") dans le .jenga");
				y += u.s(20);
			}
		...
		inline float32 NkWizEditList(const NkUi &u, float32 x, float32 y, float32 wdt, NkVector<NkString> &items,
									 char *addBuf, int32 addCap, int32 fieldId, NkNewWsState *w, float32 dt,
									 bool blockBg, uint32 plusTex = 0) {
			...
			int32 rm = -1;
			if (items.Empty())
				u.TextV(box.x + u.s(10), box.y, u.s(28), "(vide)", NkCol::mutedFg);
			...
			NkWizField(u, {x, y, wdt - addBtnW - u.s(8), fH}, addBuf, addCap, fieldId, w, dt, blockBg, u.s(10));
			{
				...
				const bool hv = !blockBg && u.Hit(br);
				u.Rect(br, hv ? NkCol::hover : NkCol::muted, NkR::md * u.S);
				NkOwIco(u, plusTex, "plus", {br.x + u.s(14), br.y + (fH - u.s(12)) * 0.5f, u.s(12), u.s(12)},
						NkCol::foreground);
				u.TextV(br.x + u.s(30), br.y, fH, NkT("btn.add"), NkCol::foreground);
				if (hv && u.click && addBuf[0]) {
					items.PushBack(NkString(addBuf));
					addBuf[0] = '\0';
					if (w->focus == fieldId)
						w->focus = -1;
				}
		...
		inline const char *const *NkWizTcs(int32 *n) {
			static const char *k[] = {"(auto)",		"clang-mingw",	"zig-linux-x64", "android-ndk",
									  "emscripten", "clang-native", "clang-cl"};
			if (n)
				*n = 7;
			return k;
		...
		inline const char *const *NkWizOpt(int32 *n) {
			static const char *k[] = {"Off", "Size", "Speed", "Full"};
			if (n)
				*n = 4;
			return k;
		}

		inline const char *const *NkWizMem(int32 *n) {
			static const char *k[] = {"16 MB", "32 MB", "64 MB", "128 MB", "256 MB"};
			if (n)
				*n = 5;
			return k;
		}

		inline int32 NkWizMemMB(int32 i) {
			const int32 m[] = {16, 32, 64, 128, 256};
			return (i >= 0 && i < 5) ? m[i] : 32;
		}
		...
		inline float32 NkProjPills(const NkUi &u, float32 x, float32 y, float32 maxW, NkVector<NkString> &items,
			...
			if (w->projFltTarget == listId) {
				...
				if (cx + fw + u.s(70) > x + maxW) {
					cx = x;
					y += ph + u.s(6);
				}
				NkWizField(u, {cx, y - u.s(4), fw, u.s(26)}, w->projFltAdd, (int32)sizeof(w->projFltAdd), 200, w, dt,
						   blockBg, u.s(8));
				...
				if (commit) {
					if (w->projFltAdd[0])
						items.PushBack(NkString(w->projFltAdd));
					w->projFltAdd[0] = '\0';
					w->projFltTarget = -1;
					w->focus = -1;
				}
		...
		inline void NkCodeDrawLine(const NkUi &u, float32 x, float32 y, const NkString &line) {
			static const char *KW[] = {"with", "from", "import", "as", "True", "False", "None",
									   "and",  "or",   "not",	 "if", "else", "def",	"return"};
			const NkColor cDef = NkCol::foreground, cKw = NkColor{197, 134, 192, 255}, cFn = NkColor{86, 156, 214, 255},
						  cStr = NkColor{206, 145, 120, 255}, cNum = NkColor{181, 206, 168, 255},
						  cCom = NkColor{106, 153, 85, 255};
			const char *s = line.CStr();
			const usize n = line.Size();
			float32 cx = x;
			auto emit = [&](const NkString &seg, const NkColor &col) {
				if (seg.Empty())
					return;
				u.Text(cx, y, seg.CStr(), col);
				cx += u.TextW(seg.CStr());
			};
			...
			while (i < n) {
				...
				if (isAl(c)) {
					...
					while (k < n && s[k] == ' ')
						++k;
					const bool fn = (k < n && s[k] == '(');
					bool kw = false;
					for (const char *kw2 : KW)
						if (seg == kw2) {
							kw = true;
							break;
						}
					emit(seg, kw ? cKw : (fn ? cFn : cDef));
					i = j;
					continue;
				}
				if (c >= '0' && c <= '9') {
		...
		inline NkCodeSelSt &NkCodeSel() {
			static NkCodeSelSt s;
			return s;
		}

		inline void NkWizCodeBox(const NkUi &u, const NkRect &rect, const NkString &code, NkNewWsState *w, bool blockBg,
								 bool lineNums, int32 *caretEdit = nullptr, float32 blink = 0.f,
								 bool followCaret = false) {
			const int32 caretPos = caretEdit ? *caretEdit : -1;
			bool caretMoved =
				followCaret; // l'auto-scroll ne suit le caret QUE s'il vient de bouger (sinon la molette est annulee)
			...
			if (caretEdit && !blockBg && u.Hit(rect) && u.click) {
				int32 line = (int32)((u.mp.y - (rect.y + u.s(6)) + w->codeSY) / rowH);
				if (line < 0)
					line = 0;
				const char *s = code.CStr();
				int32 gi = 0, cur = 0;
				while (cur < line && s[gi]) {
					if (s[gi] == '\n')
						++cur;
				...
				NkString acc;
				for (int32 k = 0;; ++k) {
					const char ch = s[gi + k];
					const float32 wv = u.TextW(acc.CStr());
					const float32 d = (wv > px) ? (wv - px) : (px - wv);
					if (d < bestd) {
						bestd = d;
						col = k;
					}
			...
			int32 nLines = 1;
			for (const char *s = code.CStr(); *s; ++s)
				if (*s == '\n')
					++nLines;
			float32 maxW = 0.f;
			{
				...
				for (const char *s = code.CStr();; ++s) {
					if (*s == '\n' || *s == '\0') {
						const float32 wv = u.TextW(ln.CStr());
						if (wv > maxW)
							maxW = wv;
						ln.Clear();
						if (*s == '\0')
			...
			if (!blockBg && u.Hit(rect) && u.ctx->input.wheel != 0.f) {
				if (u.ctx->input.shiftDown)
					w->codeSX -= u.ctx->input.wheel * u.s(40);
				else
					w->codeSY -= u.ctx->input.wheel * u.s(40);
			...
			if (caretPos >= 0) {
				...
				{
					const char *s = code.CStr();
					for (int32 k = 0; k < caretPos && s[k]; ++k) {
						if (s[k] == '\n') {
							++caretLine;
							cc = 0;
						} else
				...
				if (caretMoved) {
					...
					if (cy - w->codeSY < 0.f)
						w->codeSY = cy;
					if (cy + rowH - w->codeSY > viewH - u.s(10))
						w->codeSY = cy + rowH - viewH + u.s(10);
					if (caretX - w->codeSX < 0.f)
						w->codeSX = caretX;
					if (caretX + u.s(10) - w->codeSX > viewW)
						w->codeSX = caretX + u.s(10) - viewW;
				}
			}
			if (w->codeSY < 0.f)
				w->codeSY = 0.f;
			...
			float32 ly = rect.y + u.s(6) - w->codeSY;
			int32 ln = 1;
			NkString line;
			for (const char *s = code.CStr();; ++s) {
				if (*s == '\n' || *s == '\0') {
			...
			if (caretPos >= 0 && blink < 0.5f) {
				...
				if (px >= rect.x + gutter - u.s(2) && px <= rect.x + rect.w - sbW)
					u.dl->AddRectFilled({px, py, u.s(1.5f), lh}, NkCol::foreground, 0.f);
			}
			u.dl->PopClipRect();
			if (lineNums)
				u.dl->AddRectFilled({rect.x + gutter - u.s(5), rect.y + u.s(2), 1.f, rect.h - u.s(4)}, NkCol::border,
									0.f);
			if (!u.down) {
				if (w->codeBar == 1 || w->codeBar == 2)
					w->codeBar = 0;
			} // relache le drag
			if (maxSY > 0.5f) {
		...
		inline bool NkWizCodeEditInput(const NkUi &u, NkString &buf, int32 &caret, bool blockBg) {
			...
			if (caret < 0)
				caret = 0;
			if (caret > (int32)buf.Size())
				caret = (int32)buf.Size();
			const int32 caret0 = caret;
			const usize len0 = buf.Size();
			auto eraseAt = [&](int32 i) {
				if (i < 0 || i >= (int32)buf.Size())
					return;
				NkString o;
			...
			auto insertAt = [&](int32 i, char c) {
				NkString o;
				const char *s = buf.CStr();
				for (int32 k = 0; k < i && s[k]; ++k)
					o += s[k];
				o += c;
				for (int32 k = i; s[k]; ++k)
					o += s[k];
			...
			const int32 len = (int32)buf.Size();
			const char *s0 = buf.CStr();
			if (u.ctx->input.KeyPressedRepeat(NkGuiKey::Left) && caret > 0)
				--caret;
			if (u.ctx->input.KeyPressedRepeat(NkGuiKey::Right) && caret < len)
				++caret;
			if (u.ctx->input.KeyPressed(NkGuiKey::Home)) {
				while (caret > 0 && s0[caret - 1] != '\n')
					--caret;
			}
			if (u.ctx->input.KeyPressed(NkGuiKey::End)) {
				while (caret < len && s0[caret] != '\n')
					++caret;
			}
			if (u.ctx->input.KeyPressedRepeat(NkGuiKey::Up) || u.ctx->input.KeyPressedRepeat(NkGuiKey::Down)) {
				...
				if (u.ctx->input.KeyPressedRepeat(NkGuiKey::Up)) {
					const int32 ls = caret - col;
					if (ls > 0) {
						int32 ps = ls - 1;
						while (ps > 0 && s0[ps - 1] != '\n')
							--ps;
						int32 pl = 0;
						while (s0[ps + pl] && s0[ps + pl] != '\n')
							++pl;
			...
			if (u.ctx->input.KeyPressedRepeat(NkGuiKey::Delete) && caret < (int32)buf.Size())
				eraseAt(caret);
			if (u.ctx->input.KeyPressed(NkGuiKey::Enter)) {
				insertAt(caret, '\n');
				++caret;
			}
		...
		inline void NkWizColScroll(const NkUi &u, const NkRect &rect, float32 contentH, float32 contentW, float32 &sy,
								   float32 &sx, bool blockBg) {
			...
			if (!blockBg && u.Hit(rect) && u.ctx->input.wheel != 0.f) {
				if (u.ctx->input.shiftDown)
					sx -= u.ctx->input.wheel * u.s(40);
				else
					sy -= u.ctx->input.wheel * u.s(40);
				u.ctx->input.wheel = 0.f;
			}
			if (sy < 0.f)
				sy = 0.f;
		...
		inline float32 NkProjDef(const NkUi &u, const NkRect &view, NkNewWsState *w, NkCodeDialogs *dlg, float32 dt,
								 bool blockBg, const NkIcons &ic) {
			(void)dlg;
			...
			const NkRect leftRect = {view.x + u.s(12), view.y + u.s(6), view.w - rightFullW - u.s(22),
									 view.h - u.s(12)};
			const NkRect rightRect = {view.x + view.w - rightFullW - u.s(6), view.y + u.s(6), rightFullW,
									  view.h - u.s(12)};
			...
			NkWizLabel(u, cx, y, NkT("nws.projname"));
			y += u.s(20);
			NkWizField(u, {cx, y, cw, fH}, p.name, (int32)sizeof(p.name), 100, w, dt, blockBg, u.s(10));
			y += fH + u.s(16);
			NkWizLabel(u, cx, y, NkT("nws.projkind"));
			y += u.s(22);
			{
				int32 nK = 0;
				const char *const *kinds = NkWizKinds(&nK);
				...
				for (int32 k = 0; k < nShown; ++k) {
					...
					const bool selk = (p.kind == k);
					const bool hv = !blockBg && u.Hit(c);
					u.Panel(c, selk ? NkCol::secondary : NkCol::input, selk ? NkCol::primary : NkCol::border,
							NkR::md * u.S);
					NkOwIco(u, NkWizKindTex(ic, k), NkWizKindIcon(k),
							{c.x + (kcw - u.s(20)) * 0.5f, c.y + u.s(8), u.s(20), u.s(20)},
							selk ? NkCol::primary : NkCol::mutedFg);
					const float32 tw = u.TextW(kinds[k]);
					u.Text(c.x + (kcw - tw) * 0.5f, c.y + u.s(32), kinds[k], selk ? NkCol::primary : NkCol::mutedFg);
					if (hv && u.click)
						p.kind = k;
				}
				y += kch + u.s(8);
				const char *desc[] = {NkT("nws.kind0"), NkT("nws.kind1"), NkT("nws.kind2"), NkT("nws.kind3"),
									  NkT("nws.kind4")};
				NkWizHint(u, cx, y, cw, (p.kind >= 0 && p.kind < 5) ? desc[p.kind] : "");
				y += u.s(28);
			}
			NkWizLabel(u, cx, y, NkT("nws.langdialect"));
			y += u.s(20);
			{
				int32 nL = 0, nD = 0;
				const char *const *langs = NkWizLangs(&nL);
				...
				u.Text(cx, y, NkT("gen.language"), NkCol::mutedFg);
				u.Text(cx + cwid + u.s(16), y, NkT("nws.dialect"), NkCol::mutedFg);
			...
			NkWizLabel(u, cx, y, NkT("nws.projloc"));
			y += u.s(20);
			NkWizField(u, {cx, y, cw - u.s(38), fH}, p.location, (int32)sizeof(p.location), 101, w, dt, blockBg,
					   u.s(10));
			{
				const NkRect br = {cx + cw - u.s(32), y, u.s(32), fH};
				u.Rect(br, NkCol::muted, NkR::md * u.S);
				NkOwIco(u, ic.ouvrirDossier, "folder", {br.x + u.s(9), br.y + u.s(9), u.s(14), u.s(14)},
						NkCol::mutedFg);
			}
			y += fH + u.s(16);
			NkWizLabel(u, cx, y, NkT("nws.sources"));
			y += u.s(20);
			y += NkWizEditList(u, cx, y, cw, p.sources, w->projSrcAdd, (int32)sizeof(w->projSrcAdd), 102, w, dt,
							   blockBg, ic.plus) +
				 u.s(14);
			NkWizLabel(u, cx, y, NkT("nws.includes"));
			y += u.s(20);
			y += NkWizEditList(u, cx, y, cw, p.includes, w->projIncAdd, (int32)sizeof(w->projIncAdd), 103, w, dt,
							   blockBg, ic.plus) +
				 u.s(8);
			...
			NkWizLabel(u, rx, ry, NkT("nws.builddirs"));
			ry += u.s(20);
			{
				const float32 fh2 = u.s(28);
				u.Text(rx, ry, NkT("nws.objdir"), NkCol::mutedFg);
				ry += u.s(16);
				NkWizField(u, {rx, ry, rightW, fh2}, p.objDir, (int32)sizeof(p.objDir), 130, w, dt, blockBg, u.s(10));
				ry += fh2 + u.s(8);
				u.Text(rx, ry, NkT("nws.targetdir"), NkCol::mutedFg);
				ry += u.s(16);
				NkWizField(u, {rx, ry, rightW, fh2}, p.tgtDir, (int32)sizeof(p.tgtDir), 131, w, dt, blockBg, u.s(10));
				ry += fh2 + u.s(8);
				// Bin : par defaut identique a target ; lien pour resynchroniser.
				u.Text(rx, ry, NkT("nws.bindir"), NkCol::mutedFg);
				{
					const char *lk = StrEq(p.binDir, p.tgtDir) ? "= target" : "resync = target";
					const float32 lw = u.TextW(lk);
				...
				ry += u.s(16);
				NkWizField(u, {rx, ry, rightW, fh2}, p.binDir, (int32)sizeof(p.binDir), 132, w, dt, blockBg, u.s(10));
				ry += fh2 + u.s(10);
			}
			NkWizLabel(u, rx, ry, NkT("nws.deps"));
			ry += u.s(20);
			// On ne peut dependre que d'une bibliotheque (staticlib/sharedlib), jamais d'une application.
			{
				int32 nLibs = 0;
				for (usize i = 0; i < w->projects.Size(); ++i) {
					if ((int32)i == w->projEditIdx)
						continue;
					const int32 k = w->projects[i].kind;
					if (k == 2 || k == 3)
						++nLibs;
				}
				...
				for (usize i = 0; i < w->projects.Size(); ++i) {
					...
					const int32 k = w->projects[i].kind;
					if (k != 2 && k != 3)
						continue; // seules les libs
					const char *nm = w->projects[i].name;
					bool dep = false;
					int32 di = -1;
					for (usize j = 0; j < p.dependsOn.Size(); ++j)
						if (StrEq(p.dependsOn[j].CStr(), nm)) {
							dep = true;
							di = (int32)j;
							break;
						}
					if (NkWizCheck(u, box.x + u.s(12), dy, nm, dep, blockBg)) {
						if (dep && di >= 0)
							p.dependsOn.Erase(p.dependsOn.Begin() + di);
						else
							p.dependsOn.PushBack(NkString(nm));
					}
					// icone du type de projet (a droite de la ligne)
					NkOwIco(u, NkWizKindTex(ic, k), NkWizKindIcon(k),
							{box.x + rightW - u.s(26), dy - u.s(7), u.s(14), u.s(14)},
							dep ? NkCol::accent : NkCol::mutedFg);
					dy += u.s(26);
				}
				if (nLibs == 0) {
					u.TextV(box.x + u.s(12), dy - u.s(8), u.s(22), "(aucune bibliotheque disponible)", NkCol::mutedFg);
				}
				ry = box.y + boxH + u.s(16);
			}
			NkWizLabel(u, rx, ry, NkT("nws.genfiles"));
			ry += u.s(20);
			{
				...
				if (hasMain) {
					if (NkWizCheck(u, box.x + u.s(12), gy, NkT("nws.genmain"), p.genMain, blockBg))
						p.genMain = !p.genMain;
					gy += u.s(26);
				}
				if (NkWizCheck(u, box.x + u.s(12), gy, NkT("nws.genreadme"), p.genReadme, blockBg))
					p.genReadme = !p.genReadme;
				gy += u.s(26);
				...
				if (!isTestKind) {
					...
					u.Panel(tg, p.genTest ? NkCol::secondary : NkCol::input, p.genTest ? NkCol::primary : NkCol::border,
							NkR::md * u.S);
					NkOwIco(u, ic.kTest, "check-circle",
							{tg.x + u.s(8), tg.y + (tg.h - u.s(14)) * 0.5f, u.s(14), u.s(14)},
							p.genTest ? NkCol::primary : NkCol::mutedFg);
					u.TextV(tg.x + u.s(28), tg.y, tg.h, NkT("nws.tests"),
							p.genTest ? NkCol::primary : NkCol::foreground);
					const char *st = p.genTest ? "ON" : "OFF";
					u.TextV(tg.x + tg.w - u.TextW(st) - u.s(10), tg.y, tg.h, st,
							p.genTest ? NkCol::success : NkCol::mutedFg);
					if (thv && u.click)
						p.genTest = !p.genTest;
				}
				ry += boxH + u.s(10);
			...
			if (p.genTest) {
				NkWizLabel(u, rx, ry, NkT("nws.testprops"));
				ry += u.s(20);
				const NkRect box = {rx, ry, rightW, u.s(98)};
				u.Panel(box, NkCol::surface, NkCol::border, NkR::md * u.S);
				u.Text(box.x + u.s(12), box.y + u.s(10), NkT("nws.testfiles"), NkCol::mutedFg);
				NkWizField(u, {box.x + u.s(12), box.y + u.s(28), rightW - u.s(24), u.s(28)}, p.testFiles,
						   (int32)sizeof(p.testFiles), 140, w, dt, blockBg, u.s(10));
				if (NkWizCheck(u, box.x + u.s(12), box.y + u.s(76), NkT("nws.desktoponly"), p.testDesktopOnly, blockBg))
					p.testDesktopOnly = !p.testDesktopOnly;
				ry += u.s(108);
			}
		...
		inline float32 NkProjFilters(const NkUi &u, const NkRect &view, NkNewWsState *w, float32 dt, bool blockBg,
									 const NkIcons &ic) {
			NkWizProject &p = w->projDraft;
			const float32 rightW = (view.w > u.s(760)) ? u.s(300) : u.s(250);
			const float32 fH = u.s(28), sb = u.s(10), colGap = u.s(16);
			int32 nTc = 0;
			const char *const *tcs = NkWizTcs(&nTc);
			int32 nK = 0;
			const char *const *kinds = NkWizKinds(&nK);
			...
			const float32 cw = leftRect.w - u.s(16) - sb;
			const float32 leftTop = leftRect.y + u.s(8);
			float32 leftMaxX = 0.f;
			(void)leftMaxX;
			u.dl->PushClipRect(leftRect, true);
			float32 y = leftTop - w->fltLSY;
			NkWizLabel(u, cx, y, NkT("nws.platfilters"));
			u.Text(cx + u.TextW(NkT("nws.platfilters")) + u.s(8), y, NkT("nws.filterhint"), NkCol::mutedFg);
			y += u.s(24);
			const int32 nOs = NkWizOsCount();
			int32 shownOs = 0;
			for (int32 oi = 0; oi < nOs; ++oi) {
				...
				u.Text(box.x + u.s(14) + u.TextW(NkWizOsName(oi)) + u.s(8), box.y + u.s(10), NkWizOsFilt(oi),
					   NkCol::mutedFg);
				// Bouton supprimer ce filtre systeme.
				{
					const NkRect xr = {box.x + cw - u.s(24), box.y + u.s(8), u.s(16), u.s(16)};
					const bool xhv = !blockBg && u.Hit(xr);
				...
				if (isAndroid) {
					u.Text(box.x + u.s(14), fy + u.s(6), "Type override", NkCol::mutedFg);
					NkWizCombo(u, {box.x + u.s(120), fy, u.s(150), fH}, 600 + oi * 10 + 1, w, kinds, nK, &o.androidKind,
							   blockBg);
					fy += u.s(34);
				}
				...
				{
					const int32 tcOs = NkWizTcOs(o.toolchain);
					if (tcOs >= 0 && tcOs != oi) {
						u.Icon("alert-triangle", {box.x + u.s(286), fy + u.s(6), u.s(13), u.s(13)}, NkCol::accent);
						u.TextEllipsis(box.x + u.s(304), fy + u.s(6), cw - u.s(320), NkT("nws.tcotheros"),
									   NkCol::accent);
					}
				}
				if (isWeb) {
					u.Text(box.x + u.s(286), fy + u.s(6), "Mem", NkCol::mutedFg);
					int32 nM = 0;
					const char *const *mem = NkWizMem(&nM);
					NkWizCombo(u, {box.x + u.s(322), fy, u.s(90), fH}, 600 + oi * 10 + 2, w, mem, nM, &o.webMem,
							   blockBg);
				}
				fy += u.s(34);
				u.Text(box.x + u.s(14), fy + u.s(2), "Links", NkCol::mutedFg);
				NkProjPills(u, box.x + u.s(120), fy, pillsW, o.links, oi * 2, w, dt, blockBg, ic.plus);
				fy += linksH + u.s(6);
				u.Text(box.x + u.s(14), fy + u.s(2), "Defines", NkCol::mutedFg);
				NkProjPills(u, box.x + u.s(120), fy, pillsW, o.defines, oi * 2 + 1, w, dt, blockBg, ic.plus);
				fy += defsH;
				y += inner + u.s(10);
			}
			if (!shownOs) {
				u.TextV(cx, y, u.s(22), "(aucun filtre systeme — ajoutez-en ci-dessous)", NkCol::mutedFg);
				y += u.s(28);
			}
			// Ajouter un filtre systeme : plateformes encore absentes (chips cliquables).
			{
				...
				if (anyOff) {
					NkWizLabel(u, cx, y, NkT("nws.addsysfilter"));
					y += u.s(22);
					float32 chx = cx, chy = y;
					const float32 chh = u.s(26);
					for (int32 oi = 0; oi < nOs; ++oi) {
						if (p.os[oi].on)
							continue;
						const char *nm = NkWizOsName(oi);
						const float32 cw2 = u.TextW(nm) + u.s(30);
						if (chx + cw2 > cx + cw) {
							chx = cx;
							chy += chh + u.s(6);
						}
						const NkRect b = {chx, chy, cw2, chh};
						const bool hv = !blockBg && u.Hit(b);
						u.Panel(b, hv ? NkCol::hover : NkCol::input, NkCol::border, NkR::md * u.S);
						NkOwIco(u, ic.plus, "plus", {b.x + u.s(8), b.y + (chh - u.s(11)) * 0.5f, u.s(11), u.s(11)},
								hv ? NkCol::foreground : NkCol::primary);
			...
			for (usize c = 0; c < p.custFlt.Size(); ++c) {
				...
				u.Text(box.x + u.s(14), box.y + u.s(10), NkT("nws.customfilter"),
					   cf.ok ? NkCol::success : NkCol::accent);
				if (cf.ok)
					NkOwIco(
						u, ic.valideSimple, "check",
						{box.x + u.s(14) + u.TextW(NkT("nws.customfilter")) + u.s(8), box.y + u.s(9), u.s(14), u.s(14)},
						NkCol::success);
				u.Text(box.x + u.s(14), box.y + u.s(32), NkT("nws.expression"), NkCol::mutedFg);
				NkWizField(u, {box.x + u.s(14), box.y + u.s(46), cw - u.s(28), u.s(26)}, cf.expr,
						   (int32)sizeof(cf.expr), 400 + (int32)c, w, dt, blockBg, u.s(10));
				// Toolchain (OS-dependante = traitement special).
				u.Text(box.x + u.s(14), box.y + u.s(82), "Toolchain", NkCol::mutedFg);
				NkWizCombo(u, {box.x + u.s(90), box.y + u.s(78), u.s(160), fH}, 700 + (int32)c, w, tcs, nTc,
						   &cf.toolchain, blockBg);
				{
				...
				NkProjPills(u, box.x + u.s(90), box.y + u.s(114), cw - u.s(110), cf.defines, 500 + (int32)c * 2, w, dt,
							blockBg, ic.plus);
				u.Text(box.x + u.s(14), box.y + u.s(148), "Links", NkCol::mutedFg);
				NkProjPills(u, box.x + u.s(90), box.y + u.s(146), cw - u.s(110), cf.links, 500 + (int32)c * 2 + 1, w,
							dt, blockBg, ic.plus);
				...
				NkOwIco(u, ic.valideSimple, "check", {vr.x + u.s(14), vr.y + (bh - u.s(14)) * 0.5f, u.s(14), u.s(14)},
						NkCol::primaryFg);
				...
				u.Icon("trash", {dr.x + u.s(14), dr.y + (bh - u.s(14)) * 0.5f, u.s(14), u.s(14)},
					   dhv ? NkCol::primaryFg : NkCol::danger);
				u.TextV(dr.x + u.s(34), dr.y, bh, NkT("tc.delete"), dhv ? NkCol::primaryFg : NkCol::foreground);
				if (dhv && u.click) {
					p.custFlt.Erase(p.custFlt.Begin() + c);
					break;
				}
				y += u.s(224);
			}
			{
				const NkRect ar = {cx, y, u.s(240), u.s(28)};
				const bool hv = !blockBg && u.Hit(ar);
				NkOwIco(u, ic.plus, "plus", {ar.x, ar.y + u.s(8), u.s(12), u.s(12)},
						hv ? NkCol::foreground : NkCol::primary);
				u.TextV(ar.x + u.s(18), ar.y, u.s(28), NkT("nws.addcustomfilter"),
						hv ? NkCol::foreground : NkCol::primary);
				if (hv && u.click) {
					NkWizProject::CustFlt cf;
					p.custFlt.PushBack(cf);
				}
				y += u.s(34);
			}
			...
			int32 nO = 0;
			const char *const *opt = NkWizOpt(&nO);
			...
			for (usize c = 0; c < p.cfgFlt.Size(); ++c) {
				const NkWizProject::CfgFlt &cf = p.cfgFlt[c];
				for (usize d = 0; d < cf.defines.Size(); ++d) {
					const float32 wv = u.s(92) + u.TextW(cf.defines[d].CStr()) + u.s(50);
					if (wv > cfgBoxW)
						cfgBoxW = wv;
				}
			}
			...
			for (usize c = 0; c < p.cfgFlt.Size(); ++c) {
				...
				NkWizCombo(u, {box.x + u.s(80), cyy, u.s(110), fH}, 360 + (int32)c, w, opt, nO, &cf.opt, blockBg);
				cyy += u.s(34);
				u.Text(box.x + u.s(12), cyy + u.s(4), "Symbols", NkCol::mutedFg);
				if (NkWizCheck(u, box.x + u.s(80), cyy + u.s(10), "Activer", cf.sym, blockBg))
					cf.sym = !cf.sym;
				ry += boxH + u.s(10);
			}
			if (p.cfgFlt.Empty()) {
				u.TextV(crx, ry, u.s(22), "(aucune configuration cochee a l'etape 1)", NkCol::mutedFg);
				ry += u.s(26);
			}
			...
			NkWizLabel(u, rLabelX, apY, NkT("nws.codepreview"));
			{
				...
				float32 apH = (rightRect.y + rightRect.h) - apBoxY - u.s(2);
				if (apH < u.s(120))
					apH = u.s(120);
		...
		inline float32 NkProjSummary(const NkUi &u, const NkRect &body, NkNewWsState *w, float32 dt, bool blockBg,
									 const NkIcons &ic) {
			NkWizProject &p = w->projDraft;
			...
			NkWizLabel(u, cx, y, NkT("nws.insertformat"));
			y += u.s(22);
			{
				...
				for (int32 i = 0; i < 2; ++i) {
					...
					const bool selb = (p.separateFile == opts[i].sep);
					const bool hv = !blockBg && u.Hit(b);
					u.Panel(b, selb ? NkCol::secondary : (hv ? NkCol::hover : NkCol::input),
							selb ? NkCol::primary : NkCol::border, NkR::md * u.S);
					u.TextV(b.x + (bw - u.TextW(opts[i].lab)) * 0.5f, b.y, u.s(34), opts[i].lab,
							selb ? NkCol::primary : NkCol::foreground);
					// Changer de format -> revenir au code genere du nouveau format + remonter (imports visibles).
					if (hv && u.click && p.separateFile != opts[i].sep) {
						p.separateFile = opts[i].sep;
						w->codeSX = w->codeSY = 0.f;
						w->extractEdited = false;
						w->extractEditing = false;
						p.hasCustomCode = false;
						p.customCode.Clear();
					}
				}
				y += u.s(44);
			...
			// Boutons : Modifier/Terminer + Regenerer (a droite du libelle).
			{
				...
				const float32 ew = u.TextW(el) + u.s(30);
				const NkRect er = {cx + cw - ew, y - u.s(5), ew, u.s(22)};
				const bool ehv = !blockBg && u.Hit(er);
				u.Rect(er, w->extractEditing ? NkCol::success : (ehv ? NkCol::hover : NkCol::muted), NkR::md * u.S);
				NkOwIco(u, w->extractEditing ? ic.valideSimple : ic.editer, w->extractEditing ? "check" : "edit",
						{er.x + u.s(8), er.y + u.s(4), u.s(13), u.s(13)},
						w->extractEditing ? NkCol::primaryFg : NkCol::foreground);
				u.TextV(er.x + u.s(24), er.y, u.s(22), el, w->extractEditing ? NkCol::primaryFg : NkCol::foreground);
				if (ehv && u.click) {
					w->focusClaimed = true;
					if (!w->extractEditing) {
						w->extractBuf = p.separateFile ? w->BuildProjectFile(p) : w->BuildProjectJenga(p);
						w->extractCaret = 0;
						w->extractEditing = true;
						w->extractEdited = true;
						w->codeSX = w->codeSY = 0.f;
					} else
						w->extractEditing = false;
				}
				if (w->extractEdited) {
					const char *rl = "Regenerer";
					const float32 rw = u.TextW(rl) + u.s(16);
					const NkRect rr = {cx + cw - ew - rw - u.s(8), y - u.s(5), rw, u.s(22)};
					const bool rhv = !blockBg && u.Hit(rr);
			...
			if (w->extractEditing) {
				...
				const NkString code = w->extractEdited
										  ? w->extractBuf
										  : (p.separateFile ? w->BuildProjectFile(p) : w->BuildProjectJenga(p));
			...
			NkWizLabel(u, rx, ry, NkT("nws.willcreate"));
			ry += u.s(22);
			auto created = [&](const char *path) {
				const NkRect b = {rx, ry, rightW, u.s(30)};
				u.Panel(b, NkCol::surface, NkCol::border, NkR::md * u.S);
				NkOwIco(u, ic.valideSimple, "check-circle", {b.x + u.s(10), b.y + u.s(8), u.s(14), u.s(14)},
						NkCol::success);
			...
			ry += u.s(22);
			auto chip = [&](uint32 tex, const char *drawn, const char *txt) {
				const NkRect b = {rx, ry, rightW, u.s(30)};
				u.Panel(b, NkCol::surface, NkCol::border, NkR::md * u.S);
				NkOwIco(u, tex, drawn, {b.x + u.s(10), b.y + u.s(8), u.s(14), u.s(14)}, NkCol::accent);
				u.TextV(b.x + u.s(32), b.y, u.s(30), txt, NkCol::foreground);
				ry += u.s(36);
			};
			int32 nK = 0, nD = 0;
			const char *const *kinds = NkWizKinds(&nK);
		...
		inline void NkWizProjDialog(const NkUi &u, const NkRect &r, NkNewWsState *w, NkCodeDialogs *dlg, float32 dt,
									bool blockBg, const NkIcons &ic) {
			NkWizProject &p = w->projDraft;
			...
			// En-tete
			NkOwIco(u, ic.nouveau, "plus-circle", {modal.x + u.s(22), modal.y + u.s(14), u.s(16), u.s(16)},
					NkCol::primary);
			...
			u.Rect({modal.x + u.s(16), modal.y + u.s(36), mw - u.s(32), 1.f},
				   NkCol::border); // separateur sous l'en-tete
			...
			switch (w->projStep) {
				case 0:
					contentH = NkProjDef(u, view, w, dlg, dt, blockBg, ic);
					break; // colonnes a scroll independant
				case 1:
					contentH = NkProjFilters(u, view, w, dt, blockBg, ic);
					break; // colonnes a scroll independant
				case 2:
					contentH = NkProjSummary(u, bodyR, w, dt, blockBg, ic);
					break;
				default:
					break;
			}
			...
			if (u.click && !blockBg && !w->focusClaimed && u.Hit(view)) {
				w->focus = -1;
				w->extractEditing = false;
			}
			...
			if (u.Hit(view) && u.ctx->input.wheel != 0.f && !blockBg) {
				w->projScroll -= u.ctx->input.wheel * u.s(40);
				u.ctx->input.wheel = 0.f;
				if (w->projScroll < 0.f)
					w->projScroll = 0.f;
			...
			const int32 fa = NkWizFooter(u, {modal.x, modal.y + mh - footH, mw, footH}, w->projStep, 2, blockBg, valid,
										 NkT("nws.create.proj"), true, ic.valideSimple);
			if (fa == 1) {
				w->projDlg = false;
				w->focus = -1;
			} else if (fa == -1 && w->projStep > 0) {
				w->projStep--;
				w->focus = -1;
				w->projScroll = 0.f;
				w->codeSX = w->codeSY = 0.f;
				w->fltLSY = w->fltLSX = w->fltRSY = w->fltRSX = 0.f;
				w->cfgSY = w->cfgSX = 0.f;
			} else if (fa == 3 && valid)
				w->CommitProjDlg(); // Creer maintenant (defauts)
			else if (fa == 2 && valid) {
				if (w->projStep < 2) {
					w->projStep++;
					w->focus = -1;
		...
		inline float32 NkWizStep3(const NkUi &u, const NkRect &body, NkNewWsState *w, float32 dt, bool blockBg,
			...
			auto hasTarget = [&](const char *key) -> bool {
				for (int32 i = 0; i < nTc; ++i)
					if (w->tcDet[i].found && w->tcDet[i].target.Contains(key))
						return true;
				return false;
			};
			...
			y += u.s(22);
			{
				...
				u.Panel(box, NkCol::surface, NkCol::border, NkR::md * u.S);
				if (NkWizCheck(u, box.x + u.s(14), box.y + u.s(24),
							   "RegisterJengaGlobalToolchains() - Detection automatique au demarrage", w->tcAuto,
							   blockBg))
					w->tcAuto = !w->tcAuto;
				y += u.s(48) + u.s(20);
			}
			...
			}
			{
				...
				const bool hv = !blockBg && u.Hit(rr);
			...
			y += u.s(22);
			{
				...
				for (int32 i = 0; i < nTc; ++i) {
					...
					NkOwIco(u, t.found ? ic.valideSimple : 0u, t.found ? "check-circle" : "alert-triangle",
							{box.x + u.s(14), ry2 + u.s(13), u.s(16), u.s(16)}, tint);
					// Colonnes avec ellipsis pour ne JAMAIS deborder de la case (largeurs bornees, sans chevauchement).
					u.TextEllipsis(box.x + u.s(40), ry2 + u.s(5), cw * 0.36f - u.s(46), t.name.CStr(),
								   t.found ? NkCol::foreground : NkCol::mutedFg);
					u.TextEllipsis(box.x + u.s(40), ry2 + u.s(22), cw * 0.36f - u.s(46), t.version.CStr(),
								   NkCol::mutedFg);
					u.TextEllipsis(box.x + cw * 0.38f, ry2 + u.s(13), cw * 0.26f, t.target.CStr(), NkCol::mutedFg);
					u.TextEllipsis(box.x + cw * 0.66f, ry2 + u.s(13), cw * 0.32f - u.s(8), t.path.CStr(),
								   t.found ? NkCol::mutedFg : NkCol::danger);
					ry2 += rowH;
					if (i + 1 < nTc)
						u.Rect({box.x + u.s(10), ry2 - u.s(1), cw - u.s(20), 1.f}, NkCol::border);
				}
				y = box.y + box.h + u.s(20);
			}
			// TOOLCHAINS SELECTIONNES
			NkWizLabel(u, cx, y, "TOOLCHAINS SELECTIONNES POUR CE WORKSPACE");
			y += u.s(22);
			{
				const int32 rows = (nTc + 2) / 3;
				const NkRect box = {cx, y, cw, u.s(16) + (rows > 0 ? rows : 1) * u.s(30)};
				u.Panel(box, NkCol::surface, NkCol::border, NkR::md * u.S);
				...
				for (int32 i = 0; i < nTc; ++i) {
					...
					if (w->tcDet[i].found) {
						if (NkWizCheck(u, bxx, byy, w->tcDet[i].name.CStr(), w->tcSel[i], blockBg))
							w->tcSel[i] = !w->tcSel[i];
					} else {
						NkWizCheck(u, bxx, byy, w->tcDet[i].name.CStr(), false, true);
					} // indisponible -> grise
				}
				y = box.y + box.h + u.s(20);
			}
			...
			{
				const float32 aw = u.s(150);
				const NkRect addR = {cx + cw - aw, y - u.s(4), aw, u.s(26)};
				const bool hv = !blockBg && u.Hit(addR);
				...
				if (hv && u.click) {
					w->tcEditIdx = -1;
					w->tcDraft = NkNewWsState::CustomTc{};
					w->tcDlg = true;
					w->tcDlgScroll = 0.f;
					w->tcDlgScrollX = 0.f;
					w->tcTested = false;
			...
			y += u.s(26);
			{
				const float32 rh = u.s(30);
				const float32 ch = u.s(14) + (w->customTc.Empty() ? u.s(22) : w->customTc.Size() * rh);
				const NkRect box = {cx, y, cw, ch};
				u.Panel(box, NkCol::surface, NkCol::border, NkR::md * u.S);
				if (w->customTc.Empty())
					u.TextV(box.x + u.s(14), box.y, ch, "(aucun toolchain custom defini)", NkCol::mutedFg);
				else {
					float32 dy = box.y + u.s(8);
					int32 rm = -1, ed = -1;
					for (usize i = 0; i < w->customTc.Size(); ++i) {
						NkOwIco(u, ic.toolchains, "cpu", {box.x + u.s(12), dy + u.s(3), u.s(14), u.s(14)},
								NkCol::accent);
						u.TextEllipsis(box.x + u.s(34), dy, cw - u.s(90), w->customTc[i].name, NkCol::foreground);
						// Bouton EDITER
						const NkRect er = {box.x + cw - u.s(48), dy - u.s(3), u.s(18), u.s(18)};
						const bool ehv = !blockBg && u.Hit(er);
						NkOwIco(u, ic.editer, "edit", {er.x + u.s(3), er.y + u.s(3), u.s(13), u.s(13)},
								ehv ? NkCol::primary : NkCol::mutedFg);
						if (ehv && u.click)
							ed = (int32)i;
						// Bouton SUPPRIMER
						const NkRect xr = {box.x + cw - u.s(24), dy - u.s(3), u.s(18), u.s(18)};
						const bool xhv = !blockBg && u.Hit(xr);
						NkOwIco(u, ic.corbeille, "x", {xr.x + u.s(3), xr.y + u.s(3), u.s(13), u.s(13)},
								xhv ? NkCol::danger : NkCol::mutedFg);
						if (xhv && u.click)
							rm = (int32)i;
						dy += rh;
					}
					...
					if (rm >= 0)
						w->customTc.Erase(w->customTc.Begin() + rm);
				}
				y += ch + u.s(22);
			}
			...
			NkWizLabel(u, rx, ry, "TOOLCHAIN PAR PLATEFORME");
			ry += u.s(26);
			(void)hasTarget;
			...
			auto rowTc = [&](const char *os, int32 platIdx, int32 id) {
				...
				if (w->tcPlatN[platIdx] > 0)
					NkWizCombo(u, {rx + u.s(86), ry, rightW - u.s(86), u.s(30)}, id, w, w->tcPlatPtrs[platIdx],
							   w->tcPlatN[platIdx], &w->tcPlatSel[platIdx], blockBg);
				else
					u.TextV(rx + u.s(86), ry, u.s(30), "Aucun toolchain detecte", NkCol::mutedFg);
				ry += u.s(38);
			};
			rowTc("Windows", 0, 800);
			rowTc("Linux", 1, 801);
			...
			ry += u.s(8);
			NkWizHint(u, rx, ry, rightW, "Genere usetoolchain(\"xxx\") dans with filter(\"system:X\")");
			ry += u.s(24);
		...
		inline const char *const *NkWizTcFam(int32 *n) {
			static const char *k[] = {"Clang", "GCC", "MSVC", "NDK", "Emscripten", "Zig", "SDK", "OHOS-SDK (HarmonyOS)",
									  "DevEco"};
			if (n)
				*n = 9;
			return k;
		...
		inline const char *const *NkWizTcOsList(int32 *n) {
			static const char *k[] = {"Windows", "Linux", "macOS", "Android", "iOS", "Web", "HarmonyOS"};
			if (n)
				*n = 7;
			return k;
		}

		// Architectures / ABI CONFORMES au systeme choisi (os index = NkWizTcOsList : 0 Win,1 Linux,2 mac,3 Android,4
		// iOS,5 Web,6 HarmonyOS).
		...
		inline void NkWizTcDialog(const NkUi &u, const NkRect &r, NkNewWsState *w, NkCodeDialogs *dlg, float32 dt,
								  bool blockBg, const NkIcons &ic) {
			...
			u.Text(modal.x + u.s(46), modal.y + u.s(16), tcEdit ? "EDITER LE TOOLCHAIN" : "AJOUTER UN TOOLCHAIN MANUEL",
				   NkCol::foreground);
			{
				const NkRect xr = {modal.x + mw - u.s(30), modal.y + u.s(14), u.s(18), u.s(18)};
				const bool xhv = u.Hit(xr);
			...
			const float32 cw =
				(leftRect.x + leftRect.w - u.s(10) - sb) - (leftRect.x + u.s(20)); // largeur contenu (fixe)
			const float32 leftTop = leftRect.y + u.s(12);
			u.dl->PushClipRect(leftRect, true);
			float32 y = leftTop - w->tcDlgScroll;
			int32 nF = 0, nO = 0;
			const char *const *FAM = NkWizTcFam(&nF);
			const char *const *OSL = NkWizTcOsList(&nO);
			NkWizLabel(u, cx, y, "IDENTITE");
			y += u.s(20);
			u.Text(cx, y + u.s(7), "Nom", NkCol::mutedFg);
			NkWizField(u, {cx + u.s(110), y, cw - u.s(110), fH}, t.name, (int32)sizeof(t.name), 900, w, dt, bg,
					   u.s(10));
			y += fH + u.s(12);
			...
			NkWizField(u, {cx + u.s(372), y, cw - u.s(372), fH}, t.triple, (int32)sizeof(t.triple), 901, w, dt, bg,
					   u.s(10));
			y += fH + u.s(24);
			NkWizLabel(u, cx, y, "CIBLE");
			y += u.s(22);
			// Arch (ABI) et Env DEPENDENT du systeme choisi (ex: Android -> ABI arm64-v8a...). On reclampe a chaque
			// changement.
			int32 nA = 0, nE = 0;
			const char *const *ARC = NkWizTcArchFor(t.os, &nA);
			const char *const *ENV = NkWizTcEnvFor(t.os, &nE);
			if (t.arch >= nA || t.arch < 0)
				t.arch = 0;
			if (t.env >= nE || t.env < 0)
				t.env = 0;
			...
			NkWizCombo(u, {cx + u.s(340), y, u.s(170), fH}, 912, w, ARC, nA, &t.arch, bg);
			y += fH + u.s(10);
			...
			u.TextEllipsis(cx + u.s(344), y + u.s(7), cw - u.s(344),
						   "ABI/runtime (libc) - dernier champ du triple LLVM", NkCol::mutedFg);
			y += fH + u.s(24);
			u.Text(cx, y, "CHEMINS DES OUTILS", NkCol::mutedFg);
			u.Text(cx + u.TextW("CHEMINS DES OUTILS") + u.s(10), y, "- compilateurs = fichiers, sysroot = dossier",
				   NkCol::mutedFg);
			y += u.s(22);
			...
			y += u.s(18);
			NkWizLabel(u, cx, y, "FLAGS SUPPLEMENTAIRES");
			y += u.s(22);
			u.Text(cx, y + u.s(2), "cflags", NkCol::mutedFg);
			NkProjPills(u, cx + u.s(90), y, cw - u.s(100), t.cflags, 900, w, dt, bg, ic.plus);
			y += NkProjPillsH(u, cw - u.s(100), t.cflags, w->projFltTarget == 900) + u.s(6);
			u.Text(cx, y + u.s(2), "cxxflags", NkCol::mutedFg);
			NkProjPills(u, cx + u.s(90), y, cw - u.s(100), t.cxxflags, 901, w, dt, bg, ic.plus);
			y += NkProjPillsH(u, cw - u.s(100), t.cxxflags, w->projFltTarget == 901) + u.s(6);
			u.Text(cx, y + u.s(2), "ldflags", NkCol::mutedFg);
			NkProjPills(u, cx + u.s(90), y, cw - u.s(100), t.ldflags, 902, w, dt, bg, ic.plus);
			y += NkProjPillsH(u, cw - u.s(100), t.ldflags, w->projFltTarget == 902) + u.s(6);
			...
			NkWizLabel(u, rx, ry, "TESTER");
			ry += u.s(20);
			{
				const NkRect br = {rx, ry, rightW, u.s(32)};
				const bool hv = !bg && u.Hit(br);
			...
			{
				const NkRect box = {rx, ry, rightW, u.s(100)};
				u.Panel(box, NkCol::surface, NkCol::border, NkR::md * u.S);
				if (!w->tcTested) {
					u.TextV(box.x + u.s(12), box.y, u.s(100), "(cliquez sur Lancer la verification)", NkCol::mutedFg);
				} else {
					struct Chk {
							const char *lab;
							const char *path;
					...
					const Chk items[] = {{"Compilateur C", t.cc, false},
										 {"Compilateur C++", t.cxx, false},
										 {"Archiver", t.ar, false},
										 {"Linker", t.ld, false},
										 {"Sysroot", t.sysroot, true}};
					...
					if (!bg && u.Hit(box) && u.ctx->input.wheel != 0.f) {
						w->tcTestScroll -= u.ctx->input.wheel * u.s(30);
						u.ctx->input.wheel = 0.f;
					}
					if (w->tcTestScroll < 0.f)
						w->tcTestScroll = 0.f;
					if (w->tcTestScroll > maxS)
						w->tcTestScroll = maxS;
					...
					u.dl->PopClipRect();
					if (maxS > 0.5f) {
						const NkRect tr = {box.x + box.w - sbW - u.s(2), box.y + u.s(2), sbW, box.h - u.s(4)};
						u.dl->AddRectFilled(tr, NkColor{18, 21, 26, 150}, sbW * 0.5f);
						float32 th = box.h * (box.h / contentH);
						if (th < u.s(20))
							th = u.s(20);
						const float32 tt = tr.y + (tr.h - th) * (w->tcTestScroll / maxS);
						u.dl->AddRectFilled({tr.x + u.s(1), tt, sbW - u.s(2), th}, NkColor{70, 76, 84, 255},
											sbW * 0.5f);
					}
				}
				ry += u.s(108);
			}
			NkWizLabel(u, rx, ry, "ENREGISTREMENT");
			ry += u.s(20);
			{
				const NkRect box = {rx, ry, rightW, u.s(80)};
				u.Panel(box, NkCol::surface, NkCol::border, NkR::md * u.S);
				if (NkWizCheck(u, box.x + u.s(12), box.y + u.s(16), "Registre global", t.regGlobal, bg))
					t.regGlobal = !t.regGlobal;
				u.TextEllipsis(box.x + u.s(34), box.y + u.s(32), rightW - u.s(44), "~/.jenga/toolchains_registry.json",
							   NkCol::mutedFg);
				// Consequence du registre global : disponible dans tous les workspaces.
				NkOwIco(u, t.regGlobal ? ic.valideSimple : 0u, t.regGlobal ? "check" : "minus",
						{box.x + u.s(14), box.y + u.s(54), u.s(13), u.s(13)},
						t.regGlobal ? NkCol::success : NkCol::mutedFg);
				u.TextEllipsis(box.x + u.s(34), box.y + u.s(52), rightW - u.s(44),
							   "Disponible dans tous les workspaces", t.regGlobal ? NkCol::foreground : NkCol::mutedFg);
				ry += u.s(88);
			}
			NkWizLabel(u, rx, ry, "APERCU .JENGA");
			ry += u.s(20);
			{
			...
			{
				const float32 bw = u.TextW(okLbl) + u.s(50);
				const NkRect ar2 = {modal.x + mw - u.s(20) - bw, by, bw, u.s(34)};
				...
				if (hv && u.click) {
					if (tcEdit && w->tcEditIdx < (int32)w->customTc.Size())
						w->customTc[w->tcEditIdx] = t; // remplace l'existant
					else
						w->customTc.PushBack(t); // nouveau
					w->tcDlg = false;
					w->tcEditIdx = -1;
					w->focus = -1;
				}
			}
			{
				...
				const NkRect cr = {modal.x + mw - u.s(20) - (u.TextW(okLbl) + u.s(50)) - u.s(8) - bw, by, bw, u.s(34)};
				if (u.Button(cr, "Annuler", NkCol::muted, NkCol::hover, NkCol::foreground, NkR::md * u.S) && !bg) {
					w->tcDlg = false;
					w->tcEditIdx = -1;
					w->focus = -1;
				}
		...
		// ── ETAPE 4 — Plateformes & Architectures ──
		inline const char *const *NkWizApiLvl(int32 *n) {
			static const char *k[] = {"21", "23", "24", "26", "28", "30", "31", "33", "34", "35"};
			if (n)
				*n = 10;
			return k;
		}

		inline const char *const *NkWizEmMem(int32 *n) {
			static const char *k[] = {"16 MB", "32 MB", "64 MB", "128 MB", "256 MB"};
			if (n)
				*n = 5;
			return k;
		...
		inline const char *const *NkWizEmStack(int32 *n) {
			static const char *k[] = {"1 MB", "2 MB", "4 MB", "8 MB", "16 MB"};
			if (n)
				*n = 5;
			return k;
		}

		inline float32 NkWizStep4(const NkUi &u, const NkRect &body, NkNewWsState *w, float32 dt, bool blockBg,
								  const NkIcons &ic) {
			(void)dt;
			if (w->tcDetDone)
				w->RefreshTcPtrs(); // pointeurs frais (le statut toolchain par OS s'appuie dessus)
			const float32 padL = u.s(28);
			const float32 rightW = (body.w > u.s(760)) ? u.s(280) : u.s(240);
			const float32 cx = body.x + padL;
			const float32 rx = body.x + body.w - u.s(28) - rightW;
			...
			// Table des OS cibles : nom, enum, categorie (0 Desktop,1 Mobile,2 Web,3 Consoles), plateforme de
			// detection, flag, message si indispo, icone (1 warn / 2 lock).
			struct OsT {
					const char *name;
					const char *en;
					int32 cat;
			...
			// ===== COLONNE GAUCHE =====
			float32 y = body.y + u.s(20);
			NkWizLabel(u, cx, y, "SYSTEMES D'EXPLOITATION CIBLES (targetoses)");
			y += u.s(22);
			{
				...
				for (int32 cat = 0; cat < 4; ++cat) {
					...
					u.dl->AddRectFilled({box.x + u.s(4), by, cw - u.s(8), headerH - u.s(4)}, NkCol::muted,
										NkR::sm * u.S);
					...
					for (int32 i = 0; i < nOss; ++i) {
						...
						if (avail) {
							if (NkWizCheck(u, box.x + u.s(16), by + u.s(13), o.name, *o.flag, blockBg))
								*o.flag = !*o.flag;
						} else {
							*o.flag = false;
							NkWizCheck(u, box.x + u.s(16), by + u.s(13), o.name, false, true);
						}
						// enum cible
						...
						if (hasTc) {
							...
							NkOwIco(u, o.ri == 2 ? ic.lock : 0u, o.ri == 2 ? "lock" : "alert-triangle",
									...
									o.ri == 2 ? NkCol::mutedFg : NkCol::accent);
							u.TextEllipsis(box.x + cw - u.s(174), by + u.s(6), u.s(166),
										   o.req ? o.req : "Non configure", NkCol::mutedFg);
						}
						by += rowH;
					}
				}
			...
			NkWizLabel(u, cx, y, "ARCHITECTURES CIBLES (targetarchs)");
			y += u.s(22);
			{
				struct ArchT {
						const char *name;
				...
				const ArchT archs[] = {
					{"x86_64", "TargetArch.X86_64", &w->archX64},	 {"ARM64", "TargetArch.ARM64", &w->archArm64},
					{"WASM32", "TargetArch.WASM32", &w->archWasm32}, {"x86", "TargetArch.X86", &w->archX86},
					{"ARM", "TargetArch.ARM", &w->archArm},			 {"RISCV64", "TargetArch.RISCV64", &w->archRiscv64},
					{"WASM64", "TargetArch.WASM64", &w->archWasm64}, {"MIPS", "TargetArch.MIPS", &w->archMips},
				};
				const int32 nArch = (int32)(sizeof(archs) / sizeof(archs[0]));
				const int32 cols = 3;
				const int32 rows = (nArch + cols - 1) / cols;
				...
				for (int32 i = 0; i < nArch; ++i) {
					...
					if (NkWizCheck(u, ax, ay, archs[i].name, *archs[i].flag, blockBg))
						*archs[i].flag = !*archs[i].flag;
			...
			NkWizLabel(u, rx, ry, "ABIS ANDROID");
			ry += u.s(22);
			{
				...
				if (NkWizCheck(u, box.x + u.s(14), box.y + u.s(18), "arm64-v8a", w->abiArm64v8a, dis))
					w->abiArm64v8a = !w->abiArm64v8a;
				if (NkWizCheck(u, box.x + u.s(140), box.y + u.s(18), "x86_64", w->abiX86_64, dis))
					w->abiX86_64 = !w->abiX86_64;
				if (NkWizCheck(u, box.x + u.s(14), box.y + u.s(42), "armeabi-v7a", w->abiArmeabi, dis))
					w->abiArmeabi = !w->abiArmeabi;
				if (NkWizCheck(u, box.x + u.s(140), box.y + u.s(42), "x86", w->abiX86, dis))
					w->abiX86 = !w->abiX86;
				int32 nApi = 0;
				const char *const *API = NkWizApiLvl(&nApi);
				u.Text(box.x + u.s(14), box.y + u.s(66), "API Min", NkCol::mutedFg);
				NkWizCombo(u, {box.x + rightW - u.s(86), box.y + u.s(62), u.s(72), u.s(26)}, 820, w, API, nApi,
						   &w->androidApiMin, dis);
				u.Text(box.x + u.s(14), box.y + u.s(94), "API Target", NkCol::mutedFg);
				NkWizCombo(u, {box.x + rightW - u.s(86), box.y + u.s(90), u.s(72), u.s(26)}, 821, w, API, nApi,
						   &w->androidApiTarget, dis);
				u.Text(box.x + u.s(14), box.y + u.s(122), "Compile SDK", NkCol::mutedFg);
				NkWizCombo(u, {box.x + rightW - u.s(86), box.y + u.s(118), u.s(72), u.s(26)}, 822, w, API, nApi,
						   &w->androidCompileSdk, dis);
				u.Text(box.x + u.s(14), box.y + u.s(146), "App ID", NkCol::mutedFg);
				NkWizField(u, {box.x + u.s(72), box.y + u.s(142), rightW - u.s(86), u.s(26)}, w->androidAppId,
						   (int32)sizeof(w->androidAppId), 150, w, dt, dis, u.s(8));
				ry += u.s(176);
			}
			// OPTIONS EMSCRIPTEN (si Web coche)
			NkWizLabel(u, rx, ry, "OPTIONS EMSCRIPTEN (Web)");
			ry += u.s(22);
			{
				...
				const bool dis = !on || blockBg;
				int32 nM = 0, nS = 0;
				const char *const *MEM = NkWizEmMem(&nM);
				const char *const *STK = NkWizEmStack(&nS);
				u.Text(box.x + u.s(14), box.y + u.s(18), "Initial Memory", NkCol::mutedFg);
				NkWizCombo(u, {box.x + rightW - u.s(96), box.y + u.s(14), u.s(82), u.s(26)}, 823, w, MEM, nM,
						   &w->emInitMem, dis);
				u.Text(box.x + u.s(14), box.y + u.s(46), "Stack Size", NkCol::mutedFg);
				NkWizCombo(u, {box.x + rightW - u.s(96), box.y + u.s(42), u.s(82), u.s(26)}, 824, w, STK, nS,
						   &w->emStackSize, dis);
				u.Text(box.x + u.s(14), box.y + u.s(74), "Export Name", NkCol::mutedFg);
				NkWizField(u, {box.x + u.s(96), box.y + u.s(70), rightW - u.s(110), u.s(26)}, w->emExportName,
						   (int32)sizeof(w->emExportName), 151, w, dt, dis, u.s(8));
				ry += u.s(110);
			}
		...
		inline float32 NkWizStep5(const NkUi &u, const NkRect &body, NkNewWsState *w, float32 dt, bool blockBg,
								  const NkIcons &ic) {
			if (w->tcDetDone)
				w->RefreshTcPtrs(); // BuildJenga lit tcPlatPtrs -> on garantit des pointeurs valides (anti-crash)
			...
			for (usize i = 0; i < w->projects.Size(); ++i)
				if (w->projects[i].separateFile)
					sepProj.PushBack((int32)i);
			const int32 nTabs = 1 + (int32)sepProj.Size();
			if (w->wsTab < 0 || w->wsTab >= nTabs)
				w->wsTab = 0;
			const bool isWs = (w->wsTab == 0);
			// Bouton Copier (toujours) + Editer (onglet workspace uniquement).
			{
				const char *cl = "Copier";
				const float32 cwd = u.TextW(cl) + u.s(34);
				const NkRect cr = {cx + cw - cwd, y - u.s(4), cwd, u.s(24)};
				const bool chv = !blockBg && u.Hit(cr);
				...
				NkString curCode = isWs ? (w->wsExtractEdited ? w->wsExtractBuf : w->BuildJenga())
										: w->BuildProjectFile(w->projects[sepProj[w->wsTab - 1]]);
				if (chv && u.click)
					u.ctx->SetClipboard(curCode.CStr());
				if (isWs) {
					const char *el = w->wsExtractEditing ? "Terminer" : "Editer manuellement";
					const float32 ew = u.TextW(el) + u.s(34);
					const NkRect er = {cx + cw - cwd - u.s(8) - ew, y - u.s(4), ew, u.s(24)};
					const bool ehv = !blockBg && u.Hit(er);
					u.Rect(er, w->wsExtractEditing ? NkCol::success : (ehv ? NkCol::hover : NkCol::muted),
						   NkR::md * u.S);
					NkOwIco(u, w->wsExtractEditing ? ic.valideSimple : ic.editer,
							w->wsExtractEditing ? "check" : "edit", {er.x + u.s(10), er.y + u.s(5), u.s(13), u.s(13)},
							w->wsExtractEditing ? NkCol::primaryFg : NkCol::foreground);
					u.TextV(er.x + u.s(26), er.y, u.s(24), el,
							w->wsExtractEditing ? NkCol::primaryFg : NkCol::foreground);
					if (ehv && u.click) {
						w->focusClaimed = true;
						if (!w->wsExtractEditing) {
							w->wsExtractBuf = w->BuildJenga();
							w->wsExtractCaret = 0;
							w->wsExtractEditing = true;
							w->wsExtractEdited = true;
							w->codeSX = w->codeSY = 0.f;
						} else
							w->wsExtractEditing = false;
					}
				}
			}
			y += u.s(24);
			...
			if (nTabs > 1) {
				...
				for (int32 t = 0; t < nTabs; ++t) {
					...
					u.Panel(tr, act ? NkCol::secondary : (hv ? NkCol::hover : NkCol::surface),
			...
			if (isWs && w->wsExtractEditing) {
				...
				if (w->blink > 1.f)
					w->blink -= 1.f;
				NkWizCodeBox(u, {cx, y, cw, codeH}, w->wsExtractBuf, w, blockBg, true, &w->wsExtractCaret, w->blink,
							 moved);
			} else {
			...
			// CE QUI SERA CREE
			NkWizLabel(u, cx, y, "CE QUI SERA CREE");
			y += u.s(22);
			...
			auto fileLine = [&](const NkString &path) {
				...
				u.TextEllipsis(cx + u.s(24), y, cw - u.s(28), path.CStr(), NkCol::foreground);
				y += u.s(24);
			};
			{
				NkString f = folder;
			...
			for (usize i = 0; i < w->projects.Size(); ++i) {
				...
				if (p.genMain && (p.kind == 0 || p.kind == 1)) {
					NkString f = folder;
					f += "/";
					f += p.location;
					f += "src/main.cpp";
		...
		inline int32 NkNewWsPanel(const NkUi &u, const NkRect &r, NkNewWsState *w, NkCodeState *st, NkCodeDialogs *dlg,
								  float32 dt, const NkIcons &ic) {
			(void)dlg;
			w->EnsureInit(st);
			if (w->focus == 3)
				w->jengaManual = true; // l'utilisateur edite le nom de fichier
			w->SyncDerived();
			...
			const bool blockBg =
				(w->comboOpen >= 0) || (dlg && dlg->pickerOpen) || w->projDlg || w->tcDlg || NkTxtMenu().open;
			w->focusClaimed =
				false; // reinitialise : un champ posera true s'il capte le clic (sinon clic vide = defocus)
			...
			NkOwIco(u, ic.nouveau, "plus-circle", {r.x + u.s(28), r.y + (hH - u.s(18)) * 0.5f, u.s(18), u.s(18)},
					NkCol::primary);
			...
			const char *labels[] = {NkT("nws.step.ws"), NkT("nws.step.projects"), NkT("nav.toolchains"),
									NkT("nav.platforms"), NkT("nws.step.summary")};
			...
			switch (w->step) {
				case 0:
					contentH = NkWizStep1(u, body, w, dlg, dt, blockBg, ic);
					break;
				case 1:
					contentH = NkWizStep2(u, body, w, dt, blockBg, ic);
					break;
				case 2:
					contentH = NkWizStep3(u, body, w, dt, blockBg, ic);
					break;
				case 3:
					contentH = NkWizStep4(u, body, w, dt, blockBg, ic);
					break;
				case 4:
					contentH = NkWizStep5(u, body, w, dt, blockBg, ic);
					break;
				default:
					break;
			}
			u.dl->PopClipRect();
			// Molette APRES le contenu : un visualiseur de code survole l'a deja consommee (sinon le scroll global la
			// mange).
			if (u.Hit(view) && u.ctx->input.wheel != 0.f && !blockBg) {
				w->scroll -= u.ctx->input.wheel * u.s(40);
				u.ctx->input.wheel = 0.f;
			}
			...
			const int32 fa = NkWizFooter(u, {r.x, r.y + r.h - footH, r.w, footH}, w->step, 4, blockBg, nextOk,
										 NkT("nws.create.ws"), true, ic.valideSimple);
			if (fa == 1)
				result = 1; // Annuler -> Accueil
			else if (fa == -1 && w->step > 0) {
				w->step--;
				w->focus = -1;
				w->scroll = 0.f;
			} else if (fa == 2 && nextOk) {
				...
				if (w->Generate(dlg))
					result = 1;
			} // Creer maintenant (defauts)

			// ── Modale « Ajouter / Editer un projet » (3 etapes, par-dessus la page) ──
			if (w->projDlg)
				NkWizProjDialog(u, r, w, dlg, dt, (w->comboOpen >= 0), ic);
			// ── Modale « Ajouter un Toolchain Manuel » (etape 3) ──
			if (w->tcDlg)
				NkWizTcDialog(u, r, w, dlg, dt, (w->comboOpen >= 0), ic);
			...
			if (w->comboOpen >= 0 && w->comboOpts && w->comboSel) {
				...
				for (int32 k = 0; k < w->comboN; ++k) {
					const float32 tw = u.TextW(w->comboOpts[k]) + u.s(30);
					if (tw > ddw)
						ddw = tw;
				}
				float32 ddx = w->comboR.x;
				if (ddx + ddw > r.x + r.w - u.s(8))
					ddx = r.x + r.w - u.s(8) - ddw; // garde le menu dans la fenetre
				if (ddx < r.x + u.s(8))
					ddx = r.x + u.s(8);
				float32 ddy = w->comboR.y + w->comboR.h + u.s(2);
				const float32 ddh = w->comboN * ih + u.s(6);
				if (ddy + ddh > r.y + r.h - u.s(8))
					ddy = w->comboR.y - ddh - u.s(2); // bascule au-dessus si deborde en bas
				...
				for (int32 k = 0; k < w->comboN; ++k) {
					...
					u.TextEllipsis(ir.x + u.s(8), ir.y + (ih - u.Lh()) * 0.5f, ir.w - u.s(12), w->comboOpts[k],
								   NkCol::foreground);
					if (hv && u.click) {
						*w->comboSel = k;
						chose = true;

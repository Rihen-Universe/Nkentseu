		...
		inline void NkStripAnsiInto(const char *s, char *dst, usize cap) {
			...
			for (const char *p = s; *p && j + 1 < cap; ++p) {
				if (*p == 0x1b && p[1] == '[') { // CSI : ESC '[' params final
					p += 2;
					while (*p && !(*p >= '@' && *p <= '~'))
						++p; // params jusqu'a l'octet final
					if (!*p)
						break; // (le ++p de la boucle saute l'octet final)
				} else if (*p == '\r' || *p == '\n') {
			...
			public:
				...
				bool Start(const NkString &command) {
					if (mRunning)
						return false;
					if (mThread.Joinable())
						mThread.Join(); // recycle un thread precedent termine
					mCmd = command;
					mRunning = true;
					mDone = false;
					mExit = 0;
...
#endif
					if (!pipe) {
						Push(NkString("[erreur] impossible de lancer la commande"));
						mExit = -1;
						mDone = true;
						mRunning = false;
						return;

...
namespace nkentseu {
	namespace nkcode {
		...
		bool NkPty::Start(const NkString &cmdline, int16 cols, int16 rows, const NkString &cwd) {
			...
			if (!pCreate || !pClose)
				return false; // ConPTY indisponible (< Win10 1809)
			...
			bool attrOk = si.lpAttributeList &&
						  ...
						  UpdateProcThreadAttribute(si.lpAttributeList, 0, PROC_THREAD_ATTRIBUTE_PSEUDOCONSOLE, mPC,
													sizeof(mPC), NULL, NULL);
			if (!attrOk) {
			...
			if (wn <= 0)
				wn = 1;
			nkentseu::NkVector<wchar_t> wcmd;
			wcmd.Resize(static_cast<usize>(wn));
...
#if defined(_WIN32)
			if (!mRunning || !mInWrite || !data || len == 0)
				return;
			DWORD wr = 0;
		...
		void NkPty::Stop() {
			...
			if (mInWrite) {
				CloseHandle((HANDLE)mInWrite);
				mInWrite = NULL;
			}
			// 2) Ferme le pseudo-console -> termine le client, debloque ReadFile.
			if (mPC && mCloseFn) {
				((PFN_ClosePseudoConsole)mCloseFn)(mPC);
				mPC = NULL;
			}

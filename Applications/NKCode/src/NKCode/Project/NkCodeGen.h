...
namespace nkentseu {
	namespace nkcode {
		...
		inline NkString GenerateWorkspace(const NkPath &root, const char *rawName) noexcept {
			...
			if (NkFile::Exists(file))
				return NkString(); // ne pas ecraser
		...
		inline const char *const *NkConfigNames(int32 *n) noexcept {
			static const char *c[] = {"Debug", "Release", "Profile", "Shipping"};
			if (n)
				*n = 4;
			return c;
		}
		...
		inline const char *const *NkArchNames(int32 *n) noexcept {
			static const char *a[] = {"x86_64", "x86", "arm64", "arm", "wasm32"};
			if (n)
				*n = 5;
			return a;
		}
		...
		inline NkString GenerateWorkspaceEx(const NkPath &dir, const NkWorkspaceOpts &o) noexcept {
			...
			if (name.Empty())
				return NkString();
			if (!NkDirectory::Exists(dir) && !NkDirectory::CreateRecursive(dir))
				return NkString();
			...
			int32 nC = 0, nA = 0;
			const char *const *cfgN = NkConfigNames(&nC);
			const char *const *archN = NkArchNames(&nA);
			NkString cfgs = join(o.cfg, cfgN, nC);
			if (cfgs.Empty())
				cfgs = "\"Debug\", \"Release\"";
			...
			auto osOn = [&](const char *nm) -> bool {
				if (!o.os)
					return false;
				for (int32 i = 0; i < o.nOs; ++i)
					if (o.os[i] && StrEqA(o.osNames[i], nm))
						return true;
				return false;
			};
			...
			if (o.toolchain && *o.toolchain) {
				c += "    usetoolchain(\"";
				c += o.toolchain;
				c += "\")\n";
			}
			if (o.startProject && *o.startProject) {
				c += "    startproject(\"";
				c += o.startProject;
				c += "\")\n";
			}
			...
			if (osOn("Android")) {
				if (o.androidSdk && *o.androidSdk) {
					c += "    androidsdkpath(r\"";
					c += o.androidSdk;
					c += "\")\n";
				}
				if (o.androidNdk && *o.androidNdk) {
					c += "    androidndkpath(r\"";
					c += o.androidNdk;
					c += "\")\n";
				}
				if (o.javaJdk && *o.javaJdk) {
					c += "    javajdkpath(r\"";
					c += o.javaJdk;
					c += "\")\n";
				}
			}
			if (osOn("HarmonyOS") && o.harmonySdk && *o.harmonySdk) {
				c += "    harmonysdk(r\"";
				c += o.harmonySdk;
				c += "\")\n";
			}
			if ((osOn("XboxSeries") || osOn("XboxOne")) && o.gdkPath && *o.gdkPath) {
				c += "    gdkpath(r\"";
				c += o.gdkPath;
				c += "\")\n";
			}
		...
		inline const char *const *NkDialects(int32 *n) noexcept {
			static const char *d[] = {"C++17", "C++20", "C++23", "C++14"};
			if (n)
				*n = 4;
			return d;
		}
		...
		inline NkString GenerateProjectEx(const NkPath &root, const NkPath &workspaceJenga,
										  const NkProjectOpts &opts) noexcept {
			...
			if (kindIdx < 0 || kindIdx >= nk)
				kindIdx = 0;
			if (langIdx < 0 || langIdx >= nl)
				langIdx = 0;
			...
			if (NkDirectory::Exists(dir))
				return NkString(); // ne pas ecraser un dossier existant
			NkPath srcDir = dir / "src";
			if (!NkDirectory::CreateRecursive(srcDir))
				return NkString();
			...
			if (kd.app) {
				...
				src += "    printf(\"";
				src += name;
				src += " - genere par NKCode\\n\");\n";
				src += "    return 0;\n}\n";
			} else {
				src += "// ";
				src += name;
				src += " - bibliotheque generee par NKCode.\n\n";
				src += isC ? "int " : "int ";
				src += name;
				src += "_Version(void) { return 1; }\n";
			}
			...
			if (opts.filesPattern && *opts.filesPattern) {
				NkString fp = NkPyList(opts.filesPattern);
				c += "    files([";
				c += fp;
				c += "])\n\n";
			} else {
			...
			auto listProp = [&](const char *fn, const char *csv) {
				if (csv && *csv) {
					NkString l = NkPyList(csv);
					if (!l.Empty()) {
						c += "    ";
						c += fn;
						c += "([";
			...
			if (opts.defines && *opts.defines) {
				NkString dl2 = NkPyList(opts.defines);
				if (!dl2.Empty()) {
					c += "    defines([";
					c += dl2;
					c += "])\n";
			...
			if (kd.app && opts.appVersion && *opts.appVersion) {
				c += "    appversion(\"";
				c += opts.appVersion;
				c += "\")\n";
			}
			if (kd.app && opts.appPublisher && *opts.appPublisher) {
				c += "    apppublisher(\"";
				c += opts.appPublisher;
				c += "\")\n";
			}
			if (kd.app && opts.appIcon && *opts.appIcon) {
				c += "    appicon(\"";
				c += opts.appIcon;
				c += "\")\n";
			}
			...
			if (!NkFile::WriteAllText(projJenga, c))
				return NkString();
		...
		inline NkString GenerateProject(const NkPath &root, const NkPath &workspaceJenga, const char *rawName,
										int32 kindIdx, int32 langIdx) noexcept {
			NkProjectOpts o;
			o.name = rawName;
			o.kindIdx = kindIdx;
			o.langIdx = langIdx;

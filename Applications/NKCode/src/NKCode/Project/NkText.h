...
namespace nkentseu {
	namespace nkcode {
		...
		inline NkString NkJsonStr(const char *j, const char *key) {
			...
			const char *k = NkFindSub(j, pat.CStr());
			if (!k)
				return NkString();
		...
		inline int64 NkDefineValue(const NkVector<NkString> &defs, const char *name, int32 nameLen, bool *found) {
			for (usize i = 0; i < defs.Size(); ++i) {
				...
				int32 k = 0;
				while (k < nameLen && d[k] && d[k] != '=' && d[k] == name[k])
					++k;
		...
		inline int64 NkPPParsePrimary(NkPPCur &c) {
			...
			if ((*c.p >= 'A' && *c.p <= 'Z') || (*c.p >= 'a' && *c.p <= 'z') || *c.p == '_') {
				const char *s = c.p;
				int32 L = 0;
				while (NkPPIsIdc(c.p[L]))
					++L;
				c.p += L;
				bool f = false;
				int64 v = NkDefineValue(*c.defs, s, L, &f);
				return f ? v : 0;
			}
			c.ok = false;
			return 1; // inconnu -> actif par prudence
		}

		inline int64 NkPPParseAnd(NkPPCur &c) {
			int64 v = NkPPParsePrimary(c);
			for (;;) {
				while (*c.p == ' ' || *c.p == '\t')
					++c.p;
				if (c.p[0] == '&' && c.p[1] == '&') {
					c.p += 2;
					int64 r = NkPPParsePrimary(c);
					v = (v && r) ? 1 : 0;
				} else
					break;
			}
		...
		inline int64 NkPPParseOr(NkPPCur &c) {
			int64 v = NkPPParseAnd(c);
			for (;;) {
				while (*c.p == ' ' || *c.p == '\t')
					++c.p;
				if (c.p[0] == '|' && c.p[1] == '|') {
					c.p += 2;
					int64 r = NkPPParseAnd(c);
					v = (v || r) ? 1 : 0;
				} else
					break;
			}
		...
		inline bool NkPPEvalActive(const char *expr, const NkVector<NkString> &defs) {
			NkPPCur c{expr, &defs, true};
			const int64 v = NkPPParseOr(c);
			while (*c.p == ' ' || *c.p == '\t')
				++c.p;
			if (*c.p != '\0')
				c.ok = false;

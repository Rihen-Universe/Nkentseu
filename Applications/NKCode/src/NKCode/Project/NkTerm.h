		...
		class NkTerm {
			public:
				...
				usize TotalLines() const {
					return mScroll.Size() + mScreen.Size();
				}
				...
				usize CursorLine() const {
					return mScroll.Size() + static_cast<usize>(mCy);
				}
			...
			private:
				...
				void BlankLine(Line &ln) {
					ln.Clear();
					for (int16 c = 0; c < mCols; ++c)
						ln.PushBack(NkTermCell{});
				}

				void EnsureWidth(Line &ln) {
				...
				void ScrollUp() {
					...
					if (mTop == 0 && !mAlt) {
						...
						while (mScroll.Size() > 5000)
							mScroll.Erase(mScroll.Begin());
					}
					for (int16 r = mTop; r < mBot; ++r)
						mScreen[r] = mScreen[r + 1];
				...
				void Ground_(unsigned char b) {
					...
					if (b == '\t') {
						int16 n = static_cast<int16>((mCx / 8 + 1) * 8);
					...
					if (b == 0x07)
						return; // BEL
					if (b < 0x20)
						return; // autres controles ignores
					if (b < 0x80) {
						PutGlyph(b);
						return;
					}
				...
				int32 Params(int32 *out, int32 maxN, bool *priv) {
					...
					if (n < maxN)
						out[n++] = any ? cur : (n == 0 ? -1 : 0); // -1 = "absent" pour 1er param
					return n;
				}
				...
				void Dispatch(char fin) {
					...
					switch (fin) {
						case 'H':
						case 'f': {
							int32 r = P(0, 1), c = P(1, 1);
							mCy = Clamp(r - 1, 0, mRows - 1);
							mCx = Clamp(c - 1, 0, mCols - 1);
						} break;
						case 'A':
							mCy = Clamp(mCy - P(0, 1), 0, mRows - 1);
							break;
						case 'B':
							mCy = Clamp(mCy + P(0, 1), 0, mRows - 1);
							break;
						case 'C':
							mCx = Clamp(mCx + P(0, 1), 0, mCols - 1);
							break;
						case 'D':
							mCx = Clamp(mCx - P(0, 1), 0, mCols - 1);
							break;
						...
						case 'F':
							...
							break;
						case 'G':
							mCx = Clamp(P(0, 1) - 1, 0, mCols - 1);
							break;
						case 'd':
							mCy = Clamp(P(0, 1) - 1, 0, mRows - 1);
							break;
						case 'J':
						...
						case 'M':
							...
							break;
						case 'r':
							mTop = Clamp(P(0, 1) - 1, 0, mRows - 1);
							mBot = Clamp(P(1, mRows) - 1, 0, mRows - 1);
							if (mBot < mTop)
								mBot = mTop;
							mCx = 0;
							mCy = mTop;
				...
				void EraseDisplay(int32 mode) {
					...
					if (mode == 0) { // du curseur a la fin
						EnsureWidth(mScreen[mCy]);
						for (int16 c = mCx; c < mCols; ++c)
							mScreen[mCy][c] = Blank();
						for (int16 r = mCy + 1; r < mRows; ++r)
							BlankLine(mScreen[r]);
					} else if (mode == 1) { // du debut au curseur
						for (int16 r = 0; r < mCy; ++r)
							BlankLine(mScreen[r]);
						EnsureWidth(mScreen[mCy]);
						for (int16 c = 0; c <= mCx && c < mCols; ++c)
							mScreen[mCy][c] = Blank();
					}
				}
				...
				void EraseChars(int32 cnt) {
					EnsureWidth(mScreen[mCy]);
					for (int16 c = mCx; c < mCx + cnt && c < mCols; ++c)
						mScreen[mCy][c] = Blank();
				}
					...
					for (int32 i = 0; i < n; ++i) {
						...
						if (c == 0) {
							...
							if (i + 1 < n && p[i + 1] == 5 && i + 2 < n) {
								...
								col = NkColor{(uint8)p[i + 2], (uint8)p[i + 3], (uint8)p[i + 4], 255};
								i += 4;
							}
							if (c == 38)
								mFg = col;
				...
				static NkColor DefFg() {
					return {204, 204, 204, 255};
				}

				static NkColor DefBg() {
					return {0, 0, 0, 0};
				} // transparent (fond terminal)

				static NkColor DefBg2() {
					return {13, 17, 23, 255};
				} // fond opaque pour reverse
				...
				static NkColor Xterm256(int32 idx) {
					if (idx < 16)
						return Ansi16(idx);
					if (idx >= 232) {
						uint8 v = static_cast<uint8>(8 + 10 * (idx - 232));
						return {v, v, v, 255};
					}
					int32 i = idx - 16;
					int32 r = (i / 36) % 6, g = (i / 6) % 6, b = i % 6;

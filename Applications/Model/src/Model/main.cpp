...
static NkShaderHandle LoadShadersForAPI(NkIDevice *device, NkGraphicsApi api) {
	...
	switch (api) {
		...
		case NkGraphicsApi::NK_GFX_API_SOFTWARE: {
			// auto* vsFn = new NkVertexShaderSoftware([](const void* vdata, uint32 idx, const void* udata) ->
			// NkVertexSoftware {
			...
			// auto* fsFn = new NkPixelShaderSoftware([](const NkVertexSoftware& frag, const void* udata, const void*)
			// -> math::NkVec4f {
			...
			//                 if (f) {
			//                     fclose(f);
			//                     compiled = swbridge::NkCompileFile("Resources/Shaders/Model/triangle.sw.sksl",
			//                     kNkVertex2DStride); loaded = compiled.success;
			//                 }
			//             }

			//             // Chemin 2 : fallback inline (toujours disponible, ne dépend d'aucun fichier)
			//             if (!loaded) {
			//                 logger.Warnf("[Shader] Software: fichier .sw.sksl non trouve, utilisation du fallback
			//                 inline");
			...
			//                 compiled = swbridge::NkCompile(NkString(kFallback), "inline_fallback.sw.sksl",
			//                 kNkVertex2DStride); loaded = compiled.success;
			//             }
        ...
        void frag_main() {
            FragColor = vec4(vColor, 1.0);
				...
				compiled = swbridge::NkCompileSources(NkString(kVertSrc), NkString(kFragSrc), "inline.vert.sw.sksl",
													  "inline.frag.sw.sksl");
				loaded = compiled.success;
			}
...
int nkmain(const NkEntryState &state) {
	...
	while (appState.running) {
		...
		if (appState.size.width != (int)device->GetSwapchainWidth() ||
			appState.size.height != (int)device->GetSwapchainHeight()) {

...
namespace nkentseu {
	namespace nkpa {
		...
		void Turtle::Update(float32 dt) {
			...
			mNeck.Update(center + NkVector2f{::cosf(mAgent.HeadAng()) * 18.f, ::sinf(mAgent.HeadAng()) * 18.f});
		...
		void Turtle::DrawLeg(MeshBuilder &mb, const Chain &leg) const {
			...
			for (int32 i = 0; i < NL - 1; ++i) {
				mb.AddQuadGrad(leg.Seg(i).pos, 6.f, mSkinR, mSkinG, mSkinB, 1.f, leg.Seg(i + 1).pos, 5.f, mSkinR * 0.8f,
							   mSkinG * 0.8f, mSkinB * 0.7f, 1.f);
		...
		void Turtle::Draw(MeshBuilder &mb) const {
			...
			// ── Cou ──────────────────────────────────────────────────────────────────
			{
				...
				for (int32 i = 0; i < NC - 1; ++i) {
					mb.AddQuadGrad(mNeck.Seg(i).pos, 5.f, mSkinR, mSkinG, mSkinB, 1.f, mNeck.Seg(i + 1).pos, 5.f,
								   mSkinR, mSkinG, mSkinB, 1.f);
			...
			// Ombre portée
			mb.AddEllipse(center + NkVector2f{3.f, 4.f}, 30.f, 22.f, cAng, 0.05f, 0.05f, 0.02f, 0.3f, 20);
			// Base de la carapace
			...
			for (int32 row = -1; row <= 1; ++row) {
				for (int32 col = -1; col <= 1; ++col) {
					NkVector2f pc =
						center + NkVector2f{::cosf(cAng) * (float32)col * 11.f - ::sinf(cAng) * (float32)row * 8.f,
											::sinf(cAng) * (float32)col * 11.f + ::cosf(cAng) * (float32)row * 8.f};
					mb.AddCircle(pc, 4.5f, mShellR * 0.65f, mShellG * 0.65f, mShellB * 0.5f, 0.6f, 6);
				}
			}
			// Reflet
			mb.AddEllipse(center - NkVector2f{5.f, 6.f}, 14.f, 8.f, cAng + 0.3f, mShellR * 1.5f, mShellG * 1.5f,
						  mShellB * 1.2f, 0.25f, 14);

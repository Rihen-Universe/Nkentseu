...
namespace nkentseu {
	namespace nkpa {
		...
		void Cat::Update(float32 dt) {
			...
			NkVector2f tailTarget = tailBase + NkVector2f{::cosf(curlAng) * 35.f, ::sinf(curlAng) * 35.f};
		...
		void Cat::DrawLeg(MeshBuilder &mb, const Chain &leg) const {
			...
			for (int32 i = 0; i < NL - 1; ++i) {
				...
				if (rad1 < 2.f)
					rad1 = 2.f;
				mb.AddQuadGrad(leg.Seg(i).pos, rad0, mCr * 0.85f, mCg * 0.8f, mCb * 0.75f, 1.f, leg.Seg(i + 1).pos,
							   rad1, mCr * 0.7f, mCg * 0.65f, mCb * 0.6f, 1.f);
		...
		void Cat::Draw(MeshBuilder &mb) const {
			// ── Queue ────────────────────────────────────────────────────────────────
			{
				...
				for (int32 i = 0; i < NT - 1; ++i) {
					...
					mb.AddQuadGrad(mTail.Seg(i).pos, rad0, mCr, mCg, mCb, 1.f, mTail.Seg(i + 1).pos, rad1, mCr * 0.85f,
								   mCg * 0.8f, mCb * 0.75f, 1.f);
					mb.AddCircle(mTail.Seg(i).pos, rad0, mCr, mCg, mCb, 1.f, 10);
				}
				// Bout de queue légèrement plus clair
				mb.AddCircle(mTail.Seg(mTail.Count() - 1).pos, 5.f, mCr * 1.15f, mCg * 1.1f, mCb * 1.05f, 1.f, 10);
			}
			...
			for (int32 i = 0; i < N - 1; ++i) {
				...
				mb.AddQuadGrad(mBody.Seg(i).pos, BodyRadius(i), mCr * sh, mCg * sh, mCb * sh, 1.f, mBody.Seg(i + 1).pos,
							   BodyRadius(i + 1), mCr * sh, mCg * sh, mCb * sh, 1.f);
			...
			for (int32 s = -1; s <= 1; s += 2) {
				NkVector2f earBase0 =
					head + perp * (6.f * (float32)s) + NkVector2f{::cosf(ang) * 6.f, ::sinf(ang) * 6.f};
				NkVector2f earBase1 =
					head + perp * (10.f * (float32)s) + NkVector2f{::cosf(ang) * 4.f, ::sinf(ang) * 4.f};
				NkVector2f earTip =
					head + perp * (8.f * (float32)s) + NkVector2f{::cosf(ang) * 16.f, ::sinf(ang) * 16.f};
				mb.AddTriFlat(earBase0, earBase1, earTip, mCr, mCg, mCb, 1.f);
				// Intérieur rose
				NkVector2f iBase0 =
					head + perp * (6.5f * (float32)s) + NkVector2f{::cosf(ang) * 6.5f, ::sinf(ang) * 6.5f};
				NkVector2f iBase1 =
					head + perp * (9.5f * (float32)s) + NkVector2f{::cosf(ang) * 4.5f, ::sinf(ang) * 4.5f};
				NkVector2f iTip = head + perp * (8.f * (float32)s) + NkVector2f{::cosf(ang) * 13.f, ::sinf(ang) * 13.f};

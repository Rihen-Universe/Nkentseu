	...
	namespace nkpa {
		...
		void Centipede::Draw(MeshBuilder &mb) const {
			...
			for (int32 i = 1; i < N; ++i) {
				...
				NkVector2f kL =
					base + perp * 6.f +
					NkVector2f{::cosf(ang + PA_PI * 0.5f + wave) * legL, ::sinf(ang + PA_PI * 0.5f + wave) * legL};
			...
			for (int32 i = 0; i < N - 1; ++i) {
				...
				mb.AddQuadGrad(mBody.Seg(i).pos, 7.f, mCr + alt, mCg, mCb, 1.f, mBody.Seg(i + 1).pos, 7.f, mCr + alt,
							   mCg, mCb, 1.f);
			...
			for (int32 s = -1; s <= 1; s += 2) {
				...
				NkVector2f base = head + perp * (4.f * (float32)s) + NkVector2f{::cosf(ang) * 8.f, ::sinf(ang) * 8.f};

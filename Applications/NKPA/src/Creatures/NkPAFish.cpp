	...
	namespace nkpa {
		...
		void Fish::Draw(MeshBuilder &mb) const {
			...
			for (int32 i = 0; i < N - 1; ++i) {
				...
				mb.AddQuadGrad(mBody.Seg(i).pos, BodyRadius(i), r0, g0, mCb, 1.f, mBody.Seg(i + 1).pos,
							   BodyRadius(i + 1), r1, g1, mCb, 1.f);
			}
			for (int32 i = 0; i < N; ++i) {
			...
			// ── Nageoire dorsale (segments 2–4) ─────────────────────────────────────
			{
				...
				mb.AddFin(p1, p2, tip, mCr, mCg, mCb * 0.7f, 0.7f, mCr * 0.5f, mCg * 0.5f, mCb * 0.4f, 0.4f);
			}
			...
			// ── Nageoires pectorales (segment 3) ────────────────────────────────────
			{
				...
				NkVector2f bl0 = {pos3.x + ::cosf(ang) * r3 * 0.8f, pos3.y + ::sinf(ang) * r3 * 0.8f};
				NkVector2f bl1 = {pos3.x - ::cosf(ang) * r3 * 0.8f, pos3.y - ::sinf(ang) * r3 * 0.8f};

				// Côté gauche
				float32 angL = ang + PA_PI * 0.5f + wave;
				NkVector2f tipL = {pos3.x + ::cosf(angL) * 26.f, pos3.y + ::sinf(angL) * 26.f};
				mb.AddFin(bl0, bl1, tipL, mCr, mCg * 0.8f, mCb * 0.6f, 0.65f, mCr * 0.3f, mCg * 0.3f, mCb * 0.3f, 0.3f);

				// Côté droit
				float32 angR = ang - PA_PI * 0.5f - wave;
				NkVector2f tipR = {pos3.x + ::cosf(angR) * 26.f, pos3.y + ::sinf(angR) * 26.f};
				mb.AddFin(bl0, bl1, tipR, mCr, mCg * 0.8f, mCb * 0.6f, 0.65f, mCr * 0.3f, mCg * 0.3f, mCb * 0.3f, 0.3f);
			}
			...
			// ── Nageoire caudale (double lobe) ───────────────────────────────────────
			{
				...
				NkVector2f tipD = {tailPos.x + ::cosf(ang + PA_PI - wave) * tailW,
								   tailPos.y + ::sinf(ang + PA_PI - wave) * tailW};
				mb.AddFin(baseL, tailPos, tipU, mCr, mCg * 0.7f, mCb * 0.5f, 0.75f, mCr * 0.2f, mCg * 0.2f, mCb * 0.2f,
						  0.3f);
				mb.AddFin(tailPos, baseR, tipD, mCr, mCg * 0.7f, mCb * 0.5f, 0.75f, mCr * 0.2f, mCg * 0.2f, mCb * 0.2f,
						  0.3f);
			}
			...
			// ── Oeil ────────────────────────────────────────────────────────────────
			{
				...
				NkVector2f eyeOff = {::cosf(ang + 0.5f) * 8.f, ::sinf(ang + 0.5f) * 8.f};

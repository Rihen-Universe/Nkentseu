	...
	namespace nkpa {
		...
		void Lizard::DrawLeg(MeshBuilder &mb, const Chain &leg) const {
			...
			for (int32 i = 0; i < NL - 1; ++i) {
				...
				mb.AddQuadGrad(leg.Seg(i).pos, rad0, mCr * 0.8f, mCg * 0.7f, mCb * 0.6f, 1.f, leg.Seg(i + 1).pos, rad1,
							   mCr * 0.6f, mCg * 0.5f, mCb * 0.4f, 1.f);
			...
			for (int32 c = -1; c <= 1; ++c) {
				...
				NkVector2f cBase = tip + NkVector2f{::cosf(ang + PA_PI) * 3.f, ::sinf(ang + PA_PI) * 3.f};
		...
		void Lizard::Draw(MeshBuilder &mb) const {
			...
			for (int32 i = 0; i < NT - 1; ++i) {
				...
				mb.AddQuadGrad(mTail.Seg(i).pos, rad0, mCr * 0.9f, mCg * 0.8f, mCb * 0.5f, 1.f, mTail.Seg(i + 1).pos,
							   rad1, mCr * 0.7f, mCg * 0.6f, mCb * 0.3f, 1.f);

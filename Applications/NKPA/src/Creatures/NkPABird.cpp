...
namespace nkentseu {
	namespace nkpa {
		...
		void Bird::Draw(MeshBuilder &mb) const {
			...
			mb.AddTriFlat(shoulder - NkVector2f{4.f, 0.f}, shoulder + NkVector2f{4.f, 0.f}, elbow, mWr, mWg, mWb,
						  0.85f);
			...
			mb.AddTriFlat(shoulder - NkVector2f{4.f, 0.f}, shoulder + NkVector2f{4.f, 0.f}, elbow2, mWr, mWg, mWb,
						  0.85f);
			...
			// ── Queue (segments 3-5) ─────────────────────────────────────────────────
			{
				...
				mb.AddFin(tailBase + tPerp * 8.f, tailBase - tPerp * 8.f, tailTip, mCr, mCg, mCb, 1.f, mCr * 0.7f,
						  mCg * 0.7f, mCb * 0.7f, 0.6f);
			}
			...
			for (int32 i = 0; i < N - 1; ++i) {
				...
				if (rad1 < 2.f)
					rad1 = 2.f;
				mb.AddQuadGrad(mBody.Seg(i).pos, rad0, mCr, mCg, mCb, 1.f, mBody.Seg(i + 1).pos, rad1, mCr, mCg, mCb,
							   1.f);
			...
			mb.AddTriFlat(beakBase + beakPerp * 3.f, beakBase - beakPerp * 3.f, beakTip, 0.9f, 0.7f, 0.1f, 1.f);

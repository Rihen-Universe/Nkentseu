	...
	namespace nkpa {
		...
		void Snake::Draw(MeshBuilder &mb) const {
			...
			for (int32 i = 0; i < N - 1; ++i) {
				...
				SegColor(i, r0, g0, b0);
				float32 r1, g1, b1;
				SegColor(i + 1, r1, g1, b1);
				mb.AddQuadGrad(mBody.Seg(i).pos, BodyRadius(i), r0, g0, b0, 1.f, mBody.Seg(i + 1).pos,
							   BodyRadius(i + 1), r1, g1, b1, 1.f);
			}
			for (int32 i = 0; i < N; ++i) {
				float32 r0, g0, b0;
				SegColor(i, r0, g0, b0);
			...
			{
				float32 ang = mBody.Seg(0).angle + PA_PI;
				NkVector2f snout = {mBody.Seg(0).pos.x + ::cosf(ang) * 14.f, mBody.Seg(0).pos.y + ::sinf(ang) * 14.f};
				float32 wave = ::sinf(mTime * 8.f) * 0.25f;
				NkVector2f mid = snout + NkVector2f{::cosf(ang) * 8.f, ::sinf(ang) * 8.f};
				NkVector2f tipL = mid + NkVector2f{::cosf(ang + 0.5f + wave) * 6.f, ::sinf(ang + 0.5f + wave) * 6.f};
				NkVector2f tipR = mid + NkVector2f{::cosf(ang - 0.5f + wave) * 6.f, ::sinf(ang - 0.5f + wave) * 6.f};

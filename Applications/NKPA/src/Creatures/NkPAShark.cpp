...
namespace nkentseu {
	namespace nkpa {
		...
		float32 Shark::BodyRadius(int32 i) const {
			...
			if (t < 0.1f)
				return 10.f + t / 0.1f * 8.f; // museau effilé
			if (t < 0.5f)
				return 18.f;						 // corps cylindrique
		...
		void Shark::Draw(MeshBuilder &mb) const {
			...
			// ── Nageoire dorsale (haute, segment 3–5) ────────────────────────────────
			{
				...
				mb.AddFin(p1, p2, tip, mCr * 0.8f, mCg * 0.8f, mCb * 0.8f, 0.9f, mCr * 0.5f, mCg * 0.5f, mCb * 0.6f,
						  0.6f);
			}
			...
			NkVector2f perp = {-::sinf(ang), ::cosf(ang)};
			mb.AddTriFlat(head + perp * 14.f, head - perp * 14.f, snout, mCr * 1.05f, mCg * 1.05f, mCb * 1.0f, 1.f);
			mb.AddCircle(head, 14.f, mCr, mCg, mCb, 1.f, 16);
			// Ventre clair
			mb.AddEllipse(head + NkVector2f{::cosf(ang) * 8.f, ::sinf(ang) * 8.f}, 12.f, 6.f, ang, belly, belly, belly,
						  0.7f, 12);

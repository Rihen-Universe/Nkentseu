	...
	namespace nkpa {
		...
		class Agent {
			public:
				...
				void Init(NkVector2f startPos, float32 speed, float32 undulAmp, float32 undulFreq, int32 worldW,
						  int32 worldH);

				/// Met à jour l'agent pour dt secondes et retourne la nouvelle position de la tête.
				NkVector2f Update(float32 dt);

				NkVector2f Pos() const {
					return mPos;
				}

				NkVector2f Vel() const {
					return mVel;
				}

				float32 HeadAng() const {
					return mHeadAngle;
				}

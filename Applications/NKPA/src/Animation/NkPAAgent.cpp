...
namespace nkentseu {
	namespace nkpa {
		...
		void Agent::Init(NkVector2f startPos, float32 speed, float32 undulAmp, float32 undulFreq, int32 worldW,
						 int32 worldH) {
			mPos = startPos;
			mVel = {0.f, 0.f};
			mSpeed = speed;
		...
		void Agent::PickNewTarget() {
			float32 margin = 80.f;
			mTarget = {Randf(margin, (float32)mWorldW - margin), Randf(margin, (float32)mWorldH - margin)};
		}

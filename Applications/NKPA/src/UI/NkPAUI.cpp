	...
	namespace nkpa {
		...
		void NkPAUI::BuildFrame(float32 dt, NkPAUIState &state) {
			...
			if (nkui::NkUIWindow::Begin(im.ctx, im.wm, *im.ctx.dl, *font, im.ls, "NKPA Controls", &open,
				...
				// Vitesse globale
				nkui::NkUI::SliderFloat(im.ctx, im.ls, *im.ctx.dl, *font, "Speed", state.speedScale, 0.1f, 3.0f);
		...
		static void ConvertDrawList(const nkui::NkUIDrawList &dl, MeshBuilder &mb) {
			for (uint32 ci = 0; ci < dl.cmdCount; ++ci) {
				...
				if (cmd.type != nkui::NkUIDrawCmdType::NK_TRIANGLES)
					continue;
				for (uint32 ii = 0; ii + 2 < cmd.idxCount; ii += 3) {
					...
					mb.AddTriangle({v0.pos.x, v0.pos.y}, r0, g0, b0, a0, {v1.pos.x, v1.pos.y}, r1, g1, b1, a1,

...
namespace nkentseu {
	namespace nkpa {
		...
		bool MeshBuilder::IsEmpty() const {
			return mPixels.IsEmpty();
		}

		int32 MeshBuilder::VertexCount() const {
			return (int32)mPixels.Size();
		}

		// ─────────────────────────────────────────────────────────────────────────────

		void MeshBuilder::AddTriangle(NkVector2f a, float32 ra, float32 ga, float32 ba, float32 aa, NkVector2f b,
									  float32 rb, float32 gb, float32 bb, float32 ab, NkVector2f c, float32 rc,
									  float32 gc, float32 bc, float32 ac) {
		...
		void MeshBuilder::AddTriFlat(NkVector2f a, NkVector2f b, NkVector2f c, float32 r, float32 g, float32 bl,
									 float32 al) {
		...
		void MeshBuilder::AddCircle(NkVector2f center, float32 radius, float32 r, float32 g, float32 b, float32 a,
									int32 segs) {
		...
		void MeshBuilder::AddEllipse(NkVector2f center, float32 rx, float32 ry, float32 angle, float32 r, float32 g,
									 float32 b, float32 a, int32 segs) {
		...
		void MeshBuilder::AddQuad(NkVector2f a, float32 ra_, NkVector2f b, float32 rb_, float32 r, float32 g,
								  float32 bl, float32 al) {
		...
		void MeshBuilder::AddQuadGrad(NkVector2f a, float32 ra_, float32 r0, float32 g0, float32 b0_, float32 a0_,
									  NkVector2f b, float32 rb_, float32 r1, float32 g1, float32 b1_, float32 a1_) {
			NkVector2f d = (b - a).Normalized().Normal();
		...
		void MeshBuilder::AddFin(NkVector2f base0, NkVector2f base1, NkVector2f tip, float32 r0, float32 g0, float32 b0,
								 float32 a0, float32 rt, float32 gt, float32 bt, float32 at) {
			AddTriangle(base0, r0, g0, b0, a0, base1, r0, g0, b0, a0, tip, rt, gt, bt, at);
		}

		// ─────────────────────────────────────────────────────────────────────────────

		void MeshBuilder::AddBackground(float32 w, float32 h, float32 tlR, float32 tlG, float32 tlB, float32 brR,
										float32 brG, float32 brB) {
			// Deux triangles couvrant tout l'écran avec dégradé coin TL→BR
			AddTriangle({0, 0}, tlR, tlG, tlB, 1.f, {w, 0}, (tlR + brR) * 0.5f, (tlG + brG) * 0.5f, (tlB + brB) * 0.5f,
						1.f, {0, h}, (tlR + brR) * 0.5f, (tlG + brG) * 0.5f, (tlB + brB) * 0.5f, 1.f);
			AddTriangle({w, 0}, (tlR + brR) * 0.5f, (tlG + brG) * 0.5f, (tlB + brB) * 0.5f, 1.f, {w, h}, brR, brG, brB,
						1.f, {0, h}, (tlR + brR) * 0.5f, (tlG + brG) * 0.5f, (tlB + brB) * 0.5f, 1.f);
		}

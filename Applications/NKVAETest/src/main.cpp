// =============================================================================
// NKVAETest — auto-encodeur variationnel : GÉNÉRER depuis du bruit (NKGen).
//   Entraîne un VAE sur des motifs 8x8, montre la reconstruction, puis TIRE
//   z ~ N(0,1) et décode -> des images INÉDITES (création sans source).
// =============================================================================
...
static void PrintImage(const float* img, const char* title) {
    ...
    for (uint32 y = 0; y < WH; ++y) {
        ...
        for (uint32 x = 0; x < WH; ++x) {
            float v = img[y * WH + x]; if (v<0)v=0; if (v>1)v=1;
            printf("%c", ramp[(int)(v * 5.f + 0.5f)]);
        }
        printf("\n");
    }
}
...
int main() {
    printf("=== NKVAETest : générer des images depuis du bruit (VAE) ===\n\n");

    // Jeu : 4 motifs x 12 = 48 images 8x8 (aplaties -> [N,64]).
    const uint32 NCLS = 4, PER = 12, N = NCLS * PER;
    NkTensor X = NkTensor::Zeros(NkShape{ (int64)N, (int64)D });
    {
        float* xp = X.DataAs<float>(); float proto[D]; uint32 s = 4242u;
    ...
    const uint32 LAT = 8;
    gen::NkVAE vae(D, 32, LAT, 20260705u);
    ...
    for (int e = 0; e <= 600; ++e) {
        ...
        NkVar recon = autograd::MSE(xhat, xin);                 // reconstruction
        NkVar kl    = gen::KLDivergence(mu, logvar);            // rapproche de N(0,1)
    ...
    // Reconstruction (μ, sans bruit) d'un exemple par motif.
    ...
    const float* xp = X.DataAs<float>();
    {
        ...
        const float* rp = rec.DataAs<float>();
        for (uint32 c = 0; c < NCLS; ++c) {
            uint32 i = c * PER;
            char t1[40], t2[40];
            snprintf(t1, sizeof(t1), "motif %u original", c);
            snprintf(t2, sizeof(t2), "motif %u reconstruit", c);
            PrintImage(xp + i * D, t1);
            PrintImage(rp + i * D, t2);
        }
    }

    // GÉNÉRATION : z ~ N(0,1) -> Decode -> images inédites.
    printf("\n-- GÉNÉRATION depuis du bruit z ~ N(0,1) --\n");
    const uint32 G = 6;
    ...
    for (uint32 s = 0; s < G; ++s) {
        char title[40]; snprintf(title, sizeof(title), "échantillon %u (généré)", s);
        PrintImage(gp + s * D, title);
        double m = 0, v = 0;
        for (uint32 j = 0; j < D; ++j) m += gp[s * D + j]; m /= D;
        for (uint32 j = 0; j < D; ++j) { double d = gp[s * D + j] - m; v += d * d; }
        totalVar += v / D;
    }
    ...
    check(reconMse < 0.05,  "VAE reconstruit correctement (recon MSE < 0.05)");
    check(avgVar   > 0.01,  "les images générées ont de la STRUCTURE (pas uniformes)");

    printf("\n=== Résultat : %d OK, %d échec(s) (var moy générée = %.4f) ===\n", pass, fail, avgVar);
    return fail == 0 ? 0 : 1;
}

...
struct NkAtlasUploadContext {
		...
		static nk_handle Upload(nk_uint32 pageIndex, const nk_uint8 *pixels, nk_size byteSize, nk_uint32 width,
								nk_uint32 height, nk_uint32 channels, nk_handle existingHandle, void *userPtr) {
			auto *ctx = static_cast<NkAtlasUploadContext *>(userPtr);
			if (!ctx || !ctx->device || !pixels)
...
class NkTextRenderer {
	public:
		...
		//  init()
		// =========================================================================
		bool init(NkIDevice *device, const char *fontPath, NkRenderPassHandle swapRP,
		...
		void shutdown() {
			...
			if (mDs2D.IsValid())
				mDevice->FreeDescriptorSet(mDs2D);
			if (mDs3D.IsValid())
				mDevice->FreeDescriptorSet(mDs3D);
			if (mDsl2D.IsValid())
				mDevice->DestroyDescriptorSetLayout(mDsl2D);
			if (mDsl3D.IsValid())
				mDevice->DestroyDescriptorSetLayout(mDsl3D);
			if (mPipe2D.IsValid())
				mDevice->DestroyPipeline(mPipe2D);
			if (mPipe3D.IsValid())
				mDevice->DestroyPipeline(mPipe3D);
			if (mShader2D.IsValid())
				mDevice->DestroyShader(mShader2D);
			if (mShader3D.IsValid())
				mDevice->DestroyShader(mShader3D);
			if (mVbo2D.IsValid())
				mDevice->DestroyBuffer(mVbo2D);
			if (mVbo3D.IsValid())
				mDevice->DestroyBuffer(mVbo3D);
			if (mUbo2D.IsValid())
				mDevice->DestroyBuffer(mUbo2D);
			if (mUbo3D.IsValid())
				mDevice->DestroyBuffer(mUbo3D);
			if (mUboCam.IsValid())
				mDevice->DestroyBuffer(mUboCam);
			if (mSampler.IsValid())
		...
		// =========================================================================
		void drawText2D(const char *text, float x, float y, float sw, float sh, NkVec4f color = {1.f, 1.f, 1.f, 1.f},
						float ptSize = 0.f) {
			if (!mEnabled || !text || sw <= 0.f || sh <= 0.f)
				return;
			if (ptSize <= 0.f)
				ptSize = mSize2D;
			...
			if (!mShaper.ShapeUtf8(text, (nk_size)-1, sp, mShapeResult2D).IsOk())
				return;
			...
			for (nk_uint32 l = 0u; l < mShapeResult2D.lineCount; ++l) {
				...
				for (nk_uint32 gi = 0u; gi < run.glyphCount; ++gi) {
					if (mVerts2D.Size() + 6u > NK_MAX_VERTS_2D)
						break;

					const NkShapedGlyph &sg = run.Glyph(gi);
					if (sg.isWhitespace || sg.isLineBreak)
						continue;

					// Récupère le glyphe depuis le cache/rastériseur
					const NkCachedGlyph *glyph = nullptr;
					NkFontResult fr = mFontLib.GetGlyph(mFontId, sg.codepoint, ptSize, 96.f, rp, glyph);
					if (!fr.IsOk() || !glyph || !glyph->isValid || !glyph->isInAtlas)
						continue;
		...
		// =========================================================================
		void drawText3D(const char *text, const NkMat4f &worldMat, NkVec4f color = {1.f, 0.9f, 0.2f, 1.f},
			...
			if (!mShaper.ShapeUtf8(text, (nk_size)-1, sp, mShapeResult3D))
				return;
			...
			auto worldPt = [&](float lx, float ly) -> NkVec3f {
				...
				return NkVec3f(m[0] * lx + m[4] * ly + m[12], m[1] * lx + m[5] * ly + m[13],
							   m[2] * lx + m[6] * ly + m[14]);
			};
			...
			for (nk_uint32 l = 0u; l < mShapeResult3D.lineCount; ++l) {
				...
				for (nk_uint32 gi = 0u; gi < run.glyphCount; ++gi) {
					if (mVerts3D.Size() + 6u > NK_MAX_VERTS_3D)
						break;
		...
		//  endFrame()
		// =========================================================================
		void endFrame(NkICommandBuffer *cmd, const NkMat4f &matView, const NkMat4f &matProj) {
			if (!mEnabled || !cmd)
				return;
		...
		nk_uint32 glyphs2D() const {
			return mVerts2D.Size() / 6u;
		}

		nk_uint32 glyphs3D() const {
			return mVerts3D.Size() / 6u;
		}
	...
	private:
		...
		bool buildBuffers() {
			...
			if (!mUbo2D.IsValid() || !mUbo3D.IsValid() || !mUboCam.IsValid())
				return false;
		...
		bool buildDescriptorSets() {
			...
			if (!mDs2D.IsValid() || !mDs3D.IsValid())
				return false;
			bindUbos();
			return true;
		}
		...
		void refreshDescriptors() {
			if (!mAtlasCtx.hTexture.IsValid())
				return;
			if (mAtlasCtx.hTexture.id == mLastAtlasHandle)
				return;
...
static NkVector<Vtx3D> MakeCube(float r, float g, float b) {
	...
	for (const auto &f : faces)
		for (int i : idx)
			v.PushBack({NkVec3f(f.v[i][0], f.v[i][1], f.v[i][2]), NkVec3f(f.n[0], f.n[1], f.n[2]), NkVec3f(r, g, b)});
	return v;
}
...
int nkmain(const NkEntryState &state) {
	...
	NkWindow window;
	if (!window.Create(wc)) {
		logger.Info("[TextDemo] Fenetre : echec\n");
		return 1;
	}
	...
	if (!cmd) {
		logger.Info("[TextDemo] CommandBuffer : echec\n");
		return 1;
	}
	...
	eventSystem.AddEventCallback<NkWindowResizeEvent>([&](NkWindowResizeEvent *e) {
		W = (nk_uint32)e->GetWidth();
		H = (nk_uint32)e->GetHeight();
	});
	...
	while (running) {
		...
		// ── Draw cubes Phong ──────────────────────────────────────────────────
		{
			...
			if (hPhongDS.IsValid())
				cmd->BindDescriptorSet(hPhongDS, 0u);
		...
		// ❶ "NKENTSEU" — flotte et tourne au-dessus de la scène
		{
			...
			NkMat4f w = NkMat4f::Translation(NkVec3f(0.f, bobY, 0.f)) * NkMat4f::RotationY(NkAngle(rotAngle));
			textRenderer->drawText3D("NKENTSEU", w, {1.f, 0.85f, 0.15f, 1.f}, 72.f, true);
		}

		// ❷ "ENGINE" — couché sur le sol
		{
			NkMat4f w = NkMat4f::Translation(NkVec3f(0.f, -0.55f, 1.f)) * NkMat4f::RotationX(NkAngle(-90.f)) *
		...
		{
			NkMat4f w = NkMat4f::Translation(NkVec3f(-4.f, 1.f, -1.f)) * NkMat4f::RotationY(NkAngle(30.f));
			textRenderer->drawText3D("Rendu SDF\nfrom scratch\nsans FreeType", w, {0.95f, 0.95f, 0.95f, 0.85f}, 24.f,
									 false);
		}
		...
		// ❶ FPS
		{
			...
			textRenderer->drawText2D(buf, 12.f, 12.f, (float)W, (float)H, {1.f, 1.f, 0.3f, 1.f}, 18.f);
		}
		...
		// ❷ Backend
		{
			...
			textRenderer->drawText2D(buf, 12.f, 36.f, (float)W, (float)H, {0.65f, 0.85f, 1.f, 0.90f}, 13.f);
		}
		...
		// ❸ Stats — alignement droit avec measureText2DWidth (membre, pas local)
		{
			...
			snprintf(buf, sizeof(buf), "Atlas : %u page(s)   Cache : %.0f%%",
					 textRenderer->fontLib().Atlas().PageCount(), textRenderer->fontLib().Cache().HitRate() * 100.f);
			float tw = textRenderer->measureText2DWidth(buf, 12.f);
			textRenderer->drawText2D(buf, (float)W - tw - 14.f, 12.f, (float)W, (float)H, {0.45f, 1.f, 0.55f, 0.85f},
									 12.f);
		}

		// ❹ Contrôles bas-gauche
		textRenderer->drawText2D("F11 : plein ecran   |   ESC : quitter", 12.f, (float)H - 26.f, (float)W, (float)H,
								 {0.55f, 0.55f, 0.55f, 0.75f}, 12.f);

		// ❺ Description centre-bas
		textRenderer->drawText2D("Texte 2D (HUD SDF) + Texte 3D (espace monde)", (float)W * 0.5f - 260.f,
								 (float)H - 48.f, (float)W, (float)H, {0.88f, 0.78f, 0.95f, 0.80f}, 14.f);
	...
	if (hCubeA.IsValid())
		device->DestroyBuffer(hCubeA);
	if (hCubeB.IsValid())
		device->DestroyBuffer(hCubeB);
	if (hCubeC.IsValid())
		device->DestroyBuffer(hCubeC);
	if (hPhongDS.IsValid())
		device->FreeDescriptorSet(hPhongDS);
	if (hPhongDsl.IsValid())
		device->DestroyDescriptorSetLayout(hPhongDsl);
	if (hPhongUBO.IsValid())
		device->DestroyBuffer(hPhongUBO);
	if (hPhongPipe.IsValid())
		device->DestroyPipeline(hPhongPipe);
	if (hPhongShader.IsValid())
		device->DestroyShader(hPhongShader);

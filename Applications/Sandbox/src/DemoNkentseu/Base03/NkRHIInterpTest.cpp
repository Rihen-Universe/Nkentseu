...
static NkGraphicsApi ParseBackend(const NkVector<NkString> &args) {
	for (const auto &a : args) {
		...
		if (a == "-bdx11" || a == "--backend=dx11")
			return NkGraphicsApi::NK_GFX_API_DX11;
		if (a == "-bdx12" || a == "--backend=dx12")
			return NkGraphicsApi::NK_GFX_API_DX12;
		if (a == "-bgl" || a == "--backend=opengl")
...
static int ParseTest(const NkVector<NkString> &args) {
	for (const auto &a : args) {
		...
		if (a == "-t3")
			return 3;
		if (a == "-t4b")
			return 5; // 4 SRV : combine les 4
		if (a == "-t4")
			return 4; // 4 SRV : sample slot 0
		if (a == "-t5")
			return 6; // chaîne 3 passes (RT transients enchaînés)
		if (a == "-t6")
			return 7; // chaîne + HDR transient RGBA16F
		if (a == "-t7")
			return 8; // signature tonemap : 4 SRV set + push const b0/space1
	}
...
static bool IsDX(NkGraphicsApi a) {
	return a == NkGraphicsApi::NK_GFX_API_DX11 || a == NkGraphicsApi::NK_GFX_API_DX12;
}

static bool IsVK(NkGraphicsApi a) {
	return a == NkGraphicsApi::NK_GFX_API_VULKAN;
}

// Construit un NkShaderDesc cross-backend depuis GLSL (GL+VK) ou HLSL (DX).
static NkShaderHandle MakeShader(NkIDevice *dev, NkGraphicsApi api, const char *dbg, const char *glslVS,
								 const char *glslFS, const char *hlslVS, const char *hlslFS,
								 const char *hlslVSEntry = "VSMain", const char *hlslPSEntry = "PSMain") {
	NkShaderDesc sd;
	sd.debugName = dbg;
	if (IsDX(api)) {
		...
		if (!vr.success || !fr.success) {
			logger.Info("[InterpTest] SPIRV compile FAIL ({0})\n", dbg);
			return {};
		}
...
static NkTextureHandle MakeSolidTex(NkIDevice *dev, uint32 w, uint32 h, uint8 r, uint8 g, uint8 b, const char *dbg) {
...
static void ReadbackAndLog(NkIDevice *dev, NkICommandBuffer *cmd, NkTextureHandle rt, uint32 w, uint32 h,
						   const char *tag, const char *apiName) {
	...
	if (!rb.IsValid()) {
		logger.Info("[InterpTest] readback buffer FAIL\n");
		return;
	}
	...
	NkVector<uint8> px;
	px.Resize((uint32)sz);
	if (!dev->ReadBuffer(rb, px.Data(), sz, 0)) {
		logger.Info("[InterpTest] ReadBuffer FAIL\n");
		dev->DestroyBuffer(rb);
		return;
	}

	auto P = [&](uint32 x, uint32 y) {
		const uint8 *p = &px[(y * w + x) * 4];
		return NkFormat("({0},{1},{2})", (int)p[0], (int)p[1], (int)p[2]);
	};
	uint32 cx = w / 2, cy = h / 2;
	logger.Info("[InterpTest] {0} {1}: TL={2} TR={3} BL={4} BR={5} C={6}\n", tag, apiName, P(2, 2).CStr(),
				P(w - 3, 2).CStr(), P(2, h - 3).CStr(), P(w - 3, h - 3).CStr(), P(cx, cy).CStr());
	dev->DestroyBuffer(rb);
}
...
int nkmain(const NkEntryState &state) {
	...
	NkWindowConfig wc;
	wc.title = NkFormat("InterpTest T{0} {1}", test, apiName);
	wc.width = 512;
	wc.height = 512;
	wc.centered = true;
	wc.resizable = false;
	NkWindow window;
	if (!window.Create(wc)) {
		logger.Info("[InterpTest] window FAIL\n");
		return 1;
	}

	NkDeviceInitInfo ii;
	ii.api = api;
	ii.surface = window.GetSurfaceDesc();
	ii.width = window.GetSize().width;
	ii.height = window.GetSize().height;
	ii.context.vulkan.validationLayers = false;
	ii.context.vulkan.debugMessenger = false;
	ii.context.vulkan.srgbSwapchain = false;
	NkIDevice *dev = NkDeviceFactory::Create(ii);
	if (!dev || !dev->IsValid()) {
		logger.Info("[InterpTest] device FAIL\n");
		window.Close();
		return 1;
	}
	...
	fbd.renderPass = hRTRP;
	fbd.colorAttachments.PushBack(hRT);
	fbd.depthAttachment = hRTDepth;
	fbd.width = RTW;
	fbd.height = RTH;
	...
	NkShaderHandle hGeomSh;
	NkPipelineHandle hGeomPipe;
	NkBufferHandle hVBO;
	if (test == 1 || test == 3) {
		...
		pd.renderPass = hRTRP;
		pd.debugName = "geomPipe";
		hGeomPipe = dev->CreateGraphicsPipeline(pd);
	}

	// ── T2 fullscreen pipeline (VertexID, no VBO) ────────────────────────────
	NkShaderHandle hFsSh;
	NkPipelineHandle hFsPipe;
	if (test == 2) {
		// Sélection de la variante de morphing HLSL (GL/VK gardent le GLSL de base).
		const char *hvs = kHLSL_FsVS;
		const char *hps = kHLSL_FsPS;
		const char *eVS = "VSMain";
		const char *ePS = "PSMain";
		switch (variant) {
			case 1:
				hvs = kHLSL_v1VS;
				hps = kHLSL_v1PS;
				break; // +SV_IsFrontFace
			case 2:
				hvs = kHLSL_v2VS;
				hps = kHLSL_v2PS;
				break; // structs nommés + SV_InstanceID
			case 3:
				hvs = kHLSL_v3VS;
				hps = kHLSL_v3PS;
				eVS = "main";
				ePS = "main";
				break; // exact générateur
			default:
				break;
		}
		...
		NkGraphicsPipelineDesc pd;
		pd.shader = hFsSh; // pas de vertexLayout
		pd.topology = NkPrimitiveTopology::NK_TRIANGLE_LIST;
		pd.rasterizer = NkRasterizerDesc::NoCull();
		pd.depthStencil = NkDepthStencilDesc::NoDepth();
		pd.blend = NkBlendDesc::Opaque();
	...
	NkTextureHandle hSrcRT;
	NkFramebufferHandle hSrcFBO;
	NkRenderPassHandle hSrcRP = hRTRP;
	NkShaderHandle hSampSh;
	NkPipelineHandle hSampPipe;
	NkSamplerHandle hSamp;
	NkDescSetHandle hSampLayout;
	NkDescSetHandle hSampSet;
	if (test == 3) {
		...
		NkFramebufferDesc sfbd;
		sfbd.renderPass = hRTRP;
		sfbd.colorAttachments.PushBack(hSrcRT);
		sfbd.depthAttachment = hRTDepth;
		sfbd.width = RTW;
		sfbd.height = RTH;
		hSrcFBO = dev->CreateFramebuffer(sfbd);
		auto eff = dev->GetFramebufferRenderPass(hSrcFBO);
		if (eff.IsValid())
			hSrcRP = eff;
		...
		pd.blend = NkBlendDesc::Opaque();
		pd.renderPass = hRTRP;
		pd.debugName = "sampPipe";
		if (hSampLayout.IsValid())
			pd.descriptorSetLayouts.PushBack(hSampLayout);
	...
	NkTextureHandle hT4[4];
	for (auto &h : hT4)
		h = {};
	NkSamplerHandle hT4Samp;
	NkDescSetHandle hT4Layout, hT4Set;
	NkShaderHandle hT4Sh;
	NkPipelineHandle hT4Pipe;
	...
	if (test == 4 || test == 5 || test == 8) {
		...
		pd.blend = NkBlendDesc::Opaque();
		pd.renderPass = hRTRP;
		pd.debugName = "multi4Pipe";
		if (hT4Layout.IsValid())
			pd.descriptorSetLayouts.PushBack(hT4Layout);
		if (t7)
			pd.AddPushConstant(NkShaderStage::NK_ALL_GRAPHICS, 0, 48); // p0,p1,p2
	...
	NkTextureHandle hChA, hChB;
	NkFramebufferHandle hChAFBO, hChBFBO;
	NkRenderPassHandle hChARP = hRTRP, hChBRP = hRTRP;
	NkSamplerHandle hChSamp;
	NkDescSetHandle hChLayout, hChSetA, hChSetB;
	NkShaderHandle hChGeomSh, hChSampSh;
	NkPipelineHandle hChGeomPipe, hChSampPipeA, hChSampPipeB;
	...
	if (test == 6 || test == 7) {
		...
		tad.bindFlags = tad.bindFlags | NkBindFlags::NK_SHADER_RESOURCE;
		tad.debugName = "chainA";
		hChA = dev->CreateTexture(tad);
		NkRenderPassDesc rpA;
		rpA.AddColor(NkAttachmentDesc::Color(fmtA)).SetDepth(NkAttachmentDesc::Depth());
		hChARP = dev->CreateRenderPass(rpA);
		NkFramebufferDesc fbA;
		fbA.renderPass = hChARP;
		fbA.colorAttachments.PushBack(hChA);
		fbA.depthAttachment = hRTDepth;
		fbA.width = RTW;
		fbA.height = RTH;
		hChAFBO = dev->CreateFramebuffer(fbA);
		{
			auto e = dev->GetFramebufferRenderPass(hChAFBO);
			if (e.IsValid())
				hChARP = e;
		}
		...
		tbd.bindFlags = tbd.bindFlags | NkBindFlags::NK_SHADER_RESOURCE;
		tbd.debugName = "chainB";
		hChB = dev->CreateTexture(tbd);
		NkFramebufferDesc fbB;
		fbB.renderPass = hRTRP;
		fbB.colorAttachments.PushBack(hChB);
		fbB.depthAttachment = hRTDepth;
		fbB.width = RTW;
		fbB.height = RTH;
		hChBFBO = dev->CreateFramebuffer(fbB);
		{
			auto e = dev->GetFramebufferRenderPass(hChBFBO);
			if (e.IsValid())
				hChBRP = e;
		}
		...
		hChVBO = dev->CreateBuffer(NkBufferDesc::Vertex(sizeof(tri), tri));
		{
			...
			pd.depthStencil = NkDepthStencilDesc::NoDepth();
			pd.blend = NkBlendDesc::Opaque();
			pd.renderPass = hChARP;
			pd.debugName = "chGeomPipe";
		...
		hChSampSh = MakeShader(dev, api, "chsamp", kGLSL_SampVS, kGLSL_SampFS, kHLSL_SampVS, kHLSL_SampPS);
		{
			...
			pd.depthStencil = NkDepthStencilDesc::NoDepth();
			pd.blend = NkBlendDesc::Opaque();
			pd.renderPass = hChBRP;
			pd.debugName = "chSampPipeB";
			if (hChLayout.IsValid())
				pd.descriptorSetLayouts.PushBack(hChLayout);
	...
	while (running) {
		...
		if ((test == 6 || test == 7) && hChGeomPipe.IsValid()) {
			...
			if (hChSetA.IsValid())
				cmd->BindDescriptorSet(hChSetA, 0);
			cmd->Draw(3, 1, 0, 0);
			cmd->EndRenderPass();
		}

		// Passe principale dans hRT (cible de readback)
		cmd->SetClearColor(0.1f, 0.1f, 0.5f, 1.f);
		cmd->SetClearDepth(1.f); // bleu = baseline T0
		...
		if (test == 1 && hGeomPipe.IsValid()) {
			...
			if (hChSetB.IsValid())
				cmd->BindDescriptorSet(hChSetB, 0);
			cmd->Draw(3, 1, 0, 0);
		}
		cmd->EndRenderPass();
		cmd->End();
		...
		if (!readbackDone && frameCount >= 2) {
			readbackDone = true;
			char tag[8];
			snprintf(tag, sizeof(tag), "T%d", test);
		...
		frameCount++;
		if (frameCount >= 6)
			running = false; // auto-quit après readback + qq frames
	}

	dev->WaitIdle();
	// ── Nettoyage (Create→Destroy) ───────────────────────────────────────────
	if (hChSampPipeA.IsValid())
		dev->DestroyPipeline(hChSampPipeA);
	if (hChSampPipeB.IsValid())
		dev->DestroyPipeline(hChSampPipeB);
	if (hChGeomPipe.IsValid())
		dev->DestroyPipeline(hChGeomPipe);
	if (hChSampSh.IsValid())
		dev->DestroyShader(hChSampSh);
	if (hChGeomSh.IsValid())
		dev->DestroyShader(hChGeomSh);
	if (hChVBO.IsValid())
		dev->DestroyBuffer(hChVBO);
	if (hChSetA.IsValid())
		dev->FreeDescriptorSet(hChSetA);
	if (hChSetB.IsValid())
		dev->FreeDescriptorSet(hChSetB);
	if (hChLayout.IsValid())
		dev->DestroyDescriptorSetLayout(hChLayout);
	if (hChSamp.IsValid())
		dev->DestroySampler(hChSamp);
	if (hChAFBO.IsValid())
		dev->DestroyFramebuffer(hChAFBO);
	if (hChBFBO.IsValid())
		dev->DestroyFramebuffer(hChBFBO);
	if (hChARP.IsValid() && hChARP.id != hRTRP.id)
		dev->DestroyRenderPass(hChARP);
	if (hChA.IsValid())
		dev->DestroyTexture(hChA);
	if (hChB.IsValid())
		dev->DestroyTexture(hChB);
	if (hT4Pipe.IsValid())
		dev->DestroyPipeline(hT4Pipe);
	if (hT4Sh.IsValid())
		dev->DestroyShader(hT4Sh);
	if (hT4Set.IsValid())
		dev->FreeDescriptorSet(hT4Set);
	if (hT4Layout.IsValid())
		dev->DestroyDescriptorSetLayout(hT4Layout);
	if (hT4Samp.IsValid())
		dev->DestroySampler(hT4Samp);
	for (auto &h : hT4)
		if (h.IsValid())
			dev->DestroyTexture(h);
	if (hSampSet.IsValid())
		dev->FreeDescriptorSet(hSampSet);
	if (hSampLayout.IsValid())
		dev->DestroyDescriptorSetLayout(hSampLayout);
	if (hSamp.IsValid())
		dev->DestroySampler(hSamp);
	if (hSampPipe.IsValid())
		dev->DestroyPipeline(hSampPipe);
	if (hSampSh.IsValid())
		dev->DestroyShader(hSampSh);
	if (hSrcFBO.IsValid())
		dev->DestroyFramebuffer(hSrcFBO);
	if (hSrcRT.IsValid())
		dev->DestroyTexture(hSrcRT);
	if (hFsPipe.IsValid())
		dev->DestroyPipeline(hFsPipe);
	if (hFsSh.IsValid())
		dev->DestroyShader(hFsSh);
	if (hGeomPipe.IsValid())
		dev->DestroyPipeline(hGeomPipe);
	if (hGeomSh.IsValid())
		dev->DestroyShader(hGeomSh);
	if (hVBO.IsValid())
		dev->DestroyBuffer(hVBO);
	dev->DestroyCommandBuffer(cmd);
	if (hRTFBO.IsValid())
		dev->DestroyFramebuffer(hRTFBO);
	if (hRTRP.IsValid())
		dev->DestroyRenderPass(hRTRP);
	if (hRTDepth.IsValid())
		dev->DestroyTexture(hRTDepth);
	if (hRT.IsValid())
		dev->DestroyTexture(hRT);

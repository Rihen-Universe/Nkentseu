...
static NkVector<Vtx3D> MakeCube(float r = 1.f, float g = 0.45f, float b = 0.2f) {
	...
	for (const auto &f : faces)
		for (int i : idx)
			v.PushBack({NkVec3f(f.vx[i][0], f.vx[i][1], f.vx[i][2]), NkVec3f(f.nx, f.ny, f.nz),
	...
	for (int i = 0; i < stacks; i++) {
		...
		for (int j = 0; j < slices; j++) {
			...
			auto mk = [&](float phi, float th) -> Vtx3D {
				...
				return {NkVec3f(x * .5f, y * .5f, z * .5f), NkVec3f(x, y, z), NkVec4f(r, g, b, 0.f),
...
static bool IsSupportedImageExtension(const std::string &e) {
	return e == ".png" || e == ".jpg" || e == ".jpeg" || e == ".bmp" || e == ".tga" || e == ".hdr" || e == ".ppm" ||
		   e == ".pgm" || e == ".pbm" || e == ".qoi" || e == ".gif" || e == ".ico" || e == ".webp" || e == ".svg";
}
...
static int TextureCandidateScore(const std::filesystem::path &p) {
	...
	int score = 0;
	if (fl.find("checker") != std::string::npos)
		score += 200;
	if (fl.find("albedo") != std::string::npos)
		score += 850;
	if (fl.find("diffuse") != std::string::npos)
		score += 900;
	if (fl.find("basecolor") != std::string::npos)
		score += 820;
	if (fl.find("color") != std::string::npos)
		score += 300;
	if (fl.find("brick") != std::string::npos)
		score += 120;
	if (fl.find("ao") != std::string::npos || fl.find("ambient") != std::string::npos)
		score -= 800;
	if (fl.find("normal") != std::string::npos)
		score -= 900;
	if (fl.find("rough") != std::string::npos)
		score -= 900;
	if (fl.find("metal") != std::string::npos)
		score -= 900;
	if (fl.find("spec") != std::string::npos)
		score -= 700;
	if (fl.find("height") != std::string::npos || fl.find("disp") != std::string::npos)
		score -= 700;
	if (pl.find("resources\\textures") != std::string::npos || pl.find("resources/textures") != std::string::npos)
		score += 120;
...
static std::string FindTextureInResources() {
	...
	for (const fs::path &root : roots) {
		std::error_code e;
		if (!fs::exists(root, e) || !fs::is_directory(root, e))
			continue;
	...
	std::sort(cands.begin(), cands.end(),
			  [](const Cand &a, const Cand &b) { return a.score != b.score ? a.score > b.score : a.path < b.path; });
	return cands.front().path;
}

static bool SaveImageVariants(const NkImage &img, const std::filesystem::path &dir, const std::string &stem) {
	namespace fs = std::filesystem;
	std::error_code ec;
...
static bool SaveSoftwareScene(NkSoftwareDevice *swDev, const std::filesystem::path &dir, const std::string &stem) {
	if (!swDev)
		return false;
...
// =============================================================================
static bool CompileNkSLShader(NkIDevice *device, NkGraphicsApi api, const char *nkslVertSrc, const char *nkslFragSrc,
							  const char *debugName, NkShaderDesc &outDesc,
	...
	// ── Vertex ───────────────────────────────────────────────────────────────
	auto vertResult = compiler.CompileWithReflection(NkString(nkslVertSrc), NkSLStage::NK_VERTEX, target, opts,
	...
	if (!vertResult.result.success) {
		...
		logger.Infof("[NkSL] %s vertex OK (%zu bytes source, %zu bytes bytecode) target=%s\n", debugName,
					 (size_t)vertResult.result.source.Size(), (size_t)vertResult.result.bytecode.Size(),
	...
	// ── Fragment ─────────────────────────────────────────────────────────────
	auto fragResult = compiler.CompileWithReflection(NkString(nkslFragSrc), NkSLStage::NK_FRAGMENT, target, opts,
	...
	if (!fragResult.result.success) {
		...
		logger.Infof("[NkSL] %s fragment OK (%zu bytes source) target=%s\n", debugName,
					 (size_t)fragResult.result.source.Size(), NkSLTargetName(target));
	...
	if (outReflect)
		*outReflect = vertResult.reflection;
...
static std::string FindFontFile() {
	...
	const std::vector<std::string> pref = {"Roboto-Regular.ttf",  "Roboto-Light.ttf",  "OpenSans-Light.ttf",
...
struct NkFontLibrary {
		...
		NkFontFace *LoadFont(const void *, usize, uint32) {
			return nullptr;
		}
...
static void Ortho2DMVP(float x, float y, float scaleX, float scaleY, float W, float H, bool depthZeroToOne,
					   float out[16]) {
	const float l = 0.f, r = W, t = H, b = 0.f, nearZ = -1.f, farZ = 1.f;
	const float rl = 1.f / (r - l), tb = 1.f / (t - b), fn = 1.f / (farZ - nearZ);
	const float ortho[16] = {2.f * rl,
							 0.f,
							 0.f,
							 0.f,
							 0.f,
							 ...
							 0.f,
							 0.f,
							 (depthZeroToOne ? fn : -2.f * fn),
							 0.f,
							 -(r + l) * rl,
							 -(t + b) * tb,
							 (depthZeroToOne ? -nearZ * fn : (farZ + nearZ) * fn),
							 1.f};
	const float model[16] = {scaleX, 0.f, 0.f, 0.f, 0.f, scaleY, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f, x, y, 0.f, 1.f};
	for (int col = 0; col < 4; col++)
		for (int row = 0; row < 4; row++) {
			float s = 0.f;
			for (int k = 0; k < 4; k++)
				s += ortho[k * 4 + row] * model[col * 4 + k];
			out[col * 4 + row] = s;
		}
}

static void Billboard3DMVP(const NkVec3f &worldPos, float worldW, float worldH, const NkMat4f &view,
						   const NkMat4f &proj, bool flipBillY, float out[16]) {
	...
	model.data[0] = rx * worldW;
	model.data[1] = ry * worldW;
	model.data[2] = rz * worldW;
	model.data[4] = ux * worldH;
	model.data[5] = uy * worldH;
	model.data[6] = uz * worldH;
...
static NkTextQuad CreateTextQuad(NkIDevice *device, NkDescSetHandle hTextLayout, NkFontFace *face, const char *text,
								 bool flipV = false) {
	NkTextQuad q{};
	if (!device || !face || !text || !*text)
		return q;
	uint32 bmpW = 0, bmpH = 0;
	auto bmp = BuildStringBitmap(face, text, bmpW, bmpH);
	if (bmp.empty() || !bmpW || !bmpH)
		return q;
	...
	if (hTextLayout.IsValid()) {
		...
		if (q.hDesc.IsValid()) {
			...
			w[0].buffer = q.hUBO;
			w[0].bufferOffset = 0;
			w[0].bufferRange = sizeof(TextUboData);
			w[1].set = q.hDesc;
			w[1].binding = 1;
			w[1].type = NkDescriptorType::NK_COMBINED_IMAGE_SAMPLER;
			w[1].texture = q.hTex;
			w[1].sampler = q.hSampler;
...
static void DestroyTextQuad(NkIDevice *device, NkTextQuad &q) {
	if (!device)
		return;
	if (q.hDesc.IsValid())
		device->FreeDescriptorSet(q.hDesc);
	if (q.hUBO.IsValid())
		device->DestroyBuffer(q.hUBO);
	if (q.hVBO.IsValid())
		device->DestroyBuffer(q.hVBO);
	if (q.hSampler.IsValid())
		device->DestroySampler(q.hSampler);
	if (q.hTex.IsValid())
		device->DestroyTexture(q.hTex);
	q = {};
}

static void RenderTextQuad(NkICommandBuffer *cmd, NkIDevice *device, const NkTextQuad &q, NkPipelineHandle hTextPipe,
						   const float mvp[16], float r, float g, float b, float a) {
	if (!cmd || !device || !q.hVBO.IsValid() || !q.hUBO.IsValid() || !hTextPipe.IsValid())
		return;
	TextUboData ubo{};
	...
	if (q.hDesc.IsValid())
		cmd->BindDescriptorSet(q.hDesc, 0);
...
int nkmain(const NkEntryState &state) {
	...
	dii.api = targetApi;
	dii.surface = surface;
	dii.height = window.GetSize().height;
	dii.width = window.GetSize().width;
	...
	const bool depthZeroToOne =
		targetApi == NkGraphicsApi::NK_GFX_API_VULKAN || targetApi == NkGraphicsApi::NK_GFX_API_DX11 ||
		targetApi == NkGraphicsApi::NK_GFX_API_DX12 || targetApi == NkGraphicsApi::NK_GFX_API_METAL;
	...
	// ── Shader principal ─────────────────────────────────────────────────────
	NkShaderDesc mainShaderDesc;
	mainShaderDesc.debugName = "Phong3D_NkSL";
	NkSLReflection mainReflect;
	if (!CompileNkSLShader(device, targetApi, kNkSL_MainVert, kNkSL_MainFrag, "Phong3D", mainShaderDesc, mainSrcs,
						   &mainReflect)) {
	...
	logger.Infof("[NkSL] Main shader reflection: %u resources, %u vertex inputs\n",
				 (unsigned)mainReflect.resources.Size(), (unsigned)mainReflect.vertexInputs.Size());
	...
	for (auto &vi : mainReflect.vertexInputs)
		logger.Infof("[NkSL]   vertex input location=%u name=%s components=%u\n", vi.location, vi.name.CStr(),
					 vi.components);

	// ── Shadow shader ─────────────────────────────────────────────────────────
	NkShaderDesc shadowShaderDesc;
	shadowShaderDesc.debugName = "Shadow_NkSL";
	...
	if (targetApi != NkGraphicsApi::NK_GFX_API_METAL) {
		shadowShaderOk = CompileNkSLShader(device, targetApi, kNkSL_ShadowVert, kNkSL_ShadowFrag, "Shadow",
										   shadowShaderDesc, shadowSrcs, nullptr);
		if (!shadowShaderOk)
			logger.Warnf("[NkSL] Shadow shader failed — shadow map disabled\n");
	}

	// ── Shader texte ──────────────────────────────────────────────────────────
	NkShaderDesc textShaderDesc;
	textShaderDesc.debugName = "Text_NkSL";
	bool textShaderOk =
		CompileNkSLShader(device, targetApi, kNkSL_TextVert, kNkSL_TextFrag, "Text", textShaderDesc, textSrcs, nullptr);
	if (!textShaderOk)
	...
	const bool shaderNeedsShadowSampler =
		targetApi == NkGraphicsApi::NK_GFX_API_OPENGL || targetApi == NkGraphicsApi::NK_GFX_API_VULKAN ||
		targetApi == NkGraphicsApi::NK_GFX_API_DX11 || targetApi == NkGraphicsApi::NK_GFX_API_DX12;
	const bool shaderNeedsAlbedoSampler = targetApi != NkGraphicsApi::NK_GFX_API_SOFTWARE;
	const bool wantsShadowResources = shaderNeedsShadowSampler || targetApi == NkGraphicsApi::NK_GFX_API_SOFTWARE;
	...
	pipeDesc.debugName = "Pipeline_NkSL_Phong";
	if (hLayout.IsValid())
	...
	if (wantsShadowResources) {
		...
		ssd.magFilter = NkFilter::NK_LINEAR;
		ssd.minFilter = NkFilter::NK_LINEAR;
		ssd.mipFilter = NkMipFilter::NK_NONE;
		ssd.minLod = ssd.maxLod = 0.f;
		...
		if (hasShadowMap && shadowShaderOk && hShadowShader.IsValid() && !HasArg(state.GetArgs(), "--safe-shadows")) {
			NkVertexLayout svl;
			svl.AddAttribute(0, 0, NkGPUFormat::NK_RGB32_FLOAT, 0, "POSITION", 0).AddBinding(0, sizeof(Vtx3D));
			NkGraphicsPipelineDesc spd{};
			spd.shader = hShadowShader;
			spd.vertexLayout = svl;
			...
			spd.renderPass = hShadowRP;
			spd.debugName = "ShadowPipeline_NkSL";
			if (hLayout.IsValid())
				spd.descriptorSetLayouts.PushBack(hLayout);
	...
	if (loadedTextureImage && loadedTextureImage->IsValid()) {
		...
		NkSamplerDesc asd{};
		asd.magFilter = NkFilter::NK_LINEAR;
		asd.minFilter = NkFilter::NK_LINEAR;
	...
	if (targetApi == NkGraphicsApi::NK_GFX_API_SOFTWARE) {
		...
		if (sw && getenv("NK_SW_VM")) {
			...
			NkSLCompileOptions bcOpts;
			bcOpts.optimize = false;
			bcOpts.debugInfo = true;
			...
			if (!vr.result.success)
				for (auto &e : vr.result.errors)
					logger.Errorf("[NkSL-VM] vert: line %u: %s\n", e.line, e.message.CStr());
			if (!fr.result.success)
				for (auto &e : fr.result.errors)
					logger.Errorf("[NkSL-VM] frag: line %u: %s\n", e.line, e.message.CStr());
				...
				if (dv && df && s_vmVert.IsValid() && s_vmFrag.IsValid()) {
					...
					sw->vertFn = [vStride](const void *vdata, uint32 idx, const void *udata) -> NkVertexSoftware {
						...
						bool ok = NkSLVM::Execute(s_vmVert, env);
						{
							...
							if (s_vl < 3) {
								...
								logger.Infof("[NkSL-VM] vert#%d ok=%d pos=(%.3f,%.3f,%.3f,%.3f) wp=(%.2f,%.2f,%.2f)\n",
											 s_vl, ok ? 1 : 0, outBuf[0], outBuf[1], outBuf[2], outBuf[3], outBuf[4],
											 outBuf[5], outBuf[6]);
							}
						}
						...
						for (uint32 k = 0; k < nv && k < 16; ++k)
							out.attrs[k] = outBuf[4 + k]; // varyings
						out.attrCount = nv;
						return out;
					};
					sw->fragFn = [swShadowTex, swAlbedoTex](const NkVertexSoftware &frag, const void *udata,
															const void *) -> math::NkVec4f {
						SwTexCtx ctx;
						for (auto &smp : s_vmFrag.samplers)
							if (smp.index < 8) {
								ctx.tex[smp.index] = smp.isShadow ? swShadowTex : swAlbedoTex;
								ctx.isShadow[smp.index] = smp.isShadow;
							}
						float outBuf[8] = {0};
						NkSLVMEnv env;
						...
						env.sampleTex = [](void *c, int s, float u, float v, float /*lod*/, float o[4]) {
							auto *t = (SwTexCtx *)c;
							NkSWTexture *tex = (s >= 0 && s < 8) ? t->tex[s] : nullptr;
							if (tex) {
								auto col = tex->Sample(u, v, 0);
								o[0] = col.x;
								o[1] = col.y;
								o[2] = col.z;
								o[3] = col.w;
						...
						env.sampleShadow = [](void *c, int s, float u, float v, float z) -> float {
							...
							uint32 W = tex->Width(), H = tex->Height();
							if (!W || !H)
								return 1.f;
						...
						env.texSize = [](void *c, int s, float o[2]) {
							auto *t = (SwTexCtx *)c;
							NkSWTexture *tex = (s >= 0 && s < 8) ? t->tex[s] : nullptr;
							o[0] = tex ? (float)tex->Width() : 1.f;
							o[1] = tex ? (float)tex->Height() : 1.f;
						};
						bool ok = NkSLVM::Execute(s_vmFrag, env);
						{
							...
							if (s_fl < 3) {
								s_fl++;
								logger.Infof(
									"[NkSL-VM] frag#%d ok=%d color=(%.3f,%.3f,%.3f,%.3f) in0=(%.2f,%.2f,%.2f)\n", s_fl,
									ok ? 1 : 0, outBuf[0], outBuf[1], outBuf[2], outBuf[3], frag.attrs[0],
									frag.attrs[1], frag.attrs[2]);
							}
						}
					...
					vmOk = true;
					logger.Infof(
						"[NkSL-VM] Software : VM bytecode ACTIVE (vert outFloats=%u, frag inFloats=%u, samplers=%zu)\n",
						s_vmVert.outputFloats, s_vmFrag.inputFloats, (size_t)s_vmFrag.samplers.Size());
				}
			}
			if (!vmOk)
				logger.Infof("[NkSL-VM] Software : bytecode KO -> fallback lambdas manuelles\n");
		}
		...
		if (sw && !vmOk) {
			sw->vertFn = [](const void *vdata, uint32 idx, const void *udata) -> NkVertexSoftware {
				...
				out.attrs[0] = wp.x;
				out.attrs[1] = wp.y;
				out.attrs[2] = wp.z; // worldPos
				NkVec4f lsc = mul4(ubo->lightVP, wp.x, wp.y, wp.z, wp.w);
				out.attrs[3] = lsc.x;
				out.attrs[4] = lsc.y; // shadowCoord
				out.attrs[5] = lsc.z;
				out.attrs[6] = lsc.w;
				out.attrs[7] = v->uv.x;
				out.attrs[8] = v->uv.y;	   // uv (0..kPlaneUVScale)
			...
			sw->fragFn = [swShadowTex, swAlbedoTex](const NkVertexSoftware &frag, const void *udata,
				...
				if (diff < 0.f)
					diff = 0.f;
				float vx = ubo->eyePosW[0] - frag.attrs[0], vy = ubo->eyePosW[1] - frag.attrs[1],
					  vz = ubo->eyePosW[2] - frag.attrs[2];
				float vl2 = vx * vx + vy * vy + vz * vz;
				if (vl2 > 1e-6f) {
					float iv = 1.f / NkSqrt(vl2);
					vx *= iv;
					vy *= iv;
					vz *= iv;
				}
				...
				if (hl2 > 1e-6f) {
					float ih = 1.f / NkSqrt(hl2);
					hx *= ih;
					hy *= ih;
					hz *= ih;
				}
				float sp = nx * hx + ny * hy + nz * hz;
				if (sp < 0.f)
					sp = 0.f;
				float spec = sp * sp;
				spec *= spec;
				...
				if (swAlbedoTex && frag.attrs[9] > 0.5f) {
					...
					float u = frag.attrs[7] * (1.f / kPlaneUVScale);
					u = u < 0.f ? 0.f : (u > 1.f ? 1.f : u);
					float v = frag.attrs[8] * (1.f / kPlaneUVScale);
				...
				if (swShadowTex) {
					...
					if (su >= 0.f && su <= 1.f && sv >= 0.f && sv <= 1.f && sz <= 1.f) {
						...
						if (px >= swShadowTex->Width())
							px = swShadowTex->Width() - 1;
						if (py >= swShadowTex->Height())
							py = swShadowTex->Height() - 1;
	...
	if (textShaderOk && hTextShader.IsValid() && hTextLayout.IsValid()) {
		...
		tpd.renderPass = hRP;
		tpd.debugName = "Pipeline_NkSL_Text";
	...
	const std::filesystem::path capturesDir = std::filesystem::path("Build") / "Captures" / "NkRHIDemoNkSL";
	...
	while (running) {
		...
		if (hasShadowMap && hShadowFBO.IsValid() && hShadowRP.IsValid()) {
			...
			if (ok && useRealShadowPass && hShadowPipe.IsValid()) {
				...
				auto fillShadow = [&](UboData &su, const NkMat4f &mm) {
					...
					Mat4ToArray(id, su.view);
					Mat4ToArray(id, su.proj);
		...
		// ── Texte 3D billboard ───────────────────────────────────────────────
		if (textOk) {
			const bool flipBillY =
				(targetApi == NkGraphicsApi::NK_GFX_API_VULKAN || targetApi == NkGraphicsApi::NK_GFX_API_DX12);
			float bbMVP[16];
	...
	if (hTextPipe.IsValid())
		device->DestroyPipeline(hTextPipe);
	if (hTextShader.IsValid())
		device->DestroyShader(hTextShader);
	if (hTextLayout.IsValid())
		device->DestroyDescriptorSetLayout(hTextLayout);

...
class NkFontRenderer {
	public:
		...
		bool Rebuild(NkRenderPassHandle rp, bool sdf) {
			...
			if (!BuildLayouts())
				return false;
			if (!BuildPipelines(rp))
				return false;
			if (!BuildBuffers())
				return false;
			if (!BuildDescriptors())
				return false;
			if (!BuildPipeline3DNoTex(rp))
				return false;
		...
		void DrawText2D(NkFont *fnt, const char *text, float px, float py, float sw, float sh, float r = 1, float g = 1,
						float b = 1, float a = 1, float scale = 1.f) {
			if (!mEnabled || !fnt || !text)
				return;
		...
		static void TriangulateContour(const NkVector<NkVec2f> &contour, NkVector<uint32_t> &outIndices) {
			if (contour.Size() < 3)
				return;
		...
		void GenerateText3DMesh(NkFont *fnt, const char *text, const NkMat4f &worldMatrix, float r, float g, float b,
								float a, float extrusionDepth, NkText3DVertexCallback callback, void *userData) const {
			if (!fnt || !text || !callback)
				return;
			...
			auto WorldPos = [&](float lx, float ly, float lz) -> NkVec3f {
				...
				return NkVec3f{m[0] * lx + m[4] * ly + m[8] * lz + m[12], m[1] * lx + m[5] * ly + m[9] * lz + m[13],
			...
			while (*p) {
				...
				if (!fnt->GetGlyphOutlinePoints(cp, outline))
					continue;
				if (outline.Size() < 3)
					continue;
				...
				for (const auto &contour : contours) {
					...
					for (size_t t = 0; t + 2 < triIndices.Size(); t += 3) {
						...
						FontVtx3D_NoTex tri[3] = {{frontPts[i0].x, frontPts[i0].y, frontPts[i0].z, normFront.x,
												   normFront.y, normFront.z, r, g, b, a},
												  {frontPts[i1].x, frontPts[i1].y, frontPts[i1].z, normFront.x,
												   normFront.y, normFront.z, r, g, b, a},
												  {frontPts[i2].x, frontPts[i2].y, frontPts[i2].z, normFront.x,
												   normFront.y, normFront.z, r, g, b, a}};
					...
					for (size_t t = 0; t + 2 < triIndices.Size(); t += 3) {
						uint32_t i0 = triIndices[t], i1 = triIndices[t + 1], i2 = triIndices[t + 2];
						FontVtx3D_NoTex tri[3] = {{backPts[i0].x, backPts[i0].y, backPts[i0].z, normBack.x, normBack.y,
												   normBack.z, r, g, b, a},
												  {backPts[i2].x, backPts[i2].y, backPts[i2].z, normBack.x, normBack.y,
												   normBack.z, r, g, b, a},
												  {backPts[i1].x, backPts[i1].y, backPts[i1].z, normBack.x, normBack.y,
												   normBack.z, r, g, b, a}};
						callback(tri, 3, userData);
					}
					...
					for (size_t i = 0; i < contour.Size(); ++i) {
						...
						FontVtx3D_NoTex quad[6] = {{a0.x, a0.y, a0.z, sideNorm.x, sideNorm.y, sideNorm.z, r, g, b, a},
		...
		void EndFrame(NkICommandBuffer *cmd, const NkMat4f &view, const NkMat4f &proj, float sdfSmoothing = 0.1f) {
			if (!mEnabled || !cmd)
				return;
			RefreshDescriptors();
		...
		NkVector<FontVtx3D_NoTex> &Get3DNoTexVertices() {
			return mVerts3DNoTex;
		}

		void Clear3DNoTexVertices() {
			mVerts3DNoTex.Clear();
		}
	...
	private:
		...
		void DestroyGPU() {
			...
			auto d = mDevice;
			if (mDs2D.IsValid())
				d->FreeDescriptorSet(mDs2D);
			if (mDs3D.IsValid())
				d->FreeDescriptorSet(mDs3D);
			if (mDsl2D.IsValid())
				d->DestroyDescriptorSetLayout(mDsl2D);
			if (mDsl3D.IsValid())
				d->DestroyDescriptorSetLayout(mDsl3D);
			if (mPipe2D.IsValid())
				d->DestroyPipeline(mPipe2D);
			if (mPipe3D.IsValid())
				d->DestroyPipeline(mPipe3D);
			if (mShd2D.IsValid())
				d->DestroyShader(mShd2D);
			if (mShd3D.IsValid())
				d->DestroyShader(mShd3D);
			if (mVbo2D.IsValid())
				d->DestroyBuffer(mVbo2D);
			if (mVbo3D.IsValid())
				d->DestroyBuffer(mVbo3D);
			if (mUbo3D.IsValid())
				d->DestroyBuffer(mUbo3D);
			if (mSampler.IsValid())
				d->DestroySampler(mSampler);
			if (mAtlasTex.IsValid())
				d->DestroyTexture(mAtlasTex);
			if (mDs3DNoTex.IsValid())
				d->FreeDescriptorSet(mDs3DNoTex);
			if (mDsl3DNoTex.IsValid())
				d->DestroyDescriptorSetLayout(mDsl3DNoTex);
			if (mPipe3DNoTex.IsValid())
				d->DestroyPipeline(mPipe3DNoTex);
			if (mShd3DNoTex.IsValid())
				d->DestroyShader(mShd3DNoTex);
			if (mVbo3DNoTex.IsValid())
				d->DestroyBuffer(mVbo3DNoTex);
			if (mUbo3DNoTex.IsValid())
				d->DestroyBuffer(mUbo3DNoTex);
			mEnabled = false;
		}
...
int nkmain(const NkEntryState &state) {
	...
	while (running) {
		...
		// Texte 3D extrudé avec contours réels
		{
			...
			NkMat4f w = NkMat4f::RotationY(NkAngle(rot)) * NkMat4f::Translation(NkVec3f(0, y, 0)) *
						NkMat4f::Scaling(NkVec3f(1.f, -1.5f, 1.f));
			fr->GenerateText3DMesh(fr->font3D, "NKENTSEU", w, 1.0f, 0.5f, 0.2f, 1.0f, 0.2f, vertexCallback, fr);
		}
		{
			NkMat4f w = NkMat4f::RotationX(NkAngle(0)) * NkMat4f::Translation(NkVec3f(0, -0.5f, 1.2f)) *
						NkMat4f::Scaling(NkVec3f(1.f, -1.5f, 1.f));
		...
		{
			NkString s = NkFormat("FPS:{0:.0f} | {1} | {2} | {3:.0f}px | +/- taille | S=SDF", fpsDisp, kindStr, modeStr,
								  targetSize);
			fr->DrawText2D(fr->fontRef, s.CStr(), 12, 12, (float)W, (float)H, 1, 1, 0.3f, 1, textScale);
		}
		fr->DrawText2D(fr->fontRef, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", 12, 12 + lineH, (float)W, (float)H, 1, 1, 1, 1,
					   textScale);
		fr->DrawText2D(fr->fontRef, "abcdefghijklmnopqrstuvwxyz 0123456789", 12, 12 + lineH * 2, (float)W, (float)H,
					   0.9f, 0.9f, 0.9f, 1, textScale);
		fr->DrawText2D(fr->fontRef, "Bonjour le monde! Hello World! @#$%", 12, 12 + lineH * 3, (float)W, (float)H, 1,
					   0.5f, 0.5f, 1, textScale);
		fr->DrawText2D(fr->fontRef, "Fix: advanceX/oversampleH + rasteŕiseur correct", 12, 12 + lineH * 4, (float)W,
					   (float)H, 0.6f, 0.9f, 1, 1, textScale);
		fr->DrawText2D(fr->fontRef, "WOFF + OTF/CFF Type2 + SDF mode", 12, 12 + lineH * 5, (float)W, (float)H, 0.7f, 1,
					   0.7f, 1, textScale);
		fr->DrawText2D(fr->font2D, "ESC:quit  +/-:size  S:SDF mode", 12, (float)H - 26, (float)W, (float)H, 0.5f, 0.5f,
					   0.5f, 0.8f, 1);

...
// â”€â”€ DÃ©tection plateforme
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#include "NKPlatform/NkPlatformDetect.h"
#include "NKWindow/NKMain.h"

// â”€â”€ Headers NKEngine
...
// â”€â”€ SPIR-V prÃ©compilÃ© pour Vulkan
...
// â”€â”€ Passe principale Phong + shadow mapping PCF
...
static bool HasArg(const nkentseu::NkVector<nkentseu::NkString> &args, const char *longName,
				   const char *shortName = nullptr) {
	for (size_t i = 1; i < args.Size(); ++i) {
		if (args[i] == longName)
			return true;
		if (shortName && args[i] == shortName)
			return true;
	}
	return false;
}
...
static bool IsSupportedImageExtension(const std::string &extLower) {
	return extLower == ".png" || extLower == ".jpg" || extLower == ".jpeg" || extLower == ".bmp" ||
		   extLower == ".tga" || extLower == ".hdr" || extLower == ".ppm" || extLower == ".pgm" || extLower == ".pbm" ||
		   extLower == ".qoi" || extLower == ".gif" || extLower == ".ico" || extLower == ".webp" || extLower == ".svg";
}
...
static int TextureCandidateScore(const std::filesystem::path &p) {
	...
	if (fileLower.find("checker") != std::string::npos)
		score += 200;
	if (fileLower.find("albedo") != std::string::npos)
		score += 850;
	if (fileLower.find("diffuse") != std::string::npos)
		score += 900;
	if (fileLower.find("basecolor") != std::string::npos)
		score += 820;
	if (fileLower.find("color") != std::string::npos)
		score += 300;
	if (fileLower.find("brickwall") != std::string::npos)
		score += 300;
	if (fileLower.find("brick") != std::string::npos)
		score += 120;

	if (fileLower.find("ao") != std::string::npos || fileLower.find("ambient") != std::string::npos)
		score -= 800;
	if (fileLower.find("normal") != std::string::npos)
		score -= 900;
	if (fileLower.find("rough") != std::string::npos)
		score -= 900;
	if (fileLower.find("metal") != std::string::npos)
		score -= 900;
	if (fileLower.find("spec") != std::string::npos)
		score -= 700;
	if (fileLower.find("height") != std::string::npos || fileLower.find("disp") != std::string::npos ||
		fileLower.find("displace") != std::string::npos)
		score -= 700;
	if (fileLower.find("opacity") != std::string::npos || fileLower.find("alpha") != std::string::npos ||
		fileLower.find("mask") != std::string::npos)
		score -= 300;
...
static bool SaveImageVariants(const NkImage &image, const std::filesystem::path &outputDir, const std::string &stem) {
	namespace fs = std::filesystem;
	std::error_code ec;
	...
	const bool okPng = image.SavePNG((base + ".png").c_str());
	anyOk = anyOk || okPng;
	const bool okJpg = image.SaveJPEG((base + ".jpg").c_str(), 92);
	anyOk = anyOk || okJpg;
	const bool okBmp = image.SaveBMP((base + ".bmp").c_str());
	anyOk = anyOk || okBmp;
	const bool okTga = image.SaveTGA((base + ".tga").c_str());
	anyOk = anyOk || okTga;
	const bool okPpm = image.SavePPM((base + ".ppm").c_str());
	anyOk = anyOk || okPpm;
	const bool okQoi = image.SaveQOI((base + ".qoi").c_str());
	anyOk = anyOk || okQoi;
	const bool okHdr = image.SaveHDR((base + ".hdr").c_str());
	anyOk = anyOk || okHdr;
	logger.Info("[RHIFullDemoImage] Save {0} => png={1} jpg={2} bmp={3} tga={4} ppm={5} qoi={6} hdr={7}\\n",
				stem.c_str(), okPng ? 1 : 0, okJpg ? 1 : 0, okBmp ? 1 : 0, okTga ? 1 : 0, okPpm ? 1 : 0, okQoi ? 1 : 0,
				okHdr ? 1 : 0);
	return anyOk;
}

static bool SaveSoftwareScene(NkSoftwareDevice *swDev, const std::filesystem::path &outputDir,
...
static void AddSpirvOrFallback(NkShaderDesc &sd, NkShaderStage stage, const char *glsl, const char *dbgName,
							   const uint32 *fallbackSpv, uint64 fallbackBytes) {
	if (NkShaderConverter::CanGlslToSpirv()) {
		NkSLStage sl = (stage == NkShaderStage::NK_VERTEX) ? NkSLStage::NK_VERTEX : NkSLStage::NK_FRAGMENT;
		...
		if (res.success) {
			...
			logger.Info("[ShaderConvert] {0} — SPIRV runtime OK ({1} mots)\n", dbgName,
						(unsigned long long)res.SpirvWordCount());
			return;
		}
...
static NkShaderDesc MakeShaderDesc(NkGraphicsApi api) {
	...
	switch (api) {
		...
		case NkGraphicsApi::NK_GFX_API_VULKAN:
			AddSpirvOrFallback(sd, NkShaderStage::NK_VERTEX, kGLSL_Vert, "Phong3DText.vert", kVkRHIFullDemoImageVertSpv,
...
static NkShaderDesc MakeShadowShaderDesc(NkGraphicsApi api) {
	...
	switch (api) {
		...
		case NkGraphicsApi::NK_GFX_API_METAL:
			...
			break;
		case NkGraphicsApi::NK_GFX_API_VULKAN:
			sd.AddSPIRV(NkShaderStage::NK_VERTEX, kVkShadowVertSpv, (uint64)kVkShadowVertSpvWordCount * sizeof(uint32));
			sd.AddSPIRV(NkShaderStage::NK_FRAGMENT, kVkShadowFragSpv,
...
static std::string FindFontFile() {
	...
	const std::vector<std::string> preferred = {"Roboto-Regular.ttf",  "Roboto-Light.ttf",	"OpenSans-Light.ttf",
...
static void Ortho2DMVP(float x, float y, float scaleX, float scaleY, float W, float H, bool depthZeroToOne,
	...
	float ortho[16] = {2.f * rl,
					   ...
					   1.f};
	float model[16] = {scaleX, 0.f, 0.f, 0.f, 0.f, scaleY, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f, x, y, 0.f, 1.f};
...
static NkShaderDesc MakeTextShaderDesc(NkGraphicsApi api) {
	...
	switch (api) {
		...
		case NkGraphicsApi::NK_GFX_API_METAL:
			...
			break;
		case NkGraphicsApi::NK_GFX_API_VULKAN:
			sd.AddSPIRV(NkShaderStage::NK_VERTEX, kVkTextVertSpv, (uint64)kVkTextVertSpvWordCount * sizeof(uint32));
			sd.AddSPIRV(NkShaderStage::NK_FRAGMENT, kVkTextFragSpv, (uint64)kVkTextFragSpvWordCount * sizeof(uint32));
			break;
		default:
...
int nkmain(const nkentseu::NkEntryState &state) {
	...
	// â”€â”€ Surface native + init info device
	...
	logger.Info("[RHIFullDemo] Init step: geometry generated cube={0} sphere={1} plane={2}\n",
				(unsigned long long)cubeVerts.Size(), (unsigned long long)sphereVerts.Size(),
	...
	logger.Info("[RHIFullDemo] Init step: VBO created cube={0} sphere={1} plane={2}\n", hCube.IsValid() ? 1 : 0,
				hSphere.IsValid() ? 1 : 0, hPlane.IsValid() ? 1 : 0);
	...
	logger.Info("[RHIFullDemo] Init step: UBO created {0}/{1}/{2}\n", hUBO[0].IsValid() ? 1 : 0,
				hUBO[1].IsValid() ? 1 : 0, hUBO[2].IsValid() ? 1 : 0);
	...
	if (hasShadowMap) {
		logger.Info("[RHIFullDemo] Shadow map activee ({0}x{0}) mode={1}\n", kShadowSize,
					useRealShadowPass ? "real" : "safe-clear");
	} else {
	...
	std::vector<uint8_t> fontBytes;
	{
		...
		if (!fontPath.empty()) {
			...
			logger.Info("[RHIFullText] Police: {0} ({1} octets)", fontPath.c_str(),
						(unsigned long long)fontBytes.size());
		}
		if (!fontBytes.empty()) {
			ftLib.Init();
	...
	textVtxLayout.AddAttribute(0, 0, NkGPUFormat::NK_RG32_FLOAT, 0, "POSITION", 0)
	...
	// â”€â”€ Constantes de convention NDC selon l'API
	// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ depthZeroToOne :
	// Vulkan/DX/Metal clip Z âˆˆ [0,1], OpenGL/SW clip Z âˆˆ [-1,1]
	...
	logger.Info("[RHIFullDemoImage] Boucle principale. ESC=quitter, WASD=camera, Fleches=lumiere, F5=save scene, "
				"F6=save source\n");
	...
	while (running) {
		...
		// â”€â”€ Matrices de transformation
		...
		NkMat4f matLightProj = NkMat4f::Orthogonal(NkVec2f(-5.f, -5.f), NkVec2f(5.f, 5.f), 1.f, 20.f, depthZeroToOne);
		...
		if (requestSaveScene) {
			...
			if (targetApi == NkGraphicsApi::NK_GFX_API_SOFTWARE) {
				...
				logger.Info("[RHIFullDemoImage] Save scene GPU non implemente pour {0} (offscreen + readback a "
							"brancher dans la demo).\n",

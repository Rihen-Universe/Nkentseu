...
static bool LoadGL(NkIGraphicsContext *ctx) {
	...
	if (!loader) {
		printf("[RHIDemo] OpenGL loader introuvable\n");
		return false;
	}
...
#endif
	if (!ver) {
		printf("[RHIDemo] gladLoadGL échoué\n");
		return false;
	}
...
static NkVector<Vertex3D> MakeCube(float r, float g, float b) {
	...
	Face faces[6] = {{{0, 0, 1}, {{-.5f, -.5f, .5f}, {.5f, -.5f, .5f}, {.5f, .5f, .5f}, {-.5f, .5f, .5f}}},
...
struct ShadowMap {
		...
		bool Init() {
			...
			glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT24, kShadowRes, kShadowRes, 0, GL_DEPTH_COMPONENT,
						 GL_FLOAT, nullptr);
...
int nkmain(const nkentseu::NkEntryState &) {
	...
	if (!window.IsOpen()) {
		printf("[RHIDemo] Fenêtre échouée\n");
		return -1;
	}

	auto ctx = NkContextFactory::Create(window, desc);
	if (!ctx) {
		printf("[RHIDemo] Contexte GL échoué\n");
		window.Close();
		return -2;
	}
	if (!LoadGL(ctx)) {
	...
	Mesh2D tri2d, quad2d;
	{
		using V = Mesh2D::V2D;
		NkVector<V> tv;
		tv.PushBack({20, 60, 1, 0.8f, 0});
		tv.PushBack({80, 60, 0, 1, 0.2f});
		tv.PushBack({50, 110, 0.2f, 0.4f, 1});
		tri2d.Upload(tv);

		NkVector<V> qv;
		qv.PushBack({1180, 60, 1, 1, 0});
		qv.PushBack({1260, 60, 1, 0, 1});
		qv.PushBack({1260, 130, 0, 1, 1});
		qv.PushBack({1180, 60, 1, 1, 0});
		qv.PushBack({1260, 130, 0, 1, 1});
		qv.PushBack({1180, 130, 0.5f, 0, 0.5f});
		quad2d.Upload(qv);
	}
	...
	ShadowMap shadowMap;
	if (!shadowMap.Init())
		printf("[RHIDemo] Shadow FBO incomplet (continuons quand même)\n");
	...
	auto LightDir = [&]() -> NkVec3f {
		return NkVec3f(NkCos(lightPitch) * NkSin(lightYaw), NkSin(lightPitch), NkCos(lightPitch) * NkCos(lightYaw))
			.Normalized();
	};
	...
	while (running && window.IsOpen()) {
		...
		// Caméra orbitale
		NkVec3f camEye(camTarget.x + camDist * NkCos(camPitch) * NkSin(camYaw), camTarget.y + camDist * NkSin(camPitch),
		...
		// Modèles 3D
		NkMat4f mCube = NkMat4f::Translation(NkVec3f(0.f, 0.5f, 0.f)) * NkMat4f::RotationY(NkAngle(time * 0.8f)) *

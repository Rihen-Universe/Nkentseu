...
static const VtxBg kBgVerts[] = {
	{-1, -1, 0.04f, 0.04f, 0.12f}, {1, -1, 0.04f, 0.04f, 0.12f}, {1, 1, 0.08f, 0.08f, 0.20f},
	{-1, -1, 0.04f, 0.04f, 0.12f}, {1, 1, 0.08f, 0.08f, 0.20f},	 {-1, 1, 0.08f, 0.08f, 0.20f},
};
...
static void MulMV4(const float m[16], float x, float y, float z, float w, float &ox, float &oy, float &oz, float &ow) {
...
static NkGraphicsApi ParseBackend(const NkVector<NkString> &args) {
	for (usize i = 1; i < args.Size(); i++) {
		...
		if (::strncmp(s, "--backend=vulkan", 16) == 0 || ::strncmp(s, "-bvk", 4) == 0)
			return NkGraphicsApi::NK_GFX_API_VULKAN;
		if (::strncmp(s, "--backend=dx11", 14) == 0 || ::strncmp(s, "-bdx11", 6) == 0)
			return NkGraphicsApi::NK_GFX_API_DX11;
		if (::strncmp(s, "--backend=dx12", 14) == 0 || ::strncmp(s, "-bdx12", 6) == 0)
			return NkGraphicsApi::NK_GFX_API_DX12;
		if (::strncmp(s, "--backend=metal", 15) == 0 || ::strncmp(s, "-bmtl", 5) == 0)
			return NkGraphicsApi::NK_GFX_API_METAL;
		if (::strncmp(s, "--backend=sw", 12) == 0 || ::strncmp(s, "-bsw", 4) == 0)
			return NkGraphicsApi::NK_GFX_API_SOFTWARE;
		if (::strncmp(s, "--backend=opengl", 16) == 0 || ::strncmp(s, "-bgl", 4) == 0)
...
static const char *sFontPaths[] = {
	...
	"Resources/Fonts/Roboto/Roboto-Regular.ttf", "Resources/Fonts/opensans/OpenSans-Regular.ttf",
	"Resources/Fonts/Antonio-Regular.ttf", "Resources/Fonts/OpenSans-Light.ttf",
	// ── Polices système Windows ─────────────────────────────────────────────
	"C:/Windows/Fonts/arial.ttf", "C:/Windows/Fonts/Arial.ttf", "C:/Windows/Fonts/consola.ttf",
	...
	"/Library/Fonts/Arial.ttf", "/System/Library/Fonts/Supplemental/Arial.ttf", nullptr};
...
static uint32 BuildWord(NkFontFace *face, const char *text, float pen_x, float pen_y, float cr, float cg, float cb,
...
static NkVertexSoftwareShader MakeBgVertSW() {
	return [](const void *vdata, uint32 idx, const void *) -> NkVertexSoftware {
		...
		out.position = {v->x, v->y, 0, 1};
		out.color = {v->r, v->g, v->b, 1};
		return out;
	};
}

static NkSWPixelShader MakeBgFragSW() {
	return [](const NkVertexSoftware &v, const void *, const void *) -> NkSWColor { return v.color; };
}
...
static NkSWPixelShader MakeTextFragSW() {
	return [](const NkVertexSoftware &v, const void *udata, const void *) -> NkSWColor {
		const TextUbo *ub = static_cast<const TextUbo *>(udata);
		if (!ub || !ub->swAtlas)
			return {v.color.r, v.color.g, v.color.b, 1.f};
...
static NkShaderHandle MakeBgShader(NkIDevice *dev, NkGraphicsApi api) {
	...
	if (api == NkGraphicsApi::NK_GFX_API_DX11 || api == NkGraphicsApi::NK_GFX_API_DX12) {
		...
		vs = MakeBgVertSW();
		fs = MakeBgFragSW();
		NkShaderStageDesc svs;
		svs.stage = NkShaderStage::NK_VERTEX;
		svs.cpuVertFn = &vs;
		sd.stages.PushBack(svs);
		NkShaderStageDesc sfs;
		sfs.stage = NkShaderStage::NK_FRAGMENT;
		sfs.cpuFragFn = &fs;
		sd.stages.PushBack(sfs);
	} else {
...
int nkmain(const NkEntryState &state) {
	...
	if (!faceRihen || !faceNkentseu || !faceNoge) {
		logger_src.Errorf("[FontDemo] Echec chargement NkFontFace (r=%p n=%p u=%p)\n", (void *)faceRihen,
						  (void *)faceNkentseu, (void *)faceNoge);
		::free(fontData);
		lib.Destroy();
		return 1;
	}
	...
	gTextVertCount = 0;
	uint32 n;
	n = BuildWord(faceRihen, "RIHEN", 60.f, 200.f, 1.00f, 0.75f, 0.15f, gTextVerts + gTextVertCount,
				  kMaxTextVerts - gTextVertCount);
	gTextVertCount += n;
	n = BuildWord(faceNkentseu, "NKENTSEU", 60.f, 310.f, 0.20f, 0.80f, 1.00f, gTextVerts + gTextVertCount,
				  kMaxTextVerts - gTextVertCount);
	gTextVertCount += n;
	n = BuildWord(faceNoge, "Noge", 60.f, 420.f, 0.75f, 1.00f, 0.20f, gTextVerts + gTextVertCount,
				  kMaxTextVerts - gTextVertCount);
	gTextVertCount += n;
	...
	NkTextureDesc atlasTd = NkTextureDesc::Tex2D((uint32)atlasW, (uint32)atlasH, NkGPUFormat::NK_RGBA8_UNORM);
	...
	if (gTextVertCount > 0)
		hVBText = device->CreateBuffer(NkBufferDesc::Vertex((uint64)gTextVertCount * sizeof(VtxText), gTextVerts));
	...
	auto makePipe = [&](NkShaderHandle sh, NkVertexLayout vl, bool blend, NkDescSetHandle dsl) -> NkPipelineHandle {
		...
		if (dsl.IsValid())
			pd.descriptorSetLayouts.PushBack(dsl);
	...
	if (isSW) {
		...
		if (hAtlasTex.IsValid())
			swAtlas = swDev->GetTex(hAtlasTex.id);
	}
	...
	while (running) {
		...
		cmd->Begin();
		{
			...
			uint64 off = 0;
			cmd->BindVertexBuffer(0, hVBBg, off);
			cmd->Draw(6, 1, 0, 0);
	...
	if (hVBText.IsValid())
		device->DestroyBuffer(hVBText);
	device->DestroyBuffer(hUBOText);
	device->DestroyShader(hShBg);
	device->DestroyShader(hShText);
	device->DestroyPipeline(hPipeBg);
	device->DestroyPipeline(hPipeText);
	device->DestroyTexture(hAtlasTex);
	if (hAtlasSampler.IsValid())
		device->DestroySampler(hAtlasSampler);

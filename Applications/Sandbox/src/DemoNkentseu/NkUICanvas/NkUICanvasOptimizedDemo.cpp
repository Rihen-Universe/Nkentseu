...
static void UploadGray8(uint32 texId, const uint8 *data, int32 w, int32 h) {
	if (gBackend)
		gBackend->UploadTextureGray8(texId, data, w, h);
}
...
static HCURSOR UiCursorToWin32(nkui::NkUIMouseCursor cursor) {
	switch (cursor) {
		case nkui::NkUIMouseCursor::NK_TEXT_INPUT:
			return LoadCursorW(nullptr, IDC_IBEAM);
		case nkui::NkUIMouseCursor::NK_HAND:
			return LoadCursorW(nullptr, IDC_HAND);
		case nkui::NkUIMouseCursor::NK_RESIZE_NS:
			return LoadCursorW(nullptr, IDC_SIZENS);
		case nkui::NkUIMouseCursor::NK_RESIZE_WE:
			return LoadCursorW(nullptr, IDC_SIZEWE);
		case nkui::NkUIMouseCursor::NK_RESIZE_NWSE:
			return LoadCursorW(nullptr, IDC_SIZENWSE);
		case nkui::NkUIMouseCursor::NK_RESIZE_NESW:
...
static void ScatterBalls(Ball *balls, float32 W, float32 H) {
	static const NkColor2D palette[NB_BALLS] = {{255, 130, 70, 255}, {90, 200, 255, 255},  {130, 230, 140, 255},
												{255, 210, 90, 255}, {220, 120, 255, 255}, {255, 100, 140, 255},
												{120, 180, 255, 255}};
...
static void RenderDemoFrame(DemoFrame &f) {
	...
	if (sz.x != f.lastW || sz.y != f.lastH) {
		if (f.lastW != 0 && sz.x > 0 && sz.y > 0)
			f.target->OnResize(sz.x, sz.y);
		f.lastW = sz.x;
		f.lastH = sz.y;
...
int nkmain(const NkEntryState &state) {
	...
	if (!window.Create(cfg)) {
		logger.Error("[nkuiopt] window failed");
		return -1;
	}
	...
	if (!target.IsValid()) {
		logger.Error("[nkuiopt] NkRenderWindow init FAILED");
		window.Close();
		return -2;
	}
	...
	if (!ctx.Init(static_cast<int32>(cfg.width), static_cast<int32>(cfg.height), fontConfig)) {
		logger.Error("[nkuiopt] NkUIContext::Init failed");
		window.Close();
		return -3;
	}
	...
	if (uiFontId < 0)
		uiFontId = ctx.fontManager.LoadEmbedded(NkEmbeddedFontId::ProggyClean, 16.f);
	if (uiFontId < 0)
		uiFontId = ctx.fontManager.LoadFromFile("C:/Windows/Fonts/segoeui.ttf", 18.f);
	if (uiFontId < 0) {
		logger.Warn("[nkuiopt] aucune police chargee");
		uiFontId = 0;
	}
	...
	DemoFrame fdata{&window, &target, &ctx, &ui, &wm, &ls, font, &input, &clock, balls, 180.f, 0, 0, 0};

...
int nkmain(const NkEntryState &state) {
	...
	if (!window.Create(cfg)) {
		logger.Error("[nkuicanvas] window failed");
		return -1;
	}
	...
	if (uiFontId < 0) {
		logger.Warn("[nkuicanvas] aucune police chargee -> texte indisponible");
		uiFontId = 0;
	}

	ctx.fontManager.UploadDirtyAtlases(reinterpret_cast<void *>(&UploadGray8));
	logger.Infof("[nkuicanvas] fonts: %d, atlas: %d, fontId=%d", ctx.fontManager.numFonts, ctx.fontManager.numAtlases,
				 uiFontId);

...
namespace nkentseu {
	...
	// =============================================================================
	static NkWindowConfig MakeWindowConfig(const char *title, uint32 w = 1280, uint32 h = 720) {
		NkWindowConfig cfg;
		cfg.title = title;
		cfg.width = w;
		cfg.height = h;
	...
	int DemoOpenGL(const NkEntryState & /*state*/) {
		...
		NkWindow window(cfg);
		if (!window.IsOpen()) {
			printf("[DemoGL] Window failed\n");
			return -1;
		}

		auto ctx = NkContextFactory::Create(desc, window);
		if (!ctx) {
			printf("[DemoGL] Context failed\n");
			return -2;
		}
...
//
	...
	int DemoDirectX11(const NkEntryState & /*state*/) {
		...
		D3DCompile(hlslSrc, strlen(hlslSrc), nullptr, nullptr, nullptr, "VS", "vs_5_0", 0, 0, &vsBlob, &errBlob);
		D3DCompile(hlslSrc, strlen(hlslSrc), nullptr, nullptr, nullptr, "PS", "ps_5_0", 0, 0, &psBlob, &errBlob);
...
//
	...
	int DemoSoftware(const NkEntryState & /*state*/) {
		NkWindowConfig cfg = MakeWindowConfig("NkEngine — Software Renderer Demo", 800, 600);
	...
	int DemoAutoAPI(const NkEntryState &state) {
		...
		RunLoop(window, ctx.get(), [&](NkIGraphicsContext *activeCtx, float dt) {
			...
			if (api == NkGraphicsApi::NK_GFX_API_OPENGL || api == NkGraphicsApi::NK_GFX_API_OPENGLES ||
...
int nkmain(const nkentseu::NkEntryState &state) {
	...
	if (strcmp(demo, "opengl") == 0)
		return DemoOpenGL(state);
	if (strcmp(demo, "opengles") == 0)
		return DemoOpenGL(state);
	if (strcmp(demo, "webgl") == 0)
		return DemoOpenGL(state);
	if (strcmp(demo, "vulkan") == 0)
		return DemoVulkan(state);
	if (strcmp(demo, "software") == 0)
		return DemoSoftware(state);
	if (strcmp(demo, "auto") == 0)
		return DemoAutoAPI(state);

#if defined(NKENTSEU_PLATFORM_WINDOWS)
	if (strcmp(demo, "dx11") == 0)
		return DemoDirectX11(state);
	if (strcmp(demo, "dx12") == 0)
		return DemoDirectX12(state);
#endif

#if defined(NKENTSEU_PLATFORM_MACOS) || defined(NKENTSEU_PLATFORM_IOS)
	if (strcmp(demo, "metal") == 0)
		return DemoMetal(state);
#endif

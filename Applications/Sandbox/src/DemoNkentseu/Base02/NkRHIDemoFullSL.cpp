...
static NkVector<Vtx3D> MakeCube(float r = 1.f, float g = 0.45f, float b = 0.2f) {
	...
	static const Face faces[6] = {{{{P, N, P}, {P, P, P}, {N, P, P}, {N, N, P}}, 0, 0, 1},
...
static NkShaderDesc MakeShadowShaderDesc(NkGraphicsApi api) {
	...
	switch (api) {
		...
		case NkGraphicsApi::NK_GFX_API_DX12:
			...
			break;
		default:
			break; // Vulkan/Metal : shadow non implémentée
	}
	return sd;
}
...
int nkmain(const nkentseu::NkEntryState &state) {
	...
	if (!window.Create(winCfg)) {
		logger.Info("[RHIFullDemo] Fenêtre échouée\n");
		return 1;
	}
...

	{
		...
		if (!sd.stages.IsEmpty())
			hShadowShader = device->CreateShader(sd);
	}
	...
	if (hasShadow) {
		...
		sd.minLod = sd.maxLod = 0.f;
		hShadowSampler = device->CreateSampler(sd);

		hShadowRP = device->CreateRenderPass(NkRenderPassDesc::ShadowMap());

		NkFramebufferDesc fd{};
		fd.renderPass = hShadowRP;
		fd.depthAttachment = hShadowTex;
		fd.width = kShadowSize;
		fd.height = kShadowSize;
		hShadowFBO = device->CreateFramebuffer(fd);

		NkVertexLayout svl;
		svl.AddAttribute(0, 0, NkGpuFormat::NK_RGB32_FLOAT, 0, "POSITION", 0).AddBinding(0, (uint32)sizeof(Vtx3D));
	...
	while (running) {
		...
		NkMat4f matModels[3] = {NkMat4f::RotationY(NkAngle(rotAngle)) * NkMat4f::RotationX(NkAngle(rotAngle * .5f)),
	...
	for (uint32 f = 0; f < kFramesInFlight; f++) {
		if (cmds[f])
			device->DestroyCommandBuffer(cmds[f]);
		if (hFences[f].IsValid())
			device->DestroyFence(hFences[f]);
		for (uint32 i = 0; i < kObjCount; i++) {
			if (hDescSet[i][f].IsValid())
				device->FreeDescriptorSet(hDescSet[i][f]);
			if (hUBO[i][f].IsValid())
				device->DestroyBuffer(hUBO[i][f]);
		}
	}
	if (hLayout.IsValid())
		device->DestroyDescriptorSetLayout(hLayout);

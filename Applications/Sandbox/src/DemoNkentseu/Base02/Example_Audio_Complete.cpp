...
int nk::nk_main(int argc, char **argv) {
	...
	audioEngine.setListenerOrientation(0.0f, 0.0f, -1.0f, // Forward (regardant vers -Z)
									   0.0f, 1.0f, 0.0f	  // Up
	);
	...
	eventSystem.setEventCallback<nk::WindowCloseEvent>([&](auto *e) { running = false; });

	eventSystem.setEventCallback<nk::KeyPressedEvent>([&](auto *e) {
		if (e->getKey() == nk::Key::Escape)
			running = false;
	});
		...
		if (!spectrogram.empty()) {
			...
			for (size_t t = 0; t < spectrogram.size(); t++) {
				for (size_t f = 0; f < spectrogram[t].size(); f++) {
					...
					renderer.fillRect(x, y, std::max(1, static_cast<int>(xStep)), std::max(1, static_cast<int>(yStep)),
									  color);
				}
			}
		}

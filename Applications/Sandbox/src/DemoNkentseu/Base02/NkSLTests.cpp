...
#define NK_EXPECT_NO_ERRORS(res, msg)                                                                                  \
	do {                                                                                                               \
...
static void TestLexer() {
	...
	// Littéraux
	{
		...
		auto t1 = lex.Next();
		NK_EXPECT_EQ((int)t1.kind, (int)NkSLTokenKind::LIT_INT, "int literal");
		auto t2 = lex.Next();
		NK_EXPECT_EQ((int)t2.kind, (int)NkSLTokenKind::LIT_UINT, "uint literal");
		auto t3 = lex.Next();
		NK_EXPECT_EQ((int)t3.kind, (int)NkSLTokenKind::LIT_FLOAT, "float literal");
		auto t4 = lex.Next();
		NK_EXPECT_EQ((int)t4.kind, (int)NkSLTokenKind::LIT_FLOAT, "float f suffix");
		auto t5 = lex.Next();
		NK_EXPECT_EQ((int)t5.kind, (int)NkSLTokenKind::LIT_INT, "hex literal");
		NK_EXPECT_EQ(t5.intVal, 255LL, "hex value");
		auto t6 = lex.Next();
		NK_EXPECT_EQ((int)t6.kind, (int)NkSLTokenKind::LIT_BOOL, "true literal");
		NK_EXPECT_TRUE(t6.boolVal, "true value");
		auto t7 = lex.Next();
		NK_EXPECT_EQ((int)t7.kind, (int)NkSLTokenKind::LIT_BOOL, "false literal");
...
static void TestStageInterface() {
	...
	// Location mismatch
	{
		...
		for (auto &e : errs)
			if (e.message.Contains("Location") || e.message.Contains("location"))
				hasLocError = true;

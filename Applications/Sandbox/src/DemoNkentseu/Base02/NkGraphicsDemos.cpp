...
#define NK_DEMO_GL_APIENTRY
	...
	struct DemoGLFunctions {
			...
			using VertexAttribPointerFn = void(NK_DEMO_GL_APIENTRY *)(GLuint, GLint, GLenum, GLboolean, GLsizei,
																	  const void *);
			...
			bool Ready() const {
				return CreateShader && ShaderSource && CompileShader && GetShaderiv && GetShaderInfoLog &&
					   CreateProgram && AttachShader && LinkProgram && GetProgramiv && GetProgramInfoLog &&
					   DeleteShader && DeleteProgram && UseProgram && GetUniformLocation && Uniform1f &&
	...
	template <typename T> bool LoadDemoGLProc(T &out, NkOpenGLContextData::ProcAddressFn loader, const char *name) {
	...
	void RunLoop(NkWindow &win, NkIGraphicsContext *ctx, LoopCtx &lc, OnFrameFn &&onFrame, int maxFrames = 0) {
	...
	bool LoadDemoOpenGLFunctions(NkIGraphicsContext *ctx) {
		...
		const bool requiredReady =
			gGL.CreateShader && gGL.ShaderSource && gGL.CompileShader && gGL.GetShaderiv && gGL.GetShaderInfoLog &&
			gGL.CreateProgram && gGL.AttachShader && gGL.LinkProgram && gGL.GetProgramiv && gGL.GetProgramInfoLog &&
			gGL.DeleteShader && gGL.DeleteProgram && gGL.UseProgram && gGL.GetUniformLocation && gGL.Uniform1f &&
			gGL.GenBuffers && gGL.BindBuffer && gGL.BufferData && gGL.DeleteBuffers && gGL.EnableVertexAttribArray &&
			gGL.VertexAttribPointer && gGL.DrawArrays && gGL.DrawElements;
		if (!requiredReady) {
		...
		gGLSupportsVAO = LoadDemoGLProc(gGL.GenVertexArrays, loader, "glGenVertexArrays") &&
...
float4 PSMain(VSOut i) : SV_TARGET {
    ...
    return float4(col, 1.0);
		...
		HRESULT hr =
			D3DCompile(hlsl, strlen(hlsl), nullptr, nullptr, nullptr, "VSMain", "vs_5_0", 0, 0, &vsBlob, &errBlob);
		if (FAILED(hr)) {
...
namespace {
	...
	GLuint LinkGL(GLuint vs, GLuint fs, const char *attrib0 = nullptr, const char *attrib1 = nullptr) {
		...
		if (attrib0 && gGL.BindAttribLocation)
			gGL.BindAttribLocation(p, 0, attrib0);
		if (attrib1 && gGL.BindAttribLocation)
			gGL.BindAttribLocation(p, 1, attrib1);
		gGL.LinkProgram(p);
		GLint ok = 0;
...
static bool DemoOpenGL(const NkEntryState &) {
	...
	if (!ctx) {
		logger.Errorf("[GL] Context creation failed");
		return false;
	}
	...
	const char *vsT = useGLES2Shaders	? R"(#version 100
attribute vec2 aPos;
attribute vec3 aCol;
uniform float uTime;
varying vec3 vCol;
void main(){
    ...
    vCol = aCol * (0.6 + 0.4*sin(uTime*3.0 + phase));
					  ...
					  : useGLES3Shaders ? R"(#version 300 es
...
void main(){
	...
	const char *fsT = useGLES2Shaders	? R"(#version 100
precision mediump float;
varying vec3 vCol;
uniform float uTime;
void main(){
...
void main(){
	...
	// ── Quad UV rotatif ─────────────────────────────────────────────────
	const char *vsQ = useGLES2Shaders	? R"(#version 100
attribute vec2 aPos;
attribute vec2 aUV;
uniform float uTime;
varying vec2 vUV;
...
void main(){
	...
	const char *fsQ = useGLES2Shaders	? R"(#version 100
precision mediump float;
varying vec2 vUV;
uniform float uTime;
void main(){
...
void main(){
	...
	float triV[] = {
		0.0f, 0.6f, 1.f, 0.25f, 0.25f, -0.5f, -0.45f, 0.25f, 1.f, 0.25f, 0.5f, -0.45f, 0.25f, 0.25f, 1.f,
	};
	float quadV[] = {
	...
	GLuint progT = LinkGL(CompileGL(GL_VERTEX_SHADER, vsT), CompileGL(GL_FRAGMENT_SHADER, fsT),
						  useGLES2Shaders ? "aPos" : nullptr, legacyAttr1Tri);
	GLuint progQ = LinkGL(CompileGL(GL_VERTEX_SHADER, vsQ), CompileGL(GL_FRAGMENT_SHADER, fsQ),
						  useGLES2Shaders ? "aPos" : nullptr, legacyAttr1Quad);
	...
	if (!progT || !progQ) {
		...
		if (gGLSupportsVAO && vaoT)
			gGL.DeleteVertexArrays(1, &vaoT);
		if (vboT)
			gGL.DeleteBuffers(1, &vboT);
		if (gGLSupportsVAO && vaoQ)
			gGL.DeleteVertexArrays(1, &vaoQ);
		if (vboQ)
...
static void DemoVulkan(const NkEntryState &) {
	...
	NkIGraphicsContext *ctx = NkContextFactory::Create(win, desc);
	if (!ctx) {
		logger.Errorf("[Vulkan] Context failed");
		return;
	}
	...
	RunLoop(
		...
		[&](float, float t) {
			...
			if (!d || cmd == VK_NULL_HANDLE || vkPipe.pipeline == VK_NULL_HANDLE)
				return;

			VkDemoPush push{t, static_cast<float>(d->swapExtent.width), static_cast<float>(d->swapExtent.height), 0.0f};
			vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, vkPipe.pipeline);
			vkCmdPushConstants(cmd, vkPipe.layout, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0,
							   sizeof(push), &push);
...
static void DemoDirectX11(const NkEntryState &) {
	...
	NkIGraphicsContext *ctx = NkContextFactory::Create(win, desc);
	if (!ctx) {
		logger.Errorf("[DX11] Context failed");
		return;
	}
...
float4 main(float4 pos:SV_POSITION, float4 col:COLOR) : SV_TARGET {
	...
	if (FAILED(hr) || !vsBlob) {
		logger.Errorf("[DX11] VS compile failed: %s", errBlob ? (const char *)errBlob->GetBufferPointer() : "unknown");
		ctx->Shutdown();
		delete ctx;
		return;
	}
	errBlob.Reset();
	hr = D3DCompile(psHLSL, strlen(psHLSL), nullptr, nullptr, nullptr, "main", "ps_5_0", 0, 0, &psBlob, &errBlob);
	if (FAILED(hr) || !psBlob) {
		logger.Errorf("[DX11] PS compile failed: %s", errBlob ? (const char *)errBlob->GetBufferPointer() : "unknown");
		ctx->Shutdown();
		delete ctx;
		return;
	}
	...
	D3D11_BUFFER_DESC cbd{sizeof(CB), D3D11_USAGE_DYNAMIC, D3D11_BIND_CONSTANT_BUFFER, D3D11_CPU_ACCESS_WRITE, 0, 0};
	...
	D3D11_BUFFER_DESC vbd{(UINT)(verts.Size() * sizeof(Vert)), D3D11_USAGE_DEFAULT, D3D11_BIND_VERTEX_BUFFER, 0, 0, 0};
	...
	RunLoop(
		...
		[&](float, float t) {
			...
			D3D12_VIEWPORT viewport = {0.0f, 0.0f, static_cast<float>(d->width), static_cast<float>(d->height),
									   0.0f, 1.0f};
			D3D12_RECT scissorRect = {0, 0, static_cast<LONG>(d->width), static_cast<LONG>(d->height)};

			cmd->RSSetViewports(1, &viewport);
			cmd->RSSetScissorRects(1, &scissorRect);
			logger.Warn("005");

			float push[4] = {t, static_cast<float>(d->width), static_cast<float>(d->height), 0.0f};
			logger.Warn("006");
...
static void DemoMetal(const NkEntryState &) {
	...
	NkIGraphicsContext *ctx = NkContextFactory::Create(win, desc);
	if (!ctx) {
		logger.Errorf("[Metal] Context failed");
		return;
	}
...
fragment float4 circle_frag(CFrag in [[stage_in]],
	...
	NSError *err = nil;
	id<MTLLibrary> lib = [device newLibraryWithSource:[NSString stringWithUTF8String:mslSrc] options:nil error:&err];
	if (!lib) {
		logger.Errorf("[Metal] MSL error: %s", err ? [[err localizedDescription] UTF8String] : "?");
		ctx->Shutdown();
		delete ctx;
		return;
	}
	...
	id<MTLRenderPipelineState> triPSO = [device newRenderPipelineStateWithDescriptor:triPD error:&err];
	...
	id<MTLRenderPipelineState> circlePSO = [device newRenderPipelineStateWithDescriptor:circlePD error:&err];
	...
	id<MTLBuffer> triVB = [device newBufferWithBytes:tverts length:sizeof(tverts) options:MTLResourceStorageModeShared];

	struct Uniforms {
			float time, resX, resY, pad;
	};
	...
	RunLoop(win, ctx, lc, [&](float, float t) {
		...
		if (!rawEnc)
			return;
		id<MTLRenderCommandEncoder> enc = (__bridge id<MTLRenderCommandEncoder>)rawEnc;
	...
	RunLoop(win, ctx, lc, [&](float, float t) {
		...
		for (uint32 y = 0; y < H; ++y) {
			...
			for (uint32 x = 0; x < W; ++x) {
				...
				fb->SetPixel(x, y, (uint8)(pr * bright * 255), (uint8)(pg * bright * 255), (uint8)(pb * bright * 255));
			}
		}
...
static void DemoCompute(const NkEntryState &) {
	...
	if (!gfx) {
		logger.Errorf("[Compute] GL context failed");
		return;
	}

	NkIComputeContext *compute = NkContextFactory::ComputeFromGraphics(gfx);
	if (!compute) {
		logger.Errorf("[Compute] ComputeFromGraphics failed");
		gfx->Shutdown();
		delete gfx;
		return;
	}
...
void main(){
	...
	bd.cpuWritable = true;
	bd.cpuReadable = false;
	bd.sizeBytes = N * 4;
	bd.initialData = A.Data();
	auto bufA = compute->CreateBuffer(bd);
	bd.sizeBytes = N * 4;
	bd.initialData = B.Data();
	auto bufB = compute->CreateBuffer(bd);
	bd.sizeBytes = N * 4;
	bd.initialData = nullptr;
	bd.cpuReadable = true;
	auto bufC = compute->CreateBuffer(bd);
...
static void DemoAutoAPI(const NkEntryState &) {
...
#if defined(NKENTSEU_PLATFORM_WINDOWS)
		NkGraphicsApi::NK_GFX_API_DX12,	  NkGraphicsApi::NK_GFX_API_DX11,
#endif
	...
	NkIGraphicsContext *ctx = NkContextFactory::CreateWithFallback(
		win, fallbackOrder, (uint32)(sizeof(fallbackOrder) / sizeof(fallbackOrder[0])));
	if (!ctx) {
		logger.Errorf("[Auto] Aucune API disponible!");
		return;
	}
...
int nkmain(const NkEntryState &state) {
	...
	auto parseDemoFromArg = [&](const char *arg) -> const char * {
		if (!arg || !*arg)
			return nullptr;
		static const char *kKnown[] = {"opengl", "opengles", "webgl",	 "vulkan",	"dx11",
									   "dx12",	 "metal",	 "software", "compute", "auto"};
		for (const char *k : kKnown) {
			if (equalsIgnoreCase(arg, k))
				return k;
		}
		return nullptr;
	};

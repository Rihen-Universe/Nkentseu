...
static NkTextureHandle CreateSolidTexture(NkRHIDevice &dev, uint8_t r, uint8_t g, uint8_t b, uint8_t a = 255) {
...
int main() {
	...
	if (!device) {
		logger.Error("Vulkan init failed");
		return -1;
	}
	...
	NkMesh cubeMesh = NkMesh::Create(*device, kCubeVertices, 24, kCubeIndices, 36);
	...
	// 10 cubes verts en grille
	{
		...
		for (int row = 0; row < 2; ++row) {
			for (int col = 0; col < 5; ++col) {
				...
				MakeTranslation(obj.transform, (col - 2.f) * 1.5f, -2.5f + row * 1.5f, -3.f);
				obj.color[0] = 0.2f;
				obj.color[1] = 0.8f;
				obj.color[2] = 0.2f;
				obj.color[3] = 1.f;

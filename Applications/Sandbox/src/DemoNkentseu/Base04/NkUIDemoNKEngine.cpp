...
static void AddUtf8ToInput(NkUIInputState &input, const char *utf8) {
	...
	for (int32 i = 0; utf8[i];) {
		...
		if (c0 < 0x80u) {
			...
			cp = ((c0 & 0x1Fu) << 6) | (static_cast<uint8>(utf8[i + 1]) & 0x3Fu);
			n = 2;
		} else if ((c0 & 0xF0u) == 0xE0u && utf8[i + 1] && utf8[i + 2]) {
			cp = ((c0 & 0x0Fu) << 12) | ((static_cast<uint8>(utf8[i + 1]) & 0x3Fu) << 6) |
				 (static_cast<uint8>(utf8[i + 2]) & 0x3Fu);
			n = 3;
		} else if ((c0 & 0xF8u) == 0xF0u && utf8[i + 1] && utf8[i + 2] && utf8[i + 3]) {
			cp = ((c0 & 0x07u) << 18) | ((static_cast<uint8>(utf8[i + 1]) & 0x3Fu) << 12) |
				 ((static_cast<uint8>(utf8[i + 2]) & 0x3Fu) << 6) | (static_cast<uint8>(utf8[i + 3]) & 0x3Fu);
			n = 4;
		}
...
static HCURSOR UiCursorToWin32(NkUIMouseCursor cursor) {
	switch (cursor) {
		case NkUIMouseCursor::NK_TEXT_INPUT:
			return LoadCursorW(nullptr, IDC_IBEAM);
		case NkUIMouseCursor::NK_HAND:
			return LoadCursorW(nullptr, IDC_HAND);
		case NkUIMouseCursor::NK_RESIZE_NS:
			return LoadCursorW(nullptr, IDC_SIZENS);
		case NkUIMouseCursor::NK_RESIZE_WE:
			return LoadCursorW(nullptr, IDC_SIZEWE);
		case NkUIMouseCursor::NK_RESIZE_NWSE:
			return LoadCursorW(nullptr, IDC_SIZENWSE);
		case NkUIMouseCursor::NK_RESIZE_NESW:
...
static void ApplyUiCursor(NkWindow &window, const NkUIContext &ctx) {
	...
	if (!cur || !window.mData.mHwnd)
		return;
	POINT p{};
	if (!GetCursorPos(&p))
		return;
	RECT wr{};
	if (!GetWindowRect(window.mData.mHwnd, &wr))
		return;
...
static bool DemoTextureUploadRGBA8(void * /*device*/, uint32 texId, const uint8 *data, int32 w, int32 h) {
	if (!sBackendForUpload)
		return false;
...
static void LoadFonts(NkUIFontManager &mgr) {
	...
	for (int32 i = 0; kCandidates[i].path && loaded < 4; ++i) {
		const int32 idx = mgr.LoadFromFile(kCandidates[i].path, kCandidates[i].size, kCandidates[i].name);

		if (idx >= 0) {
			logger.Infof("[nkmain] Police chargée: %s (%.0fpx, idx=%d)\n", kCandidates[i].path, kCandidates[i].size,
						 idx);
			++loaded;
...
int nkmain(const NkEntryState &state) {
	...
	wm.Init();
	dock.Init({0.f, ctx.theme.metrics.titleBarHeight, static_cast<float32>(W),
	...
	// qui délègue à NkUINKRHIBackend::UploadTextureGray8() (Gray8 → RGBA8).
	{
		...
		ctx.fontManager.UploadDirtyAtlases(reinterpret_cast<void *>(&FontAtlasUploadGray8));

		logger.Infof("[nkmain] Polices uploadées sur le GPU (%d polices, %d atlas)\n", ctx.fontManager.numFonts,
					 ctx.fontManager.numAtlases);
	}
	...
	demo->SetGPUUploadFunc(reinterpret_cast<void *>(&DemoTextureUploadRGBA8),
	...
	events.AddEventCallback<NkTextInputEvent>([&](NkTextInputEvent *e) { AddUtf8ToInput(input, e->GetUtf8()); });
	...
	while (running) {
		...
		for (int32 i = 0; i < ctx.fontManager.numAtlases; ++i) {
			if (ctx.fontManager.atlases[i].dirty) {
				anyDirty = true;
				break;
			}
		}

	...
	namespace nkui {
		...
		class NkUIDemo {
			public:
				...
				bool Init(NkUIContext &ctx, NkUIWindowManager &wm);

				void Destroy();

				void Render(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDockManager &dock, NkUILayoutStack &ls,
							float32 dt);
			...
			private:
				...
				void WinOverlap(NkUIContext &ctx, NkUIWindowManager &wm, NkUILayoutStack &ls, int32 idx,
								const char *name, NkVec2 pos, NkVec2 size);
				void RenderMainMenuBar(NkUIContext &ctx, NkUIDrawList &dl);
				void WinDockInspector(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDockManager &dock,
									  NkUILayoutStack &ls);
				void WinDockConsole(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDockManager &dock,
									NkUILayoutStack &ls);
		...
		inline void NkUIDemo::UploadTextureIfNeeded() {
			...
			mTextureUploaded =
				(*reinterpret_cast<UploadFn *>(&mGPUUploadFunc))(mGPUDevice, kTexId, mTexturePixels, kTexW, kTexH);
			mTextureDirty = false;
		}

		inline bool NkUIDemo::Init(NkUIContext &ctx, NkUIWindowManager &wm) {
			(void)wm;
		...
		inline void NkUIDemo::WinControl(NkUIContext &ctx, NkUIWindowManager &wm, NkUILayoutStack &ls) {
			...
			if (NkUIWindow::Begin(ctx, wm, dl, *mFont, ls, "UI Test Lab", &mShowControl)) {
				...
				char ids[160];
				::snprintf(ids, sizeof(ids), "hot=%u active=%u focus=%u hoveredWnd=%u", ctx.hotId, ctx.activeId,
						   ctx.focusId, ctx.wm ? ctx.wm->hoveredId : 0u);
		...
		inline void NkUIDemo::WinWidgets(NkUIContext &ctx, NkUIWindowManager &wm, NkUILayoutStack &ls) {
			...
			static const char *listItems[] = {"Alpha",	 "Bravo", "Charlie", "Delta", "Echo",
			...
			if (NkUIWindow::Begin(ctx, wm, dl, *mFont, ls, "Widgets", &mShowWidgets)) {
				...
				NkUI::SliderFloat(ctx, ls, dl, *mFont, "Multiline Spacing", mTextInputOpts.multilineLineSpacing, 1.0f,
								  1.8f, "%.2f");
				NkUI::SliderFloat(ctx, ls, dl, *mFont, "Multiline Char Spacing", mTextInputOpts.multilineCharSpacing,
								  0.0f, 3.0f, "%.2f");

				NkUI::InputText(ctx, ls, dl, *mFont, "Input", mInputText, static_cast<int32>(sizeof(mInputText)), 0.f,
								&mTextInputOpts);
		...
		inline void NkUIDemo::WinInput(NkUIContext &ctx, NkUIWindowManager &wm, NkUILayoutStack &ls) {
			...
			if (NkUIWindow::Begin(ctx, wm, dl, *mFont, ls, "Input Diagnostics", &mShowInput)) {
				...
				NkUI::Text(ctx, ls, dl, *mFont, line);
				::snprintf(line, sizeof(line), "Mouse delta: %.2f %.2f", ctx.input.mouseDelta.x,
						   ctx.input.mouseDelta.y);
				NkUI::Text(ctx, ls, dl, *mFont, line);
				::snprintf(line, sizeof(line), "Wheel V/H: %.2f %.2f", ctx.input.mouseWheel, ctx.input.mouseWheelH);
				NkUI::Text(ctx, ls, dl, *mFont, line);
				::snprintf(line, sizeof(line), "Buttons down: L=%d R=%d M=%d", ctx.input.mouseDown[0] ? 1 : 0,
						   ctx.input.mouseDown[1] ? 1 : 0, ctx.input.mouseDown[2] ? 1 : 0);
				NkUI::Text(ctx, ls, dl, *mFont, line);
				::snprintf(line, sizeof(line), "Clicked: L=%d R=%d M=%d", ctx.input.mouseClicked[0] ? 1 : 0,
						   ctx.input.mouseClicked[1] ? 1 : 0, ctx.input.mouseClicked[2] ? 1 : 0);
				NkUI::Text(ctx, ls, dl, *mFont, line);
		...
		inline void NkUIDemo::WinUnicode(NkUIContext &ctx, NkUIWindowManager &wm, NkUILayoutStack &ls) {
			...
			if (NkUIWindow::Begin(ctx, wm, dl, *mFont, ls, "Unicode + Font", &mShowUnicode)) {
				...
				NkUI::InputText(ctx, ls, dl, *mFont, "Edit UTF-8", mUnicodeInput,
								static_cast<int32>(sizeof(mUnicodeInput)));
		...
		inline void NkUIDemo::WinOverlap(NkUIContext &ctx, NkUIWindowManager &wm, NkUILayoutStack &ls, int32 idx,
										 const char *name, NkVec2 pos, NkVec2 size) {
			NkUIDrawList &dl = ctx.layers[NkUIContext::LAYER_WINDOWS];
			if (mResetLayout) {
		...
		inline void NkUIDemo::RenderMainMenuBar(NkUIContext &ctx, NkUIDrawList &dl) {
			...
			if (!NkUIMenu::BeginMenuBar(ctx, menuDL, *mFont, barR))
				return;
		...
		inline void NkUIDemo::WinDockInspector(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDockManager &dock,
											   NkUILayoutStack &ls) {
			if (!mShowDocking || !mShowDockInspector || !mFont)
				return;
		...
		inline void NkUIDemo::WinDockConsole(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDockManager &dock,
											 NkUILayoutStack &ls) {
			if (!mShowDocking || !mShowDockConsole || !mFont)
				return;
		...
		inline void NkUIDemo::WinDockAssets(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDockManager &dock,
											NkUILayoutStack &ls) {
			if (!mShowDocking || !mShowDockAssets || !mFont)
				return;
			...
			if (ws && ws->isDocked) {
				...
				if (dock.BeginDocked(ctx, wm, dl, "Dock Assets")) {
					static const char *assets[] = {"textures/checker.png", "textures/gradient.png",	 "models/cube.mesh",
												   "models/sphere.mesh",   "scenes/demo_scene.json", "fonts/arial.ttf"};
		...
		inline void NkUIDemo::WinDocking(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDockManager &dock,
										 NkUILayoutStack &ls) {
			if (!mShowDocking || !mFont)
				return;
			(void)ctx;
			(void)ls;
		...
		inline void NkUIDemo::Render(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDockManager &dock,
									 NkUILayoutStack &ls, float32 dt) {
			...
			bg.AddRectFilled({0.f, 0.f, static_cast<float32>(ctx.viewW), static_cast<float32>(ctx.viewH)},
			...
			if (mShowDocking) {
				const NkRect dockRect = {0.f, menuH, static_cast<float32>(ctx.viewW),
										 static_cast<float32>(ctx.viewH) - menuH};
			...
			// Keep them on the windows layer so popup layers (menus/combos) stay visible on top.
			{
				...
				static const char *rootValuePlacementItems[] = {"Above thumb",	   "Below thumb", "Left of slider",
																"Right of slider", "In label",	  "Hidden"};

				NkUISliderValueOptions rootValueOpts{};
				switch (rootValuePlacement) {
					default:
					case 0:
						rootValueOpts.placement = NkUISliderValuePlacement::NK_ABOVE_THUMB;
						break;
					case 1:
						rootValueOpts.placement = NkUISliderValuePlacement::NK_BELOW_THUMB;
						break;
					case 2:
						rootValueOpts.placement = NkUISliderValuePlacement::NK_LEFT_OF_SLIDER;
						break;
					case 3:
						rootValueOpts.placement = NkUISliderValuePlacement::NK_RIGHT_OF_SLIDER;
						break;
					case 4:
						rootValueOpts.placement = NkUISliderValuePlacement::NK_IN_LABEL;
						break;
					case 5:
						rootValueOpts.placement = NkUISliderValuePlacement::NK_HIDDEN;
						break;
				}
				...
				NkUI::SliderFloat(ctx, ls, root, *mFont, "Root Opacity##root_opacity", rootOpacity, 0.f, 1.f, "%.2f",
								  260.f, &rootValueOpts);
				NkUI::EndRow(ctx, ls);

				NkUI::Combo(ctx, ls, root, *mFont, "Root Value Pos##root_value_pos", rootValuePlacement,
							rootValuePlacementItems, 6, 220.f);
			...
			std::sort(tasks, tasks + taskCount, [](const Task &a, const Task &b) { return a.order < b.order; });
		...
		//  WinFileBrowser
		// =====================================================================
		inline void NkUIDemo::WinFileBrowser(NkUIContext &ctx, NkUIWindowManager &wm, NkUILayoutStack &ls) {
			(void)ls;
			if (!mShowFileBrowser || !mFont)
				return;
			NkUIDrawList &dl = ctx.layers[NkUIContext::LAYER_WINDOWS];
			...
			if (NkUIWindow::Begin(ctx, wm, dl, *mFont, ls, "File Browser", &mShowFileBrowser)) {
				...
				if (ws) {
					...
					const NkRect contentR = {ws->pos.x + pad, ws->pos.y + tbH + pad, ws->size.x - pad * 2.f,
											 ws->size.y - tbH - pad * 2.f};
					NkUIFileBrowserResult r =
						NkUIFileBrowser::Draw(ctx, dl, *mFont, 0xFB001u, contentR, mFBConfig, mFBState, mFBProvider);
			...
			if (NkUIWindow::Begin(ctx, wm, dl, *mFont, ls, "Content Browser", &mShowContentBrowser)) {
				...
				if (ws) {
					...
					const NkRect contentR = {ws->pos.x + pad, ws->pos.y + tbH + pad, ws->size.x - pad * 2.f,
											 ws->size.y - tbH - pad * 2.f};
					NkUIContentBrowserResult r =
						NkUIContentBrowser::Draw(ctx, dl, *mFont, 0xFB001u, contentR, mCBConfig, mCBState, mFBProvider);
		...
		//  WinScene3D
		// =====================================================================
		inline void NkUIDemo::WinScene3D(NkUIContext &ctx, NkUIWindowManager &wm, NkUILayoutStack &ls, float32 dt) {
			(void)ls;
			if (!mShowScene3D || !mFont)
				return;
			NkUIDrawList &dl = ctx.layers[NkUIContext::LAYER_WINDOWS];
			...
			NkVP3DResult result = NkUIViewport3D::Draw(ctx, dl, *mFont, 0x0201u, contentR, mVPConfig, mVPState, dt);
			...
			if (result.event == NkVP3DEvent::NK_VP3D_OBJECT_SELECTED && result.objIdx >= 0)
				...
				PushLog("Moved: %s  (%.2f, %.2f, %.2f)", o.name, o.transform.position.x, o.transform.position.y,
		...
		// =====================================================================
		inline void NkUIDemo::WinSceneVp3D(NkUIContext &ctx, NkUIWindowManager &wm, NkUILayoutStack &ls, float32 dt) {
			(void)ls;
			(void)dt;
			if (!mShowSceneViewport3D || !mFont)
				return;
			NkUIDrawList &dl = ctx.layers[NkUIContext::LAYER_WINDOWS];
			...
			if (!NkUIWindow::Begin(ctx, wm, dl, *mFont, ls, "3D Scene", &mShowSceneViewport3D))
				return;
			...
			const NkRect winContent = {ws->pos.x + pad, ws->pos.y + tbH + pad, ws->size.x - pad * 2.f,
									   ws->size.y - tbH - pad * 2.f};
			const NkRect toolbarR = {winContent.x, winContent.y, winContent.w, toolH};
			const NkRect inspR = {winContent.x + winContent.w - inspW, winContent.y + toolH + pad, inspW,
								  winContent.h - toolH - pad};
			const NkRect viewR = {winContent.x, winContent.y + toolH + pad, winContent.w - inspW - pad,
								  winContent.h - toolH - pad};
			...
			dl.AddRectFilled(toolbarR, ctx.theme.colors.bgHeader, 4.f);
			{
				...
				for (const auto &b : btns) {
					...
					dl.AddRectFilled(br,
									 active ? ctx.theme.colors.accent
											: (hov ? ctx.theme.colors.buttonHover : ctx.theme.colors.buttonBg),
									 3.f);
					dl.AddRect(br, active ? ctx.theme.colors.accentHover : ctx.theme.colors.border, 1.f, 3.f);
					mFont->RenderText(dl, {bx + 6.f, by + (btnH - mFont->size) * 0.5f}, b.label,
									  ctx.theme.colors.textPrimary, btnW - 8.f, false);
					if (hov && ctx.input.IsMouseClicked(0))
						mS3DGizmoCfg.mode = b.mode;
					bx += btnW + 4.f;
				}
				...
				// Snap
				{
					...
					dl.AddRectFilled(br,
									 on ? ctx.theme.colors.accent
										: (hov ? ctx.theme.colors.buttonHover : ctx.theme.colors.buttonBg),
									 3.f);
					mFont->RenderText(dl, {bx + 6.f, by + (btnH - mFont->size) * 0.5f}, on ? "Snap ON" : "Snap",
									  ctx.theme.colors.textPrimary, 60.f, false);
					if (hov && ctx.input.IsMouseClicked(0))
						mS3DGizmoCfg.snap.enabled = !on;
					bx += 72.f;
				}
				...
				// Toggle grille
				{
					...
					dl.AddRectFilled(br,
									 ge ? ctx.theme.colors.accent
										: (hov ? ctx.theme.colors.buttonHover : ctx.theme.colors.buttonBg),
									 3.f);
					mFont->RenderText(dl, {bx + 6.f, by + (btnH - mFont->size) * 0.5f}, ge ? "Grille" : "[Grid]",
									  ctx.theme.colors.textPrimary, 52.f, false);
					if (hov && ctx.input.IsMouseClicked(0))
						mS3DGridCfg.enabled = !ge;
					bx += 64.f;
				}
				(void)bx;
			}
			...
			const bool gizmoWasActive =
				...
				(mS3DObjects[mS3DSelectedObj].gizmoState.active || mS3DObjects[mS3DSelectedObj].gizmoState.hovered);
			...
			for (int32 oi = 0; oi < mNumS3DObjects; ++oi) {
				...
				if (obj.shape == 0) {
					...
					NkVec2 v[8];
					v[0] = project(px - hx, py - hy, pz - hz);
					v[1] = project(px + hx, py - hy, pz - hz);
					v[2] = project(px + hx, py + hy, pz - hz);
					v[3] = project(px - hx, py + hy, pz - hz);
					v[4] = project(px - hx, py - hy, pz + hz);
					v[5] = project(px + hx, py - hy, pz + hz);
					v[6] = project(px + hx, py + hy, pz + hz);
					v[7] = project(px - hx, py + hy, pz + hz);
					...
					for (int32 plane = 0; plane < 3; ++plane) {
						...
						for (int32 k = 0; k <= seg; ++k) {
							...
							NkVec2 pt;
							if (plane == 0)
								pt = project(px + ca, py + sa, pz);
							else if (plane == 1)
								pt = project(px + ca, py, pz + sa);
							else
								pt = project(px, py + ca, pz + sa);
							if (k > 0)
				...
				if (isSel)
					dl.AddCircle(center2D, 16.f, {255, 240, 80, 120}, 16, 3.f);
			...
			if (mS3DSelectedObj >= 0 && mS3DSelectedObj < mNumS3DObjects) {
				...
				NkUIGizmoResult gr =
					NkUIGizmo::Manipulate3D(ctx, dl, gdesc, mS3DGizmoCfg, obj.gizmoState, obj.transform);
			...
			dl.AddRect(inspR, ctx.theme.colors.border, 1.f, 3.f);
			{
				...
				for (int32 oi = 0; oi < mNumS3DObjects; ++oi) {
					...
					if (isSel)
						dl.AddRectFilled(row, ctx.theme.colors.accent.WithAlpha(80), 3.f);
					...
					mFont->RenderText(dl, {ix + 36.f, iy + ty}, mS3DObjects[oi].name,
									  isSel ? ctx.theme.colors.textPrimary : ctx.theme.colors.textSecondary, iw - 38.f,
									  false);
				...
				if (mS3DSelectedObj >= 0 && mS3DSelectedObj < mNumS3DObjects) {
					...
					mFont->RenderText(dl, {ix + 8.f, iy + (bh - mFont->size) * 0.5f}, "Reset Transform",
									  ctx.theme.colors.textPrimary, iw - 10.f, false);
					...
					iy += 14.f;
					mFont->RenderText(dl, {ix, iy}, "Cliquez un objet\npour inspecter.", ctx.theme.colors.textSecondary,
									  iw, false);
				}
			}

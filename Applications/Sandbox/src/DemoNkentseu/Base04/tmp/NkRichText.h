	...
	namespace nkui {
		...
		class NKUI_API NkSyntaxHighlighter {
			public:
				 ...
				 */
				static NkVector<NkSyntaxToken> Highlight(const char *text, Language lang,
														 const NkSyntaxTheme &theme = NkSyntaxTheme(),
														 NkMemArena *arena = nullptr) noexcept;

				/**
				 * @brief Rendu de code colorisé
				 */
				static void RenderCode(NkUIDrawList &dl, const char *text, Language lang, NkVec2 pos, NkUIFont *font,
									   const NkSyntaxTheme &theme = NkSyntaxTheme(), float32 maxWidth = -1.f,
									   bool showLineNumbers = true) noexcept;

			private:
				// Parsers par langage
				static void HighlightC(const char *text, NkVector<NkSyntaxToken> &out, const NkSyntaxTheme &theme,
									   NkMemArena *arena) noexcept;
				static void HighlightPython(const char *text, NkVector<NkSyntaxToken> &out, const NkSyntaxTheme &theme,
											NkMemArena *arena) noexcept;
				static void HighlightJavaScript(const char *text, NkVector<NkSyntaxToken> &out,
												const NkSyntaxTheme &theme, NkMemArena *arena) noexcept;
				static void HighlightJSON(const char *text, NkVector<NkSyntaxToken> &out, const NkSyntaxTheme &theme,
										  NkMemArena *arena) noexcept;
				static void HighlightGLSL(const char *text, NkVector<NkSyntaxToken> &out, const NkSyntaxTheme &theme,
										  NkMemArena *arena) noexcept;
		...
		class NKUI_API NkRichText {
			public:
				 ...
				 */
				void SetDefaultFont(NkUIFont *font) noexcept {
					mDefaultFont = font;
				}
				 ...
				 */
				void SetDefaultStyle(const NkTextStyle &style) noexcept {
					mDefaultStyle = style;
				}
				 ...
				 */
				float32 Render(NkUIDrawList &dl, NkVec2 pos, float32 maxWidth = -1.f,
							   const char **hoverLink = nullptr) noexcept;

				/**
				 * @brief Rendu avec gestion des clics sur liens
				 */
				float32 RenderInteractive(NkUIDrawList &dl, NkVec2 pos, const NkUIInputState &input,
										  float32 maxWidth = -1.f, const char **clickedLink = nullptr) noexcept;
				 ...
				 */
				uint32 GetFragmentCount() const noexcept {
			...
			private:
				...
				float32 RenderFragment(NkUIDrawList &dl, NkTextFragment &frag, NkVec2 pos, float32 &curX, float32 &curY,
									   float32 lineHeight, float32 maxWidth, bool interactive,
		...
		class NKUI_API NkHtmlTextParser {
			public:
				 ...
				 */
				static NkRichText Parse(const char *html, NkUIFont *defaultFont,
										NkColor defaultColor = NkColor{255, 255, 255, 255}) noexcept;

				/**
				 * @brief Parse un fichier HTML
				 */
				static NkRichText ParseFile(const char *filepath, NkUIFont *defaultFont,
											NkColor defaultColor = NkColor{255, 255, 255, 255}) noexcept;
			...
			private:
				...
				static const char *ParseAttribute(const char *ptr, const char *name, char *value,

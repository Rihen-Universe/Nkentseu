...
namespace nkentseu {
	namespace nkui {
		...
		void NkRichText::AddCode(const char *code, NkSyntaxHighlighter::Language lang, NkUIFont *codeFont) noexcept {
			if (!codeFont)
				codeFont = mDefaultFont;
		...
		float32 NkRichText::RenderFragment(NkUIDrawList &dl, NkTextFragment &frag, NkVec2 pos, float32 &curX,
										   float32 &curY, float32 lineHeight, float32 maxWidth, bool interactive,
										   const NkUIInputState *input, const char **hoverLink,
										   const char **clickedLink) noexcept {
			if (!frag.text || !*frag.text)
				return lineHeight;
			...
			while (*p) {
				...
				if (*p != ' ') {
					...
					if (interactive && frag.linkUrl && input) {
						...
						if (input->mousePos.x >= x && input->mousePos.x <= charEndX && input->mousePos.y >= y &&
							input->mousePos.y <= y + fontSize) {
							renderColor = {renderColor.r, renderColor.g, 255, 255};
							if (hoverLink)
								*hoverLink = frag.linkUrl;
							if (clickedLink && input->IsMouseClicked(0))
								*clickedLink = frag.linkUrl;
						}
					}
		...
		float32 NkRichText::Render(NkUIDrawList &dl, NkVec2 pos, float32 maxWidth, const char **hoverLink) noexcept {
		...
		float32 NkRichText::RenderInteractive(NkUIDrawList &dl, NkVec2 pos, const NkUIInputState &input,
			...
			for (auto &frag : mFragments) {
				lineHeight = RenderFragment(dl, frag, pos, curX, curY, lineHeight, maxWidth, true, &input,
											&hoverLinkResult, &clickedLinkResult);
			}

			if (clickedLink && clickedLinkResult)
				*clickedLink = clickedLinkResult;
		...
		static const char *cKeywords[] = {
			...
			"class",	"namespace",	 "new",			"delete",	"virtual",	"override",	 "final",	 "public",
			"private",	"protected",	 "template",	"typename", "using",	"constexpr", "nullptr",	 "decltype",
			"noexcept", "static_assert", "thread_local"};
		static const uint32 cKeywordsCount = sizeof(cKeywords) / sizeof(cKeywords[0]);

		// Types C/C++
		static const char *cTypes[] = {"int8_t",   "int16_t",  "int32_t", "int64_t",   "uint8_t",  "uint16_t",
		...
		void NkSyntaxHighlighter::HighlightC(const char *text, NkVector<NkSyntaxToken> &out, const NkSyntaxTheme &theme,
				...
				switch (state) {
					case NORMAL:
						if (*p == '/' && *(p + 1) == '/') {
							...
							while (IsDigit(*p) || *p == '.' || *p == 'x' || *p == 'X' || (*p >= 'a' && *p <= 'f') ||
								   (*p >= 'A' && *p <= 'F'))
								p++;
		...
		NkVector<NkSyntaxToken> NkSyntaxHighlighter::Highlight(const char *text, Language lang,
		...
		void NkSyntaxHighlighter::RenderCode(NkUIDrawList &dl, const char *text, Language lang, NkVec2 pos,
											 NkUIFont *font, const NkSyntaxTheme &theme, float32 maxWidth,
											 bool showLineNumbers) noexcept {
			if (!font)
				return;
		...
		bool NkHtmlTextParser::ParseTag(const char *&ptr, Tag &out) noexcept {
			...
			if (tagLen >= sizeof(out.name))
				tagLen = sizeof(out.name) - 1;
		...
		const char *NkHtmlTextParser::ParseAttribute(const char *ptr, const char *name, char *value,
													 uint32 maxLen) noexcept {
			char search[64];
		...
		NkSyntaxHighlighter::Language NkHtmlTextParser::ParseLanguage(const char *str) noexcept {
			...
			if (strcmp(str, "c") == 0 || strcmp(str, "cpp") == 0)
				return NkSyntaxHighlighter::LANG_CPP;
			if (strcmp(str, "python") == 0 || strcmp(str, "py") == 0)
				return NkSyntaxHighlighter::LANG_PYTHON;
			if (strcmp(str, "javascript") == 0 || strcmp(str, "js") == 0)
				return NkSyntaxHighlighter::LANG_JAVASCRIPT;
			if (strcmp(str, "json") == 0)
				return NkSyntaxHighlighter::LANG_JSON;
			if (strcmp(str, "xml") == 0 || strcmp(str, "html") == 0)
				return NkSyntaxHighlighter::LANG_XML;
			if (strcmp(str, "glsl") == 0)
				return NkSyntaxHighlighter::LANG_GLSL;
			if (strcmp(str, "hlsl") == 0)
				return NkSyntaxHighlighter::LANG_HLSL;
			return NkSyntaxHighlighter::LANG_TEXT;
		}

		NkRichText NkHtmlTextParser::Parse(const char *html, NkUIFont *defaultFont, NkColor defaultColor) noexcept {
			NkRichText richText;

...
namespace nkentseu {
	namespace nkui {
		...
		bool NkUIFontLoader::Init(const NkUIFontConfig &config, usize permanentBytes, usize scratchBytes,
								  int32 atlasWidth, int32 atlasHeight, void *gpuDevice,
		...
		uint32 NkUIFontLoader::LoadFontFromFile(const char *filepath, float32 fontSize, bool preloadASCII) noexcept {
		...
		void NkUIFontLoader::PreloadASCIIFromNKFont(NkFontFace *face) noexcept {
			...
			static const uint32 kCommonEmoji[] = {0x1F600, 0x1F601, 0x1F602, 0x1F603, 0x1F604, 0x1F605, 0x1F606,
												  0x1F609, 0x1F60A, 0x1F60D, 0x1F618, 0x1F60E, 0x1F622, 0x1F44D,
												  0x1F64F, 0x1F525, 0x1F389, 0x1F680, 0x1F3AE, 0x2705,	0x2764,
												  0x1F44C, 0x1F914, 0x1F44E, 0x1F4A1};
			int32 emojiLoaded = 0;
		...
		bool NkUIFontLoader::RasterizeGlyphToAtlas(NkFontFace *face, uint32 codepoint, NkUIGlyph &outGlyph) noexcept {
			...
			for (int32 row = 0; row < height; ++row) {
				memory::NkCopy(mAtlas->pixels + (y + row) * NkUIFontAtlas::ATLAS_W + x, src + row * srcStride, width);
			}

			mAtlas->AddGlyph(codepoint, x, y, width, height, nkGlyph.metrics.xAdvance.ToFloat(),
		...
		uint32 NkUIFontLoader::LoadSystemFont(const char *fontName, float32 fontSize, bool preloadASCII) noexcept {
...
#ifdef _WIN32
			const char *systemPaths[] = {"C:\\Windows\\Fonts\\", nullptr};
#else
			const char *systemPaths[] = {"/usr/share/fonts/", "/usr/local/share/fonts/", nullptr};
#endif
			...
			const char *namesToTry[] = {fontName, noSpaceName, lowerName, nullptr};
			...
			for (int p = 0; systemPaths[p]; ++p) {
				for (int n = 0; namesToTry[n]; ++n) {
					for (int e = 0; extensions[e]; ++e) {
						snprintf(fullPath, sizeof(fullPath), "%s%s%s", systemPaths[p], namesToTry[n], extensions[e]);
		...
		void NkUIFontLoader::UploadAtlas() noexcept {
			...
			if (mGPUUploadFunc) {
				...
				// Appeler le callback
				bool success = mGPUUploadFunc(mGPUDevice, mAtlas->texId, uploadData, NkUIFontAtlas::ATLAS_W,
											  NkUIFontAtlas::ATLAS_H);

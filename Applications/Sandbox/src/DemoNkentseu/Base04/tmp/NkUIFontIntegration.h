	...
	namespace nkui {
		...
		class NKUI_API NkUIFontLoader {
			public:
				...
				bool Init(const NkUIFontConfig &config = NkUIFontConfig(), usize permanentBytes = 8 * 1024 * 1024,
						  usize scratchBytes = 2 * 1024 * 1024, int32 atlasWidth = 512, int32 atlasHeight = 512,
						  void *gpuDevice = nullptr, NkGPUUploadFunc uploadFunc = nullptr) noexcept;
				...
				uint32 GetFontCount() const noexcept {
					return mNumFonts;
				}

				void UploadAtlas() noexcept;

				NkUIFontAtlas *GetAtlas() noexcept {
					return mAtlas;
				}

				NkFontLibrary *GetFontLibrary() noexcept {
					return &mFontLib;
				}

				void SetGPUDevice(void *device) noexcept {
					mGPUDevice = device;
				}

				void SetGPUUploadFunc(NkGPUUploadFunc func) noexcept {
					mGPUUploadFunc = func;
				}

				void *GetGPUDevice() const noexcept {
					return mGPUDevice;
				}

				NkGPUUploadFunc GetGPUUploadFunc() const noexcept {
					return mGPUUploadFunc;
				}
				...
				const uint8 *GetAtlasPixels() const {
					return mAtlas ? mAtlas->pixels : nullptr;
				}

				// Indique si l'atlas doit être uploadé en RGBA8
				bool RequiresRGBAConversion() const {
					return uploadRGBA8;
				}

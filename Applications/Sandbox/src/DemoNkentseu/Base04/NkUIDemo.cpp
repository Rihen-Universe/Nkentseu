...
// DONT L'OBJET est NKUI (cible NKUID), module deprecie remplace par NKGui.
...
class NkFontRenderer2D {
	public:
		...
		bool init(NkIDevice *dev, NkRenderPassHandle swapRP, const char *fontPath) {
			...
			for (int i = 0; i < 4; ++i) {
				mFonts[i] = NkUIFont{};
				if (mBridges[i].InitFromFile(&atlas, &mFonts[i], fontPath, entries[i].size,
			...
			// 3. Upload atlas → GPU (Gray8 → RGBA8)
			{
				...
				if (hasPixels) {
					...
					NkTextureDesc td = NkTextureDesc::Tex2D(NkUIFontAtlas::ATLAS_W, NkUIFontAtlas::ATLAS_H,
					...
					dev->WriteTexture(mAtlasTex, rgba, static_cast<uint32>(NkUIFontAtlas::ATLAS_W * 4));
					delete[] rgba;
					...
					logger.Info("[NkUIDemo] Atlas GPU: {0}x{1}, {2} glyphes\n", NkUIFontAtlas::ATLAS_W,
								NkUIFontAtlas::ATLAS_H, atlas.numGlyphs);
				} else {
		...
		void shutdown() {
			...
			auto &d = *mDev;
			if (mDs.IsValid())
				d.FreeDescriptorSet(mDs);
			if (mDsl.IsValid())
				d.DestroyDescriptorSetLayout(mDsl);
			if (mPipe.IsValid())
				d.DestroyPipeline(mPipe);
			if (mShd.IsValid())
				d.DestroyShader(mShd);
			if (mVbo.IsValid())
				d.DestroyBuffer(mVbo);
			if (mUbo.IsValid())
				d.DestroyBuffer(mUbo);
			if (mSampler.IsValid())
		...
		// pos.y = baseline
		void text(NkUIFont *fnt, const char *str, float px, float py, float r = 1.f, float gc = 1.f, float b = 1.f,
				  float a = 1.f, float scale = 1.f) {
			if (!mOk || !fnt || !str)
				return;
			...
			while (*p) {
				...
				if (cp == ' ') {
					curX += fnt->metrics.spaceWidth * scale;
					continue;
				}
				if (cp == '\t') {
					curX += fnt->metrics.tabWidth * scale;
					continue;
				}
				...
				if (g && g->x1 > g->x0 && g->y1 > g->y0) {
					...
					pushQuad(left, top, gw, gh, g->u0, g->v0, g->u1, g->v1, r, gc, b, a);
					curX += g->advanceX * scale;
				} else {
					// Espace-placeholder si pas de glyphe dans l'atlas
					curX += (fnt->metrics.spaceWidth > 0.f ? fnt->metrics.spaceWidth : fnt->size * 0.5f) * scale;
				}
			}
		}

		// Coin haut-gauche → convertit Y automatiquement
		void textAt(NkUIFont *fnt, const char *str, float left, float top, float r = 1.f, float gc = 1.f, float b = 1.f,
					float a = 1.f, float scale = 1.f) {
			if (!fnt)
				return;
	...
	private:
		...
		static int decodeUTF8(const char *s, uint32 &cp) {
			...
			if ((c0 & 0xE0) == 0xC0 && u[1]) {
				cp = ((c0 & 0x1F) << 6) | (u[1] & 0x3F);
				return 2;
			}
			if ((c0 & 0xF0) == 0xE0 && u[1] && u[2]) {
				cp = ((c0 & 0x0F) << 12) | ((u[1] & 0x3F) << 6) | (u[2] & 0x3F);
				return 3;
			}
			if ((c0 & 0xF8) == 0xF0 && u[1] && u[2] && u[3]) {
				cp = ((c0 & 0x07) << 18) | ((u[1] & 0x3F) << 12) | ((u[2] & 0x3F) << 6) | (u[3] & 0x3F);
				return 4;
			}
			cp = '?';
			return 1;
		...
		void pushQuad(float x, float y, float w, float h, float u0, float v0, float u1, float v1, float r, float g,
					  float b, float a) {
			if (mVerts.Size() + 6u > MAX_VERTS)
				return;
		...
		void rebindDescriptors() {
			...
			if (mUbo.IsValid()) {
				...
				w.buffer = mUbo;
				w.bufferRange = sizeof(ScreenUBO);
				mDev->UpdateDescriptorSets(&w, 1u);
			}
		}
};
...
struct TextPrinter {
		...
		void line(NkUIFont *fnt, const char *txt, float x = 10.f, float r = 1.f, float g = 1.f, float b = 1.f,
				  float a = 1.f, float scale = 1.f) {
			if (!fr || !fnt || !txt)
				return;
...
int nkmain(const NkEntryState & /*state*/) {
	...
	NkWindow window;
	if (!window.Create(wc)) {
		logger.Error("[NkUIDemo] Fenêtre échec\n");
		return 1;
	}
	...
	if (!cmd) {
		logger.Error("[NkUIDemo] CommandBuffer échec\n");
		return 1;
	}
	...
	static const char *kFontPaths[] = {"Resources/Fonts/Roboto-Regular.ttf",
	...
	for (int i = 0; kFontPaths[i] && !frOk; ++i) {
		...
		if (frOk)
			logger.Info("[NkUIDemo] Police chargée: {0}\n", kFontPaths[i]);
	}
	...
	ev.AddEventCallback<NkKeyPressEvent>([&](NkKeyPressEvent *e) {
		switch (e->GetKey()) {
			...
			case NkKey::NK_NUMPAD_ADD:
				userScale = userScale + 0.1f > 4.f ? 4.f : userScale + 0.1f;
				break;
			case NkKey::NK_NUMPAD_SUB:
				userScale = userScale - 0.1f < 0.25f ? 0.25f : userScale - 0.1f;
				break;
			case NkKey::NK_R:
				userScale = 1.f;
				break;
	...
	while (running) {
		...
		float dt = static_cast<float>(now - tPrev);
		tPrev = now;
		fpsAcc += dt;
		++fpsCnt;
		if (fpsAcc >= 0.5) {
		...
		{
			char buf[256];
			snprintf(buf, sizeof(buf), "FPS: %.0f  |  NkUI + NKFont  |  Scale: %.2fx (+/-/R)  |  S=stats", fps,
					 userScale);
			tp.reset(8.f);
		...
		tp.line(fUI, "Pipeline: TTF/OTF parser → Rasterizer → Atlas R8 → Quad render", 10.f, 0.6f, 0.9f, 1.f, 1.f,
				userScale);
		tp.line(fUI, "NkUIFontBridge → NkFontParser + NkFontRasterizer → NkUIFontAtlas → NkUIFont", 10.f, 0.6f, 0.9f,
				1.f, 1.f, userScale);

		if (showStats) {
			tp.nl(fSmall, 0.5f);
			char buf[256];
			snprintf(buf, sizeof(buf), "Atlas: %dx%d px | Glyphes: %d | texId: %u", NkUIFontAtlas::ATLAS_W,
					 NkUIFontAtlas::ATLAS_H, fr->atlas.numGlyphs, fr->atlas.texId);
			tp.line(fSmall, buf, 10.f, 0.6f, 0.6f, 0.6f, 1.f);

			if (fUI) {
				snprintf(buf, sizeof(buf), "FontUI: asc=%.1f desc=%.1f lineH=%.1f spaceW=%.1f | builtin=%s",
						 fUI->metrics.ascender, fUI->metrics.descender, fUI->metrics.lineHeight,
						 fUI->metrics.spaceWidth, fUI->isBuiltin ? "OUI" : "NON");
				tp.line(fSmall, buf, 10.f, 0.6f, 0.6f, 0.6f, 1.f);
			}

			snprintf(buf, sizeof(buf), "Resolution: %ux%u | UserScale: %.2fx", W, H, userScale);
			tp.line(fSmall, buf, 10.f, 0.6f, 0.6f, 0.6f, 1.f);
		}
		...
		// Instructions bas d'écran
		{
			...
			tp.line(fBot, "+/- : zoom  |  R : reset  |  S : stats  |  ESC : quitter", 10.f, 0.5f, 0.5f, 0.5f, 0.8f);
		}

// DEREGISTREE DU BUILD (campagne retrait NKUI, 2026-08-17, decision R1) : fait
// partie de la demo NKUIDemo, DONT L'OBJET est NKUI (module deprecie remplace
// par NKGui). Source conservee ; elle meurt avec le module NKUI le jour ou
// Rodolf en decide la suppression (compteur a zero). Voir RhiSandbox.jenga.
...
float4 PSMain(PSIn i) : SV_Target {
    ...
    return i.col * tc;
		...
		bool NkUINKRHIBackend::CreatePipeline() {
			...
			layout.AddAttribute(0, 0, NkGPUFormat::NK_RG32_FLOAT, 0, "POSITION", 0)
		...
		bool NkUINKRHIBackend::EnsureGeometryBuffers(uint64 requiredVtxBytes, uint64 requiredIdxBytes,
													 bool allowShrink) {
			if (!mDevice) {
				return false;
			}

			if (requiredVtxBytes < kMinVBOCap)
				requiredVtxBytes = kMinVBOCap;
			if (requiredIdxBytes < kMinIBOCap)
				requiredIdxBytes = kMinIBOCap;
		...
		void NkUINKRHIBackend::Submit(NkICommandBuffer *cmd, const NkUIContext &ctx, uint32 fbW, uint32 fbH) {
			if (!cmd || !mPipeline.IsValid() || !mVBO.IsValid() || !mIBO.IsValid() || !mUBO.IsValid() || fbW == 0 ||
				fbH == 0) {
				return;
			}
			...
			for (int32 layer = 0; layer < NkUIContext::LAYER_COUNT; ++layer) {
				...
				std::memcpy(mScratchVtx.Data() + static_cast<usize>(baseVtx), dl.vtx,
		...
		bool NkUINKRHIBackend::UploadTextureInternal(uint32 texId, const uint8 *data, int32 width, int32 height,
													 bool rgba8) {
			...
			const bool canReuse = existing && existing->texture.IsValid() && existing->descSet.IsValid() &&

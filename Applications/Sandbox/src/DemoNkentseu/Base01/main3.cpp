...
namespace {
	...
	class AppLayer {
		public:
			...
			bool FullscreenDirty() const {
				return mFullscreenDirty;
			}
			...
			void ClearFullscreenDirty() {
				mFullscreenDirty = false;
			}

			NkVec2f GetPhase() const {
				return mPhase;
			}
			...
			NkVec2u GetViewport() const {
				return {mViewW, mViewH};
			}

			// RAsumA stats pour HUD debug
			void PrintStats() const {
				logger.Info("[Stats] keys={0} mouse={1} wheel={2} drops={3} gamepads={4} resize={5}", mKeyPresses,
							mMouseClicks, mWheelTicks, mDrops, mGamepadEvents, mResizes);
			}
		...
		private:
			...
			bool OnDropEnter(NkDropEnterEvent &e) {
				logger.Info("[Drop] Enter pos=({0},{1}) numFiles={2} hasText={3}", e.data.x, e.data.y, e.data.numFiles,
			...
			bool OnGamepadButton(NkGamepadButtonPressEvent &e) {
				...
				if (e.GetButton() == NkGamepadButton::NK_GP_SOUTH) {
					mNeon = !mNeon;
					NkGamepads().Rumble(e.GetGamepadIndex(), 0.3f, 0.3f, 0.f, 0.f, 60);
					return true;
				}
	...
	static void DrawPlasma(NkRenderer &r, uint32 w, uint32 h, float t, NkVec2f phase, float sat) {
		if (!w || !h)
			return;
		...
		for (uint32 y = 0; y < h; y += blk) {
			...
			for (uint32 x = 0; x < w; x += blk) {
				...
				float mix = (NkSin((fx + phase.x) * 13.5f + t * 1.7f) + NkSin((fy + phase.y) * 11.f - t * 1.3f) +
							 NkSin(rd * 24.f - t * 2.1f)) *
							0.333f;

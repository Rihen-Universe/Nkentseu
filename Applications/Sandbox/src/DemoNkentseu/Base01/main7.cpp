...
namespace {
	...
	struct Stats {
			...
			void Print(float elapsed) const {
				logger.Info("[HEALTH t={0}s] pushed={1} polled={2} diff={3} gpAxis={4} gpBtn={5} rumble={6}", elapsed,
							(unsigned long long)pushed.Load(), (unsigned long long)polled.Load(),
							(long long)(pushed.Load() - polled.Load()), (unsigned long long)gpAxis.Load(),
							(unsigned long long)gpBtn.Load(), (unsigned long long)gpRumble.Load());
			}
	};
	...
	static void DrawPlasma(NkRenderer &r, uint32 w, uint32 h, float t) {
		...
		for (uint32 y = 0; y < h; y += blk) {
			...
			for (uint32 x = 0; x < w; x += blk) {
				...
				float mix = (math::NkSin(fx * 13.5f + t * 1.7f) + math::NkSin(fy * 11.f - t * 1.3f) +
	...
	static void PollGamepad(uint32 idx, GpState &gs, Stats &stats) {
		...
		for (uint32 bi = static_cast<uint32>(NkGamepadButton::NK_GP_SOUTH);
			...
			if (down != gs.buttons[bi]) {
				...
				++stats.gpBtn;
				if (down) {
					logger.Info("[GP#{0}] Button {1} ({2}) pressed", idx, bi, NkGamepadButtonToString(btn));
					// Rumble sur chaque nouveau bouton pressA
					gp.Rumble(idx, 0.1f + static_cast<float>(bi) * 0.02f, 0.2f + static_cast<float>(bi) * 0.015f, 0.f,
							  0.f, 40);
					++stats.gpRumble;
				}
			}
...
int nkmain(const nkentseu::NkEntryState & /*state*/) {
	...
	NkGamepads().SetConnectCallback([](const NkGamepadInfo &info, bool connected) {
		logger.Info("[Gamepad] {0} #{1} ({2})", connected ? "CONNECTED" : "DISCONNECTED", info.index, info.name);
	});
	...
	es.AddEventCallback<NkGamepadAxisEvent>([&stats](NkGamepadAxisEvent *ev) {
		++stats.gpAxis;
		(void)ev;
	});
	es.AddEventCallback<NkGamepadButtonPressEvent>([&stats](NkGamepadButtonPressEvent *ev) {
		++stats.gpBtn;
		logger.Info("[GP Event] ButtonPress {0} idx={1}", (int)ev->GetButton(), ev->GetGamepadIndex());
	});
	es.AddEventCallback<NkGamepadButtonReleaseEvent>([&stats](NkGamepadButtonReleaseEvent *ev) {
		++stats.gpBtn;
	...
	while (running && window.IsOpen()) {
		...
		if (++gpPrint % 120 == 0) {
			for (uint32 gpIdx = 0; gpIdx < NK_MAX_GAMEPADS; ++gpIdx) {
				if (!gpStates[gpIdx].connected)
					continue;
				logger.Info("[GP#{0}] LX={1} LY={2} RX={3} RY={4} LT={5} RT={6}", gpIdx,

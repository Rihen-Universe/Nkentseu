...
static void ApplyAppDataConfigPair(nkentseu::NkAppData &d, nkentseu::NkString key, nkentseu::NkString value) {
	key.Trim().ToLower();
	value.Trim();
	...
	while (start < text.Size()) {
		...
		if (!line.Empty()) {
			const bool isComment =
				line[0] == '#' || line[0] == ';' || (line.Size() >= 2 && line[0] == '/' && line[1] == '/');
...
namespace {
	...
	// ---------------------------------------------------------------------------
	static void DrawPlasma(NkRenderer &r, uint32 w, uint32 h, float t, float px, float py, float sat, float hue) {
		if (!w || !h)
			return;
		...
		for (uint32 y = 0; y < h; y += blk) {
			...
			for (uint32 x = 0; x < w; x += blk) {
				...
				float mix = (math::NkSin((fx + px) * 13.5f + t * 1.7f) + math::NkSin((fy + py) * 11.f - t * 1.3f) +
							...
							0.333f;
				uint8 ri =
					(uint8)(ClampUnit((0.5f + 0.5f * math::NkSin(6.28f * (mix + hue + 0.f)) - 0.5f) * sat + 0.5f) *
							255);
				uint8 gi =
					(uint8)(ClampUnit((0.5f + 0.5f * math::NkSin(6.28f * (mix + hue + 0.33f)) - 0.5f) * sat + 0.5f) *
							255);
				uint8 bi =
					(uint8)(ClampUnit((0.5f + 0.5f * math::NkSin(6.28f * (mix + hue + 0.66f)) - 0.5f) * sat + 0.5f) *
							255);
...
int nkmain(const nkentseu::NkEntryState & /*state*/) {
	...
	NkContext contextA{};
	NkContext contextB{};
	if (!NkContextCreate(windowA, contextA, &contextConfig) || !NkContextCreate(windowB, contextB, &contextConfig)) {
	...
	logger.Info("[ex05] Window A id={0}, Window B id={1}", static_cast<unsigned long long>(wins[0].id),
	...
	while (wins[0].open || wins[1].open) {
		...
		for (auto &ws : wins) {
			...
			DrawPlasma(ws.renderer, w, h, ws.time, ws.phaseX, ws.phaseY, ws.saturation, ws.hueShift);

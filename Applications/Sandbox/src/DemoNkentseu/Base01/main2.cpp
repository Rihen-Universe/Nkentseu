...
static const char *EventTypeName(nkentseu::NkEventType::Value t) {
	...
	switch (t) {
		...
		case NkEventType::NK_GAMEPAD_AXIS_MOTION:
			return "GamepadAxis";
	...
	while (running && window.IsOpen()) {
		...
		while ((ev = es.PollEvent()) != nullptr) {
			...
			if (auto *e = ev->As<NkWindowCloseEvent>()) {
				...
				char d[64];
				logger.Info("MouseMove event: pos=({0},{1}) delta=({2},{3})", e->GetX(), e->GetY(), e->GetDeltaX(),
							e->GetDeltaY());
				line += d;
			} else if (auto *e = ev->As<NkMouseButtonPressEvent>()) {
				char d[64];
				logger.Info("MouseButtonPress event: btn={0} pos=({1},{2})", static_cast<int>(e->GetButton()),
							e->GetX(), e->GetY());
				line += d;

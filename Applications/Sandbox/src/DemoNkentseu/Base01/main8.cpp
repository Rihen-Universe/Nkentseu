...
namespace {
	...
	static void DrawStick(NkIGraphicsContext *ctx, float cx, float cy, float size, float ax, float ay, bool pressed) {
		ax = SafeAxis11(ax);
		ay = SafeAxis11(ay);
	...
	static bool IsEscapePressed(const NkKeyPressEvent *k) {
		if (!k)
			return false;
		return k->GetKey() == NkKey::NK_ESCAPE || k->GetScancode() == NkScancode::NK_SC_ESCAPE;
	}

	enum class NkPadProfile : uint32 { AUTO = 0, XBOX, PS4, PS3, SWITCH_PRO, GENERIC_DINPUT, COUNT };
	...
	static NkPadProfile DetectPadProfile(const NkGamepadInfo &info) {
		...
		if (info.type == NkGamepadType::NK_GP_TYPE_XBOX || info.vendorId == 0x045E || Contains(name, "xbox") ||
			Contains(name, "xinput")) {
		...
		if (info.vendorId == 0x057E || Contains(name, "nintendo") || Contains(name, "switch") ||
			Contains(name, "pro controller")) {
	...
	static void MapXboxLikeLogicalButtons(uint32 padIndex) {
		auto &gp = NkGamepads();
		auto mapBtn = [&](NkGamepadButton btn) { gp.SetButtonMapByIndex(padIndex, static_cast<uint32>(btn), btn); };
	...
	static void ProbeRawInput(uint32 padIndex, RawProbeState &probe) {
		...
		for (uint32 b = 0; b < NkGamepadSystem::BUTTON_COUNT; ++b) {
			if (raw.buttons[b] == probe.buttons[padIndex][b])
				continue;
			const bool wasDown = probe.buttons[padIndex][b];
			probe.buttons[padIndex][b] = raw.buttons[b];
			logger.Infof("[RAW GP%u] button[%u] %s", padIndex, b, raw.buttons[b] ? "DOWN" : "UP");
			...
			if (b >= kKnownButtonCount && !wasDown && raw.buttons[b]) {
				logger.Infof("[LOW-LEVEL GP%u] UNKNOWN_BUTTON code=%u (raw_index=%u)", padIndex,
							 static_cast<unsigned>(b - kRawButtonBase), static_cast<unsigned>(b));
			}
		}
		...
		for (uint32 a = 0; a < NkGamepadSystem::AXIS_COUNT; ++a) {
			...
			if (!math::NkIsFinite(now))
				now = 0.f;
			if (math::NkFabs(now - prev) < 0.20f)
				continue;
			...
			if (a >= kKnownAxisCount) {
				logger.Infof("[LOW-LEVEL GP%u] UNKNOWN_AXIS code=%u (raw_index=%u) value=%.3f", padIndex,
							 static_cast<unsigned>(a - kKnownAxisCount), static_cast<unsigned>(a), now);
			}
		}
	}

	static void DrawPs3Layout(NkIGraphicsContext *ctx, uint32 width, uint32 height, bool connected, uint32 padIndex) {
		if (width == 0 || height == 0)
			return;
...
int nkmain(const nkentseu::NkEntryState & /*state*/) {
	...
	if (kEnableRuntimeHidHooks) {
		NkGamepads().SetConnectCallback([&presetApplied](const NkGamepadInfo &info, bool connected) {
			logger.Infof("[Gamepad] %s #%u (%.127s) vid=0x%04X pid=0x%04X type=%u",
						 connected ? "CONNECTED" : "DISCONNECTED", info.index, info.name,
						 static_cast<unsigned>(info.vendorId), static_cast<unsigned>(info.productId),
		...
		events.AddEventCallback<NkHidConnectEvent>([&hidMapper](NkHidConnectEvent *ev) {
			...
			logger.Infof("[HID] CONNECT dev=%llu name=%.127s", static_cast<unsigned long long>(info.deviceId),
						 info.name);
		});

		events.AddEventCallback<NkHidButtonPressEvent>([&hidMapper](NkHidButtonPressEvent *ev) {
			...
			if (hidMapper.MapButtonEvent(*ev, logical, state)) {
				logger.Infof("[HID] BUTTON dev=%llu phys=%u -> logical=%u",
							 static_cast<unsigned long long>(ev->GetDeviceId()), ev->GetButtonIndex(), logical);
			}
		});

		events.AddEventCallback<NkHidAxisEvent>([&hidMapper](NkHidAxisEvent *ev) {
			...
			if (hidMapper.MapAxisEvent(*ev, logicalAxis, mappedValue)) {
				logger.Infof("[HID] AXIS dev=%llu phys=%u -> logical=%u val=%.3f",
							 static_cast<unsigned long long>(ev->GetDeviceId()), ev->GetAxisIndex(), logicalAxis,
							 mappedValue);
			}
		});
	}
	...
	while (lc.running) {
		...
		while (NkEvent *ev = events.PollEvent()) {
			...
			if constexpr (kInputDiagLogs) {
				if (ev->Is<NkMouseEnterEvent>())
					logger.Info("[MOUSE] enter");
				if (ev->Is<NkMouseLeaveEvent>())
					logger.Info("[MOUSE] leave");
				if (auto *mb = ev->As<NkMouseButtonPressEvent>()) {
					logger.Infof("[MOUSE] press btn=%s x=%d y=%d", NkMouseButtonToString(mb->GetButton()), mb->GetX(),
								 mb->GetY());
				}
				if (auto *mb = ev->As<NkMouseButtonReleaseEvent>()) {
					logger.Infof("[MOUSE] release btn=%s x=%d y=%d", NkMouseButtonToString(mb->GetButton()), mb->GetX(),
								 mb->GetY());
				}
			}
			if (auto *k = ev->As<NkKeyPressEvent>()) {
				...
				switch (k->GetKey()) {
					...
					case NkKey::NK_F8: {
						...
						if (next >= static_cast<uint32>(NkPadProfile::COUNT))
							next = 0u;
		...
		if (autoPreset) {
			for (uint32 i = 0; i < NK_MAX_GAMEPADS; ++i) {
				if (!NkGamepads().IsConnected(i) || presetApplied[i])
					continue;
				...
				logger.Infof("[Mapping] profile=%s applied on slot %u (%s)", ProfileName(resolved), i, info.name);
			}
		}

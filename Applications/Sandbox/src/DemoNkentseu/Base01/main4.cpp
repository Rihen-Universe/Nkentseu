...
static nkentseu::NkAppData gPatternPollingAppData{
	nkentseu::NkGraphicsApi::NK_GFX_API_SOFTWARE, false, false, "SandboxPatternPolling", "1.0.0", false};
...
namespace {
	...
	// ---------------------------------------------------------------------------
	static void DrawPlasma(NkRenderer &r, uint32 w, uint32 h, float t, NkVec2f ph, float sat) {
		if (!w || !h)
			return;
		...
		for (uint32 y = 0; y < h; y += blk) {
			...
			for (uint32 x = 0; x < w; x += blk) {
				...
				float mix = (NkSin((fx + ph.x) * 13.5f + t * 1.7f) + NkSin((fy + ph.y) * 11.f - t * 1.3f) +
	...
	static void DrawHud(NkRenderer &r, const AppState &s) {
		...
		// Indicateur Neon
		uint32 neonCol = s.neon ? NkRenderer::PackColor(0, 255, 140, 255) : NkRenderer::PackColor(80, 80, 80, 255);
		DrawFillRect(r, 4, 4, 14, 14, neonCol);

		// Indicateur fullscreen
		uint32 fsCol = s.fullscreen ? NkRenderer::PackColor(255, 200, 0, 255) : NkRenderer::PackColor(80, 80, 80, 255);
		...
		uint32 gpCol = gpOk ? NkRenderer::PackColor(50, 220, 50, 255) : NkRenderer::PackColor(80, 80, 80, 255);
...
int nkmain(const nkentseu::NkEntryState & /*state*/) {
	...
	while (s.running && window.IsOpen()) {
		...
		if (gp.IsConnected(0)) {
			...
			if (NkFabs(lx) > deadzone)
				s.phase.x += lx * 0.025f;
			if (NkFabs(ly) > deadzone)
				s.phase.y += ly * 0.025f;

			// Stick droit X : saturation
			if (NkFabs(rx) > deadzone)
				s.saturation = NkMax(0.1f, NkMin(3.f, s.saturation + rx * 0.01f));

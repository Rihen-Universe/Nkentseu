...
namespace {
	...
	static float SafeDt(float dt) {
		if (!math::NkIsFinite(dt) || dt <= 0.f || dt > 0.25f)
			return 1.f / 60.f;
		return dt;
	}
	...
	// Plasma visuel
	// ---------------------------------------------------------------------------
	static void DrawPlasma(NkRenderer &r, uint32 w, uint32 h, float t, float px, float py, float sat) {
		if (!w || !h)
			return;
		if (!math::NkIsFinite(t))
			t = 0.f;
		if (!math::NkIsFinite(px))
			px = 0.f;
		if (!math::NkIsFinite(py))
			py = 0.f;
		if (!math::NkIsFinite(sat))
			sat = 1.f;
		...
		for (uint32 y = 0; y < h; y += blk) {
			...
			for (uint32 x = 0; x < w; x += blk) {
				...
				float mix = (math::NkSin((fx + px) * 13.5f + t * 1.7f) + math::NkSin((fy + py) * 11.f - t * 1.3f) +
							 math::NkSin(rd * 24.f - t * 2.1f)) *
							0.333f;
				uint8 ri = static_cast<uint8>(
					ClampUnit((0.5f + 0.5f * math::NkSin(6.28f * (mix + 0.f)) - 0.5f) * sat + 0.5f) * 255);
				uint8 gi = static_cast<uint8>(
					ClampUnit((0.5f + 0.5f * math::NkSin(6.28f * (mix + 0.33f)) - 0.5f) * sat + 0.5f) * 255);
				uint8 bi = static_cast<uint8>(
					ClampUnit((0.5f + 0.5f * math::NkSin(6.28f * (mix + 0.66f)) - 0.5f) * sat + 0.5f) * 255);
...
int nkmain(const nkentseu::NkEntryState & /*state*/) {
	...
	es.AddEventCallback<NkWindowCloseEvent>([&state](NkWindowCloseEvent * /*ev*/) {
		logger.Info("[CB:WindowMgr] Close event");
		state.running = false;
	});

	es.AddEventCallback<NkWindowResizeEvent>([&state, &renderer](NkWindowResizeEvent *ev) {
	...
	es.AddEventCallback<NkWindowFocusGainedEvent>(
		[](NkWindowFocusGainedEvent * /*ev*/) { logger.Info("[CB:WindowMgr] Focus gained"); });

	es.AddEventCallback<NkWindowFocusLostEvent>(
		[](NkWindowFocusLostEvent * /*ev*/) { logger.Info("[CB:WindowMgr] Focus lost"); });

	es.AddEventCallback<NkWindowMoveEvent>(
		[](NkWindowMoveEvent *ev) { logger.Info("[CB:WindowMgr] Moved ({0},{1})", ev->GetX(), ev->GetY()); });

	// -- Sous-systAme "Input/Keyboard" ----------------------------------------

	es.AddEventCallback<NkKeyPressEvent>([&state, &window](NkKeyPressEvent *ev) {
		++state.keyCount;
		switch (ev->GetKey()) {
	...
	es.AddEventCallback<NkKeyReleaseEvent>(
		[](NkKeyReleaseEvent *ev) { logger.Info("[CB:Input] Key released key={0}", (int)ev->GetKey()); });

	es.AddEventCallback<NkTextInputEvent>(
		[](NkTextInputEvent *ev) { logger.Info("[CB:Input] Typed codepoint=U+{0}", ev->GetCodepoint()); });

	// -- Sous-systAme "Mouse" -------------------------------------------------

	es.AddEventCallback<NkMouseMoveEvent>([&state](NkMouseMoveEvent *ev) {
		++state.mouseCount;
	...
	es.AddEventCallback<NkMouseWheelVerticalEvent>([&state](NkMouseWheelVerticalEvent *ev) {
	...
	es.AddEventCallback<NkMouseButtonPressEvent>([](NkMouseButtonPressEvent *ev) {
		logger.Info("[CB:Mouse] Press btn={0} at ({1},{2})", (int)ev->GetButton(), ev->GetX(), ev->GetY());
	});

	es.AddEventCallback<NkMouseEnterEvent>([](NkMouseEnterEvent * /*ev*/) { logger.Info("[CB:Mouse] Enter window"); });

	es.AddEventCallback<NkMouseLeaveEvent>([](NkMouseLeaveEvent * /*ev*/) { logger.Info("[CB:Mouse] Leave window"); });

	// -- Sous-systAme "Drag & Drop" --------------------------------------------

	es.AddEventCallback<NkDropEnterEvent>([](NkDropEnterEvent *ev) {
		logger.Info("[CB:DnD] Enter pos=({0},{1}) files={2} text={3}", ev->data.x, ev->data.y, ev->data.numFiles,
					ev->data.hasText ? "yes" : "no");
	});

	es.AddEventCallback<NkDropLeaveEvent>([](NkDropLeaveEvent * /*ev*/) { logger.Info("[CB:DnD] Leave"); });

	es.AddEventCallback<NkDropFileEvent>([&state](NkDropFileEvent *ev) {
		++state.dropCount;
	...
	es.AddEventCallback<NkDropTextEvent>([&state](NkDropTextEvent *ev) {
		++state.dropCount;
	...
	NkGamepads().SetConnectCallback([](const NkGamepadInfo &info, bool connected) {
		logger.Info("[CB:Gamepad] {0} #{1} ({2})", connected ? "CONNECTED" : "DISCONNECTED", info.index, info.name);
	});

	// AvAnement de connexion (NkGamepadConnectEvent)
	es.AddEventCallback<NkGamepadConnectEvent>(
		[](NkGamepadConnectEvent *ev) { logger.Info("[CB:Gamepad] ConnectEvent idx={0}", ev->GetGamepadIndex()); });

	// AvAnement de dAconnexion (NkGamepadDisconnectEvent)
	es.AddEventCallback<NkGamepadDisconnectEvent>([](NkGamepadDisconnectEvent *ev) {
		logger.Info("[CB:Gamepad] DisconnectEvent idx={0}", ev->GetGamepadIndex());
	});

	es.AddEventCallback<NkGamepadButtonPressEvent>([&state](NkGamepadButtonPressEvent *ev) {
		++state.gpCount;
		logger.Info("[CB:Gamepad] Button={0} idx={1}", (int)ev->GetButton(), ev->GetGamepadIndex());
		if (ev->GetButton() == NkGamepadButton::NK_GP_SOUTH) {
			state.neon = !state.neon.Load();
			NkGamepads().Rumble(ev->GetGamepadIndex(), 0.25f, 0.5f, 0.f, 0.f, 80);
		}
	...
	es.AddEventCallback<NkGamepadAxisEvent>([&state](NkGamepadAxisEvent *ev) {
		float v = ev->GetValue();
		if (math::NkFabs(v) < 0.12f)
			return; // dead-zone
	...
	//     (Version simplifiAe : on utilise un compteur et on retire tous les callbacks de resize aprAs 3)
	{
		...
		es.AddEventCallback<NkWindowResizeEvent>([&es](NkWindowResizeEvent *ev) {
			++resizeCount;
			logger.Info("[CB:OneShotx3] Resize #{0} -> {1}x{2}", resizeCount, ev->GetWidth(), ev->GetHeight());
			if (resizeCount >= 3) {
	...
	while (state.running.Load() && window.IsOpen()) {
		...
		if (!state.running.Load() || !window.IsOpen())
			break;
		...
		DrawPlasma(renderer, w, h, state.time, state.phaseX, state.phaseY, state.saturation);
		renderer.EndFrame();
		renderer.Present();
	...
	logger.Info("[Stats] keys={0} mouse={1} drops={2} gamepad={3}", state.keyCount.Load(), state.mouseCount.Load(),

...
namespace {
	...
	uint32 PackWaveColor(float r, float g, float b) {
		return NkRenderer::PackColor(static_cast<uint8>(ClampUnit(r) * 255.f), static_cast<uint8>(ClampUnit(g) * 255.f),
									 static_cast<uint8>(ClampUnit(b) * 255.f), 255);
	}

	void DrawPlasma(NkRenderer &renderer, uint32 width, uint32 height, float t, const NkVec2f &phase, float sat) {
		if (!width || !height)
			return;
		...
		for (uint32 y = 0; y < height; y += blk) {
			...
			for (uint32 x = 0; x < width; x += blk) {
				...
				float mix = (NkSin((fx + phase.x) * 13.5f + t * 1.7f) + NkSin((fy + phase.y) * 11.0f - t * 1.3f) +
	...
	class GameLayer {
		public:
			...
			NkVec2f GetPhaseOffset() const {
				return mPhaseOffset;
			}

			float GetSaturation() const {
				return mSaturationBoost;
			}

			bool IsNeonMode() const {
				return mNeonMode;
			}

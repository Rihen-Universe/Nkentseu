...
class RotateBehaviour : public NkBehaviour {
	public:
		...
		void OnUpdate(float dt) override {
			...
			if (!go)
				return;
			go->Transform()->Rotate(NkQuatf::FromAxisAngle({0, 1, 0}, speed * dt * (3.14159f / 180.f)));
		}
...
class OrbitCameraBehaviour : public NkBehaviour {
	public:
		...
		void OnUpdate(float dt) override {
			...
			go->Transform()->SetLocalRotation(NkQuatf::LookAt({x, 2.f, z}, {0, 0, 0}, {0, 1, 0}));
		}

		const char *GetTypeName() const override {
			return "OrbitCameraBehaviour";
		}
};
...
class Hello3DApp : public NkApplication {
	public:
		explicit Hello3DApp(const NkApplicationConfig &cfg) : NkApplication(cfg) {
		}

...
struct PBRGameScene {
		...
		bool Init(NkIDevice *dev, uint32 w, uint32 h) {
			...
			NkTexId groundAlbedo = tex->CreateCheckerboard(512, 64, {0.3f, 0.3f, 0.3f, 1}, {0.4f, 0.4f, 0.4f, 1});
		...
		void Render(float32 dt, float32 time) {
			...
			r3d->SetSun({-0.3f, -1.f, -0.5f}, // direction
			...
			r3d->ClearLights();
			r3d->AddLight(NkLight::Directional({-0.3f, -1.f, -0.5f}, {1.f, 0.95f, 0.8f}, 5.f, true));
			// Points lights dynamiques
			float32 a = time * 0.5f;
			r3d->AddLight(NkLight::Point({sinf(a) * 3.f, 1.5f, cosf(a) * 3.f}, {1, 0.3f, 0.1f}, 4.f, 8.f));
			r3d->AddLight(
				NkLight::Point({sinf(a + 2.1f) * 3.f, 1.5f, cosf(a + 2.1f) * 3.f}, {0.1f, 0.5f, 1.f}, 4.f, 8.f));
			r3d->AddLight(
				NkLight::Point({sinf(a + 4.2f) * 3.f, 1.5f, cosf(a + 4.2f) * 3.f}, {0.2f, 1.f, 0.3f}, 4.f, 8.f));

			// ── Sol ───────────────────────────────────────────────────────────────────
			r3d->DrawMesh(meshGround, NkTransform3D::TRS({0, -0.5f, 0}, {0, 0, 0}, {20, 1, 20}), matGround);
			...
			for (int32 ix = -3; ix <= 3; ++ix) {
				for (int32 iz = -3; iz <= 3; ++iz) {
					...
					r3d->DrawMesh(meshSphere,
								  NkTransform3D::TRS({ix * 2.f, 0.f, iz * 2.f}, {0, 0, 0}, {0.8f, 0.8f, 0.8f}), mat);
				}
			}

			// ── Sphère de verre au centre ─────────────────────────────────────────────
			r3d->DrawMesh(meshSphere, NkTransform3D::TRS({0, 1.5f, 0}, {0, 0, 0}, {1.5f, 1.5f, 1.5f}), matGlass);
			...
			for (int32 i = 0; i < 3; ++i) {
				...
				r3d->DrawMesh(
					meshSphere,
					NkTransform3D::TRS({sinf(angle) * 3.f, 1.5f, cosf(angle) * 3.f}, {0, 0, 0}, {0.2f, 0.2f, 0.2f}),
					matEmissive);
			}
		...
		NkICommandBuffer *mCmd_placeholder() {

...
static NkIDevice *CreateDeviceWithBackendFallback(const NkDeviceInitInfo &info, NkGraphicsApi requestedApi,
	...
	if (explicitBackend && requestedApi != NkGraphicsApi::NK_GFX_API_NONE) {
		...
		logger.Warn("[Demo] Backend demande indisponible: {0} - fallback auto", NkGraphicsApiName(requestedApi));
	}

#if defined(NKENTSEU_PLATFORM_WINDOWS)
	return NkDeviceFactory::CreateWithFallback(info, {NkGraphicsApi::NK_GFX_API_OPENGL, NkGraphicsApi::NK_GFX_API_DX12,
													  NkGraphicsApi::NK_GFX_API_DX11, NkGraphicsApi::NK_GFX_API_VULKAN,
													  NkGraphicsApi::NK_GFX_API_SOFTWARE});
#elif defined(NKENTSEU_PLATFORM_MACOS)
	return NkDeviceFactory::CreateWithFallback(info,
											   {NkGraphicsApi::NK_GFX_API_METAL, NkGraphicsApi::NK_GFX_API_VULKAN,
												NkGraphicsApi::NK_GFX_API_OPENGL, NkGraphicsApi::NK_GFX_API_SOFTWARE});
#else
	return NkDeviceFactory::CreateWithFallback(
		info, {NkGraphicsApi::NK_GFX_API_VULKAN, NkGraphicsApi::NK_GFX_API_OPENGL, NkGraphicsApi::NK_GFX_API_SOFTWARE});
#endif
}
...
int nkmain(const NkEntryState &state) {
	...
	NkRenderer renderer;
	if (!renderer.Init(device, rendCfg)) {
	...
	if (!font24)
		font24 = renderer.Fonts().GetDefault(24);

	// ── Charger des textures ──────────────────────────────────────────────────
	auto &texMgr = renderer.Textures();
	NkTexture2D *texBrickAlbedo =
		texMgr.LoadTexture2D("Resources/Textures/PBR/gold/albedo.png", NkTextureParams::Albedo());
	NkTexture2D *texBrickNormal =
		texMgr.LoadTexture2D("Resources/Textures/PBR/gold/normal.png", NkTextureParams::Normal());
	NkTexture2D *texBrickORM = texMgr.LoadTexture2D("Resources/Textures/PBR/gold/ao.png", NkTextureParams::Linear());
	NkTexture2D *texWoodAlbedo =
		texMgr.LoadTexture2D("Resources/Textures/PBR/wall/albedo.png", NkTextureParams::Albedo());
	NkTexture2D *texWoodNormal =
		texMgr.LoadTexture2D("Resources/Textures/PBR/wall/normal.png", NkTextureParams::Normal());
	NkTexture2D *texGrassAlbedo =
		texMgr.LoadTexture2D("Resources/Textures/PBR/grass/albedo.png", NkTextureParams::Albedo());
	NkTexture2D *texLeafAlbedo =
		texMgr.LoadTexture2D("Resources/Textures/PBR/grass/albedo.png", NkTextureParams::Albedo());
	NkTexture2D *texSkinAlbedo =
		texMgr.LoadTexture2D("Resources/Textures/PBR/platic/albedo.png", NkTextureParams::Albedo());
	NkTexture2D *texSkinNormal =
		texMgr.LoadTexture2D("Resources/Textures/PBR/gold/normal.png", NkTextureParams::Normal());
	NkTexture2D *texWaterNormal =
		texMgr.LoadTexture2D("Resources/Textures/PBR/gold/normal.png", NkTextureParams::Normal());
	...
	NkMaterial *matBrick =
		renderer.CreatePBRMaterial("Brick", {1, 1, 1, 1}, 0.f, 0.5f, texBrickAlbedo, texBrickNormal, texBrickORM);
	matBrick->SetUVTiling(3.f, 3.f);

	// PBR bois (roughness élevée)
	NkMaterial *matWood =
		renderer.CreatePBRMaterial("Wood", {0.8f, 0.6f, 0.3f, 1}, 0.f, 0.7f, texWoodAlbedo, texWoodNormal);
	...
	NkMaterial *matGlass = renderer.CreateGlassMaterial("Glass", 1.52f, {0.9f, 0.95f, 1.f, 0.1f});

	// Émissif bleu (panneaux LED, etc.)
	NkMaterial *matEmissive = renderer.CreateEmissiveMaterial("GlowBlue", {0.2f, 0.5f, 1.f, 1.f}, 5.f);

	// Toon cartoon rouge
	NkMaterial *matToon = renderer.CreateToonMaterial("ToonRed", {0.9f, 0.2f, 0.1f, 1.f}, 4, true);

	// Peau (SSS)
	NkMaterial *matSkin = renderer.CreateSkinMaterial("Skin", texSkinAlbedo, texSkinNormal, {1.0f, 0.4f, 0.3f, 1.f});

	// Eau
	NkMaterial *matWater = renderer.CreateWaterMaterial("Water", texWaterNormal, {0.1f, 0.4f, 0.7f, 0.6f});

	// Cheveux
	NkMaterial *matHair = renderer.CreateHairMaterial("Hair", {0.2f, 0.1f, 0.05f, 1.f});

	// Voiture (clear coat)
	NkMaterial *matCar = renderer.CreateVehicleMaterial("CarPaint", {0.8f, 0.1f, 0.1f, 1.f}, nullptr, 1.f);

	// Feuillage
	NkMaterial *matFoliage = renderer.CreateFoliageMaterial("Leaves", texLeafAlbedo, nullptr, 0.4f);

	// Sol herbe (Principled)
	NkMaterial *matGrass = renderer.CreatePBRMaterial("Grass", {0.3f, 0.6f, 0.2f, 1}, 0.f, 0.9f, texGrassAlbedo);
	...
	if (meshCube)
		meshCube->BuildWireframe(device);
	if (meshSphere)
		meshSphere->BuildWireframe(device);
	...
	auto *titleMesh =
		renderer.TextRenderer().CreateExtruded3DText(device, font48, "NKRenderer", matEmissive, {0.05f, 0.005f, 1});
	if (titleMesh) {
	...
	const NkRenderMode renderModes[] = {
		NkRenderMode::NK_SOLID,		NkRenderMode::NK_WIREFRAME, NkRenderMode::NK_POINTS,
		NkRenderMode::NK_NORMALS,	NkRenderMode::NK_UV,		NkRenderMode::NK_ALBEDO,
		NkRenderMode::NK_ROUGHNESS, NkRenderMode::NK_METALLIC,	NkRenderMode::NK_AMBIENT_OCCLUSION,
	};
	const char *modeNames[] = {"Solid PBR", "Wireframe", "Points",	 "Normals", "UV",
							   "Albedo",	"Roughness", "Metallic", "AO"};
	int currentMode = 0;

	// ── Events ────────────────────────────────────────────────────────────────
	events.AddEventCallback<NkWindowCloseEvent>([&](NkWindowCloseEvent *) { app.running = false; });
	events.AddEventCallback<NkKeyPressEvent>([&](NkKeyPressEvent *e) {
		...
		if (e->GetKey() == NkKey::NK_G)
			app.showGizmos = !app.showGizmos;
		if (e->GetKey() == NkKey::NK_F5)
			renderer.CaptureFrame("capture.png");
		if (e->GetKey() == NkKey::NK_F1)
			renderer.SetupGameScene(scene);
		if (e->GetKey() == NkKey::NK_F2)
			renderer.SetupModelingScene(scene);
		if (e->GetKey() == NkKey::NK_F3)
			renderer.SetupFilmScene(scene);
		if (e->GetKey() == NkKey::NK_F4)
			renderer.SetupArchScene(scene);
	...
	while (app.running) {
		...
		if (app.keys[(uint32)NkKey::NK_S] || app.keys[(uint32)NkKey::NK_DOWN])
			app.camPitch -= camSpd * app.dt;
		if (app.keys[(uint32)NkKey::NK_Q])
			app.camDist -= zoomSpd * app.dt;
		if (app.keys[(uint32)NkKey::NK_E])
			app.camDist += zoomSpd * app.dt;

...
static void TestTriangleDataURI() {
	...
	if (data.vertices.Size() == 3) {
		...
		Check(ApproxEq(v0.pos.x, 0.f) && ApproxEq(v0.pos.y, 0.f) && ApproxEq(v0.pos.z, 0.f), "v0 == (0,0,0)");
		Check(ApproxEq(v1.pos.x, 1.f) && ApproxEq(v1.pos.y, 0.f) && ApproxEq(v1.pos.z, 0.f), "v1 == (1,0,0)");
		Check(ApproxEq(v2.pos.x, 0.f) && ApproxEq(v2.pos.y, 1.f) && ApproxEq(v2.pos.z, 0.f), "v2 == (0,1,0)");
		// NORMAL absente -> calculee par-face : (0,0,1) pour ce triangle CCW XY.
		Check(ApproxEq(v0.normal.z, 1.f) || ApproxEq(v0.normal.z, -1.f), "normale calculee (|z|=1) car NORMAL absente");
		// COLOR absente -> blanc.
		Check(v0.color == 0xFFFFFFFFu, "couleur defaut = blanc");
	}
	if (data.indices.Size() == 3) {
		Check(data.indices[0] == 0 && data.indices[1] == 1 && data.indices[2] == 2, "indices == [0,1,2]");
	}
	Check(ApproxEq(data.bounds.min.x, 0.f) && ApproxEq(data.bounds.min.y, 0.f) && ApproxEq(data.bounds.max.x, 1.f) &&
			  ApproxEq(data.bounds.max.y, 1.f),
		  "bounds = ([0,0,0]..[1,1,0])");
...
static void TestRealSample(const char *relPath, const char *label) {
	...
	// bounds non degenere
	bool boundsOk = data.bounds.max.x >= data.bounds.min.x && data.bounds.max.y >= data.bounds.min.y &&
	...
	std::printf("    -> %u vertices, %u indices, %u submeshes, bounds=[%.3f,%.3f,%.3f]..[%.3f,%.3f,%.3f]\n",
				(unsigned)data.vertices.Size(), (unsigned)data.indices.Size(), (unsigned)data.subMeshes.Size(),
				data.bounds.min.x, data.bounds.min.y, data.bounds.min.z, data.bounds.max.x, data.bounds.max.y,
				data.bounds.max.z);
}
...
static void TestTriangleGLB() {
	...
	for (int i = 0; i < 3; ++i) {
		uint8 b[2];
		std::memcpy(b, &idx[i], 2);
		bin.PushBack(b[0]);
		bin.PushBack(b[1]);
	}
	while ((bin.Size() & 3u) != 0)
		bin.PushBack(0); // pad chunk a 4 octets

	NkString json = "{\"asset\":{\"version\":\"2.0\"},"
					"\"buffers\":[{\"byteLength\":42}],"
					"\"bufferViews\":[{\"buffer\":0,\"byteOffset\":0,\"byteLength\":36,\"target\":34962},"
					"{\"buffer\":0,\"byteOffset\":36,\"byteLength\":6,\"target\":34963}],"
					"\"accessors\":[{\"bufferView\":0,\"componentType\":5126,\"count\":3,\"type\":\"VEC3\"},"
					"{\"bufferView\":1,\"componentType\":5123,\"count\":3,\"type\":\"SCALAR\"}],"
					"\"meshes\":[{\"name\":\"glbtri\",\"primitives\":[{\"attributes\":{\"POSITION\":0},\"indices\":1,"
					"\"mode\":4}]}]}";
	NkVector<uint8> jsonBytes;
	for (nk_size i = 0; i < json.Length(); ++i)
		jsonBytes.PushBack((uint8)json.CStr()[i]);
	while ((jsonBytes.Size() & 3u) != 0)
		jsonBytes.PushBack((uint8)' '); // pad espaces
	...
	for (nk_size i = 0; i < jsonBytes.Size(); ++i)
		glb.PushBack(jsonBytes[i]);
	PushU32LE(glb, (uint32)bin.Size());
	PushU32LE(glb, 0x004E4942u); // 'BIN\0'
	for (nk_size i = 0; i < bin.Size(); ++i)
		glb.PushBack(bin[i]);
	...
	if (ok) {
		...
		Check(ApproxEq(data.bounds.max.x, 2.f) && ApproxEq(data.bounds.max.y, 2.f), "bounds max == (2,2,0)");
	}
	NkFile::Delete(path.CStr());
}

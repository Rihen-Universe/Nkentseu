	...
	namespace demo {
		...
		bool DemoGLTF_Init(DemoCtx &ctx) {
			...
			if (lower.EndsWith(".gltf") || lower.EndsWith(".glb"))
				...
				loaded = LoadOBJ(path, data);
			else if (lower.EndsWith(".stl"))
				loaded = LoadSTL(path, data);
			else if (lower.EndsWith(".ply"))
				loaded = LoadPLY(path, data);
			else if (lower.EndsWith(".fbx"))
				loaded = LoadFBX(path, data);
			else if (lower.EndsWith(".dae"))
				loaded = LoadDAE(path, data);
			...
			logger.Info("[DemoGLTF] Init OK — '{0}' : {1} v / {2} i, {3} materiaux, "
						...
						path.CStr(), st->vertexCount, st->indexCount, st->matCount, st->center.x, st->center.y,
		...
		void DemoGLTF_Frame(DemoCtx &ctx, float32 dt) {
			...
			camData.position = {st->center.x + cosf(st->angle) * st->radius, st->center.y + st->radius * 0.45f,
			...
			fill.position = {st->center.x, st->center.y + st->radius, st->center.z + st->radius};
			...
			if (st->loaded) {
				...
				if (subCount > 0 && st->matCount > 0) {
					for (uint32 s = 0; s < subCount; ++s) {
						NkMatInstHandle h = st->materials.InstanceForMaterial(st->subMeshMaterial[s]);
			...
			if (auto *overlay = ctx.renderer->GetOverlay()) {
				...
				overlay->DrawText({20.f, 35.f}, "DemoGLTF  |  API : %s", NkGraphicsApiName(ctx.api));
				overlay->DrawText({20.f, 55.f}, "Model : %s", st->modelName.CStr());
				if (st->loaded) {
					overlay->DrawText({20.f, 75.f}, "verts: %u  indices: %u  materials: %u", st->vertexCount,
									  st->indexCount, st->matCount);
				} else {
					overlay->DrawText({20.f, 75.f}, "** CHARGEMENT ECHOUE **");
				}
				overlay->DrawText({20.f, 95.f}, "FPS : %.0f  dt: %.2f ms", dt > 1e-5f ? 1.f / dt : 0.f, dt * 1000.f);
				overlay->EndOverlay();
			}

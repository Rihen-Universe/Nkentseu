...
namespace nkentseu {
	namespace demo {
		...
		class DemoCamera {
			public:
				renderer::NkOrbitCameraController3D &Controller() {
					return mCtrl;
				}

				const renderer::NkOrbitCameraController3D &Controller() const {
					return mCtrl;
				}
				...
				void InstallEvents() {
					...
					NkEvents().AddEventCallback<NkKeyPressEvent>(
						[this](NkKeyPressEvent *e) { OnKey(e->GetKey(), true); });
					NkEvents().AddEventCallback<NkKeyReleaseEvent>(
						[this](NkKeyReleaseEvent *e) { OnKey(e->GetKey(), false); });
					NkEvents().AddEventCallback<NkMouseButtonPressEvent>([this](NkMouseButtonPressEvent *e) {
						if (e->IsLeft())
							mLeftDown = true;
						if (e->IsMiddle())
							mMiddleDown = true;
					...
					NkEvents().AddEventCallback<NkMouseButtonReleaseEvent>([this](NkMouseButtonReleaseEvent *e) {
						if (e->IsLeft())
							mLeftDown = false;
						if (e->IsMiddle())
							mMiddleDown = false;
					...
					NkEvents().AddEventCallback<NkMouseMoveEvent>(
						[this](NkMouseMoveEvent *e) { OnMouseMove(e->GetDeltaX(), e->GetDeltaY()); });
					NkEvents().AddEventCallback<NkMouseWheelVerticalEvent>(
						[this](NkMouseWheelVerticalEvent *e) { mCtrl.Zoom((float32)e->GetDeltaY()); });
				}
				...
				void SetKeyMoveSpeed(float32 v) {
					mKeyMoveSpeed = v;
				}
			...
			private:
				void OnKey(NkKey key, bool down) {
					switch (key) {
						...
						case NkKey::NK_T:
							if (down)
								mCtrl.SetAutoOrbit(!mCtrl.IsAutoOrbit());
							break;
						case NkKey::NK_HOME:
							if (down)
								mCtrl.Recenter();

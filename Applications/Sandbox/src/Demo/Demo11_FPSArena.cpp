	...
	namespace demo {
		...
		struct FPSCamera {
				...
				void Apply(NkCamera3D &cam) const {
					cam.SetPosition(pos);
					NkVec3f tgt = {pos.x + cosf(pitch) * cosf(yaw), pos.y + sinf(pitch),
		...
		static NkTexHandle CreateSingleTileTexture(NkTextureLibrary *texLib, NkVec3f tileColor, NkVec3f groutColor,
												   const char *dbgName) {
		...
		static NkTexHandle CreateFloorTileTexture(NkTextureLibrary *texLib) {
			return CreateSingleTileTexture(texLib, {0.92f, 0.62f, 0.18f}, // jaune-orange tile
		...
		static NkTexHandle CreateWallTileTexture(NkTextureLibrary *texLib) {
			return CreateSingleTileTexture(texLib, {0.92f, 0.92f, 0.90f}, // tile blanc casse
		...
		bool Demo11_FPSArena_Init(DemoCtx &ctx) {
			...
			if (st->matBarricade && st->matBarricade->IsValid()) {
				st->matBarricade
					->SetAlbedo({0.82f, 0.32f, 0.28f}) // rouge-saumon
					->SetMetallic(0.0f)
					->SetRoughness(0.75f);
			}
			...
			if (st->matBarrel && st->matBarrel->IsValid()) {
				st->matBarrel
					->SetAlbedo({0.28f, 0.55f, 0.85f}) // bleu plastique
					->SetMetallic(0.0f)
					->SetRoughness(0.55f);
			}
			st->matSphere = NkMaterial::Create(matSys, NkMaterialType::NK_PBR_METALLIC);
			if (st->matSphere && st->matSphere->IsValid()) {
				st->matSphere->SetAlbedo({0.75f, 0.75f, 0.75f})->SetMetallic(0.2f)->SetRoughness(0.45f);
			}
			...
			if (st->matText && st->matText->IsValid()) {
				st->matText
					->SetAlbedo({0.85f, 0.65f, 0.20f}) // dore
					->SetMetallic(0.7f)
					->SetRoughness(0.35f);
			}
			...
			auto SetKey = [state](NkKey k, bool down) {
				switch (k) {
					...
					case NkKey::NK_D:
						...
						break;
					case NkKey::NK_UP:
						state->cam.keyArrowU = down;
						break;
					case NkKey::NK_DOWN:
						state->cam.keyArrowD = down;
						break;
					case NkKey::NK_LEFT:
						state->cam.keyArrowL = down;
						break;
					case NkKey::NK_RIGHT:
						state->cam.keyArrowR = down;
						break;
					case NkKey::NK_SPACE:
						state->cam.keyUp = down;
						break;
					case NkKey::NK_LCTRL:
						state->cam.keyDown = down;
						break;
					case NkKey::NK_LSHIFT:
						state->cam.keySprint = down;
						break;
					default:
						break;
				}
		...
		void Demo11_FPSArena_Frame(DemoCtx &ctx, float32 dt) {
			...
			auto SubmitBoxM = [&](NkVec3f pos, NkVec3f sizeM, NkMaterial *m, bool castShadow = true) {
				NkDrawCall3D dc;
				dc.mesh = st->meshCube;
			...
			auto SubmitSphereM = [&](NkVec3f pos, float32 diamM, NkMaterial *m) {
				...
				dc.aabb = {{pos.x - r, pos.y - r, pos.z - r}, {pos.x + r, pos.y + r, pos.z + r}};
				dc.castShadow = true;
				if (m && m->IsValid())
					dc.material = m->GetInstHandle();
				r3d->Submit(dc);
			...
			auto SubmitCylinderM = [&](NkVec3f basePos, float32 diamM, float32 heightM, NkMaterial *m) {
				const float32 r = diamM * 0.5f;
			...
			auto SubmitStaircase = [&](NkVec3f basePos, NkVec3f dir, int nSteps, float32 stepDepthM,
									   float32 stepHeightM, float32 widthM, NkMaterial *m) {
			...
			SubmitStaircase({0.f, 0.f, 6.6f}, {0.f, 0.f, 1.f}, 3, 0.4f, 0.2f, 2.f, st->matWall);
			...
			SubmitText3D(st->textRihen, NkMat4f::Translate({-4.f, 2.5f, -14.7f}));
			...
			if (auto *overlay = ctx.renderer->GetOverlay()) {
				...
				overlay->DrawText({20.f, 35.f}, "Demo11 FPS Arena  |  API : %s  |  FPS : %.0f",
								  NkGraphicsApiName(ctx.api), dt > 1e-5f ? 1.f / dt : 0.f);
				overlay->DrawText({20.f, 55.f}, "Pos : %.1f %.1f %.1f  yaw=%.2f pitch=%.2f", st->cam.pos.x,
								  st->cam.pos.y, st->cam.pos.z, st->cam.yaw, st->cam.pitch);
				overlay->DrawText(
					{20.f, 75.f},
					"WASD/Arrows : move  Mouse : look  SPACE/CTRL : up/dn  SHIFT : sprint  TAB : toggle mouseLook");
				overlay->DrawText({20.f, 95.f}, "MouseLook=%s   F1 hide=[%s]   F2 clip=[%s]   F3 both=[%s]",
								  st->cam.mouseLook ? "ON" : "OFF", st->mouseHidden ? "X" : " ",
								  st->mouseClipped ? "X" : " ", (st->mouseHidden && st->mouseClipped) ? "X" : " ");
				overlay->EndOverlay();
			}

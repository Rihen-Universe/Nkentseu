...
namespace nkentseu {
	namespace demo {
		...
		void DemoIK_Frame(DemoCtx &ctx, float32 dt) {
			...
			NkVec3f target = {sinf(t * 1.3f) * reach * 0.85f, reach * 0.5f + cosf(t * 0.9f) * reach * 0.45f,
			...
			for (uint32 i = 0; i < cnt && i < st->pose.Size(); ++i)
				st->pose[i] = res[i];
			...
			if (auto *mesh = ctx.renderer->GetMeshSystem()) {
				...
				auto submitSphere = [&](NkVec3f p, float32 r, NkVec3f color, float32 rough, bool shadow) {
					NkDrawCall3D dc;
					dc.mesh = sphere;
				...
				for (uint32 i = 0; i < st->pose.Size(); ++i)
					submitSphere(P(i), (i == 0) ? 0.20f : 0.13f,
								 (i == 0) ? NkVec3f{0.2f, 1.f, 0.35f} : NkVec3f{0.25f, 0.75f, 1.f}, 0.4f, true);
				// Cible (rouge)

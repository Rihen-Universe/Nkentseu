...
namespace nkentseu { namespace demo {
    ...
    static gen::NkMesh GenerateShape() {
        ...
        if (vc) {
            ...
            for (uint32 i=0;i<vc;++i) for (int k=0;k<3;++k){ float p=m.positions[i*3+k]; if(p<mn[k])mn[k]=p; if(p>mx[k])mx[k]=p; }
            float c[3]={(mn[0]+mx[0])*.5f,(mn[1]+mx[1])*.5f,(mn[2]+mx[2])*.5f}, ext=0.f;
            for (int k=0;k<3;++k){ float e=mx[k]-mn[k]; if(e>ext)ext=e; } if(ext<1e-6f)ext=1e-6f;
            float sc=2.0f/ext;
            for (uint32 i=0;i<vc;++i){ m.positions[i*3+0]=(m.positions[i*3+0]-c[0])*sc; m.positions[i*3+1]=(m.positions[i*3+1]-c[1])*sc+1.2f; m.positions[i*3+2]=(m.positions[i*3+2]-c[2])*sc; }
        }
    ...
    bool DemoNKGen_Init(DemoCtx& ctx) {
        ...
        if (!sSinkAdded) {
            memory::NkSharedPtr<NkISink> fileSink(new NkFileSink(NkString("Build/nkgen_demo.log"), /*truncate*/ true));
        ...
        logger.Info("[DemoNKGen] Forme generee : {0} sommets, {1} triangles\n",
                    (uint64)m.VertexCount(), (uint64)m.TriangleCount());
        ...
        if (auto* r3d = ctx.renderer->GetRender3D()) r3d->SetInfiniteGridEnabled(true);

        // Cible offscreen (readback) pour capturer une image du rendu moteur -> PNG.
        renderer::NkOffscreenDesc od;
        od.width = ctx.width; od.height = ctx.height;
        od.hdr = false; od.colorFmt = NkGPUFormat::NK_RGBA8_UNORM;
        od.hasDepth = true; od.readable = true; od.readback = true;
        od.name = "NKGenCapture";
        st->rt = ctx.renderer->CreateOffscreen(od);

        logger.Info("[DemoNKGen] Init termine, mesh valide={0}, offscreen={1}\n",
                    st->mesh.IsValid() ? 1 : 0, (st->rt && st->rt->IsValid()) ? 1 : 0);
    ...
    void DemoNKGen_Frame(DemoCtx& ctx, float32 dt) {
        ...
        camData.up = {0.f,1.f,0.f}; camData.fovY = 60.f;
        ...
        sun.direction = {-0.4f,-1.f,-0.3f}; sun.color = {1.f,0.96f,0.9f}; sun.intensity = 3.f;
        sun.castShadow = true; sun.shadowStatic = true; sctx.lights.PushBack(sun);
        NkLightDesc fill; fill.type = NkLightType::NK_POINT; fill.position = {2.5f,2.f,1.5f};
        fill.color = {0.5f,0.6f,1.f}; fill.intensity = 4.f; fill.range = 12.f; sctx.lights.PushBack(fill);
        ...
        auto* meshSys = ctx.renderer->GetMeshSystem();
        {
            ...
            dc.aabb = {{-40,0,-40},{40,0,40}}; dc.castShadow = false;
            dc.tint = {0.12f,0.12f,0.13f}; dc.metallic = 0.f; dc.roughness = 0.92f;
            r3d->Submit(dc);
        }
        // Le maillage GÉNÉRÉ par l'IA — matériau type "terracotta" + rotation lente.
        {
            NkDrawCall3D dc; dc.mesh = st->mesh;
            dc.transform = NkMat4f::RotationY(NkAngle::FromRad(ctx.totalTime * 0.5f));
            dc.aabb = {{-1.2f,0.f,-1.2f},{1.2f,2.6f,1.2f}};
            dc.tint = {0.85f,0.45f,0.28f}; dc.metallic = 0.f; dc.roughness = 0.55f;
            dc.castShadow = true;
            r3d->Submit(dc);
        }
    ...
    void DemoNKGen_Shutdown(DemoCtx& ctx) {
        auto* st = (DemoNKGenState*)ctx.userData;
        if (st && st->rt) ctx.renderer->DestroyOffscreen(st->rt);
        delete st;

...
namespace nkentseu {
	namespace demo {
		...
		bool Demo6_HierarchicalMaterials_Init(DemoCtx &ctx) {
			...
			st->parent->SetAlbedo(kPalette6[0])->SetMetallic(st->parentMetallic)->SetRoughness(0.35f);
				...
				switch (e->GetKey()) {
					...
					case NkKey::NK_NUM7: {
						...
						if (idx >= 0 && idx < kPaletteSize6) {
							state->colorIdx = idx;
							if (state->parent)
								state->parent->SetAlbedo(kPalette6[idx]);
						}
						break;
					}
					...
					case NkKey::NK_M: {
						...
						if (state->parent)
							state->parent->SetMetallic(state->parentMetallic);
						break;
					}
					// R : reset override albedo sur childB (re-link au parent)
					case NkKey::NK_R: {
						if (state->childB)
							state->childB->ResetPBROverride(NK_PBR_O_ALBEDO);
		...
		void Demo6_HierarchicalMaterials_Frame(DemoCtx &ctx, float32 dt) {
			...
			auto SubmitSphere = [&](NkMaterial *mat, NkVec3f pos, float32 scale) {
				...
				dc.transform = NkMat4f::Translate(pos) * NkMat4f::Scale({scale, scale, scale});
			...
			if (auto *overlay = ctx.renderer->GetOverlay()) {
				...
				overlay->DrawText({20.f, 35.f}, "Demo6 M.4 Hierarchical  |  API : %s", NkGraphicsApiName(ctx.api));
				overlay->DrawText({20.f, 55.f}, "parent : albedo=#%d (%.2f, %.2f, %.2f)  metal=%.0f  rough=0.35",
								  st->colorIdx, col.x, col.y, col.z, st->parentMetallic);
				overlay->DrawText({20.f, 75.f}, "A (gauche)=herite tout  |  B (droite)=override albedo (bleu)");
				overlay->DrawText({20.f, 95.f}, "C (avant)=override metal+rough (plastique)  |  grand=enfant de A");
				overlay->DrawText({20.f, 115.f}, "1-7:couleur parent  M:metal  R:reset B  Y:reset C");
				overlay->DrawText({20.f, 135.f}, "FPS : %.0f", dt > 1e-5f ? 1.f / dt : 0.f);

	...
	namespace demo {
		...
		bool Demo2D_Init(DemoCtx &ctx) {
			...
			NkEvents().AddEventCallback<NkKeyPressEvent>([st](NkKeyPressEvent *e) {
				switch (e->GetKey()) {
					case NkKey::NK_NUM1:
						st->lightOn[0] = !st->lightOn[0];
						break;
					case NkKey::NK_NUM2:
						st->lightOn[1] = !st->lightOn[1];
						break;
					case NkKey::NK_NUM3:
						st->lightOn[2] = !st->lightOn[2];
						break;
					case NkKey::NK_NUM0:
						st->shadowsOn = !st->shadowsOn;
						break;
					default:
						break;
				}
		...
		void Demo2D_Frame(DemoCtx &ctx, float32 dt) {
			...
			if (st->spritePos.x < minX) {
				st->spritePos.x = minX;
				st->spriteVel.x = -st->spriteVel.x;
			}
			if (st->spritePos.x > maxX) {
				st->spritePos.x = maxX;
				st->spriteVel.x = -st->spriteVel.x;
			}
			if (st->spritePos.y < minY) {
				st->spritePos.y = minY;
				st->spriteVel.y = -st->spriteVel.y;
			}
			if (st->spritePos.y > maxY) {
				st->spritePos.y = maxY;
				st->spriteVel.y = -st->spriteVel.y;
			}
			...
			lights[0].position = {cx + cosf(st->lightTime * 0.7f) * orbitR, cy + sinf(st->lightTime * 0.7f) * orbitR};
			...
			r2d->SetLit(true);
			r2d->FillRect({0, 0, (float32)ctx.width, (float32)ctx.height}, {0.5f, 0.5f, 0.5f, 1.f});

			// Mur central blanc plus brillant (fond different de la grille)
			const float32 wallW = 700.f, wallH = 350.f;
			r2d->FillRect({cx - wallW * 0.5f, cy - wallH * 0.5f, wallW, wallH}, {1.f, 1.f, 1.f, 1.f});
			...
			for (int i = 0; i < 3; i++) {
				r2d->FillRect(
					{aabbs[i].min.x, aabbs[i].min.y, aabbs[i].max.x - aabbs[i].min.x, aabbs[i].max.y - aabbs[i].min.y},
					{1.f, 1.f, 1.f, 1.f});
			}
			...
			for (int i = 0; i < 3; i++) {
				NkVec4f col = st->lightOn[i] ? NkVec4f{lights[i].color.x, lights[i].color.y, lights[i].color.z, 1.f}
			...
			if (auto *overlay = ctx.renderer->GetOverlay()) {
				...
				overlay->DrawText({20.f, 55.f}, "FPS approx: %.1f  |  dt: %.2f ms", dt > 1e-4f ? 1.f / dt : 0.f,
								  dt * 1000.f);
				overlay->DrawText({20.f, 75.f}, "1/2/3: toggle R/G/B  |  0: shadows %s", st->shadowsOn ? "ON" : "OFF");
				overlay->DrawText({20.f, 95.f}, "lights: R=%s G=%s B=%s", st->lightOn[0] ? "ON" : "off",
								  st->lightOn[1] ? "ON" : "off", st->lightOn[2] ? "ON" : "off");
				overlay->EndOverlay();
			}

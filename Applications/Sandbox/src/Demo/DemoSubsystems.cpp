	...
	namespace demo {
		...
		void DemoSubsystems_Frame(DemoCtx &ctx, float32 dt) {
			...
			if (st->toggleTimer >= 2.0f) {
				...
				const char *targetName = target == NK_SS_RENDER3D		? "RENDER3D"
			...
			if (auto *r2d = ctx.renderer->GetRender2D()) {
				...
				NkVec4f bg = {0.10f + 0.10f * sinf(t * 0.7f), 0.12f + 0.10f * sinf(t * 0.5f + 1.f),
							  0.15f + 0.10f * sinf(t * 0.3f + 2.f), 1.f};
				r2d->FillRect({0, 0, (float32)ctx.width, (float32)ctx.height}, bg);

				// Petite barre d'etat en bas
				r2d->FillRect({0, (float32)ctx.height - 24, (float32)ctx.width, 24}, {0, 0, 0, 0.6f});

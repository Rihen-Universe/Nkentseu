	...
	namespace demo {
		...
		void Demo7_MaterialFunctions_Frame(DemoCtx &ctx, float32 dt) {
			...
			if (auto *overlay = ctx.renderer->GetOverlay()) {
				...
				overlay->DrawText({20.f, 35.f}, "Demo7 M.5 Material Functions  |  API : %s",
								  NkGraphicsApiName(ctx.api));
				overlay->DrawText({20.f, 55.f}, "Shader: m5demo.frag.vk.glsl  +  4 #include depuis Include/");
				overlay->DrawText(
					{20.f, 75.f},
					"Effets : HSV cycle (NkColor) | FBM (NkNoise) | Toon ramp (NkToonRamp) | Fresnel halo (NkFresnel)");
				overlay->DrawText({20.f, 95.f}, "halo intensity=%.1f  rimPower=%.2f  color#%d  tintMix=%.0f",
								  st->haloIntensity, st->rimPower, st->haloColorIdx, st->tintMix);
				overlay->DrawText({20.f, 115.f}, "I/K:intensity  U/J:rimPower  H:cycle color  1/2:HSV/tint");

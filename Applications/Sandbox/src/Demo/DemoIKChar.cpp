...
namespace nkentseu {
	namespace demo {
		...
		static void ComputeIKWorld(DemoIKCharState *st, const NkMat4f *res, uint32 resCount, NkVector<NkMat4f> &out) {
			const uint32 jc = (uint32)st->bind.Size();
			if ((uint32)out.Size() != jc)
				out.Resize(jc);
			...
			if (cn >= 1)
				out[(uint32)st->chain[0]] = st->bind[(uint32)st->chain[0]];
		...
		bool DemoIKChar_Init(DemoCtx &ctx) {
			...
			if (!EvaluateGLTFWorldJoints(data, -1, 0.f, st->bind, st->parentJoint) || st->bind.Empty()) {
			...
			for (uint32 j = 0; j < jc; ++j)
				if (!hasChild[j] && depth[j] > bestDepth) {
					bestDepth = depth[j];
					leaf = (int32)j;
				}
			if (leaf < 0)
			...
			NkVector<int32> upChain; // feuille -> racine
			{
				int32 cur = leaf;
				uint32 n = 0;
				while (cur >= 0 && n < maxLen) {
					upChain.PushBack(cur);
					cur = st->parentJoint[cur];
					++n;
				}
			}
			st->chain.Clear();
			for (uint32 i = upChain.Size(); i > 0; --i)
				st->chain.PushBack(upChain[i - 1]);
			...
			// La logique reste DANS la démo — le solveur IK (engine) ignore tout ça.
			{
				...
				NkEvents().AddEventCallback<NkMouseButtonPressEvent>([sp](NkMouseButtonPressEvent *e) {
					if (e && e->GetButton() == NkMouseButton::NK_MB_LEFT) {
						sp->dragging = true;
						sp->hasManual = true;
					}
				});
			...
			if (getenv("NK_IK_TEAR_DIAG") && st->meshLoaded && !data.skinnedVertices.Empty()) {
				...
				auto skinPos = [&](const NkVector<NkMat4f> &sm, uint32 vi) -> NkVec3f {
					...
					for (int b = 0; b < 4; b++) {
						...
						if (j >= 0 && j < (int)sm.Size() && w > 0.f) {
							for (int e = 0; e < 16; e++)
								m.data[e] += w * sm[(uint32)j].data[e];
							wsum += w;
						}
					}
					if (wsum < 1e-4f)
				...
				auto metric = [&](const NkVector<NkMat4f> &sm, const char *tag) {
					...
					for (uint32 i = 0; i + 2 < (uint32)D.indices.Size(); i += 3) {
						...
						for (int e = 0; e < 3; e++) {
							...
							const auto &ra = D.skinnedVertices[a].pos;
							const auto &rb = D.skinnedVertices[b].pos;
							float32 dl = sqrtf((pa.x - pb.x) * (pa.x - pb.x) + (pa.y - pb.y) * (pa.y - pb.y) +
											   (pa.z - pb.z) * (pa.z - pb.z));
							float32 bl = sqrtf((ra.x - rb.x) * (ra.x - rb.x) + (ra.y - rb.y) * (ra.y - rb.y) +
											   (ra.z - rb.z) * (ra.z - rb.z));
							if (bl > 1e-6f) {
								float32 r = dl / bl;
								if (r > maxR)
				...
				{
					NkVector<NkMat4f> bb;
					EvaluateGLTFPose(D, -1, 0.f, bb);
					metric(bb, "BIND   ");
				}
				// (B) ANIM glTF native (animIdx 0) a t=1.0s
				{
					NkVector<NkMat4f> ab;
					EvaluateGLTFPose(D, 0, 1.0f, ab);
					metric(ab, "ANIM   ");
				}
				...
				{
					NkVector<NkMat4f> wb;
					ComputeIKWorld(st, st->bind.Data(), jc, wb);
					...
					for (uint32 j = 0; j < jc; ++j) {
						...
						for (int r = 0; r < 4; r++)
							for (int c = 0; c < 4; c++) {
								float32 id = (r == c) ? 1.f : 0.f;
								float32 e = fabsf(p.mat[c][r] - id);
								if (e > invErr)
									invErr = e;
							}
					}
					logger.Info("[DemoIKChar][TEAR] FK-recon : topo.Size={0}/{1}  maxPosErr={2} (joint {3}, "
								"parent={4})  invErr={5}\n",
								(uint32)st->topo.Size(), jc, maxErr, worst, st->parentJoint[worst], invErr);
					NkVector<NkMat4f> ib;
					ib.Resize(jc);
					for (uint32 j = 0; j < jc; ++j) {
						NkMat4f m = (j < (uint32)D.inverseBind.Size()) ? D.inverseBind[j] : NkMat4f::Identity();
						ib[j] = wb[j] * m;
					}
					metric(ib, "FK-BIND ");
				}
				...
				if (st->ready) {
					NkVec3f tgt = {st->anchor.x + st->reach * 0.55f, st->anchor.y - st->reach * 0.35f,
								   st->anchor.z + st->reach * 0.4f};
			...
			if (getenv("NK_IK_TEAR_DIAG"))
				for (uint32 i = 0; i < (uint32)st->chain.Size(); ++i)
					logger.Info("[DemoIKChar][CHAIN] os {0} : joint={1} parent={2}\n", i, (uint32)st->chain[i],
								st->parentJoint[(uint32)st->chain[i]]);

			logger.Info("[DemoIKChar] '{0}' : {1} joints, membre IK = {2} os (feuille={3}, "
						"reach={4}m), mesh={5}, center=({6},{7},{8}) r={9}\n",
						path.CStr(), jc, (uint32)st->chain.Size(), leaf, st->reach, st->meshLoaded ? 1 : 0,
		...
		void DemoIKChar_Frame(DemoCtx &ctx, float32 dt) {
			...
			if (!st->dragging)
				st->camAngle += dt * 0.25f;
			const float32 orbit = st->camAngle;
			NkCamera3DData camData;
			camData.position = {st->center.x + sinf(orbit) * st->radius, st->center.y + st->radius * 0.10f,
			...
			NkVec3f animTarget = {st->anchor.x + sinf(t * 1.1f) * st->reach * 0.9f,
					...
					if (dl > 1e-6f && nl > 1e-6f) {
						...
						if (fabsf(denom) > 1e-5f) {
							...
							if (tt > 0.f) {
								...
								if (rd > mx && rd > 1e-6f) {
									float32 s = mx / rd;
									hit = {st->anchor.x + rel.x * s, st->anchor.y + rel.y * s,
										   st->anchor.z + rel.z * s};
								}
								st->manualTarget = hit;
							}
			...
			if (!meshSubmitted) {
				for (uint32 j = 0; j < jc; ++j) {
					...
					r3d->DrawDebugSphere(
						WP(j), jr, st->inChain[j] ? NkVec4f{1.f, 0.8f, 0.2f, 1.f} : NkVec4f{0.4f, 0.7f, 1.f, 1.f});
				}
			} else {
				for (uint32 i = 0; i + 1 < (uint32)st->chain.Size(); ++i)
					r3d->DrawDebugLine(WP((uint32)st->chain[i]), WP((uint32)st->chain[i + 1]),
									   {1.f, 0.55f, 0.10f, 1.f});
			}
			if (!st->chain.Empty())
			...
			if (auto *overlay = ctx.renderer->GetOverlay()) {
				...
				overlay->DrawText({20.f, 35.f}, "DemoIKChar (NkAnima M0 d/d-bis)  |  API : %s",

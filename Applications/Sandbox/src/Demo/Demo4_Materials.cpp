...
namespace nkentseu {
	namespace demo {
		...
		bool Demo4_Materials_Init(DemoCtx &ctx) {
			...
			// L'instance retournee remplace st->mats[0] (sphere #1 "or") au draw.
			{
				...
				if (matLib && matLib->IsValid()) {
					...
					if (st->goldFromAsset.IsValid()) {
						logger.Info("[Demo4][PhaseG] Loaded '/Materials/Gold' OK -> sphere #1 will use asset-loaded "
									"material\n");
					} else {
			...
			logger.Info("[Demo4] Camera : LEFT-drag rotate | wheel zoom | WASD/fleches pan | T:auto-orbit | F:FPS | "
						"HOME:reset\n");
			return true;
		}
		...
		void Demo4_Materials_Frame(DemoCtx &ctx, float32 dt) {
			...
			if (auto *overlay = ctx.renderer->GetOverlay()) {
				...
				overlay->DrawText({20.f, 35.f}, "Demo Materials  |  API : %s  |  actif : %d (%s)",

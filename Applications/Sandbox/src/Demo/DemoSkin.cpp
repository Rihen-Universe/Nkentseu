...
namespace nkentseu {
	namespace demo {
		...
		bool DemoSkin_Init(DemoCtx &ctx) {
			...
			// multi-submesh multi-materiau par sous-maillage = amelioration future).
			{
				...
				if (matSys && texLib) {
					...
					logger.Info(
						"[DemoSkin] materiaux glTF : {0} instances, {1} textures, 1er submesh matIdx={2} valid={3}\n",
						(uint32)st->matSet.instances.Size(), (uint32)st->matSet.textures.Size(), matIdx,
						st->skinMat.IsValid() ? 1 : 0);
				...
				for (uint32 vi = 0; vi < (uint32)data.skinnedVertices.Size(); ++vi) {
					...
					for (int b = 0; b < 4; b++) {
						...
						if (j >= 0 && j < (int)st->bones.Size() && w > 0.f) {
							for (int e = 0; e < 16; e++)
								m.data[e] += w * st->bones[(uint32)j].data[e];
							wsum += w;
						}
					}
					if (wsum < 1e-4f)
			...
			logger.Info("[DemoSkin] Init OK — '{0}' : skinned={1} {2}v/{3}i joints={4} anims={5} "
						...
						path.CStr(), st->skinned ? 1 : 0, st->vertexCount, st->indexCount, st->jointCount,
						st->animCount, (uint32)data.skinnedVertices.Size(), st->center.x, st->center.y, st->center.z,
						st->radius);
			return true;
		}
		...
		void DemoSkin_Frame(DemoCtx &ctx, float32 dt) {
			...
			static int orbit = -1;
			if (orbit == -1) {
				const char *v = getenv("NK_ORBIT");
				orbit = (v && v[0] && v[0] != '0') ? 1 : 0;
			}
			...
			camData.position = {st->center.x + sinf(angle) * st->radius, st->center.y + st->radius * 0.15f,
			...
			if (getenv("NK_NOMODEL")) { /* sol seul */
				...
				if (!st->bones.Empty()) {
					...
					dc.tint = st->skinMat.IsValid() ? NkVec3f{1.f, 1.f, 1.f} : NkVec3f{0.85f, 0.55f, 0.45f};
					dc.alpha = 1.f;
					dc.aabb = st->bounds;
					dc.castShadow = true;
			...
			if (auto *overlay = ctx.renderer->GetOverlay()) {
				...
				if (st->loaded) {
					overlay->DrawText({20.f, 75.f}, "skinned:%d verts:%u idx:%u joints:%u anims:%u bones:%u",
									  st->skinned ? 1 : 0, st->vertexCount, st->indexCount, st->jointCount,
									  st->animCount, (uint32)st->bones.Size());
					overlay->DrawText({20.f, 95.f}, "animTime: %.2fs  submitted:%d", st->animTime, submitted ? 1 : 0);
				} else {

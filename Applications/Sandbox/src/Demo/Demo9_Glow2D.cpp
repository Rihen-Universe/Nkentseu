	...
	namespace demo {
		...
		void Demo9_Glow2D_Frame(DemoCtx &ctx, float32 dt) {
			...
			if (auto *overlay = ctx.renderer->GetOverlay()) {
				...
				overlay->DrawText({20.f, 35.f}, "Demo9 Phase E Materials 2D (v0 Glow2D)  |  API : %s",
								  NkGraphicsApiName(ctx.api));
				overlay->DrawText({20.f, 55.f},
								  "Gauche : sprite normal  |  Centre : glow rouge  |  Droite : glow color #%d",
								  st->colorIdx);
				overlay->DrawText({20.f, 75.f}, "intensity=%.2f  power=%.2f", st->intensity, st->power);
				overlay->DrawText({20.f, 95.f}, "I/K intensity  U/J power  H cycle color");

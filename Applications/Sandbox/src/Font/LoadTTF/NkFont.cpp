...
namespace nkentseu {
	...
	void NkPrintTableDirectory(NkDirectoryTable *tblDir, int32 tblSize) {
		...
		for (int32 i = 0; i < tblSize; ++i) {
			...
			// logger.Info("{0} \t {1}.{2}.{3}.{4} \t\t {5} \t {6}", i + 1, (char)t->tagC[3], (char)t->tagC[2],
			// (char)t->tagC[1], (char)t->tagC[0], t->length, t->decalage);
			logger.Infof("%d \t %c%c%c%c \t %d \t %d", i + 1, (char)t->tagC[3], (char)t->tagC[2], (char)t->tagC[1],
						 (char)t->tagC[0], t->length, t->decalage);
		}
	}

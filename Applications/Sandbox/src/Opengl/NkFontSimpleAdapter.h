	...
	class NkFontSimpleAdapter {
		public:
			...
			void GetAtlasDimensions(uint32_t &outWidth, uint32_t &outHeight) const {
				outWidth = mLib->Atlas().PageCount() > 0 ? mLib->Atlas().Page(0).width() : 0u;
				outHeight = mLib->Atlas().PageCount() > 0 ? mLib->Atlas().Page(0).height() : 0u;
			}
		...
		private:
			...
			bool CacheGlyphInfo() {
				...
				for (nk_codepoint cp = 32; cp <= 126; ++cp) {
					const NkCachedGlyph *cachedGlyph = nullptr;
					if (mLib->GetGlyph(mFontId, cp, mPointSize, mDpi, params, cachedGlyph).IsOk() && cachedGlyph &&
						cachedGlyph->isValid && cachedGlyph->isInAtlas) {

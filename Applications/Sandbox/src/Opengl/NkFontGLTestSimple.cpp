...
int main() {
	...
	if (info) {
		logger->Info("[TestSimple] Font '{} {}' loaded with {} glyphs", info->familyName, info->styleName,
					 info->numGlyphs);
	}
	...
	logger->Info("[TestSimple] Atlas: {} pages, {} glyphes", fontLib.Atlas().PageCount(), fontLib.Atlas().GlyphCount());
	...
	GLuint atlasTex = 0;
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, page.width(), page.height(), 0, GL_RED, GL_UNSIGNED_BYTE, page.data());

		glBindTexture(GL_TEXTURE_2D, 0);
		logger->Info("[TestSimple] GL Texture {} created {}x{}", atlasTex, page.width(), page.height());
	}
	...
	while (running && frameCount < 300) { // Run 5 seconds at 60 FPS
		...
		for (const char *p = text; *p; ++p) {
			...
			logger->Debug("[Glyph] Found cp={} uv=({:.3f},{:.3f})-({:.3f},{:.3f}) bearing=({},{}) dims={}x{}", cp,
						  gl->uvX0, gl->uvY0, gl->uvX1, gl->uvY1, gl->bearingX, gl->bearingY, gl->width, gl->height);
		...
		logger->Info("[Frame] Built {} vertices from '{}' ({} chars)", vertices.size(), text, strlen(text));

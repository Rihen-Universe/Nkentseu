...
class NkGLTextCtx {
	public:
		...
		static nk_handle uploadCallback(nk_uint32 pageIdx, const nk_uint8 *pixels, nk_size byteSize, nk_uint32 w,
										nk_uint32 h, nk_uint32 ch, nk_handle existingHandle, void *userPtr) {
			...
			GLenum intFmt = (ch == 1u) ? GL_R8 : (ch == 3u) ? GL_RGB8 : GL_RGBA8;
			GLenum extFmt = (ch == 1u) ? GL_RED : (ch == 3u) ? GL_RGB : GL_RGBA;
			...
			if (ctx->mAtlasTex == 0) {
				...
				glTexImage2D(GL_TEXTURE_2D, 0, intFmt, (GLsizei)w, (GLsizei)h, 0, extFmt, GL_UNSIGNED_BYTE, pixels);
				...
				glBindTexture(GL_TEXTURE_2D, ctx->mAtlasTex);
				glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
				glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, (GLsizei)w, (GLsizei)h, extFmt, GL_UNSIGNED_BYTE, pixels);
			...
			for (nk_size i = 0u; i < byteSize; ++i)
				if (pixels[i])
					++nonZero;
			logger.Info("[NkGLTest] Pixels non-nuls dans la page {0}: {1} / {2} ({3:.1f}%)\n", pageIdx, nonZero,
						byteSize, byteSize ? 100.f * nonZero / byteSize : 0.f);
		...
		bool init(const char *fontPath) {
			...
			// ③ Charge la fonte
			NkFontResult fr = mLib.LoadFromFile(fontPath, NK_LOAD_OUTLINES | NK_LOAD_KERNING, mFontId);
			if (!fr.IsOk()) {
		...
		void drawText2D(const char *text, float x, float y, float sw, float sh, float r = 1.f, float g = 1.f,
						float b = 1.f, float a = 1.f, float ptSize = 0.f) {
			if (!text || sw <= 0.f || sh <= 0.f) {
				if (!text)
					logger.Info("[DrawText2D] EARLY EXIT: text=NULL");
				else if (sw <= 0.f)
					logger.Info("[DrawText2D] EARLY EXIT: sw={0}", sw);
				else
					logger.Info("[DrawText2D] EARLY EXIT: sh={0}", sh);
				return;
			}
			if (ptSize <= 0.f)
				ptSize = size2D;
			...
			logger.Debug("[DrawText2D] ShapeUtf8 OK for '{0}': {1} lines", text, mShape.lineCount);
			...
			for (nk_uint32 l = 0u; l < mShape.lineCount; ++l) {
				...
				nk_uint32 skipped_ws = 0, skipped_result = 0, skipped_null = 0, skipped_invalid = 0,
						  skipped_notinatlasу = 0, added = 0;
				...
				logger.Info(
					"[DrawText2D] Line {0} summary: added={1} ws={2} result={3} null={4} invalid={5} notatlas={6}", l,
					added, skipped_ws, skipped_result, skipped_null, skipped_invalid, skipped_notinatlasу);
			}
		}

		// ─── drawText3D : espace local → espace monde via worldMat ───────────────

		void drawText3D(const char *text, const NkMat4f &worldMat, float r = 1.f, float g = 1.f, float b = 1.f,
						float a = 1.f, float ptSize = 0.f, bool centered = true) {
			if (!text)
				return;
			if (ptSize <= 0.f)
				ptSize = size3D;
			...
			mShape.Clear();
			if (!mShaper.ShapeUtf8(text, (nk_size)-1, sp, mShape).IsOk())
				return;
			...
			for (nk_uint32 l = 0u; l < mShape.lineCount; ++l) {
				...
				for (nk_uint32 gi = 0u; gi < run.glyphCount; ++gi) {
					...
					const NkCachedGlyph *gl = nullptr;
					if (!mLib.GetGlyph(mFontId, sg.codepoint, ptSize, 96.f, rp, gl).IsOk() || !gl || !gl->isValid ||
						!gl->isInAtlas)
						continue;
		...
		void shutdown() {
			if (mVao2D) {
				glDeleteVertexArrays(1, &mVao2D);
				mVao2D = 0;
			}
			if (mVbo2D) {
				glDeleteBuffers(1, &mVbo2D);
				mVbo2D = 0;
			}
			if (mVao3D) {
			...
			if (mProg2D) {
				glDeleteProgram(mProg2D);
				mProg2D = 0;
			}
			if (mProg3D) {
				glDeleteProgram(mProg3D);
				mProg3D = 0;
			}
			if (mAtlasTex) {
				glDeleteTextures(1, &mAtlasTex);
				mAtlasTex = 0;
			}
			mLib.Shutdown();
		}
...
int nkmain(const NkEntryState &state) {
	...
	while (running) {
		...
		NkMat4f view = NkMat4f::LookAt(NkVec3f(0.f, 1.5f, 4.f), NkVec3f(0, 0, 0), NkVec3f(0, 1, 0));
		NkMat4f proj = NkMat4f::Perspective(NkAngle(60.f), (float)W / (float)H, 0.1f, 100.f);
		...
		if (ctx->BeginFrame()) {
			...
			// "NKENTSEU" — flotte et tourne
			{
				...
				NkMat4f w = NkMat4f::Translation(NkVec3f(0.f, y, 0.f)) * NkMat4f::RotationY(NkAngle(rotAngle));
				gltctx->drawText3D("NKENTSEU", w, 1.f, 0.85f, 0.2f, 1.f, 64.f, true);
			}

			// "GL Test" au sol (plan horizontal)
			{
				NkMat4f w = NkMat4f::Translation(NkVec3f(0.f, -0.3f, 0.3f)) * NkMat4f::RotationX(NkAngle(-90.f));
			...
			{
				char buf[256];
				snprintf(buf, sizeof(buf), "FPS: %.0f  Atlas: %u page(s)  Glyphes: %u", fpsDisp,
						 gltctx->mLib.Atlas().PageCount(), gltctx->mLib.Atlas().GlyphCount());
				gltctx->drawText2D(buf, 12.f, 12.f, (float)W, (float)H, 1.f, 1.f, 0.3f, 1.f, 16.f);
			}

			gltctx->drawText2D("NKFont from-scratch — sans FreeType, sans NKRHI", 12.f, 34.f, (float)W, (float)H, 0.6f,
							   0.9f, 1.f, 1.f, 14.f);

			// Lignes de validation caractères
			gltctx->drawText2D("ABCDEFGHIJKLMNOPQRSTUVWXYZ", 12.f, 60.f, (float)W, (float)H, 1.f, 1.f, 1.f, 1.f, 18.f);
			gltctx->drawText2D("abcdefghijklmnopqrstuvwxyz", 12.f, 84.f, (float)W, (float)H, 0.9f, 0.9f, 0.9f, 1.f,
							   18.f);
			gltctx->drawText2D("0123456789 !@#$%^&*()", 12.f, 108.f, (float)W, (float)H, 0.8f, 0.8f, 1.f, 1.f, 18.f);

			// Texte coloré
			gltctx->drawText2D("Bonjour le monde!", 12.f, 140.f, (float)W, (float)H, 1.f, 0.5f, 0.5f, 1.f, 22.f);
			gltctx->drawText2D("Hello World!", 12.f, 168.f, (float)W, (float)H, 0.5f, 1.f, 0.5f, 1.f, 22.f);
			gltctx->drawText2D("NKFont rocks!", 12.f, 196.f, (float)W, (float)H, 0.5f, 0.5f, 1.f, 1.f, 22.f);

			// Instructions
			gltctx->drawText2D("D = dump atlas PGM  |  ESC = quitter", 12.f, (float)H - 28.f, (float)W, (float)H, 0.5f,
							   0.5f, 0.5f, 0.8f, 13.f);

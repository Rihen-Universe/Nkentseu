...
namespace mou {
	...
	namespace {
		const char *kBasketFile[CouleursGame::NUM_COLORS] = {"basket_red.svg", "basket_yellow.svg", "basket_green.svg",
															 "basket_blue.svg", "basket_orange.svg"};
		const char *kFruitFile[CouleursGame::NUM_COLORS] = {"fruit_tomate.svg", "fruit_mangue.svg", "fruit_avocat.svg",
															"fruit_safou.svg", "fruit_papaye.svg"};
		const char *kFruitName[CouleursGame::NUM_COLORS] = {"Tomate", "Mangue", "Avocat", "Safou", "Papaye"};
		const char *kFruitVoice[CouleursGame::NUM_COLORS] = {"fruit_tomate", "fruit_mangue", "fruit_avocat",
															 "fruit_safou", "fruit_papaye"};
	...
	void CouleursGame::StartLevel() noexcept {
		...
		if (mActiveColors > NUM_COLORS)
			mActiveColors = NUM_COLORS;
		if (mActiveColors < 1)
			mActiveColors = 1;
		mFruitCount = mLevels.GetInt(mLevel, "fruits", 3 + mLevel);
		if (mFruitCount > MAX_FRUITS)
			mFruitCount = MAX_FRUITS;
		if (mFruitCount < 1)
			mFruitCount = 1;
		...
		for (int32 c = mActiveColors - 1; c > 0; --c) {
			...
			const int32 t = mPlantColor[c];
			mPlantColor[c] = mPlantColor[j];
			mPlantColor[j] = t;
		}
		mSortedCount = 0;
		mNeedSpawn = true;
	...
	void CouleursGame::Update(float32 dt) noexcept {
		mTime += dt;
		mFb.Update(dt);
		if (mFb.ReadyToAdvance()) {
			mLevel = (mLevel + 1) % LevelCount();
			StartLevel();
		} else if (mFb.ReadyToRetry()) {
			StartLevel();
		} // rejoue le même niveau
	}
	...
	void CouleursGame::Render(const MouFrame &frame) noexcept {
		...
		for (int32 p = 0; p < nP; ++p) {
			...
			MouPlant::Draw(frame, bush ? mBushTex : mTreeTex, bush ? MouPlant::BUSH_AR : MouPlant::TREE_AR, px,
						   plantTop, plantW, plantH, prx[p], pry[p], prw[p], prh[p]);
		}
		if (mNeedSpawn && W > 1.f) {
			for (int32 p = 0; p < nP; ++p) {
				const int32 col = mPlantColor[p];
				const bool bush = (col == 0);
				int32 idxs[MAX_FRUITS];
				int32 cnt = 0;
				for (int32 i = 0; i < mFruitCount; ++i)
					if (mFruits[i].color == col)
						idxs[cnt++] = i;
				float32 sx[MAX_FRUITS], sy[MAX_FRUITS];
				MouPlant::Slots(prx[p], pry[p], prw[p], prh[p], bush ? 0.46f : 0.34f, 0.58f, bush ? 0.34f : 0.42f, cnt,
								fruitSz, sx, sy);
				for (int32 k = 0; k < cnt; ++k) {
					mFruits[idxs[k]].homeX = sx[k];
					mFruits[idxs[k]].homeY = sy[k];
					mFruits[idxs[k]].x = sx[k];
					mFruits[idxs[k]].y = sy[k];
				}
			}
			mNeedSpawn = false;
		}
		...
		frame.TextCentered(frame.titleFont, 0.f, W, 28.f, "Cueille les fruits et range-les !", C::TEXT_DARK());
		...
		for (int32 c = 0; c < mActiveColors; ++c) {
			...
			if (mBasketTex[c])
				frame.Image(mBasketTex[c], basketX[c], basketY, basketSz, basketSz);
			else {
				frame.Rect(basketX[c], basketY, basketSz, basketSz, ColorVal(c), 18.f);
			}
		}
		...
		if (mDragging >= 0 && (frame.pointerReleased || !playing)) {
			...
			int32 matched = -1;
			for (int32 c = 0; c < mActiveColors; ++c)
				if (InRect(cx, cy, basketX[c], basketY, basketSz, basketSz)) {
					matched = c;
					break;
				}
			if (matched == f.color) {
				...
				if (mSortedCount >= mFruitCount)
					mFb.Win(W); // mission réussie
			} else {
				if (matched >= 0)
					mFb.Bad(cx, cy); // mauvais panier -> perd une étoile
				f.x = f.homeX;
				f.y = f.homeY; // le fruit retourne sur sa branche
			}
			mDragging = -1;
		}
		...
		for (int32 i = 0; i < mFruitCount; ++i) {
			if (mFruits[i].sorted || i == mDragging)
				continue;
		...
		if (frame.Button(24.f, H - bh - 24.f, bw, bh, "Retour", C::CORAL(), C::ORANGE(), C::TEXT_ON_COLOR(), &border,
						 frame.titleFont)) {
			mWantExit = true;
		}
	}

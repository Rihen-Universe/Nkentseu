...
namespace mou {
	...
	namespace {
		const char *kFaceFile[MemoireGame::NUM_FACES] = {"fruit_mangue.svg", "fruit_tomate.svg", "fruit_avocat.svg",
	...
	bool MemoireGame::Init() noexcept {
		mWantExit = false;
		if (mAssets) {
			for (int32 f = 0; f < NUM_FACES; ++f)
				mFaceTex[f] = mAssets->LoadSvg(kFaceFile[f], 200, 200);
	...
	void MemoireGame::StartLevel() noexcept {
		...
		for (int32 p = 0; p < mPairs; ++p) {
			mCards[2 * p].face = order[p];
			mCards[2 * p].up = false;
			mCards[2 * p].matched = false;
			mCards[2 * p + 1].face = order[p];
			mCards[2 * p + 1].up = false;
			mCards[2 * p + 1].matched = false;
		}
		// Mélange du plateau (Fisher-Yates).
		for (int32 i = mCardCount - 1; i > 0; --i) {
			const int32 j = (int32)(Rand01() * (float32)(i + 1));
			const Card t = mCards[i];
			mCards[i] = mCards[j];
			mCards[j] = t;
		}
	...
	void MemoireGame::Update(float32 dt) noexcept {
		...
		if (mHideTimer > 0.f) {
			...
			if (mHideTimer <= 0.f) {
				if (mFlipA >= 0)
					mCards[mFlipA].up = false;
				if (mFlipB >= 0)
					mCards[mFlipB].up = false;
				mFlipA = mFlipB = -1;
			}
		}
	...
	void MemoireGame::Render(const MouFrame &frame) noexcept {
		...
		for (int32 i = 0; i < mCardCount; ++i) {
			...
			if (card.matched) {
				...
				if (mFaceTex[card.face])
					frame.Image(mFaceTex[card.face], x + (cell - is) * 0.5f, y + (cell - is) * 0.5f, is, is);
			} else if (card.up) {
				...
				frame.Border(x, y, cell, cell, C::INK(), 5.f, 18.f);
				frame.TextCentered(frame.titleFont, x, cell, y + (cell - frame.LineH(frame.titleFont)) * 0.5f, "?",
								   C::TEXT_ON_COLOR());
			}
			...
			if (canFlip && over && frame.pointerReleased && !card.up && !card.matched && mFlipB < 0) {
				...
				if (mFlipA < 0) {
					...
					if (mCards[mFlipA].face == mCards[mFlipB].face) {
						mCards[mFlipA].matched = true;
						mCards[mFlipB].matched = true;

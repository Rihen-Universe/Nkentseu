...
namespace mou {
	...
	namespace {
		const char *kObjFile[CalculGame::NUM_TYPES] = {"fruit_mangue.svg", "fruit_tomate.svg", "fruit_papaye.svg",
													   "fruit_avocat.svg", "fruit_safou.svg"};
		const char *kObjPlural[CalculGame::NUM_TYPES] = {"mangues", "tomates", "papayes", "avocats", "safous"};
	...
	void CalculGame::Render(const MouFrame &frame) noexcept {
		...
		auto drawCluster = [&](int32 n, float32 rx, float32 rw, int32 crossFrom) {
			...
			for (int32 i = 0; i < nn; ++i) {
				...
				if (tex)
					frame.Image(tex, sx[i], sy[i], objSz, objSz,
								crossed ? math::NkColor{255, 255, 255, 80} : math::NkColor{255, 255, 255, 255});
				if (crossed) {
					frame.Line(math::NkVec2f{sx[i] + 8.f, sy[i] + 8.f},
							   math::NkVec2f{sx[i] + objSz - 8.f, sy[i] + objSz - 8.f}, C::CORAL(), 5.f);
					frame.Line(math::NkVec2f{sx[i] + objSz - 8.f, sy[i] + 8.f},
							   math::NkVec2f{sx[i] + 8.f, sy[i] + objSz - 8.f}, C::CORAL(), 5.f);
				}
			}
		};

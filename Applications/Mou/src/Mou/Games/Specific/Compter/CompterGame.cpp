...
namespace mou {
	...
	namespace {
		const char *kObjFile[CompterGame::NUM_TYPES] = {"fruit_mangue.svg", "fruit_tomate.svg", "fruit_avocat.svg",
														"fruit_papaye.svg", "fruit_safou.svg"};
		const char *kObjPlural[CompterGame::NUM_TYPES] = {"mangues", "tomates", "avocats", "papayes", "safous"};
		const char *kNombreVoice[11] = {"nombre_1", "nombre_1", "nombre_2", "nombre_3", "nombre_4", "nombre_5",
	...
	bool CompterGame::Init() noexcept {
		...
		if (mAssets) {
			for (int32 i = 0; i < NUM_TYPES; ++i)
				mObjTex[i] = mAssets->LoadSvg(kObjFile[i], 200, 200);
	...
	void CompterGame::StartLevel() noexcept {
		...
		for (int32 i = 2; i > 0; --i) {
			...
			const int32 t = mAnswers[i];
			mAnswers[i] = mAnswers[j];
			mAnswers[j] = t;
		}

		for (int32 i = 0; i < MAX_OBJ; ++i)
			mObjs[i].counted = false;
		mNeedSpawn = true;
		mFb.ResetLevel(3);
	}
	...
	void CompterGame::Render(const MouFrame &frame) noexcept {
		...
		if (mNeedSpawn && W > 1.f) {
			...
			for (int32 t = 0; t < nTrees; ++t) {
				...
				for (int32 k = 0; k < cnt; ++k) {
					mObjs[start + k].x = sx[k];
					mObjs[start + k].y = sy[k];
				}
			}
			mNeedSpawn = false;
		}
		...
		if (mFb.IsPlaying() && frame.pointerPressed) {
			for (int32 i = 0; i < mCount; ++i) {
				if (!mObjs[i].counted &&
					InRect(frame.pointer.x, frame.pointer.y, mObjs[i].x, mObjs[i].y, objSz, objSz)) {
		...
		for (int32 i = 0; i < mCount; ++i) {
			if (mObjs[i].counted)
				frame.CircleOutline(math::NkVec2f{mObjs[i].x + objSz * 0.5f, mObjs[i].y + objSz * 0.5f}, objSz * 0.58f,
									C::SUCCESS(), 6.f);
			if (mObjTex[mObjType])
				frame.Image(mObjTex[mObjType], mObjs[i].x, mObjs[i].y, objSz, objSz);
		}
		...
		for (int32 i = 0; i < 3; ++i) {
			...
			char num[8];
			std::snprintf(num, sizeof(num), "%d", mAnswers[i]);
			frame.TextCentered(frame.titleFont, cx, cardW, cardY + (cardH - frame.LineH(frame.titleFont)) * 0.5f, num,
							   C::TEXT_DARK());
			if (mFb.IsPlaying() && over && frame.pointerReleased) {
				const float32 ccx = cx + cardW * 0.5f, ccy = cardY + cardH * 0.5f;
				if (mAnswers[i] == mCount) {
					mFb.Good(ccx, ccy);
					mFb.Win(W);
				} else {
					mFb.Bad(ccx, ccy);

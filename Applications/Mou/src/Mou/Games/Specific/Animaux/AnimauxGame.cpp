...
namespace mou {
	...
	namespace {
		...
		const char *kAnimalName[AnimauxGame::NUM_ANIMALS] = {"canard",	"margouillat", "elephant", "lion",
		...
		const char *kAnimalArt[AnimauxGame::NUM_ANIMALS] = {"le ", "le ", "l'", "le ", "le ", "la ", "le ", "l'"};
	...
	bool AnimauxGame::Init() noexcept {
		mWantExit = false;
		if (mAssets) {
			for (int32 a = 0; a < NUM_ANIMALS; ++a)
				mAnimalTex[a] = mAssets->LoadSvg(kAnimalFile[a], 220, 220);
	...
	void AnimauxGame::NewRound() noexcept {
		...
		int32 order[NUM_ANIMALS];
		for (int32 i = 0; i < NUM_ANIMALS; ++i)
			order[i] = i;
		...
		if (mTargetSlot >= mChoices)
			mTargetSlot = mChoices - 1;
	...
	void AnimauxGame::StartLevel() noexcept {
		...
		if (mChoices > MAX_CHOICES)
			mChoices = MAX_CHOICES;
		if (mChoices < 2)
			mChoices = 2;

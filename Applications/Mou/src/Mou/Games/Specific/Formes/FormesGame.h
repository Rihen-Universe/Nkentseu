...
namespace mou {
	...
	class FormesGame : public MouGame {
		public:
			...
			bool WantExit() const noexcept override {
				return mWantExit;
			}

			nkentseu::int32 LevelCount() const noexcept override {
				return mLevels.Count() > 0 ? mLevels.Count() : 8;
			}

			nkentseu::int32 GetStars() const noexcept override {
				return mFb.StarsLeft();
			}

			MouFeedback::Cue ConsumeAudioCue() noexcept override {
				return mFb.ConsumeCue();
			}
		...
		private:
			...
			struct Slot {
					nkentseu::int32 shape = 0;
					nkentseu::float32 x = 0.f, y = 0.f;
					bool filled = false;
			};
			...
			struct Piece {
					...
					nkentseu::float32 x = 0.f, y = 0.f, homeX = 0.f, homeY = 0.f;
					bool placed = false;
			};

...
namespace mou {
	...
	namespace {
		const char *kShapeFile[FormesGame::NUM_SHAPES] = {"shape_circle.svg", "shape_square.svg", "shape_triangle.svg",
														  "shape_star.svg", "shape_heart.svg"};
		const char *kShapeVoice[FormesGame::NUM_SHAPES] = {"forme_rond", "forme_carre", "forme_triangle",
														   "forme_etoile", "forme_coeur"};
	...
	bool FormesGame::Init() noexcept {
		mWantExit = false;
		if (mAssets) {
			for (int32 s = 0; s < NUM_SHAPES; ++s)
				mShapeTex[s] = mAssets->LoadSvg(kShapeFile[s], 200, 200);
	...
	void FormesGame::Render(const MouFrame &frame) noexcept {
		...
		if (mNeedSpawn && W > 1.f) {
			...
			for (int32 i = 0; i < mCount; ++i) {
				mPieces[i].homeX = px0 + i * (pieceSz + gapP);
				mPieces[i].homeY = pieceY;
				mPieces[i].x = mPieces[i].homeX;
				mPieces[i].y = mPieces[i].homeY;
			}
			mNeedSpawn = false;
		}
		...
		for (int32 i = 0; i < mCount; ++i) {
			...
			if (mShapeTex[s.shape]) {
				if (s.filled) {
					...
					int32 col = i;
					for (int32 k = 0; k < mCount; ++k)
						if (mPieces[k].shape == s.shape) {
							col = mPieces[k].color;
							break;
						}
		...
		if (mDragging >= 0 && (frame.pointerReleased || !playing)) {
			...
			for (int32 i = 0; i < mCount; ++i)
				if (!mSlots[i].filled &&
					InRect(cx, cy, mSlots[i].x - 8.f, mSlots[i].y - 8.f, slotSz + 16.f, slotSz + 16.f)) {
					hit = i;
					break;
				}
			if (hit >= 0 && mSlots[hit].shape == p.shape) {
				mSlots[hit].filled = true;
				p.placed = true;
				++mPlaced;
				...
				if (hit >= 0)
					mFb.Bad(cx, cy); // mauvais trou -> perd une étoile
				p.x = p.homeX;
				p.y = p.homeY;
			}
			mDragging = -1;
		...
		for (int32 i = 0; i < mCount; ++i) {
			if (mPieces[i].placed || i == mDragging)
				continue;
			if (mShapeTex[mPieces[i].shape])
				frame.Image(mShapeTex[mPieces[i].shape], mPieces[i].x, mPieces[i].y, pieceSz, pieceSz,
							ShapeColor(mPieces[i].color));
		}
		if (mDragging >= 0) {
			const Piece &p = mPieces[mDragging];
			const float32 s = pieceSz * 1.15f;
			if (mShapeTex[p.shape])
				frame.Image(mShapeTex[p.shape], p.x - (s - pieceSz) * 0.5f, p.y - (s - pieceSz) * 0.5f, s, s,
							ShapeColor(p.color));
		}

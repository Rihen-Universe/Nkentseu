...
namespace mou {
	...
	void MouFx::Add(const P &p) noexcept {
		if (mCount < MAX)
			...
			mP[(mRng) % MAX] = p; // recycle si plein
	}
	...
	void MouFx::Update(float32 dt) noexcept {
		...
		for (int32 i = 0; i < mCount; ++i) {
			...
			p.life -= dt;
			if (p.life <= 0.f)
				continue; // mort -> compacté hors tableau
			p.x += p.vx * dt;
			p.y += p.vy * dt;
			if (p.kind == 0)
				p.vy += 1100.f * dt; // gravité confettis
			else if (p.kind == 1) {
				p.vx *= 0.92f;
				p.vy *= 0.92f;
			}

...
namespace mou {
	...
	static const GameInfo kGameInfoTable[] = {
		{GameId::Couleurs, "Les Couleurs", "Reconnais et trie les couleurs",
		 "Glisse chaque ballon dans le panier de sa couleur. Pour les 3 ans et plus.", GameStatus::Prototype, true},
		{GameId::Compter, "Compter", "Compte de 1 a 10",
		 "Compte les objets puis touche le bon chiffre. Pour les 4 ans et plus.", GameStatus::Prototype, true},
		{GameId::Calcul, "Calculs", "Petits calculs amusants",
		 "Additions et soustractions illustrees jusqu'a 10. Pour les 5 ans et plus.", GameStatus::Prototype, true},
		{GameId::Formes, "Les Formes", "Range les formes", "Glisse chaque forme dans son trou. Pour les 3 ans et plus.",
		 GameStatus::Prototype, true},
		{GameId::Animaux, "Les Animaux", "Touche le bon animal",
		 "Ecoute la consigne et touche le bon animal. Pour les 3 ans et plus.", GameStatus::Prototype, true},
		{GameId::Memoire, "La Memoire", "Retrouve les paires",
		 "Retourne les cartes et retrouve les paires de fruits. Pour les 4 ans et plus.", GameStatus::Prototype, true}};

	nkentseu::memory::NkUniquePtr<MouGame> GameFactory::CreateGame(GameId id, nkentseu::memory::NkAllocator *allocator,
		...
		if (game)
			game->SetAssets(assets); // assets dispo AVANT Init()
		if (game && !game->Init()) {

...
namespace mou {
	...
	class MouFeedback {
		public:
			...
			void Init(nkentseu::uint32 starTex) noexcept {
				mStarTex = starTex;
			}
			...
			nkentseu::int32 StarsLeft() const noexcept {
				return mLeft;
			}

			bool ReadyToAdvance() const noexcept {
				return mState == State::Won && mTimer > 2.3f;
			}

			bool ReadyToRetry() const noexcept {
				return mState == State::Failed && mTimer > 2.0f;
			}

			/// Récupère + efface le dernier signal audio (à router vers NKAudio).
			Cue ConsumeCue() noexcept {
				Cue c = mCue;
				mCue = Cue::None;
				return c;
			}

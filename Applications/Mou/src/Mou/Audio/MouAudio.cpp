...
namespace mou {
	...
	void MouAudio::PlaySfx(Sfx id) noexcept {
		if (!mInitialized || !mSfxLoaded)
			return;
		const int i = (int)id;
		if (i < 0 || i >= (int)Sfx::COUNT || !mSfx[i].IsValid())
			return;
		VoiceParams p;
	...
	void MouAudio::PlayVoice(const char *key) noexcept {
		...
		if (!s.IsValid())
			return; // voix pas encore fournie -> silencieux

		if (mVoice[mVoiceNext].IsValid())
			AudioLoader::Free(mVoice[mVoiceNext]);
		mVoice[mVoiceNext] = s;
		VoiceParams p;
		p.volume = settings::EffectiveSfx();
		p.bus = "Voice";
	...
	void MouAudio::Pause() noexcept {
		if (mInitialized && mHasSample)
			AudioEngine::Instance().Pause(mMusicHandle);
	}

	void MouAudio::Resume() noexcept {
		if (mInitialized && mHasSample)
			AudioEngine::Instance().Resume(mMusicHandle);
	}

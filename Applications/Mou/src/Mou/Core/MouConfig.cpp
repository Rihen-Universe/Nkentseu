...
namespace mou {
	...
	namespace settings {
		...
		nkentseu::float32 EffectiveMusic() noexcept {
			return muted ? 0.f : musicVolume;
		}

		nkentseu::float32 EffectiveSfx() noexcept {
			return muted ? 0.f : sfxVolume;
		}
		...
		void Load() noexcept {
			nkentseu::NkFile f;
			if (!f.Open(kPath, nkentseu::NkFileMode::NK_READ))
				return; // pas encore de réglages -> défauts
			char buf[256];
			const nkentseu::usize sz = (nkentseu::usize)f.Size();
			const nkentseu::usize rd = f.Read(buf, sz < sizeof(buf) - 1 ? sz : sizeof(buf) - 1);
			f.Close();
			buf[rd] = 0;
			float m = musicVolume, s = sfxVolume;
			int mu = muted ? 1 : 0;

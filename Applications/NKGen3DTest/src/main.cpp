        ...
        for (uint32 y = 0; y < G; ++y)
            for (uint32 x = 0; x < G; ++x) {
                ...
                switch (kind) {
                    ...
                    case 2: on = (fx*fx + fy*fy) <= 4.2f; break;                      // cylindre (axe z)
                    default: {                                                        // pyramide
...
int main() {
    ...
    for (uint32 c = 0; c < NCLS; ++c) {
        ...
        char t[64]; snprintf(t, sizeof(t), "%s reconstruite", names[c]);
    ...
    for (int k = 0; k < 3; ++k) {
        ...
        char t1[48]; snprintf(t1, sizeof(t1), "forme générée (t=%.2f)", t);

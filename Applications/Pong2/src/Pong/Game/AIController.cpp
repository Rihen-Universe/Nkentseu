...
namespace nkentseu {
	namespace pong {
		...
		// ─────────────────────────────────────────────────────────────────────
		static float PredictBallY(float ballX, float ballY, float ballVX, float ballVY, float paddleX, float arenaH,
								  int anticipation) {
			if (anticipation == 0)
				return ballY;
		...
		// ─────────────────────────────────────────────────────────────────────
		float AIController::Update(float dt, float ballX, float ballY, float ballVX, float ballVY, float paddleX,
			...
			if (mDifficulty == AIDifficulty::Chaos) {
				...
				if (math::NkFabs(dir) <= step)
					return dir;
				return (dir > 0.0f ? step : -step);
			}

			// Cible : prediction balle (anticipation selon niveau)
			float targetY = PredictBallY(ballX, ballY, ballVX, ballVY, paddleX, arenaH, mAnticipation);

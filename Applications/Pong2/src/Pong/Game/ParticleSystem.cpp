...
namespace nkentseu {
	namespace pong {
		...
		void ParticleSystem::EmitBurst(float x, float y, int count, math::NkColor color, float speedMin, float speedMax,
		...
		void ParticleSystem::EmitDirectional(float x, float y, int count, math::NkColor color, float dirAngle,
											 float spread, float speedMin, float speedMax, float lifeMin, float lifeMax,
		...
		void ParticleSystem::EmitGoal(float wallX, float arenaH, math::NkColor color) {
			const int N = 40;
			for (int i = 0; i < N; ++i) {
				Particle p;
				p.x = wallX;

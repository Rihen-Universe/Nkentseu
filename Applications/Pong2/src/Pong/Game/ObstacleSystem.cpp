...
namespace nkentseu {
	namespace pong {
		...
		static float DistPointSegment(float px, float py, float ax, float ay, float bx, float by, float &closestX,
									  float &closestY) {
		...
		static bool TestCircleVsConvexPoly(float bx, float by, float br, const Vec2 *verts, int n, float &nx,
										   float &ny) {
			float bestDist = 1e9f;
			...
			for (int i = 0; i < n; ++i) {
				...
				float cx, cy;
				const float d = DistPointSegment(bx, by, a.x, a.y, bv.x, bv.y, cx, cy);
				if (d < bestDist) {
					bestDist = d;
					bestNX = bx - cx;
					bestNY = by - cy;
			...
			if (len > 0.0001f) {
				...
				nx = 0.0f;
				ny = -1.0f; // fallback : balle au centre exact
			}
			return true;
		}

		// Test cercle-cercle : balle vs obstacle Circle (rayon = w/2).
		static bool TestCircleVsCircle(float bx, float by, float br, float cx, float cy, float cr, float &nx,
									   float &ny) {
		...
		static void ReflectVelocity(float vx, float vy, float nx, float ny, float &outVX, float &outVY) {
		...
		static void PlaceAtFraction(Obstacle &o, float arenaW, float arenaH, float fracX, float fracY, float jitterX,
									float jitterY) {
			...
			if (o.y < margin)
				o.y = margin;
			if (o.x + o.w > arenaW - margin)
				o.x = arenaW - margin - o.w;
			if (o.y + o.h > arenaH - margin)
				o.y = arenaH - margin - o.h;
		}
		...
		static bool AABBOverlaps(const Obstacle &a, const Obstacle &b, float margin) {
			return a.x + a.w + margin > b.x && a.x - margin < b.x + b.w && a.y + a.h + margin > b.y &&
				   a.y - margin < b.y + b.h;
		}

		static bool OverlapsAny(const NkVector<Obstacle> &list, const Obstacle &o, float margin) {
			for (uint32 i = 0; i < list.Size(); ++i) {
				if (AABBOverlaps(o, list[i], margin))
					return true;
			}
			return false;
		}
		...
		static bool TryPlaceNoOverlap(Obstacle &o, const NkVector<Obstacle> &list, float arenaW, float arenaH,
									  float fracX, float fracY, float jitterX, float jitterY, int maxTries = 12,
									  float margin = 6.0f) {
		...
		static float RandPower() {
			return 0.6f + Rand01() * 1.0f;
		}
		...
		static float RandPowerLevel(int level) {
			switch (level) {
				case 1:
					return 0.4f + Rand01() * 0.4f; // 0.4..0.8 (-40%)
				case 3:
					return 1.0f + Rand01() * 1.0f; // 1.0..2.0 (+40..+100%)
				case 2:
				default:
					return 0.6f + Rand01() * 1.0f; // 0.6..1.6 (default)
			}
		}

		// Helper : retourne soit le count force par l'utilisateur (s.obsCount[type]
		// si > 0) soit un random dans [randMin, randMax].
		static int ResolveCount(const GameSettings &s, ObstacleType t, int randMin, int randMax) {
		...
		static void AssignRandomMotion(Obstacle &o, float arenaW, float arenaH, float scale) {
			...
			switch (o.type) {
				case ObstacleType::Wall:
					...
					if (rollPercent < 25)
						o.motion = ObstacleMotion::Translate;
					else if (rollPercent < 45)
						o.motion = ObstacleMotion::Rotate;
					else if (rollPercent < 50)
						o.motion = ObstacleMotion::Blink;
					break;
				...
				case ObstacleType::Mine:
					...
					if (rollPercent < 25)
						o.motion = ObstacleMotion::Warp;
					else if (rollPercent < 50)
						o.motion = ObstacleMotion::SpawnDespawn;
			...
			switch (o.motion) {
				...
				case ObstacleMotion::Rotate:
					...
					o.rotationSpeed = (Rand01() > 0.5f ? 1.0f : -1.0f) * (0.5f + Rand01() * 1.5f); // 0.5..2.0 rad/s
					break;
		...
		void ObstacleSystem::SpawnFromSettings(const GameSettings &s, float arenaW, float arenaH, float scale) {
			mObstacles.Clear();
			...
			(void)cx;
			(void)cy; // reserve pour les positions centrees historiques
			...
			if (s.obsActive[(int)ObstacleType::Wall]) {
				...
				const ObstacleShape shapes[5] = {ObstacleShape::Rectangle, ObstacleShape::Circle,
				...
				for (int i = 0; i < count; ++i) {
					...
					o.w = side;
					o.h = side;
					o.rotation = (o.shape != ObstacleShape::Rectangle && o.shape != ObstacleShape::Circle)
									 ? Rand11() * 0.6f
									 : 0.0f;
					const int slot = i % 6;
					...
					if (!TryPlaceNoOverlap(o, mObstacles, arenaW, arenaH, fracsX[slot], fracsY[slot], 0.04f, 0.05f))
						continue;
			...
			if (s.obsActive[(int)ObstacleType::Gravity]) {
				...
				for (int i = 0; i < count; ++i) {
					...
					o.h = o.w;
					if (!TryPlaceNoOverlap(o, mObstacles, arenaW, arenaH, 0.30f + i * 0.40f, 0.35f + Rand01() * 0.30f,
										   0.05f, 0.08f))
						continue;
					mObstacles.PushBack(o);
				}
			...
			if (s.obsActive[(int)ObstacleType::Portal]) {
				...
				for (int i = 0; i < count; ++i) {
					...
					o.w = (22.0f + Rand01() * 12.0f) * scale;
					o.h = o.w;
					if (!TryPlaceNoOverlap(o, mObstacles, arenaW, arenaH, fX[i], fY[i], 0.04f, 0.06f))
						continue;
					mObstacles.PushBack(o);
				}
			}
			...
			if (s.obsActive[(int)ObstacleType::Mine]) {
				...
				for (int i = 0; i < count; ++i) {
					...
					o.w = (16.0f + Rand01() * 12.0f) * scale;
					o.h = o.w;
					if (!TryPlaceNoOverlap(o, mObstacles, arenaW, arenaH, 0.15f + Rand01() * 0.70f,
										   0.15f + Rand01() * 0.70f, 0.0f, 0.0f))
						continue;
					mObstacles.PushBack(o);
				}
			}
			...
			if (s.obsActive[(int)ObstacleType::AirCurrent]) {
				...
				for (int i = 0; i < count; ++i) {
					...
					o.h = 14.0f * scale;
					if (!TryPlaceNoOverlap(o, mObstacles, arenaW, arenaH, fX[i], fY[i], 0.04f, 0.02f))
						continue;
					mObstacles.PushBack(o);
				}
			}
			...
			if (s.obsActive[(int)ObstacleType::Magnet]) {
				...
				for (int i = 0; i < count; ++i) {
					...
					if (!TryPlaceNoOverlap(o, mObstacles, arenaW, arenaH, i == 0 ? 0.12f : 0.88f,
										   0.30f + Rand01() * 0.40f, 0.0f, 0.05f))
						continue;
					mObstacles.PushBack(o);
				}
			}
			...
			if (s.obsActive[(int)ObstacleType::GhostMirror]) {
				...
				for (int i = 0; i < count; ++i) {
					...
					o.w = (28.0f + Rand01() * 12.0f) * scale;
					o.h = o.w;
					if (!TryPlaceNoOverlap(o, mObstacles, arenaW, arenaH, i == 0 ? 0.30f : 0.70f,
										   0.55f + Rand01() * 0.25f, 0.05f, 0.05f))
						continue;
					mObstacles.PushBack(o);
				}
			}
			...
			if (s.obsActive[(int)ObstacleType::BonusStar]) {
				...
				for (int i = 0; i < count; ++i) {
					...
					o.h = 22.0f * scale;
					o.collected = false;
					if (!TryPlaceNoOverlap(o, mObstacles, arenaW, arenaH, 0.20f + Rand01() * 0.60f,
										   0.15f + Rand01() * 0.70f, 0.0f, 0.0f))
						continue;
					mObstacles.PushBack(o);
				}
		...
		// ─────────────────────────────────────────────────────────────────────
		static void GetColorsForType(ObstacleType t, math::NkColor &outColor, math::NkColor &outGlow) {
			switch (t) {
				case ObstacleType::Wall:
		...
		void ObstacleSystem::AppendFromNet(ObstacleType type, ObstacleShape shape, float xTopLeft, float yTopLeft,
										   float w, float h, float rotation, bool active, bool visible) {
			Obstacle o{};
			o.type = type;
			o.shape = shape;
			o.x = xTopLeft;
		...
		// ─────────────────────────────────────────────────────────────────────
		void ObstacleSystem::Render(GLRenderer2D &r, float ax, float ay, float scale) const {
			for (uint32 i = 0; i < mObstacles.Size(); ++i) {
				...
				switch (o.type) {
						...
						switch (o.shape) {
							...
							case ObstacleShape::Hexagon: {
								...
								for (int k = 0; k < n; ++k) {
									...
									r.DrawTriangle(pcx, pcy, a0.x + ax, a0.y + ay, a1.x + ax, a1.y + ay, o.color);
								}
								// Outline (lignes joignant les sommets)
								for (int k = 0; k < n; ++k) {
									const Vec2 &a0 = verts[k];
									const Vec2 &a1 = verts[(k + 1) % n];
									r.DrawLine(a0.x + ax, a0.y + ay, a1.x + ax, a1.y + ay, o.glowColor, lineW);
								}
								break;
							}
						}
					...
					case ObstacleType::Gravity: {
						...
						for (int k = 0; k < 3; ++k) {
							...
							math::NkColor c = WithAlpha(o.color, (0.20f + 0.15f * k) + pulse01 * 0.10f);
							r.DrawCircleOutline(ocx, ocy, rr, c, 1.5f * lineW, 32);
						}
						r.DrawCircle(ocx, ocy, math::NkMax(2.0f, 3.0f * scale), WithAlpha(o.color, 0.7f), 12);
						break;
					}
					case ObstacleType::Portal: {
					...
					case ObstacleType::Mine: {
						...
						r.DrawCircle(ocx, ocy, o.w * 0.30f, WithAlpha(o.color, pulse * 0.6f), 16);
						for (int k = 0; k < 8; ++k) {
							const float a = 6.28318f * k / 8.0f;
							const float r1 = o.w * 0.35f;
							const float r2 = o.w * 0.50f;
							r.DrawLine(ocx + math::NkCos(a) * r1, ocy + math::NkSin(a) * r1, ocx + math::NkCos(a) * r2,
									   ocy + math::NkSin(a) * r2, WithAlpha(o.color, pulse), 1.5f * lineW);
						}
						break;
					}
					case ObstacleType::AirCurrent: {
					...
					case ObstacleType::Magnet: {
						...
						r.DrawQuadOutline(ox, oy, o.w, o.h, WithAlpha(o.glowColor, 0.6f + pulse01 * 0.3f), lineW);
						break;
					}
					case ObstacleType::GhostMirror: {
					...
					case ObstacleType::BonusStar: {
						...
						for (int k = 0; k < 5; ++k) {
							...
							r.DrawLine(ocx + math::NkCos(a1) * rad * scl, ocy + math::NkSin(a1) * rad * scl,
									   ocx + math::NkCos(a2) * rad * scl, ocy + math::NkSin(a2) * rad * scl, o.color,
									   1.5f * lineW);
						}
						break;
					}
		...
		// ─────────────────────────────────────────────────────────────────────
		ObstacleHit ObstacleSystem::CheckCollision(float bx, float by, float br, float vx, float vy, float dt60) const {
			(void)dt60;
			ObstacleHit hit{};
			for (uint32 i = 0; i < mObstacles.Size(); ++i) {
				const Obstacle &o = mObstacles[i];
				...
				const bool isZone = (o.type == ObstacleType::Gravity || o.type == ObstacleType::AirCurrent);
				// AABB-Circle approximative (ball traitee comme AABB
				// englobant r). Suffisant pour gameplay.
				const bool overlap = (bx + br > o.x && bx - br < o.x + o.w && by + br > o.y && by - br < o.y + o.h);
				if (!overlap)
					continue;
				...
				switch (o.type) {
					...
					case ObstacleType::Gravity: {
						...
						if (dist > 0.001f) {
							...
							const float tangentBoost = (dist < radius * 0.4f) ? 0.5f : 0.0f;
							const float nxT = -nyR;
							const float nyT = nxR;
							hit.hit = true;
							hit.setVel = true;

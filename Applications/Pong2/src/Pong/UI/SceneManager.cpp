			...
			switch (mPendingOp) {
				...
				case Op::Pop: {
					if (mStack.Size() > 0) {
						...
						Scene *newTop = (mStack.Size() > 0) ? mStack[mStack.Size() - 1] : nullptr;
						if (newTop != nullptr)
							newTop->OnResumedFromChild(ctx);
					}
					break;
				}
				case Op::PopToRoot: {
					...
					while (mStack.Size() > 1) {
						Scene *old = mStack[mStack.Size() - 1];
						mStack.PopBack();
						logger.Info("[SceneManager] PopToRoot drop: {0}", old ? old->Name() : "(null)");
						mToDestroy.PushBack(old);
					}
					break;
				}

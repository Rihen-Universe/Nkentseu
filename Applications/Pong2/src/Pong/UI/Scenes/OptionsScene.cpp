	...
	namespace pong {
		...
		static const CatDesc kCats[OptionsScene::kCatCount] = {
			{OptionsScene::Cat_Help,
			 ...
			 {0, 245, 255, 255},
			 true},
			{OptionsScene::Cat_Audio, "AUDIO", "VOLUME MUSIQUE, EFFETS, ANNONCE - A VENIR", {255, 215, 0, 255}, false},
			...
			{OptionsScene::Cat_Controls,
			 ...
			 {255, 107, 0, 255},
			 false},
			{OptionsScene::Cat_Network, "RESEAU", "REGION, PING - A VENIR (NKNETWORK)", {80, 255, 100, 255}, false},
		};
		...
		int OptionsScene::HitTestCard(float sx, float sy) const {
			for (int i = 0; i < (int)kCatCount; ++i) {
				if (sx >= mCardX[i] && sx <= mCardX[i] + mCardW && sy >= mCardY[i] && sy <= mCardY[i] + mCardH)
					return i;
			}
			return -1;
		}
		...
		void OptionsScene::OnRender(AppContext &ctx) {
			...
			// et les epaisseurs d'outline.
			{
				...
				f.DrawStringCenteredScaled(r, FontAtlas::HeadlineSlot, scale, (float)W * 0.5f,
										   (float)ctx.safe.TopY() + Pct::H(H, 0.024f, 10.0f, 32.0f), "OPTIONS",
										   theme::White());
			}
			...
			for (int i = 0; i < (int)kCatCount; ++i) {
				...
				r.DrawQuadOutline(bx, by, availW, cardH, bd, focused && d.enabled ? 2.5f * scale : 1.0f);
				...
				f.DrawStringScaled(r, FontAtlas::SubtitleSlot, scale, bx + cardPadL, by + titleY, d.title, titleCol);

				// Sous-titre
				math::NkColor subCol = {255, 255, 255, (uint8_t)((d.enabled ? 160 : 80) * a)};
				f.DrawStringScaled(r, FontAtlas::SmallSlot, scale, bx + cardPadL, by + subY, d.sub, subCol);

				// Marqueur "BIENTOT" si desactive
				if (!d.enabled) {
					math::NkColor warn = {255, 200, 60, (uint8_t)(180 * a)};
					f.DrawStringScaled(r, FontAtlas::SmallSlot, scale, bx + availW - warnW, by + titleY, "BIENTOT",
									   warn);
				}
		...
		void OptionsScene::Activate(AppContext &ctx, Category cat) {
			switch (cat) {
				...
				case Cat_Network:
				default:
					logger.Info("[Options] Category activated (TODO): {0}", kCats[(int)cat].title);
					break;
			}
		}
		...
		void OptionsScene::OnEvent(AppContext &ctx, NkEvent &ev) {
			...
			if (auto *k = ev.As<NkKeyPressEvent>()) {
				switch (k->GetKey()) {
					case NkKey::NK_ESCAPE:
						...
						return;
					case NkKey::NK_UP:
						mFocusIndex = (mFocusIndex - 1 + (int)kCatCount) % (int)kCatCount;
						return;
					case NkKey::NK_DOWN:
						mFocusIndex = (mFocusIndex + 1) % (int)kCatCount;
						return;
					case NkKey::NK_ENTER:
					case NkKey::NK_SPACE:
			...
			if (auto *mr = ev.As<NkMouseButtonReleaseEvent>()) {
				...
				if (mr->GetButton() == NkMouseButton::NK_MB_LEFT) {
					...
					if (idx >= 0) {
						mFocusIndex = idx;
						if (kCats[idx].enabled)
							Activate(ctx, (Category)idx);
					}
				}
				return;
			}

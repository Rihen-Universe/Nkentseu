...
namespace nkentseu {
	namespace pong {
		...
		static constexpr const char *kDonateBaseUrl = "https://www.linkedin.com/company/rihen-universe";
		static constexpr const char *kShareText =
			"Decouvre Pong Ultra Arena — un Pong avec obstacles, bonus, malus et IA.";
		static constexpr const char *kShareUrl = "https://www.linkedin.com/company/rihen-universe";
		...
		static const Social kSocials[SupporterScene::kSocialCount] = {
			...
			{"GITHUB",
			 "@Rihen-Universe",
			 "https://github.com/Rihen-Universe",
			 {60, 70, 85, 255},
			 "GH",
		...
		void SupporterScene::OnEnter(AppContext & /*ctx*/) {
			...
			for (int i = 0; i < (int)kSocialCount; ++i) {
				...
				if (!mSocialLoaded[i]) {
					logger.Warn("[Supporter] icone manquante : {0} -> fallback badge texte", kSocials[i].iconPath);
				}
			}
		...
		bool SupporterScene::HitTestShare(float sx, float sy) const {
			return sx >= mShareX && sx <= mShareX + mShareW && sy >= mShareY && sy <= mShareY + mShareH;
		}

		int SupporterScene::HitTestAmount(float sx, float sy) const {
			for (int i = 0; i < kAmountCount + 1; ++i) {
				if (sx >= mAmountX[i] && sx <= mAmountX[i] + mAmountW && sy >= mAmountY[i] &&
					sy <= mAmountY[i] + mAmountH)
					return i;
			}
			return -1;
		}
		...
		int SupporterScene::HitTestSocial(float sx, float sy) const {
			for (int i = 0; i < (int)kSocialCount; ++i) {
				if (sx >= mSocialX[i] && sx <= mSocialX[i] + mSocialW && sy >= mSocialY[i] &&
					sy <= mSocialY[i] + mSocialH)
					return i;
			}
			return -1;
		}
		...
		void SupporterScene::DoOpenSocial(AppContext & /*ctx*/, int idx) {
			if (idx < 0 || idx >= (int)kSocialCount)
				return;
			logger.Info("[Supporter] OPEN SOCIAL {0} -> {1}", kSocials[idx].name, kSocials[idx].url);
		...
		void SupporterScene::OnRender(AppContext &ctx) {
			...
			const float scrollBot = (float)ctx.safe.TopY() + (float)ctx.safe.SafeH() - Pct::H(H, 0.015f, 8.0f, 24.0f);
			...
			// ecran ca s'aere. On garde scale pour le rendu glyphe.
			{
				...
				if (screenY + storyH >= scrollTop - 20.0f * scale && screenY <= scrollBot + 20.0f * scale) {
					...
					r.DrawQuad(gridLeft, screenY, bandW, storyH, {255, 107, 0, 220});

					float ly = screenY + vPad;
					for (int i = 0; i < kStoryN; ++i) {
						// Premiere ligne = headline (subtitle slot), reste body
						const bool isHead = (i == 0);
						const math::NkColor c = isHead ? math::NkColor{0, 245, 255, (uint8_t)(255 * enterA)}
													   : math::NkColor{255, 255, 255, (uint8_t)(220 * enterA)};
						f.DrawStringScaled(r, isHead ? FontAtlas::SubtitleSlot : FontAtlas::BodySlot, scale,
										   gridLeft + textPadL, ly, kStory[i], c);
			...
			// proportionnee au viewport.
			{
				...
				if (mShareY + mShareH >= scrollTop - 20.0f * scale && mShareY <= scrollBot + 20.0f * scale) {
					...
					r.DrawQuadOutline(cx - s2, cy - s2 * 0.6f, s2 * 1.6f, s2 * 1.6f, {0, 245, 255, 240}, 1.5f);
					r.DrawLine(cx + s2 * 0.4f, cy - s2 * 1.3f, cx + s2 * 1.3f, cy - s2 * 1.3f, {0, 245, 255, 240},
							   2.0f);
					r.DrawLine(cx + s2 * 1.3f, cy - s2 * 1.3f, cx + s2 * 1.3f, cy - s2 * 0.4f, {0, 245, 255, 240},
							   2.0f);

					f.DrawStringScaled(r, FontAtlas::SubtitleSlot, scale, mShareX + textPadL, mShareY + titlePadT,
									   "PARTAGER LE JEU", theme::White());
					f.DrawStringScaled(r, FontAtlas::SmallSlot, scale, mShareX + textPadL, mShareY + subPadT,
									   "RECOMMANDE A UN AMI — 100% GRATUIT", {255, 255, 255, 180});
				}
			...
			// Hauteur card et padding inter-cards en % H.
			{
				...
				f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, (float)W * 0.5f, screenSecY,
				...
				for (int i = 0; i < total; ++i) {
					...
					math::NkColor accent = isFree ? math::NkColor{204, 119, 255, 255}
												  : (usd == 0 ? math::NkColor{160, 160, 160, 255}
															  : (usd >= 10 ? math::NkColor{255, 107, 0, 255}
																		   : math::NkColor{0, 245, 255, 255}));
					math::NkColor bg = accent;
					bg.a = (uint8_t)(30 * enterA);
					math::NkColor bd = accent;
					bd.a = (uint8_t)(220 * enterA);
					r.DrawQuad(mAmountX[i], mAmountY[i], mAmountW, mAmountH, bg);
					r.DrawQuadOutline(mAmountX[i], mAmountY[i], mAmountW, mAmountH, bd, 1.5f);

					char buf[16];
					if (isFree)
						std::snprintf(buf, sizeof(buf), "LIBRE");
					else if (usd == 0)
						std::snprintf(buf, sizeof(buf), "0$ MERCI");
					else
						std::snprintf(buf, sizeof(buf), "%d$", usd);
					f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, mAmountX[i] + mAmountW * 0.5f,
											   mAmountY[i] + labelPadT, buf, accent);

					const char *sub = isFree ? "SAISIR UN MONTANT" : (usd == 0 ? "GRATUIT, JUSTE MERCI" : "DON UNIQUE");
					math::NkColor subC = {255, 255, 255, (uint8_t)(160 * enterA)};
					f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, mAmountX[i] + mAmountW * 0.5f,
											   mAmountY[i] + subPadT, sub, subC);
				}
			...
			// Hauteur de ligne en % H pour rester lisible mobile/desktop.
			{
				...
				f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, (float)W * 0.5f, subY,
										   "ENVOIE DIRECTEMENT VERS CES COMPTES", {255, 255, 255, 160});
				worldY += subH;

				for (int i = 0; i < kPayIdsN; ++i) {
					const float rx = gridLeft;
					const float ry = worldY - mScrollY + scrollTop;
					math::NkColor bg = kPayIds[i].accent;
					bg.a = (uint8_t)(22 * enterA);
					math::NkColor bd = kPayIds[i].accent;
					bd.a = (uint8_t)(180 * enterA);
					...
					f.DrawStringScaled(r, FontAtlas::BodySlot, scale, rx + textPadL, ry + textPadT, kPayIds[i].label,
									   lab);
					f.DrawStringScaled(r, FontAtlas::BodySlot, scale, rx + availW * 0.5f, ry + textPadT,
									   kPayIds[i].value, {255, 255, 255, (uint8_t)(230 * enterA)});
					worldY += rowH + rowPad;
				}
				worldY += sectionGap;
			}
			...
			// (rad, hauteur card, padding) en % H/W.
			{
				...
				const int cols = (((float)W > (float)H * 1.2f) && availW > Pct::W(W, 0.6f, 400.0f, 900.0f)) ? 5 : 3;
				...
				for (int i = 0; i < (int)kSocialCount; ++i) {
					...
					math::NkColor bg = kSocials[i].accent;
					bg.a = (uint8_t)(40 * enterA);
					math::NkColor bd = kSocials[i].accent;
					bd.a = (uint8_t)(220 * enterA);
					...
					if (mSocialLoaded[i] && mSocialIcons[i].IsValid()) {
						...
						r.DrawTexturedQuadRGBA(ix, iy, iconW, iconH, 0.0f, 0.0f, 1.0f, 1.0f, tint);
					} else {
						f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, cx, cy - rad * 0.5f,
												   kSocials[i].badge, {255, 255, 255, (uint8_t)(240 * enterA)});
					}

					// Nom + handle a droite
					f.DrawStringScaled(r, FontAtlas::BodySlot, scale, mSocialX[i] + socialTextL,
									   mSocialY[i] + nameTopPad, kSocials[i].name,
									   {255, 255, 255, (uint8_t)(240 * enterA)});
					f.DrawStringScaled(r, FontAtlas::SmallSlot, scale, mSocialX[i] + socialTextL,
									   mSocialY[i] + handleTopPad, kSocials[i].handle, {255, 255, 255, 170});
				}
			...
			// 6. Footer
			{
				...
				f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, (float)W * 0.5f, screenFootY,
										   "MERCI D'ETRE LA. — RIHEN", {255, 107, 0, 200});
				worldY += footerH;
			}
			...
			// couvre toute la zone reservee + bouton RETOUR + titre.
			{
				...
				r.DrawQuad(0.0f, scrollTop - sepH, (float)W, sepH, {0, 245, 255, 120});
				...
				f.DrawStringCenteredScaled(r, FontAtlas::HeadlineSlot, scale, (float)W * 0.5f,
										   (float)ctx.safe.TopY() + titleTopP, "SUPPORTER", theme::White());
			}
			...
			if (auto *mr = ev.As<NkMouseButtonReleaseEvent>()) {
				if (mr->GetButton() == NkMouseButton::NK_MB_LEFT && mMouseDown) {
					...
					if (!mDragWasScroll) {
						...
						if (HitTestShare(px, py)) {
							DoShare(ctx);
							return;
						}
						const int ai = HitTestAmount(px, py);
						if (ai >= 0) {
							DoDonate(ctx, (ai == kAmountCount) ? -1 : ai);
							return;
						}
			...
			if (auto *te = ev.As<NkTouchEndEvent>()) {
				if (te->GetNumTouches() > 0) {
					...
					if (!mDragWasScroll) {
						...
						if (HitTestShare(tp.clientX, tp.clientY)) {
							DoShare(ctx);
							return;
						}

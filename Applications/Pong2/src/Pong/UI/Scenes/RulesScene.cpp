...
namespace nkentseu {
	namespace pong {
				...
				case 4: // Mine : cercle rouge + pics
					...
					for (int k = 0; k < 8; ++k) {
						...
						r.DrawLine(cx + math::NkCos(a) * r1, cy + math::NkSin(a) * r1, cx + math::NkCos(a) * r2,
								   cy + math::NkSin(a) * r2, MakeColor(255, 64, 64, 230), 1.5f);
					}
					break;
		...
		static void DrawOrbIcon(GLRenderer2D &r, math::NkColor color, bool isBonus, float cx, float cy, float rad) {
		...
		static float DrawIllustration(GLRenderer2D &r, FontAtlas &f, IllustrationKind kind, float x, float y, float w,
									  float scale, float anim) {
			...
			switch (kind) {
				...
				case kIllModes: {
					...
					for (int i = 0; i < 4; ++i) {
						...
						r.DrawCircle(pX + pondW * 0.5f, pY + pondH * 0.5f, 2.0f * scale, {255, 255, 255, 220}, 10);
				...
				case kIllControls: {
					...
					// Cellule 1 : P1 = W/S
					{
						...
						r.DrawQuadOutline(cx - keyW * 0.5f, cy - keyH - 2.0f * scale, keyW, keyH, {0, 245, 255, 220},
										  1.0f);
					...
					// Cellule 2 : P2 = UP/DOWN
					{
						...
						r.DrawQuadOutline(cx - keyW * 0.5f, cy - keyH - 2.0f * scale, keyW, keyH, {255, 107, 0, 220},
										  1.0f);
				...
				case kIllParams: {
					...
					const float keyW = 22.0f * scale;
					{
						...
						r.DrawQuadOutline(cx - 38.0f * scale, cy - 12.0f * scale, keyW, 24.0f * scale,
										  {0, 245, 255, 220}, 1.0f);
						...
						r.DrawQuadOutline(cx + 16.0f * scale, cy - 12.0f * scale, keyW, 24.0f * scale,
										  {0, 245, 255, 220}, 1.0f);
						f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, cx + 27.0f * scale, cy - 7.0f * scale,
												   "+", {0, 245, 255, 240});
		...
		void RulesScene::OnRender(AppContext &ctx) {
			...
			for (int s = 0; s < mNumSections; ++s) {
				...
				if (screenTitleY + titleBarH >= scrollTop - titleClipMargin &&
					...
					r.DrawQuadOutline(gridLeft, screenTitleY, availW, titleBarH, bd, isOpen ? 2.0f : 1.5f);
			...
			// qui scrolle dessous). Bande horizontale + RETOUR + titre.
			{
				...
				r.DrawQuad(0.0f, scrollTop - hdrSep, (float)W, hdrSep, {0, 245, 255, 120});

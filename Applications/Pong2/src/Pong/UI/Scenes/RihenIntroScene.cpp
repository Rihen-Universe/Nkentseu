		...
		void RihenIntroScene::OnRender(AppContext &ctx) {
			...
			if (mState == State::Loading) {
				...
				if (mLoadingLogo.IsValid()) {
					...
					r.DrawTexturedQuadRGBA(logoCX - logoW * 0.5f, logoCY - logoH * 0.5f, logoW, logoH, 0.0f, 0.0f, 1.0f,
										   1.0f, theme::White());
				}
			...
			if (mFramesLoaded > 0) {
				...
				r.DrawTexturedQuadRGBA(drawX, drawY, drawW, drawH, 0.0f, 0.0f, 1.0f, 1.0f, theme::White());
			}
			...
			if (mCurrentFrame >= fadeStartFrame) {
				...
				r.DrawQuad(0.0f, 0.0f, static_cast<float>(W), static_cast<float>(H), whiteFade);
			}

...
namespace nkentseu {
	namespace pong {
		...
		static void DrawMiniField(GLRenderer2D &r, float x, float y, float w, float h, float t) {
			...
			for (int i = 0; i < segs; ++i) {
				...
				if (a < 0.0f)
					a = 0.0f;
				r.DrawQuad(x + i * segW, midY - 1.0f, segW + 1.0f, 2.0f, AlphaF(theme::Cyan(), a * 0.30f));
			}
		...
		void SplashScene::OnRender(AppContext &ctx) {
			...
			f.DrawStringShadowCentered(r, FontAtlas::DisplaySlot, cx, logoY, "PONG", theme::White(), shadowC, 3);

			// ── Sous-titre cyan ──────────────────────────────────────────────
			const float subY = logoY + (72.0f + 12.0f) * scale;
			f.DrawStringShadowCentered(r, FontAtlas::SubtitleSlot, cx, subY, "ULTRA  ARENA  EDITION", theme::Cyan(),
									   theme::Cyan(), 1);
			...
			f.DrawStringCentered(r, FontAtlas::BodySlot, cx, badgeY + badgePadY, badge, theme::Orange());
			...
			f.DrawStringCentered(r, FontAtlas::BodySlot, cx, ctaY + ctaPadY, cta, AlphaF(theme::Cyan(), 0.95f));

			// ── Credits (zone safe basse) ────────────────────────────────────
			const char *credits = "(C) 2026  PONG ULTRA ARENA  -  v1.1  -  DESIGNED BY RIHEN";
			f.DrawStringCentered(r, FontAtlas::SmallSlot, cx, sa.BottomY(12.0f) - 16.0f, credits,
								 AlphaF(theme::White(), 0.30f));

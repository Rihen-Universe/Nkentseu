...
namespace nkentseu {
	namespace pong {
		...
		void NetworkLobbyScene::OnExit(AppContext &ctx) {
			...
			if (ctx.network != nullptr && ctx.network->State() != NetworkState::Idle) {
		...
		void NetworkLobbyScene::OnUpdate(AppContext &ctx, float dt) {
			...
			if (ctx.network != nullptr && ctx.network->State() == NetworkState::Connected &&
				...
				for (const auto &msg : incoming) {
					if (msg.size < 1)
						continue;
					if (msg.data[0] != netproto::kMsgStartMatch)
						continue;
					if (msg.size < sizeof(netproto::PktStartMatch))
						continue;
					...
					if (ctx.settings != nullptr) {
						...
						ctx.settings->winByTwo = (pkt.flags & netproto::kStartFlagWinByTwo) != 0;
						ctx.settings->powerUpsEnabled = (pkt.flags & netproto::kStartFlagPowerUpsOn) != 0;
		...
		bool NetworkLobbyScene::HitTest(float sx, float sy, float bx, float by, float bw, float bh) const {
		...
		void NetworkLobbyScene::DoHost(AppContext &ctx) {
			...
			logger.Info("[NetLobby] StartHost(port={0}) -> {1}", kDefaultPort, ok ? "OK" : "FAIL");
			...
			if (ok && ctx.discovery != nullptr) {
				ctx.discovery->StartBeacon(ctx.myCountry, ctx.myCity, ctx.myCode, kDefaultPort);
			}
		}
		...
		void NetworkLobbyScene::DoJoinSlot(AppContext &ctx, int slotIdx) {
			if (slotIdx < 0 || slotIdx >= mHostBtnCount)
				return;
			if (ctx.network == nullptr)
				return;
			...
			logger.Info("[NetLobby] DoJoinSlot[{0}] {1}:{2} -> {3}", slotIdx, ip, (unsigned)port, ok ? "OK" : "FAIL");
		...
		void NetworkLobbyScene::DoAcceptChallenger(AppContext &ctx, int slotIdx) {
			if (slotIdx < 0 || slotIdx >= mChalBtnCount)
				return;
			if (ctx.network == nullptr)
				return;
			...
			logger.Info("[NetLobby] AcceptChallenger[{0}] peerId={1}", slotIdx, (unsigned long long)peerId);
		}
		...
		void NetworkLobbyScene::DoLaunch(AppContext &ctx) {
			if (ctx.network == nullptr)
				return;
			if (ctx.network->State() != NetworkState::Connected)
				return;
		...
		void NetworkLobbyScene::OnRender(AppContext &ctx) {
			...
			f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, mBackX + mBackW * 0.5f, mBackY + mBackH * 0.18f,
									   "RETOUR", theme::Cyan());
			f.DrawStringCenteredScaled(r, FontAtlas::HeadlineSlot, scale, Wf * 0.5f,
									   (float)ctx.safe.TopY() + Pct::H(H, 0.022f, 12.0f, 32.0f), "MULTIJOUEUR RESEAU",
									   theme::White());
			...
			{
				const float idY = (float)ctx.safe.TopY() + Pct::H(H, 0.072f, 38.0f, 90.0f);
				char myBuf[96];
				std::snprintf(myBuf, sizeof(myBuf), "TOI : %s/%s-%s", ctx.myCountry, ctx.myCity, ctx.myCode);
				f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, Wf * 0.5f, idY, myBuf,
										   {0, 245, 255, 230});
				// Si on a un pair identifie, l'affiche sous le notre.
				if (ctx.network != nullptr && ctx.network->HasPeerIdentity()) {
					char peerBuf[96];
					std::snprintf(peerBuf, sizeof(peerBuf), "VS  %s/%s-%s", ctx.network->PeerCountry(),
								  ctx.network->PeerCity(), ctx.network->PeerCode());
					f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, Wf * 0.5f,
											   idY + Pct::H(H, 0.038f, 20.0f, 44.0f), peerBuf, {255, 107, 0, 230});
				}
			}

			// ── Status global (centre) ────────────────────────────────────
			const NetworkState st = (ctx.network != nullptr) ? ctx.network->State() : NetworkState::Idle;
			...
			const float idBlockExtra =
				(ctx.network != nullptr && ctx.network->HasPeerIdentity()) ? Pct::H(H, 0.038f, 20.0f, 44.0f) : 0.0f;
			float y = (float)ctx.safe.TopY() + Pct::H(H, 0.165f, 88.0f, 200.0f) + idBlockExtra;

			f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, Wf * 0.5f, y, statusText, statusCol);
			y += Pct::H(H, 0.045f, 24.0f, 52.0f);

			// Erreur (si presente)
			if (ctx.network != nullptr && ctx.network->LastError()[0] != '\0') {
				f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, Wf * 0.5f, y, ctx.network->LastError(),
										   {255, 100, 100, 220});
			...
			if (mView == 0 && st == NetworkState::Idle) {
				...
				mHostY = y;
				{
					...
					f.DrawStringScaled(r, FontAtlas::SubtitleSlot, scale, mHostX + padX, mHostY + titleY, "HEBERGER",
									   theme::Cyan());
					char buf[64];
					std::snprintf(buf, sizeof(buf), "PORT %u — DEMANDE A L'AUTRE JOUEUR DE TE REJOINDRE",
								  (unsigned)kDefaultPort);
					f.DrawStringScaled(r, FontAtlas::SmallSlot, scale, mHostX + padX, mHostY + subY, buf,
									   {255, 255, 255, 180});
				}
				y += mHostH + cardGap;
				...
				mJoinY = y;
				{
					...
					f.DrawStringScaled(r, FontAtlas::SubtitleSlot, scale, mJoinX + padX, mJoinY + titleY, "REJOINDRE",
									   {255, 107, 0, 240});
					f.DrawStringScaled(r, FontAtlas::SmallSlot, scale, mJoinX + padX, mJoinY + subY,
			...
			if (mView == 1 && st == NetworkState::Idle) {
				f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, Wf * 0.5f, y,
										   "PARTIES VISIBLES SUR TON RESEAU LOCAL", {255, 255, 255, 200});
				y += Pct::H(H, 0.034f, 18.0f, 38.0f);

				NkVector<HostInfo> hosts;
				if (ctx.discovery != nullptr)
					ctx.discovery->GetHosts(hosts);

				if (hosts.Size() == 0) {
					// Animation 3 points pour signaler "scan en cours".
					const float pulse = 0.5f + 0.5f * math::NkSin(mTime * 3.0f);
					math::NkColor col = {255, 215, 0, (uint8_t)(160 + 80 * pulse)};
					f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, Wf * 0.5f, y, "RECHERCHE EN COURS...",
											   col);
					y += Pct::H(H, 0.040f, 22.0f, 48.0f);
					f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, Wf * 0.5f, y,
											   "VERIFIE QUE TON ADVERSAIRE EST SUR LE MEME WIFI", {255, 255, 255, 140});
					y += Pct::H(H, 0.040f, 22.0f, 48.0f);
				} else {
					...
					const float maxScrollJ = (mContentHJoin > mViewportHJoin) ? (mContentHJoin - mViewportHJoin) : 0.0f;
					if (mScrollJoin < 0.0f)
						mScrollJoin = 0.0f;
					if (mScrollJoin > maxScrollJ)
						mScrollJoin = maxScrollJ;
					...
					for (int i = 0; i < nShow; ++i) {
						...
						char nameBuf[96];
						std::snprintf(nameBuf, sizeof(nameBuf), "%s/%s-%s", h.country, h.city, h.code);
						f.DrawStringScaled(r, FontAtlas::SubtitleSlot, scale, bx + padX, by + titleDY, nameBuf,
										   {255, 107, 0, 240});
						f.DrawStringScaled(r, FontAtlas::SmallSlot, scale, bx + padX, by + subDY, "TAPE POUR REJOINDRE",
										   {255, 255, 255, 180});
				...
				f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, mCancelX + mCancelW * 0.5f,
										   mCancelY + cancelPadT, "ANNULER", {255, 100, 100, 240});
			}
			...
			if (st == NetworkState::Hosting) {
				...
				if (ctx.network != nullptr)
					ctx.network->GetChallengers(chals);
				if (chals.Size() == 0) {
					// Aucun challenger pour l'instant.
					const float pulse = 0.5f + 0.5f * math::NkSin(mTime * 3.0f);
					f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, Wf * 0.5f, y,
											   "AUCUN JOUEUR POUR LE MOMENT...",
											   {255, 215, 0, (uint8_t)(160 + 80 * pulse)});
					y += Pct::H(H, 0.040f, 22.0f, 48.0f);
					f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, Wf * 0.5f, y,
											   "TON IDENTIFIANT EST DEJA VISIBLE SUR LE RESEAU", {255, 255, 255, 140});
					y += Pct::H(H, 0.038f, 20.0f, 44.0f);
				} else {
					f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, Wf * 0.5f, y,
											   "CHOISIS UN CHALLENGER A AFFRONTER", {255, 255, 255, 220});
					y += Pct::H(H, 0.034f, 18.0f, 38.0f);
					...
					const float maxScrollH = (mContentHHost > mViewportHHost) ? (mContentHHost - mViewportHHost) : 0.0f;
					if (mScrollHost < 0.0f)
						mScrollHost = 0.0f;
					if (mScrollHost > maxScrollH)
						mScrollHost = maxScrollH;
					const float scrollAreaTop = y;
					for (int i = 0; i < nShow; ++i) {
						...
						char nameBuf[96];
						if (c.hasIdentity) {
							std::snprintf(nameBuf, sizeof(nameBuf), "%s/%s-%s", c.country, c.city, c.code);
						} else {
							std::snprintf(nameBuf, sizeof(nameBuf), "JOUEUR (HANDSHAKE EN COURS...)");
						}
						f.DrawStringScaled(r, FontAtlas::SubtitleSlot, scale, bx + padX, by + titleDY, nameBuf,
										   theme::Cyan());
						f.DrawStringScaled(r, FontAtlas::SmallSlot, scale, bx + padX, by + subDY,
										   "TAPE POUR ACCEPTER CE JOUEUR", {255, 255, 255, 180});
						mChalBtnX[i] = bx;
						mChalBtnY[i] = by;
						mChalBtnW[i] = bw;
						mChalBtnH[i] = bh;
					...
					if (mContentHHost > mViewportHHost) {
						...
						f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, Wf * 0.5f, y,
												   "FAIS DEFILER POUR VOIR LES AUTRES JOUEURS", {255, 255, 255, 130});
						y += Pct::H(H, 0.028f, 14.0f, 32.0f);
					}
				}
			...
			if (ctx.network != nullptr && ctx.network->WasRejectedByHost()) {
				...
				f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, Wf * 0.5f, y,
										   "TU AS ETE REFUSE PAR L'HOTE", {255, 100, 100, 240});
				y += Pct::H(H, 0.038f, 20.0f, 44.0f);
				f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, Wf * 0.5f, y,
				...
				f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, mCancelX + mCancelW * 0.5f,
										   mCancelY + cancelPadT, "CHERCHER UN AUTRE HOTE", theme::Cyan());
			}
			...
			if (st == NetworkState::Connected) {
				const bool isHost = ctx.network != nullptr && ctx.network->Role() == NetworkRole::Host;
				const float pulse = 0.5f + 0.5f * math::NkSin(mTime * 3.0f);
				...
				if (isHost) {
					...
					f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, Wf * 0.5f, y, "TU ES L'HOTE",
											   theme::Cyan());
					y += Pct::H(H, 0.042f, 22.0f, 50.0f);
					f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, Wf * 0.5f, y,
											   "CONFIGURE LE MATCH PUIS LANCE-LE.", {255, 255, 255, 200});
					y += Pct::H(H, 0.024f, 12.0f, 28.0f);
					f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, Wf * 0.5f, y,
											   "LA PARTIE DEMARRERA POUR TOI ET TON ADVERSAIRE.", {255, 255, 255, 160});
					...
					mLaunchW = math::NkMin(availW * 0.75f, Pct::W(W, 0.45f, 240.0f, 560.0f));
					...
					f.DrawStringCenteredScaled(r, FontAtlas::HeadlineSlot, scale, mLaunchX + mLaunchW * 0.5f,
											   mLaunchY + mLaunchH * 0.25f, "CONFIGURER LE MATCH",
											   {255, 255, 255, 250});
				} else {
					// CLIENT : 2 sous-etats.
					...
					if (!validated) {
						f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, Wf * 0.5f, y,
												   "EN ATTENTE DE VALIDATION DE L'HOTE...", {255, 215, 0, 240});
					} else {
						f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, Wf * 0.5f, y,
					...
					const float cardW = math::NkMin(availW * 0.8f, Pct::W(W, 0.55f, 300.0f, 720.0f));
					...
					f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, cardX + cardW * 0.5f,
											   cardY + cardH * 0.20f, "EN ATTENTE DE L'HOTE...", {255, 215, 0, 240});
					// 3 points animes
					static const char *dots[] = {".", "..", "..."};
					const int dotIdx = ((int)(mTime * 2.0f)) % 3;
					f.DrawStringCenteredScaled(r, FontAtlas::HeadlineSlot, scale, cardX + cardW * 0.5f,
											   cardY + cardH * 0.55f, dots[dotIdx], theme::Cyan());
				}
			}
		...
		void NetworkLobbyScene::OnEvent(AppContext &ctx, NkEvent &ev) {
			...
			if (auto *mr = ev.As<NkMouseButtonReleaseEvent>()) {
				...
				if (mr->GetButton() == NkMouseButton::NK_MB_LEFT) {
					...
					if (ctx.network != nullptr && ctx.network->WasRejectedByHost() &&
						...
						DoReturnAfterReject(ctx);
						return;
					}
					if (st == NetworkState::Connected && HitTest(px, py, mLaunchX, mLaunchY, mLaunchW, mLaunchH)) {
						DoLaunch(ctx);
						return;
					}
					if ((st == NetworkState::Hosting || st == NetworkState::Joining || mView == 1) &&
						HitTest(px, py, mCancelX, mCancelY, mCancelW, mCancelH)) {
						DoCancel(ctx);
						return;
					}
					...
					if (mView == 1 && st == NetworkState::Idle) {
						for (int i = 0; i < mHostBtnCount; ++i) {
							if (HitTest(px, py, mHostBtnX[i], mHostBtnY[i], mHostBtnW[i], mHostBtnH[i])) {
								DoJoinSlot(ctx, i);
								return;
							}
						}
					...
					if (st == NetworkState::Hosting) {
						for (int i = 0; i < mChalBtnCount; ++i) {
							if (HitTest(px, py, mChalBtnX[i], mChalBtnY[i], mChalBtnW[i], mChalBtnH[i])) {
			...
			if (auto *tm = ev.As<NkTouchMoveEvent>()) {
				if (tm->GetNumTouches() == 0)
					return;
				const NkTouchPoint &tp = tm->GetTouch(0);
				if ((long long)tp.id != mActiveTouchId)
					return;
			...
			if (auto *te = ev.As<NkTouchEndEvent>()) {
				...
				if (te->GetNumTouches() > 0) {
					...
					if (mView == 1 && st == NetworkState::Idle) {
						for (int i = 0; i < mHostBtnCount; ++i) {
							if (HitTest(tp.clientX, tp.clientY, mHostBtnX[i], mHostBtnY[i], mHostBtnW[i],
										mHostBtnH[i])) {
								DoJoinSlot(ctx, i);
								return;
							}
					...
					if (st == NetworkState::Hosting) {
						for (int i = 0; i < mChalBtnCount; ++i) {
							if (HitTest(tp.clientX, tp.clientY, mChalBtnX[i], mChalBtnY[i], mChalBtnW[i],
										mChalBtnH[i])) {
								DoAcceptChallenger(ctx, i);
								return;
							}

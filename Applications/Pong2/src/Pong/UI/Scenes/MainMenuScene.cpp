...
namespace nkentseu {
	namespace pong {
		...
		static inline int VisibleItemCount() {
			return kHasQuitItem ? (int)MainMenuScene::kItemCount : (int)MainMenuScene::kItemCount - 1;
		}
		...
		static MenuLayout ComputeLayout(const AppContext &ctx) {
			...
			const float headerReserveH = L.twoCols ? 0.0f : Pct::H(vH, 0.26f, 140.0f, 260.0f);
			const float availForList = L.safeH - footerReserveH - headerReserveH - L.padding * 2.0f;
			const float neededList = nSections * (L.sectionH + L.sectionGap + 4.0f) + nItems * (L.itemH + L.itemGap);
			if (neededList > availForList && nItems > 0) {
				const float overhead = nSections * (L.sectionH + L.sectionGap + 4.0f) + nItems * L.itemGap;
				const float maxItemH = (availForList - overhead) / nItems;
				if (maxItemH > 28.0f)
					L.itemH = math::NkMin(L.itemH, maxItemH);
			}
			...
			L.leftPanelH =
				logoPx + 4.0f + smallPx + 14.0f + (L.showMiniField ? (L.miniFieldH + 12.0f) : 0.0f) + smallPx;

			// Liste droite : 2 titres de section + 5 items + gaps
			L.rightListH = nSections * (L.sectionH + L.sectionGap) + nItems * (L.itemH + L.itemGap) +
			...
			if (L.twoCols) {
				...
				if (L.leftY < L.safeY + L.padding)
					L.leftY = L.safeY + L.padding;
				if (L.listY < L.safeY + L.padding)
					L.listY = L.safeY + L.padding;
			} else {
				...
				if (startY < L.safeY + L.padding)
					startY = L.safeY + L.padding;
				L.leftY = startY;
		...
		// layout responsive.
		// ─────────────────────────────────────────────────────────────────────
		static void DrawMiniField(GLRenderer2D &r, FontAtlas &f, float x, float y, float w, float h,
			...
			r.DrawCircleOutline(midX, y + h * 0.5f, ringR, {255, 255, 255, 40}, 1.0f, 32);

			// Score 0 : 0 (scaled selon la hauteur du mini-field)
			f.DrawStringCenteredScaled(r, scoreSlot, localScale, midX, y + h * 0.5f - 14.0f * localScale, "0 : 0",
									   {255, 255, 255, 220});
		...
		// ─────────────────────────────────────────────────────────────────────
		static void DrawIcon(GLRenderer2D &r, int iconId, float boxX, float boxY, float boxSize, math::NkColor c) {
			...
			switch (iconId) {
				...
				case 7: {
					...
					r.DrawLine(cx - cupW * 0.5f, cy - cupH * 0.4f, cx - cupW * 0.7f, cy - cupH * 0.1f, c, 1.5f);
					r.DrawLine(cx + cupW * 0.5f, cy - cupH * 0.4f, cx + cupW * 0.7f, cy - cupH * 0.1f, c, 1.5f);
				...
				default: {
					// Croix (Quitter)
					r.DrawLine(cx - hb * 0.45f, cy - hb * 0.45f, cx + hb * 0.45f, cy + hb * 0.45f, c, 2.0f);
					r.DrawLine(cx + hb * 0.45f, cy - hb * 0.45f, cx - hb * 0.45f, cy + hb * 0.45f, c, 2.0f);
					break;
				}
			}
		...
		static void DrawBadge(GLRenderer2D &r, FontAtlas &f, FontAtlas::SizeSlot slot, float scale, float x, float y,
							  const char *text, math::NkColor accent) {
		...
		// ─────────────────────────────────────────────────────────────────────
		static float DrawItemCard(GLRenderer2D &r, FontAtlas &f, const MenuLayout &L, int itemIndex, float baseY,
								  bool focused, float globalTime, float alphaItem) {
			...
			math::NkColor titleCol = theme::White();
			titleCol.a = static_cast<uint8>(245 * alphaItem);
			math::NkColor subCol = {255, 255, 255, 100};
			subCol.a = static_cast<uint8>(100 * alphaItem);
			f.DrawStringScaled(r, L.slotTitle, L.scale, textX, cardY + cardH * 0.18f, it.title, titleCol);
			f.DrawStringScaled(r, L.slotSmall, L.scale, textX, cardY + cardH * 0.62f, it.sub, subCol);

			// Badge optionnel
			if (it.badge != 0) {
				const char *btext = (it.badge == 1) ? "NOUVEAU" : "EN LIGNE";
				math::NkColor bcol =
					(it.badge == 1) ? math::NkColor{255, 215, 0, 255} : math::NkColor{255, 107, 0, 255};
				bcol.a = static_cast<uint8>(255 * alphaItem);
				const float bw = f.MeasureWidthScaled(L.slotSmall, L.scale, btext) + 12.0f * L.scale;
			...
			f.DrawStringScaled(r, L.slotTitle, L.scale, cardX + cardW - 20.0f * L.scale, cardY + cardH * 0.20f, ">",
							   arrowCol);
		...
		void MainMenuScene::OnRender(AppContext &ctx) {
			...
			float headerEndY = 0.0f;
			{
				...
				f.DrawStringShadowScaled(r, L.slotLogo, L.scale, hx, hy, "PONG", theme::White(), theme::Cyan(), 3);
				const float logoPx = kSlotPx[(int)L.slotLogo];
				hy += logoPx * L.scale + 4.0f * L.scale;

				f.DrawStringScaled(r, L.slotSmall, L.scale, hx, hy, "ULTRA ARENA EDITION", theme::Cyan());
				hy += kSlotPx[(int)L.slotSmall] * L.scale + 14.0f * L.scale;

				if (L.showMiniField) {
					DrawMiniField(r, f, hx, hy, L.miniFieldW, L.miniFieldH, L.slotTitle);
					hy += L.miniFieldH + 12.0f * L.scale;
				}

				f.DrawStringScaled(r, L.slotSmall, L.scale, hx, hy, "CONCU PAR RIHEN", {255, 255, 255, 80});
			...
			for (int i = 0; i < visibleCount; ++i) {
				...
				if (it.sectionIdx != prevSection) {
					if (prevSection >= 0)
						ry += 4.0f * L.scale;
					f.DrawStringScaled(r, L.slotSmall, L.scale, L.listX, ry, kSections[it.sectionIdx].title,
									   {255, 255, 255, 90});
				...
				if (i < (int)MainMenuScene::kItemCount)
					mCardItemYs[i] = ry;
			...
			f.DrawStringCenteredScaled(
				r, FontAtlas::SmallSlot, L.scale, L.safeX + L.safeW * 0.5f, L.safeY + L.safeH - hintOffsetY,
				"FLECHES : NAV       ENTER : SELECT       ECHAP : QUITTER       TAP / CLIC : SELECT", hintCol);
		...
		void MainMenuScene::OnEvent(AppContext &ctx, NkEvent &ev) {
			...
			if (auto *kp = ev.As<NkKeyPressEvent>()) {
				...
				} else if (k == NkKey::NK_ENTER || k == NkKey::NK_NUMPAD_ENTER || k == NkKey::NK_SPACE) {
			...
			if (auto *mp = ev.As<NkMouseButtonPressEvent>()) {
				if (mp->GetButton() == NkMouseButton::NK_MB_LEFT) {
					const int idx = HitTestItem((float)mp->GetX(), (float)mp->GetY());
					if (idx >= 0) {
						mFocusIndex = idx;
		...
		void MainMenuScene::ActivateItem(AppContext &ctx, ItemId item) {
			switch (item) {
				...
				default:
					logger.Info("[MainMenu] Item activated (TODO): {0}", kItems[item].title);
					break;
			}
		}

...
namespace nkentseu {
	namespace pong {
		...
		static void DrawObsIcon(GLRenderer2D &r, int iconId, float bx, float by, float bs, math::NkColor c) {
				...
				case 0: {
					...
					for (int row = 0; row < 2; ++row)
						for (int col = 0; col < 3; ++col) {
							const float xx = cx - bw * 1.5f + col * bw + ((row & 1) ? bw * 0.5f : 0.0f);
							const float yy = cy - bh + row * bh;
							r.DrawQuadOutline(xx, yy, bw - 1.0f, bh - 1.0f, c, 1.0f);
						}
					break;
				}
				case 1:
					r.DrawCircleOutline(cx, cy, hb * 0.6f, c, 2.0f, 32);
					r.DrawCircle(cx, cy, hb * 0.2f, c, 16);
					break;
				case 2:
					for (int i = 0; i < 3; ++i)
						r.DrawCircleOutline(cx, cy, hb * (0.25f + 0.18f * i), c, 1.5f, 32);
					r.DrawCircle(cx, cy, 2.5f, c, 10);
					break;
				case 3:
					for (int i = 0; i < 3; ++i) {
						const float yy = cy - 6.0f + i * 6.0f;
						r.DrawQuad(cx - hb * 0.5f, yy, hb, 1.5f, c);
					}
					break;
				case 4:
					r.DrawTriangle(cx, cy - hb * 0.5f, cx + hb * 0.45f, cy, cx, cy + hb * 0.5f, c);
					r.DrawTriangle(cx, cy - hb * 0.5f, cx - hb * 0.45f, cy, cx, cy + hb * 0.5f, c);
					break;
				case 5:
					r.DrawCircle(cx, cy, hb * 0.35f, c, 16);
					for (int i = 0; i < 8; ++i) {
						const float a = 6.28318f * i / 8.0f;
						const float r1 = hb * 0.40f;
						const float r2 = hb * 0.55f;
						r.DrawLine(cx + math::NkCos(a) * r1, cy + math::NkSin(a) * r1, cx + math::NkCos(a) * r2,
								   cy + math::NkSin(a) * r2, c, 1.5f);
					}
					break;
				case 6:
					r.DrawQuad(cx - hb * 0.4f, cy - hb * 0.3f, hb * 0.2f, hb * 0.6f, c);
					r.DrawQuad(cx + hb * 0.2f, cy - hb * 0.3f, hb * 0.2f, hb * 0.6f, c);
					r.DrawQuad(cx - hb * 0.4f, cy + hb * 0.20f, hb * 0.8f, hb * 0.1f, c);
					break;
				case 7:
				default: {
					...
					for (int i = 0; i < 5; ++i) {
						...
						r.DrawLine(cx + math::NkCos(a1) * rad, cy + math::NkSin(a1) * rad, cx + math::NkCos(a2) * rad,
								   cy + math::NkSin(a2) * rad, c, 1.5f);
					}
					break;
				}
			}
		...
		static const int kScoreOptionsN = (int)(sizeof(kScoreOptions) / sizeof(kScoreOptions[0]));
		// timeLimit (en secondes) : 0 = chrono ascendant sans limite,
		// sinon presets classiques (1min, 1m30, 2min, ..., 5min).
		static const float kTimeOptions[] = {0.0f, 60.0f, 90.0f, 120.0f, 154.0f, 180.0f, 240.0f, 300.0f};
		static const int kTimeOptionsN = (int)(sizeof(kTimeOptions) / sizeof(kTimeOptions[0]));
		...
		static int FindTimeIndex(float t) {
			...
			for (int i = 0; i < kTimeOptionsN; ++i) {
				const float d = (kTimeOptions[i] > t) ? (kTimeOptions[i] - t) : (t - kTimeOptions[i]);
				if (d < best) {
					best = d;
					bi = i;
				}
		...
		static void ApplyParamButton(int btn, int &scoreIndex, int &timeIndex, bool &winByTwo, int &speedIndex);

		static void FormatScore(int score, char *out, int outSize) {
			if (score <= 0)
				std::snprintf(out, outSize, "ILLIMITE");
			else
				std::snprintf(out, outSize, "%d PTS", score);
		}

		static void FormatTime(float t, char *out, int outSize) {
			if (t <= 0.001f) {
				std::snprintf(out, outSize, "AUCUN");
				return;
			}
		...
		static int FindSpeedIndex(float mul) {
			...
			int bi = 0;
			for (int i = 0; i < kSpeedOptionsN; ++i) {
				const float d = (kSpeedOptions[i] > mul) ? (kSpeedOptions[i] - mul) : (mul - kSpeedOptions[i]);
				if (d < best) {
					best = d;
					bi = i;
				}
		...
		void SelectMatchConfigScene::OnEnter(AppContext &ctx) {
			...
			if (ctx.settings != nullptr) {
				...
				mShowDifficulty = (m == GameMode::VsAI || m == GameMode::AIvsAI);
			...
			logger.Info("[MatchConfig] OnEnter showDiff={0}", mShowDifficulty ? 1 : 0);
		}
			...
			if (mArmedStepper > 0) {
				...
				if (mHoldTime > kHoldDelay) {
					...
					while (mRepeatAccum >= kRepeatInterval) {
						...
						ApplyParamButton(mArmedStepper, mScoreIndex, mTimeIndex, mWinByTwo, mSpeedIndex);
					}
				}
			}
		}
		...
		void SelectMatchConfigScene::OnRender(AppContext &ctx) {
			...
			for (float x = 0; x < (float)W; x += step)
				r.DrawQuad(x, 0.0f, 1.0f, (float)H, grid);
			for (float y = 0; y < (float)H; y += step)
				r.DrawQuad(0.0f, y, (float)W, 1.0f, grid);
			...
			{
				f.DrawStringScaled(r, FontAtlas::SubtitleSlot, scale, gridLeft, worldY - mScrollY + scrollTop,
				...
				// Ligne 1 : POINTS POUR GAGNER
				{
					...
					if (screenLY + lineH >= scrollTop - 4.0f && screenLY <= scrollBot + 4.0f) {
						f.DrawStringScaled(r, FontAtlas::BodySlot, scale, gridLeft, screenLY + lineH * 0.30f,
										   "POINTS POUR GAGNER", {255, 255, 255, 220});
						// Stepper a droite
						const float rxStart = gridLeft + availW - (btnW * 2 + valW);
						// Boutons -
						mScoreMinusX = rxStart;
						mScoreMinusY = ly + (lineH - btnH) * 0.5f;
						r.DrawQuad(mScoreMinusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH, {0, 245, 255, 40});
						r.DrawQuadOutline(mScoreMinusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH,
										  {0, 245, 255, 200}, 1.5f);
						f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, mScoreMinusX + btnW * 0.5f,
												   screenLY + (lineH - btnH) * 0.5f + btnH * 0.18f, "-", theme::Cyan());
						// Valeur centrale
						char buf[24];
						FormatScore(kScoreOptions[mScoreIndex], buf, sizeof(buf));
						f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, rxStart + btnW + valW * 0.5f,
												   screenLY + lineH * 0.30f, buf, theme::White());
						// Bouton +
						mScorePlusX = rxStart + btnW + valW;
						mScorePlusY = mScoreMinusY;
						r.DrawQuad(mScorePlusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH, {0, 245, 255, 40});
						r.DrawQuadOutline(mScorePlusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH, {0, 245, 255, 200},
										  1.5f);
						f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, mScorePlusX + btnW * 0.5f,
												   screenLY + (lineH - btnH) * 0.5f + btnH * 0.18f, "+", theme::Cyan());
					}
					worldY += lineH;
				}
				...
				// Ligne 2 : TEMPS LIMITE
				{
					...
					if (screenLY + lineH >= scrollTop - 4.0f && screenLY <= scrollBot + 4.0f) {
						f.DrawStringScaled(r, FontAtlas::BodySlot, scale, gridLeft, screenLY + lineH * 0.30f,
										   "TEMPS LIMITE", {255, 255, 255, 220});
						const float rxStart = gridLeft + availW - (btnW * 2 + valW);
						mTimeMinusX = rxStart;
						mTimeMinusY = ly + (lineH - btnH) * 0.5f;
						r.DrawQuad(mTimeMinusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH, {0, 245, 255, 40});
						r.DrawQuadOutline(mTimeMinusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH, {0, 245, 255, 200},
										  1.5f);
						f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, mTimeMinusX + btnW * 0.5f,
												   screenLY + (lineH - btnH) * 0.5f + btnH * 0.18f, "-", theme::Cyan());
						char buf[24];
						...
						r.DrawQuad(mTimePlusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH, {0, 245, 255, 40});
						r.DrawQuadOutline(mTimePlusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH, {0, 245, 255, 200},
										  1.5f);
						f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, mTimePlusX + btnW * 0.5f,
												   screenLY + (lineH - btnH) * 0.5f + btnH * 0.18f, "+", theme::Cyan());
					}
					worldY += lineH;
				}
				...
				// Ligne 3 : VITESSE BALLE (stepper multiplicateur >= 1)
				{
					...
					if (screenLY + lineH >= scrollTop - 4.0f && screenLY <= scrollBot + 4.0f) {
						f.DrawStringScaled(r, FontAtlas::BodySlot, scale, gridLeft, screenLY + lineH * 0.30f,
										   "VITESSE BALLE", {255, 255, 255, 220});
						const float rxStart = gridLeft + availW - (btnW * 2 + valW);
						mSpeedMinusX = rxStart;
						mSpeedMinusY = ly + (lineH - btnH) * 0.5f;
						r.DrawQuad(mSpeedMinusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH, {0, 245, 255, 40});
						r.DrawQuadOutline(mSpeedMinusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH,
										  {0, 245, 255, 200}, 1.5f);
						f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, mSpeedMinusX + btnW * 0.5f,
												   screenLY + (lineH - btnH) * 0.5f + btnH * 0.18f, "-", theme::Cyan());
						char buf[24];
						...
						r.DrawQuad(mSpeedPlusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH, {0, 245, 255, 40});
						r.DrawQuadOutline(mSpeedPlusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH, {0, 245, 255, 200},
										  1.5f);
						f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, mSpeedPlusX + btnW * 0.5f,
												   screenLY + (lineH - btnH) * 0.5f + btnH * 0.18f, "+", theme::Cyan());
					}
					worldY += lineH;
				}
				...
				// Ligne 4 : VICTOIRE AVEC 2 PTS D'ECART (toggle)
				{
					...
					if (screenLY + lineH >= scrollTop - 4.0f && screenLY <= scrollBot + 4.0f) {
						f.DrawStringScaled(r, FontAtlas::BodySlot, scale, gridLeft, screenLY + lineH * 0.30f,
										   "VICTOIRE A 2 PTS D'ECART", {255, 255, 255, 220});
						...
						math::NkColor bg =
							mWinByTwo ? math::NkColor{0, 245, 255, 80} : math::NkColor{255, 255, 255, 30};
						math::NkColor bd =
							mWinByTwo ? math::NkColor{0, 245, 255, 220} : math::NkColor{255, 255, 255, 100};
						r.DrawQuad(tx, screenTY, toggleW, toggleH, bg);
						r.DrawQuadOutline(tx, screenTY, toggleW, toggleH, bd, 1.5f);
						f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, tx + toggleW * 0.5f,
												   screenTY + toggleH * 0.20f, mWinByTwo ? "ON" : "OFF",
												   mWinByTwo ? theme::Cyan() : math::NkColor{255, 255, 255, 160});
					}
					worldY += lineH;
				}
			...
			auto renderGrid = [&](const char *title, int currentIndex, float *outX, float *outY) {
				f.DrawStringScaled(r, FontAtlas::SubtitleSlot, scale, gridLeft, worldY - mScrollY + scrollTop, title,
								   sectionCol);
				worldY += Pct::H(H, 0.052f, 28.0f, 56.0f);
				...
				for (int i = 0; i < kDiffCount; ++i) {
					...
					if (screenBY + cardH < scrollTop - 4.0f)
						continue;
					if (screenBY > scrollBot + 4.0f)
						continue;
					...
					math::NkColor bg = d.color;
					bg.a = focused ? 50 : 14;
					math::NkColor bord = d.color;
					bord.a = focused ? 220 : 70;
					if (focused) {
					...
					r.DrawQuad(bx, screenBY, cardW, cardH, bg);
					r.DrawQuadOutline(bx, screenBY, cardW, cardH, bord, focused ? 2.5f * scale : 1.0f);
					...
					f.DrawStringShadowScaled(r, FontAtlas::HeadlineSlot, scale, bx + padX, screenBY + rankY, d.rank,
											 rankCol, d.color, 1);

					math::NkColor nameCol = d.color;
					nameCol.a = static_cast<uint8>(255 * a);
					f.DrawStringScaled(r, FontAtlas::BodySlot, scale, bx + padX, screenBY + nameY, d.name, nameCol);
					...
					for (int dn = 0; dn < 5; ++dn) {
						...
						if (dn < d.dots) {
							...
							r.DrawCircle(ddx, dotsY, dotR, dotC, 12);
						} else {
					...
					f.DrawStringScaled(r, FontAtlas::SmallSlot, scale, bx + padX, screenBY + descY, d.desc, descCol);
				}
			...
			if (mShowDifficulty) {
				renderGrid(mShowDifficultyR ? "DIFFICULTE  IA  GAUCHE" : "DIFFICULTE  IA", mDiffIndex, mDiffX, mDiffY);
			}
			if (mShowDifficultyR) {
				renderGrid("DIFFICULTE  IA  DROITE", mDiffIndexR, mDiffXR, mDiffYR);
			}

			// Section OBSTACLES (toujours visible)
			f.DrawStringScaled(r, FontAtlas::SubtitleSlot, scale, gridLeft, worldY - mScrollY + scrollTop,
							   "OBSTACLES  &  TERRAIN", sectionCol);
			worldY += Pct::H(H, 0.052f, 28.0f, 56.0f);

			{
				...
				for (int i = 0; i < kObsCount; ++i) {
					...
					bord.a = static_cast<uint8>(bord.a * a);
					r.DrawQuad(bx, screenBY, cardW, cardH, bg);
					r.DrawQuadOutline(bx, screenBY, cardW, cardH, bord, 1.5f * scale);

					const float iconSize = cardH * 0.35f;
					const float iconX = bx + (cardW - iconSize) * 0.5f;
					const float iconY = screenBY + cardH * 0.07f;
					math::NkColor iconC = active ? d.color : math::NkColor{255, 255, 255, 80};
					...
					r.DrawQuad(cx2, cy2, cs, cs, cbg);
					r.DrawQuadOutline(cx2, cy2, cs, cs, cbd, 1.5f * scale);
					if (active) {
						// Coche
						math::NkColor white = {0, 0, 0, 230};
						white.a = static_cast<uint8>(230 * a);
						const float pad = cs * 0.22f;
						r.DrawLine(cx2 + pad, cy2 + cs * 0.5f, cx2 + cs * 0.45f, cy2 + cs - pad, white, 2.0f);
						r.DrawLine(cx2 + cs * 0.45f, cy2 + cs - pad, cx2 + cs - pad, cy2 + pad, white, 2.0f);
					}

					math::NkColor nameC =
						active ? math::NkColor{255, 255, 255, 245} : math::NkColor{255, 255, 255, 100};
					nameC.a = static_cast<uint8>(nameC.a * a);
					f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, bx + cardW * 0.5f,
											   screenBY + cardH * 0.58f, d.name, nameC);
					math::NkColor descC = {255, 255, 255, 130};
					descC.a = static_cast<uint8>(descC.a * a);
					f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, bx + cardW * 0.5f,
											   screenBY + cardH * 0.78f, d.desc, descC);
				}
				worldY += rows * (cardH + pad);
			...
			// Bouton RETOUR (haut-gauche) — dimensions en % viewport
			{
				...
				f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, mBackX + bw * 0.5f, mBackY + bh * 0.30f,
										   "< RETOUR", theme::Cyan());
			}

			// Titre + sous-titre centres en haut. Y exprime en % de H pour
			// suivre la hauteur de mTopReserve.
			f.DrawStringShadowCenteredScaled(r, FontAtlas::HeadlineSlot, scale, cx,
											 safeY + Pct::H(H, 0.028f, 14.0f, 32.0f), "CONFIGURATION DE LA PARTIE",
											 theme::White(), theme::Cyan(), 3);
			f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, cx, safeY + Pct::H(H, 0.085f, 50.0f, 100.0f),
									   mShowDifficulty ? "CHOISIS UNE DIFFICULTE ET CONFIGURE LES OBSTACLES"
			...
			math::NkColor lbg = theme::Cyan();
			lbg.a = 50;
			math::NkColor lbd = theme::Cyan();
			lbd.a = 230;
			r.DrawQuad(mLaunchX, mLaunchY, launchW, launchH, lbg);
			r.DrawQuadOutline(mLaunchX, mLaunchY, launchW, launchH, lbd, 2.0f * scale);
			f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, cx, mLaunchY + launchH * 0.30f,
			...
			if (mEditingObsIndex >= 0 && mEditingObsIndex < kObsCount) {
				...
				const float pW = math::NkMin(safeW * 0.85f, Pct::W(W, 0.55f, 320.0f, 600.0f));
				const float pH = Pct::H(H, 0.55f, 280.0f, 460.0f);
				mEditPanelW = pW;
				mEditPanelH = pH;
				mEditPanelX = cx - pW * 0.5f;
				mEditPanelY = safeY + (safeH - pH) * 0.5f;
				r.DrawQuad(mEditPanelX, mEditPanelY, pW, pH, {8, 16, 28, 250});
				r.DrawQuadOutline(mEditPanelX, mEditPanelY, pW, pH, d.color, 2.0f * scale);

				// Titre + close (offsets relatifs au panneau)
				const float panelPadX = pW * 0.05f;
				const float panelPadY = pH * 0.045f;
				f.DrawStringShadowScaled(r, FontAtlas::SubtitleSlot, scale, mEditPanelX + panelPadX,
										 mEditPanelY + panelPadY, d.name, d.color, d.color, 1);
				...
				mEditBtnH = closeS;
				r.DrawQuad(mEditCloseX, mEditCloseY, closeS, closeS, {255, 64, 64, 60});
				r.DrawQuadOutline(mEditCloseX, mEditCloseY, closeS, closeS, {255, 64, 64, 200}, 1.5f);
				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, mEditCloseX + closeS * 0.5f,
										   mEditCloseY + closeS * 0.18f, "x", {255, 64, 64, 240});
				...
				// NOMBRE
				f.DrawStringScaled(r, FontAtlas::BodySlot, scale, lx, ly + lineH * 0.28f, "NOMBRE D'ELEMENTS",
								   {255, 255, 255, 220});
				mEditCountMinusX = rx;
				mEditCountMinusY = ly + (lineH - bh) * 0.5f;
				r.DrawQuad(mEditCountMinusX, mEditCountMinusY, bw, bh, {0, 245, 255, 40});
				r.DrawQuadOutline(mEditCountMinusX, mEditCountMinusY, bw, bh, {0, 245, 255, 200}, 1.5f);
				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, mEditCountMinusX + bw * 0.5f,
										   mEditCountMinusY + bh * 0.20f, "-", theme::Cyan());
				char nbuf[8];
				std::snprintf(nbuf, sizeof(nbuf), "%d", mEditCount);
				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, rx + bw + valW * 0.5f, ly + lineH * 0.28f,
										   nbuf, theme::White());
				mEditCountPlusX = rx + bw + valW;
				mEditCountPlusY = mEditCountMinusY;
				r.DrawQuad(mEditCountPlusX, mEditCountPlusY, bw, bh, {0, 245, 255, 40});
				r.DrawQuadOutline(mEditCountPlusX, mEditCountPlusY, bw, bh, {0, 245, 255, 200}, 1.5f);
				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, mEditCountPlusX + bw * 0.5f,
										   mEditCountPlusY + bh * 0.20f, "+", theme::Cyan());
				ly += lineH;

				// FORCE
				f.DrawStringScaled(r, FontAtlas::BodySlot, scale, lx, ly + lineH * 0.28f, "FORCE",
								   {255, 255, 255, 220});
				mEditPowerMinusX = rx;
				mEditPowerMinusY = ly + (lineH - bh) * 0.5f;
				r.DrawQuad(mEditPowerMinusX, mEditPowerMinusY, bw, bh, {0, 245, 255, 40});
				r.DrawQuadOutline(mEditPowerMinusX, mEditPowerMinusY, bw, bh, {0, 245, 255, 200}, 1.5f);
				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, mEditPowerMinusX + bw * 0.5f,
										   mEditPowerMinusY + bh * 0.20f, "-", theme::Cyan());
				const char *pwlbl = (mEditPowerLevel == 1) ? "FAIBLE" : (mEditPowerLevel == 3) ? "ELEVEE" : "NORMALE";
				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, rx + bw + valW * 0.5f, ly + lineH * 0.28f,
										   pwlbl, theme::White());
				mEditPowerPlusX = rx + bw + valW;
				mEditPowerPlusY = mEditPowerMinusY;
				r.DrawQuad(mEditPowerPlusX, mEditPowerPlusY, bw, bh, {0, 245, 255, 40});
				r.DrawQuadOutline(mEditPowerPlusX, mEditPowerPlusY, bw, bh, {0, 245, 255, 200}, 1.5f);
				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, mEditPowerPlusX + bw * 0.5f,
										   mEditPowerPlusY + bh * 0.20f, "+", theme::Cyan());
				ly += lineH;

				// IMPREVISIBLE (toggle)
				f.DrawStringScaled(r, FontAtlas::BodySlot, scale, lx, ly + lineH * 0.28f, "IMPREVISIBLE (BOUGE)",
								   {255, 255, 255, 220});
				...
				mEditChaoticW = tW;
				mEditChaoticH = tH;
				math::NkColor tbg = mEditChaotic ? math::NkColor{0, 245, 255, 80} : math::NkColor{255, 255, 255, 30};
				math::NkColor tbd = mEditChaotic ? math::NkColor{0, 245, 255, 220} : math::NkColor{255, 255, 255, 100};
				r.DrawQuad(mEditChaoticX, mEditChaoticY, tW, tH, tbg);
				r.DrawQuadOutline(mEditChaoticX, mEditChaoticY, tW, tH, tbd, 1.5f);
				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, mEditChaoticX + tW * 0.5f,
										   mEditChaoticY + tH * 0.20f, mEditChaotic ? "ON" : "OFF",
										   mEditChaotic ? theme::Cyan() : math::NkColor{255, 255, 255, 160});
				ly += lineH;

				// Bouton APPLIQUER (centre bas du panneau)
				const float aW = math::NkMin(pW * 0.6f, Pct::W(W, 0.25f, 160.0f, 280.0f));
				...
				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, mEditApplyX + aW * 0.5f,
										   mEditApplyY + aH * 0.28f, "APPLIQUER", theme::Cyan());
			}
			...
			if (mMaxScroll > 0.5f) {
				...
				const float thumbH = math::NkMax(Pct::H(H, 0.030f, 18.0f, 40.0f), sbH * frac);
				const float thumbY = sbY + (sbH - thumbH) * (mScrollY / mMaxScroll);
				r.DrawQuad(sbX, thumbY, sbW, thumbH, {0, 245, 255, 180});
			}
		...
		int SelectMatchConfigScene::HitTestDiff(float worldX, float worldY) const {
			if (!mShowDifficulty)
				return -1;
			for (int i = 0; i < kDiffCount; ++i) {
				if (worldX >= mDiffX[i] && worldX <= mDiffX[i] + mDiffW && worldY >= mDiffY[i] &&
					worldY <= mDiffY[i] + mDiffH)
					return i;
			}
			return -1;
		}
		...
		int SelectMatchConfigScene::HitTestDiffR(float worldX, float worldY) const {
			if (!mShowDifficultyR)
				return -1;
			for (int i = 0; i < kDiffCount; ++i) {
				if (worldX >= mDiffXR[i] && worldX <= mDiffXR[i] + mDiffW && worldY >= mDiffYR[i] &&
					worldY <= mDiffYR[i] + mDiffH)
					return i;
			}
			return -1;
		}
		...
		int SelectMatchConfigScene::HitTestObs(float worldX, float worldY) const {
			for (int i = 0; i < kObsCount; ++i) {
				if (worldX >= mObsX[i] && worldX <= mObsX[i] + mObsW && worldY >= mObsY[i] &&
					worldY <= mObsY[i] + mObsH)
					return i;
			}
			return -1;
		}
		...
		bool SelectMatchConfigScene::HitTestLaunch(float screenX, float screenY) const {
			return screenX >= mLaunchX && screenX <= mLaunchX + mLaunchW && screenY >= mLaunchY &&
				   screenY <= mLaunchY + mLaunchH;
		}

		bool SelectMatchConfigScene::HitTestBack(float screenX, float screenY) const {
			return screenX >= mBackX && screenX <= mBackX + mBackW && screenY >= mBackY && screenY <= mBackY + mBackH;
		}

		bool SelectMatchConfigScene::HitTestObsCheck(float worldX, float worldY, int &outIndex) const {
			for (int i = 0; i < kObsCount; ++i) {
				if (worldX >= mObsCheckX[i] && worldX <= mObsCheckX[i] + mObsCheckW && worldY >= mObsCheckY[i] &&
					worldY <= mObsCheckY[i] + mObsCheckH) {
					outIndex = i;
					return true;
				}
			}
		...
		int SelectMatchConfigScene::HitTestEdit(float sx, float sy) const {
			...
			if (in(mEditCountMinusX, mEditCountMinusY, mEditBtnW, mEditBtnH))
				return 1;
			if (in(mEditCountPlusX, mEditCountPlusY, mEditBtnW, mEditBtnH))
				return 2;
			if (in(mEditPowerMinusX, mEditPowerMinusY, mEditBtnW, mEditBtnH))
				return 3;
			if (in(mEditPowerPlusX, mEditPowerPlusY, mEditBtnW, mEditBtnH))
				return 4;
			if (in(mEditChaoticX, mEditChaoticY, mEditChaoticW, mEditChaoticH))
				return 5;
			if (in(mEditCloseX, mEditCloseY, mEditBtnH, mEditBtnH))
				return 6;
			if (in(mEditApplyX, mEditApplyY, mEditApplyW, mEditApplyH))
				return 7;
			return 0;
		}
		...
		int SelectMatchConfigScene::HitTestParam(float wx, float wy) const {
			...
			if (inRect(mScoreMinusX, mScoreMinusY, mStepperW, mStepperH))
				return 1;
			if (inRect(mScorePlusX, mScorePlusY, mStepperW, mStepperH))
				return 2;
			if (inRect(mTimeMinusX, mTimeMinusY, mStepperW, mStepperH))
				return 3;
			if (inRect(mTimePlusX, mTimePlusY, mStepperW, mStepperH))
				return 4;
			if (inRect(mTogglePtsX, mTogglePtsY, mToggleW, mToggleH))
				return 5;
			if (inRect(mSpeedMinusX, mSpeedMinusY, mStepperW, mStepperH))
				return 6;
			if (inRect(mSpeedPlusX, mSpeedPlusY, mStepperW, mStepperH))
				return 7;
			return 0;
		}
		...
		void SelectMatchConfigScene::Launch(AppContext &ctx) {
			...
			logger.Info("[MatchConfig] Launch (diff={0}, {1}/8 obs, score={2}, time={3})",
						mShowDifficulty ? mDiffIndex : -1, countOn, kScoreOptions[mScoreIndex],
			...
			if (ctx.network != nullptr && ctx.network->State() == NetworkState::Connected &&
				ctx.network->Role() == NetworkRole::Host && ctx.settings != nullptr) {
				...
				uint8 flags = 0;
				if (ctx.settings->winByTwo)
					flags |= netproto::kStartFlagWinByTwo;
				if (ctx.settings->powerUpsEnabled)
					flags |= netproto::kStartFlagPowerUpsOn;
				pkt.flags = flags;
		...
		void SelectMatchConfigScene::OnEvent(AppContext &ctx, NkEvent &ev) {
			...
			if (auto *kp = ev.As<NkKeyPressEvent>()) {
				...
				if (k == NkKey::NK_DOWN)
					mScrollY = ClampScroll(mScrollY + scrollStep);
						...
						if (mEditingObsIndex >= 0) {
							...
							switch (eb) {
								...
								case 2:
									...
									return;
								case 3:
									if (mEditPowerLevel > 1)
										mEditPowerLevel--;
									return;
								case 4:
									if (mEditPowerLevel < 3)
										mEditPowerLevel++;
									return;
								case 5:
									mEditChaotic = !mEditChaotic;
									return;
								case 6:
									CloseEditPanel();
									return;
			...
			if (auto *mm = ev.As<NkMouseMoveEvent>()) {
				if (mMouseDown && mDragActive) {
					...
					if (math::NkFabs(py - mDragStartY) > 6.0f)
						mDragWasScroll = true;
			...
			if (auto *tm = ev.As<NkTouchMoveEvent>()) {
				...
				for (uint32 i = 0; i < tm->GetNumTouches(); ++i) {
					...
					if ((long long)tp.id != mDragTouchId)
						continue;

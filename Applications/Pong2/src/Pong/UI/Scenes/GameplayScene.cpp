...
namespace nkentseu {
	namespace pong {
		...
		// Rendu
		// ─────────────────────────────────────────────────────────────────────
		static void DrawArenaField(GLRenderer2D &r, float ax, float ay, float aw, float ah, float scale) {
		...
		static void DrawHUDTop(GLRenderer2D &r, FontAtlas &f, int W, float hudH, float scale, int scoreL, int scoreR,
							   float timeShown, int maxScore, bool countdown) {
			...
			r.DrawCircle(gx + avatarR, cyH, avatarR2, {0, 245, 255, 38}, 32);
			...
			r.DrawCircle(dx - avatarR, cyH, avatarR2, {255, 107, 0, 38}, 32);
		...
		static void DrawHUDBottom(GLRenderer2D &r, FontAtlas &f, int W, int H, float hudH, float scale, bool paused) {
		...
		// ─────────────────────────────────────────────────────────────────────
		static void DrawBallAndTrail(GLRenderer2D &r, float ax, float ay, float bx, float by, float br,
		...
		// ─────────────────────────────────────────────────────────────────────
		static void DrawPaddle(GLRenderer2D &r, float x, float y, float w, float h, math::NkColor c) {
			...
			for (int i = 6; i >= 1; --i) {
				...
				r.DrawQuad(x - pad, y - pad, w + pad * 2.0f, h + pad * 2.0f, glow);
			}
			...
			r.DrawQuad(x + 1.0f, y + 1.0f, w - 2.0f, h * 0.35f, {255, 255, 255, 50});
		}
		...
		void GameplayScene::OnRender(AppContext &ctx) {
			...
			r.Clear(theme::Dark().r / 255.0f, theme::Dark().g / 255.0f, theme::Dark().b / 255.0f, 1.0f);
			r.Begin(W, H);
			...
			if (ah > 0.0f) {
				...
				if (mPowerUps.IsBlind(-1)) {
					r.DrawQuad(ax, ay, aw * 0.5f, ah, {0, 0, 0, 180});
				}
				if (mPowerUps.IsBlind(+1)) {
					r.DrawQuad(ax + aw * 0.5f, ay, aw * 0.5f, ah, {0, 0, 0, 180});
				}
				...
				// gauche sont alignes a gauche, ceux du joueur droit a droite.
				{
					...
					for (int i = 0; i < mPowerUps.EffectCount(); ++i) {
						...
						r.DrawCircle(bx + 8.0f * mScale + dotR, by + badgeH * 0.5f, dotR, fg, 12);
						// Texte nom (court)
						...
						r.DrawQuad(bx + 2.0f * mScale, by + badgeH - barH - 2.0f, barW, barH, {255, 255, 255, 40});
						r.DrawQuad(bx + 2.0f * mScale, by + badgeH - barH - 2.0f, barW * frac, barH, bd);
						++cnt;
					}
				}
			...
			// les avatars/scores P1/P2 (extremites).
			{
				...
				if (mPaused) {
					...
					r.DrawTriangle(ix - tw * 0.5f, iy - ih, ix - tw * 0.5f, iy + ih, ix + tw * 0.5f, iy, ColP1());
				} else {
					// 2 barres pause
			...
			if (mNetPeerLost) {
				...
				r.DrawQuad(0.0f, 0.0f, W, H, {0, 0, 0, (uint8)(180 * fade)});
				...
				r.DrawQuad(px, py, panelW, panelH, {24, 28, 36, (uint8)(230 * fade)});
				r.DrawQuadOutline(px, py, panelW, panelH, {255, 80, 80, (uint8)(220 * fade)}, 2.0f * mScale);
				// Titre
				...
				r.DrawQuad(mLostMenuBtnX, mLostMenuBtnY, mLostMenuBtnW, mLostMenuBtnH,
						   {255, 64, 64, (uint8)(60 * fade)});
				r.DrawQuadOutline(mLostMenuBtnX, mLostMenuBtnY, mLostMenuBtnW, mLostMenuBtnH,

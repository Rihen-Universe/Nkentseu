...
namespace nkentseu {
	namespace pong {
		...
		void NogeIntroScene::OnEnter(AppContext & /*ctx*/) {
			...
			mRihenLogoLoaded = mRihenLogo.LoadFromFile("Resources/Pong/Textures/logo.svg");
		}
		...
		// ─────────────────────────────────────────────────────────────────────
		static void DrawNeonHexagon(GLRenderer2D &r, float cx, float cy, float radius, float progress,
									math::NkColor primary, math::NkColor glow) {
		...
		void NogeIntroScene::OnRender(AppContext &ctx) {
			...
			if (t < kT_HexDraw)
				hexProgress = t / kT_HexDraw;
			// Rotation lente apres traçage complet
			(void)hexProgress;
			DrawNeonHexagon(r, cx, hexY, hexR, hexProgress, theme::Cyan(), AlphaF(theme::Cyan(), 0.40f));
			...
			if (t > kT_HexDraw) {
				...
				r.DrawTriangle(cx, hexY - dSize, cx + dSize, hexY, cx, hexY + dSize, AlphaF(theme::Cyan(), 0.95f));
				r.DrawTriangle(cx, hexY - dSize, cx, hexY + dSize, cx - dSize, hexY, AlphaF(theme::Cyan(), 0.95f));
			}
			...
			f.DrawStringCentered(r, FontAtlas::SmallSlot, cx, hexY + hexR + 24.0f * scale, "POWERED  BY",
								 AlphaF(theme::White(), textAlpha * 0.45f));

			// "NOGE" en grand (subtitle slot, blanc + glow cyan)
			const char *noge = "NOGE";
			f.DrawStringShadowCentered(r, FontAtlas::HeadlineSlot, cx, hexY + hexR + 44.0f * scale, noge,
									   AlphaF(theme::White(), textAlpha), AlphaF(theme::Cyan(), textAlpha), 2);

			// "GAME ENGINE" sous-titre cyan
			f.DrawStringCentered(r, FontAtlas::BodySlot, cx, hexY + hexR + 110.0f * scale, "GAME  ENGINE",
			...
			if (mRihenLogoLoaded) {
				...
				r.DrawTexturedQuadRGBA(logoX, logoY, logoW, logoH, 0.0f, 0.0f, 1.0f, 1.0f,
									   AlphaF(theme::White(), textAlpha * 0.85f));
			} else {
				// Fallback texte si le bitmap n'est pas dispo.
				f.DrawStringCentered(r, FontAtlas::SmallSlot, cx, (float)H - 28.0f, "DESIGNED BY RIHEN  -  ENGINE v0.1",
			...
			if (t >= kT_Hold) {
				...
				if (a > 1.0f)
					a = 1.0f;
				r.DrawQuad(0.0f, 0.0f, (float)W, (float)H, AlphaF(theme::Dark(), a));
			}

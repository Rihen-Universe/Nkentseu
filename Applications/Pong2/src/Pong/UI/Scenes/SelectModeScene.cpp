...
namespace nkentseu {
	namespace pong {
		...
		static const ModeDesc kModes[SelectModeScene::kModeCount] = {
			{GameMode::Local, "JOUER EN LOCAL", "1V1 - MEME ECRAN, 2 JOUEURS", 0, {0, 245, 255, 255}, true},
			{GameMode::NetworkLAN,
			 "MULTIJOUEUR RESEAU",
			 "1V1 EN LAN — HEBERGER OU REJOINDRE",
			 1,
			 {255, 107, 0, 255},
			 true},
			{GameMode::VsAI, "VS IA", "JOUER CONTRE L'ORDINATEUR", 2, {0, 245, 255, 255}, true},
			{GameMode::AIvsAI, "IA VS IA", "MODE DEMO - 2 IA S'AFFRONTENT", 3, {204, 119, 255, 255}, true},
		};

		// ── Icones simples (style identique aux icones MainMenu) ─────────────
		static void DrawModeIcon(GLRenderer2D &r, int iconId, float boxX, float boxY, float boxSize, math::NkColor c) {
		...
		void SelectModeScene::OnRender(AppContext &ctx) {
			...
			f.DrawStringShadowCenteredScaled(r, FontAtlas::HeadlineSlot, scale, cx, titleY, "CHOISIS UN MODE",
											 theme::White(), theme::Cyan(), 3);
			f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, cx, subtitleY,
									   "FLECHES GAUCHE/DROITE  -  ENTREE  -  ECHAP", {255, 255, 255, 110});
			...
			const float cardH = math::NkMin(Pct::H(H, 0.55f, 200.0f, 480.0f), cardW * 1.20f);
			mCardW = cardW;
			mCardH = cardH;
			...
			for (int i = 0; i < kModeCount; ++i) {
				...
				math::NkColor bg = m.accent;
				bg.a = focused ? 50 : 18;
				math::NkColor bord = m.accent;
				bord.a = focused ? 220 : 80;
				if (disabled) {
					bg.a = 10;
					bord.a = 40;
				}
				...
				const float strokeFocused = math::NkClamp(Pct::MinDim(W, H) * 0.0025f, 1.5f, 4.0f);
				r.DrawQuad(bx, by + slideY, cardW, cardH, bg);
				r.DrawQuadOutline(bx, by + slideY, cardW, cardH, bord, focused ? strokeFocused : 1.0f);
				...
				// Titre + sous-titre
				math::NkColor titleC = disabled ? math::NkColor{255, 255, 255, 100} : theme::White();
				titleC.a = static_cast<uint8>(titleC.a * a);
				math::NkColor subC = {255, 255, 255, 130};
				subC.a = static_cast<uint8>(subC.a * a);

				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, bx + cardW * 0.5f,
										   by + slideY + cardH * 0.62f, m.title, titleC);
				f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, bx + cardW * 0.5f,
										   by + slideY + cardH * 0.78f, m.sub, subC);
			}

			// Footer hint — marge bas en % de H
			const float footerMargin = Pct::H(H, 0.03f, 16.0f, 40.0f);
			f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, cx, safeY + safeH - footerMargin,
									   "ENTREE / TAP : LANCER       ECHAP : RETOUR MENU", {255, 255, 255, 110});
		...
		int SelectModeScene::HitTestCard(float px, float py) const {
			for (int i = 0; i < kModeCount; ++i) {
				if (px >= mCardXs[i] && px <= mCardXs[i] + mCardW && py >= mCardYs[i] && py <= mCardYs[i] + mCardH)
					return i;
			}
			return -1;
		}
		...
		bool SelectModeScene::HitTestBack(float px, float py) const {
			return px >= mBackX && px <= mBackX + mBackW && py >= mBackY && py <= mBackY + mBackH;
		}
		...
		void SelectModeScene::ActivateMode(AppContext &ctx, int index) {
			...
			if (m.mode == GameMode::NetworkLAN || m.mode == GameMode::NetworkOnline) {

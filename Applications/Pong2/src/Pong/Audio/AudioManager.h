	...
	namespace pong {
		...
		class AudioManager {
			public:
				...
				void PlayPaddle(float32 v = 1.0f) noexcept {
					Play(SoundId::PaddleHit, v);
				}

				void PlayWall(float32 v = 1.0f) noexcept {
					Play(SoundId::WallHit, v);
				}

				void PlayScore(float32 v = 1.0f) noexcept {
					Play(SoundId::Score, v);
				}

				void PlayBonus(float32 v = 1.0f) noexcept {
					Play(SoundId::Bonus, v);
				}

				void PlayMenu(float32 v = 0.6f) noexcept {
					Play(SoundId::MenuSelect, v);
				}

				void PlayStart(float32 v = 1.0f) noexcept {
					Play(SoundId::MatchStart, v);
				}

				/// Volume master global [0..1] (affecte tous les sons).
				void SetMasterVolume(float32 v) noexcept;

				float32 MasterVolume() const noexcept {

	...
	namespace pong {
		...
		bool NetworkSession::StartHost(uint16 port) {
			...
			mConnMgr->onPeerConnected = [this](net::NkPeerId peer) {
				...
				logger.Info("[Net] Challenger connected (host). peerId={0} total={1}", (unsigned long long)peer.value,
							mPeerCount.load());
			};
			mConnMgr->onPeerDisconnected = [this](net::NkPeerId peer, const char *reason) {
				if (mPeerCount.load() > 0)
					mPeerCount.fetch_sub(1);
				...
				logger.Info("[Net] Challenger disconnected (host). peerId={0} reason={1}",
							(unsigned long long)peer.value, reason ? reason : "(unknown)");
			};

			logger.Info("[Net] StartHost -> calling NkConnectionManager::StartServer(port={0}, maxClients={1})", port,
						kMaxClients);
			const auto r = mConnMgr->StartServer(port, kMaxClients);
			logger.Info("[Net] StartServer returned code={0} (NK_NET_OK=0)", (int)r);
			if (r != net::NkNetResult::NK_NET_OK) {
				std::snprintf(mLastError, sizeof(mLastError), "StartServer echoue (code %d) sur port %u", (int)r,
							  (unsigned)port);
		...
		bool NetworkSession::StartJoin(const char *ip, uint16 port) {
			Shutdown();
			if (mConnMgr == nullptr || ip == nullptr || ip[0] == '\0')
				return false;
			...
			mConnMgr->onPeerDisconnected = [this](net::NkPeerId /*peer*/, const char *reason) {
				...
				logger.Info("[Net] Disconnected from host. Reason: {0}", reason ? reason : "(unknown)");
			};

			logger.Info("[Net] StartJoin -> parsing addr ip='{0}' port={1}", ip, port);
			const net::NkAddress addr(ip, port);
			if (!addr.IsValid()) {
				std::snprintf(mLastError, sizeof(mLastError), "Adresse IP invalide : %s", ip);
				mState.store(NetworkState::Idle);
				mRole = NetworkRole::None;
				logger.Error("[Net] {0}", mLastError);
				return false;
			...
			if (r != net::NkNetResult::NK_NET_OK) {
				std::snprintf(mLastError, sizeof(mLastError), "Connect echoue (code %d) vers %s:%u", (int)r, ip,
							  (unsigned)port);
				mState.store(NetworkState::Idle);
				mRole = NetworkRole::None;
				logger.Error("[Net] {0}", mLastError);
		...
		void NetworkSession::Shutdown() {
			if (mConnMgr != nullptr && (mState.load() != NetworkState::Idle)) {
				mConnMgr->Shutdown();
		...
		void NetworkSession::Tick(float dt) {
			...
			if (mDebugLogElapsed >= 1.5f) {
				...
				if (st == NetworkState::Hosting) {
					logger.Info("[Net][HB] HOSTING port=7777 peers={0} (en attente)", mPeerCount.load());
				} else if (st == NetworkState::Joining) {
					logger.Info("[Net][HB] JOINING (elapsed={0:.1}s / timeout {1}s)", mJoinElapsed,
								(int)kJoinTimeoutSec);
				} else if (st == NetworkState::Connected) {
					logger.Info("[Net][HB] CONNECTED peers={0}", mPeerCount.load());
				}
			}
			...
			if (st == NetworkState::Joining) {
				...
				if (mJoinElapsed > kJoinTimeoutSec) {
					std::snprintf(mLastError, sizeof(mLastError), "Timeout : aucun hote a repondu en %ds",
								  (int)kJoinTimeoutSec);
					if (mConnMgr != nullptr)
						mConnMgr->Shutdown();
					mState.store(NetworkState::Disconnected);
					mPeerCount.store(0);
					logger.Info("[Net] Join timeout — host didn't respond in {0}s", (int)kJoinTimeoutSec);
				}
			} else {
				mJoinElapsed = 0.0f;
			}
		...
		void NetworkSession::DrainInternal() {
			...
			for (uint32 i = 0; i < all.Size(); ++i) {
				...
				if (msg.size >= sizeof(netproto::PktHello) && msg.data != nullptr &&
					...
					logger.Info("[Net] PktHello recu du pair : {0}/{1}-{2}", mPeerCountry, mPeerCity, mPeerCode);
					...
					if (mRole == NetworkRole::Host) {
						UpdateChallengerIdentity(msg.from.value, pkt.country, pkt.city, pkt.code);
					}
					continue;
				}
				if (msg.size >= 1 && msg.data != nullptr && msg.data[0] == netproto::kMsgAccept) {
					mValidatedByHost = true;
					logger.Info("[Net] PktAccept recu : valide par l'host");
					continue;
				}
				if (msg.size >= 1 && msg.data != nullptr && msg.data[0] == netproto::kMsgReject) {
					mRejectedByHost = true;
					std::snprintf(mRejectReason, sizeof(mRejectReason), "L'hote a choisi un autre joueur");
		...
		void NetworkSession::SetLocalIdentity(const char *country, const char *city, const char *code) noexcept {
		...
		void NetworkSession::SendHello() {
			...
			if (mState.load() != NetworkState::Connected)
				return;
			if (mLocalCountry[0] == '\0')
				return; // pas encore identite local
			...
			const bool ok = Broadcast(reinterpret_cast<const uint8 *>(&pkt), (uint32)sizeof(pkt), /*reliable*/ 1);
			if (ok) {
				mHelloSent = true;
				logger.Info("[Net] PktHello envoye : {0}/{1}-{2}", mLocalCountry, mLocalCity, mLocalCode);
			}
		}
		...
		void NetworkSession::RemoveChallenger(uint64 peerId) noexcept {
			...
			for (uint32 i = 0; i < mChallengers.Size(); ++i) {
				if (mChallengers[i].peerId == peerId) {
					...
					if (i != last)
						mChallengers[i] = mChallengers[last];
		...
		void NetworkSession::UpdateChallengerIdentity(uint64 peerId, const char *country, const char *city,
			...
			for (uint32 i = 0; i < mChallengers.Size(); ++i) {
				if (mChallengers[i].peerId == peerId) {
					...
					if (country)
						std::strncpy(mChallengers[i].country, country, sizeof(mChallengers[i].country) - 1);
					if (city)
						std::strncpy(mChallengers[i].city, city, sizeof(mChallengers[i].city) - 1);
					if (code)
						std::strncpy(mChallengers[i].code, code, sizeof(mChallengers[i].code) - 1);
			...
			if (country)
				std::strncpy(info.country, country, sizeof(info.country) - 1);
			if (city)
				std::strncpy(info.city, city, sizeof(info.city) - 1);
			if (code)
				std::strncpy(info.code, code, sizeof(info.code) - 1);
			mChallengers.PushBack(info);
		...
		void NetworkSession::AcceptChallenger(uint64 peerId) noexcept {
			...
			if (!found) {
				logger.Warn("[Net] AcceptChallenger : peerId {0} introuvable", (unsigned long long)peerId);
				return;
			}
			// 1) Envoie PktAccept au choisi.
			netproto::PktAccept ack{netproto::kMsgAccept};
			const auto ar = mConnMgr->SendTo(net::NkPeerId{peerId}, reinterpret_cast<const uint8 *>(&ack),
											 (uint32)sizeof(ack), net::NkNetChannel::NK_NET_CHANNEL_RELIABLE_ORDERED);
			logger.Info("[Net] AcceptChallenger peerId={0} -> SendTo PktAccept code={1}", (unsigned long long)peerId,
						(int)ar);
			...
			for (uint32 i = 0; i < toDisconnect.Size(); ++i) {
				(void)mConnMgr->SendTo(net::NkPeerId{toDisconnect[i]}, reinterpret_cast<const uint8 *>(&rej),
									   (uint32)sizeof(rej), net::NkNetChannel::NK_NET_CHANNEL_RELIABLE_ORDERED);
				mConnMgr->Disconnect(net::NkPeerId{toDisconnect[i]}, "Host a choisi un autre challenger");
			}
		...
		void NetworkSession::RejectChallenger(uint64 peerId) noexcept {
			...
			if (mConnMgr == nullptr)
				return;
			netproto::PktReject rej{netproto::kMsgReject};
			(void)mConnMgr->SendTo(net::NkPeerId{peerId}, reinterpret_cast<const uint8 *>(&rej), (uint32)sizeof(rej),
								   net::NkNetChannel::NK_NET_CHANNEL_RELIABLE_ORDERED);
			mConnMgr->Disconnect(net::NkPeerId{peerId}, "Refuse par l'hote");

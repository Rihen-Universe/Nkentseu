...
namespace nkentseu {
	namespace pong {
		...
		// BEACON (cote HOST)
		// ─────────────────────────────────────────────────────────────────────
		bool NetworkDiscovery::StartBeacon(const char *country, const char *city, const char *code,
										   uint16 gamePort) noexcept {
			...
			if (country)
				std::strncpy(mBeaconPkt.country, country, sizeof(mBeaconPkt.country) - 1);
			if (city)
				std::strncpy(mBeaconPkt.city, city, sizeof(mBeaconPkt.city) - 1);
			if (code)
				std::strncpy(mBeaconPkt.code, code, sizeof(mBeaconPkt.code) - 1);
			...
			mBeaconTimer = 0.0f;
			logger.Info("[Discovery] Beacon started : {0}/{1}-{2} on port {3}", mBeaconPkt.country, mBeaconPkt.city,
						mBeaconPkt.code, (unsigned)gamePort);
			return true;
		}
		...
		void NetworkDiscovery::Tick(float dt) noexcept {
			...
			if (mBeaconSock != nullptr) {
				...
				if (mBeaconTimer <= 0.0f) {
					...
					if (rs != net::NkNetResult::NK_NET_OK) {
						...
						if ((++errCount % 30) == 1) {
							logger.Warn("[Discovery] Beacon SendTo FAILED ({0}, occurrence #{1})", (int)rs, errCount);
						}
					}
			...
			if (mScanSock != nullptr) {
				...
				for (int i = 0; i < kMaxPerTick; ++i) {
					...
					if (rr != net::NkNetResult::NK_NET_OK)
						break;
					if (received == 0)
						break;
					// Filtre : taille + magic exact + version.
					if (received < sizeof(PktBeacon))
						continue;
					PktBeacon pkt;
					std::memcpy(&pkt, buf, sizeof(pkt));
					if (std::memcmp(pkt.magic, kBeaconMagic, 4) != 0)
						continue;
					if (pkt.version != kBeaconVersion)
						continue;
					UpdateHost(pkt, from);
				}
		...
		// Helpers internes
		// ─────────────────────────────────────────────────────────────────────
		void NetworkDiscovery::UpdateHost(const PktBeacon &pkt, const net::NkAddress &from) noexcept {
			...
			for (int i = 0; i < mHostCount; ++i) {
				if (std::strcmp(mHosts[i].ip, ipBuf) == 0 && std::strcmp(mHosts[i].code, pkt.code) == 0) {
			...
			h.ageSec = 0.0f;
			logger.Info("[Discovery] Nouveau host : {0}/{1}-{2} @ {3}:{4}", h.country, h.city, h.code, h.ip,
						(unsigned)h.gamePort);
		}
		...
		void NetworkDiscovery::GarbageCollectHosts() noexcept {
			...
			for (int src = 0; src < mHostCount; ++src) {
				if (mHosts[src].ageSec <= kHostTtlSec) {
					if (dst != src)
						mHosts[dst] = mHosts[src];
					++dst;
				} else {
					logger.Info("[Discovery] Host expire : {0}/{1}-{2}", mHosts[src].country, mHosts[src].city,
								mHosts[src].code);
				}
			}
			mHostCount = dst;

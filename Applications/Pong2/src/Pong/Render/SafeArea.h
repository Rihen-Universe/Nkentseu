...
namespace nkentseu {
	namespace pong {
		...
		struct SafeArea {
				...
				float TopY() const noexcept {
					return top;
				}

				float BottomY() const noexcept {
					return vpH - bottom;
				}
				...
				float SafeW() const noexcept {
					return vpW - left - right;
				}

				float SafeH() const noexcept {
					return vpH - top - bottom;
				}

				float SafeCX() const noexcept {
					return left + SafeW() * 0.5f;
				}

				float SafeCY() const noexcept {
					return top + SafeH() * 0.5f;
				}

				/// Marge supplementaire interieure (en plus des insets) — utile
				/// pour eloigner le contenu UI des bords sur tactile.
				float TopY(float padding) const noexcept {
					return top + padding;
				}

				float BottomY(float padding) const noexcept {
					return vpH - bottom - padding;
				}

				float LeftX(float padding) const noexcept {
					return left + padding;
				}

				float RightX(float padding) const noexcept {
					return vpW - right - padding;
				}
		};

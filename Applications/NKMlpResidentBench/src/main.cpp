...
static NkTensor FillT(const NkShape& sh, double scale, int seed) {
    ...
    float* d = new float[n];
    for (int64 i = 0; i < n; i++) d[i] = (float)(scale * std::sin(0.017 * (double)(i + 1) + 0.3 * seed));
...
int main() {
    ...
    printf("Modèle : x[%lld,%lld] -> Dense(%lld,%lld)+ReLU -> Dense(%lld,%lld) -> SoftmaxCE\n",
           (long long)B,(long long)IN,(long long)IN,(long long)H,(long long)H,(long long)OUT);
    ...
    // Cible one-hot [B,OUT]
    float* td = new float[B * OUT]; for (int64 i = 0; i < B * OUT; i++) td[i] = 0.f;
    for (int64 b = 0; b < B; b++) td[b * OUT + (b % OUT)] = 1.f;
    NkTensor tCpu = NkTensor::FromData(NkShape{ B, OUT }, td, NkDType::NK_F32); delete[] td;
    ...
    auto step = [&](bool gpu, NkTensor& dW1, NkTensor& db1, NkTensor& dW2, NkTensor& db2, double& lossOut) {
        ...
        dW1 = W1.Grad().ToCPU().Contiguous(); db1 = b1.Grad().ToCPU().Contiguous();
        dW2 = W2.Grad().ToCPU().Contiguous(); db2 = b2.Grad().ToCPU().Contiguous();
    };
    ...
    auto trainRun = [&](bool useGpu, int steps, double& finalLoss, double& msPerStep, bool verbose) {
        ...
        params.PushBack(W1); params.PushBack(b1); params.PushBack(W2); params.PushBack(b2);
        ...
        for (int s = 0; s < steps; s++) {
            ...
            if (verbose && (s % 8 == 0 || s == steps - 1)) printf("      step %3d : loss = %.4f\n", s, loss);
        }
    ...
    // (A) CPU
    NkTensor aW1, ab1, aW2, ab2; double lossA = 0.0;
    const int Rc = 5;
    step(false, aW1, ab1, aW2, ab2, lossA);
    auto t0 = std::chrono::high_resolution_clock::now();
    for (int r = 0; r < Rc; r++) step(false, aW1, ab1, aW2, ab2, lossA);
    ...
    if (gpu.IsAvailable()) {
        NkTensor gW1, gb1, gW2, gb2; double lossG = 0.0;
        step(true, gW1, gb1, gW2, gb2, lossG);   // warmups
        step(true, gW1, gb1, gW2, gb2, lossG);
        const int Rg = 20;
        auto t2 = std::chrono::high_resolution_clock::now();
        for (int r = 0; r < Rg; r++) step(true, gW1, gb1, gW2, gb2, lossG);
        ...
        printf("  [%s] la perte descend (%.4f -> %.4f) ET GPU-résident == CPU (|Δ|=%.4f)\n",
               trained ? " OK " : "FAIL", start, glA, std::fabs(clA - glA));

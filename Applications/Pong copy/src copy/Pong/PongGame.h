...
namespace nkentseu {
    ...
    struct Obstacle {
        ...
        void Update(float dt, float fieldW, float fieldH, float fieldTop = 0.0f) noexcept
        {
            ...
            // Rebond sur le bord haut (respecte la barre HUD)
            if (y < fieldTop)
            {
                y  = fieldTop;
                vy = -vy;
    ...
    struct Paddle {
        ...
        /// @brief Retourne la coordonnee X du centre horizontal de la raquette.
        ...
        // ── Déplacements (portrait) ───────────────────────────────────────────────

        /// @brief Deplace la raquette vers la gauche (portrait).
        void MoveLeft(float dt) noexcept  { x -= speed * dt; }

        /// @brief Deplace la raquette vers la droite (portrait).
        void MoveRight(float dt) noexcept { x += speed * dt; }

        // ── Clamping ──────────────────────────────────────────────────────────────

        /// @brief Contraint la raquette dans les limites verticales (mode paysage).
        ...
        /// @brief Contraint la raquette dans les limites horizontales (mode portrait).
        void ClampToFieldH(float fieldLeft, float fieldRight) noexcept
        {
            if (x < fieldLeft)  x = fieldLeft;
            if (x + w > fieldRight) x = fieldRight - w;
        }
    };
    ...
    class PongGame {
        public:
            ...
            // ── Gestes tactiles pour mode portrait (swipe gauche/droite) ─────────────
            /// @brief Position X initiale du toucher (portrait : swipe horizontal).
            float mTouchStartX = -1.0f;

            /// @brief Position X courante du toucher (portrait : swipe horizontal).
            float mTouchCurrentX = -1.0f;

            void SetState(GameState newState) noexcept
            {
                mState = newState;
            }
        ...
        private:
            ...
            /// @brief Y du haut du terrain de jeu (en dessous de la barre HUD).
            ///        La barre HUD occupe [0, mFieldY[. Le terrain occupe [mFieldY, H[.
            float mFieldY = 0.0f;

            // ── Helpers de layout (inline) ────────────────────────────────────────────

            /// @brief Retourne la hauteur du terrain de jeu (H - mFieldY).
            float FieldH() const noexcept {
                return static_cast<float>(mRenderer.Height()) - mFieldY;
            }

            /// @brief Retourne la hauteur de la barre HUD.
            float HudH() const noexcept { return mFieldY; }
        public:
            /// @brief Vrai si l'ecran est en mode portrait (H >= W).
            ///        En portrait les raquettes sont en haut / bas (horizontales).
            bool IsPortrait() const noexcept {
                return mRenderer.Height() >= mRenderer.Width();
            }

...
static void SetupPaddlesForOrientation(
    ...
    uint32_t W, uint32_t H, float fieldY)
{
    float fH = static_cast<float>(H) - fieldY;
    bool portrait = (H >= W);
    ...
    if (!portrait)
    {
        ...
        player.h = 90.0f;
        player.x = 20.0f;
        player.y = fieldY + fH * 0.5f - player.h * 0.5f;

        ai.w = 12.0f;
        ai.h = 90.0f;
        ai.x = static_cast<float>(W) - 20.0f - ai.w;
        ai.y = fieldY + fH * 0.5f - ai.h * 0.5f;
    }
    else
    {
        // ── Portrait ────────────────────────────────────────────────────────
        float pw = static_cast<float>(W) * 0.38f;
        if (pw > 140.0f) pw = 140.0f;
        float ph = 14.0f;

        player.w = pw;
        player.h = ph;
        player.x = static_cast<float>(W) * 0.5f - pw * 0.5f;
        player.y = static_cast<float>(H) - ph - 18.0f;

        ai.w = pw;
        ai.h = ph;
        ai.x = static_cast<float>(W) * 0.5f - pw * 0.5f;
        ai.y = fieldY + 10.0f;
    }
}
...
void PongGame::Init()
{
    ...
    SetupPaddlesForOrientation(mPlayer, mAI, W, H, mFieldY);
...
void PongGame::OnResize()
{
    ...
    if (mBall.x < 0.0f || mBall.x > static_cast<float>(W) ||
        mBall.y < mFieldY || mBall.y > static_cast<float>(H))
    {
        ResetBall(true);
    }
...
void PongGame::ResetBall(bool leftServe)
{
    ...
    if (!IsPortrait())
    {
        // Paysage : balle part horizontalement
        dx = leftServe ? 1.0f : -1.0f;
        dy = math::NkSin(noise);
    }
    else
    {
        // Portrait : balle part verticalement
        // leftServe = service vers le bas (vers le joueur), -1 = vers le haut (vers l'IA)
        dx = math::NkSin(noise);
        dy = leftServe ? 1.0f : -1.0f;
    }
...
void PongGame::UpdateBall(float dt)
{
    ...
    {
        // ═══════════════════════════════════════════════════════════════════
        // MODE PAYSAGE : rebonds haut/bas, buts gauche/droite
        // ═══════════════════════════════════════════════════════════════════

        // Rebond sur le mur haut (sous la barre HUD)
        if (mBall.y - mBall.radius < mFieldY)
        {
            mBall.y  = mFieldY + mBall.radius;
            mBall.vy = math::NkAbs(mBall.vy);
            SpawnParticles(mBall.x, mFieldY, gamecolors::BallCol(), 5, 70.0f);
        }
    ...
    {
        // ═══════════════════════════════════════════════════════════════════
        // MODE PORTRAIT : rebonds gauche/droite, buts haut/bas
        // ═══════════════════════════════════════════════════════════════════

        // Rebond sur le mur gauche
        if (mBall.x - mBall.radius < 0.0f)
        {
            mBall.x  = mBall.radius;
            mBall.vx = math::NkAbs(mBall.vx);
            SpawnParticles(0.0f, mBall.y, gamecolors::BallCol(), 5, 70.0f);
        }

        // Rebond sur le mur droit
        if (mBall.x + mBall.radius > W)
        {
            mBall.x  = W - mBall.radius;
            mBall.vx = -math::NkAbs(mBall.vx);
            SpawnParticles(W, mBall.y, gamecolors::BallCol(), 5, 70.0f);
        }

        // But en haut (cote IA) -> joueur marque
        if (mBall.y < mFieldY - mBall.radius)
        {
            ++mPlayer.score;
            mLastScorer = 0; mShake = 0.4f; mFlashAlpha = 1.0f;
            mFlashColor = gamecolors::Player(); mGoalTimer = 1.8f;
            mState = GameState::GoalFlash;
            SpawnParticles(mBall.x, mFieldY, gamecolors::Player(), 35, 200.0f, true);
            return;
        }

        // But en bas (cote joueur) -> IA marque
        if (mBall.y > H + mBall.radius)
        {
            ++mAI.score;
            mLastScorer = 1; mShake = 0.4f; mFlashAlpha = 1.0f;
            mFlashColor = gamecolors::AI(); mGoalTimer = 1.8f;
            mState = GameState::GoalFlash;
            SpawnParticles(mBall.x, H, gamecolors::AI(), 35, 200.0f, true);
            return;
        }
    }
                             ...
                             bool leftHeld, bool rightHeld)
{
    ...
    {
        // ═══════════════════════════════════════════════════════════════════
        // MODE PAYSAGE : mouvement vertical, IA suit ball.y
        // ═══════════════════════════════════════════════════════════════════

        if (upHeld)   mPlayer.MoveUp(dt);
        if (downHeld) mPlayer.MoveDown(dt);
        mPlayer.ClampToField(mFieldY, H);
        ...
        mAI.ClampToField(mFieldY, H);
    }
    else
    {
        // ═══════════════════════════════════════════════════════════════════
        // MODE PORTRAIT : mouvement horizontal, IA suit ball.x
        // ═══════════════════════════════════════════════════════════════════

        if (leftHeld)  mPlayer.MoveLeft(dt);
        if (rightHeld) mPlayer.MoveRight(dt);
        mPlayer.ClampToFieldH(0.0f, W);

        // IA : suit ball.x
        float futureBx = mBall.x + mBall.vx * predict;
        float targetX  = futureBx - mAI.w * 0.5f + mAI.aiError;
        float diffX    = targetX - mAI.x;
        float maxMove  = mAI.aiSpeed * dt * mAI.aiReaction;

        if (math::NkAbs(diffX) > maxMove)
            mAI.x += (diffX > 0.0f) ? maxMove : -maxMove;
        else
            mAI.x = targetX;

        mAI.ClampToFieldH(0.0f, W);
    }
...
void PongGame::RenderTouchButtons()
{
                            ...
                            math::NkColor accent)
    {
        ...
        math::NkColor fill(accent.r/6, accent.g/6, accent.b/6, 180);
        for (int row = y; row < y + h; ++row) {
            for (int col = x; col < x + w; ++col) {
                math::NkColor dst = mRenderer.GetPixel(col, row);
                mRenderer.SetPixel(col, row, Blend(dst, fill));

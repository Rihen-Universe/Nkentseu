	...
	namespace pong {
		...
		class PowerUpSystem {
			public:
				...
				void Render(renderer::NkRenderer2D &r, FontAtlas &f, float arenaOX, float arenaOY, float scale) const;
				...
				bool CheckPaddleCollision(int side, float paddleX, float paddleY, float paddleW, float paddleH,
				...
				// ── Acces HUD ──────────────────────────────────────────────────
				int EffectCount() const noexcept {
					return (int)mEffects.Size();
				}

				const ActiveEffect &GetEffect(int i) const {
					return mEffects[i];
				}

				int DropCount() const noexcept {
					return (int)mDrops.Size();
				}

				const PowerUpDrop &GetDrop(int i) const {
					return mDrops[i];
				}
				...
				void CheckBallCollision(float bx, float by, float br, float ballVX, float ballVY);
			...
			private:
				...
				// ── Helpers internes ──────────────────────────────────────────
				int SideIdx(int side) const {
					return (side > 0) ? 1 : 0;
				}

				float RandPeriod();
				void ScheduleNextDrop();
				void AddEffect(bool isBonus, uint8 kind, int side, float duration, int charges = 0);

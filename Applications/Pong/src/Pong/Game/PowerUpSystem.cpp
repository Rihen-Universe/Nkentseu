...
namespace nkentseu {
	namespace pong {
		...
		static BonusDescL BonusDesc(BonusType b) {
			switch (b) {
				case BonusType::GiantPaddle:
					return {8.0f, 0, MakeColor(255, 215, 0)};
				case BonusType::PaddleSpeed:
					return {6.0f, 0, MakeColor(0, 245, 255)};
				case BonusType::DoublePoint:
					return {60.0f, 1, MakeColor(255, 180, 60)};
				case BonusType::Shield:
					return {12.0f, 1, MakeColor(0, 200, 255)};
				case BonusType::SlowBall:
					return {5.0f, 0, MakeColor(80, 255, 100)};
				case BonusType::RandomStar:
					return {0.0f, 0, MakeColor(255, 255, 255)};
			}
		...
		static MalusDescL MalusDesc(MalusType m) {
			switch (m) {
				case MalusType::Blind:
					return {2.0f, 0, MakeColor(80, 0, 120)};
				case MalusType::MiniPaddle:
					return {6.0f, 0, MakeColor(160, 160, 160)};
				case MalusType::InvertControls:
					return {4.0f, 0, MakeColor(255, 60, 200)};
				case MalusType::Freeze:
					return {1.5f, 0, MakeColor(120, 200, 255)};
				case MalusType::FastBall:
					return {10.0f, 0, MakeColor(255, 60, 60)};
				case MalusType::TeleportPaddle:
					return {0.0f, 0, MakeColor(255, 255, 255)};
			}
		...
		float PowerUpSystem::RandPeriod() {
			return mDropPeriodMin + Rand01() * (mDropPeriodMax - mDropPeriodMin);
		}
		...
		void PowerUpSystem::Update(float dt, float arenaW, float arenaH, float scale) {
			...
			for (uint32 i = 0; i < mDrops.Size();) {
				...
				if (d.y - d.r > arenaH + 8.0f || d.x + d.r < -32.0f || d.x - d.r > arenaW + 32.0f) {
					mDrops.RemoveAt(i);
					continue;
				}
				++i;
		...
		void PowerUpSystem::Render(renderer::NkRenderer2D &r, FontAtlas & /*f*/, float arenaOX, float arenaOY,
								   float scale) const {
			(void)scale;
			for (uint32 i = 0; i < mDrops.Size(); ++i) {
				...
				if (!d.alive)
					continue;
				const math::NkColor base =
					d.isBonus ? BonusDesc((BonusType)d.kind).color : MalusDesc((MalusType)d.kind).color;
				...
				if (d.isBonus) {
					...
					r.DrawLine({cx - k, cy - k}, {cx + k, cy + k}, MakeColor(255, 255, 255, 220), 2.0f);
					r.DrawLine({cx - k, cy + k}, {cx + k, cy - k}, MakeColor(255, 255, 255, 220), 2.0f);
				}
			}
		}
		...
		void PowerUpSystem::ApplyBonus(BonusType b, int collectorSide) {
			if (b == BonusType::RandomStar) {
				...
				static const BonusType kReal[5] = {BonusType::GiantPaddle, BonusType::PaddleSpeed,
												   BonusType::DoublePoint, BonusType::Shield, BonusType::SlowBall};
		...
		void PowerUpSystem::ApplyRandomBonus(int collectorSide) {
			...
			const BonusType all[6] = {BonusType::GiantPaddle, BonusType::PaddleSpeed, BonusType::DoublePoint,
									  BonusType::Shield,	  BonusType::SlowBall,	  BonusType::RandomStar};
			ApplyBonus(all[(int)(Rand01() * 6.0f) % 6], collectorSide);
		}

		void PowerUpSystem::ApplyRandomMalus(int targetSide) {
			const MalusType all[6] = {MalusType::Blind,	 MalusType::MiniPaddle, MalusType::InvertControls,
									  MalusType::Freeze, MalusType::FastBall,	MalusType::TeleportPaddle};
		...
		// ────────────────────────────────────────────────────────────────────
		void PowerUpSystem::CheckBallCollision(float bx, float by, float br, float ballVX, float ballVY) {
			for (uint32 i = 0; i < mDrops.Size(); ++i) {
				PowerUpDrop &d = mDrops[i];
				if (!d.alive)
					continue;
		...
		// ────────────────────────────────────────────────────────────────────
		void PowerUpSystem::SetNetNotification(int side, bool isBonus, uint8 kind, float timeLeft) {
		...
		bool PowerUpSystem::CheckPaddleCollision(int side, float paddleX, float paddleY, float paddleW, float paddleH,
												 float *outX, float *outY, math::NkColor *outColor) {
			for (uint32 i = 0; i < mDrops.Size(); ++i) {
				PowerUpDrop &d = mDrops[i];
				if (!d.alive)
					continue;
				...
				// augmente du rayon.
				if (d.x + d.r < paddleX || d.x - d.r > paddleX + paddleW)
					continue;
				if (d.y + d.r < paddleY || d.y - d.r > paddleY + paddleH)
					continue;
				...
				if (outY)
					*outY = d.y;
				if (outColor)
					*outColor = d.isBonus ? BonusDesc((BonusType)d.kind).color : MalusDesc((MalusType)d.kind).color;
				// Applique l'effet.
				if (d.isBonus) {
		...
		float PowerUpSystem::GetPaddleSpeedMul(int side) const {
			if (HasEffect(true, (uint8)BonusType::PaddleSpeed, side))
				return 2.0f;
			return 1.0f;
		}
		...
		float PowerUpSystem::GetBallSpeedMul() const {
			...
			for (uint32 i = 0; i < mEffects.Size(); ++i) {
				const ActiveEffect &e = mEffects[i];
				if (e.isBonus && e.kind == (uint8)BonusType::SlowBall)
					return 0.5f;
				if (!e.isBonus && e.kind == (uint8)MalusType::FastBall)
					return 3.0f;
			}
			return 1.0f;
		}
		...
		bool PowerUpSystem::ConsumeShield(int side) {
			for (uint32 i = 0; i < mEffects.Size(); ++i) {
				ActiveEffect &e = mEffects[i];
				if (e.isBonus && e.kind == (uint8)BonusType::Shield && e.side == side && e.charges > 0) {
					e.charges--;
					if (e.charges <= 0)
						e.timeLeft = 0.0f; // expire au prochain update
					return true;
				}
			}
			return false;
		...
		bool PowerUpSystem::ConsumeDoublePoint(int side) {
			for (uint32 i = 0; i < mEffects.Size(); ++i) {
				ActiveEffect &e = mEffects[i];
				if (e.isBonus && e.kind == (uint8)BonusType::DoublePoint && e.side == side && e.charges > 0) {
					e.charges--;
					if (e.charges <= 0)
						e.timeLeft = 0.0f;
					return true;
		...
		math::NkColor PowerUpSystem::GetEffectColor(int i) const {
			const ActiveEffect &e = mEffects[i];
			if (e.isBonus)
				return BonusDesc((BonusType)e.kind).color;
		...
		// ── Helpers internes ────────────────────────────────────────────────
		void PowerUpSystem::AddEffect(bool isBonus, uint8 kind, int side, float duration, int charges) {
			ActiveEffect e{};
			e.isBonus = isBonus;
			e.kind = kind;
			e.side = side;

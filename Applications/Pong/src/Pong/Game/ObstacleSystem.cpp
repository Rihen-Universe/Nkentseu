...
namespace nkentseu {
	namespace pong {
		...
		// ─────────────────────────────────────────────────────────────────────
		void ObstacleSystem::Render(renderer::NkRenderer2D &r, float ax, float ay, float scale) const {
			for (uint32 i = 0; i < mObstacles.Size(); ++i) {
				...
				switch (o.type) {
						...
						switch (o.shape) {
							...
							case ObstacleShape::Hexagon: {
								...
								for (int k = 0; k < n; ++k) {
									...
									r.DrawFilledTriangle({pcx, pcy}, {a0.x + ax, a0.y + ay}, {a1.x + ax, a1.y + ay},
														 o.color);
								}
					...
					case ObstacleType::Gravity: {
						...
						r.DrawFilledCircle({ocx, ocy}, math::NkMax(2.0f, 3.0f * scale), WithAlpha(o.color, 0.7f), 12);
						break;
					}
					case ObstacleType::Portal: {
					...
					case ObstacleType::Mine: {
						...
						r.DrawFilledCircle({ocx, ocy}, o.w * 0.30f, WithAlpha(o.color, pulse * 0.6f), 16);
						for (int k = 0; k < 8; ++k) {
					...
					case ObstacleType::Magnet: {
						...
						r.DrawRectOutline({ox, oy, o.w, o.h}, WithAlpha(o.glowColor, 0.6f + pulse01 * 0.3f), lineW);
						break;
					}
					case ObstacleType::GhostMirror: {

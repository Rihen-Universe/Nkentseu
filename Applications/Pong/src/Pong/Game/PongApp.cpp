...
namespace nkentseu {
	namespace pong {
		...
		bool PongApp::Init() {
			...
			NkContextDesc desc;
			desc.api =
				(pcfg.backend == NkGraphicsApi::NK_GFX_API_AUTO) ? PongConfig::PickBestForPlatform() : pcfg.backend;
			logger.Info("[PongApp] Backend graphique : {0}", PongConfig::BackendName(desc.api));
			...
			if (mTarget == nullptr || !mTarget->IsValid()) {
				logger.Error("[PongApp] NkRenderWindow init FAILED (backend {0})", PongConfig::BackendName(desc.api));
				return false;
			}
			...
			africa::PickRandomCountryCityCode(mMyCountry, sizeof(mMyCountry), mMyCity, sizeof(mMyCity), mMyCode,
											  sizeof(mMyCode));
			logger.Info("[PongApp] Identite reseau : {0}/{1}-{2}", mMyCountry, mMyCity, mMyCode);

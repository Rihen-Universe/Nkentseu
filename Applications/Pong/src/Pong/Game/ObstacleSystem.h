...
namespace nkentseu {
	namespace pong {
		...
		// ─────────────────────────────────────────────────────────────────────
		enum class ObstacleShape : uint8 { Rectangle, Circle, Triangle, Diamond, Hexagon };
		...
		class ObstacleSystem {
			public:
				...
				void SpawnFromSettings(const GameSettings &s, float arenaW, float arenaH, float scale);
				...
				ObstacleHit CheckCollision(float bx, float by, float br, float vx, float vy, float dt60) const;
				...
				const Obstacle &Get(int i) const {
					return mObstacles[(size_t)i];
				}
				...
				void AppendFromNet(ObstacleType type, ObstacleShape shape, float xTopLeft, float yTopLeft, float w,
								   float h, float rotation, bool active, bool visible);

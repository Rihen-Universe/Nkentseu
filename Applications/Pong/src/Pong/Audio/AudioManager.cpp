...
namespace nkentseu {
	namespace pong {
		...
		static AudioSample MakeWallHit() {
			AudioSample s = AudioGenerator::GenerateTone(220.0f, 0.05f, WaveformType::SQUARE, 48000, 0.6f);
			AdsrEnvelope env;
		...
		static AudioSample MakeBonus() {
			...
			AudioSample s = AudioGenerator::GenerateChord(freqs, 3, /*duration*/ 0.40f, amplitudes, 48000);
			AdsrEnvelope env;
		...
		static AudioSample MakeMenuSelect() {
			AudioSample s = AudioGenerator::GenerateTone(1200.0f, 0.04f, WaveformType::SINE, 48000, 0.5f);
			AdsrEnvelope env;
		...
		bool AudioManager::Initialize() {
			...
			logger.Info("[AudioManager] AudioEngine OK : backend={0}, {1} Hz, {2} ch, latency={3:.1}ms",
						AudioEngine::Instance().GetBackendName(), AudioEngine::Instance().GetSampleRate(),
						AudioEngine::Instance().GetChannels(), AudioEngine::Instance().GetLatencyMs());
			...
			logger.Info("[AudioManager] {0} SFX generes procedural en RAM", (int)SoundId::COUNT);
		...
		void AudioManager::Play(SoundId id, float32 volume) noexcept {
			...
			if (idx < 0 || idx >= (int)SoundId::COUNT)
				return;
			if (!mSamples[idx].IsValid())
				return;

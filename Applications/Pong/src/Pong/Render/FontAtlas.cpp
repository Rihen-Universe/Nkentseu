...
namespace nkentseu {
	namespace pong {
		...
		bool FontAtlas::Init(renderer::NkRenderer2D &r) {
			...
			logger.Info("[FontAtlas] using embedded font: {0}", NkFontEmbedded::GetName(fontId));
			...
			for (int s = 0; s < slotN; ++s) {
				...
				if (mFonts[s] == nullptr) {
					logger.Error("[FontAtlas] AddToAtlas failed for slot {0} ({1}px)", s, kSizePx[s]);
				}
			}
		...
		float FontAtlas::MeasureWidth(SizeSlot s, const char *text) const {
			if (text == nullptr || mFonts[s] == nullptr)
				return 0.0f;
		...
		// ─────────────────────────────────────────────────────────────────────
		static void PushGlyphQuad(NkVector<renderer::NkVertex> &verts, NkVector<uint32> &idx, float gx, float gy,
								  float gw, float gh, float u0, float v0, float u1, float v1, const math::NkColor &c) {
			...
			vtx.u = u0;
			vtx.v = v0;
			verts.PushBack(vtx); // TL
			vtx.x = gx + gw;
			vtx.y = gy;
			vtx.u = u1;
			vtx.v = v0;
			verts.PushBack(vtx); // TR
			vtx.x = gx + gw;
			vtx.y = gy + gh;
			vtx.u = u1;
			vtx.v = v1;
			verts.PushBack(vtx); // BR
			vtx.x = gx;
			vtx.y = gy + gh;
			vtx.u = u0;
			vtx.v = v1;
			verts.PushBack(vtx); // BL
			idx.PushBack(base + 0);
			idx.PushBack(base + 1);
			idx.PushBack(base + 2);
			idx.PushBack(base + 0);
		...
		// ─────────────────────────────────────────────────────────────────────
		float FontAtlas::DrawString(renderer::NkRenderer2D &r, SizeSlot s, float x, float y, const char *text,
									math::NkColor color) {
			...
			if (!verts.Empty()) {
				r.DrawVertices(verts.Data(), static_cast<uint32>(verts.Size()), idx.Data(),
							   static_cast<uint32>(idx.Size()), mTexture);
			}
			return cursorX - x;
		}

		float FontAtlas::DrawStringCentered(renderer::NkRenderer2D &r, SizeSlot s, float cx, float y, const char *text,
											math::NkColor color) {
		...
		// ─────────────────────────────────────────────────────────────────────
		float FontAtlas::MeasureWidthScaled(SizeSlot s, float scale, const char *text) const {
			if (text == nullptr || mFonts[s] == nullptr)
				return 0.0f;
			return mFonts[s]->CalcTextSizeX(text) * scale;
		}

		float FontAtlas::DrawStringScaled(renderer::NkRenderer2D &r, SizeSlot s, float scale, float x, float y,
										  const char *text, math::NkColor color) {
			if (text == nullptr || mFonts[s] == nullptr || mTexture == nullptr || scale <= 0.0f) {
				return 0.0f;
			}
			NkFont *font = mFonts[s];
			...
			while (*p != '\0') {
				...
				if (g == nullptr)
					continue;
				if (g->visible) {
					PushGlyphQuad(verts, idx, cursorX + g->x0 * scale, cursorY + g->y0 * scale, (g->x1 - g->x0) * scale,
								  (g->y1 - g->y0) * scale, g->u0, g->v0, g->u1, g->v1, color);
				}
		...
		float FontAtlas::DrawStringCenteredScaled(renderer::NkRenderer2D &r, SizeSlot s, float scale, float cx, float y,
												  const char *text, math::NkColor color) {
			const float w = MeasureWidthScaled(s, scale, text);
			return DrawStringScaled(r, s, scale, cx - w * 0.5f, y, text, color);
		}

		void FontAtlas::DrawStringShadowScaled(renderer::NkRenderer2D &r, SizeSlot s, float scale, float x, float y,
			...
			for (int oy = -blur; oy <= blur; ++oy) {
				for (int ox = -blur; ox <= blur; ++ox) {
					...
					if (c.a > 4) {
						DrawStringScaled(r, s, scale, x + static_cast<float>(ox), y + static_cast<float>(oy), text, c);
					}
				}
			}
		...
		void FontAtlas::DrawStringShadowCenteredScaled(renderer::NkRenderer2D &r, SizeSlot s, float scale, float cx,
													   float y, const char *text, math::NkColor textColor,
													   math::NkColor shadowColor, int blur) {
			const float w = MeasureWidthScaled(s, scale, text);
			DrawStringShadowScaled(r, s, scale, cx - w * 0.5f, y, text, textColor, shadowColor, blur);
		}
		...
		// ─────────────────────────────────────────────────────────────────────
		void FontAtlas::DrawStringShadow(renderer::NkRenderer2D &r, SizeSlot s, float x, float y, const char *text,
										 math::NkColor textColor, math::NkColor shadowColor, int blur) {
			for (int oy = -blur; oy <= blur; ++oy) {
				for (int ox = -blur; ox <= blur; ++ox) {
					if (ox == 0 && oy == 0)
						continue;
					...
					c.a = static_cast<uint8_t>(a * shadowColor.a);
					if (c.a > 4) {
						DrawString(r, s, x + static_cast<float>(ox), y + static_cast<float>(oy), text, c);
					}
				}
			}
		...
		void FontAtlas::DrawStringShadowCentered(renderer::NkRenderer2D &r, SizeSlot s, float cx, float y,
												 const char *text, math::NkColor textColor, math::NkColor shadowColor,
												 int blur) {
			const float w = MeasureWidth(s, text);
			DrawStringShadow(r, s, cx - w * 0.5f, y, text, textColor, shadowColor, blur);
		}

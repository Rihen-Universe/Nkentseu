...
#pragma pack(pop)
			static_assert(sizeof(PktParticleBurst) == 1 + 4 + 4 + 4 + 1 + 6, "PktParticleBurst layout cassee");
...
#pragma pack(pop)
			static_assert(sizeof(PktParticleGoal) == 1 + 4 + 4, "PktParticleGoal layout cassee");
...
#pragma pack(pop)
			static_assert(sizeof(PktStartMatch) == 1 + 2 + 2 + 4 + 1 + 4 + 8, "PktStartMatch layout cassee");

			enum StartMatchFlag : uint8 { kStartFlagWinByTwo = 1 << 0, kStartFlagPowerUpsOn = 1 << 1 };
...
#pragma pack(pop)
			static_assert(sizeof(PktObstacleEntry) == 14, "PktObstacleEntry layout cassee");

			/// Bits du champ flags dans PktObstacleEntry.
			enum ObstacleFlag : uint8 { kObsActive = 1 << 0, kObsVisible = 1 << 1 };
...
#pragma pack(pop)
			static_assert(sizeof(PktState) == 1				  // type
			...
			// =============================================================
			enum StateFlag : uint8 { kFlagPaused = 1 << 0, kFlagGameOver = 1 << 1, kFlagGoalFlash = 1 << 2 };

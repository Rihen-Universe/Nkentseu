...
#undef None
		...
		class NetworkSession {
			public:
				...
				NetworkRole Role() const noexcept {
					return mRole;
				}

				int PeerCount() const noexcept {
					return mPeerCount.load();
				}
				...
				bool Broadcast(const uint8 *data, uint32 size, uint8 reliable = 1);
				...
				void SetLocalIdentity(const char *country, const char *city, const char *code) noexcept;

				/// Identifiant Pays/Ville-Code du peer distant (vide tant qu'on
				/// n'a pas recu son PktHello). Garanti null-terminated.
				const char *PeerCountry() const noexcept {
					return mPeerCountry;
				}

				const char *PeerCity() const noexcept {
					return mPeerCity;
				}

				const char *PeerCode() const noexcept {
					return mPeerCode;
				}

				/// true des qu'on a recu un Hello (utilisable pour gate l'UI).
				bool HasPeerIdentity() const noexcept {
					return mPeerCountry[0] != '\0';
				}
				...
				bool IsValidatedByHost() const noexcept {
					return mValidatedByHost;
				}

				/// true si on a recu PktReject (l'host nous a refuse).
				bool WasRejectedByHost() const noexcept {
					return mRejectedByHost;
				}
			...
			private:
				...
				void UpdateChallengerIdentity(uint64 peerId, const char *country, const char *city,

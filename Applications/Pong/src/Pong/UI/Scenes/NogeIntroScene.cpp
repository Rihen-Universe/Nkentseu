...
namespace nkentseu {
	namespace pong {
		...
		void NogeIntroScene::OnEnter(AppContext &ctx) {
			...
			mRihenLogoLoaded = mRihenLogo.LoadFromFile(*ctx.renderer->GetBackend(), "Resources/Pong/Textures/logo.svg");
		}
		...
		// ─────────────────────────────────────────────────────────────────────
		static void DrawNeonHexagon(renderer::NkRenderer2D &r, float cx, float cy, float radius, float progress,
		...
		void NogeIntroScene::OnRender(AppContext &ctx) {
			...
			if (t > kT_HexDraw) {
				...
				r.DrawFilledTriangle({cx, hexY - dSize}, {cx + dSize, hexY}, {cx, hexY + dSize},
									 AlphaF(theme::Cyan(), 0.95f));
				r.DrawFilledTriangle({cx, hexY - dSize}, {cx, hexY + dSize}, {cx - dSize, hexY},
									 AlphaF(theme::Cyan(), 0.95f));
			}
			...
			if (mRihenLogoLoaded) {
				const float aspect =
					((mRihenLogo.GetHeight() > 0) ? (float)mRihenLogo.GetWidth() / (float)mRihenLogo.GetHeight()
												  : 1.0f);
			...
			if (t >= kT_Hold) {
				...
				if (a > 1.0f)
					a = 1.0f;
				r.DrawFilledRect({0.0f, 0.0f, (float)W, (float)H}, AlphaF(theme::Dark(), a));
			}
		}

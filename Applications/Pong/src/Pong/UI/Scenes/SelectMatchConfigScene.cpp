...
namespace nkentseu {
	namespace pong {
		...
		static void DrawObsIcon(renderer::NkRenderer2D &r, int iconId, float bx, float by, float bs, math::NkColor c) {
			...
			switch (iconId) {
				...
				case 3:
					...
					break;
				case 4:
					r.DrawFilledTriangle({cx, cy - hb * 0.5f}, {cx + hb * 0.45f, cy}, {cx, cy + hb * 0.5f}, c);
					r.DrawFilledTriangle({cx, cy - hb * 0.5f}, {cx - hb * 0.45f, cy}, {cx, cy + hb * 0.5f}, c);
					break;
				case 5:
					...
					for (int i = 0; i < 8; ++i) {
						...
						r.DrawLine({cx + math::NkCos(a) * r1, cy + math::NkSin(a) * r1},
								   {cx + math::NkCos(a) * r2, cy + math::NkSin(a) * r2}, c, 1.5f);
					}
					break;
				case 6:
				...
				default: {
					...
					for (int i = 0; i < 5; ++i) {
						...
						r.DrawLine({cx + math::NkCos(a1) * rad, cy + math::NkSin(a1) * rad},
								   {cx + math::NkCos(a2) * rad, cy + math::NkSin(a2) * rad}, c, 1.5f);
					}
					break;
				}
			}
		...
		void SelectMatchConfigScene::OnRender(AppContext &ctx) {
			...
			for (float x = 0; x < (float)W; x += step)
				r.DrawFilledRect({x, 0.0f, 1.0f, (float)H}, grid);
			for (float y = 0; y < (float)H; y += step)
				r.DrawFilledRect({0.0f, y, (float)W, 1.0f}, grid);
			...
			// ── Section PARAMETRES DU MATCH (toujours visible, en 1er) ──────
			{
				...
				// Ligne 1 : POINTS POUR GAGNER
				{
					...
					if (screenLY + lineH >= scrollTop - 4.0f && screenLY <= scrollBot + 4.0f) {
						...
						r.DrawFilledRect({mScoreMinusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH},
										 {0, 245, 255, 40});
						r.DrawRectOutline({mScoreMinusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH},
										  {0, 245, 255, 200}, 1.5f);
						...
						r.DrawFilledRect({mScorePlusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH},
										 {0, 245, 255, 40});
						r.DrawRectOutline({mScorePlusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH},
										  {0, 245, 255, 200}, 1.5f);
				...
				// Ligne 2 : TEMPS LIMITE
				{
					...
					if (screenLY + lineH >= scrollTop - 4.0f && screenLY <= scrollBot + 4.0f) {
						...
						r.DrawFilledRect({mTimeMinusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH},
										 {0, 245, 255, 40});
						r.DrawRectOutline({mTimeMinusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH},
										  {0, 245, 255, 200}, 1.5f);
						...
						r.DrawFilledRect({mTimePlusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH}, {0, 245, 255, 40});
						r.DrawRectOutline({mTimePlusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH},
										  {0, 245, 255, 200}, 1.5f);
				...
				// Ligne 3 : VITESSE BALLE (stepper multiplicateur >= 1)
				{
					...
					if (screenLY + lineH >= scrollTop - 4.0f && screenLY <= scrollBot + 4.0f) {
						...
						r.DrawFilledRect({mSpeedMinusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH},
										 {0, 245, 255, 40});
						r.DrawRectOutline({mSpeedMinusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH},
										  {0, 245, 255, 200}, 1.5f);
						...
						r.DrawFilledRect({mSpeedPlusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH},
										 {0, 245, 255, 40});
						r.DrawRectOutline({mSpeedPlusX, screenLY + (lineH - btnH) * 0.5f, btnW, btnH},
										  {0, 245, 255, 200}, 1.5f);
			...
			auto renderGrid = [&](const char *title, int currentIndex, float *outX, float *outY) {
				...
				for (int i = 0; i < kDiffCount; ++i) {
					...
					r.DrawRectOutline({bx, screenBY, cardW, cardH}, bord, focused ? 2.5f * scale : 1.0f);
			...
			if (mEditingObsIndex >= 0 && mEditingObsIndex < kObsCount) {
				...
				r.DrawFilledRect({mEditPanelX, mEditPanelY, pW, pH}, {8, 16, 28, 250});
				r.DrawRectOutline({mEditPanelX, mEditPanelY, pW, pH}, d.color, 2.0f * scale);
				...
				mEditBtnH = closeS;
				r.DrawFilledRect({mEditCloseX, mEditCloseY, closeS, closeS}, {255, 64, 64, 60});
				r.DrawRectOutline({mEditCloseX, mEditCloseY, closeS, closeS}, {255, 64, 64, 200}, 1.5f);
			...
			if (mMaxScroll > 0.5f) {
				...
				r.DrawFilledRect({sbX, thumbY, sbW, thumbH}, {0, 245, 255, 180});
			}
		}

...
namespace nkentseu {
	namespace pong {
		...
		static const int kSectionsN = (int)(sizeof(kSections) / sizeof(kSections[0]));
		...
		static void DrawObstacleIcon(renderer::NkRenderer2D &r,
									 ...
									 float cx, float cy, float rad, float pulse01) {
			switch (kind) {
				...
				case 4: // Mine : cercle rouge + pics
					...
					for (int k = 0; k < 8; ++k) {
						...
						r.DrawLine({cx + math::NkCos(a) * r1, cy + math::NkSin(a) * r1},
								   {cx + math::NkCos(a) * r2, cy + math::NkSin(a) * r2}, MakeColor(255, 64, 64, 230),
								   1.5f);
					}
					break;
		...
		static void DrawOrbIcon(renderer::NkRenderer2D &r, math::NkColor color, bool isBonus, float cx, float cy,
								float rad) {
		...
		static float IllustrationHeight(IllustrationKind kind, float scale) {
			...
			switch (kind) {
				case kIllObstacles:
					return iconH * 2 + pad; // grille 4x2 icone+label
				case kIllBonus:
					return iconH + pad;
				case kIllMalus:
					return iconH + pad;
		...
		static float DrawIllustration(renderer::NkRenderer2D &r, FontAtlas &f, IllustrationKind kind, float x, float y,
									  float w, float scale, float anim) {
			...
			switch (kind) {
				case kIllObstacles: {
					...
					static const char *kLbl[8] = {"MUR",  "PORTAIL", "GRAVITE", "AIMANT",
					...
					for (int i = 0; i < 8; ++i) {
						...
						f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, cx, iconCY + rad + 10.0f * scale,
				...
				case kIllBonus: {
					...
					static const B kB[6] = {
						{{255, 215, 0, 255}, "GEANT"},	 {{0, 245, 255, 255}, "VITESSE"},
						{{255, 180, 60, 255}, "DBL PT"}, {{0, 200, 255, 255}, "BOUCLIER"},
						{{80, 255, 100, 255}, "LENTE"},	 {{255, 255, 255, 255}, "SURPRISE"},
					};
					const int cols = 6;
					const float cellW = w / cols;
					const float cellH = kIconRowH * scale;
				...
				case kIllMalus: {
					...
					static const M kM[6] = {
						{{80, 0, 120, 255}, "AVEUGLE"},	  {{160, 160, 160, 255}, "MINI"},
						{{255, 60, 200, 255}, "INVERSE"}, {{120, 200, 255, 255}, "GEL"},
						{{255, 60, 60, 255}, "RAPIDE"},	  {{255, 255, 255, 255}, "TELEPORT"},
					};
					const int cols = 6;
					const float cellW = w / cols;
					const float cellH = kIconRowH * scale;
				...
				case kIllModes: {
					...
					for (int i = 0; i < 4; ++i) {
						...
						r.DrawFilledCircle({pX + pondW * 0.5f, pY + pondH * 0.5f}, 2.0f * scale, {255, 255, 255, 220},
										   10);
						// Marqueurs IA pour mode 1 (droite) et 2 (les deux)
						if (i == 1 || i == 2) {
							f.DrawStringScaled(r, FontAtlas::SmallSlot, scale, pX + pondW - 8.0f * scale,
											   padYR - 6.0f * scale, "AI", {255, 107, 0, 220});
						}
						if (i == 2) {
							f.DrawStringScaled(r, FontAtlas::SmallSlot, scale, pX, padYL - 6.0f * scale, "AI",
											   {0, 245, 255, 220});
						}
						f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, cx, cy + pondH * 0.55f, kLbl[i],
												   {255, 255, 255, 200});
					}
					return rowH + pad;
				}
				...
				case kIllControls: {
					...
					// Cellule 1 : P1 = W/S
					{
						...
						r.DrawRectOutline({cx - keyW * 0.5f, cy - keyH - 2.0f * scale, keyW, keyH}, {0, 245, 255, 220},
										  1.0f);
						f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, cx, cy - keyH + 3.0f * scale, "W",
												   {255, 255, 255, 240});
						...
						f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, cx, cy + 6.0f * scale, "S",
												   {255, 255, 255, 240});
						f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, cx, cy + keyH + 10.0f * scale, "P1",
												   {0, 245, 255, 220});
					}
					// Cellule 2 : P2 = UP/DOWN
					{
						...
						r.DrawRectOutline({cx - keyW * 0.5f, cy - keyH - 2.0f * scale, keyW, keyH}, {255, 107, 0, 220},
										  1.0f);
						f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, cx, cy - keyH + 3.0f * scale, "^",
												   {255, 255, 255, 240});
						...
						f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, cx, cy + 6.0f * scale, "v",
												   {255, 255, 255, 240});
						f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, cx, cy + keyH + 10.0f * scale, "P2",
												   {255, 107, 0, 220});
					}
					// Cellule 3 : Touch drag
					{
						...
						f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, cx, cy + keyH + 10.0f * scale,
												   "DRAG", {255, 255, 255, 220});
					}
					return rowH + pad + 12.0f * scale;
				}
				...
				case kIllDrops: {
					...
					for (int i = 0; i < 2; ++i) {
						...
						// Fleche vers le bas
						r.DrawLine({cx, cy + rad + 3.0f * scale}, {cx, cy + rad + 16.0f * scale}, {255, 255, 255, 180},
								   1.5f);
						r.DrawLine({cx, cy + rad + 16.0f * scale}, {cx - 4.0f * scale, cy + rad + 12.0f * scale},
								   {255, 255, 255, 180}, 1.5f);
						r.DrawLine({cx, cy + rad + 16.0f * scale}, {cx + 4.0f * scale, cy + rad + 12.0f * scale},
								   {255, 255, 255, 180}, 1.5f);
						f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, cx, cy + rad + 22.0f * scale,
												   isB ? "BONUS" : "MALUS", {255, 255, 255, 200});
					}
					return rowH + pad + 10.0f * scale;
				}
				...
				case kIllParams: {
					...
					const float keyW = 22.0f * scale;
					{
						const float cx = x + cellW * 0.5f;
						const float cy = y + rowH * 0.45f;
						r.DrawFilledRect({cx - 38.0f * scale, cy - 12.0f * scale, keyW, 24.0f * scale},
										 {0, 245, 255, 40});
						r.DrawRectOutline({cx - 38.0f * scale, cy - 12.0f * scale, keyW, 24.0f * scale},
										  {0, 245, 255, 220}, 1.0f);
						f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, cx - 27.0f * scale, cy - 7.0f * scale,
												   "-", {0, 245, 255, 240});
						f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, cx, cy - 7.0f * scale, "11",
												   {255, 255, 255, 240});
						r.DrawFilledRect({cx + 16.0f * scale, cy - 12.0f * scale, keyW, 24.0f * scale},
										 {0, 245, 255, 40});
						r.DrawRectOutline({cx + 16.0f * scale, cy - 12.0f * scale, keyW, 24.0f * scale},
										  {0, 245, 255, 220}, 1.0f);
						f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, cx + 27.0f * scale, cy - 7.0f * scale,
												   "+", {0, 245, 255, 240});
						// Label decale vers le bas pour ne pas chevaucher le bouton +.
						f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, cx, cy + 24.0f * scale, "SCORE",
												   {255, 255, 255, 200});
					}
					{
						const float cx = x + cellW * 1.5f;
						...
						f.DrawStringCenteredScaled(r, FontAtlas::SubtitleSlot, scale, cx, cy - 8.0f * scale, "01:30",
												   {0, 245, 255, 240});
						f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, cx, cy + 24.0f * scale, "TEMPS",
												   {255, 255, 255, 200});
					}
					return rowH + pad + 8.0f * scale;
				}
				...
				case kIllObjectif: {
					...
					const float cy = y + rowH * 0.5f - labelH * 0.3f;
					f.DrawStringCenteredScaled(r, FontAtlas::SmallSlot, scale, cx, cy - 22.0f * scale, "PREMIER A",
											   {255, 255, 255, 180});
					f.DrawStringCenteredScaled(r, FontAtlas::DisplaySlot, scale, cx, cy - 8.0f * scale, "21",
											   {0, 245, 255, 240});
					return rowH + pad;
				}
		...
		bool RulesScene::HitTestBack(float sx, float sy) const {
			return sx >= mBackX && sx <= mBackX + mBackW && sy >= mBackY && sy <= mBackY + mBackH;
		}

		int RulesScene::HitTestTitle(float wx, float wy) const {
			if (wx < mTitleX || wx > mTitleX + mTitleW)
				return -1;
		...
		void RulesScene::OnRender(AppContext &ctx) {
			...
			const float scrollBot = (float)ctx.safe.TopY() + (float)ctx.safe.SafeH() - mBottomReserve;
			...
			for (int s = 0; s < mNumSections; ++s) {
				...
				if (screenTitleY + titleBarH >= scrollTop - titleClipMargin &&
					...
					math::NkColor bg = isOpen ? math::NkColor{0, 245, 255, (uint8_t)(50 * enterA)}
											  : math::NkColor{0, 245, 255, (uint8_t)(20 * enterA)};
					math::NkColor bd = isOpen ? math::NkColor{0, 245, 255, (uint8_t)(220 * enterA)}
											  : math::NkColor{0, 245, 255, (uint8_t)(90 * enterA)};
					r.DrawFilledRect({gridLeft, screenTitleY, availW, titleBarH}, bg);
					r.DrawRectOutline({gridLeft, screenTitleY, availW, titleBarH}, bd, isOpen ? 2.0f : 1.5f);

					// Texte titre, centre vertical
					math::NkColor txt = isOpen ? math::NkColor{255, 255, 255, (uint8_t)(255 * enterA)}
											   : math::NkColor{255, 255, 255, (uint8_t)(220 * enterA)};
					f.DrawStringScaled(r, FontAtlas::SubtitleSlot, scale, gridLeft + titlePadL,
									   screenTitleY + titleBarH * 0.22f, sec.title, txt);

					// Chevron a droite : "v" si ferme, "^" si ouvert.
					const char *chev = isOpen ? "-" : "+";
					f.DrawStringScaled(r, FontAtlas::SubtitleSlot, scale, gridLeft + availW - chevPadR,
									   screenTitleY + titleBarH * 0.22f, chev, txt);
				}
				worldY += titleBarH;
				...
				if (isOpen) {
					...
					if (illH > 0.0f && illScreenY + illH >= scrollTop - bodyClip &&
						illScreenY <= scrollBot + bodyClip) {
						DrawIllustration(r, f, sec.illustration, illX, illScreenY, illW, scale, mTime);
					}
					worldY += illH;
					if (illH > 0.0f)
						worldY += illGap; // gap entre illustration et corps
					...
					while (*p) {
						...
						if (screenLY >= scrollTop - bodyClip && screenLY <= scrollBot + bodyClip) {
							math::NkColor c = {255, 255, 255, (uint8_t)(220 * enterA)};
							f.DrawStringScaled(r, FontAtlas::BodySlot, scale, gridLeft + bodyPad, screenLY, line, c);
						}
						worldY += lineH;
					}
			...
			// qui scrolle dessous). Bande horizontale + RETOUR + titre.
			{
				...
				r.DrawFilledRect({0.0f, scrollTop - hdrSep, (float)W, hdrSep}, {0, 245, 255, 120});
				...
				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, scale, mBackX + mBackW * 0.5f,
										   mBackY + mBackH * 0.18f, "RETOUR", theme::Cyan());
				f.DrawStringCenteredScaled(r, FontAtlas::HeadlineSlot, scale, (float)W * 0.5f,
										   (float)ctx.safe.TopY() + Pct::H(H, 0.024f, 10.0f, 32.0f), "REGLES DU JEU",
										   theme::White());
			}
		}

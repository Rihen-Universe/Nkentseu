...
namespace nkentseu {
	namespace pong {
		...
		static void DrawMiniField(renderer::NkRenderer2D &r, float x, float y, float w, float h, float t) {
			...
			for (int i = 0; i < segs; ++i) {
				...
				if (a < 0.0f)
					a = 0.0f;
				r.DrawFilledRect({x + i * segW, midY - 1.0f, segW + 1.0f, 2.0f}, AlphaF(theme::Cyan(), a * 0.30f));
			}

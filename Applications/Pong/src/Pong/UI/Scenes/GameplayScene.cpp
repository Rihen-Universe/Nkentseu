...
namespace nkentseu {
	namespace pong {
		...
		static math::NkColor ColP1() {
			return {0, 245, 255, 255};
		} // cyan

		static math::NkColor ColP2() {
			return {255, 107, 0, 255};
		} // orange

		static math::NkColor ColField() {
			return {0, 245, 255, 30};
		} // border arene

		static math::NkColor ColDash() {
			return {255, 255, 255, 20};
		} // ligne pointillee

		static math::NkColor ColRing() {
		...
		void GameplayScene::OnEnter(AppContext &ctx) {
			...
			mTimeLeft = (ctx.settings != nullptr && ctx.settings->timeLimit > 0.0f) ? ctx.settings->timeLimit : 0.0f;
			mTimeUp = 0.0f;
			mGameOver = false;
			mWinner = 0;
			mTrailHead = 0;
			...
			mHUDTopH = math::NkMin(kHUDTopHBase * mScale, (float)ctx.viewportH * 0.12f);
			mHUDBotH = math::NkMin(kHUDBotHBase * mScale, (float)ctx.viewportH * 0.12f);
			...
			mIsNetwork = (ctx.network != nullptr && ctx.network->State() == NetworkState::Connected);
			...
			if (mIsNetwork) {
				logger.Info("[Gameplay] Mode reseau actif role={0}", mIsHost ? "HOST" : "CLIENT");
			}
			...
			if (ctx.settings != nullptr) {
				...
				logger.Info("[Gameplay] Obstacles spawn: {0} (seed={1})", mObstacles.Count(),
			...
			const float mul =
				(ctx.settings != nullptr && ctx.settings->ballSpeedMul >= 1.0f) ? ctx.settings->ballSpeedMul : 1.0f;
			mBallSpeedMul = mul;
			...
			logger.Info("[Gameplay] OnEnter arena {0}x{1} scale={2}", (int)arenaW, (int)arenaH, mScale);
		}
		...
		void GameplayScene::RequestPauseToggle(AppContext &ctx) {
			...
			if (ctx.network != nullptr) {
				...
				ctx.network->Broadcast(reinterpret_cast<const uint8 *>(&pkt), sizeof(pkt), 1 /*reliable*/);
		...
		// ────────────────────────────────────────────────────────────────────
		void GameplayScene::EmitBurstNet(AppContext &ctx, float x, float y, int count, math::NkColor color,
										 float speedMin, float speedMax, float lifeMin, float lifeMax, float sizeMin,
										 float sizeMax) {
			// Emission locale (HOST et mode local).
			mParticles.EmitBurst(x, y, count, color, speedMin, speedMax, lifeMin, lifeMax, sizeMin, sizeMax);
			...
			const float aH = math::NkMax(1.0f, (float)ctx.viewportH - mHUDTopH - mHUDBotH);
			if (aW <= 0.f || aH <= 0.f)
				return;
			...
			pkt.yN = y / aH;
			pkt.colorRGBA =
				((uint32)color.r) | ((uint32)color.g << 8) | ((uint32)color.b << 16) | ((uint32)color.a << 24);
			...
			ctx.network->Broadcast(reinterpret_cast<const uint8 *>(&pkt), sizeof(pkt), 0 /*unreliable*/);
		}
		...
		// ────────────────────────────────────────────────────────────────────
		void GameplayScene::EmitGoalNet(AppContext &ctx, float wallX, float arenaH, math::NkColor color) {
			// Emission locale (HOST et mode local).
		...
		void GameplayScene::OnUpdate(AppContext &ctx, float dt) {
			...
			if (mIsNetwork && ctx.network != nullptr && !mNetPeerLost) {
				...
				if (netSt != NetworkState::Connected) {
					mNetPeerLost = true;
					mNetLostTimer = 0.0f;
					logger.Info("[Gameplay] Adversaire parti (state={0})", (int)netSt);
				}
			}
			if (mNetPeerLost) {
				mNetLostTimer += dt;
			...
			if (mIsNetwork && ctx.network != nullptr) {
				...
				for (const auto &msg : incoming) {
					...
					if (sRxDiagCount < 5) {
						logger.Info("[Net][RX] role={0} type={1} size={2}", mIsHost ? "HOST" : "CLIENT", (int)type,
									(int)msg.size);
						sRxDiagCount++;
					}

					if (mIsHost && type == netproto::kMsgInput && msg.size >= sizeof(netproto::PktInput)) {
						...
						mPaused = !mPaused;
						logger.Info("[Net][RX] HOST pause toggle -> {0}", mPaused ? "paused" : "resumed");
						...
						mParticles.EmitBurst((float)px, (float)py, (int)pb.count, col, speedMin, speedMax, lifeMin,
											 lifeMax, sizeMin, sizeMax);
					...
					} else if (!mIsHost && type == netproto::kMsgState && msg.size >= sizeof(netproto::PktState)) {
						...
						const int n = math::NkMin((int)st.numDrops, (int)netproto::kMaxDropsSync);
						for (int k = 0; k < n; ++k) {
						...
						// differente entre HOST et CLIENT).
						{
							...
							mObstacles.Clear();
							const int nObs = math::NkMin((int)st.numObstacles, (int)netproto::kMaxObstaclesSync);
							for (int k = 0; k < nObs; ++k) {
								...
								mObstacles.AppendFromNet(t, s, xTL, yTL, wPx, hPx, rot, active, visible);
							}
						}
						// Le flash de but cote client : on declenche localement
						// si le snapshot dit qu'un but vient d'arriver.
						if ((st.flags & netproto::kFlagGoalFlash) != 0 && mGoalFlashAlpha <= 0.0f) {
			...
			if (mPaddleLY > arenaH - mPaddleH)
				mPaddleLY = arenaH - mPaddleH;
			if (mPaddleRY > arenaH - mPaddleH)
				mPaddleRY = arenaH - mPaddleH;
			if (mPaddleLY < 0.0f)
				mPaddleLY = 0.0f;
			if (mPaddleRY < 0.0f)
				mPaddleRY = 0.0f;
			...
			if (mIsNetwork && !mIsHost) {
				...
				if (arenaH > 0.0f)
					mNetLocalPaddleYN = mPaddleRY / arenaH;
				const float spdN = (mPaddleSpd * dt60) / math::NkMax(1.0f, arenaH);
				if (mKeyW || mKeyUp)
					mNetLocalPaddleYN -= spdN;
				if (mKeyS || mKeyDown)
					mNetLocalPaddleYN += spdN;
				const float pHN = mPaddleH / math::NkMax(1.0f, arenaH);
				if (mNetLocalPaddleYN < 0.0f)
					mNetLocalPaddleYN = 0.0f;
				if (mNetLocalPaddleYN > 1.0f - pHN)
					mNetLocalPaddleYN = 1.0f - pHN;
				...
				if (mNetSendInputTimer >= kInputDt && ctx.network != nullptr) {
					...
					const bool ok =
						ctx.network->Broadcast(reinterpret_cast<const uint8 *>(&in), sizeof(in), 0 /*unreliable*/);
					static float sLastTxYN = -1.0f;
					if (math::NkFabs(sLastTxYN - in.paddleYN) > 0.02f) {
						logger.Info("[Net][TX] CLIENT input paddleYN={0} ok={1}", in.paddleYN, ok ? "true" : "false");
			...
			if (mIsNetwork && mIsHost && ctx.network != nullptr) {
				...
				if (mNetSendStateTimer >= kStateDt) {
					...
					uint8 flags = 0;
					if (mPaused)
						flags |= netproto::kFlagPaused;
					if (mGameOver)
						flags |= netproto::kFlagGameOver;
					if (mGoalFlashAlpha > 0.5f)
						flags |= netproto::kFlagGoalFlash;
					st.flags = flags;
					...
					uint8 effL = 0, effR = 0;
					if (mPowerUps.IsFrozen(-1))
						effL |= netproto::kEffFrozen;
					if (mPowerUps.IsBlind(-1))
						effL |= netproto::kEffBlind;
					if (mPowerUps.HasInvertedControls(-1))
						effL |= netproto::kEffInverted;
					if (mPowerUps.IsFrozen(+1))
						effR |= netproto::kEffFrozen;
					if (mPowerUps.IsBlind(+1))
						effR |= netproto::kEffBlind;
					if (mPowerUps.HasInvertedControls(+1))
						effR |= netproto::kEffInverted;
					st.effFlagsL = effL;
					st.effFlagsR = effR;
					// Drops (jusqu'a kMaxDropsSync = 4)
					const int nDrops = math::NkMin((int)netproto::kMaxDropsSync, mPowerUps.DropCount());
					st.numDrops = (uint8)nDrops;
					for (int k = 0; k < nDrops; ++k) {
						const PowerUpDrop &d = mPowerUps.GetDrop(k);
						netproto::PktDropEntry &e = st.drops[k];
						e.isBonusKind = (uint8)((d.isBonus ? 0x80 : 0x00) | (d.kind & 0x7F));
						e.xN = d.x * invW;
						e.yN = d.y * invH;
					}
					...
					auto packNotif = [](const PowerUpSystem::Notification &n, uint8 &outNotif, uint8 &outTimeQ) {
						if (!n.active || n.timeLeft <= 0.0f) {
							outNotif = 0;
							outTimeQ = 0;
							return;
						}
						outNotif = (uint8)((n.isBonus ? 0x80 : 0x00) | (n.kind & 0x7F));
						const float q = n.timeLeft * 100.0f;
						outTimeQ = (uint8)(q > 255.f ? 255 : (q < 0.f ? 0 : q));
					};
					packNotif(mPowerUps.GetNotification(-1), st.notifL, st.notifLeftL_q);
					packNotif(mPowerUps.GetNotification(+1), st.notifR, st.notifLeftR_q);
					...
					const int nObs = math::NkMin((int)netproto::kMaxObstaclesSync, mObstacles.Count());
					st.numObstacles = (uint8)nObs;
					for (int k = 0; k < nObs; ++k) {
						const Obstacle &o = mObstacles.Get(k);
						netproto::PktObstacleEntry &e = st.obstacles[k];
						e.typeShape = (uint8)(((int)o.type & 0x0F) << 4 | ((int)o.shape & 0x0F));
						uint8 ofl = 0;
						if (o.active)
							ofl |= netproto::kObsActive;
						if (o.visible)
							ofl |= netproto::kObsVisible;
						e.flags = ofl;
						...
						while (rotClamped > 3.14159265f)
							rotClamped -= 6.28318530f;
						while (rotClamped < -3.14159265f)
							rotClamped += 6.28318530f;
					...
					ctx.network->Broadcast(reinterpret_cast<const uint8 *>(&st), sizeof(st), 0 /*unreliable*/);
				}
			}
			...
			if (mPowerUps.TryTeleport(-1, fracY))
				mPaddleLY = (arenaH - pHL) * fracY;
			if (mPowerUps.TryTeleport(+1, fracY))
				mPaddleRY = (arenaH - pHR) * fracY;
			...
			if (mPowerUps.CheckPaddleCollision(-1, mPaddleLX, mPaddleLY, mPaddleW, pHL, &catchX, &catchY, &catchCol)) {
				EmitBurstNet(ctx, catchX, catchY, 18, catchCol, 60.0f, 220.0f, 0.4f, 0.8f, 2.0f, 4.0f);
			}
			if (mPowerUps.CheckPaddleCollision(+1, mPaddleRX, mPaddleRY, mPaddleW, pHR, &catchX, &catchY, &catchCol)) {
				EmitBurstNet(ctx, catchX, catchY, 18, catchCol, 60.0f, 220.0f, 0.4f, 0.8f, 2.0f, 4.0f);
			}
		...
		void GameplayScene::StepPaddles(float arenaH, float dt60) {
			...
			if (frozenL) {
				...
				mPaddleLY += mAILeft.Update(dt, mBallX, mBallY, mBallVX, mBallVY, mPaddleLX, mPaddleLY, pHL,
											(float)0.0f, arenaH, mPaddleSpd * mPowerUps.GetPaddleSpeedMul(-1));
			} else {
			...
			if (frozenR) {
				...
				const float dt = dt60 / 60.0f;
				mPaddleRY += mAIRight.Update(dt, mBallX, mBallY, mBallVX, mBallVY, mPaddleRX, mPaddleRY, pHR,
											 (float)0.0f, arenaH, mPaddleSpd * mPowerUps.GetPaddleSpeedMul(+1));
			...
			if (mPaddleLY < 0.0f)
				mPaddleLY = 0.0f;
			if (mPaddleLY > arenaH - pHL)
				mPaddleLY = arenaH - pHL;
			if (mPaddleRY < 0.0f)
				mPaddleRY = 0.0f;
			if (mPaddleRY > arenaH - pHR)
				mPaddleRY = arenaH - pHR;
		}
		...
		void GameplayScene::StepBall(AppContext &ctx, float arenaW, float arenaH, float dt60) {
			...
			if (mGoalCooldown <= 0.0f) {
				...
				if (mBallX - mBallR < 0.0f) {
					TriggerGoal(ctx, -1, arenaW, arenaH);
					return;
				}
				if (mBallX + mBallR > arenaW) {
					TriggerGoal(ctx, +1, arenaW, arenaH);
					return;
				}
			}
			...
			for (int i = 0; i < 2; ++i) {
				...
				if (mBallX + mBallR > px && mBallX - mBallR < px + pw && mBallY + mBallR > py &&
					mBallY - mBallR < py + ph) {
					...
					if (mIsNetwork) {
						EmitBurstNet(ctx, mBallX, mBallY, 14, col, 80.0f, 260.0f, 0.25f, 0.6f, 1.8f, 3.5f);
					} else {
						mParticles.EmitDirectional(mBallX, mBallY, 14, col, ang, 0.6f, 80.0f, 260.0f, 0.25f, 0.6f, 1.8f,
												   3.5f);
					}
			...
			// pas annules par le plancher (sinon plancher ~4 alors que init = 40).
			{
				...
				const float spd = math::NkSqrt(mBallVX * mBallVX + mBallVY * mBallVY);
				if (spd < minSpd) {
					if (spd > 0.001f) {
			...
			const ObstacleHit oh = mObstacles.CheckCollision(mBallX, mBallY, mBallR, mBallVX, mBallVY, dt60);
			...
			if (oh.bonusStarCollected && mLastToucher != 0) {
				if (!mIsNetwork || mIsHost)
					mPowerUps.ApplyRandomBonus(mLastToucher);
				EmitBurstNet(ctx, mBallX, mBallY, 24,
							 oh.particleColor.a > 0 ? oh.particleColor : math::NkColor{255, 215, 0, 255}, 80.0f, 280.0f,
							 0.5f, 1.0f, 2.0f, 4.0f);
			}
			...
			if (oh.hit && oh.particleCount > 0) {
				EmitBurstNet(ctx, mBallX, mBallY, oh.particleCount, oh.particleColor, 60.0f, 200.0f, 0.3f, 0.7f, 1.5f,
							 3.0f);
			}
			...
			for (int c = 0; c < oh.chainCount; ++c) {
				EmitBurstNet(ctx, oh.chainX[c], oh.chainY[c], 18, oh.particleColor, 50.0f, 220.0f, 0.4f, 0.8f, 1.8f,
							 3.5f);
			}
			if (oh.hit) {
				if (oh.reflectX)
		...
		void GameplayScene::TriggerGoal(AppContext &ctx, int side, float arenaW, float arenaH) {
			...
			if (mPowerUps.ConsumeShield(scoredOnSide)) {
				...
				logger.Info("[Gameplay] Shield absorb goal P{0}", scoredOnSide > 0 ? 2 : 1);
				return;
			}
			...
			if (side > 0)
				...
				mScoreR += goalValue;
			logger.Info("[Gameplay] GOAL P{0} (+{1}) ! score {2} - {3}", side > 0 ? 1 : 2, goalValue, mScoreL, mScoreR);
		...
		void GameplayScene::CheckWinConditions(const GameSettings &s) {
			...
			if (maxScore > 0 && (mScoreL >= maxScore || mScoreR >= maxScore)) {
				if (!s.winByTwo || absDiff >= 2) {
					...
					logger.Info("[Gameplay] GAME OVER (score) winner=P{0}", mWinner > 0 ? 1 : (mWinner < 0 ? 2 : 0));
					return;
				}
			}
			...
			if (s.timeLimit > 0.0f && mTimeLeft <= 0.0f) {
				mGameOver = true;
				mWinner = (diff > 0) ? +1 : (diff < 0 ? -1 : 0);
				logger.Info("[Gameplay] GAME OVER (time) winner=P{0}", mWinner > 0 ? 1 : (mWinner < 0 ? 2 : 0));
			}
		}
		...
		// Rendu
		// ─────────────────────────────────────────────────────────────────────
		static void DrawArenaField(renderer::NkRenderer2D &r, float ax, float ay, float aw, float ah, float scale) {
			...
			for (float x = ax; x < ax + aw; x += gridStep) {
				r.DrawFilledRect({x, ay, lineThick, ah}, gridCol);
			}
			for (float y = ay; y < ay + ah; y += gridStep) {
				r.DrawFilledRect({ax, y, aw, lineThick}, gridCol);
			}
			// Bordure cyan
			r.DrawRectOutline({ax, ay, aw, ah}, ColField(), lineThick);
			...
			for (float yy = ay + 4.0f * scale; yy < ay + ah - 4.0f * scale; yy += dashStep) {
				r.DrawFilledRect({midX - lineThick * 0.5f, yy, lineThick, dashLen}, ColDash());
			}
			// Cercle central r=50 scaled
			r.DrawCircleOutline({midX, ay + ah * 0.5f}, 50.0f * scale, ColRing(), lineThick, 48);
			// Goal glows lateraux (4 bandes degrade) scales
			const float bandW = 10.0f * scale;
			for (int i = 0; i < 4; ++i) {
				const float a = 1.0f - (float)i / 4.0f;
				math::NkColor cl = ColP1();
				cl.a = static_cast<uint8_t>(45 * a);
				math::NkColor cr = ColP2();
				cr.a = static_cast<uint8_t>(45 * a);
				r.DrawFilledRect({ax + i * bandW, ay, bandW, ah}, cl);
				r.DrawFilledRect({ax + aw - (i + 1) * bandW, ay, bandW, ah}, cr);
			}
		}
		...
		static void DrawHUDTop(renderer::NkRenderer2D &r, FontAtlas &f, int W, float hudH, float scale, int scoreL,
							   int scoreR, float timeShown, int maxScore, bool countdown) {
			r.DrawFilledRect({0, 0, (float)W, hudH}, {5, 10, 20, 235});
			r.DrawFilledRect({0, hudH - 1, (float)W, 1.0f}, {0, 245, 255, 26});
			...
			r.DrawCircleOutline({gx + avatarR, cyH}, avatarR, ColP1(), 2.0f * scale, 32);
			r.DrawFilledCircle({gx + avatarR, cyH}, avatarR2, {0, 245, 255, 38}, 32);
			f.DrawStringCenteredScaled(r, slotLabel, scale, gx + avatarR, cyH - 7.0f * scale, "P1", ColP1());
			const float textOff = (avatarR * 2.0f) + 10.0f * scale;
			f.DrawStringScaled(r, slotLabel, scale, gx + textOff, 10.0f * scale, "JOUEUR 1", ColP1());
			char buf[16];
			std::snprintf(buf, sizeof(buf), "%d", scoreL);
			f.DrawStringShadowScaled(r, slotScore, scale, gx + textOff, 22.0f * scale, buf, {255, 255, 255, 255},
									 ColP1(), 2);
			...
			f.DrawStringCenteredScaled(r, slotCenter, scale, (float)W * 0.5f, 6.0f * scale, tbuf, {255, 255, 255, 240});
			// Label dynamique selon maxScore + mode timer.
			char labelBuf[48];
			if (maxScore > 0) {
				std::snprintf(labelBuf, sizeof(labelBuf), "1 VS 1 - %d PTS%s", maxScore, countdown ? "" : "  (CHRONO)");
			} else {
				std::snprintf(labelBuf, sizeof(labelBuf), "1 VS 1 - %s", countdown ? "TEMPS LIMITE" : "CHRONO");
			}
			f.DrawStringCenteredScaled(r, slotMatch, scale, (float)W * 0.5f, 30.0f * scale, labelBuf,
									   {255, 255, 255, 80});

			const float dx = (float)W - marginX;
			r.DrawCircleOutline({dx - avatarR, cyH}, avatarR, ColP2(), 2.0f * scale, 32);
			r.DrawFilledCircle({dx - avatarR, cyH}, avatarR2, {255, 107, 0, 38}, 32);
			f.DrawStringCenteredScaled(r, slotLabel, scale, dx - avatarR, cyH - 7.0f * scale, "P2", ColP2());
			...
			f.DrawStringScaled(r, slotLabel, scale, dx - textOff - nameW, 10.0f * scale, "JOUEUR 2", ColP2());
			f.DrawStringShadowScaled(r, slotScore, scale, dx - textOff - scoreW, 22.0f * scale, buf,
									 {255, 255, 255, 255}, ColP2(), 2);
		}

		static void DrawHUDBottom(renderer::NkRenderer2D &r, FontAtlas &f, int W, int H, float hudH, float scale,
								  bool paused) {
			const float by = (float)H - hudH;
			r.DrawFilledRect({0, by, (float)W, hudH}, {5, 10, 20, 235});
			r.DrawFilledRect({0, by, (float)W, 1.0f}, {0, 245, 255, 26});

			const FontAtlas::SizeSlot slotHint = FontAtlas::SmallSlot;

			f.DrawStringCenteredScaled(r, slotHint, scale, (float)W * 0.5f, by + hudH * 0.20f,
									   "P1 : W / S    OU GLISSER GAUCHE          P2 : FLECHES    OU GLISSER DROITE",
									   {255, 255, 255, 100});
			f.DrawStringCenteredScaled(r, slotHint, scale, (float)W * 0.5f, by + hudH * 0.55f,
		...
		// ─────────────────────────────────────────────────────────────────────
		static void DrawBallAndTrail(renderer::NkRenderer2D &r, float ax, float ay, float bx, float by, float br,
									 const float *trX, const float *trY, int trHead, int trCount, int trMax) {
			...
			for (int i = 0; i < trCount; ++i) {
				...
				r.DrawFilledCircle({ax + trX[idx], ay + trY[idx]}, rad * 1.8f, glow, 16);
				// Cercle blanc translucide
				math::NkColor c = {255, 255, 255, static_cast<uint8_t>(180.0f * t)};
				r.DrawFilledCircle({ax + trX[idx], ay + trY[idx]}, rad, c, 16);
			}
			...
			for (int i = 5; i >= 1; --i) {
				...
				r.DrawFilledCircle({ax + bx, ay + by}, radius, halo, 24);
			}
			// Cercle blanc plein au centre
			r.DrawFilledCircle({ax + bx, ay + by}, br, {255, 255, 255, 255}, 24);
		}
		...
		// ─────────────────────────────────────────────────────────────────────
		static void DrawPaddle(renderer::NkRenderer2D &r, float x, float y, float w, float h, math::NkColor c) {
			...
			for (int i = 6; i >= 1; --i) {
				...
				r.DrawFilledRect({x - pad, y - pad, w + pad * 2.0f, h + pad * 2.0f}, glow);
			}
			// Outline lumineux (rebord net)
			math::NkColor edge = c;
			edge.a = 220;
			r.DrawRectOutline({x - 1.0f, y - 1.0f, w + 2.0f, h + 2.0f}, edge, 1.0f);
			// Paddle plein
			r.DrawFilledRect({x, y, w, h}, c);
			// Highlight central blanc tres leger (effet "verre" du neon HTML)
			r.DrawFilledRect({x + 1.0f, y + 1.0f, w - 2.0f, h * 0.35f}, {255, 255, 255, 50});
		}
		...
		void GameplayScene::OnRender(AppContext &ctx) {
			...
			if (ah > 0.0f) {
				...
				DrawPaddle(r, ax + mPaddleLX, ay + mPaddleLY, mPaddleW, pHL_r, ColP1());
				DrawPaddle(r, ax + mPaddleRX, ay + mPaddleRY, mPaddleW, pHR_r, ColP2());

				// Trail + balle (par-dessus tout)
				DrawBallAndTrail(r, ax, ay, mBallX, mBallY, mBallR, mTrailX, mTrailY, mTrailHead, mTrailCount,
								 kTrailMax);
				...
				if (mPowerUps.IsBlind(-1)) {
					r.DrawFilledRect({ax, ay, aw * 0.5f, ah}, {0, 0, 0, 180});
				}
				if (mPowerUps.IsBlind(+1)) {
					r.DrawFilledRect({ax + aw * 0.5f, ay, aw * 0.5f, ah}, {0, 0, 0, 180});
				}
				...
				if (mGoalFlashAlpha > 0.0f) {
					...
					r.DrawFilledRect({ax, ay, aw, ah}, c);
				}
				...
				// gauche sont alignes a gauche, ceux du joueur droit a droite.
				{
					...
					for (int i = 0; i < mPowerUps.EffectCount(); ++i) {
						...
						const float bx = (e.side < 0) ? (8.0f * mScale) : ((float)W - badgeW - 8.0f * mScale);
						const float by = mHUDTopH + 6.0f * mScale + cnt * (badgeH + pad);
						...
						math::NkColor bd = fg;
						bd.a = 200;
						r.DrawFilledRect({bx, by, badgeW, badgeH}, bg);
						r.DrawRectOutline({bx, by, badgeW, badgeH}, bd, 1.5f);
						// Pastille a gauche du texte
						const float dotR = badgeH * 0.30f;
						r.DrawFilledCircle({bx + 8.0f * mScale + dotR, by + badgeH * 0.5f}, dotR, fg, 12);
						// Texte nom (court)
						f.DrawStringScaled(r, FontAtlas::SmallSlot, mScale,
										   bx + 8.0f * mScale + dotR * 2.0f + 4.0f * mScale, by + badgeH * 0.20f,
										   mPowerUps.GetEffectName(i), {255, 255, 255, 230});
						// Barre de temps en bas du badge.
						const float barH = 2.0f * mScale;
						const float barW = badgeW - 4.0f * mScale;
						const float frac =
							(e.duration > 0.001f) ? math::NkClamp(e.timeLeft / e.duration, 0.0f, 1.0f) : 1.0f;
						r.DrawFilledRect({bx + 2.0f * mScale, by + badgeH - barH - 2.0f, barW, barH},
										 {255, 255, 255, 40});
						r.DrawFilledRect({bx + 2.0f * mScale, by + badgeH - barH - 2.0f, barW * frac, barH}, bd);
						++cnt;
					}
				}
				...
				for (int side = -1; side <= 1; side += 2) {
					...
					const float tx =
						(side < 0) ? ((float)W * 0.25f - toastW * 0.5f) : ((float)W * 0.75f - toastW * 0.5f);
					...
					math::NkColor bg = c;
					bg.a = static_cast<uint8_t>(160 * alpha);
					math::NkColor bd = c;
					bd.a = static_cast<uint8_t>(240 * alpha);
					r.DrawFilledRect({tx, ty, toastW, toastH}, bg);
					r.DrawRectOutline({tx, ty, toastW, toastH}, bd, 2.0f);

					// Prefixe + nom : "+ BOUCLIER" ou "- AVEUGLE"
					char buf[64];
					std::snprintf(buf, sizeof(buf), "%s %s", n.isBonus ? "+" : "-", mPowerUps.NotifName(n));
					math::NkColor txtCol = {255, 255, 255, 255};
					txtCol.a = static_cast<uint8_t>(255 * alpha);
					f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, mScale, tx + toastW * 0.5f, ty + toastH * 0.20f,
											   buf, txtCol);
				}
			}

			// HUDs : choisir mode timer selon GameSettings
			const bool countdown = (ctx.settings != nullptr && ctx.settings->timeLimit > 0.0f);
			const float timeShown = countdown ? mTimeLeft : mTimeUp;
			const int maxScore = (ctx.settings != nullptr) ? ctx.settings->maxScore : 11;
			DrawHUDTop(r, f, W, mHUDTopH, mScale, mScoreL, mScoreR, timeShown, maxScore, countdown);
			...
			// les avatars/scores P1/P2 (extremites).
			{
				...
				// Fond + border cyan
				math::NkColor bg = ColP1();
				bg.a = 30;
				math::NkColor bd = ColP1();
				bd.a = 180;
				r.DrawFilledRect({mPauseBtnX, mPauseBtnY, mPauseBtnW, mPauseBtnH}, bg);
				r.DrawRectOutline({mPauseBtnX, mPauseBtnY, mPauseBtnW, mPauseBtnH}, bd, 1.5f * mScale);
				...
				if (mPaused) {
					...
					r.DrawFilledTriangle({ix - tw * 0.5f, iy - ih}, {ix - tw * 0.5f, iy + ih}, {ix + tw * 0.5f, iy},
										 ColP1());
				} else {
					// 2 barres pause
					const float bw = mPauseBtnW * 0.10f;
					const float gap = mPauseBtnW * 0.08f;
					r.DrawFilledRect({ix - gap - bw, iy - ih, bw, ih * 2.0f}, ColP1());
					r.DrawFilledRect({ix + gap, iy - ih, bw, ih * 2.0f}, ColP1());
				}
			}
			...
			if (mGameOver) {
				r.DrawFilledRect({0, 0, (float)W, (float)H}, {5, 10, 20, 230});
				const float cx = (float)W * 0.5f;
				const float cy = (float)H * 0.5f;

				// Titre dynamique selon vainqueur
				const char *title = (mWinner > 0) ? "VICTOIRE  P1" : (mWinner < 0) ? "VICTOIRE  P2" : "EGALITE";
				...
				f.DrawStringShadowCenteredScaled(r, FontAtlas::HeadlineSlot, mScale, cx, titleY, title,
												 {255, 255, 255, 255}, titleC, 3);

				char scoreBuf[24];
				std::snprintf(scoreBuf, sizeof(scoreBuf), "%d  -  %d", mScoreL, mScoreR);
				f.DrawStringCenteredScaled(r, FontAtlas::DisplaySlot, mScale, cx, scoreY, scoreBuf,
										   {255, 255, 255, 230});
				...
				math::NkColor pbg = ColP1();
				pbg.a = 32;
				math::NkColor pbd = ColP1();
				pbd.a = 200;
				r.DrawFilledRect({mGOReplayBtnX, mGOReplayBtnY, btnW, btnH}, pbg);
				r.DrawRectOutline({mGOReplayBtnX, mGOReplayBtnY, btnW, btnH}, pbd, 2.0f * mScale);
				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, mScale, cx, mGOReplayBtnY + btnH * 0.30f,
				...
				r.DrawFilledRect({mGOMenuBtnX, mGOMenuBtnY, btnW, btnH}, mbg);
				r.DrawRectOutline({mGOMenuBtnX, mGOMenuBtnY, btnW, btnH}, mbd, 2.0f * mScale);
				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, mScale, cx, mGOMenuBtnY + btnH * 0.30f,
										   "RETOUR MENU  (ECHAP)", {255, 64, 64, 230});
			}
			...
			// Les NkClamp evitent les valeurs degenerees aux extremes.
				...
				r.DrawFilledRect({0, 0, (float)W, (float)H}, {5, 10, 20, 220});
				const float cx = (float)W * 0.5f;
				const float cy = (float)H * 0.5f;
				...
				const float totalH = titleH + subtitleH + 4.0f * btnH + 3.0f * btnGap;
				const float blockTop = cy - totalH * 0.5f;
				const float titleY = blockTop;
				const float subtitleY = titleY + titleH;

				// Titre PAUSE
				f.DrawStringShadowCenteredScaled(r, FontAtlas::HeadlineSlot, mScale, cx, titleY, "PAUSE",
												 {255, 255, 255, 255}, ColP1(), 3);
				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, mScale, cx, subtitleY, "LE MATCH EST EN ATTENTE",
										   {255, 255, 255, 120});
				...
				mResumeBtnY = y0;
				math::NkColor rbg = ColP1();
				rbg.a = 32;
				math::NkColor rbd = ColP1();
				rbd.a = 200;
				r.DrawFilledRect({mResumeBtnX, mResumeBtnY, mResumeBtnW, mResumeBtnH}, rbg);
				r.DrawRectOutline({mResumeBtnX, mResumeBtnY, mResumeBtnW, mResumeBtnH}, rbd, 2.0f * mScale);
				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, mScale, cx, mResumeBtnY + btnH * 0.30f,
				...
				r.DrawFilledRect({mConfigBtnX, mConfigBtnY, mConfigBtnW, mConfigBtnH}, cbg);
				r.DrawRectOutline({mConfigBtnX, mConfigBtnY, mConfigBtnW, mConfigBtnH}, cbd, 2.0f * mScale);
				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, mScale, cx, mConfigBtnY + btnH * 0.30f,
				...
				r.DrawFilledRect({mModeBtnX, mModeBtnY, mModeBtnW, mModeBtnH}, jbg);
				r.DrawRectOutline({mModeBtnX, mModeBtnY, mModeBtnW, mModeBtnH}, jbd, 2.0f * mScale);
				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, mScale, cx, mModeBtnY + btnH * 0.30f, "CHOIX DU JEU",
										   {224, 160, 255, 240});
				...
				math::NkColor mbg = {255, 64, 64, 32};
				math::NkColor mbd = {255, 64, 64, 200};
				r.DrawFilledRect({mMenuBtnX, mMenuBtnY, mMenuBtnW, mMenuBtnH}, mbg);
				r.DrawRectOutline({mMenuBtnX, mMenuBtnY, mMenuBtnW, mMenuBtnH}, mbd, 2.0f * mScale);
				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, mScale, cx, mMenuBtnY + btnH * 0.30f,
										   "RETOUR MENU  (ECHAP)", {255, 64, 64, 230});
			}
			...
			if (mNetPeerLost) {
				...
				r.DrawFilledRect({0.0f, 0.0f, W, H}, {0, 0, 0, (uint8)(180 * fade)});
				...
				r.DrawFilledRect({px, py, panelW, panelH}, {24, 28, 36, (uint8)(230 * fade)});
				r.DrawRectOutline({px, py, panelW, panelH}, {255, 80, 80, (uint8)(220 * fade)}, 2.0f * mScale);
				// Titre
				f.DrawStringCenteredScaled(r, FontAtlas::HeadlineSlot, mScale, cx, py + padTop, "ADVERSAIRE PARTI",
										   {255, 80, 80, (uint8)(250 * fade)});
				// Sous-titre
				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, mScale, cx, py + padTop + lineGap,
										   "LE MATCH EST TERMINE.", {255, 255, 255, (uint8)(220 * fade)});
				f.DrawStringCenteredScaled(
					r, FontAtlas::SmallSlot, mScale, cx, py + padTop + lineGap + Pct::H(vH, 0.035f, 18.0f, 36.0f),
					"TON ADVERSAIRE A QUITTE OU PERDU LA CONNEXION.", {255, 255, 255, (uint8)(180 * fade)});
				...
				r.DrawFilledRect({mLostMenuBtnX, mLostMenuBtnY, mLostMenuBtnW, mLostMenuBtnH},
								 {255, 64, 64, (uint8)(60 * fade)});
				r.DrawRectOutline({mLostMenuBtnX, mLostMenuBtnY, mLostMenuBtnW, mLostMenuBtnH},
								  {255, 64, 64, (uint8)(220 * fade)}, 2.0f * mScale);
				f.DrawStringCenteredScaled(r, FontAtlas::BodySlot, mScale, cx, mLostMenuBtnY + 14.0f * mScale,
										   "RETOUR MENU  (ECHAP)", {255, 200, 200, (uint8)(240 * fade)});
			}
		}
		...
		void GameplayScene::OnEvent(AppContext &ctx, NkEvent &ev) {
			...
			if (auto *kp = ev.As<NkKeyPressEvent>()) {
				...
				if (mNetPeerLost) {
					if (k == NkKey::NK_ESCAPE || k == NkKey::NK_ENTER || k == NkKey::NK_NUMPAD_ENTER ||
						k == NkKey::NK_SPACE) {
				...
				// pour distinguer "client pas focus" de "bug code").
				{
					...
					if (sKeyDiag < 8) {
						logger.Info("[Gameplay][KP] role={0} key={1}",
									mIsNetwork ? (mIsHost ? "HOST" : "CLIENT") : "LOCAL", (int)k);
						sKeyDiag++;
					}
				}
				switch (k) {
			...
			auto handleUIButtons = [&](float x, float y) -> bool {
				...
				if (mNetPeerLost) {
					if (PointInRect(x, y, mLostMenuBtnX, mLostMenuBtnY, mLostMenuBtnW, mLostMenuBtnH)) {
						ctx.scenes->PopToRoot();
						return true;
					}
				...
				if (mGameOver) {
					if (PointInRect(x, y, mGOReplayBtnX, mGOReplayBtnY, mGOReplayBtnW, mGOReplayBtnH)) {
						RequestReplay(ctx);
						return true;
					}
					if (PointInRect(x, y, mGOMenuBtnX, mGOMenuBtnY, mGOMenuBtnW, mGOMenuBtnH)) {
			...
			if (auto *tb = ev.As<NkTouchBeginEvent>()) {
				for (uint32 i = 0; i < tb->GetNumTouches(); ++i) {
					...
					if (handleUIButtons(tp.clientX, tp.clientY))
						continue;

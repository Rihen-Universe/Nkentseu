...
namespace nkentseu {
	namespace demo {
		...
		bool FontAtlas::Init() {
			...
			logger.Info("[FontAtlas] atlas built: {0}x{1}, texId={2}", w, h, mTextureId);
			return true;
		}
		...
		// ─────────────────────────────────────────────────────────────────────
		float FontAtlas::DrawString(GLRenderer2D &r, SizeSlot s, float x, float y, const char *text,
									math::NkColor color) {
			...
			while (*p != '\0') {
				...
				if (g->visible) {
					...
					r.DrawTexturedQuad(gx, gy, gw, gh, g->u0, g->v0, g->u1, g->v1, color);
				}
				cursorX += g->advanceX;
			}
			return cursorX - x;
		...
		float FontAtlas::DrawStringCentered(GLRenderer2D &r, SizeSlot s, float cx, float y, const char *text,
											math::NkColor color) {
		...
		float FontAtlas::DrawStringScaled(GLRenderer2D &r, SizeSlot s, float scale, float x, float y, const char *text,
										  math::NkColor color) {
			if (text == nullptr || mFonts[s] == nullptr || mTextureId == 0 || scale <= 0.0f) {
				return 0.0f;
			}
			NkFont *font = mFonts[s];
			r.BindTexture(mTextureId);
		...
		float FontAtlas::DrawStringCenteredScaled(GLRenderer2D &r, SizeSlot s, float scale, float cx, float y,
		...
		void FontAtlas::DrawStringShadowScaled(GLRenderer2D &r, SizeSlot s, float scale, float x, float y,
		...
		void FontAtlas::DrawStringShadowCenteredScaled(GLRenderer2D &r, SizeSlot s, float scale, float cx, float y,
													   const char *text, math::NkColor textColor,
		...
		// ─────────────────────────────────────────────────────────────────────
		void FontAtlas::DrawStringShadow(GLRenderer2D &r, SizeSlot s, float x, float y, const char *text,
		...
		void FontAtlas::DrawStringShadowCentered(GLRenderer2D &r, SizeSlot s, float cx, float y, const char *text,
												 math::NkColor textColor, math::NkColor shadowColor, int blur) {
			const float w = MeasureWidth(s, text);

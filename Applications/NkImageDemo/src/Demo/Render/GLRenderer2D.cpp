...
namespace nkentseu {
	namespace demo {
		...
		void GLRenderer2D::PushQuad(float x, float y, float w, float h, float u0, float v0, float u1, float v1,
		...
		void GLRenderer2D::DrawCircleOutline(float cx, float cy, float radius, math::NkColor c, float thickness,
											 int segments) {
			if (segments < 3)
				segments = 3;
			for (int i = 0; i < segments; ++i) {
		...
		void GLRenderer2D::DrawTexturedQuad(float x, float y, float w, float h, float u0, float v0, float u1, float v1,
											math::NkColor c) {
			// useTex = 1.0 -> alpha mask (font atlas GL_R8)
			PushQuad(x, y, w, h, u0, v0, u1, v1, c, 1.0f);
		}

		void GLRenderer2D::DrawTexturedQuadRGBA(float x, float y, float w, float h, float u0, float v0, float u1,
												float v1, math::NkColor c) {

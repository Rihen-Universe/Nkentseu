...
namespace nkentseu {
	namespace demo {
		...
		static const char *kSupportedExts[] = {
			".png", ".PNG", ".jpg", ".JPG", ".jpeg", ".JPEG", ".bmp", ".BMP", ".tga", ".TGA", ".gif", ".GIF",
			".hdr", ".HDR", ".qoi", ".QOI", ".webp", ".WEBP", ".svg", ".SVG", ".ppm", ".PPM", ".ico", ".ICO",
		};
		...
		void ViewerApp::Render() {
			...
			if (mAnimTextures.Size() > 0 && mAnimFrame >= 0 && (uint32)mAnimFrame < mAnimTextures.Size()) {
			...
			if (cur != nullptr && cur->IsValid()) {
				...
				mRenderer.DrawTexturedQuadRGBA(drawX, drawY, drawW, drawH, 0.0f, 0.0f, 1.0f, 1.0f,
											   {255, 255, 255, 255});
			}
			...
			if (mLoaded) {
				char buf[256];
				std::snprintf(buf, sizeof(buf), "[%d/%d]  %s", mCurrentIdx + 1, (int)mFiles.Size(),
							  mMeta.fileName.CStr());
				mFont.DrawStringScaled(mRenderer, FontAtlas::BodySlot, scale, 12.0f, hudY + 12.0f, buf,
									   {255, 255, 255, 230});

				if (mMeta.animated && mAnimTextures.Size() > 1) {
					std::snprintf(buf, sizeof(buf), "%s  %dx%d  frame %d/%d  (%u ms)", mMeta.format.CStr(), mMeta.width,
								  mMeta.height, mAnimFrame + 1, (int)mAnimTextures.Size(),
								  (unsigned)mAnimDelaysMs[(uint32)mAnimFrame]);
				} else if (mIsHdr) {
					std::snprintf(buf, sizeof(buf), "%s  %dx%d  RGB96F  exposure=%.2f  gamma=%.2f", mMeta.format.CStr(),
								  mMeta.width, mMeta.height, mHdrExposure, mHdrGamma);
				} else {
					std::snprintf(buf, sizeof(buf), "%s  %dx%d  %d-channel", mMeta.format.CStr(), mMeta.width,
								  mMeta.height, mMeta.channels);
				}
				mFont.DrawStringScaled(mRenderer, FontAtlas::SmallSlot, scale, 12.0f, hudY + 36.0f, buf,
									   {0, 245, 255, 220});
			} else {
				mFont.DrawStringCenteredScaled(mRenderer, FontAtlas::BodySlot, scale, W * 0.5f, hudY + 18.0f,
											   "AUCUN FICHIER IMAGE TROUVE", {255, 100, 100, 220});
			}

			// Hints clavier coin droit. Sur HDR on rajoute le tip exposure.
			if (mIsHdr) {
				mFont.DrawStringScaled(mRenderer, FontAtlas::SmallSlot, scale, W - 380.0f, hudY + 36.0f,
									   "<- ->: NAV  +/-: EXPOSURE  ECHAP: QUIT", {255, 255, 255, 140});
			} else {
				mFont.DrawStringScaled(mRenderer, FontAtlas::SmallSlot, scale, W - 240.0f, hudY + 36.0f,
									   "<- ->: NAV     ECHAP: QUIT", {255, 255, 255, 140});
			}
		...
		void ViewerApp::ScanFolder(const NkString &folder) {
			...
			NkVector<NkString> entries = NkDirectory::GetFiles(folder.CStr(), "*");
			uint32 added = 0;
			...
			logger.Info("[Viewer] Scan {0} -> {1} fichier(s) image", folder.CStr(), added);
		}
		...
		void ViewerApp::LoadCurrent() {
			...
			if (isHdr) {
				...
				if (data != nullptr && bytes > 0) {
					...
					if (hdr.IsValid()) {
						...
						if (mTexture.IsValid()) {
							mLoaded = true;
							logger.Info("[Viewer] HDR {0}  ({1}x{2}, RGB96F, exposure={3})", mMeta.fileName.CStr(),
				...
				if (data != nullptr && bytes > 0) {
					...
					if (anim != nullptr && anim->frameCount > 0) {
						...
						if (mAnimTextures.Size() > 0) {
							mLoaded = true;
							logger.Info("[Viewer] GIF anim {0}  ({1}x{2}, {3} frames, loop={4})", mMeta.fileName.CStr(),
										mMeta.width, mMeta.height, mMeta.frameCount, mAnimLoopCount);
							return;
						}
					}
			...
			mLoaded = true;
			logger.Info("[Viewer] Loaded {0}  ({1}x{2}, {3}ch, {4})", mMeta.fileName.CStr(), mMeta.width, mMeta.height,

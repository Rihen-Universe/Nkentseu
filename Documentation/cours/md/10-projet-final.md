...
c'est la plus utile : **il assemble**.

Nous allons construire, du fichier de build jusqu'à l'exécutable qui s'affiche,
une petite application appelée `NkAtelier`. Elle tient en un seul
`main.cpp`, elle ne dépend d'**aucun fichier externe** — pas une
...
1. **Panneau « Image »** — une image *fabriquée à l'exécution*
   par les primitives de dessin de NKImage, redimensionnée en vignette,
   envoyée au backend et affichée par le widget `Image`. Un curseur
   change une couleur ; l'image est refabriquée et ré-uploadée.
2. **Panneau « Son »** — trois boutons qui jouent trois sons
   *synthétisés* par `AudioGenerator`, sur le bus « UI », avec
   une case à cocher pour couper ce bus.
3. **Panneau « Vidéo »** — au démarrage, l'application *écrit*
   une petite vidéo MJPEG dans un fichier AVI, puis la *relit* image
...
>
> Une application d'apprentissage qui exige un `logo.png` et un
> `clic.wav` échoue à la première exécution, parce que le répertoire
...
> (`Applications/NKGuiDemo/src/NKGuiDemo/main.cpp:294-299`).
>
> Ici, chaque ressource est **produite** :
>
> - la police vient de `NkFontEmbedded` (chapitre 6) ;
> - l'image est dessinée par `NkImage::FillRect` et consorts
>   (chapitre 5) ;
> - le son est synthétisé par `AudioGenerator::GenerateTone`
>   (chapitre 7) ;
> - la vidéo est écrite par `NkVideoWriter` avant d'être relue
>   (chapitre 8).
>
> **L'application ne peut pas échouer faute de fichier.** C'est également ce
> qui la rend facile à donner à quelqu'un d'autre.

## Étape 0 — l'arborescence
...
## Étape 1 — le fichier de build

### Le contenu

Le fichier ci-dessous est modelé sur
`Applications/NKGuiDemo/NKGuiDemo.jenga`, allégé de ce dont nous n'avons
pas besoin (la réflexion, la détection de Vulkan) et augmenté de ce qu'il nous
faut (`NKAudio`, `NKMedia`).
...
- `windowedapp()` — application fenêtrée, pas console. Sous Windows,
  cela évite qu'une console noire s'ouvre derrière votre fenêtre ;
- `nkentseudependson([…])` — la liste des modules. Elle
  *doit* contenir tout ce que vous incluez, transitivement ou non ;
- `extra_includes` — `NKGlad` est nécessaire parce que le
  backend OpenGL de NkCanvas expose ses en-têtes dans les vôtres ;
- les blocs `with filter(…)` — la configuration par plateforme.
  Ne les inventez pas : recopiez-les d'une application existante qui
  fonctionne.

> **⚠️ La liste des dépendances n'est pas décorative**
>
> Un module absent de `nkentseudependson` ne provoque pas une erreur claire
> « module manquant ». Il produit soit une erreur de compilation sur un en-tête
> introuvable — encore lisible —, soit une erreur d'*édition de liens* sur des
> symboles non résolus, qui ne dit pas quel module ajouter.
>
> Le réflexe : quand un symbole `nkentseu::quelquechose` manque à l'édition
> de liens, cherchez dans quel module il est déclaré, et ajoutez ce module.

### Enregistrer le projet dans l'espace de travail

Un `.jenga` isolé n'est pas construit. Il faut l'inclure depuis le
descripteur racine, `Nkentseu.jenga`, à côté des autres applications :
...
## Étape 2 — le squelette : fenêtre, contexte, backend, police
...
D'abord, **on inclut l'en-tête parapluie de NKGui**
(`NKGui/NKGui.h`) et jamais un en-tête interne — le chapitre 1 en a donné
la raison : c'est lui qui neutralise les macros de X11 sous Linux.

Ensuite, **on n'inclut pas d'en-tête parapluie pour NKMedia** : il n'en
existe pas. On prend les deux sous-en-têtes précis dont on a besoin.

### Fenêtre et cible de rendu
...
Le point d'entrée s'appelle `nkmain` et vient de
`NKWindow/NKMain.h` : c'est lui qui gère les différences de démarrage
entre plateformes.

> **⚠️ Si rien ne s'affiche, changez de backend**
>
...
> (`Kernel/Runtime/NKCanvas/ROADMAP.md`, lignes 185-200 et 601-607).
>
> Ces états sont datés et le dépôt bouge — mais si votre fenêtre s'ouvre noire,
> **la première chose à essayer n'est pas de relire votre code** : c'est de
> remplacer `NK_GFX_API_DX11` par `NK_GFX_API_OPENGL` et de
> relancer. Cinq secondes, et cela élimine la moitié des causes possibles.

### Contexte NKGui et backend
...
**L'ordre compte** : le backend reçoit le renderer, donc le renderer doit
exister. Et, souvenez-vous du chapitre 5, toutes les textures doivent être
créées *après* l'initialisation du renderer — ce qui est le cas ici,
...
ajoutez un champ de saisie, il vous faudra en plus `NkTextInputEvent` et
la traduction des touches d'édition — la démo en donne le modèle complet
(`Applications/NKGuiDemo/src/NKGuiDemo/main.cpp:367-405`).

### La boucle, et l'invariant d'ordre
...
>
> `événements` → `BeginFrame(dt)` →
> `widgets` → `EndFrame()` → `Clear`
> → `Submit` × 2 → `Display`.
>
> Les **deux** `Submit` ne sont pas une coquille : le second envoie la
...
Le bornage de `dt` mérite aussi un mot. Quand l'utilisateur déplace la
fenêtre, le système bloque la boucle : le *delta time* suivant peut valoir
...
2. `Resize` renvoie un pointeur, donc `Free()` obligatoire —
   y compris si vous ajoutez un `return` anticipé après cette ligne ;
3. la recopie passe par `RowPtr(y)`, pas par un `memcpy`
   global ;
4. le tampon `rgba` est `static` : on ne réalloue pas 147 Ko à
   chaque changement de curseur.

### L'appeler, et l'afficher
...
>
> Le test `teinte != teinteAppliquee` est la seule chose qui sépare une
> application fluide d'une application qui redessine et ré-uploade une texture
> soixante fois par seconde sans raison.
>
> C'est un réflexe à prendre : dans une interface en mode immédiat, *déclarer*
> un widget est bon marché, mais *réagir* à un widget doit être conditionné au
> changement. Les widgets qui renvoient `true` en cas de modification —
> `SliderFloat`, `Checkbox`, `DragFloat` — sont faits pour
> cela.

## Étape 5 — le son (NKAudio)

### Synthétiser trois sons

`AudioGenerator` produit des `AudioSample` sans aucun fichier.
...
Remarquez que `Play` est appelé sans récupérer le *handle* : ce sont
des sons courts, on ne les pilotera pas. Remarquez aussi le `if` autour de
`GetBus` : il renvoie `nullptr` si le moteur n'est pas initialisé.

> **⚠️ N'écrivez jamais `cfg.backend = DIRECTSOUND`**
>
> Rappel du chapitre 7 : le backend DirectSound est un stub qui renvoie
> `true` sans rien ouvrir
> (`Kernel/Runtime/NKAudio/src/NKAudio/NkAudioBackends.cpp:445-451`). Vous
> auriez un moteur « initialisé », des voix « en lecture », et le silence complet.
> Laissez `AUTO`.

## Étape 6 — la vidéo (NKMedia)
...
Notez que `Close()` est appelé **sur tous les chemins**, y compris
quand `ok` est devenu `false`. C'est la règle du chapitre 8 : sans
finalisation, le fichier existe et n'est lisible par personne.

Notez aussi la construction de la ligne : `rgb + y * w * 3`. Ici, pas de
*stride* aligné — nous fabriquons nous-mêmes un tampon contigu, c'est ce que
`WriteFrame` attend. Le *stride* est une notion de `NkImage`,
pas de `NkVideoWriter`.
...
si `Open` échoue, c'est que l'écriture a mal tourné.

### Cadencer et afficher
...
> 3. **`NkVideoFrame` est déclarée hors boucle.** Son
>    `NkVector` garde sa capacité ; la déclarer à l'intérieur
>    provoquerait une allocation par image.

> **⚠️ Le rebouclage n'est exact que sur les formats non compressés en temps**
>
> `SeekFrame(0)` fonctionne bien ici parce que notre vidéo est en MJPEG :
> chaque image est indépendante. Sur du H.264, `SeekFrame(n)` ne fait que
...
> **⚠️ L'upload répété dépend de `NkTexture::Update`**
>
> Regardez l'implémentation du pont : `UploadImageRGBA` crée la texture au
> premier appel, puis appelle `tex->Update(...)` aux suivants
> (`Kernel/Runtime/NKCanvas/src/NKCanvas/UI/NkGuiCanvasBackend.h:94-117`).
> C'est exactement le bon comportement — on ne recrée pas la texture par image.
>
> Mais souvenez-vous de l'avertissement de Pong (chapitre 6) : « `Update`
> exige `gTextureBackend.Update` qui n'est pas câblé sur tous les backends
> → échec silencieux » (`Applications/Pong/src/Pong/Render/FontAtlas.cpp:107-109`).
>
...
`Build/Bin/Debug-Windows/NkAtelier/` — ou
`Debug-Linux`, `Debug-macOS` selon la plateforme, suivant le
`targetdir` que nous avons écrit dans le `.jenga`.
...
> **⚠️ Le répertoire courant au lancement**
>
> Notre application écrit `nkatelier_demo.avi` **dans le répertoire
...
>
> Ce n'est pas grave ici — nous écrivons *et* lisons au même endroit dans la
> même exécution — mais c'est exactement le mécanisme qui fait qu'un
> `assets/logo.png` « introuvable » l'est seulement dans certaines
...
## Le catalogue des silences

Voici la partie la plus utile de ce chapitre. Toutes les pannes ci-dessous ont
un point commun : **aucune ne produit de message d'erreur**. Gardez cette
liste sous la main.

| **Symptôme** | **Cause à vérifier en premier** |
|---|---|
| Fenêtre noire, rien du tout | Backend graphique. Essayez `NK_GFX_API_OPENGL`. |
| Tout s'affiche sauf les textes | Atlas de police non uploadé, ou `ctx.font` non affecté. |
| Les menus passent *derrière* | Second `Submit(ctx.dlOverlay, …)` oublié. |
| L'image apparaît « penchée » | `memcpy` global au lieu de `RowPtr(y)`. |
| L'image est vide | Texture créée avant `renderer.Initialize()`. |
| Aucun son, mais tout indique que ça joue | Backend `DIRECTSOUND` demandé explicitement. |
| Grésillement ou plantage à la fermeture | `Free(sample)` appelé avant `engine.Shutdown()`. |
| Le fichier vidéo est illisible | `Close()` non appelé sur le `NkVideoWriter`. |
| La vidéo reste sur la première image | `NkTexture::Update` non câblé dans ce backend. |
| La vidéo est à l'envers | `flipVertical` avec un framebuffer OpenGL. |
| Vignette de mauvaise qualité malgré Lanczos | `NK_LANCZOS3` retombe sur du bilinéaire. |
| Aucun pair ne se connecte | Poignée de main asynchrone : attendre `ConnectedPeerCount()`. |
| Erreurs d'édition de liens `__imp_socket` | `ws2_32` absent du `links()`. |

> **✅ La méthode générale**
>
> Ces silences ont tous la même parade, et c'est une habitude à prendre plutôt
> qu'une astuce : **journalisez ce que vous chargez, et journalisez aussi ce
> que vous ne chargez pas**.
>
> Toutes les fonctions d'initialisation de ce chapitre ont un `else` qui
...
> que fait la démo du dépôt : « `Image demo introuvable (cwd) -> section image vide` »
> (`Applications/NKGuiDemo/src/NKGuiDemo/main.cpp:422`).

## Le programme, vu de haut
...
**La difficulté du moteur n'est pas dans la quantité de code à écrire, elle
...
### Enregistrer ce que l'atelier affiche

Ajoutez un bouton « Enregistrer » qui, à chaque frame,
capture la fenêtre et pousse l'image dans un `NkVideoRecorder` en mode
MJPEG :

**`Esquisse — écrit pour le cours (modèle : NKViewportDemo/main.cpp:329-362)`**
...
silencieusement, et `End()` doit être appelé même si la fenêtre est fermée
brutalement.

### Ajouter le réseau

Ajoutez à `NkAtelier` un quatrième panneau : deux boutons « Héberger » et
...
Vous aurez besoin d'ajouter `NKNetwork` aux dépendances du
`.jenga` *et* `ws2_32` au `links()` sous Windows. Le
chapitre 9 donne la structure à suivre : une petite classe de session, un
`Tick(dt)` par frame, et une interface qui ne voit qu'un état.

### Faire du décodage un travail de fond
...
**mesurer** : entourez `ReadFrame` d'un chronomètre mural et
regardez. Et souvenez-vous de l'avertissement du chapitre 8 : mesurez le temps
mur, pas le *PTS* de la vidéo.

## Exercices

> **✏️ 1 — Faire tomber chaque panneau, un par un**
>
> Pour chacun des quatre modules, cassez volontairement une chose et notez ce que
> vous observez :
>
> - n'appelez pas `UploadFontGray8` ;
> - supprimez le `vignette->Free()` et lancez l'application quelques
...
> **✏️ 2 — Une vraie vignette de fichier**
>
> Ajoutez un cinquième panneau qui charge une image *depuis le disque* avec
> `NkImage::Load`, en essayant plusieurs chemins candidats comme le fait la
> démo du dépôt. Affichez le format d'origine (`SourceFormat()`), les
> dimensions, le nombre de canaux et le *stride*.
>
...
> **✏️ 3 — Le mélangeur**
>
> Ajoutez trois `SliderFloat` — Master, UI, Music — reliés aux bus
> correspondants par `GetBus(…)->SetVolume(…)`, en n'appelant le
> *setter* que lorsque la valeur change.
>
> Ajoutez ensuite un son long en boucle sur le bus « Music »
> (`vp.looping = true`) et vérifiez, à l'oreille, la formule du
...
> **✏️ 4 — Une chaîne complète, de l'image à la vidéo et retour**
>
> Modifiez `EcrireVideoDemo` pour qu'elle ne dessine plus ses pixels à la
> main, mais qu'elle utilise une `NkImage` et ses primitives de dessin —
> comme `ConstruireVignette` — puis qu'elle envoie
> `image.Pixels()` à `WriteFrame`.
>
> Attention : `WriteFrame` attend un tampon **contigu**, et
> `NkImage::Pixels()` a un *stride* aligné sur 4 octets. Choisissez
> donc des dimensions telles que `stride == width * bpp`, ou bien recopiez
...
> **✏️ 5 — Le vôtre**
>
> Reprenez `NkAtelier` et transformez-le en quelque chose qui vous est
...
>
> Une seule contrainte, et c'est celle de tout ce cours : **pour chaque
...
> `.cpp`, pas le commentaire ; le `ROADMAP.md`, pas le
> `.jenga` — et vous savez que la réponse est parfois « non ».

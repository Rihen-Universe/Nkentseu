...
| 4 | ~~Dimensions de la surfacique sans effet~~ **RÉSOLU le 10 août** (`0c6e8a11` + correctif de signe) : diffus exact par facteur de forme (Baum, arête par arête — attention au SENS DU PARCOURS des sommets, le signe s'inverse), spéculaire par point représentatif (Karis). Reste : les **tables LTC** pour le spéculaire exact (à prendre telles quelles) |
| 5 | ~~Pénombre uniforme~~ **RÉSOLU le 10 août** (`95867652`) : PCSS réel via `tShadowAtlasRaw` (binding 12). En mode Pénombre, « Douceur » = taille de la source |

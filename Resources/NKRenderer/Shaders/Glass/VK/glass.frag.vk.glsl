...
layout(set=1,binding=1,std140) uniform ObjectUBO{mat4 model,normalMatrix;vec4 tint;float metallic,roughness,aoStrength,emissiveStrength,normalStrength,clearcoat,clearcoatRoughness,subsurface;vec4 sssColor;}uObj;
layout(set=2,binding=3) uniform sampler2D tAlbedo;layout(set=2,binding=4) uniform sampler2D tNormal;layout(set=0,binding=9) uniform samplerCube tEnvPrefilter;layout(set=2,binding=5) uniform sampler2D tSceneColor;
void main(){

// ============================================================
// render2d.frag.vk.glsl — NKRenderer v5.0 — Render2D Fragment (Vulkan)
// Sync avec embedded GL fallback dans NkRender2D.cpp (kRender2D_FS).
// Difference VK : decorations `set=0, binding=N` au lieu de `binding=N` seul.
// ============================================================
#version 460 core
...
layout(std140, set=0, binding=1) uniform Lights2DUBO {
    Light2D lights[16];
    vec4    ambient;
} uL2D;

layout(std140, set=0, binding=10) uniform Shadows2DUBO {
    vec4 occluders[32];
    vec4 meta;
    vec4 isoMat;
} uSh2D;

layout(std140, set=0, binding=11) uniform ShadowsAABB2DUBO {
    vec4 boxes[32];
    vec4 metaAABB;
} uShAABB2D;
...
layout(set=0, binding=2) uniform sampler2D tCookie0;
layout(set=0, binding=3) uniform sampler2D tCookie1;
layout(set=0, binding=4) uniform sampler2D tCookie2;
layout(set=0, binding=5) uniform sampler2D tCookie3;
layout(set=0, binding=6) uniform sampler2D tCookie4;
layout(set=0, binding=7) uniform sampler2D tCookie5;
layout(set=0, binding=8) uniform sampler2D tCookie6;
layout(set=0, binding=9) uniform sampler2D tCookie7;

layout(set=0, binding=12) uniform sampler2D tNormal;

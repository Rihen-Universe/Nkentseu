...
namespace nkentseu {
	...
	namespace reflection {
		...
		class NKENTSEU_REFLECTION_API NkRegistry {
			public:
				...
				template <typename T> const NkType *GetType() const {
					...
					static NkType type(typeid(T).name(), sizeof(T), alignof(T), DetermineCategory<T>());
					return &type;
				}
				 ...
				 */
				template <typename E> const NkEnumDescriptor *GetEnum() const {
			...
			private:
				 ...
				 */
				NkRegistry() : mTypeCount(0), mClassCount(0), mOnRegisterCallback() {
		...
		template <typename T> auto NkReflectAutoLinkClassImpl(int) -> decltype(&T::GetStaticClass(), void()) {
			const NkClass &cls = T::GetStaticClass();
			const_cast<NkType &>(NkTypeOf<T>()).SetClass(&cls);
		}

		template <typename T> void NkReflectAutoLinkClassImpl(...) {
			// T n'est pas une classe reflechie : rien a lier.
		}

		template <typename T> void NkReflectAutoLinkClass() {
...
public:                                                                                                                \
	static const ::nkentseu::reflection::NkClass &GetStaticClass() {                                                   \
		static ::nkentseu::reflection::NkClass classInfo(#ClassName, sizeof(ClassName),                                \
		...
		static bool s_linked = []() -> bool {                                                                          \
			const_cast<::nkentseu::reflection::NkType &>(::nkentseu::reflection::NkTypeOf<ClassName>())                \
				.SetClass(&classInfo);                                                                                 \
			return true;                                                                                               \
		}();                                                                                                           \
		(void)s_linked;                                                                                                \
 ...
 */
#define NKENTSEU_REFLECT_PROPERTY(PropertyName) NKENTSEU_REFLECT_PROPERTY_FLAGS(PropertyName, 0ULL)
...
public:                                                                                                                \
	static ::nkentseu::reflection::NkProperty &Get##PropertyName##Property() {                                         \
		static ::nkentseu::reflection::NkProperty property(                                                            \
			#PropertyName, ::nkentseu::reflection::NkTypeOf<decltype(((SelfType *)0)->PropertyName)>(),                \
	...
	struct PropertyName##_NkPropReg {                                                                                  \
			PropertyName##_NkPropReg() {                                                                               \
				...
				const_cast<::nkentseu::reflection::NkClass &>(SelfType::GetStaticClass()).AddProperty(&prop);          \
			}                                                                                                          \
	};                                                                                                                 \
	...
	namespace {                                                                                                        \
		struct NKENTSEU_CONCAT(ClassName, _Registrar) {                                                                \
				NKENTSEU_CONCAT(ClassName, _Registrar)() {                                                             \
					::nkentseu::reflection::NkRegistry::Get().RegisterClass(&ClassName::GetStaticClass());             \
				}                                                                                                      \
		};                                                                                                             \

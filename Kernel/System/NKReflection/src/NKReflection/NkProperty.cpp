		...
		nk_bool NkProperty::SetValueGeneric(void *instance, const NkReflectVariant &value) const {
			...
			if (IsReadOnly()) {
				NK_FOUNDATION_LOG_WARN(
					"NkProperty::SetValueGeneric : propriete '%s' en lecture seule, ecriture ignoree", GetName());
				return false;
			}
			...
			if (srcCat == dstCat && value.GetType() != nullptr && value.GetType()->GetSize() == type.GetSize()) {
			...
			if (dstCat == NkTypeCategory::NK_ENUM && value.GetType() != nullptr &&
				...
				switch (n) {
					case 1: {
						nk_int8 v = static_cast<nk_int8>(raw);
						CopyRawBytes(ptr, &v, 1);
						return true;
					}
					case 2: {
						nk_int16 v = static_cast<nk_int16>(raw);
						CopyRawBytes(ptr, &v, 2);
						return true;
					}
					case 4: {
						nk_int32 v = static_cast<nk_int32>(raw);
						CopyRawBytes(ptr, &v, 4);
						return true;
					}
					case 8: {
						nk_int64 v = raw;
						CopyRawBytes(ptr, &v, 8);
						return true;
					}
					default:
						return false;
		...
		nk_bool NkProperty::WritePrimitiveCoerced(void *dst, NkTypeCategory dstCat, const NkReflectVariant &value) {
			switch (dstCat) {
				case NkTypeCategory::NK_BOOL: {
					nk_bool v = value.ToBool();
					CopyRawBytes(dst, &v, sizeof(v));
					return true;
				}
				case NkTypeCategory::NK_INT8: {
					nk_int8 v = static_cast<nk_int8>(value.ToInt64());
					CopyRawBytes(dst, &v, sizeof(v));
					return true;
				}
				case NkTypeCategory::NK_INT16: {
					nk_int16 v = static_cast<nk_int16>(value.ToInt64());
					CopyRawBytes(dst, &v, sizeof(v));
					return true;
				}
				case NkTypeCategory::NK_INT32: {
					nk_int32 v = static_cast<nk_int32>(value.ToInt64());
					CopyRawBytes(dst, &v, sizeof(v));
					return true;
				}
				case NkTypeCategory::NK_INT64: {
					nk_int64 v = value.ToInt64();
					CopyRawBytes(dst, &v, sizeof(v));
					return true;
				}
				case NkTypeCategory::NK_UINT8: {
					nk_uint8 v = static_cast<nk_uint8>(value.ToInt64());
					CopyRawBytes(dst, &v, sizeof(v));
					return true;
				}
				case NkTypeCategory::NK_UINT16: {
					nk_uint16 v = static_cast<nk_uint16>(value.ToInt64());
					CopyRawBytes(dst, &v, sizeof(v));
					return true;
				}
				case NkTypeCategory::NK_UINT32: {
					nk_uint32 v = static_cast<nk_uint32>(value.ToInt64());
					CopyRawBytes(dst, &v, sizeof(v));
					return true;
				}
				case NkTypeCategory::NK_UINT64: {
					nk_uint64 v = static_cast<nk_uint64>(value.ToInt64());
					CopyRawBytes(dst, &v, sizeof(v));
					return true;
				}
				case NkTypeCategory::NK_FLOAT32: {
					nk_float32 v = static_cast<nk_float32>(value.ToFloat64());
					CopyRawBytes(dst, &v, sizeof(v));
					return true;
				}
				case NkTypeCategory::NK_FLOAT64: {
					nk_float64 v = value.ToFloat64();
					CopyRawBytes(dst, &v, sizeof(v));
					return true;
				}
				default:

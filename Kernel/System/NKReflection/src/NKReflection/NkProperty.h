	...
	namespace reflection {
		...
		class NKENTSEU_REFLECTION_API NkProperty {
			public:
				 ...
				 */
				NkProperty(const nk_char *name, const NkType &type, nk_usize offset, nk_uint32 flags = 0)
					: mName(name), mType(&type), mOffset(offset), mFlags(flags), mMetaFlags(NK_REFLECT_META_NONE),
					  mEditMeta(), mContainer(nullptr), mGetter(), mSetter() {
				}
				 ...
				 */
				template <typename T> const T &GetValue(void *instance) const {
					if (mGetter.IsValid()) {
				 ...
				 */
				template <typename T> void SetValue(void *instance, const T &value) const {
				...
				static NkProperty MakeWithAccessors(const nk_char *name, const NkType &type, nk_usize offset,
													GetterType &&getter = GetterType(),
													SetterType &&setter = SetterType(), nk_uint32 flags = 0) {
				...
				static NkProperty MakeFromMember(ClassType *instance, ValueType ClassType::*memberPtr,
												 const nk_char *name, const NkType &typeMeta, nk_uint32 flags = 0) {
					NKENTSEU_UNUSED(instance);
					nk_usize offset = reinterpret_cast<nk_usize>(&(reinterpret_cast<ClassType *>(0)->*memberPtr));
				 ...
				 */
				template <typename ClassType, typename ValueType>
				static NkProperty MakeFromAccessors(ClassType *instance, ValueType (ClassType::*getterPtr)() const,
													void (ClassType::*setterPtr)(ValueType), const nk_char *name,
													const NkType &typeMeta, nk_uint32 flags = 0) {

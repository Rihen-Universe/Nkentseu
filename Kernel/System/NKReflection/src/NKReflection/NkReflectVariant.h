	...
	namespace reflection {
		...
		class NKENTSEU_REFLECTION_API NkReflectVariant {
			public:
				...
				NkReflectVariant() : mType(nullptr), mHeap(nullptr), mSize(0), mIsString(false) {
					ZeroBuffer();
				}
				...
				NkReflectVariant(const NkReflectVariant &other)
					: mType(nullptr), mHeap(nullptr), mSize(0), mIsString(false) {
					ZeroBuffer();
					CopyFrom(other);
				}
				...
				NkReflectVariant(NkReflectVariant &&other) noexcept
					: mType(other.mType), mHeap(other.mHeap), mSize(other.mSize), mIsString(other.mIsString) {
					CopyBufferRaw(other);
					other.mType = nullptr;
					other.mHeap = nullptr;
					other.mSize = 0;
				 ...
				 */
				template <typename T> static NkReflectVariant From(const T &value) {
					NkReflectVariant v;
					v.Set<T>(value);
					return v;
				}
				 ...
				 */
				template <typename T> nk_bool Get(T &out) const {
					if (!IsValid()) {
						return false;
					}
				 ...
				 */
				template <typename T> T GetOr(T def) const {
					T tmp = def;
					if (Get<T>(tmp)) {
						return tmp;
					}
				...
				nk_int64 ToInt64() const {
					...
					switch (mType->GetCategory()) {
						case NkTypeCategory::NK_BOOL:
							return ReadAs<nk_bool>() ? 1 : 0;
						case NkTypeCategory::NK_INT8:
							return static_cast<nk_int64>(ReadAs<nk_int8>());
						case NkTypeCategory::NK_INT16:
							return static_cast<nk_int64>(ReadAs<nk_int16>());
						case NkTypeCategory::NK_INT32:
							return static_cast<nk_int64>(ReadAs<nk_int32>());
						case NkTypeCategory::NK_INT64:
							return ReadAs<nk_int64>();
						case NkTypeCategory::NK_UINT8:
							return static_cast<nk_int64>(ReadAs<nk_uint8>());
						case NkTypeCategory::NK_UINT16:
							return static_cast<nk_int64>(ReadAs<nk_uint16>());
						case NkTypeCategory::NK_UINT32:
							return static_cast<nk_int64>(ReadAs<nk_uint32>());
						case NkTypeCategory::NK_UINT64:
							return static_cast<nk_int64>(ReadAs<nk_uint64>());
						case NkTypeCategory::NK_FLOAT32:
							return static_cast<nk_int64>(ReadAs<nk_float32>());
						case NkTypeCategory::NK_FLOAT64:
							return static_cast<nk_int64>(ReadAs<nk_float64>());
				...
				nk_float64 ToFloat64() const {
					...
					switch (mType->GetCategory()) {
						case NkTypeCategory::NK_BOOL:
							return ReadAs<nk_bool>() ? 1.0 : 0.0;
						case NkTypeCategory::NK_INT8:
							return static_cast<nk_float64>(ReadAs<nk_int8>());
						case NkTypeCategory::NK_INT16:
							return static_cast<nk_float64>(ReadAs<nk_int16>());
						case NkTypeCategory::NK_INT32:
							return static_cast<nk_float64>(ReadAs<nk_int32>());
						case NkTypeCategory::NK_INT64:
							return static_cast<nk_float64>(ReadAs<nk_int64>());
						case NkTypeCategory::NK_UINT8:
							return static_cast<nk_float64>(ReadAs<nk_uint8>());
						case NkTypeCategory::NK_UINT16:
							return static_cast<nk_float64>(ReadAs<nk_uint16>());
						case NkTypeCategory::NK_UINT32:
							return static_cast<nk_float64>(ReadAs<nk_uint32>());
						case NkTypeCategory::NK_UINT64:
							return static_cast<nk_float64>(ReadAs<nk_uint64>());
						case NkTypeCategory::NK_FLOAT32:
							return static_cast<nk_float64>(ReadAs<nk_float32>());
						case NkTypeCategory::NK_FLOAT64:
							return ReadAs<nk_float64>();
						case NkTypeCategory::NK_ENUM:
							return static_cast<nk_float64>(ReadEnumUnderlying());
						default:
							return 0.0;
					}
				}
			...
			private:
				...
				template <typename T> void StoreTyped(const T &value) {
					StoreTypedImpl<T>(value, traits::NkIsSame<T, NkString>());
				}

				// Specialisation NkString.
				template <typename T> void StoreTypedImpl(const T &value, traits::NkTrueType) {
					mIsString = true;
				...
				template <typename T> void StoreTypedImpl(const T &value, traits::NkFalseType) {

...
// Validation optionnelle en mode debug
		...
		void DumpRegistry() {
			...
			logger.Infof("Types: %zu / %zu", registry.GetTypeCount(),
						 static_cast<nk_usize>(512)); // NK_MAX_TYPES
			logger.Infof("Classes: %zu / %zu", registry.GetClassCount(),
			...
			for (nk_usize i = 0; i < registry.GetClassCount(); ++i) {
				...
				if (cls) {
					logger.Infof("Class[%zu]: %s (size=%zu, base=%s, props=%zu, methods=%zu)", i, cls->GetName(),
								 cls->GetSize(), cls->GetBaseClass() ? cls->GetBaseClass()->GetName() : "none",
								 cls->GetPropertyCount(), cls->GetMethodCount());
				}
			}

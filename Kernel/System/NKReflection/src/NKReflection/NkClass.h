	...
	namespace reflection {
		...
		class NKENTSEU_REFLECTION_API NkClass {
			public:
				 ...
				 */
				NkClass(const nk_char *name, nk_usize size, const NkType &type)
					: mName(name), mSize(size), mType(&type), mBaseClass(nullptr), mProperties(), mMethods(),
				 ...
				 * @endcode
				 */
				template <typename T> static NkClass MakeFromClassType(const nk_char *name) {
				...
				static NkProperty &RegisterMemberProperty(NkClass &classMeta, ValueType ClassType::*memberPtr,
														  const nk_char *name, nk_uint32 flags = 0) {
					static NkProperty prop(name, NkTypeOf<ValueType>(),
										   reinterpret_cast<nk_usize>(&(reinterpret_cast<ClassType *>(0)->*memberPtr)),
										   flags);
				...
				static NkMethod &RegisterMemberMethod(NkClass &classMeta, ReturnType (ClassType::*methodPtr)(Args...),
													  const nk_char *name, nk_uint32 flags = 0) {
					static NkMethod method(name, NkTypeOf<ReturnType>(), flags);

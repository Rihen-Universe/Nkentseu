...
namespace nkentseu {
	...
	namespace reflection {
		...
		*/
		...
		NkRegistry::OnRegisterCallback CreateBoundRegisterCallback(HandlerType *handlerObj,
																   void (HandlerType::*handlerMethod)(const nk_char *,
																									  nk_bool)) {
			// Adaptation de la signature membre vers la signature du callback
			auto wrapper = [handlerObj, handlerMethod](const nk_char *name, nk_bool isClass) -> void {
		 ...
		 * @endcode
		 */
		const NkClass *FindDerivedClass(const nk_char *baseClassName, const nk_char *derivedClassName) {
			auto &registry = NkRegistry::Get();
		 ...
		 * @endcode
		 */
		nk_usize CollectDerivedClasses(const nk_char *baseClassName, const NkClass **outClasses, nk_usize maxCount) {

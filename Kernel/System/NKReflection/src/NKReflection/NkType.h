...
namespace nkentseu {
	...
	namespace reflection {
		...
		template <typename T> constexpr NkTypeCategory DetermineCategory();
		...
		class NKENTSEU_REFLECTION_API NkType {
			public:
				...
				NkType(const nk_char *name, nk_usize size, nk_usize alignment, NkTypeCategory category)
					: mName(name), mSize(size), mAlignment(alignment), mCategory(category), mClass(nullptr) {
				}
		...
		class NKENTSEU_REFLECTION_API NkTypeRegistry {
			...
			private:
				 ...
				 */
				NkTypeRegistry() : mTypes(mTypeStorage), mTypeCount(0), mTypeCapacity(NK_MAX_TYPES) {
				 ...
				 */
				template <typename T> static const nk_char *GetTypeName() {
		 ...
		 */
		template <typename T> const NkType &NkTypeOf() {
		 ...
		 */
		template <> inline const NkType &NkTypeOf<void>() {
			static NkType type(typeid(void).name(), 0, 0, NkTypeCategory::NK_VOID);
			return type;
		}
		 ...
		 */
		template <typename T> const NkType &NkTypeOf(const T &) {
			return NkTypeOf<T>();
		}
		 ...
		 */
		template <> constexpr NkTypeCategory DetermineCategory<::nkentseu::NkString>() {

...
namespace nkentseu {
	...
	namespace reflection {
		...
		namespace detail {
			...
			template <typename T> nk_usize NkVecGetCount(const void *c) {
				return static_cast<const NkVector<T> *>(c)->Size();
			}

			template <typename T> void *NkVecGetElement(void *c, nk_usize i) {
				NkVector<T> *v = static_cast<NkVector<T> *>(c);
				return &(*v)[i];
			}

			template <typename T> const void *NkVecGetElementC(const void *c, nk_usize i) {
				const NkVector<T> *v = static_cast<const NkVector<T> *>(c);
				return &(*v)[i];
			}

			template <typename T> void *NkVecPushBack(void *c) {
				NkVector<T> *v = static_cast<NkVector<T> *>(c);
				v->PushBack(T());
				return &(*v)[v->Size() - 1];
			}

			template <typename T> void NkVecClear(void *c) {
				static_cast<NkVector<T> *>(c)->Clear();
			}

			template <typename T> void NkVecResize(void *c, nk_usize n) {
		 ...
		 */
		template <typename C> struct NkContainerTrait {
		 ...
		 */
		template <typename T> struct NkContainerTrait<NkVector<T>> {
		 ...
		 */
		template <typename C> const NkContainerDescriptor *NkContainerOf() {

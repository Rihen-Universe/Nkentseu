	...
	class NKSTREAM_API NkConsoleStream : public NkStream {
		public:
...
#if defined(NKENTSEU_PLATFORM_WINDOWS)
				mHandle = GetStdHandle(type == NK_INPUT	  ? STD_INPUT_HANDLE
									   : type == NK_INPUT ? STD_ERROR_HANDLE
														  : STD_OUTPUT_HANDLE);
#else
			...
			usize ReadRaw(void *buffer, usize byteCount) override {
				...
				DWORD read = 0;
				if (mType == NK_INPUT) {
					ReadConsoleW(GetStdHandle(STD_INPUT_HANDLE), buffer, byteCount / sizeof(wchar_t), &read, NULL);
...
#if defined(NKENTSEU_PLATFORM_WINDOWS)
				DWORD written = 0;
				const HANDLE hConsole = GetStdHandle(mType == NK_ERROR ? STD_ERROR_HANDLE : STD_OUTPUT_HANDLE);
			...
			template <typename CharT> usize Read(CharT *buffer, usize count) {
				return ReadRaw(buffer, count * sizeof(CharT)) / sizeof(CharT);
			}

			template <typename CharT> usize Write(const CharT *data, usize count) {
				return WriteRaw(data, count * sizeof(CharT)) / sizeof(CharT);
			}

			bool Seek(usize) override {
				return false;
			} // Non supporté

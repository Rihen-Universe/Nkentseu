	...
	namespace native {
		...
		namespace io {
			...
			inline nk_uint32 ReadU32LE(const nk_uint8 *d, nk_size sz, nk_size &off) noexcept {
				...
				nk_uint32 v = static_cast<nk_uint32>(d[off]) | (static_cast<nk_uint32>(d[off + 1u]) << 8u) |
		...
		class NKENTSEU_SERIALIZATION_CLASS_EXPORT NkNativeWriter {
			...
			public:
				 ...
				 * @example Voir l'exemple dans la documentation de la classe.
				 */
				[[nodiscard]] static nk_bool WriteArchive(const NkArchive &archive, NkVector<nk_uint8> &out) noexcept {
				 ...
				 * @endcode
				 */
				[[nodiscard]] static nk_bool WriteArchiveCompressed(const NkArchive &archive, NkVector<nk_uint8> &out,
			...
			private:
				 ...
				 */
				static void WriteValue(const NkArchiveValue &v, NkNativeType t, NkVector<nk_uint8> &out) noexcept {
					switch (t) {
						case NkNativeType::NK_NATIVE_TYPE_NULL:
							break;
						case NkNativeType::NK_NATIVE_TYPE_BOOL:
				 ...
				 *       @endcode
				 */
				[[nodiscard]] static nk_bool WriteImpl(const NkArchive &archive, NkVector<nk_uint8> &out,
													   nk_bool compress, nk_bool withChecksum) noexcept {
					out.Clear();
		 ...
		 */
		inline void NkNativeWriter::WriteNodePayload(const NkArchiveNode &node, NkVector<nk_uint8> &out) noexcept {
			if (node.IsScalar()) {
		...
		class NKENTSEU_SERIALIZATION_CLASS_EXPORT NkNativeReader {
			...
			public:
				 ...
				 * @example Voir l'exemple dans la documentation de la classe.
				 */
				[[nodiscard]] static nk_bool ReadArchive(const nk_uint8 *data, nk_size size, NkArchive &out,
			...
			private:
				 ...
				 */
				static nk_bool ReadArchivePayload(const nk_uint8 *d, nk_size sz, nk_size &off, NkArchive &out,
				 ...
				 */
				static nk_bool ReadNode(const nk_uint8 *d, nk_size sz, nk_size &off, NkArchiveNode &out,
										NkString *err) noexcept {
					if (off + 5u > sz) {
						if (err) {
					...
					switch (nativeType) {
						...
						case NkNativeType::NK_NATIVE_TYPE_INT64:
							...
							out = NkArchiveNode(
								NkArchiveValue::FromInt64(static_cast<nk_int64>(io::ReadU64LE(d, sz, off))));
		 ...
		 */
		template <typename T>
		nk_bool SerializeToNative(const T &obj, NkVector<nk_uint8> &out, bool compress = false) noexcept {
			NkArchive archive;
			if (!obj.Serialize(archive)) {
				return false;
			}
			return compress ? NkNativeWriter::WriteArchiveCompressed(archive, out, true)
		 ...
		 */
		template <typename T>
		nk_bool DeserializeFromNative(const nk_uint8 *data, nk_size size, T &obj, NkString *err = nullptr) noexcept {
			NkArchive archive;

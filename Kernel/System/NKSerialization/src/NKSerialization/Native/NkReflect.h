...
namespace nkentseu {
	...
	namespace reflect_detail {
		 ...
		 */
		template <typename T, typename = void> struct HasSerialize : traits::NkFalseType {};

		template <typename T>
		struct HasSerialize<T,
							traits::NkVoidT<decltype(std::declval<const T &>().Serialize(std::declval<NkArchive &>()))>>
			: traits::NkTrueType {};
		 ...
		 * @note Retourne NkTrueType ou NkFalseType pour branchement compile-time
		 */
		template <typename T, typename = void> struct HasReflect : traits::NkFalseType {};

		template <typename T> struct HasReflect<T, traits::NkVoidT<typename T::NkReflectTag>> : traits::NkTrueType {};
		 ...
		 * @endcode
		 */
		template <typename T> void SerializeField(NkStringView key, const T &v, NkArchive &arc) noexcept {
		 ...
		 * @endcode
		 */
		template <typename T> void DeserializeField(NkStringView key, T &v, const NkArchive &arc) noexcept {
			if constexpr (traits::NkIsSame_v<T, nk_bool> || traits::NkIsSame_v<T, bool>) {
				nk_bool b = false;
				arc.GetBool(key, b);
				v = static_cast<T>(b);
	...
	namespace NkReflect {
		 ...
		 */
		template <typename T> nk_bool Serialize(const T &obj, NkArchive &arc) noexcept {
		 ...
		 * @example Voir l'exemple dans la documentation du namespace NkReflect.
		 */
		template <typename T> nk_bool Deserialize(const NkArchive &arc, T &obj) noexcept {
			if constexpr (reflect_detail::HasReflect<T>::value) {
 ...
 */
#define NK_REFLECT_FIELD(FieldName) ::nkentseu::reflect_detail::SerializeField(#FieldName, _self.FieldName, _arc);
 ...
 */
#define NK_REFLECT_FIELD_D(FieldName) ::nkentseu::reflect_detail::DeserializeField(#FieldName, _self.FieldName, _arc);
...
#define NK_REFLECT_STRUCT(TypeName, SerFields, DeserFields)                                                            \
	...
	SerFields NK_REFLECT_END_SERIALIZE() DeserFields NK_REFLECT_END()

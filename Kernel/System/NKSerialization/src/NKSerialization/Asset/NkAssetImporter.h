...
namespace nkentseu {
	...
	class NkAssetImporter {
		public:
			...
			[[nodiscard]] static NkAssetType DetectType(const char *sourcePath) noexcept {
				...
				// Meshes
				if (strcasecmp(ext, "fbx") == 0 || strcasecmp(ext, "obj") == 0 || strcasecmp(ext, "gltf") == 0 ||
					strcasecmp(ext, "glb") == 0 || strcasecmp(ext, "dae") == 0)
				...
				// Shaders
				if (strcasecmp(ext, "glsl") == 0 || strcasecmp(ext, "hlsl") == 0 || strcasecmp(ext, "vert") == 0 ||
					strcasecmp(ext, "frag") == 0 || strcasecmp(ext, "comp") == 0 || strcasecmp(ext, "spv") == 0)
				...
				// Data
				if (strcasecmp(ext, "json") == 0 || strcasecmp(ext, "csv") == 0 || strcasecmp(ext, "xml") == 0 ||
			...
			[[nodiscard]] static NkImportResult Import(const char *sourcePath, const char *outputDir,
				...
				if (!opts.overrideName.Empty()) {
					...
					name = dot ? NkString(basename, static_cast<nk_size>(dot - basename)) : NkString(basename);
				}
				...
				// ── Écriture du .nkasset ──────────────────────────────────────────────
				NkString writeErr;
				if (!NkAssetIO::Write(outPath.CStr(), meta, srcSize > 0 ? payload.Data() : nullptr, srcSize,
									  &writeErr)) {
			...
			[[nodiscard]] static NkImportResult Reimport(const char *assetPath, // chemin du .nkasset existant
														 const NkImportOptions &opts = {}) noexcept {
				NkImportResult result;
			...
			[[nodiscard]] static bool NeedsReimport(const char *assetPath) noexcept {
				NkAssetMetadata meta;
				if (!NkAssetIO::ReadMetadata(assetPath, meta))
					return false;
				...
				struct stat st{};
				if (stat(meta.sourceFilePath.CStr(), &st) != 0)
					return false;
	...
	class NkAssetCooker {
		public:
			...
			[[nodiscard]] static bool Cook(const char *inputAsset, const char *outputDir, const CookOptions &opts,
				...
				if (!NkAssetIO::ReadFull(inputAsset, meta, payload, &readErr)) {
					if (err)
						*err = NkString::Fmtf("Cook read error: %s", readErr.CStr());
					return false;
				}

				// Appliquer les transformations de cuisson
				if (opts.stripSourcePaths)
					meta.sourceFilePath.Clear();
				if (opts.stripThumbnails)
					meta.thumbnailData.Clear();
				...
				// Écriture
				return NkAssetIO::Write(outPath.CStr(), meta, payload.Empty() ? nullptr : payload.Data(),
										payload.Size(), err);
			}
	};

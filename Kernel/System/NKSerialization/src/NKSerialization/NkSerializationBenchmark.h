...
namespace nkentseu {
	...
	namespace benchmark {
		 ...
		 * @endcode
		 */
		[[nodiscard]] static BenchmarkResult RunFormat(NkSerializationFormat format,
			...
			nk_float64 totalSerializeMs = std::chrono::duration<nk_float64, std::milli>(t1 - t0).count();
			...
			if (result.originalSize > 0u) {
				result.compressionRatio =
					static_cast<nk_float64>(result.serializedSize) / static_cast<nk_float64>(result.originalSize);
			} else {
			...
			nk_float64 totalDeserializeMs = std::chrono::duration<nk_float64, std::milli>(t3 - t2).count();
		...
		static void RunAllBenchmarks(nk_uint32 iterations = 1000u) noexcept {
			...
			const NkSerializationFormat formats[] = {NkSerializationFormat::NK_JSON, NkSerializationFormat::NK_XML,
													 NkSerializationFormat::NK_YAML, NkSerializationFormat::NK_BINARY,
													 NkSerializationFormat::NK_NATIVE};

			const nk_char *names[] = {"JSON", "XML", "YAML", "Binary", "NkNative"};

			for (nk_size i = 0u; i < 5u; ++i) {
				BenchmarkResult r = RunFormat(formats[i], iterations);

				printf("%-12s %8s %10zu %12.4f %12.4f %7.2fx\n", names[i], r.success ? "OK" : "FAIL", r.serializedSize,
					   r.serializeMs, r.deserializeMs, r.compressionRatio);
			}
		 ...
		 * @endcode
		 */
		static nk_bool ExportResultsJSON(const NkVector<BenchmarkResult> &results, const nk_char *path) noexcept {
			if (!path) {
				return false;
			}
			...
			const nk_char *fmtNames[] = {"JSON", "XML", "YAML", "Binary", "NkNative", "Auto"};

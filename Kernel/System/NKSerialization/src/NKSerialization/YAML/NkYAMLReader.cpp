...
namespace nkentseu {
	...
	namespace {
		 ...
		 * @endcode
		 */
		nk_size NkParseBlock(const NkVector<NkLine> &lines, nk_size start, int blockIndent, NkArchive &out,
			...
			while (i < lines.Size()) {
				...
				NkStringView valueRaw = NkStringView(raw.CStr() + colonPos + 1u, raw.Length() - colonPos - 1u);

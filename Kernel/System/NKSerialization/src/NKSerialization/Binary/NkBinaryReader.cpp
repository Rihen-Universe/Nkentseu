...
namespace nkentseu {
	 ...
	 */
	nk_bool NkBinaryReader::ReadArchive(const nk_uint8 *data, nk_size size, NkArchive &outArchive, NkString *outError) {
		outArchive.Clear();
		...
		if (!ReadU32LE(data, size, off, magic) || !ReadU16LE(data, size, off, version) ||
			!ReadU16LE(data, size, off, reserved) || !ReadU32LE(data, size, off, count)) {
			if (outError) {
		...
		for (nk_uint32 i = 0; i < count; ++i) {
			...
			if (!ReadU32LE(data, size, off, keyLen) || !ReadU8(data, size, off, typeRaw) ||

...
namespace nkentseu {
	...
	namespace detail {
		 ...
		 * @endcode
		 */
		inline NkSerializationFormat DetectFormatFromExtension(const nk_char *path) noexcept {
			if (!path) {
		...
		inline NkSerializationFormat DetectFormatFromMagic(const nk_uint8 *data, nk_size size) noexcept {
			...
			if (data[0] == 0x4E && data[1] == 0x4B && data[2] == 0x53 && data[3] == 0x31) {
				return NkSerializationFormat::NK_NATIVE;
			}

			if (data[0] == 0x4E && data[1] == 0x4B && data[2] == 0x53 && data[3] == 0x00) {
				return NkSerializationFormat::NK_BINARY;
			}
	 ...
	 * @endcode
	 */
	inline nk_bool SerializeArchiveText(const NkArchive &archive, NkSerializationFormat format, NkString &outText,
										nk_bool pretty = true, NkString *outError = nullptr) noexcept {
		switch (format) {
	 ...
	 * @endcode
	 */
	inline nk_bool SerializeArchiveBinary(const NkArchive &archive, NkSerializationFormat format,
										  NkVector<nk_uint8> &outData, NkString *outError = nullptr) noexcept {
		switch (format) {
	 ...
	 * @endcode
	 */
	inline nk_bool DeserializeArchiveText(NkStringView text, NkSerializationFormat format, NkArchive &outArchive,
	 ...
	 * @endcode
	 */
	inline nk_bool DeserializeArchiveBinary(const nk_uint8 *data, nk_size size, NkSerializationFormat format,
											NkArchive &outArchive, NkString *outError = nullptr) noexcept {
		if (!data || size == 0) {
			if (outError) {
	 ...
	 */
	template <typename T>
	nk_bool Serialize(const T &obj, NkSerializationFormat format, NkString &outText, nk_bool pretty = true,
					  NkString *outError = nullptr) noexcept {
		static_assert(std::is_base_of<NkISerializable, T>::value, "T must inherit from NkISerializable");
	 ...
	 */
	template <typename T>
	nk_bool SerializeBinary(const T &obj, NkSerializationFormat format, NkVector<nk_uint8> &outData,
							NkString *outError = nullptr) noexcept {
		static_assert(std::is_base_of<NkISerializable, T>::value, "T must inherit from NkISerializable");
	 ...
	 */
	template <typename T>
	nk_bool Deserialize(NkStringView text, NkSerializationFormat format, T &obj,
						NkString *outError = nullptr) noexcept {
		static_assert(std::is_base_of<NkISerializable, T>::value, "T must inherit from NkISerializable");
	 ...
	 */
	template <typename T>
	nk_bool DeserializeBinary(const nk_uint8 *data, nk_size size, NkSerializationFormat format, T &obj,
							  NkString *outError = nullptr) noexcept {
		static_assert(std::is_base_of<NkISerializable, T>::value, "T must inherit from NkISerializable");
	 ...
	 */
	template <typename T>
	nk_bool SaveToFile(const T &obj, const nk_char *path,
					   NkSerializationFormat format = NkSerializationFormat::NK_AUTO_DETECT, nk_bool pretty = true,
					   NkString *outError = nullptr) noexcept {
		static_assert(std::is_base_of<NkISerializable, T>::value, "T must inherit from NkISerializable");
	 ...
	 */
	template <typename T>
	nk_bool LoadFromFile(const nk_char *path, T &obj,
		...
		if (format == NkSerializationFormat::NK_AUTO_DETECT || detail::IsBinaryFormat(format)) {
			...
			if (detail::IsBinaryFormat(format)) {
				...
				NkString text(reinterpret_cast<const nk_char *>(data.Data()), size);
	 ...
	 * @endcode
	 */
	inline memory::NkUniquePtr<NkISerializable> CreateFromArchive(const NkArchive &archive) noexcept {
		NkString typeName;

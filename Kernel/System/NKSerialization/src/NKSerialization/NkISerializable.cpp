...
namespace nkentseu {
	 ...
	 */
	void NkSerializableRegistry::RegisterFactory(const char *typeName, FactoryFn factory) noexcept {
	 ...
	 */
	template <typename T> void NkSerializableRegistry::Register(const char *typeName) noexcept {
		// Délègue à RegisterFactory avec une lambda qui appelle new T()
		// La lambda capture rien — convertible en fonction pointer
		RegisterFactory(typeName, []() -> NkISerializable * { return new T(); });
	}

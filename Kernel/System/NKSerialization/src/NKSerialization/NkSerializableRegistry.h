...
namespace nkentseu {
	...
	class NkComponentRegistry {
		public:
			...
			template <typename T>
			static void Register(const char *name, SerializeFn serFn = nullptr,
			...
			template <typename T> static void Unregister() noexcept {
				auto &reg = Global();
				NkTypeId id = NkTypeOf<T>();
				for (nk_size i = 0; i < reg.mEntries.Size(); ++i) {
					if (reg.mEntries[i].typeId == id) {
			...
			[[nodiscard]] static const Entry *FindByName(const char *name) noexcept {
				const auto &reg = Global();
				for (nk_size i = 0; i < reg.mEntries.Size(); ++i)
					if (strcmp(reg.mEntries[i].name, name) == 0)
						return &reg.mEntries[i];
				return nullptr;
			}

			template <typename T> [[nodiscard]] static const Entry *Find() noexcept {
				return Find(NkTypeOf<T>());
			}

			// ── Sérialisation/désérialisation ─────────────────────────────────────────
			[[nodiscard]] static nk_bool Serialize(NkTypeId id, const void *obj, NkArchive &out) noexcept {
				const Entry *e = Find(id);
				return e && e->serialize ? e->serialize(obj, out) : false;
			}

			[[nodiscard]] static nk_bool Deserialize(NkTypeId id, void *obj, const NkArchive &arc) noexcept {
				const Entry *e = Find(id);
				return e && e->deserialize ? e->deserialize(obj, arc) : false;
			}

			template <typename T> [[nodiscard]] static nk_bool Serialize(const T &obj, NkArchive &out) noexcept {
				return Serialize(NkTypeOf<T>(), &obj, out);
			}

			template <typename T> [[nodiscard]] static nk_bool Deserialize(T &obj, const NkArchive &arc) noexcept {
				return Deserialize(NkTypeOf<T>(), &obj, arc);
			}

			// ── Itération ─────────────────────────────────────────────────────────────
			template <typename Fn> static void ForEach(Fn &&fn) noexcept {
				const auto &reg = Global();
				for (nk_size i = 0; i < reg.mEntries.Size(); ++i)
					fn(reg.mEntries[i]);
			}
		...
		private:
			...
			template <typename T> static nk_bool DefaultSerialize(const void *obj, NkArchive &out) noexcept {
			...
			template <typename T> static nk_bool DefaultDeserialize(void *obj, const NkArchive &arc) noexcept {
				if constexpr (std::is_base_of<NkISerializable, T>::value) {
				...
				for (nk_size i = 0; i < sizeof(T); ++i) {
					...
					raw[i] = static_cast<nk_uint8>((hexDigit(hex[i * 2u]) << 4u) | hexDigit(hex[i * 2u + 1u]));
				}
				return true;
			}

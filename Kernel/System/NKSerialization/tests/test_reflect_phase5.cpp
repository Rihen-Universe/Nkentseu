...
class Character {
	...
	public:
		Character()
			: level(1), stamina(50.0f), displayName("Hero"), weapon(WeaponType::Sword), internalId(999),
			  hashCode(0xABCD) {
		}
};
...
// TEST 1 : INSPECTEUR
// =============================================================================
static const NkEditableProperty *FindProp(const NkVector<NkEditableProperty> &v, const char *name) {
...
class Calculator {
	public:
		nk_int32 Add(nk_int32 a, nk_int32 b) {
			return a + b;
		}

		nk_float32 Scale(nk_float32 v, nk_float32 k) {
			return v * k;
		}

		nk_int32 Sum3(nk_int32 a, nk_int32 b, nk_int32 c) {
			return a + b + c;
		}

		void Accumulate(nk_int32 v) {
			mTotal += v;
		}

		nk_int32 GetTotal() const {
			return mTotal;
		}
...
static void TestMethodInvocation() {
	...
	if (res) {
		EXPECT_EQ(*static_cast<nk_int32 *>(res), 7);
	}
	...
	NkMethod scale = NkMethod::MakeFromMember(&calc, &Calculator::Scale, "Scale", NkTypeOf<nk_float32>());
	...
	NkMethod acc = NkMethod::MakeFromMember(&calc, &Calculator::Accumulate, "Accumulate", NkTypeOf<void>());
	...
	NkMethod getTotal = NkMethod::MakeFromMember(&calc, &Calculator::GetTotal, "GetTotal", intType);

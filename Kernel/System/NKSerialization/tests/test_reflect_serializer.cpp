...
class Entity {
	...
	public:
		...
		Entity()
			: health(0), score(0), speed(0.0f), ratio(0.0), alive(false), kind(0), name(), position(), cacheToken(0) {
		}
};
...
static void WireReflection() {
	...
	const_cast<reflection::NkType &>(reflection::NkTypeOf<Vec3Like>()).SetClass(&vecClass);
	const_cast<reflection::NkType &>(reflection::NkTypeOf<Entity>()).SetClass(&entityClass);
	...
	static reflection::NkProperty s_cacheTokenProp("cacheToken", reflection::NkTypeOf<nk_int32>(),

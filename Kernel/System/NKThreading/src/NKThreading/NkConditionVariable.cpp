...
namespace nkentseu {
	namespace threading {
		...
		// -----------------------------------------------------------------
		NkConditionVariable::NkConditionVariable() noexcept : mInitialized(true) {
#if defined(NKENTSEU_PLATFORM_WINDOWS)
		...
		// -----------------------------------------------------------------
		nk_bool NkConditionVariable::WaitFor(NkScopedLockMutex &lock, nk_uint32 milliseconds) noexcept {
			NkMutex *mutex = lock.GetMutex();
			if (mutex == nullptr || !mutex->mInitialized || !mInitialized) {
				return false;
			}
		...
		// -----------------------------------------------------------------
		nk_bool NkConditionVariable::WaitUntil(NkScopedLockMutex &lock, nk_uint64 deadlineMs) noexcept {
			nk_uint64 now = GetMonotonicTimeMs();
			while (now < deadlineMs) {
				nk_uint64 remaining = deadlineMs - now;
				nk_uint32 waitMs = (remaining > 0xFFFFFFFFULL) ? 0xFFFFFFFFu : static_cast<nk_uint32>(remaining);
				if (waitMs == 0) {
					waitMs = 1;
				}
		...
		nk_uint64 NkConditionVariable::GetMonotonicTimeMs() noexcept {
			...
			return static_cast<nk_uint64>(ts.tv_sec) * 1000ULL + static_cast<nk_uint64>(ts.tv_nsec / 1000000ULL);
#endif
		}
...
// =================================================================
		...
		// -----------------------------------------------------------------
		timespec NkConditionVariable::MakeAbsoluteTimeout(nk_uint32 milliseconds) noexcept {
			timespec deadline{};

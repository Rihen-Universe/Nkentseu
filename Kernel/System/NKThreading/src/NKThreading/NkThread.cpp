...
namespace nkentseu {
	namespace threading {
		...
		NkThread::NkThread() noexcept
			...
			: mHandle(nullptr), mThreadId(0)
#else
			: mThread(), mJoinable(false)
#endif
		{
		...
		NkThread::ThreadId NkThread::GetID() const noexcept {
			...
			ThreadId id = 0;
			const nk_size copySize = (sizeof(pthread_t) < sizeof(ThreadId)) ? sizeof(pthread_t) : sizeof(ThreadId);
...
#elif defined(NKENTSEU_PLATFORM_MACOS) || defined(NKENTSEU_PLATFORM_IOS) || defined(NKENTSEU_PLATFORM_TVOS) ||         \
...
#elif defined(NKENTSEU_PLATFORM_LINUX) || defined(NKENTSEU_PLATFORM_MACOS) || defined(NKENTSEU_PLATFORM_IOS) ||        \
...
#if defined(NKENTSEU_PLATFORM_WINDOWS)
			mHandle = CreateThread(nullptr,				   // Security attributes (default)
...
#else
			const int result = pthread_create(&mThread,				  // Receives pthread_t handle

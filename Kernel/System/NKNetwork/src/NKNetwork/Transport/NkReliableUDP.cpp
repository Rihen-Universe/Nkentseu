...
namespace nkentseu {
	...
	namespace net {
		...
		uint32 NkReliableChannel::PrepareReliable(const uint8 *data, uint32 size, NkNetChannel ch) noexcept {
		...
		void NkReliableChannel::GatherPendingSends(NkTimestampMs now, float32 rto,
		...
		bool NkReliableChannel::InsertReceived(uint32 seqNum, const uint8 *data, uint32 size) noexcept {
			// Validation des paramètres
			if (data == nullptr || size == 0 || size > kNkMaxPayloadSize) {
				return false;
			}
		...
		void NkReliableUDP::Update(float32 dt) noexcept {
			// ── DIAG : log d'entree (premieres iterations seulement).
			{
				...
				if (idx < 5) {
					NK_NET_LOG_INFO("[NET-DIAG] RUDP::Update enter #{} socket={} valid={}", idx,
									mSocket ? "ptr" : "null", (mSocket && mSocket->IsValid()) ? 1 : 0);
				}
			}
		...
		NkNetResult NkReliableUDP::Send(const uint8 *data, uint32 size, NkNetChannel channel) noexcept {
			// Validation des paramètres
			...
			switch (channel) {
				...
				case NkNetChannel::NK_NET_CHANNEL_SEQUENCED:
					// Envoi direct sans fiabilité pour ces canaux
					{
						...
						return static_cast<NkNetResult>(mSocket->SendTo(buffer, NkRUDPHeader::kSize + size, mRemote));
					}
		...
		NkNetResult NkReliableUDP::SendFragmented(const uint8 *data, uint32 size, NkNetChannel ch) noexcept {
			...
			for (uint8 fragIdx = 0; fragIdx < fragCount; ++fragIdx) {
				...
				header.flags =
					kFlagFragment | ((ch == NkNetChannel::NK_NET_CHANNEL_RELIABLE_ORDERED)
										 ? (kFlagReliable | kFlagOrdered)
										 : (ch == NkNetChannel::NK_NET_CHANNEL_RELIABLE_UNORDERED ? kFlagReliable : 0));
		...
		bool NkReliableUDP::TryReassemble(uint8 fragIdx, uint8 fragCount, uint32 seqBase, const uint8 *data,

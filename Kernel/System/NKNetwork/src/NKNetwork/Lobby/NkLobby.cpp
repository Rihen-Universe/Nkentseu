...
namespace {
	...
	// =====================================================================
	float32 CalculateMatchScore(const NkMatchmaker::SearchParams &params,
								const NkMatchmaker::SearchResult &result) noexcept {
		...
		// Score player fit : adéquation taille de groupe vs occupation
		{
			...
			if (availableSlots >= params.playerCount) {
				...
				score += kPlayerFitWeight *
						 (static_cast<float32>(availableSlots) / static_cast<float32>(params.playerCount));
			}
	...
	namespace net {
		...
		NkNetResult NkSession::Create(const NkSessionConfig &cfg) noexcept {
			...
			if (cfg.minPlayers > cfg.maxPlayers) {
				NK_NET_LOG_ERROR("Create : minPlayers ({}) > maxPlayers ({})", cfg.minPlayers, cfg.maxPlayers);
				return NkNetResult::NK_NET_INVALID_ARG;
			}
			...
			mConnMgr.onPeerConnected = [this](NkPeerId peer) { OnPeerConnected(peer); };
			mConnMgr.onPeerDisconnected = [this](NkPeerId peer, const char *reason) {
				OnPeerDisconnected(peer, reason);
			};

			NK_NET_LOG_INFO("Session créée en tant que host : {} (port {})", cfg.name, cfg.port);
		...
		void NkSession::Leave() noexcept {
			...
			NK_NET_LOG_DEBUG("Session quittée : {} → {}", NkSessionStateStr(previousState), NkSessionStateStr(mState));
		...
		NkNetResult NkSession::Start() noexcept {
			...
			if (mPlayerCount < mConfig.minPlayers) {
				NK_NET_LOG_WARN("Start() : pas assez de joueurs ({}/{} requis)", mPlayerCount, mConfig.minPlayers);
			...
			if (readyCount < mPlayerCount) {
				NK_NET_LOG_WARN("Start() : pas tous les joueurs prêts ({}/{})", readyCount, mPlayerCount);
				return NkNetResult::NK_NET_NOT_CONNECTED;
			}
			...
			NK_NET_LOG_INFO("Session démarrée : {} joueurs, map {}", mPlayerCount, mConfig.mapName);
		...
		void NkSession::End() noexcept {
			...
			if (mState != NkSessionState::NK_SESSION_IN_PROGRESS && mState != NkSessionState::NK_SESSION_LOADING) {
		...
		void NkSession::OnPeerConnected(NkPeerId peer) noexcept {
			...
			NK_NET_LOG_INFO("Nouveau joueur rejoint : {} (slot {})", newPlayer.displayName, newPlayer.slot);
		...
		void NkSession::BroadcastPlayerInfo(const NkPlayerInfo &player) noexcept {
			...
			BroadcastLobbyMessage(static_cast<uint8>(NkLobbyMessageType::NK_LOBBY_PLAYER_INFO), payload,
								  writer.BytesWritten());
		}
		...
		void NkLobby::SendChatMessage(const char *msg) noexcept {
			...
			SafeStrCopy(chatMsg.sender, sizeof(chatMsg.sender), playerInfo ? playerInfo->displayName : "Unknown", 64);
		...
		void NkLobby::SetMap(const char *mapName) noexcept {
			...
			SafeStrCopy(mSession.mConfig.mapName, NkSessionConfig::kMaxName, mapName ? mapName : "", 128);
		...
		void NkLobby::SetGameMode(const char *mode) noexcept {
			...
			SafeStrCopy(mSession.mConfig.gameMode, sizeof(mSession.mConfig.gameMode), mode ? mode : "", 64);
		...
		void NkMatchmaker::SearchAsync(const SearchParams &params, NkFunction<void(const SearchResult &)> onFound,
									   NkFunction<void(NkNetResult)> onError, float32 timeoutSec) noexcept {
			...
			if (mSearching) {
				...
				if (onError) {
					onError(NkNetResult::NK_NET_ALREADY_CONNECTED);
				}
				return;
			}
		...
		NkNetResult NkMatchmaker::RegisterServer(const NkSessionConfig &cfg, const char *matchmakerUrl) noexcept {
			// Validation des paramètres
		...
		void NkMatchmaker::SearchWorker(const SearchParams &params, NkFunction<void(const SearchResult &)> onFound,
										NkFunction<void(NkNetResult)> onError, float32 timeoutSec) noexcept {
			// Initialisation du timer de timeout (horloge monotone réseau, en ms)
			const NkTimestampMs startTime = NkNetNowMs();
			const NkTimestampMs timeoutMs = static_cast<NkTimestampMs>(timeoutSec * 1000.f);
		...
		NkVector<NkMatchmaker::SearchResult>
		NkMatchmaker::FetchMatchmakingCandidates(const SearchParams &params) noexcept {
		...
		void NkMatchmaker::StartMatchmakerHeartbeat(const NkSessionConfig &cfg, const char *matchmakerUrl) noexcept {
		...
		NkNetResult NkDiscovery::Broadcast(const ServerInfo &info, uint16 broadcastPort) noexcept {
		...
		NkNetResult NkDiscovery::Listen(uint32 timeoutMs, NkVector<ServerInfo> &out, uint16 broadcastPort) noexcept {

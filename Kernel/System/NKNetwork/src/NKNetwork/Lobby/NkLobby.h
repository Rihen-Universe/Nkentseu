	...
	namespace net {
		...
		class NKENTSEU_NETWORK_CLASS_EXPORT NkSession {
			public:
				 ...
				 */
				[[nodiscard]] NkNetResult Join(const NkAddress &addr, const char *password = nullptr) noexcept;
		...
		class NKENTSEU_NETWORK_CLASS_EXPORT NkLobby {
			public:
				 ...
				 */
				[[nodiscard]] NkNetResult JoinSession(const NkAddress &addr, const char *pw = nullptr) noexcept;
		...
		class NKENTSEU_NETWORK_CLASS_EXPORT NkMatchmaker {
			public:
				 ...
				 */
				void SearchAsync(const SearchParams &params, NkFunction<void(const SearchResult &)> onFound,
								 NkFunction<void(NkNetResult)> onError, float32 timeoutSec = 30.f) noexcept;
				 ...
				 */
				[[nodiscard]] NkNetResult RegisterServer(const NkSessionConfig &cfg,
			...
			private:
				...
				void SearchWorker(const SearchParams &params, NkFunction<void(const SearchResult &)> onFound,
				...
				NkVector<SearchResult> FetchMatchmakingCandidates(const SearchParams &params) noexcept;

				/// Démarre le heartbeat périodique pour maintenir l'enregistrement serveur.
				void StartMatchmakerHeartbeat(const NkSessionConfig &cfg, const char *matchmakerUrl) noexcept;
		...
		class NKENTSEU_NETWORK_CLASS_EXPORT NkDiscovery {
			public:
				 ...
				 */
				[[nodiscard]] static NkNetResult Broadcast(const ServerInfo &info,
				 ...
				 */
				[[nodiscard]] static NkNetResult Listen(uint32 timeoutMs, NkVector<ServerInfo> &out,
														uint16 broadcastPort = 7778) noexcept;
		};

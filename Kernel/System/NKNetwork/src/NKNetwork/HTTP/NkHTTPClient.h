	...
	namespace net {
		...
		class NKENTSEU_NETWORK_CLASS_EXPORT NkHTTPClient {
			public:
				 ...
				 * @endcode
				 */
				[[nodiscard]] NkHTTPResponse Post(const char *url, const char *json) noexcept;
				 ...
				 * @endcode
				 */
				[[nodiscard]] uint32 SendAsync(const NkHTTPRequest &req, Callback cb) noexcept;
				 ...
				 *         } else {
				 *             NK_LOG_ERROR("Échec : {}", resp.error.IsEmpty() ? resp.statusText.CStr() :
				 * resp.error.CStr());
				 *         }
				 *     }
				 * );
				 * @endcode
				 */
				[[nodiscard]] uint32 DownloadFile(const char *url, const char *destPath,
												  NkFunction<void(uint64, uint64)> onProgress = nullptr,
												  Callback onComplete = nullptr) noexcept;
				 ...
				 * @endcode
				 */
				[[nodiscard]] static NkString Base64Encode(const uint8 *data, uint32 size) noexcept;
			...
			private:
				 ...
				 */
				[[nodiscard]] bool ParseURL(const NkString &url, NkString &scheme, NkString &host, uint16 &port,
		...
		class NKENTSEU_NETWORK_CLASS_EXPORT NkLeaderboard {
			public:
				 ...
				 * @endcode
				 */
				void SubmitScore(const char *playerName, uint64 score,
								 NkFunction<void(bool success)> cb = nullptr) noexcept;
				 ...
				 * @endcode
				 */
				void FetchTop(uint32 count, NkFunction<void(NkVector<NkLeaderboardEntry> &)> cb) noexcept;
				 ...
				 * @endcode
				 */
				void FetchPlayerRank(const char *playerName, NkFunction<void(uint32 rank, uint64 score)> cb) noexcept;

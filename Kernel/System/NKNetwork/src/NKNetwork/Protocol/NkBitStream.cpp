...
namespace nkentseu {
	...
	namespace net {
		...
		NkBitWriter::NkBitWriter(uint8 *buf, uint32 capacity) noexcept
			: mBuf(buf), mCap(capacity), mBitPos(0), mOverflow(false) {
		...
		void NkBitWriter::WriteQuatf(const NkQuatf &q) noexcept {
			...
			if ((q.x < 0.0f ? -q.x : q.x) > absMax) {
				absMax = (q.x < 0.0f ? -q.x : q.x);
				largest = 1;
			}
			if ((q.y < 0.0f ? -q.y : q.y) > absMax) {
				absMax = (q.y < 0.0f ? -q.y : q.y);
				largest = 2;
			}
			if ((q.z < 0.0f ? -q.z : q.z) > absMax) {
				largest = 3;
			}
		...
		NkBitReader::NkBitReader(const uint8 *buf, uint32 size) noexcept
			: mBuf(buf), mSize(size * 8u) // Conversion bytes → bits
			  ,
			  mBitPos(0), mOverflow(false) {
		...
		NkVec3f NkBitReader::ReadVec3fQ(float32 minV, float32 maxV, float32 prec) noexcept {
			return NkVec3f{ReadF32Q(minV, maxV, prec), ReadF32Q(minV, maxV, prec), ReadF32Q(minV, maxV, prec)};
		}

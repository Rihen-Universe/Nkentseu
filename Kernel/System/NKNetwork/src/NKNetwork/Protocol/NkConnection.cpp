...
namespace {
	...
	struct NkSystemHeader {
			...
			bool Deserialize(const nkentseu::uint8 *buf, nkentseu::uint32 bufSize) noexcept {
				...
				if (buf[0] != kProtocolMagic || buf[1] != kProtocolVersion) {
					return false;
				}
				type = buf[2];
				reserved = buf[3];
	...
	[[nodiscard]] nkentseu::uint32 GenerateChallenge() noexcept {
		...
		static const nkentseu::uint64 sSeed = static_cast<nkentseu::uint64>(nkentseu::NkChrono::Now().ToNanoseconds());

		const nkentseu::uint64 counter = sCounter.FetchAdd(1, nkentseu::NkMemoryOrder::NK_RELAXED);
	...
	// =====================================================================
	[[nodiscard]] nkentseu::uint32 ComputeChallengeResponse(nkentseu::uint32 challenge,
															nkentseu::uint64 peerId) noexcept {
...
namespace nkentseu {
	...
	namespace net {
		...
		void NkConnection::InitAsServer(NkSocket *socket, const NkAddress &remote, NkPeerId localId,
			...
			NK_NET_LOG_DEBUG("Connexion serveur initialisée pour peer {} @ {}", remoteId.value,
							 remote.ToString().CStr());
		}

		void NkConnection::InitAsClient(NkSocket *socket, const NkAddress &remote, NkPeerId localId) noexcept {
			// Validation des paramètres
			if (socket == nullptr || !socket->IsValid()) {
		...
		NkNetResult NkConnection::Disconnect(const char *reason) noexcept {
			...
			const NkNetResult result =
				mRUDP.Send(buffer, NkSystemHeader::kSize, NkNetChannel::NK_NET_CHANNEL_RELIABLE_ORDERED);

			if (result == NkNetResult::NK_NET_OK) {
				NK_NET_LOG_INFO("FIN envoyé à {} : {}", mRemoteAddr.ToString().CStr(), reason ? reason : "sans raison");
			} else {
				NK_NET_LOG_WARN("Échec d'envoi du FIN : {} — déconnexion forcée", NkNetResultStr(result));
				ForceDisconnect();
			}
		...
		void NkConnection::ForceDisconnect() noexcept {
			...
			NK_NET_LOG_DEBUG("Déconnexion forcée : {} → {}", NkConnStateStr(previousState), NkConnStateStr(mState));
		}
		...
		void NkConnection::Update(float32 dt) noexcept {
			...
			if (mState == NkConnectionState::NK_CONNECTION_SYN_SENT ||
				...
				if (now - mConnectAttemptAt > kNkTimeoutMs) {
					NK_NET_LOG_WARN("Timeout handshake avec {} — déconnexion", mRemoteAddr.ToString().CStr());
					ForceDisconnect();
					return;
				}
			}
			...
			if (mState == NkConnectionState::NK_CONNECTION_ESTABLISHED) {
				...
				if (mTimeoutTimer >= (static_cast<float32>(kNkTimeoutMs) / 1000.f)) {
					if (now - mLastActivityAt > kNkTimeoutMs) {
						NK_NET_LOG_WARN("Timeout d'inactivité avec {} — déconnexion", mRemoteAddr.ToString().CStr());
		...
		NkNetResult NkConnection::Send(const uint8 *data, uint32 size, NkNetChannel channel) noexcept {
			// Validation des paramètres
		...
		void NkConnection::OnRawReceived(const uint8 *data, uint32 size) noexcept {
			...
			auto isSystemHeader = [&](const uint8 *payload, uint32 payloadSize) -> bool {
				if (payload == nullptr || payloadSize < NkSystemHeader::kSize)
					return false;
				if (payload[0] != kProtocolMagic || payload[1] != kProtocolVersion)
					return false;
			...
			auto tryProcessSystemHeader = [&](const uint8 *payload, uint32 payloadSize) -> bool {
				if (!isSystemHeader(payload, payloadSize))
					return false;
				NkSystemHeader header;
				if (!header.Deserialize(payload, payloadSize))
					return false;
				switch (static_cast<NkSystemPacketType>(header.type)) {
					...
					case NkSystemPacketType::NK_SYS_FIN:
						NK_NET_LOG_INFO("FIN recu de {} — deconnexion", mRemoteAddr.ToString().CStr());
						ForceDisconnect();
						return true;
					default:
						return false;
		...
		void NkConnection::ProcessSYNACK(const uint8 *data, uint32 size) noexcept {
			...
			if (!header.Deserialize(data, size) ||
				header.type != static_cast<uint8>(NkSystemPacketType::NK_SYS_SYN_ACK)) {
			...
			if (header.response != expectedResponse) {
				NK_NET_LOG_ERROR("Validation du challenge échouée : attendu={}, reçu={}", expectedResponse,
								 header.response);
				ForceDisconnect();
				return;
			}
		...
		void NkConnection::ProcessACK(const uint8 *data, uint32 size) noexcept {
			...
			NkSystemHeader header;
			if (!header.Deserialize(data, size) || header.type != static_cast<uint8>(NkSystemPacketType::NK_SYS_ACK)) {
			...
			const uint32 expectedResponse = ComputeChallengeResponse(mLocalChallenge, mRemotePeerId.value);
			if (header.response != expectedResponse) {
				NK_NET_LOG_ERROR("Validation challenge ACK echouee : attendu={}, recu={}", expectedResponse,
								 header.response);
				ForceDisconnect();
				return;
			}
			...
			mStats.connectedSince = NkNetNowMs();
			mLastActivityAt = NkNetNowMs();
			NK_NET_LOG_INFO("ACK reçu — connexion établie avec client {}", mRemotePeerId.value);
		...
		void NkConnection::ProcessPong(const uint8 *data, uint32 size) noexcept {
			...
			if (sentAt != 0 && sentAt <= now) {
				...
				NK_NET_LOG_DEBUG("PONG reçu : RTT={:.1f}ms jitter={:.1f}ms", mStats.rttMs, mStats.jitterMs);
			}
		}
		...
		NkNetResult NkConnectionManager::SendTo(NkPeerId peer, const uint8 *data, uint32 size,
		...
		NkNetResult NkConnectionManager::Broadcast(const uint8 *data, uint32 size, NkNetChannel ch,
												   NkPeerId exclude) noexcept {
			// Validation des paramètres
			if (data == nullptr || size == 0) {
				return NkNetResult::NK_NET_INVALID_ARG;
			}
		...
		bool NkConnectionManager::GetConnectionStats(NkPeerId peer, NkConnectionStats &outStats) const noexcept {
		...
		void NkConnectionManager::PollSocket() noexcept {
			...
			while (true) {
				...
				NkAddress from;
				const NkNetResult r = mSocket.RecvFrom(buffer, sizeof(buffer), receivedSize, from);
				if (r != NkNetResult::NK_NET_OK)
					break;
				if (receivedSize == 0)
					break; // ← le fix critique
		...
		void NkConnectionManager::DispatchPacket(const uint8 *data, uint32 size, const NkAddress &from) noexcept {
			...
			if (conn == nullptr) {
				NK_NET_LOG_WARN("Paquet reçu de {} rejeté (limite connexions atteinte)", from.ToString().CStr());
				return;
			}
		...
		NkConnection *NkConnectionManager::FindOrCreateConnection(const NkAddress &from, bool &outIsNew) noexcept {
			outIsNew = false;
			...
			// en deux NkConnection distinctes pour le même pair.
			{
				...
				if (mConnCount >= maxConnections) {
					NK_NET_LOG_WARN("Limite de connexions atteinte ({}/{}) — rejet de {}", mConnCount, maxConnections,
									from.ToString().CStr());
					return nullptr;
				}
				...
				NK_NET_LOG_INFO("Connexion créée : {} (slot {}/{} max)", from.ToString().CStr(), slot, maxConnections);
		...
		NkConnection *NkConnectionManager::FindConnection(NkPeerId peer) noexcept {
			for (uint32 i = 0; i < mConnCount; ++i) {
				if (mConnections[i] != nullptr && mConnections[i]->GetRemotePeerId() == peer) {
		...
		NkConnection *NkConnectionManager::FindConnectionByAddr(const NkAddress &addr) noexcept {
			for (uint32 i = 0; i < mConnCount; ++i) {
				if (mConnections[i] != nullptr && mConnections[i]->GetRemoteAddr() == addr) {
					return mConnections[i];
				}
			}
			return nullptr;

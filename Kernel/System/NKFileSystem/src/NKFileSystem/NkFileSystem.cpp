...
namespace nkentseu {
	...
	NkDriveInfo::NkDriveInfo()
		: Name(), Label(), Type(NkFileSystemType::NK_UNKNOW), TotalSize(0), FreeSpace(0), AvailableSpace(0),
		  IsReady(false) {
	...
	NkFileAttributes::NkFileAttributes()
		: IsReadOnly(false), IsHidden(false), IsSystem(false), IsArchive(false), IsCompressed(false),
		  IsEncrypted(false), CreationTime(0), LastAccessTime(0), LastWriteTime(0) {
		...
		for (int i = 0; i < 26; ++i) {
			...
			if (driveMask & (1 << i)) {
				...
				if (info.IsReady) {
					...
					if (GetVolumeInformationA(driveLetter, volumeName, MAX_PATH, NULL, NULL, NULL, fileSystemName,
											  MAX_PATH)) {
	...
	NkFileAttributes NkFileSystem::GetFileAttributes(const char *path) {
		...
		if (hFile != INVALID_HANDLE_VALUE) {
			...
			if (GetFileTime(hFile, &creation, &access, &write)) {
				...
				attrs.CreationTime = (static_cast<nk_int64>(creation.dwHighDateTime) << 32) |
									 static_cast<nk_int64>(creation.dwLowDateTime);
				attrs.LastAccessTime =
					(static_cast<nk_int64>(access.dwHighDateTime) << 32) | static_cast<nk_int64>(access.dwLowDateTime);
				attrs.LastWriteTime =
					(static_cast<nk_int64>(write.dwHighDateTime) << 32) | static_cast<nk_int64>(write.dwLowDateTime);
			}
	...
	bool NkFileSystem::SetCreationTime(const char *path, nk_int64 time) {
		...
		HANDLE hFile = CreateFileA(path, FILE_WRITE_ATTRIBUTES, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING,

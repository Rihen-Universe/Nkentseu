...
namespace nkentseu {
	...
	static AAsset *TryOpenAndroidAsset(const char *path) {
		if (!sAndroidAssetMgr || !path)
			return nullptr;
		// 1. Path tel quel (cas ou androidassets a ete configure pour
		// preserver l'arborescence complete).
		AAsset *asset = AAssetManager_open(sAndroidAssetMgr, path, AASSET_MODE_BUFFER);
		if (asset)
			return asset;
		...
		if (hasResPrefix && sAndroidAssetSubFolderLen > 0) {
			...
			if (std::strncmp(path + subStart, sAndroidAssetSubFolder, sAndroidAssetSubFolderLen) == 0 &&
				...
				asset = AAssetManager_open(sAndroidAssetMgr, path + fullStrip, AASSET_MODE_BUFFER);
				if (asset)
					return asset;
			}
		}
	...
	NkFile::NkFile() : mHandle(nullptr), mPath(), mMode(NkFileMode::NK_READ), mIsOpen(false), mIsAsset(false) {
		// Constructeur par défaut : état initial "fermé"
	}

	NkFile::NkFile(const char *path, NkFileMode mode)
		: mHandle(nullptr), mPath(path ? path : ""), mMode(mode), mIsOpen(false), mIsAsset(false) {
		Open(path, mode);
	}

	NkFile::NkFile(const NkPath &path, NkFileMode mode)
		: mHandle(nullptr), mPath(path), mMode(mode), mIsOpen(false), mIsAsset(false) {
		Open(path, mode);
	}
	...
	NkFile::NkFile(NkFile &&other) noexcept
		: mHandle(other.mHandle), mPath(other.mPath), mMode(other.mMode), mIsOpen(other.mIsOpen),
	...
	bool NkFile::MoveToTrash(const char *path) {
		return NkDirectory::MoveToTrash(path);
	}

	bool NkFile::MoveToTrash(const NkPath &path) {
		return NkDirectory::MoveToTrash(path.CStr());
	}

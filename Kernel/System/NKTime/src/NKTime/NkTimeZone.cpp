...
namespace nkentseu {
	...
	namespace {
		...
		static bool StartsWithIgnoreCase(const char *val, const char *prefix, const char **outRest) noexcept {
			// Gestion des pointeurs nuls pour sécurité défensive
			if (!val || !prefix) {
				return false;
			}
		...
		static time_t UtcDateToTimeT(const NkDate &date, int32 h, int32 m, int32 s) noexcept {
			...
			return static_cast<time_t>(days * SECONDS_PER_DAY + static_cast<int64>(h) * SECONDS_PER_HOUR +
									   static_cast<int64>(m) * SECONDS_PER_MINUTE + static_cast<int64>(s));
		}
...
#if defined(_GNU_SOURCE) || defined(__APPLE__) || (defined(__GLIBC__) && !defined(NKENTSEU_PLATFORM_WINDOWS))
		...
		static bool ParseFixedOffsetSeconds(const NkString &name, int32 &outOffset) noexcept {
			...
			if (!StartsWithIgnoreCase(val, "UTC", &rest) && !StartsWithIgnoreCase(val, "GMT", &rest)) {
				return false;
			}
	...
	NkTimeZone NkTimeZone::FromName(const NkString &ianaName) noexcept {
		...
		if (EqualsIgnoreCase(ianaName, "UTC") || EqualsIgnoreCase(ianaName, "GMT") || EqualsIgnoreCase(ianaName, "Z") ||
	...
	bool NkTimeZone::operator==(const NkTimeZone &o) const noexcept {
		...
		return mKind == o.mKind && mFixedOffsetSeconds == o.mFixedOffsetSeconds && mName == o.mName;
	}

...
namespace nkentseu {
	...
	NkClock::NkTime NkClock::NkTime::From(const NkElapsedTime &delta, const NkElapsedTime &total, uint64 frame,
										  float32 fixedDt, float32 scale) noexcept {
	...
	NkClock::NkClock() noexcept : mFrameChrono(), mTotalChrono(), mTime(), mPaused(false) {
	...
	const NkClock::NkTime &NkClock::Tick() noexcept {
		...
		mTime = NkTime::From(mPaused ? NkElapsedTime{} : rawDelta, // delta nul en pause, sinon mesure réelle

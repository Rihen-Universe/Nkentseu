...
namespace nkentseu {
	...
	static inline void *pngMalloc(usize n) noexcept {
		return NkAlloc(n);
	}

	static inline void *pngCalloc(usize n, usize s) noexcept {
		return NkAllocZero(n, s);
	}

	static inline void pngFree(void *p) noexcept {
		if (p)
			NkFree(p);
	}

	static inline void pngCopy(void *d, const void *s, usize n) noexcept {
		NkCopy(static_cast<uint8 *>(d), static_cast<const uint8 *>(s), n);
	}

	static inline void pngSet(void *d, int v, usize n) noexcept {
	...
	static uint32 chunkCRC(const uint8 *tb, const uint8 *data, usize len) noexcept {
		...
		for (int i = 0; i < 4; ++i)
			c = gCRC[(c ^ tb[i]) & 0xFF] ^ (c >> 8);
		if (data)
			for (usize i = 0; i < len; ++i)
				c = gCRC[(c ^ data[i]) & 0xFF] ^ (c >> 8);
		return ~c;
	}
	...
	static void unfilter(uint8 *cur, const uint8 *prior, int32 rowB, int32 bpp, uint8 f) noexcept {
		...
		switch (f) {
			case 0:
				break;
			case 1:
				for (int32 i = bpp; i < rowB; ++i)
					cur[i] = uint8(cur[i] + cur[i - bpp]);
				break;
			case 2:
				for (int32 i = 0; i < rowB; ++i)
					cur[i] = uint8(cur[i] + prior[i]);
				break;
			case 3:
				// stb: premier groupe (i < bpp) : seul prior contribue
				for (int32 i = 0; i < bpp; ++i)
					cur[i] = uint8(cur[i] + (prior[i] >> 1));
				for (int32 i = bpp; i < rowB; ++i)
					cur[i] = uint8(cur[i] + uint8((uint32(cur[i - bpp]) + prior[i]) >> 1));
				break;
			case 4:
				for (int32 i = 0; i < bpp; ++i)
					cur[i] = uint8(cur[i] + paeth(0, prior[i], 0));
				for (int32 i = bpp; i < rowB; ++i)
					cur[i] = uint8(cur[i] + paeth(cur[i - bpp], prior[i], prior[i - bpp]));
				break;
			default:
				break;
		}
	...
	static void expandSamp(const uint8 *src, uint8 *dst, int32 n, int32 bd) noexcept {
		...
		switch (bd) {
			...
			case 4:
				for (int32 i = 0; i < n; ++i)
					dst[i] = uint8((i & 1 ? (src[i / 2] & 0xF) : (src[i / 2] >> 4)) * sc);
				return;
			case 2:
				for (int32 i = 0; i < n; ++i)
					dst[i] = uint8(((src[i / 4] >> ((3 - (i % 4)) * 2)) & 3) * sc);
				return;
			case 1:
				for (int32 i = 0; i < n; ++i)
					dst[i] = uint8(((src[i / 8] >> (7 - (i % 8))) & 1) * sc);
				return;
			default:
				return;
		}
	...
	static void expandIdx(const uint8 *src, uint8 *dst, int32 w, int32 bd) noexcept {
		switch (bd) {
			...
			case 4:
				for (int32 i = 0; i < w; ++i)
					dst[i] = (i & 1 ? (src[i / 2] & 0xF) : (src[i / 2] >> 4));
				return;
			case 2:
				for (int32 i = 0; i < w; ++i)
	...
	NkImage *NkPNGCodec::Decode(const uint8 *data, usize size) noexcept {
		...
		while (!s.IsEOF() && !s.HasError()) {
			...
			if (cType == 0x49484452u) { // IHDR
				...
				imgW = int32(s.ReadU32BE());
				imgH = int32(s.ReadU32BE());
				bd = s.ReadU8();
				ct = s.ReadU8();
				(void)s.ReadU8();
				(void)s.ReadU8();
				...
				hasTRNS = true;
				if (ct == 3) {
					int32 n = int32(cLen) < 256 ? int32(cLen) : 256;
					s.ReadBytes(palA, n);
				} else if (ct == 0 && cLen >= 2) {
					tRns[0] = tRns[1] = tRns[2] = s.ReadU16BE();
				} else if (ct == 2 && cLen >= 6) {
					tRns[0] = s.ReadU16BE();
					tRns[1] = s.ReadU16BE();
					tRns[2] = s.ReadU16BE();
				...
				if (idatLen + cLen <= size) {
					s.ReadBytes(idat + idatLen, cLen);
					idatLen += cLen;
				}
			} else if (cType == 0x49454E44u) {
				s.Seek(cData + cLen + 4);
				break;
			}
		...
		const NkImagePixelFormat fmt = outN == 1   ? NkImagePixelFormat::NK_GRAY8
		...
		for (int32 y = 0; y < imgH; ++y) {
			...
			if (ct == 3) {
				...
				for (int32 x = 0; x < imgW; ++x) {
					uint8 idx = tmp[x];
					int32 pi = (idx < palSz) ? idx : 0;
					if (outN == 3) {
						dst[x * 3] = pal[pi * 3];
						dst[x * 3 + 1] = pal[pi * 3 + 1];
						dst[x * 3 + 2] = pal[pi * 3 + 2];
					} else {
						dst[x * 4] = pal[pi * 3];
						dst[x * 4 + 1] = pal[pi * 3 + 1];
						dst[x * 4 + 2] = pal[pi * 3 + 2];
						dst[x * 4 + 3] = palA[idx];
					}
				}
			} else if (bd == 16) {
				// 16-bit → MSB (stbi__convert_16_to_8 style)
				for (int32 i = 0; i < imgW * spp; ++i)
					tmp[i] = frow[i * 2];
				if (ct == 0) {
					...
					pngCopy(dst, tmp, usize(imgW * outN));
			} else if (bd < 8) {
				expandSamp(frow, tmp, imgW * spp, bd);
				if (ct == 0) {
					uint8 tv = 0;
					if (hasTRNS)
						tv = uint8((tRns[0] & ((1 << bd) - 1)) * kDS[bd]);
					for (int32 x = 0; x < imgW; ++x) {
						uint8 g = tmp[x];
						if (outN == 1)
							dst[x] = g;
	...
	bool NkPNGCodec::Encode(const NkImage &img, uint8 *&out, usize &outSize) noexcept {
		...
		for (int32 y = 0; y < h; ++y) {
			...
			switch (ft) {
				case 0:
					pngCopy(raw + rp, row, rb);
					break;
				case 1:
					for (int32 i = 0; i < rb; ++i)
						raw[rp + i] = uint8(row[i] - (i >= ch ? row[i - ch] : 0));
					break;
				case 2:
					for (int32 i = 0; i < rb; ++i)
						raw[rp + i] = uint8(row[i] - prev[i]);
					break;
				case 3:
					for (int32 i = 0; i < rb; ++i) {
						int32 a = (i >= ch) ? row[i - ch] : 0;
						raw[rp + i] = uint8(row[i] - uint8((a + prev[i]) >> 1));
					}
					break;
				case 4:
					for (int32 i = 0; i < rb; ++i) {
						int32 a = (i >= ch) ? row[i - ch] : 0, b = prev[i], c = (i >= ch) ? prev[i - ch] : 0;
						raw[rp + i] = uint8(row[i] - paeth(a, b, c));
					}
					break;
			}
			rp += rb;
		...
		uint8 *comp = nullptr;
		usize compSz = 0;
		if (!NkDeflate::Compress(raw, rawSz, comp, compSz, 6)) {
			pngFree(raw);
			return false;
		}
		pngFree(raw);

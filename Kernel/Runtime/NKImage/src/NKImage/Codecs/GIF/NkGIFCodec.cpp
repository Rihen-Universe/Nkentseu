...
namespace nkentseu {
	...
	struct LZWDecoder {
			...
			int32 Decode(int32 code, int32 prevCode, uint8 *outBuf, int32 maxOut) noexcept {
				if (code < clearCode) {
					// Symbole littéral
				...
				if (code >= nextCode) {
					// Code spécial : c+c[0]
					if (prevCode < 0 || prevCode >= nextCode) {
						error = true;
						return 0;
					}
	...
	NkImage *NkGIFCodec::Decode(const uint8 *data, usize size) noexcept {
		...
		while (!s.IsEOF() && !s.HasError()) {
			...
			if (intro == 0x2C) { // Image Descriptor
				...
				if (frame.hasLCT)
					s.ReadBytes(frame.lct, frame.lctSize * 3);
	...
	NkGIFAnimation *NkGIFCodec::DecodeAnimation(const uint8 *data, usize size) noexcept {
		...
		while (!s.IsEOF() && !s.HasError() && frameCount < kMaxFrames) {
			...
			if (intro == 0x2C) { // Image Descriptor (debut d'une frame)
				...
				if (currentFrame.hasLCT)
					s.ReadBytes(currentFrame.lct, currentFrame.lctSize * 3);
	...
	static void MedianCut(uint32 *pixels, int32 total, uint8 *palR, uint8 *palG, uint8 *palB,
		...
		while (numBoxes < 256) {
			...
			if (rr >= gr && rr >= br)
				::qsort(subPixels, cnt, sizeof(uint32), CmpR);
			else if (gr >= rr && gr >= br)
				::qsort(subPixels, cnt, sizeof(uint32), CmpG);
			else
				::qsort(subPixels, cnt, sizeof(uint32), CmpB);
		...
		// Complète à 256
		for (int32 i = numBoxes; i < 256; ++i) {
			palR[i] = palG[i] = palB[i] = 0;
		}
	}

	// Cherche l'entrée de palette la plus proche (distance euclidienne dans RGB)
	static uint8 NearestColor(uint8 r, uint8 g, uint8 b, const uint8 *palR, const uint8 *palG, const uint8 *palB,
							  int32 palCount) noexcept {
	...
	bool NkGIFCodec::Encode(const NkImage &img, uint8 *&out, usize &outSize) noexcept {
		...
		for (int32 i = 0; i < total; ++i) {
			...
			if (useTransp && ch == 4 && p[3] < 128) {
				...
				indexBuf[i] = NearestColor(p[0], ch > 1 ? p[1] : p[0], ch > 2 ? p[2] : p[0], palR, palG, palB,

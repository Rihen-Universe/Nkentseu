...
namespace nkentseu {
	...
	NkImage *NkICOCodec::Decode(const uint8 *data, usize size) noexcept {
		...
		// ── PNG embed ─────────────────────────────────────────────────────────────
		if (bestSize >= 8 && imgData[0] == 0x89 && imgData[1] == 'P' && imgData[2] == 'N' && imgData[3] == 'G') {
		...
		if (dibSize >= 16) {
			...
			dibHeight =
				static_cast<int32>(static_cast<uint32>(imgData[8]) | (static_cast<uint32>(imgData[9]) << 8) |
								   (static_cast<uint32>(imgData[10]) << 16) | (static_cast<uint32>(imgData[11]) << 24));
		} else {
			// BITMAPCOREHEADER : offset 4 = width (int16), offset 6 = height (int16)
			dibHeight = static_cast<int32>(static_cast<uint16>(imgData[6]) | (static_cast<uint16>(imgData[7]) << 8));
		}
		...
		const int32 realHeight = (absHeight == bestW * 2 || absHeight > bestH * 2) ? absHeight / 2 : absHeight;

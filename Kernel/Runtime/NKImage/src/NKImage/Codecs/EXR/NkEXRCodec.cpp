...
namespace nkentseu {
	...
	struct EXRChannel {
			...
			bool IsValid() const noexcept {
				return pixelType >= kPixelUINT && pixelType <= kPixelFLOAT && bytesPerPixel > 0 && xSampling == 1 &&
					   ySampling == 1;
			}
	};
	...
	static int32 ReadCString(const uint8 *base, usize size, usize &pos, char *outBuf, int32 maxLen) noexcept {
		int32 i = 0;
	...
	static bool DecompressNONE(const uint8 *in, usize inSize, uint8 *out, usize outSize) noexcept {
		if (inSize != outSize)
			return false;
	...
	static bool RleDecodeRaw(const uint8 *in, usize inSize, uint8 *out, usize outSize) noexcept {
		usize si = 0, di = 0;
		while (si < inSize) {
	...
	static bool DecompressRLE(const uint8 *in, usize inSize, uint8 *out, usize outSize) noexcept {
		if (inSize >= outSize) {
	...
	static bool DecompressZIP(const uint8 *in, usize inSize, uint8 *out, usize outSize) noexcept {
		if (inSize >= outSize) {
			return DecompressNONE(in, inSize, out, outSize);
		}
		...
		if (!tmp)
			return false;
		usize written = 0;
		if (!NkDeflate::Decompress(in, inSize, tmp, outSize, written) || written != outSize) {
			NkFree(tmp);
			return false;
		}
		// 2. Reorder inverse : tmp -> out
	...
	static uint16 PizBitmapToLut(const uint8 bitmap[PIZ_BITMAP_SIZE], uint16 lut[PIZ_USHORT_RANGE]) noexcept {
		int32 k = 0;
	...
	static void PizApplyLut(const uint16 lut[PIZ_USHORT_RANGE], uint16 *data, usize count) noexcept {
	...
	static NKIMG_INLINE void PizWdec14(uint16 l, uint16 h, uint16 &a, uint16 &b) noexcept {
	...
	static NKIMG_INLINE void PizWdec16(uint16 l, uint16 h, uint16 &a, uint16 &b) noexcept {
	...
	static void PizWav2Decode(uint16 *in, int32 nx, int32 ox, int32 ny, int32 oy, uint16 mx) noexcept {
	...
	static bool PizHufUnpackLengths(PizBitReader &br, int32 im, int32 iM, uint64 lengths[HUF_ENCSIZE]) noexcept {
	...
	static bool PizHufBuildDecodeTable(const uint64 lengths[HUF_ENCSIZE], int32 im, int32 iM,
	...
	static bool PizHufDecode(PizBitReader &br, const uint64 lengths[HUF_ENCSIZE], const int64 codes[HUF_ENCSIZE],
							 int32 im, int32 iM, uint16 *out, usize count) noexcept {
		const int32 rlc = iM;
	...
	static bool DecompressPIZImpl(const uint8 *in, usize inSize, uint8 *out, usize outSize, const EXRChannel *chans,
								  int32 numChans, int32 width, int32 linesInBlock) noexcept {
		...
		if (p + 4 > pEnd)
			return false;
		uint16 minNonZero = RD_U16LE(p);
		p += 2;
		uint16 maxNonZero = RD_U16LE(p);
		p += 2;
		...
		uint32 hufCompLen = RD_U32LE(p);
		p += 4;
		if (p + hufCompLen > pEnd) {
			NkFree(lut);
			return false;
		}
		...
		for (int32 c = 0; c < numChans; ++c) {
			totalU16 += usize(width) * usize(linesInBlock) * usize(chans[c].bytesPerPixel / 2);
		}
		...
		if (!PizHufDecode(br, lengths, canon, int32(im), int32(iM), decoded, totalU16)) {
			NkFree(canon);
			NkFree(lengths);
			NkFree(decoded);
			NkFree(lut);
		...
		for (int32 li = 0; li < linesInBlock; ++li) {
			for (int32 c = 0; c < numChans; ++c) {
				...
				if (perSample == 1) {
					...
					uint16 *srcLo = chanStart[c] + usize(width) * usize(linesInBlock) + usize(li) * usize(width);
					for (int32 x = 0; x < width; ++x) {
	...
	static bool DecompressPIZ(const uint8 *in, usize inSize, uint8 *out, usize outSize) noexcept {
		if (!gPizChans || gPizNumChans == 0 || gPizWidth == 0 || gPizLines == 0)
			return false;
		return DecompressPIZImpl(in, inSize, out, outSize, gPizChans, gPizNumChans, gPizWidth, gPizLines);
	}
	...
	static float32 ReadSample(const uint8 *p, int32 pixelType) noexcept {
		switch (pixelType) {
			case kPixelHALF:
				return HalfToFloat(RD_U16LE(p));
			case kPixelFLOAT:
				return RD_F32LE(p);
			case kPixelUINT:
				return float32(RD_U32LE(p));
			default:
				return 0.f;
		}
	}
	...
	NkImage *NkEXRCodec::Decode(const uint8 *data, usize size) noexcept {
		...
		for (int32 iter = 0; iter < 256; ++iter) {
			...
			// Attributs critiques
			if (::strcmp(attrName, "channels") == 0 && ::strcmp(attrType, "chlist") == 0) {
				...
				while (cp < attrEnd && data[cp] != 0 && numChans < kMaxChannels) {
					...
					ch.xSampling = RD_I32LE(data + cp);
					cp += 4;
					ch.ySampling = RD_I32LE(data + cp);
					cp += 4;
				...
				seenChannels = true;
			} else if (::strcmp(attrName, "compression") == 0 && ::strcmp(attrType, "compression") == 0 &&
					   attrSize >= 1) {
				compression = EXRCompression(attrData[0]);
				seenCompression = true;
			} else if (::strcmp(attrName, "dataWindow") == 0 && ::strcmp(attrType, "box2i") == 0 && attrSize >= 16) {
				xMin = RD_I32LE(attrData + 0);
				yMin = RD_I32LE(attrData + 4);
				xMax = RD_I32LE(attrData + 8);
				yMax = RD_I32LE(attrData + 12);
				seenDataWindow = true;
			} else if (::strcmp(attrName, "lineOrder") == 0 && ::strcmp(attrType, "lineOrder") == 0 && attrSize >= 1) {
		...
		const bool zipPath = (compression == EXRCompression::ZIP || compression == EXRCompression::ZIPS);
		const bool supported = (compression == EXRCompression::NONE || compression == EXRCompression::RLE || zipPath ||
		...
		for (int32 i = 0; i < numChans; ++i) {
			if (::strcmp(chans[i].name, "R") == 0)
				cm.rIdx = i;
			else if (::strcmp(chans[i].name, "G") == 0)
				cm.gIdx = i;
			else if (::strcmp(chans[i].name, "B") == 0)
				cm.bIdx = i;
			else if (::strcmp(chans[i].name, "A") == 0)
				cm.aIdx = i;
			else if (::strcmp(chans[i].name, "Y") == 0)
				cm.yIdx = i;
			else if (::strcmp(chans[i].name, "Z") == 0 && cm.yIdx < 0)
				cm.yIdx = i;
		}
		...
		for (int32 b = 0; b < numBlocks; ++b) {
			...
			for (int32 li = 0; li < lineInBlock; ++li) {
				const int32 dstY = (lineOrder == 1) ? (height - 1 - (yLocalStart + li)) // DECREASING_Y -> flip
													: (yLocalStart + li);

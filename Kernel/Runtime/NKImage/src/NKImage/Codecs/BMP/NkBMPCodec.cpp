...
namespace nkentseu {
	...
	static uint8 ScaleMask(uint32 val, uint32 mask) noexcept {
		...
		if (bits == 0)
			return 0;
		if (bits >= 8)
			return static_cast<uint8>(v >> (bits - 8));
	...
	NkImage NkBMPCodec::Decode(const uint8 *data, usize size) noexcept {
		...
		if (bpp <= 8) {
			if (isCore)
				...
				palCount = (clrUsed > 0) ? static_cast<int32>(clrUsed) : (1 << bpp);
		}
			...
			for (int32 row = 0; row < height; ++row) {
				...
				for (int32 x = 0; x < width; ++x) {
					...
					if (bpp == 1) {
						...
						const uint16 v =
							static_cast<uint16>(lineBuf[x * 2]) | (static_cast<uint16>(lineBuf[x * 2 + 1]) << 8);
						if (hasBitfields) {
							r = ScaleMask(v, maskR);
							g = ScaleMask(v, maskG);
							b = ScaleMask(v, maskB);

...
namespace nkentseu {
	...
	static inline void *qM(usize n) noexcept {
		return NkAlloc(n);
	}

	static inline void qF(void *p) noexcept {
		if (p)
			NkFree(p);
	}

	static inline void qCp(void *d, const void *s, usize n) noexcept {
		NkCopy(static_cast<uint8 *>(d), static_cast<const uint8 *>(s), n);
	}

	static inline void qSt(void *d, int v, usize n) noexcept {

...
namespace nkentseu {
	...
	NkImage *NkTGACodec::Decode(const uint8 *data, usize size) noexcept {
		...
		for (int32 i = 0; i < static_cast<int32>(width) * static_cast<int32>(height); ++i) {
			...
			if (needRead) {
				if (isIndexed) {
					const int32 idx = (bpp == 16) ? static_cast<int32>(s.ReadU16LE()) : static_cast<int32>(s.ReadU8());
	...
	bool NkTGACodec::Encode(const NkImage &img, uint8 *&out, usize &outSize) noexcept {
		...
		const uint8 kFooter[26] = {0,	0,	 0,	  0,   0,	0,	 0,	  0, // extension area offset + developer dir offset
								   'T', 'R', 'U', 'E', 'V', 'I', 'S', 'I', 'O',
								   'N', '-', 'X', 'F', 'I', 'L', 'E', '.', '\0'};

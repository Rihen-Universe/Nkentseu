		...
		private:
			...
			struct VP8LBitReader {
					...
					bool IsEOF() const noexcept {
						return bytePos >= size && bitCount <= 0;
					}
			};
			...
			static bool DecodeVP8LImage(VP8LBitReader &br, VP8LDecoder &dec, int32 w, int32 h, uint32 *out) noexcept;
			static void ApplyTransforms(VP8LDecoder &dec) noexcept;
			static void ApplyPredictTransform(const uint32 *data, uint32 *pixels, int32 w, int32 h,
											  int32 bits) noexcept;
			static void ApplyColorTransform(const uint32 *data, uint32 *pixels, int32 w, int32 h, int32 bits) noexcept;
			static void ApplySubtractGreen(uint32 *pixels, int32 n) noexcept;
			static void ApplyColorIndexing(const uint32 *table, uint32 *pixels, int32 w, int32 h, int32 bits) noexcept;

			static bool EncodeVP8L(const NkImage &img, NkImageStream &out) noexcept;
			static void BuildCanonicalHuff(const uint32 *freqs, int32 n, uint8 *codeLens) noexcept;
			static void WriteHuffmanCode(VP8LBitWriter &bw, const uint8 *codeLens, int32 n) noexcept;
			static void WriteImageData(VP8LBitWriter &bw, const uint32 *argb, int32 n,
									   const VP8LHuffTree trees[5]) noexcept;

			// Utilitaires RIFF
			static bool FindChunk(const uint8 *data, usize size, uint32 fourcc, RIFFChunk &out) noexcept;

			static uint32 MakeFourCC(char a, char b, char c, char d) noexcept {
				return static_cast<uint32>(a) | (static_cast<uint32>(b) << 8) | (static_cast<uint32>(c) << 16) |

...
namespace nkentseu {
	...
	struct NKENTSEU_IMAGE_API NkXMLArena {
			...
			const char *Dup(const char *str, int32 len = -1) noexcept {
				...
				if (!dst)
					return nullptr;
				nkentseu::memory::NkCopy(dst, str, n);
				dst[n] = 0;
				return dst;
			}
	};
	...
	struct NKENTSEU_IMAGE_API NkXMLNode {
			...
			NKIMG_NODISCARD const char *GetAttr(const char *name) const noexcept {
				for (NkXMLAttr *a = attrs; a; a = a->next)
					if (a->name && NkStringView(a->name) == NkStringView(name))
						return a->value;
				return nullptr;
			}
			...
			NKIMG_NODISCARD const char *GetAttrLocal(const char *localName_) const noexcept {
				for (NkXMLAttr *a = attrs; a; a = a->next)
					if (a->localName && NkStringView(a->localName) == NkStringView(localName_))
						return a->value;
				return nullptr;
			}
			...
			NKIMG_NODISCARD NkXMLNode *FirstChildNamed(const char *tag) const noexcept {
				for (NkXMLNode *c = firstChild; c; c = c->nextSibling)
					if (c->type == NkXMLNodeType::NK_ELEMENT && c->Is(tag))
						return c;
				return nullptr;
			}
			...
			NKIMG_NODISCARD NkXMLNode *NextSiblingNamed(const char *tag) const noexcept {
				for (NkXMLNode *s = nextSibling; s; s = s->nextSibling)
					if (s->type == NkXMLNodeType::NK_ELEMENT && s->Is(tag))
						return s;
				return nullptr;
			}
			...
			void GetTextContent(char *buf, int32 bufLen) const noexcept {
				...
				for (NkXMLNode *c = firstChild; c && pos < bufLen - 1; c = c->nextSibling) {
					if (c->type == NkXMLNodeType::NK_TEXT || c->type == NkXMLNodeType::NK_CDATA) {
						...
						if (n > 0) {
							nkentseu::memory::NkCopy(buf + pos, c->text, n);
							pos += n;
						}
					}
				}
	...
	struct NKENTSEU_IMAGE_API NkXMLDocument {
			...
			void Init(usize arenaBytes = 4 * 1024 * 1024) noexcept {
			...
			int32 GetElementsByTagName(const char *tag, NkXMLNode **outNodes, int32 maxOut) const noexcept {
				int32 count = 0;
				if (docElement)
					FindByTag(docElement, tag, outNodes, count, maxOut);
				return count;
			}
		...
		private:
			static NkXMLNode *FindById(NkXMLNode *node, const char *id) noexcept {
				if (node->type == NkXMLNodeType::NK_ELEMENT) {
					...
					if (v && NkStringView(v) == NkStringView(id))
						return node;
				}
			...
			static void FindByTag(NkXMLNode *node, const char *tag, NkXMLNode **out, int32 &cnt, int32 max) noexcept {
				if (node->type == NkXMLNodeType::NK_ELEMENT && node->localName &&
					(NkStringView(tag) == "*" || NkStringView(tag) == NkStringView(node->localName))) {
					if (cnt < max)
						out[cnt++] = node;
				}
				for (NkXMLNode *c = node->firstChild; c; c = c->nextSibling)

...
namespace nkentseu {
	...
	const char *NkGpuPolicy::VendorName(NkGpuVendor vendor) {
		switch (vendor) {
			case NkGpuVendor::NK_ANY:
				return "Any";
			case NkGpuVendor::NK_NVIDIA:
				return "Nvidia";
			case NkGpuVendor::NK_AMD:
				return "AMD";
			case NkGpuVendor::NK_INTEL:
				return "Intel";
			case NkGpuVendor::NK_APPLE:
				return "Apple";
			case NkGpuVendor::NK_QUALCOMM:
				return "Qualcomm";
			case NkGpuVendor::NK_ARM:
				return "ARM";
			case NkGpuVendor::NK_MICROSOFT:
				return "Microsoft";
			default:
				return "Unknown";
		}
	...
	bool NkGpuPolicy::MatchesVendorPciId(uint32 pciVendorId, NkGpuVendor vendor) {
		switch (vendor) {
			case NkGpuVendor::NK_ANY:
				return true;
			case NkGpuVendor::NK_NVIDIA:
				return pciVendorId == 0x10DEu;
			case NkGpuVendor::NK_AMD:
				return pciVendorId == 0x1002u || pciVendorId == 0x1022u;
			case NkGpuVendor::NK_INTEL:
				return pciVendorId == 0x8086u;
			case NkGpuVendor::NK_APPLE:
				return pciVendorId == 0x106Bu;
			case NkGpuVendor::NK_QUALCOMM:
				return pciVendorId == 0x5143u;
			case NkGpuVendor::NK_ARM:
				return pciVendorId == 0x13B5u;
			case NkGpuVendor::NK_MICROSOFT:
				return pciVendorId == 0x1414u;
			default:
				return false;
		}
	}
	...
	void NkGpuPolicy::ApplyPreContext(const NkContextDesc &desc) {
		logger.Infof("[NkGpuPolicy] api=%s pref=%s vendor=%s adapterIndex=%d", NkGraphicsApiName(desc.api),
					 PreferenceName(desc.gpu.preference), VendorName(desc.gpu.vendorPreference),
		...
		if (desc.gpu.adapterIndex >= 0) {
			logger.Warnf(
				"[NkGpuPolicy] OpenGL explicit adapter index is not portable; using OS/driver selection hints.");
		}
		...
		if (desc.gpu.preference == NkGpuPreference::NK_HIGH_PERFORMANCE) {
			logger.Infof("[NkGpuPolicy] Windows OpenGL high-performance mode: prefer OS Graphics settings / NVIDIA / "
						 "AMD app profile.");
		} else if (desc.gpu.preference == NkGpuPreference::NK_LOW_POWER) {

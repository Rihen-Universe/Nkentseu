...
#include "NKCanvas/Backend/OpenGL/NkOpenGLContextData.h"
#include "NKCanvas/Backend/Vulkan/NkVulkanContextData.h"

#if defined(NKENTSEU_PLATFORM_WINDOWS)
#include "NKCanvas/Backend/DirectX/NkDirectXContextData.h"
#endif
...
namespace nkentseu {
	...
	// -------------------------------------------------------------------------
	template <typename T> static inline T *NkGetNativeAs(NkIGraphicsContext *ctx, NkGraphicsApi expected) {
		if (!ctx || ctx->GetApi() != expected)
			return nullptr;
	...
	struct NkNativeContext {
			...
			static NkOpenGLContextData::ProcAddressFn GetOpenGLProcAddressLoader(NkIGraphicsContext *ctx) {
				auto *d = OpenGL(ctx);
...
// ── Vulkan ────────────────────────────────────────────────────────────
			...
			static VkQueue GetVkComputeQueue(NkIGraphicsContext *ctx) {
				auto *d = Vulkan(ctx);
				return d ? d->computeQueue : VK_NULL_HANDLE;
			}
			...
			static VkCommandPool GetVkComputePool(NkIGraphicsContext *ctx) {
				auto *d = Vulkan(ctx);
				return d ? d->computePool : VK_NULL_HANDLE;
			}
#endif
			...
			static NkDX11ContextData *DX11(NkIGraphicsContext *ctx) {
				return NkGetNativeAs<NkDX11ContextData>(ctx, NkGraphicsApi::NK_GFX_API_DX11);
			}
			...
			static ID3D11DepthStencilView *GetDX11DSV(NkIGraphicsContext *ctx) {
				auto *d = DX11(ctx);
				return d ? d->dsv.Get() : nullptr;
			}

			// ── DirectX 12 ────────────────────────────────────────────────────────
			static NkDX12ContextData *DX12(NkIGraphicsContext *ctx) {
				return NkGetNativeAs<NkDX12ContextData>(ctx, NkGraphicsApi::NK_GFX_API_DX12);
			}
			...
			static ID3D12CommandQueue *GetDX12ComputeQueue(NkIGraphicsContext *ctx) {
				auto *d = DX12(ctx);
				return d ? d->computeCommandQueue.Get() : nullptr;
			}
			...
			static ID3D12GraphicsCommandList4 *GetDX12ComputeCmdList(NkIGraphicsContext *ctx) {
				auto *d = DX12(ctx);
				return d ? d->computeCmdList.Get() : nullptr;
			}
			...
			static void *GetMetalCommandBuffer(NkIGraphicsContext *ctx) {
				auto *d = Metal(ctx);
				return d ? d->commandBuffer : nullptr;
			}

			static void *GetMetalLayer(NkIGraphicsContext *ctx) {
				auto *d = Metal(ctx);
				return d ? d->layer : nullptr;
			}
#endif
			...
			static NkSoftwareFramebuffer *GetSoftwareBackBuffer(NkIGraphicsContext *ctx) {
				if (!ctx || ctx->GetApi() != NkGraphicsApi::NK_GFX_API_SOFTWARE)
					return nullptr;

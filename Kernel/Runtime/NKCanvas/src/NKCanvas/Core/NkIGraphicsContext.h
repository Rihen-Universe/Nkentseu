...
namespace nkentseu {
	...
	class NkIGraphicsContext {
		public:
			...
			virtual bool RecreateSurface(const NkWindow & /*window*/) {
				return true;
			}
			...
			virtual NkSwapchainCallbackHandle AddCleanUpCallback(NkSwapchainCleanFn fn) {
				return NK_INVALID_CALLBACK_HANDLE;
			}

			virtual NkSwapchainCallbackHandle AddRecreateCallback(NkSwapchainRecreateFn fn) {
				return NK_INVALID_CALLBACK_HANDLE;
			}

...
namespace nkentseu {
	...
	static void ConvertRGBAtoBGRA_ForGDI(const NkSoftwareFramebuffer &fb, void *dibBits) {
		if (!dibBits || !fb.IsValid())
			return;
		...
		for (uint32 i = 0; i < count; ++i) {
			...
			dst[i] = (p & 0xFF00FF00u) | ((p & 0x00FF0000u) >> 16u) | ((p & 0x000000FFu) << 16u);
		}
	}

	// =============================================================================
	NkSoftwareContext::~NkSoftwareContext() {
		if (mIsValid)
			Shutdown();
	}
	...
	bool NkSoftwareContext::Initialize(const NkWindow &window, const NkContextDesc &desc) {
		if (mIsValid) {
			NK_SW_ERR("Already initialized\n");
			return false;
		}
		mDesc = desc;
		const NkSurfaceDesc surf = window.GetSurfaceDesc();
		if (!surf.IsValid()) {
			NK_SW_ERR("Invalid NkSurfaceDesc\n");
			return false;
		}
	...
	void NkSoftwareContext::SetVSync(bool e) {
		mVSync = e;
	}

	bool NkSoftwareContext::GetVSync() const {
		return mVSync;
	}

	bool NkSoftwareContext::IsValid() const {
		return mIsValid;
	}

	NkGraphicsApi NkSoftwareContext::GetApi() const {
		return NkGraphicsApi::NK_GFX_API_SOFTWARE;
	}

	NkContextDesc NkSoftwareContext::GetDesc() const {
		return mDesc;
	}

	void *NkSoftwareContext::GetNativeContextData() {
		return &mData;
	}

	bool NkSoftwareContext::SupportsCompute() const {
		return true;
	}
...
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
		...
		if (!hwnd) {
			NK_SW_ERR("InitNativePresenter: surf.hwnd is null\n");
			return false;
		}
...
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
	...
	fallback_xlib: {
		...
		Visual *vis = DefaultVisual(display, DefaultScreen(display));
		int depth = DefaultDepth(display, DefaultScreen(display));
		char *data = (char *)nkentseu::memory::NkAlloc((nk_size)(w * h * 4));
...
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
	...
	void NkSoftwareContext::PresentNative() {
		...
		if (!BitBlt(static_cast<HDC>(mData.hdc), 0, 0, (int)fb.width, (int)fb.height, static_cast<HDC>(mData.dibDC), 0,
					0, SRCCOPY)) {
		...
		if (mData.useSHM) {
			...
			XShmPutImage(disp, (::Window)mData.window, gc, img, 0, 0, 0, 0, fb.width, fb.height, False);
		} else {
			XPutImage(disp, (::Window)mData.window, gc, img, 0, 0, 0, 0, fb.width, fb.height);
		}
		XFlush(disp);
		...
		xcb_put_image(conn, XCB_IMAGE_FORMAT_Z_PIXMAP, (xcb_window_t)mData.window, (xcb_gcontext_t)mData.gc,
					  (uint16_t)fb.width, (uint16_t)fb.height, 0, 0, 0, 24, (uint32_t)fb.pixels.Size(),
		...
		if (ANativeWindow_lock(win, &buf, &bounds) == 0) {
			...
			for (uint32 y = 0; y < fb.height && y < (uint32)buf.height; ++y) {
				memcpy((uint8_t *)buf.bits + y * buf.stride * 4, fb.RowPtr(y), copyW * 4);
			}
		...
		EM_ASM(
			...
			mData.canvasId, (int)fb.width, (int)fb.height, (int)(uintptr_t)fb.pixels.Data());
#endif
	}

...
#define NK_SWC_ERR(...) logger.Errorf("[NkSoftwareCompute] " __VA_ARGS__)
	...
	bool NkSoftwareComputeContext::WriteBuffer(NkComputeBuffer &buf, const void *data, uint64 bytes, uint64 offset) {
		if (!buf.valid || !data) {
			return false;
		}
	...
	bool NkSoftwareComputeContext::ReadBuffer(const NkComputeBuffer &buf, void *out, uint64 bytes, uint64 offset) {
		if (!buf.valid || !out) {
			return false;
		}
		if (offset + bytes > buf.sizeBytes) {
	...
	NkComputeShader NkSoftwareComputeContext::CreateShaderFromSource(const char *src, const char * /*entry*/) {
		NkComputeShader out;
		if (!mIsValid || !src || !*src) {
			return out;
		}
	...
	NkComputeShader NkSoftwareComputeContext::CreateShaderFromFile(const char *path, const char *entry) {
		(void)entry;
		NkVector<char> src;

...
namespace nkentseu {
	...
	struct NkOpenGLComputeContext::Functions {
			...
			bool Load(NkOpenGLComputeContext::ProcAddressLoader loader) {
				...
				return GenBuffers && BindBuffer && BufferData && DeleteBuffers && BufferSubData && MapBufferRange &&
					   UnmapBuffer && CreateShader && ShaderSource && CompileShader && GetShaderiv &&
					   GetShaderInfoLog && DeleteShader && CreateProgram && AttachShader && LinkProgram &&
					   GetProgramiv && GetProgramInfoLog && DeleteProgram && UseProgram && BindBufferBase &&
					   DispatchCompute && Finish && MemoryBarrier && GetIntegeriV && GetIntegerv;
			}
	};
	...
	bool NkOpenGLComputeContext::IsValid() const {
		return mIsValid;
	}
	...
	NkComputeBuffer NkOpenGLComputeContext::CreateBuffer(const NkComputeBufferDesc &desc) {
		...
		else if (desc.cpuWritable)
			usage = GL_DYNAMIC_DRAW;
		else if (desc.cpuReadable)
	...
	bool NkOpenGLComputeContext::ReadBuffer(const NkComputeBuffer &buf, void *outData, uint64 bytes, uint64 offset) {
		...
		mFns->BindBuffer(GL_SHADER_STORAGE_BUFFER, ssbo);
		void *ptr =
			mFns->MapBufferRange(GL_SHADER_STORAGE_BUFFER, (GLintptr)offset, (GLsizeiptr)bytes, GL_MAP_READ_BIT);
#else
		glBindBuffer(GL_SHADER_STORAGE_BUFFER, ssbo);
	...
	NkGraphicsApi NkOpenGLComputeContext::GetApi() const {
		return mApi;
	}
	...
	bool NkOpenGLComputeContext::SupportsAtomics() const {
		return mIsValid;
	}

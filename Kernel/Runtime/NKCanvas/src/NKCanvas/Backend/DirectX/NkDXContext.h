	...
	class NkDX11Context final : public NkIGraphicsContext {
		public:
			...
			ID3D11Device1 *GetDevice() {
				return mData.device.Get();
			}

			ID3D11DeviceContext1 *GetContext() {
				return mData.context.Get();
			}
	...
	class NkDX12Context final : public NkIGraphicsContext {
		public:
			...
			ID3D12CommandQueue *GetCommandQueue() {
				return mData.commandQueue.Get();
			}

			ID3D12GraphicsCommandList4 *GetCommandList() {
				return mData.cmdList.Get();
			}
			...
			static void __stdcall DebugMessageCallback(D3D12_MESSAGE_CATEGORY category, D3D12_MESSAGE_SEVERITY severity,
		...
		private:
			...
			struct NkDX12CbEntry {
					NkSwapchainCallbackHandle handle;
					NkSwapchainCleanFn fn;
			};

...
#define NK_DX11C_CHECK(hr, msg)                                                                                        \
	do {                                                                                                               \
		if (FAILED(hr)) {                                                                                              \
			NK_DX11C_ERR(msg " hr=0x%08X\n", (unsigned)(hr));                                                          \
			return;                                                                                                    \
		}                                                                                                              \
	} while (0)
#define NK_DX11C_CHECKB(hr, msg)                                                                                       \
	do {                                                                                                               \
		if (FAILED(hr)) {                                                                                              \
			NK_DX11C_ERR(msg " hr=0x%08X\n", (unsigned)(hr));                                                          \
			return {};                                                                                                 \
...
namespace nkentseu {
	...
	NkDX11ComputeContext::~NkDX11ComputeContext() {
		if (mIsValid)
			Shutdown();
	}
	...
	bool NkDX11ComputeContext::IsValid() const {
		return mIsValid;
	}

	// â”€â”€ Buffers (Structured Buffer + UAV + Staging readback)
	...
	NkComputeBuffer NkDX11ComputeContext::CreateBuffer(const NkComputeBufferDesc &desc) {
		...
		HRESULT hr = mDevice->CreateBuffer(&bd, desc.initialData ? &initData : nullptr, &bs->buf);
		if (FAILED(hr)) {
			NK_DX11C_ERR("CreateBuffer failed 0x%08X\n", (unsigned)hr);
	...
	bool NkDX11ComputeContext::WriteBuffer(NkComputeBuffer &buf, const void *data, uint64 bytes, uint64 offset) {
		if (!buf.valid || !data)
			return false;
	...
	bool NkDX11ComputeContext::ReadBuffer(const NkComputeBuffer &buf, void *outData, uint64 bytes, uint64 offset) {
	...
	NkComputeShader NkDX11ComputeContext::CreateShaderFromSource(const char *src, const char *entry) {
		NkComputeShader s;
		...
		hr = mDevice->CreateComputeShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, &cs);
		if (FAILED(hr)) {
			NK_DX11C_ERR("CreateComputeShader failed\n");
			return s;
		}
		s.handle = cs.Detach();
		s.valid = true;
	...
	NkComputeShader NkDX11ComputeContext::CreateShaderFromFile(const char *path, const char *entry) {
		NkVector<char> src;
	...
	// â”€â”€ Pipeline = juste le shader en DX11
	...
	void NkDX11ComputeContext::DestroyPipeline(NkComputePipeline &p) {
		p = {};
	}
	...
	NkGraphicsApi NkDX11ComputeContext::GetApi() const {
		return NkGraphicsApi::NK_GFX_API_DX11;
	}

	uint32 NkDX11ComputeContext::GetMaxGroupSizeX() const {
		return 1024;
	}

	uint32 NkDX11ComputeContext::GetMaxGroupSizeY() const {
		return 1024;
	}

	uint32 NkDX11ComputeContext::GetMaxGroupSizeZ() const {
		return 64;
	}

	uint64 NkDX11ComputeContext::GetSharedMemoryBytes() const {
		return 32768;
	}

	bool NkDX11ComputeContext::SupportsAtomics() const {
		return true;
	}

	bool NkDX11ComputeContext::SupportsFloat64() const {
		return false;
	}

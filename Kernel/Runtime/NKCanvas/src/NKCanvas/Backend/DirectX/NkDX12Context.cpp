...
namespace nkentseu {
	namespace {
		...
		static bool PickDx12Adapter(IDXGIFactory6 *factory, const NkContextDesc &contextDesc,
									const NkDirectX12Desc &dx12Desc, ComPtr<IDXGIAdapter4> &outAdapter,
									DXGI_ADAPTER_DESC3 &outDesc, uint32 &outIndex) {
			outAdapter.Reset();
			outDesc = {};
			outIndex = UINT32_MAX;
			...
			auto tryPick = [&](IDXGIAdapter1 *a1, uint32 index) -> bool {
				...
				if (FAILED(D3D12CreateDevice(a1, D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(&test))))
					return false;
				if (FAILED(a1->QueryInterface(IID_PPV_ARGS(&outAdapter))))
					return false;
				if (FAILED(outAdapter->GetDesc3(&outDesc)))
					return false;
				outIndex = index;
				return true;
			};
	...
	bool NkDX12Context::Initialize(const NkWindow &window, const NkContextDesc &desc) {
		if (mIsValid) {
			NK_DX12_ERR("Already initialized\n");
			return false;
		}
		mDesc = desc;
		const NkSurfaceDesc surf = window.GetSurfaceDesc();
		if (!surf.IsValid()) {
			NK_DX12_ERR("Invalid NkSurfaceDesc\n");
			return false;
		}
	...
	// =============================================================================
	void __stdcall NkDX12Context::DebugMessageCallback(D3D12_MESSAGE_CATEGORY /*category*/,
													   D3D12_MESSAGE_SEVERITY severity, D3D12_MESSAGE_ID id,
													   LPCSTR description, void * /*context*/) {
		const char *sevStr = (severity == D3D12_MESSAGE_SEVERITY_CORRUPTION) ? "CORRUPTION"
	...
	bool NkDX12Context::CreateDevice(const NkDirectX12Desc &d) {
		...
		NK_DX12_CHECK(D3D12CreateDevice(mData.adapter.Get(), D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(&mData.device)),
					  "D3D12CreateDevice");
		...
		if (d.debugDevice) {
			...
			if (SUCCEEDED(mData.device.As(&infoQueue))) {
				...
				DWORD cookie = 0;
				infoQueue->RegisterMessageCallback(&NkDX12Context::DebugMessageCallback,
				...
				NK_DX12_LOG(
					"DX12 InfoQueue1 unavailable (Windows < 10 2004 ?) — debug messages -> OutputDebugString only");
			}
		}
		NK_DX12_LOG("Device OK (adapter #%u vendor=0x%04X device=0x%04X VRAM %u MB)\n", chosenIndex,
					(unsigned)chosenDesc.VendorId, (unsigned)chosenDesc.DeviceId, mData.vramMB);
		return true;
	}
	...
	bool NkDX12Context::CreateCommandQueues(const NkDirectX12Desc &d) {
		...
		NK_DX12_CHECK(mData.device->CreateCommandQueue(&qDesc, IID_PPV_ARGS(&mData.commandQueue)),
					  "CreateCmdQueue DIRECT");

		// Compute queue dédiée
		if (d.enableComputeQueue) {
			qDesc.Type = D3D12_COMMAND_LIST_TYPE_COMPUTE;
			NK_DX12_CHECK(mData.device->CreateCommandQueue(&qDesc, IID_PPV_ARGS(&mData.computeCommandQueue)),
						  "CreateCmdQueue COMPUTE");
		}
		return true;
	}
	...
	bool NkDX12Context::CreateSwapchain(uint32 w, uint32 h, const NkDirectX12Desc &d) {
		...
		NK_DX12_CHECK(mData.factory->CreateSwapChainForHwnd(mData.commandQueue.Get(), mData.hwnd, &scDesc, nullptr,
															nullptr, &sc1),
					  "CreateSwapChain");
	...
	bool NkDX12Context::CreateDescriptorHeaps(const NkDirectX12Desc &d) {
		...
		NK_DX12_CHECK(mData.device->CreateDescriptorHeap(&sampHeapDesc, IID_PPV_ARGS(&mData.samplerHeap)),
					  "CreateSamplerHeap");
	...
	bool NkDX12Context::CreateDepthBuffer(uint32 w, uint32 h) {
		...
		NK_DX12_CHECK(mData.device->CreateCommittedResource(&hp, D3D12_HEAP_FLAG_NONE, &rd,
															D3D12_RESOURCE_STATE_DEPTH_WRITE, &clearVal,
															IID_PPV_ARGS(&mData.depthBuffer)),
					  "CreateDepthBuffer");
	...
	bool NkDX12Context::CreateCommandObjects() {
		for (uint32 i = 0; i < mData.backBufferCount; ++i) {
			NK_DX12_CHECK(mData.device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT,
															   IID_PPV_ARGS(&mData.cmdAllocators[i])),
						  "CreateCmdAllocator");
		}
		NK_DX12_CHECK(mData.device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, mData.cmdAllocators[0].Get(),
													  nullptr, IID_PPV_ARGS(&mData.cmdList)),
					  "CreateCommandList");
		mData.cmdList->Close();

		// Compute
		if (mData.computeCommandQueue) {
			NK_DX12_CHECK(mData.device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_COMPUTE,
															   IID_PPV_ARGS(&mData.computeAllocator)),
						  "CreateComputeAllocator");
			NK_DX12_CHECK(mData.device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_COMPUTE,
														  mData.computeAllocator.Get(), nullptr,
														  IID_PPV_ARGS(&mData.computeCmdList)),
						  "CreateComputeCmdList");
	...
	bool NkDX12Context::CreateSyncObjects() {
		...
		if (!mData.fenceEvent) {
			NK_DX12_ERR("CreateEvent failed\n");
			return false;
		}
	...
	NkSwapchainCallbackHandle NkDX12Context::AddCleanUpCallback(NkSwapchainCleanFn fn) {
		if (!fn)
			return NK_INVALID_CALLBACK_HANDLE;
	...
	bool NkDX12Context::BeginFrame() {
		...
		mData.cmdList->ClearDepthStencilView(mData.dsvHandle, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f,
											 0, 0, nullptr);
	...
	NkContextInfo NkDX12Context::GetInfo() const {
		NkContextInfo i;
		i.api = NkGraphicsApi::NK_GFX_API_DX12;

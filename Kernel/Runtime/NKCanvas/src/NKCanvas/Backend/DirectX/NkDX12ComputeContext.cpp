...
#define NK_DX12C_CHECK(hr, msg)                                                                                        \
	do {                                                                                                               \
		if (FAILED(hr)) {                                                                                              \
			NK_DX12C_ERR(msg " 0x%08X\n", (unsigned)(hr));                                                             \
			return;                                                                                                    \
		}                                                                                                              \
	} while (0)
#define NK_DX12C_CHECKB(hr, msg)                                                                                       \
	do {                                                                                                               \
		if (FAILED(hr)) {                                                                                              \
			NK_DX12C_ERR(msg " 0x%08X\n", (unsigned)(hr));                                                             \
			return {};                                                                                                 \
...
namespace nkentseu {
	...
	NkDX12ComputeContext::~NkDX12ComputeContext() {
		if (mIsValid)
			Shutdown();
	}
	...
	void NkDX12ComputeContext::InitFromGraphicsContext(NkIGraphicsContext *gfx) {
		...
		mDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_COMPUTE, IID_PPV_ARGS(&mAllocator));
		mDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_COMPUTE, mAllocator.Get(), nullptr,
	...
	bool NkDX12ComputeContext::IsValid() const {
		return mIsValid;
	}
	...
	NkComputeBuffer NkDX12ComputeContext::CreateBuffer(const NkComputeBufferDesc &desc) {
		...
		D3D12_RESOURCE_STATES initState =
			(hp.Type == D3D12_HEAP_TYPE_UPLOAD) ? D3D12_RESOURCE_STATE_GENERIC_READ : D3D12_RESOURCE_STATE_COMMON;

		HRESULT hr = mDevice->CreateCommittedResource(&hp, D3D12_HEAP_FLAG_NONE, &rd, initState, nullptr,
													  IID_PPV_ARGS(&bs->gpu));
		if (FAILED(hr)) {
			NK_DX12C_ERR("CreateBuffer failed\n");
			nkentseu::memory::NkGetDefaultAllocator().Delete(bs);
			return out;
		}
		...
		if (desc.cpuReadable) {
			...
			mDevice->CreateCommittedResource(&rhp, D3D12_HEAP_FLAG_NONE, &rrd, D3D12_RESOURCE_STATE_COPY_DEST, nullptr,
											 IID_PPV_ARGS(&bs->readback));
		}
	...
	bool NkDX12ComputeContext::WriteBuffer(NkComputeBuffer &buf, const void *data, uint64 bytes, uint64 offset) {
		if (!buf.valid || !data)
			return false;
		auto *bs = static_cast<DX12BufSet *>(buf.handle);
		void *mapped = nullptr;
		D3D12_RANGE range{(SIZE_T)offset, (SIZE_T)(offset + bytes)};
		if (FAILED(bs->gpu->Map(0, &range, &mapped)))
			return false;
	...
	bool NkDX12ComputeContext::ReadBuffer(const NkComputeBuffer &buf, void *outData, uint64 bytes, uint64 offset) {
		...
		if (FAILED(bs->readback->Map(0, &readRange, &mapped)))
			return false;
	...
	// â”€â”€ Shaders HLSL â†’ DXIL
	// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
	struct DX12ShaderBlob {
			ComPtr<ID3DBlob> blob;
	};

	NkComputeShader NkDX12ComputeContext::CreateShaderFromSource(const char *src, const char *entry) {
		NkComputeShader s;
		ComPtr<ID3DBlob> blob, err;
		HRESULT hr = D3DCompile(src, strlen(src), nullptr, nullptr, nullptr, entry, "cs_5_1",
								D3DCOMPILE_OPTIMIZATION_LEVEL3, 0, &blob, &err);
		if (FAILED(hr)) {
			NK_DX12C_ERR("HLSL compile: %s\n", err ? (char *)err->GetBufferPointer() : "unknown");
			return s;
		}
		auto *b = nkentseu::memory::NkGetDefaultAllocator().New<DX12ShaderBlob>();
		b->blob = blob;
		s.handle = b;
		s.valid = true;
		return s;
	...
	NkComputeShader NkDX12ComputeContext::CreateShaderFromFile(const char *path, const char *entry) {
		NkVector<char> src;
	...
	NkGraphicsApi NkDX12ComputeContext::GetApi() const {
		return NkGraphicsApi::NK_GFX_API_DX12;
	}

	uint32 NkDX12ComputeContext::GetMaxGroupSizeX() const {
		return 1024;
	}

	uint32 NkDX12ComputeContext::GetMaxGroupSizeY() const {
		return 1024;
	}

	uint32 NkDX12ComputeContext::GetMaxGroupSizeZ() const {
		return 64;
	}

	uint64 NkDX12ComputeContext::GetSharedMemoryBytes() const {
		return 32768;
	}

	bool NkDX12ComputeContext::SupportsAtomics() const {
		return true;
	}

	bool NkDX12ComputeContext::SupportsFloat64() const {
		return false;
	}

...
namespace nkentseu {
	...
	NkMetalComputeContext::~NkMetalComputeContext() {
		if (mIsValid)
			Shutdown();
	}
	...
	void NkMetalComputeContext::InitFromGraphicsContext(NkIGraphicsContext *gfx) {
		...
		if (!md || !md->device || !md->commandQueue)
			return;
	...
	bool NkMetalComputeContext::Init(const NkContextDesc &) {
		...
		if (!dev) {
			NK_MTLC_ERR("MTLCreateSystemDefaultDevice failed\n");
			return false;
		}
		...
		if (!q) {
			NK_MTLC_ERR("newCommandQueue failed\n");
			return false;
		}
	...
	void NkMetalComputeContext::Shutdown() {
		...
		if (mOwnsDevice) {
			if (mQueue) {
				CFBridgingRelease(mQueue);
				mQueue = nullptr;
			}
			if (mDevice) {
				CFBridgingRelease(mDevice);
				mDevice = nullptr;
			}
		}
	...
	bool NkMetalComputeContext::IsValid() const {
		return mIsValid;
	}
	...
	NkComputeBuffer NkMetalComputeContext::CreateBuffer(const NkComputeBufferDesc &desc) {
		...
		if (desc.initialData && (opts == MTLResourceStorageModeShared)) {
			buf = [dev newBufferWithBytes:desc.initialData length:(NSUInteger)desc.sizeBytes options:opts];
		} else {
			buf = [dev newBufferWithLength:(NSUInteger)desc.sizeBytes options:opts];
		...
		if (!buf) {
			NK_MTLC_ERR("newBufferWithLength failed\n");
			return out;
		}
	...
	bool NkMetalComputeContext::WriteBuffer(NkComputeBuffer &buf, const void *data, uint64 bytes, uint64 offset) {
		if (!buf.valid || !data)
			return false;
	...
	bool NkMetalComputeContext::ReadBuffer(const NkComputeBuffer &buf, void *outData, uint64 bytes, uint64 offset) {
	...
	// Shaders MSL
	// =============================================================================
	NkComputeShader NkMetalComputeContext::CreateShaderFromSource(const char *src, const char *entry) {
		NkComputeShader s;
		if (!mIsValid || !src)
			return s;
	...
	NkComputeShader NkMetalComputeContext::CreateShaderFromFile(const char *path, const char *entry) {
		NkVector<char> src;
		if (!ReadTextFile(path, src)) {
			NK_MTLC_ERR("Cannot open: %s\n", path);
			return {};
		}
	...
	void NkMetalComputeContext::BindBuffer(uint32 slot, NkComputeBuffer &buf) {
		...
		if (slot + 1 > mBoundBufferCount)
			mBoundBufferCount = slot + 1;
	}
	...
	void NkMetalComputeContext::Dispatch(uint32 gx, uint32 gy, uint32 gz) {
		...
		if (tgSize > 256)
			tgSize = 256; // s̩curit̩ conservative
	...
	// Capacit̩s
	// =============================================================================
	NkGraphicsApi NkMetalComputeContext::GetApi() const {
	...
	uint32 NkMetalComputeContext::GetMaxGroupSizeY() const {
		return 1024;
	}

	uint32 NkMetalComputeContext::GetMaxGroupSizeZ() const {
		return 1024;
	}
	...
	bool NkMetalComputeContext::SupportsAtomics() const {
		return true;
	}

	bool NkMetalComputeContext::SupportsFloat64() const {
		return false;
	} // Metal ne supporte pas fp64

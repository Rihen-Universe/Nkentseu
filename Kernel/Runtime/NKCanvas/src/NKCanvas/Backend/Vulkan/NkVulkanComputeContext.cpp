...
namespace nkentseu {
	...
	NkVulkanComputeContext::~NkVulkanComputeContext() {
		if (mIsValid)
			Shutdown();
	}
	...
	void NkVulkanComputeContext::InitFromGraphicsContext(NkIGraphicsContext *gfx) {
		...
		mData.computeQueue = vkData->computeQueue ? vkData->computeQueue : vkData->graphicsQueue;
		mData.computeFamily = vkData->computeFamily != UINT32_MAX ? vkData->computeFamily : vkData->graphicsFamily;
	...
	void NkVulkanComputeContext::Shutdown() {
		...
		WaitIdle();
		if (mDescPool)
			vkDestroyDescriptorPool(mData.device, mDescPool, nullptr);
		if (mPipeLayout)
			vkDestroyPipelineLayout(mData.device, mPipeLayout, nullptr);
		if (mSetLayout)
			vkDestroyDescriptorSetLayout(mData.device, mSetLayout, nullptr);
		if (mData.fence)
			vkDestroyFence(mData.device, mData.fence, nullptr);
		if (mData.commandPool)
			vkDestroyCommandPool(mData.device, mData.commandPool, nullptr);
		mIsValid = false;
		NK_VKC_LOG("Shutdown\n");
	}

	bool NkVulkanComputeContext::IsValid() const {
		return mIsValid;
	}

	// â”€â”€ MÃ©moire
	// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
	bool NkVulkanComputeContext::FindMemoryType(uint32 filter, VkMemoryPropertyFlags props, uint32 &out) {
	...
	bool NkVulkanComputeContext::CreateBuffer_Internal(uint64 size, VkBufferUsageFlags usage,
		...
		uint32 memType = 0;
		if (!FindMemoryType(req.memoryTypeBits, memProps, memType))
			return false;
	...
	bool NkVulkanComputeContext::WriteBuffer(NkComputeBuffer &buf, const void *data, uint64 bytes, uint64 offset) {
		if (!buf.valid || !data)
			return false;
		auto *pair = static_cast<VkBufPair *>(buf.handle);
		void *mapped = nullptr;
	...
	bool NkVulkanComputeContext::ReadBuffer(const NkComputeBuffer &buf, void *outData, uint64 bytes, uint64 offset) {
		if (!buf.valid || !outData)
			return false;
		auto *pair = static_cast<VkBufPair *>(buf.handle);
		void *mapped = nullptr;
		if (vkMapMemory(mData.device, pair->mem, offset, bytes, 0, &mapped) != VK_SUCCESS)
	...
	NkComputeShader NkVulkanComputeContext::CreateShaderFromFile(const char *path, const char * /*entry*/) {
		NkComputeShader s;
	...
	NkComputeShader NkVulkanComputeContext::CreateShaderFromSource(const char * /*source*/, const char * /*entry*/) {
	...
	NkComputePipeline NkVulkanComputeContext::CreatePipeline(const NkComputeShader &s) {
		...
		if (vkCreateComputePipelines(mData.device, VK_NULL_HANDLE, 1, &pipeInfo, nullptr, &pipe) != VK_SUCCESS) {
	...
	void NkVulkanComputeContext::BindBuffer(uint32 slot, NkComputeBuffer &buf) {
		if (!buf.valid || slot >= kMaxBindings)
			return;
		auto *pair = static_cast<VkBufPair *>(buf.handle);
	...
	void NkVulkanComputeContext::Dispatch(uint32 gx, uint32 gy, uint32 gz) {
		if (!mIsValid || mCurrentPipeline == VK_NULL_HANDLE)
			return;
		...
		vkCmdBindDescriptorSets(mData.commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, mPipeLayout, 0, 1, &mDescSet, 0,
								nullptr);
	...
	NkGraphicsApi NkVulkanComputeContext::GetApi() const {
	...
	bool NkVulkanComputeContext::SupportsAtomics() const {
		return true;
	}
...
namespace nkentseu {
	...
	NkComputeBuffer NkVulkanComputeContext::CreateBuffer(const NkComputeBufferDesc &) {
		return {};
	}

	void NkVulkanComputeContext::DestroyBuffer(NkComputeBuffer &b) {
		b = {};
	}

	bool NkVulkanComputeContext::WriteBuffer(NkComputeBuffer &, const void *, uint64, uint64) {
		return false;
	}

	bool NkVulkanComputeContext::ReadBuffer(const NkComputeBuffer &, void *, uint64, uint64) {
		return false;
	}

	NkComputeShader NkVulkanComputeContext::CreateShaderFromSource(const char *, const char *) {
		return {};
	}

	NkComputeShader NkVulkanComputeContext::CreateShaderFromFile(const char *, const char *) {
		return {};
	}
	...
	NkComputePipeline NkVulkanComputeContext::CreatePipeline(const NkComputeShader &) {
		return {};
	}

...
//
// Link flags (à ajouter dans NKCanvas.jenga pour Vulkan) :
...
#include <shaderc/shaderc.hpp>
		...
		inline NkVector<uint32> NkCompileGLSLToSPIRV(const char *glslSource, shaderc_shader_kind kind,
			...
			auto result = compiler.CompileGlslToSpv(glslSource, strlen(glslSource), kind, sourceName, opts);
...
#include <glslang/Public/ResourceLimits.h>
		...
		inline NkVector<uint32> NkCompileGLSLToSPIRV_Impl(const char *glslSource, EShLanguage stage) {
		...
		inline NkVector<uint32> NkCompileVertGLSL(const char *) {
			return {};
		}

		inline NkVector<uint32> NkCompileFragGLSL(const char *) {
			return {};
		}
#endif

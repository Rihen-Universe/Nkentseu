...
namespace nkentseu {
	...
	bool NkVulkanContext::Initialize(const NkWindow &window, const NkContextDesc &desc) {
		...
		if (!CreateLogicalDevice(vk))
			return false;
		if (!CreateSwapchain(surf.width, surf.height, vk))
			return false;
		if (!CreateRenderPass())
			return false;
		if (!CreateDepthResources(surf.width, surf.height))
			return false;
	...
	void NkVulkanContext::Shutdown() {
		...
		for (uint32 i = 0; i < kNkVkMaxFrames; ++i) {
			if (mData.imageAvailSem[i])
				vkDestroySemaphore(mData.device, mData.imageAvailSem[i], nullptr);
			if (mData.renderDoneSem[i])
				vkDestroySemaphore(mData.device, mData.renderDoneSem[i], nullptr);
			if (mData.inFlightFences[i])
				vkDestroyFence(mData.device, mData.inFlightFences[i], nullptr);
		}
		if (mData.commandPool)
			vkDestroyCommandPool(mData.device, mData.commandPool, nullptr);
		if (mData.computePool)
			vkDestroyCommandPool(mData.device, mData.computePool, nullptr);
		if (mData.device)
			vkDestroyDevice(mData.device, nullptr);
		if (mData.surface)
			vkDestroySurfaceKHR(mData.instance, mData.surface, nullptr);
		if (mData.debugMessenger) {
			auto fn = (PFN_vkDestroyDebugUtilsMessengerEXT)vkGetInstanceProcAddr(mData.instance,
	...
	void NkVulkanContext::DestroySwapchainDependents() {
		// Depth
		if (mData.depthView)
			vkDestroyImageView(mData.device, mData.depthView, nullptr);
		if (mData.depthImage)
			vkDestroyImage(mData.device, mData.depthImage, nullptr);
		if (mData.depthMemory)
			vkFreeMemory(mData.device, mData.depthMemory, nullptr);
	...
	bool NkVulkanContext::RecreateSwapchain(uint32 w, uint32 h) {
		...
		if (!CreateSwapchain(w, h, mDesc.vulkan)) {
			mIsValid = false;
			return false;
		}
		if (!CreateDepthResources(w, h)) {
			mIsValid = false;
			return false;
		}
		if (!CreateFramebuffers()) {
	...
	bool NkVulkanContext::BeginFrame() {
		...
		VkResult r = vkAcquireNextImageKHR(mData.device, mData.swapchain, UINT64_MAX, mData.imageAvailSem[frame],
										   VK_NULL_HANDLE, &mData.currentImageIndex);
	...
	void NkVulkanContext::SetVSync(bool enabled) {
		mVSync = enabled; /* nécessite RecreateSwapchain */
	}
	...
	bool NkVulkanContext::SupportsCompute() const {
		return mData.computeFamily != UINT32_MAX;
	}
		...
		NkVector<const char *> extensions;
		{
			...
			for (uint32 i = 0; i < (uint32)desiredExt.Size(); ++i) {
				...
				for (uint32 j = 0; j < availCount; ++j) {
					if (strcmp(avail[j].extensionName, want) == 0) {
						found = true;
						break;
					}
				}
	...
	// Debug Messenger
	// =============================================================================
	VKAPI_ATTR VkBool32 VKAPI_CALL NkVulkanContext::DebugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT severity,
	...
	bool NkVulkanContext::CreateDebugMessenger(const NkVulkanDesc &) {
		...
		auto fn =
			(PFN_vkCreateDebugUtilsMessengerEXT)vkGetInstanceProcAddr(mData.instance, "vkCreateDebugUtilsMessengerEXT");
		if (!fn) {
			NK_VK_ERR("vkCreateDebugUtilsMessengerEXT not found\n");
			return false;
		}
	...
	bool NkVulkanContext::SelectPhysicalDevice(const NkVulkanDesc &d) {
		...
		const uint32 requestedIndex =
			(mDesc.gpu.adapterIndex >= 0) ? static_cast<uint32>(mDesc.gpu.adapterIndex) : d.preferredAdapterIndex;
			...
			switch (mDesc.gpu.preference) {
				case NkGpuPreference::NK_HIGH_PERFORMANCE:
					...
					if (c.props.deviceType == VK_PHYSICAL_DEVICE_TYPE_VIRTUAL_GPU)
						return 120;
					if (c.props.deviceType == VK_PHYSICAL_DEVICE_TYPE_CPU)
						return 60;
					return 100;
		...
		if (!selected) {
			...
			for (uint32 i = 0; i < candidates.Size(); ++i) {
				...
				if (strictVendor && !matchesVendor(c))
					continue;
		...
		NK_VK_LOG("GPU #%u: %s | vendor=0x%04X | VRAM: %u MB\n", selected->index, mData.rendererName,
				  (unsigned)selected->props.vendorID, mData.vramMB);
		return true;
	}
...
namespace nkentseu {
	...
	bool NkVulkanContext::Initialize(const NkWindow &, const NkContextDesc &desc) {
		...
		mIsValid = false;
		logger.Errorf(
			"[NkVulkan] Vulkan headers not found at build time. Install Vulkan SDK/headers to enable this backend.");
		return false;
	}

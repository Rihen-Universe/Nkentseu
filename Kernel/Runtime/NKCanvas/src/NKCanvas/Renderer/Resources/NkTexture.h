	...
	namespace renderer {
		...
		class NkTexture {
			public:
				...
				bool Update(const uint8 *pixels, uint32 width, uint32 height, uint32 destX, uint32 destY);
				...
				NkTextureFilter GetFilter() const {
					return mFilter;
				}

				NkTextureWrap GetWrap() const {
					return mWrap;
				}
				...
				bool IsValid() const {
					return mHandle != nullptr || mGPUId != 0;
				}

				NkVec2f GetSize() const {
				...
				const uint8 *GetCPUPixels() const {
					return mCPUPixels.Empty() ? nullptr : mCPUPixels.Data();
				}

				bool HasCPUPixels() const {
					return !mCPUPixels.Empty();
				}

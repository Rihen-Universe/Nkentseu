...
namespace nkentseu {
	namespace renderer {
		...
		NkTexture::NkTexture(NkTexture &&other) noexcept
			: mWidth(other.mWidth), mHeight(other.mHeight), mHandle(other.mHandle), mGPUId(other.mGPUId),
			  mFilter(other.mFilter), mWrap(other.mWrap), mRenderer(other.mRenderer),
		...
		// =============================================================================
		bool NkTexture::LoadFromImage(NkIRenderer2D &renderer, const NkImage &image, const NkRect2i &area) {
			if (!image.IsValid())
				return false;
			mRenderer = &renderer;
		...
		bool NkTexture::LoadFromMemory(NkIRenderer2D &renderer, const void *data, usize sizeBytes) {
			NkImage img;
			if (!img.LoadFromMemory(data, sizeBytes))
				return false;
			return LoadFromImage(renderer, img);
		}

		// =============================================================================
		bool NkTexture::Update(const uint8 *pixels, uint32 width, uint32 height, uint32 destX, uint32 destY) {
			if (!IsValid() || !pixels || !gTextureBackend.Update)
				return false;

			if (!mCPUPixels.Empty() && destX == 0 && destY == 0 && width == mWidth && height == mHeight) {

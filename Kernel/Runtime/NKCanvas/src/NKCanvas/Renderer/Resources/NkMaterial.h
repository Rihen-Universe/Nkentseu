	...
	namespace renderer {
		...
		class NkMaterial {
			public:
				...
				// ── Shader rattache ────────────────────────────────────────────
				void SetShader(NkShader *s) noexcept {
					mShader = s;
				}

				NkShader *GetShader() noexcept {
					return mShader;
				}

				const NkShader *GetShader() const noexcept {
					return mShader;
				}

				// ── Etats GPU par defaut ───────────────────────────────────────
				void SetBlendMode(NkBlendMode m) noexcept {
					mBlendMode = m;
				}

				NkBlendMode GetBlendMode() const noexcept {
					return mBlendMode;
				}

				void SetMainTexture(const NkTexture *tex) noexcept {
					mMainTexture = tex;
				}

				const NkTexture *GetMainTexture() const noexcept {
					return mMainTexture;
				}

				// ── Uniformes scalaires/vectoriels (cumules, Apply() les pose) ─
				void SetFloat(const char *name, float32 v) {
					Push(Uniform::Float, name, &v, 1);
				}

				void SetVec2(const char *name, float32 x, float32 y) {
					float32 d[2]{x, y};
					Push(Uniform::Vec2, name, d, 2);
				}

				void SetVec3(const char *name, float32 x, float32 y, float32 z) {
					float32 d[3]{x, y, z};
					Push(Uniform::Vec3, name, d, 3);
				}

				void SetVec4(const char *name, float32 x, float32 y, float32 z, float32 w) {
					float32 d[4]{x, y, z, w};
					Push(Uniform::Vec4, name, d, 4);
				}
				...
				void SetMat4(const char *name, const float32 *mat16) {
					Push(Uniform::Mat4, name, mat16, 16);
				}
				...
				void Apply() const {
					...
					for (const auto &u : mUniforms) {
						...
						switch (u.kind) {
							case Uniform::Float:
								mShader->SetFloat(n, u.data[0]);
								break;
							case Uniform::Vec2:
								mShader->SetVec2(n, NkVec2f{u.data[0], u.data[1]});
								break;
							case Uniform::Vec3:
								mShader->SetVec3(n, u.data[0], u.data[1], u.data[2]);
								break;
							case Uniform::Vec4:
								mShader->SetVec4(n, u.data[0], u.data[1], u.data[2], u.data[3]);
								break;
							case Uniform::Mat4:
								mShader->SetMat4(n, u.data);
								break;
						}
					}

...
namespace nkentseu {
	namespace renderer {
		...
		NkShader::NkShader(NkShader &&o) noexcept
			: mGLSLVert(o.mGLSLVert), mGLSLFrag(o.mGLSLFrag), mHLSLVert(o.mHLSLVert), mHLSLFrag(o.mHLSLFrag),
			  mMSLVert(o.mMSLVert), mMSLFrag(o.mMSLFrag), mSPIRVVert(o.mSPIRVVert), mSPIRVVertWords(o.mSPIRVVertWords),
			  mSPIRVFrag(o.mSPIRVFrag), mSPIRVFragWords(o.mSPIRVFragWords), mGPUId(o.mGPUId) {
			o.mGPUId = 0;
		}
		...
		NkShader &NkShader::operator=(NkShader &&o) noexcept {
			if (this != &o) {
				Destroy();
				mGLSLVert = o.mGLSLVert;
				mGLSLFrag = o.mGLSLFrag;
				mHLSLVert = o.mHLSLVert;
				mHLSLFrag = o.mHLSLFrag;
				mMSLVert = o.mMSLVert;
				mMSLFrag = o.mMSLFrag;
				mSPIRVVert = o.mSPIRVVert;
				mSPIRVVertWords = o.mSPIRVVertWords;
				mSPIRVFrag = o.mSPIRVFrag;
				mSPIRVFragWords = o.mSPIRVFragWords;
				mGPUId = o.mGPUId;
				o.mGPUId = 0;
			}
			return *this;
		...
		void NkShader::SetSourceGLSL(const char *v, const char *f) {
			mGLSLVert = v;
			mGLSLFrag = f;
		}

		void NkShader::SetSourceHLSL(const char *v, const char *f) {
			mHLSLVert = v;
			mHLSLFrag = f;
		}

		void NkShader::SetSourceMSL(const char *v, const char *f) {
			mMSLVert = v;
			mMSLFrag = f;
		}
		...
		bool NkShader::Compile(NkIRenderer2D & /*renderer*/) {
			if (!gShaderBackend.Create)
				return false; // backend pas encore initialise
			Destroy();		  // libere precedent eventuel
			NkShaderSources src{};
			src.glslVert = mGLSLVert;
			src.glslFrag = mGLSLFrag;
			src.hlslVert = mHLSLVert;
			src.hlslFrag = mHLSLFrag;
			src.mslVert = mMSLVert;
			src.mslFrag = mMSLFrag;
			src.spirvVert = mSPIRVVert;
			src.spirvVertWords = mSPIRVVertWords;
			src.spirvFrag = mSPIRVFrag;
			src.spirvFragWords = mSPIRVFragWords;
		...
		void NkShader::SetFloat(const char *name, float32 v) {
			if (mGPUId && gShaderBackend.SetFloat)
				gShaderBackend.SetFloat(mGPUId, name, v);
		}

		void NkShader::SetVec2(const char *name, NkVec2f v) {
			if (mGPUId && gShaderBackend.SetVec2)
				gShaderBackend.SetVec2(mGPUId, name, v.x, v.y);
		}

		void NkShader::SetVec3(const char *name, float32 x, float32 y, float32 z) {
			if (mGPUId && gShaderBackend.SetVec3)
				gShaderBackend.SetVec3(mGPUId, name, x, y, z);
		}

		void NkShader::SetVec4(const char *name, float32 x, float32 y, float32 z, float32 w) {
			if (mGPUId && gShaderBackend.SetVec4)
				gShaderBackend.SetVec4(mGPUId, name, x, y, z, w);
		}
		...
		void NkShader::SetMat4(const char *name, const float32 *mat16) {
			if (mGPUId && gShaderBackend.SetMat4)
				gShaderBackend.SetMat4(mGPUId, name, mat16);
		}

		void NkShader::SetTexture(const char *name, const NkTexture *tex, uint32 slot) {
			if (!mGPUId || !gShaderBackend.SetTexture)
				return;

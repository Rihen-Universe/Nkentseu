...
namespace nkentseu {
	namespace renderer {
		...
		NkFont::NkFont(NkFont &&o) noexcept
			: mFontData(std::move(o.mFontData)), mPages(std::move(o.mPages)), mRenderer(o.mRenderer),
		...
		bool NkFont::LoadFromFile(NkIRenderer2D &renderer, const char *path) {
			...
			if (data.Empty()) {
				NK_FONT_ERR("LoadFromFile: ouverture/lecture impossible : %s", path);
				return false;
			}
		...
		NkFont::Page *NkFont::GetOrCreatePage(uint32 characterSize) const {
			...
			page->atlas = static_cast<::nkentseu::NkFontAtlas *>(memory::NkAlloc(sizeof(::nkentseu::NkFontAtlas)));
			if (!page->atlas) {
				page->~Page();
				memory::NkFree(page);
				return nullptr;
		...
		const NkGlyph &NkFont::GetGlyph(uint32 codepoint, uint32 characterSize, bool /*bold*/) const {
			...
			if (!page || !page->moduleFont || !page->atlas)
				return mScratchGlyph;

			const ::nkentseu::NkFontGlyph *g = page->moduleFont->FindGlyph((NkFontCodepoint)codepoint);
			if (!g)
				return mScratchGlyph;

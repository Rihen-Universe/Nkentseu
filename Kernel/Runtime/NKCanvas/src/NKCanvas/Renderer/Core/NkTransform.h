	...
	namespace renderer {
		...
		class NkTransform {
			public:
				...
				NkTransform(float32 a00, float32 a01, float32 a02, float32 a10, float32 a11, float32 a12) noexcept
					: mMatrix(1.f, true) {
					mMatrix.data[0] = a00;
					mMatrix.data[4] = a01;
					mMatrix.data[12] = a02;
				...
				const float32 *GetMatrix() const noexcept {
					return mMatrix.data;
				}

				/// Acces direct a la math::NkMat4f sous-jacente (interop NKRenderer/NKCamera).
				const math::NkMat4f &GetMatrix4() const noexcept {
					return mMatrix;
				}

				math::NkMat4f &GetMatrix4() noexcept {
					return mMatrix;
				}
				...
				NkTransform &Scale(NkVec2f factors) noexcept {
					mMatrix.data[0] *= factors.x;
					mMatrix.data[4] *= factors.y;
					mMatrix.data[1] *= factors.x;
					mMatrix.data[5] *= factors.y;
					return *this;
				}
				...
				NkVec2f TransformPoint(NkVec2f point) const noexcept {
					return NkVec2f(mMatrix.data[0] * point.x + mMatrix.data[4] * point.y + mMatrix.data[12],
				...
				NkRect2f TransformRect(const NkRect2f &rect) const noexcept {
					const NkVec2f corners[4] = {TransformPoint({rect.left, rect.top}),
				...
				NkTransform GetInverse() const noexcept {
					...
					return NkTransform(d * invDet, -b * invDet, -(d * tx - b * ty) * invDet, -c * invDet, a * invDet,
									   -(-c * tx + a * ty) * invDet);
				}
				...
				bool operator==(const NkTransform &other) const noexcept {
					for (int32 i = 0; i < 16; ++i)
						if (mMatrix.data[i] != other.mMatrix.data[i])
							return false;
					return true;
				}

		...
		class NkLineShape : public NkShape {
			public:
				...
				NkLineShape(NkVec2f a, NkVec2f b, float32 thickness = 1.f) noexcept
					: mA(a), mB(b), mThickness(thickness) {
					Recompute();
				}

				void SetA(NkVec2f a) noexcept {
					mA = a;
					Recompute();
				}

				void SetB(NkVec2f b) noexcept {
					mB = b;
					Recompute();
				}

				void SetEndpoints(NkVec2f a, NkVec2f b) noexcept {
					mA = a;
					mB = b;
					Recompute();
				}

				void SetThickness(float32 t) noexcept {
					mThickness = t;
					Recompute();
				}

				NkVec2f GetA() const noexcept {
					return mA;
				}

				NkVec2f GetB() const noexcept {
					return mB;
				}

				float32 GetThickness() const noexcept {
					return mThickness;
				}

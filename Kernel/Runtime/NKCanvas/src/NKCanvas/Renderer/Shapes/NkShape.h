	...
	namespace renderer {
		...
		class NkShape : public NkTransformable, public NkDrawable {
			public:
				...
				// ── Couleurs / contour ─────────────────────────────────────────
				void SetFillColor(const NkColor2D &c) noexcept {
					mFillColor = c;
				}

				NkColor2D GetFillColor() const noexcept {
					return mFillColor;
				}

				void SetOutlineColor(const NkColor2D &c) noexcept {
					mOutlineColor = c;
				}

				NkColor2D GetOutlineColor() const noexcept {
					return mOutlineColor;
				}

				void SetOutlineThickness(float32 t) noexcept {
					mOutlineThickness = t;
				}

				float32 GetOutlineThickness() const noexcept {
					return mOutlineThickness;
				}

				// ── Texture ────────────────────────────────────────────────────
				void SetTexture(const NkTexture *tex) noexcept {
					mTexture = tex;
				}

				const NkTexture *GetTexture() const noexcept {
					return mTexture;
				}

				void SetTextureRect(const NkRect2f &r) noexcept {
					mTextureRect = r;
					mHasTextureRect = true;
				}

				NkRect2f GetTextureRect() const noexcept {
					return mTextureRect;
				}
				...
				virtual NkVec2f GetPointUV(uint32 index, const NkRect2f &localBounds) const noexcept {
					...
					const float32 invW = localBounds.width > 0.f ? 1.f / localBounds.width : 0.f;
					const float32 invH = localBounds.height > 0.f ? 1.f / localBounds.height : 0.f;
					return NkVec2f((p.x - localBounds.left) * invW, (p.y - localBounds.top) * invH);
				}

...
namespace nkentseu {
	namespace renderer {
		...
		bool NkUICanvasBackend::UploadTextureRGBA8(uint32 texId, const uint8 *data, int32 width, int32 height) {
			if (!mRenderer || !data || width <= 0 || height <= 0 || texId == 0)
				return false;
		...
		void NkUICanvasBackend::UploadGray8Trampoline(uint32 texId, const uint8 *data, int32 w, int32 h) {
			if (s_uploadSelf)
				s_uploadSelf->UploadTextureGray8(texId, data, w, h);
		}
		...
		void NkUICanvasBackend::Submit(const nkui::NkUIContext &ctx, uint32 fbW, uint32 fbH) {
			...
			for (int32 layer = 0; layer < nkui::NkUIContext::LAYER_COUNT; ++layer) {
				...
				if (dl.cmdCount == 0u || dl.vtxCount == 0u || dl.idxCount == 0u)
					continue;
				...
				for (uint32 ci = 0; ci < dl.cmdCount; ++ci) {
					...
					if (dc.idxCount == 0u)
						continue;
					if (dc.type == nkui::NkUIDrawCmdType::NK_CLIP_RECT || dc.type == nkui::NkUIDrawCmdType::NK_SET_FONT)
						continue; // non geometriques
					...
					if (hasClip) {
						...
						if (x1 <= x0 || y1 <= y0)
							continue; // clip vide -> rien a dessiner
					...
					mRenderer->DrawVertices(mScratch.Data(), dl.vtxCount, dl.idx + dc.idxOffset, dc.idxCount, tex);

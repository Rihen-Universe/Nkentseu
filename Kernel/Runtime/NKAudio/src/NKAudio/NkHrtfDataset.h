	...
	namespace audio {
		...
		class NKENTSEU_AUDIO_API NkHrtfDataset {
			public:
				 ...
				 */
				bool LoadFromFile(const char *path, memory::NkAllocator *allocator = nullptr) noexcept;
				 ...
				 * @return true si succes.
				 */
				bool CreateSynthetic(int32 sampleRate, int32 irLength, int32 nAzimuths, int32 nElevations,
				...
				int32 GetSampleRate() const noexcept {
					return mSampleRate;
				}

				int32 GetIrLength() const noexcept {
					return mIrLength;
				}

				int32 GetAzimuthCount() const noexcept {
					return mNumAzimuths;
				}

				int32 GetElevationCount() const noexcept {
					return mNumElevations;
				}

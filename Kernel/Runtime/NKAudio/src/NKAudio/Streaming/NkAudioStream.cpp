...
namespace nkentseu {
	namespace audio {
			...
			while (true) {
				...
				if (chunkHdr[0] == 'f' && chunkHdr[1] == 'm' && chunkHdr[2] == 't' && chunkHdr[3] == ' ') {
					if (chunkSize < 16 || chunkSize > 64)
						goto fail;
					uint8 fmt[64];
					if (mFile.Read(fmt, chunkSize) != chunkSize)
						goto fail;
					...
					mCurFrame = 0;
					mEOF = false;
					logger.Info("[WavStream] Ouvert : {0} frames, {1} Hz, {2} ch, {3} bps", mFrameCount, mSampleRate,
								mChannels, mBitsPerSamp);
					return true;
				} else {
		...
		MemoryStream::MemoryStream(AudioSample sample) noexcept : mSample(sample), mCurFrame(0) {
		}
		...
		int32 MemoryStream::ReadFrames(float32 *outBuf, int32 maxFrames) noexcept {
			if (!mSample.data || maxFrames <= 0)
				return 0;
			const nk_int64 remaining = nk_int64(mSample.frameCount) - mCurFrame;
			if (remaining <= 0)
				return 0;
			const int32 nFrames = (nk_int64(maxFrames) > remaining) ? int32(remaining) : maxFrames;
		...
		bool MemoryStream::Seek(nk_int64 frameIdx) noexcept {
			...
			if (frameIdx < 0)
				frameIdx = 0;
			if (frameIdx > nk_int64(mSample.frameCount))
				frameIdx = nk_int64(mSample.frameCount);
			mCurFrame = frameIdx;
			return true;
		}
		...
		IAudioStream *OpenAudioStream(const char *path) noexcept {
			...
			} else if ((ext[0] == 'm' || ext[0] == 'M') && (ext[1] == 'p' || ext[1] == 'P') && ext[2] == '3' &&
					   ext[3] == 0) {

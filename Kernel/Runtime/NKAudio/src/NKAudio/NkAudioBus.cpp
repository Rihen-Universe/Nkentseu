...
namespace nkentseu {
	namespace audio {
		...
		NkAudioBus::NkAudioBus(const char *name, NkAudioBus *parent) noexcept : mParent(parent), mActiveVoiceCount(0) {
		...
		NkAudioBus *NkAudioBus::GetChild(int32 idx) const noexcept {
			if (idx < 0 || idx >= mChildCount)
				return nullptr;
			return mChildren[idx];
		}

		bool NkAudioBus::AttachChild(NkAudioBus *child) noexcept {
			if (!child || mChildCount >= MAX_CHILDREN)
				return false;
		...
		void NkAudioBus::DecrementActiveVoices() noexcept {
			...
			while (cur > 0) {
				if (mActiveVoiceCount.CompareExchangeWeak(cur, cur - 1))
					break;
			}
		}
		...
		bool NkAudioBus::AddEffect(IAudioEffect *effect) noexcept {
			if (!effect || mEffectCount >= MAX_EFFECTS)
				return false;
		...
		void NkAudioBus::SetSidechainFromBus(NkAudioBus *sourceBus, float32 amount, float32 threshold) noexcept {
		...
		void NkAudioBus::ProcessChain(float32 *outBuf, int32 frames, int32 channels) noexcept {
			...
			for (int32 i = 0; i < mEffectCount; ++i) {
				...
				if (fx)
					fx->Process(outBuf, frames, channels);
			}

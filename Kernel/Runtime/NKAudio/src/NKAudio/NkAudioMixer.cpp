...
namespace nkentseu {
	namespace audio {
		...
		AudioMixer::AudioMixer(int32 sampleRate, int32 channels, memory::NkAllocator *allocator)
			: mSampleRate(sampleRate), mChannels(channels), mAllocator(allocator) {
		}
		...
		AudioSample AudioMixer::AllocateSample(usize frameCount) const {
			...
			if (s.data)
				memset(s.data, 0, total * sizeof(float32));
			return s;
		}
		...
		AudioSample AudioMixer::Crossfade(const AudioSample &a, const AudioSample &b, float32 crossfadeLen) {
			AudioSample result{};
			...
			if (totalOut < a.frameCount)
				totalOut = a.frameCount;
		...
		AudioSample AudioMixer::Concatenate(const AudioSample *samples, int32 count) {
			...
			for (int32 i = 0; i < count; ++i)
				if (samples[i].IsValid())
					totalFrames += samples[i].frameCount;
		...
		void AudioMixer::FadeIn(AudioSample &sample, float32 duration) {
			...
			if (fadeFrames > sample.frameCount)
				fadeFrames = sample.frameCount;
			...
			for (usize f = 0; f < fadeFrames; ++f) {
				...
				for (int32 c = 0; c < ch; ++c)
					sample.data[f * (usize)ch + (usize)c] *= gain;
			}
		}
		...
		void AudioMixer::FadeOut(AudioSample &sample, float32 duration) {
			...
			for (usize f = 0; f < fadeFrames; ++f) {
				...
				for (int32 c = 0; c < ch; ++c)
					sample.data[idx * (usize)ch + (usize)c] *= gain;
			}
		}
		...
		void AudioMixer::Insert(AudioSample &target, const AudioSample &insert, float32 positionSec, bool mix) {
			if (!target.IsValid() || !insert.IsValid())
				return;

			usize startFrame = (usize)(positionSec * (float32)target.sampleRate);
			if (startFrame >= target.frameCount)
				return;

			int32 tCh = target.channels;
			int32 iCh = insert.channels;
			usize iEnd = startFrame + insert.frameCount;
			if (iEnd > target.frameCount)
				iEnd = target.frameCount;
			...
			for (usize f = startFrame; f < iEnd; ++f) {
				...
				for (int32 c = 0; c < tCh; ++c) {
					...
					if (mix)
						target.data[f * (usize)tCh + (usize)c] += src;
					else
						target.data[f * (usize)tCh + (usize)c] = src;
				}
			}

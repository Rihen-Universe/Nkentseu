...
namespace {
	...
	inline nkentseu::float32 *AllocFloat(nkentseu::usize count, nkentseu::memory::NkAllocator *alloc) {
		...
		return (nkentseu::float32 *)nkentseu::memory::NkAlloc(count * sizeof(nkentseu::float32), alloc,
	...
	inline nkentseu::float32 NkSin(nkentseu::float32 x) {
		return ::sinf(x);
	}

	inline nkentseu::float32 NkCos(nkentseu::float32 x) {
		return ::cosf(x);
	}

	inline nkentseu::float32 NkFmod(nkentseu::float32 x, nkentseu::float32 y) {
		return ::fmodf(x, y);
	}

	inline nkentseu::float32 NkPow(nkentseu::float32 b, nkentseu::float32 e) {
		return ::powf(b, e);
	}

	inline nkentseu::float32 NkExp(nkentseu::float32 x) {
		return ::expf(x);
	}

	inline nkentseu::float32 NkLog(nkentseu::float32 x) {
		return ::logf(x);
	}

	inline nkentseu::float32 NkSqrt(nkentseu::float32 x) {
		return ::sqrtf(x);
	}

	inline nkentseu::float32 NkFabs(nkentseu::float32 x) {
		return ::fabsf(x);
	}
...
namespace nkentseu {
	namespace audio {
		...
		float32 AudioGenerator::EvaluateWaveform(WaveformType waveform, float32 phase, float32 pulseWidth) {
		...
		float32 AudioGenerator::GeneratePinkNoiseSample(PinkNoiseState &state) {
			...
			static const float32 A[7] = {0.99886f, 0.99332f, 0.96900f, 0.86650f, 0.55000f, -0.7616f, 0.115926f};
			static const float32 B[7] = {0.0555179f, 0.0750759f,  0.1538520f, 0.3104856f,
		...
		AudioSample AudioGenerator::GenerateTone(float32 frequency, float32 duration, WaveformType waveform,
												 int32 sampleRate, float32 amplitude, memory::NkAllocator *allocator) {
			AudioSample result{};
			if (frequency <= 0.0f || duration <= 0.0f || sampleRate <= 0)
				return result;
		...
		AudioSample AudioGenerator::GenerateNoise(float32 duration, WaveformType type, int32 sampleRate,
												  float32 amplitude, memory::NkAllocator *allocator) {
		...
		AudioSample AudioGenerator::GenerateChirp(float32 startFreq, float32 endFreq, float32 duration,
												  int32 sampleRate, memory::NkAllocator *allocator) {
			AudioSample result{};
			if (duration <= 0.0f || sampleRate <= 0)
				return result;
		...
		AudioSample AudioGenerator::GenerateChord(const float32 *frequencies, int32 count, float32 duration,
												  const float32 *amplitudes, int32 sampleRate,
												  memory::NkAllocator *allocator) {
			AudioSample result{};
			if (!frequencies || count <= 0 || duration <= 0.0f || sampleRate <= 0)
				return result;
		...
		void AudioGenerator::ApplyEnvelope(AudioSample &sample, const AdsrEnvelope &env) {
			...
			for (usize f = 0; f < frames; ++f) {
				...
				if (f < attackSamples) {
					...
					float32 t = (decaySamples > 0) ? (float32)(f - attackSamples) / (float32)decaySamples : 0.0f;
					...
					float32 t = (releaseSamples > 0) ? (float32)(f - releaseStart) / (float32)releaseSamples : 1.0f;
		...
		void AudioGenerator::ApplyLfo(AudioSample &sample, float32 rate, float32 depth, WaveformType waveform) {
			if (!sample.IsValid() || rate <= 0.0f)
				return;
		...
		AudioSample AudioGenerator::GenerateKick(float32 duration, float32 startFreq, float32 endFreq,
												 float32 clickAttack, int32 sampleRate,
												 memory::NkAllocator *allocator) {
			AudioSample result{};
			if (duration <= 0.0f || sampleRate <= 0)
				return result;
		...
		AudioSample AudioGenerator::GenerateSnare(float32 duration, float32 toneFreq, float32 noiseMix,
												  int32 sampleRate, memory::NkAllocator *allocator) {
			AudioSample result{};
			if (duration <= 0.0f || sampleRate <= 0)
				return result;
		...
		AudioSample AudioGenerator::GenerateHihat(float32 duration, bool closed, int32 sampleRate,
												  memory::NkAllocator *allocator) {
			AudioSample result{};
			if (duration <= 0.0f || sampleRate <= 0)
				return result;

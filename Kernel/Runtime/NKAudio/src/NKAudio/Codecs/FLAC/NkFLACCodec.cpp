...
namespace nkentseu {
	namespace audio {
		...
		struct FLACBitReader {
				...
				NKENTSEU_INLINE uint32 ReadUnary() noexcept {
					...
					while (true) {
						...
						++q;
						if (q > 0x10000)
							return uint32(-1); // garde-fou anti-boucle infinie
					}
				}
		...
		static NKENTSEU_INLINE int64 FixedPredict(int32 order, const int32 *hist) noexcept {
			...
			switch (order) {
				...
				case 1:
					return int64(hist[0]);
				case 2:
					return int64(hist[0]) * 2 - int64(hist[1]);
				case 3:
					return int64(hist[0]) * 3 - int64(hist[1]) * 3 + int64(hist[2]);
				case 4:
					return int64(hist[0]) * 4 - int64(hist[1]) * 6 + int64(hist[2]) * 4 - int64(hist[3]);
				default:
					return 0;
			}
		}
		...
		static bool DecodeResidual(FLACBitReader &br, int32 blockSize, int32 predOrder, int32 *outResiduals) noexcept {
		...
		static bool DecodeSubframe(FLACBitReader &br, int32 bps, int32 blockSize, int32 *outSamples) noexcept {
			// Header subframe
			if (br.Read(1) != 0)
				return false; // bit reserve
			...
			if (subType == 0) {
				...
				if (!DecodeResidual(br, blockSize, order, res))
					return false;
				...
				if (qPrec == 0x0F)
					return false; // valeur invalide = reservee
				qPrec += 1;
		...
		static int32 DecodeBlockSize(FLACBitReader &br, uint32 code) noexcept {
			switch (code) {
				...
				case 0x5:
					return 4608;
				case 0x6:
					return int32(br.Read(8)) + 1; // 8-bit follows
				case 0x7:
					return int32(br.Read(16)) + 1; // 16-bit follows
				case 0x8:
					return 256;
				case 0x9:
					return 512;
		...
		static int32 DecodeSampleRate(FLACBitReader &br, uint32 code, int32 streamRate) noexcept {
			switch (code) {
				case 0x0:
					return streamRate;
				case 0x1:
				...
				case 0xB:
					return 96000;
				case 0xC:
					return int32(br.Read(8)) * 1000; // kHz
				case 0xD:
					return int32(br.Read(16)); // Hz
				case 0xE:
					return int32(br.Read(16)) * 10; // tens of Hz
				default:
					return -1; // 0xF = reserve
			}
		}
		...
		static int32 DecodeFrame(FLACBitReader &br, const StreamInfo &info, int32 **channelSamples, int32 maxBlockSize,
			...
			for (int32 c = 0; c < nCh; ++c) {
				...
				int32 subBps = bps;
				if ((outChannelAssign == 8 && c == 1) || (outChannelAssign == 9 && c == 0) ||
		...
		static void DecorrelateStereo(int32 channelAssign, int32 n, int32 *ch0, int32 *ch1) noexcept {
			if (channelAssign == 8) {
				// L/S : L = ch0, R = ch0 - ch1
				for (int32 i = 0; i < n; ++i)
					ch1[i] = ch0[i] - ch1[i];
		...
		AudioSample NkFLACCodec::Decode(const uint8 *data, usize size, memory::NkAllocator *allocator) noexcept {
			AudioSample empty{};
			if (!data || size < 4 + 38)
				return empty;
			...
			uint32 sr20 = (uint32(si[10]) << 12) | (uint32(si[11]) << 4) | (uint32(si[12]) >> 4);
			...
			uint64 totalLo = ((uint64(si[13]) & 0x0F) << 32) | (uint64(si[14]) << 24) | (uint64(si[15]) << 16) |
			...
			if (sr20 == 0 || nch == 0 || nch > 8 || bps < 4 || bps > 32) {
				logger.Error("[FLAC] STREAMINFO invalide : sr={0} ch={1} bps={2}", sr20, nch, bps);
				return empty;
			}
			...
			if (estFrames == 0)
				estFrames = uint64(maxBlk) * 16;
			usize estSamples = usize(estFrames) * usize(nch);
			float32 *outData =
				static_cast<float32 *>(memory::NkAlloc(estSamples * sizeof(float32), allocator, sizeof(float32)));
			if (!outData) {
			...
			while (true) {
				...
				if (blockN <= 0) {
					logger.Warn("[FLAC] Frame invalide a frame={0}, on arrete.", totalDecoded);
					break;
				}
				...
				if (totalDecoded + uint64(blockN) > capFrames) {
					...
					while (newCap < totalDecoded + uint64(blockN))
						newCap *= 2;
					usize newSamp = usize(newCap) * usize(nch);
					float32 *grown =
						static_cast<float32 *>(memory::NkAlloc(newSamp * sizeof(float32), allocator, sizeof(float32)));
					if (!grown) {
						memory::NkFree(outData, allocator);
			...
			logger.Info("[FLAC] Decode OK : {0} frames, {1} canaux, {2} Hz, {3} bps.", totalDecoded, info.channels,
						info.sampleRate, info.bps);
			return result;
		}

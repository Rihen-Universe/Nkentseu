...
namespace nkentseu {
	namespace audio {
		...
		usize NkHrtfDataset::IndexOf(int32 azIdx, int32 elevIdx) const noexcept {
			...
			return (usize(elevIdx) * usize(mNumAzimuths) + usize(azIdx)) * usize(2) * usize(mIrLength);
		}

		bool NkHrtfDataset::LoadFromFile(const char *path, memory::NkAllocator *allocator) noexcept {
			Unload();
			if (!path)
				return false;
			...
			if (header[1] != NKHR_VERSION) {
				logger.Error("[NkHrtfDataset] Version {0} non supportee (attend {1}).", header[1], NKHR_VERSION);
				f.Close();
				return false;
			}
			...
			if (mNumAzimuths <= 0 || mNumAzimuths > MAX_AZIMUTHS || mNumElevations <= 0 ||
				mNumElevations > MAX_ELEVATIONS || mIrLength <= 0 || mIrLength > MAX_IR_LENGTH) {
				logger.Error("[NkHrtfDataset] Dimensions invalides : {0}az x {1}el x {2}len.", mNumAzimuths,
							 mNumElevations, mIrLength);
				f.Close();
				return false;
			}

			// Allouer buffer total : nAz * nElev * 2 * irLen * float32
			usize total = usize(mNumAzimuths) * usize(mNumElevations) * usize(2) * usize(mIrLength) * sizeof(float32);
			mAllocator = allocator;
			mData = static_cast<float32 *>(memory::NkAlloc(total, allocator, sizeof(float32)));
			if (!mData) {
				logger.Error("[NkHrtfDataset] Allocation buffer ({0} bytes) echec.", (int)total);
				f.Close();
				return false;
			}
			mDataSize = total;
			...
			logger.Info("[NkHrtfDataset] Charge : {0}az x {1}el x {2} samples a {3} Hz "
						"({4} KB).",
						mNumAzimuths, mNumElevations, mIrLength, mSampleRate, (int)(total / 1024));
			return true;
		}
		...
		bool NkHrtfDataset::CreateSynthetic(int32 sampleRate, int32 irLength, int32 nAzimuths, int32 nElevations,
											memory::NkAllocator *allocator) noexcept {
			Unload();
			if (sampleRate <= 0 || irLength <= 0 || nAzimuths <= 0 || nElevations <= 0)
				return false;
			if (irLength > MAX_IR_LENGTH || nAzimuths > MAX_AZIMUTHS || nElevations > MAX_ELEVATIONS) {
			...
			usize total = usize(nAzimuths) * usize(nElevations) * usize(2) * usize(irLength) * sizeof(float32);
			mData = static_cast<float32 *>(memory::NkAlloc(total, allocator, sizeof(float32)));
			if (!mData) {
				logger.Error("[NkHrtfDataset] CreateSynthetic : alloc echec ({0} bytes).", (int)total);
				return false;
			}
			mDataSize = total;
			...
			if (maxItdSamples >= irLength / 2)
				maxItdSamples = (irLength / 2) - 1;
			...
			for (int32 ie = 0; ie < nElevations; ++ie) {
				...
				for (int32 ia = 0; ia < nAzimuths; ++ia) {
					...
					float32 itdSec = (kHeadRadius / kSpeedOfSound) * (::sinf(absAz) + absAz);
					if (absAz > kPi * 0.5f) {
						itdSec = (kHeadRadius / kSpeedOfSound) * (::sinf(kPi - absAz) + (kPi - absAz));
					}
		...
		NkHrirPair NkHrtfDataset::GetHRIR(float32 azimuthDeg, float32 elevationDeg) const noexcept {
			NkHrirPair result{};
			if (!mData || mNumAzimuths <= 0 || mNumElevations <= 0)
				return result;

			// Normalize azimut [0, 360)
			while (azimuthDeg < 0.0f)
				azimuthDeg += 360.0f;
			while (azimuthDeg >= 360.0f)
				azimuthDeg -= 360.0f;

			// Clamp elevation
			if (elevationDeg < mElevationMinDeg)
				elevationDeg = mElevationMinDeg;
			if (elevationDeg > mElevationMaxDeg)
				elevationDeg = mElevationMaxDeg;

			// Map azimut [0, 360) -> [0, nAz)
			float32 azNorm = azimuthDeg / 360.0f;
			int32 azIdx = int32(azNorm * float32(mNumAzimuths));
			if (azIdx >= mNumAzimuths)
				azIdx = mNumAzimuths - 1;
			if (azIdx < 0)
				azIdx = 0;

			// Map elevation [min, max] -> [0, nElev)
			float32 elNorm = (elevationDeg - mElevationMinDeg) / (mElevationMaxDeg - mElevationMinDeg);
			int32 elIdx = int32(elNorm * float32(mNumElevations));
			if (elIdx >= mNumElevations)
				elIdx = mNumElevations - 1;
			if (elIdx < 0)
				elIdx = 0;

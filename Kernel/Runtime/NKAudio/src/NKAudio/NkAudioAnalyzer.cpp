...
namespace {
	...
	inline nkentseu::float32 NkLog2f(nkentseu::float32 x) {
		return ::log2f(x);
	}
...
namespace nkentseu {
	namespace audio {
		...
		void AudioAnalyzer::ApplyWindow(float32 *buffer, int32 size, int32 windowType) {
			...
			switch (windowType) {
				...
				case 3: // Fenêtre de Blackman
					for (int32 i = 0; i < size; ++i) {
						float32 w = 0.42f - 0.50f * NkCosf(TWO_PI * (float32)i * invN) +
		...
		FftResult AudioAnalyzer::ComputeFft(const AudioSample &sample, int32 fftSize, int32 windowType,
											memory::NkAllocator *allocator) {
			FftResult result;
			if (!sample.IsValid() || fftSize <= 0)
				return result;
			...
			// Libérer temporaires
			if (allocator) {
				allocator->Deallocate(real);
				allocator->Deallocate(imag);
			} else {
				nkentseu::memory::NkFree(real);
				nkentseu::memory::NkFree(imag);
			}
		...
		void AudioAnalyzer::ComputeFftRealtime(const float32 *inputReal, float32 *outputMag, int32 fftSize) {
			if (!inputReal || !outputMag || fftSize <= 0)
				return;
		...
		void AudioAnalyzer::ComputeBandEnergies(const float32 *magnitudes, int32 fftSize, int32 sampleRate,
												float32 *bands, int32 bandCount) {
			if (!magnitudes || !bands || bandCount <= 0 || fftSize <= 0)
				return;
		...
		NkVector<FftResult> AudioAnalyzer::ComputeSpectrogram(const AudioSample &sample, int32 windowSize,
															  int32 hopSize, memory::NkAllocator *allocator) {
			NkVector<FftResult> result;
			if (!sample.IsValid() || windowSize <= 0 || hopSize <= 0)
				return result;
		...
		float32 AudioAnalyzer::YinPitchDetect(const float32 *data, int32 count, int32 sampleRate, float32 minFreq,
											  float32 maxFreq) {
			// Plage de lags
			...
			for (int32 tau = tauMin; tau < tauMax; ++tau) {
				...
				for (int32 t2 = 1; t2 <= tau; ++t2) {
					...
					runningSum += d2;
					if (runningSum > 0.0f)
						cmnd = diff * (float32)tau / runningSum;
				}
				...
				if (cmnd < threshold) {
					...
					if (tau > tauMin && tau < tauMax - 1) {
						...
						if (interp > 0.0f)
							return (float32)sampleRate / interp;
					}
		...
		float32 AudioAnalyzer::DetectPitch(const AudioSample &sample, float32 minFreq, float32 maxFreq) {
			if (!sample.IsValid())
				return 0.0f;
		...
		float32 AudioAnalyzer::DetectTempo(const AudioSample &sample, float32 minBpm, float32 maxBpm) {
			if (!sample.IsValid())
				return 0.0f;
			...
			if (lagMin < 1)
				lagMin = 1;
			if (lagMax > (int32)numBlocks / 2)
				lagMax = (int32)numBlocks / 2;

...
namespace {
	...
	inline nkentseu::float32 *AllocFloat(nkentseu::usize n, nkentseu::memory::NkAllocator *a) {
		return (nkentseu::float32 *)nkentseu::memory::NkAlloc(n * sizeof(nkentseu::float32), a,
															  sizeof(nkentseu::float32));
	}
	...
	inline nkentseu::float32 Tanh(nkentseu::float32 x) {
		return ::tanhf(x);
	}

	inline nkentseu::float32 Sinf(nkentseu::float32 x) {
		return ::sinf(x);
	}

	inline nkentseu::float32 Cosf(nkentseu::float32 x) {
		return ::cosf(x);
	}

	inline nkentseu::float32 Sqrtf(nkentseu::float32 x) {
		return ::sqrtf(x);
	}

	inline nkentseu::float32 Powf(nkentseu::float32 b, nkentseu::float32 e) {
		return ::powf(b, e);
	}

	inline nkentseu::float32 Log10f(nkentseu::float32 x) {
		return ::log10f(x);
	}

	inline nkentseu::float32 Expf(nkentseu::float32 x) {
		return ::expf(x);
	}

	inline nkentseu::float32 Fabsf(nkentseu::float32 x) {
		return ::fabsf(x);
	}
} // anonymous namespace
...
namespace nkentseu {
	namespace audio {
		...
		DelayEffect::DelayEffect(float32 delayTime, float32 feedback, float32 wet, int32 sampleRate,
								 memory::NkAllocator *allocator)
			: mDelayTime(delayTime), mFeedback(Clampf(feedback, 0.0f, 0.99f)), mSampleRate(sampleRate),
		...
		void DelayEffect::ReallocBuffer() {
			...
			mWritePos = 0;
			if (mBuffer)
				memset(mBuffer, 0, mBufferSize * sizeof(float32));
		}
		...
		void DelayEffect::Process(float32 *buffer, int32 frameCount, int32 channels) {
			if (!mEnabled || !mBuffer || mBufferSize == 0)
				return;

			usize delaySamples = (usize)(mDelayTime * (float32)mSampleRate);
			if (delaySamples >= mBufferSize / 2)
				delaySamples = mBufferSize / 2 - 1;
		...
		ReverbEffect::ReverbEffect(const Params &params, int32 sampleRate, memory::NkAllocator *allocator)
			: mDamping(params.damping), mRoomSize(params.roomSize), mPreDelay(params.preDelay), mSampleRate(sampleRate),
			  mAllocator(allocator) {
			mWetMix = params.wet;
		...
		void ReverbEffect::InitBuffers(int32 sampleRate) {
			...
			for (int32 i = 0; i < NUM_COMBS; ++i) {
				...
				usize sizeL = (usize)((float32)COMB_SIZES_MS[i] * srScale * 0.001f * (float32)sampleRate *
									  (0.5f + 0.5f * mRoomSize)) +
							  1;
				...
				mCombL[i].store = 0.0f;
				if (mCombL[i].buffer)
					memset(mCombL[i].buffer, 0, sizeL * sizeof(float32));
				...
				mCombR[i].store = 0.0f;
				if (mCombR[i].buffer)
					memset(mCombR[i].buffer, 0, sizeR * sizeof(float32));
			}
			...
			for (int32 i = 0; i < NUM_ALLPASS; ++i) {
				...
				mAllpassL[i].pos = 0;
				if (mAllpassL[i].buffer)
					memset(mAllpassL[i].buffer, 0, sizeL * sizeof(float32));

				mAllpassR[i].buffer = AllocFloat(sizeR, mAllocator);
				mAllpassR[i].size = sizeR;
				mAllpassR[i].pos = 0;
				if (mAllpassR[i].buffer)
					memset(mAllpassR[i].buffer, 0, sizeR * sizeof(float32));
			}
			...
			mPreDelayPos = 0;
			if (mPreDelayBuf)
				memset(mPreDelayBuf, 0, mPreDelaySize * 2 * sizeof(float32));
		}

		float32 ReverbEffect::ProcessComb(CombFilter &comb, float32 input) {
			if (!comb.buffer || comb.size == 0)
				return input;
		...
		void ReverbEffect::Reset() {
			for (int32 i = 0; i < NUM_COMBS; ++i) {
				if (mCombL[i].buffer)
					memset(mCombL[i].buffer, 0, mCombL[i].size * sizeof(float32));
				if (mCombR[i].buffer)
					memset(mCombR[i].buffer, 0, mCombR[i].size * sizeof(float32));
				mCombL[i].pos = mCombR[i].pos = 0;
				mCombL[i].store = mCombR[i].store = 0.0f;
			}
			for (int32 i = 0; i < NUM_ALLPASS; ++i) {
				if (mAllpassL[i].buffer)
					memset(mAllpassL[i].buffer, 0, mAllpassL[i].size * sizeof(float32));
				if (mAllpassR[i].buffer)
					memset(mAllpassR[i].buffer, 0, mAllpassR[i].size * sizeof(float32));
		...
		CompressorEffect::CompressorEffect(const Params &params, int32 sampleRate)
			: mParams(params), mSampleRate(sampleRate), mEnvelope(0.0f), mGainDb(0.0f), mGainReductionDb(0.0f) {
			ComputeCoeffs();
		}
		...
		LimiterEffect::LimiterEffect(const Params &params, int32 sampleRate)
			: mParams(params), mSampleRate(sampleRate), mEnvelope(0.0f), mGainReductionDb(0.0f), mAttackCoeff(0.0f),
			  mReleaseCoeff(0.0f), mThresholdLin(1.0f) {
			ComputeCoeffs();
		}
		...
		BiquadFilter::BiquadFilter(FilterType type, float32 cutoffHz, float32 qFactor, float32 gainDb, int32 sampleRate)
			: mType(type), mCutoffHz(cutoffHz), mQ(qFactor), mGainDb(gainDb), mSampleRate(sampleRate) {
			mZ1[0] = mZ1[1] = 0.0f;
			mZ2[0] = mZ2[1] = 0.0f;
			ComputeCoefficients();
		}
		...
		AudioEffectType BiquadFilter::GetType() const {
			switch (mType) {
				case FilterType::LOW_PASS:
					return AudioEffectType::LOW_PASS;
				case FilterType::HIGH_PASS:
					return AudioEffectType::HIGH_PASS;
				case FilterType::BAND_PASS:
					return AudioEffectType::BAND_PASS;
				case FilterType::NOTCH:
					return AudioEffectType::NOTCH;
				default:
					return AudioEffectType::EQ_PARAMETRIC;
			}
		}
		...
		void BiquadFilter::SetCutoff(float32 hz) {
			mCutoffHz = hz;
			ComputeCoefficients();
		}

		void BiquadFilter::SetQ(float32 q) {
			mQ = q;
			ComputeCoefficients();
		}

		void BiquadFilter::SetGain(float32 db) {
			mGainDb = db;
			ComputeCoefficients();
		}
		...
		void Eq3BandEffect::SetParams(const Params &p) {
			mLowShelf.SetCutoff(p.lowFreq);
			mLowShelf.SetGain(p.lowGainDb);
			mMidPeak.SetCutoff(p.midFreq);
			mMidPeak.SetQ(p.midQ);
			mMidPeak.SetGain(p.midGainDb);
			mHighShelf.SetCutoff(p.highFreq);
			mHighShelf.SetGain(p.highGainDb);
		}
		...
		void DistortionEffect::SetDrive(float32 d) {
			mDrive = d;
		}

		void DistortionEffect::SetOutput(float32 o) {
			mOutput = o;
		}
		...
		ChorusEffect::ChorusEffect(float32 rate, float32 depth, float32 feedback, float32 wet, int32 sampleRate,
								   memory::NkAllocator *allocator)
			: mRate(rate), mDepth(depth), mFeedback(feedback), mLfoPhase(0.0f), mSampleRate(sampleRate),
			  mAllocator(allocator) {
			mWetMix = wet;
			...
			mWritePos = 0;
			if (mDelayBuffer)
				memset(mDelayBuffer, 0, mDelaySize * 2 * sizeof(float32));
		}
		...
		void ChorusEffect::Process(float32 *buffer, int32 frameCount, int32 channels) {
			...
			for (int32 f = 0; f < frameCount; ++f) {
				...
				if (channels > 1) {
					...
					buffer[f * channels + 1] = buffer[f * channels + 1] * (1.0f - mWetMix) + delayedR * mWetMix;
				}
			}
		}

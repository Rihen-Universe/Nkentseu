...
namespace nkentseu {
	namespace audio {
		 ...
		 */
		enum class ResamplingQuality { LINEAR = 0, SINC_4, SINC_8 };
...
namespace nkentseu {
	namespace audio {
		...
		struct NKENTSEU_AUDIO_API AudioHandle {
				...
				NKENTSEU_INLINE bool IsValid() const {
					return id != AUDIO_INVALID_ID;
				}

				NKENTSEU_INLINE operator bool() const {
					return IsValid();
				}

				NKENTSEU_INLINE bool operator==(const AudioHandle &other) const {
					return id == other.id;
				}
		...
		struct NKENTSEU_AUDIO_API AudioSample {
				...
				// ── Durée calculée ───────────────────────────────────────────
				NKENTSEU_INLINE float32 GetDuration() const {
					return (sampleRate > 0) ? (float32)frameCount / (float32)sampleRate : 0.0f;
				}

				NKENTSEU_INLINE usize GetSampleCount() const {
					return frameCount * (usize)channels;
				}

				NKENTSEU_INLINE bool IsValid() const {
					return data != nullptr && frameCount > 0 && sampleRate > 0 && channels > 0;
				}
	...
	namespace audio {
		...
		class NKENTSEU_AUDIO_API IAudioEffect {
			public:
				...
				virtual float32 GetWetMix() const {
					return mWetMix;
				}

				virtual void SetWetMix(float32 mix) {
					mWetMix = mix;
				}
	...
	namespace audio {
		...
		class NKENTSEU_AUDIO_API AudioGenerator {
			public:
				 ...
				 */
				static AudioSample GenerateTone(float32 frequency, float32 duration,
												WaveformType waveform = WaveformType::SINE,
												int32 sampleRate = AUDIO_DEFAULT_SAMPLE_RATE, float32 amplitude = 0.8f,
												memory::NkAllocator *allocator = nullptr);
				 ...
				 * @param amplitude  Amplitude normalisée [0, 1]
				 */
				static AudioSample GenerateNoise(float32 duration, WaveformType type = WaveformType::NOISE_WHITE,
												 int32 sampleRate = AUDIO_DEFAULT_SAMPLE_RATE, float32 amplitude = 0.8f,
												 memory::NkAllocator *allocator = nullptr);
				 ...
				 */
				static AudioSample GenerateChirp(float32 startFreq, float32 endFreq, float32 duration,
												 int32 sampleRate = AUDIO_DEFAULT_SAMPLE_RATE,
												 memory::NkAllocator *allocator = nullptr);
				 ...
				 */
				static AudioSample GenerateChord(const float32 *frequencies, int32 count, float32 duration,
				 ...
				 */
				static void ApplyLfo(AudioSample &sample, float32 rate, float32 depth = 0.5f,
									 WaveformType waveform = WaveformType::SINE);
				 ...
				 */
				static AudioSample GenerateKick(float32 duration = 0.5f, float32 startFreq = 180.0f,
												float32 endFreq = 40.0f, float32 clickAttack = 0.002f,
												int32 sampleRate = AUDIO_DEFAULT_SAMPLE_RATE,
												memory::NkAllocator *allocator = nullptr);
				 ...
				 */
				static AudioSample GenerateSnare(float32 duration = 0.2f, float32 toneFreq = 200.0f,
												 float32 noiseMix = 0.7f, int32 sampleRate = AUDIO_DEFAULT_SAMPLE_RATE,
												 memory::NkAllocator *allocator = nullptr);
				 ...
				 */
				static AudioSample GenerateHihat(float32 duration = 0.05f, bool closed = true,
												 int32 sampleRate = AUDIO_DEFAULT_SAMPLE_RATE,
												 memory::NkAllocator *allocator = nullptr);
		...
		class NKENTSEU_AUDIO_API AudioMixer {
			public:
				...
				explicit AudioMixer(int32 sampleRate = AUDIO_DEFAULT_SAMPLE_RATE,
									int32 channels = AUDIO_DEFAULT_CHANNELS, memory::NkAllocator *allocator = nullptr);
				 ...
				 */
				AudioSample Mix(const AudioSample *samples, int32 count, const float32 *volumes = nullptr);
				 ...
				 */
				AudioSample Crossfade(const AudioSample &a, const AudioSample &b, float32 crossfadeLen);
				 ...
				 */
				void Insert(AudioSample &target, const AudioSample &insert, float32 positionSec, bool mix = true);
	...
	namespace audio {
		...
		class NKENTSEU_AUDIO_API AudioAnalyzer {
			public:
				 ...
				 */
				static float32 DetectTempo(const AudioSample &sample, float32 minBpm = 60.0f, float32 maxBpm = 240.0f);
				 ...
				 */
				static float32 DetectPitch(const AudioSample &sample, float32 minFreq = 50.0f,
				 ...
				 */
				static FftResult ComputeFft(const AudioSample &sample, int32 fftSize = AUDIO_DEFAULT_FFT_SIZE,
											int32 windowType = 1, memory::NkAllocator *allocator = nullptr);
				 ...
				 */
				static void ComputeFftRealtime(const float32 *inputReal, float32 *outputMag, int32 fftSize);
				 ...
				 */
				static NkVector<FftResult> ComputeSpectrogram(const AudioSample &sample, int32 windowSize = 2048,
															  int32 hopSize = 512,
															  memory::NkAllocator *allocator = nullptr);
				 ...
				 */
				static void ComputeBandEnergies(const float32 *magnitudes, int32 fftSize, int32 sampleRate,
												float32 *bands, int32 bandCount);
			...
			private:
				...
				static float32 YinPitchDetect(const float32 *data, int32 count, int32 sampleRate, float32 minFreq,
											  float32 maxFreq);
		};
	...
	namespace audio {
		...
		class NKENTSEU_AUDIO_API AudioEngine {
			public:
				 ...
				 */
				AudioHandle Play(const AudioSample &sample, const VoiceParams &params = VoiceParams{});
				 ...
				 */
				bool GenerateSyntheticHrtf(int32 irLength = 128, int32 nAzimuths = 36, int32 nElevations = 9);
				...
				void SetListenerOrientation(float32 fwdX, float32 fwdY, float32 fwdZ, float32 upX, float32 upY,
											float32 upZ);
	...
	namespace {                                                                                                        \
		struct BackendClass##_AutoRegister {                                                                           \
				BackendClass##_AutoRegister() {                                                                        \
					nkentseu::audio::AudioBackendFactory::Register(Name, []() -> nkentseu::audio::IAudioBackend * {    \
						return nkentseu::memory::NkGetDefaultAllocator().New<BackendClass>();                          \
					});                                                                                                \
				}                                                                                                      \
		};                                                                                                             \

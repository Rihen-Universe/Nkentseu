...
namespace nkentseu {
	...
	enum class NkSLTarget : uint32 {
		...
		NK_CPLUSPLUS,		// Software rasterizer (C++ généré)
		NK_BYTECODE,		// Software rasterizer (bytecode NkSL exécuté par NkSLVM) ← NOUVEAU
		NK_COUNT,
	...
	// =============================================================================
	// Stage de shader — enum CANONIQUE et UNIQUE (bitmask).
	// Unifie les anciens NkSLStage (séquentiel) et NkShaderStage (RHI). NKRHI en
	// porte le nom historique via `using NkShaderStage = NkSLStage;` (NkTypes.h) —
	// pas de duplication, pas de couplage inverse (NKRHI dépend de NKSL).
	// =============================================================================
	enum class NkSLStage : uint32 {
		...
		NK_GEOMETRY = 1 << 2,
		NK_TESS_CTRL = 1 << 3,
		NK_TESS_CONTROL = NK_TESS_CTRL, // alias (compat ancien NkSLStage)
		NK_TESS_EVAL = 1 << 4,
		NK_COMPUTE = 1 << 5,
	...
	// ── Opérateurs bitmask (étage = flags) ───────────────────────────────────
	inline NkSLStage operator|(NkSLStage a, NkSLStage b) {
		return static_cast<NkSLStage>(static_cast<uint32>(a) | static_cast<uint32>(b));
	}
	...
	inline NkSLStage operator~(NkSLStage a) {
		return static_cast<NkSLStage>(~static_cast<uint32>(a));
	}
	...
	// True si `flags` contient l'étage `s` (au moins un bit commun).
	inline bool NkSLStageHas(NkSLStage flags, NkSLStage s) {
		return (static_cast<uint32>(flags) & static_cast<uint32>(s)) != 0u;
	}

	// Conversion NkString — gère les combinaisons (ex. "VERTEX|FRAGMENT").
	inline NkString NkSLStageToString(NkSLStage s) {
		const uint32 v = static_cast<uint32>(s);
		NkString out;
		...
		static const Bit kBits[] = {
			{NkSLStage::NK_VERTEX, "VERTEX"},		{NkSLStage::NK_FRAGMENT, "FRAGMENT"},
			{NkSLStage::NK_GEOMETRY, "GEOMETRY"},	{NkSLStage::NK_TESS_CTRL, "TESS_CTRL"},
			{NkSLStage::NK_TESS_EVAL, "TESS_EVAL"}, {NkSLStage::NK_COMPUTE, "COMPUTE"},
			{NkSLStage::NK_MESH, "MESH"},			{NkSLStage::NK_TASK, "TASK"},
		};
		for (const Bit &b : kBits) {
			if (v & static_cast<uint32>(b.flag)) {
				if (!out.Empty())
					out += "|";
				out += b.name;
			}
	...
	enum class NkSLBaseType : uint32 {
		...
		NK_SAMPLER2D_SHADOW,
		NK_SAMPLER2D_ARRAY,
		NK_SAMPLER2D_ARRAY_SHADOW,
		NK_SAMPLER_CUBE,
		NK_SAMPLER_CUBE_SHADOW,
		NK_SAMPLER3D,
		NK_ISAMPLER2D,
		NK_USAMPLER2D,
		...
		NK_SAMPLER1D,
		NK_SAMPLER1D_ARRAY,
		NK_SAMPLER_CUBE_ARRAY,
		NK_SAMPLER_CUBE_ARRAY_SHADOW,
		NK_SAMPLER2DMS,
		NK_ISAMPLER1D,
		NK_USAMPLER1D,
		NK_ISAMPLER3D,
		...
		NK_ISAMPLER2D_ARRAY,
		NK_USAMPLER2D_ARRAY,
		NK_ISAMPLER_CUBE_ARRAY,
		NK_USAMPLER_CUBE_ARRAY, // <- dernier sampler
		...
		NK_IMAGE2D_ARRAY,
		NK_IIMAGE2D_ARRAY,
		NK_UIMAGE2D_ARRAY, // <- derniere image
		// Vulkan specifics
	...
	inline bool NkSLTypeIsMatrix(NkSLBaseType t) {
		return t == NkSLBaseType::NK_MAT2 || t == NkSLBaseType::NK_MAT3 || t == NkSLBaseType::NK_MAT4 ||
			   t == NkSLBaseType::NK_MAT2X3 || t == NkSLBaseType::NK_MAT2X4 || t == NkSLBaseType::NK_MAT3X2 ||
			   t == NkSLBaseType::NK_MAT3X4 || t == NkSLBaseType::NK_MAT4X2 || t == NkSLBaseType::NK_MAT4X3;
	}
	...
	enum class NkSLPrecision : uint32 { NK_NONE, NK_LOWP, NK_MEDIUMP, NK_HIGHP };
	...
	struct NkSLBinding {
			...
			bool HasInputAttachment() const {
				return inputAttachment >= 0;
			}

			bool HasImageFormat() const {
	...
	struct NkSLCompileOptions {
			...
			// GL : injecte `gl_Position.y = -gl_Position.y;` en fin de main() vertex
	...
	struct NkSLReflection {
			...
			const NkSLResourceBinding *FindBinding(uint32 set, uint32 binding) const {
				for (auto &r : resources)
					if (r.set == set && r.binding == (uint32)binding)
						return &r;
				return nullptr;
			}

...
NkSLNode* NkSLParser::ParseTopLevel() {
        ...
        if (decl) {
            switch (decl->kind) {
                ...
                case NkSLNodeKind::NK_DECL_PUSH_CONSTANT:
                    tgt = &static_cast<NkSLBlockDeclNode*>(decl)->binding; break;
                case NkSLNodeKind::NK_DECL_VAR:
                case NkSLNodeKind::NK_DECL_INPUT:
                case NkSLNodeKind::NK_DECL_OUTPUT:
                    tgt = &static_cast<NkSLVarDeclNode*>(decl)->binding; break;
                default: break;
            }
        }
        if (tgt && ann) {
            if (ann->kind == NkSLNodeKind::NK_ANNOTATION_LOCATION) {
                if (ann->binding.HasLocation()) tgt->location = ann->binding.location;
            } else { // BINDING / PUSH_CONSTANT
                if (ann->binding.HasSet())         tgt->set     = ann->binding.set;
                if (ann->binding.HasBinding())     tgt->binding = ann->binding.binding;
                if (ann->binding.HasImageFormat()) tgt->imageFormat = ann->binding.imageFormat;
            }
        }
        delete ann;
        return decl;
    ...
    if (k == NkSLTokenKind::NK_AT_STAGE || k == NkSLTokenKind::NK_AT_ENTRY ||
        k == NkSLTokenKind::NK_AT_BUILTIN) {
...
NkSLVarDeclNode* NkSLParser::ParseVarDecl(bool expectSemi) {
    ...
    while (Check(NkSLTokenKind::NK_OP_COMMA)) {
        ...
        if (d->type && Match(NkSLTokenKind::NK_LBRACKET)) {
            if (Check(NkSLTokenKind::NK_RBRACKET)) { d->type->arraySize = 0; d->type->isUnsized = true; }
            else d->type->arraySize = (uint32)Consume().intVal;
            Consume(NkSLTokenKind::NK_RBRACKET, "Expected ']' after array size");
        }
        ...
        default: {
            ...
            for (;;) {
                ...
                if (qk == NkSLTokenKind::NK_KW_CONST    || qk == NkSLTokenKind::NK_KW_LOWP    ||
                    ...
                    qk == NkSLTokenKind::NK_KW_INVARIANT) { q++; continue; }
                break;
            }
...
NkSLNode* NkSLParser::ParsePrimary() {
    ...
    if (IsTypeToken(tok.kind)) {
        ...
        if (Check(NkSLTokenKind::NK_LBRACKET)) {
            Consume(); // '['
            NkString suffix = "[";
            if (!Check(NkSLTokenKind::NK_RBRACKET)) suffix += Consume().text; // taille (littéral)

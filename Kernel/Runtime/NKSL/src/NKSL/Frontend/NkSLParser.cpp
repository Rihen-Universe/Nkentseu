...
namespace nkentseu {
	...
	namespace {
		struct NkSLDepthGuard {
				...
				explicit NkSLDepthGuard(uint32 &depth) noexcept : d(depth) {
					++d;
				}
	...
	void NkSLParser::Synchronize() {
		...
		while (!IsAtEnd()) {
			...
			if (k == NkSLTokenKind::NK_OP_SEMICOLON) {
				Consume();
				return;
			}
			if (k == NkSLTokenKind::NK_RBRACE)
				return;
			if (k == NkSLTokenKind::NK_KW_STRUCT || k == NkSLTokenKind::NK_KW_UNIFORM ||
				k == NkSLTokenKind::NK_KW_BUFFER || k == NkSLTokenKind::NK_KW_IN || k == NkSLTokenKind::NK_KW_OUT ||
				k == NkSLTokenKind::NK_AT_BINDING || k == NkSLTokenKind::NK_AT_STAGE ||
				k == NkSLTokenKind::NK_AT_ENTRY || IsTypeToken(k))
				return;
			Consume();
		}
	}
	...
	bool NkSLParser::IsQualifierToken(NkSLTokenKind k) const {
		return k == NkSLTokenKind::NK_KW_IN || k == NkSLTokenKind::NK_KW_OUT || k == NkSLTokenKind::NK_KW_INOUT ||
			   k == NkSLTokenKind::NK_KW_UNIFORM || k == NkSLTokenKind::NK_KW_BUFFER ||
			   k == NkSLTokenKind::NK_KW_SHARED || k == NkSLTokenKind::NK_KW_CONST ||
			   k == NkSLTokenKind::NK_KW_SMOOTH || k == NkSLTokenKind::NK_KW_FLAT ||
			   k == NkSLTokenKind::NK_KW_NOPERSPECTIVE || k == NkSLTokenKind::NK_KW_LOWP ||
			   k == NkSLTokenKind::NK_KW_MEDIUMP || k == NkSLTokenKind::NK_KW_HIGHP ||
			   k == NkSLTokenKind::NK_KW_INVARIANT;
	}

	// =============================================================================
	NkSLBaseType NkSLParser::TokenToBaseType(NkSLTokenKind k) const {
		switch (k) {
			case NkSLTokenKind::NK_KW_VOID:
				return NkSLBaseType::NK_VOID;
			case NkSLTokenKind::NK_KW_BOOL:
				return NkSLBaseType::NK_BOOL;
			case NkSLTokenKind::NK_KW_INT:
				return NkSLBaseType::NK_INT;
			case NkSLTokenKind::NK_KW_UINT:
				return NkSLBaseType::NK_UINT;
			case NkSLTokenKind::NK_KW_FLOAT:
			...
			case NkSLTokenKind::NK_KW_IVEC3:
				return NkSLBaseType::NK_IVEC3;
			case NkSLTokenKind::NK_KW_IVEC4:
				return NkSLBaseType::NK_IVEC4;
			case NkSLTokenKind::NK_KW_UVEC2:
				return NkSLBaseType::NK_UVEC2;
			case NkSLTokenKind::NK_KW_UVEC3:
				return NkSLBaseType::NK_UVEC3;
			case NkSLTokenKind::NK_KW_UVEC4:
				return NkSLBaseType::NK_UVEC4;
			case NkSLTokenKind::NK_KW_VEC2:
				return NkSLBaseType::NK_VEC2;
			case NkSLTokenKind::NK_KW_VEC3:
				return NkSLBaseType::NK_VEC3;
			case NkSLTokenKind::NK_KW_VEC4:
				return NkSLBaseType::NK_VEC4;
			case NkSLTokenKind::NK_KW_DVEC2:
				return NkSLBaseType::NK_DVEC2;
			case NkSLTokenKind::NK_KW_DVEC3:
				return NkSLBaseType::NK_DVEC3;
			case NkSLTokenKind::NK_KW_DVEC4:
				return NkSLBaseType::NK_DVEC4;
			case NkSLTokenKind::NK_KW_MAT2:
				return NkSLBaseType::NK_MAT2;
			case NkSLTokenKind::NK_KW_MAT3:
				return NkSLBaseType::NK_MAT3;
			case NkSLTokenKind::NK_KW_MAT4:
				return NkSLBaseType::NK_MAT4;
			case NkSLTokenKind::NK_KW_MAT2X3:
				return NkSLBaseType::NK_MAT2X3;
			case NkSLTokenKind::NK_KW_MAT2X4:
				return NkSLBaseType::NK_MAT2X4;
			case NkSLTokenKind::NK_KW_MAT3X2:
				return NkSLBaseType::NK_MAT3X2;
			case NkSLTokenKind::NK_KW_MAT3X4:
				return NkSLBaseType::NK_MAT3X4;
			case NkSLTokenKind::NK_KW_MAT4X2:
				return NkSLBaseType::NK_MAT4X2;
			case NkSLTokenKind::NK_KW_MAT4X3:
				return NkSLBaseType::NK_MAT4X3;
			case NkSLTokenKind::NK_KW_DMAT2:
				return NkSLBaseType::NK_DMAT2;
			case NkSLTokenKind::NK_KW_DMAT3:
				return NkSLBaseType::NK_DMAT3;
			case NkSLTokenKind::NK_KW_DMAT4:
				return NkSLBaseType::NK_DMAT4;
			case NkSLTokenKind::NK_KW_SAMPLER2D:
				return NkSLBaseType::NK_SAMPLER2D;
			case NkSLTokenKind::NK_KW_SAMPLER2D_SHADOW:
				return NkSLBaseType::NK_SAMPLER2D_SHADOW;
			case NkSLTokenKind::NK_KW_SAMPLER2D_ARRAY:
				return NkSLBaseType::NK_SAMPLER2D_ARRAY;
			case NkSLTokenKind::NK_KW_SAMPLER2D_ARRAY_SHADOW:
				return NkSLBaseType::NK_SAMPLER2D_ARRAY_SHADOW;
			case NkSLTokenKind::NK_KW_SAMPLERCUBE:
				return NkSLBaseType::NK_SAMPLER_CUBE;
			case NkSLTokenKind::NK_KW_SAMPLERCUBE_SHADOW:
				return NkSLBaseType::NK_SAMPLER_CUBE_SHADOW;
			case NkSLTokenKind::NK_KW_SAMPLER3D:
				return NkSLBaseType::NK_SAMPLER3D;
			case NkSLTokenKind::NK_KW_ISAMPLER2D:
				return NkSLBaseType::NK_ISAMPLER2D;
			case NkSLTokenKind::NK_KW_USAMPLER2D:
				return NkSLBaseType::NK_USAMPLER2D;
			case NkSLTokenKind::NK_KW_IMAGE2D:
				return NkSLBaseType::NK_IMAGE2D;
			case NkSLTokenKind::NK_KW_IIMAGE2D:
				return NkSLBaseType::NK_IIMAGE2D;
			case NkSLTokenKind::NK_KW_UIMAGE2D:
				return NkSLBaseType::NK_UIMAGE2D;
			// --- Samplers/images additionnels ---
			case NkSLTokenKind::NK_KW_SAMPLER1D:
				return NkSLBaseType::NK_SAMPLER1D;
			case NkSLTokenKind::NK_KW_SAMPLER1D_ARRAY:
				return NkSLBaseType::NK_SAMPLER1D_ARRAY;
			case NkSLTokenKind::NK_KW_SAMPLERCUBE_ARRAY:
				return NkSLBaseType::NK_SAMPLER_CUBE_ARRAY;
			case NkSLTokenKind::NK_KW_SAMPLERCUBE_ARRAY_SHADOW:
				return NkSLBaseType::NK_SAMPLER_CUBE_ARRAY_SHADOW;
			case NkSLTokenKind::NK_KW_SAMPLER2DMS:
				return NkSLBaseType::NK_SAMPLER2DMS;
			case NkSLTokenKind::NK_KW_ISAMPLER1D:
				return NkSLBaseType::NK_ISAMPLER1D;
			case NkSLTokenKind::NK_KW_USAMPLER1D:
				return NkSLBaseType::NK_USAMPLER1D;
			case NkSLTokenKind::NK_KW_ISAMPLER3D:
				return NkSLBaseType::NK_ISAMPLER3D;
			case NkSLTokenKind::NK_KW_USAMPLER3D:
				return NkSLBaseType::NK_USAMPLER3D;
			case NkSLTokenKind::NK_KW_ISAMPLERCUBE:
				return NkSLBaseType::NK_ISAMPLER_CUBE;
			case NkSLTokenKind::NK_KW_USAMPLERCUBE:
				return NkSLBaseType::NK_USAMPLER_CUBE;
			case NkSLTokenKind::NK_KW_ISAMPLER2D_ARRAY:
				return NkSLBaseType::NK_ISAMPLER2D_ARRAY;
			case NkSLTokenKind::NK_KW_USAMPLER2D_ARRAY:
				return NkSLBaseType::NK_USAMPLER2D_ARRAY;
			case NkSLTokenKind::NK_KW_ISAMPLERCUBE_ARRAY:
				return NkSLBaseType::NK_ISAMPLER_CUBE_ARRAY;
			case NkSLTokenKind::NK_KW_USAMPLERCUBE_ARRAY:
				return NkSLBaseType::NK_USAMPLER_CUBE_ARRAY;
			case NkSLTokenKind::NK_KW_IMAGE1D:
				return NkSLBaseType::NK_IMAGE1D;
			case NkSLTokenKind::NK_KW_IIMAGE1D:
				return NkSLBaseType::NK_IIMAGE1D;
			case NkSLTokenKind::NK_KW_UIMAGE1D:
				return NkSLBaseType::NK_UIMAGE1D;
			case NkSLTokenKind::NK_KW_IMAGE3D:
				return NkSLBaseType::NK_IMAGE3D;
			case NkSLTokenKind::NK_KW_IIMAGE3D:
				return NkSLBaseType::NK_IIMAGE3D;
			case NkSLTokenKind::NK_KW_UIMAGE3D:
				return NkSLBaseType::NK_UIMAGE3D;
			case NkSLTokenKind::NK_KW_IMAGECUBE:
				return NkSLBaseType::NK_IMAGE_CUBE;
			case NkSLTokenKind::NK_KW_IIMAGECUBE:
				return NkSLBaseType::NK_IIMAGE_CUBE;
			case NkSLTokenKind::NK_KW_UIMAGECUBE:
				return NkSLBaseType::NK_UIMAGE_CUBE;
			case NkSLTokenKind::NK_KW_IMAGE2D_ARRAY:
				return NkSLBaseType::NK_IMAGE2D_ARRAY;
			case NkSLTokenKind::NK_KW_IIMAGE2D_ARRAY:
				return NkSLBaseType::NK_IIMAGE2D_ARRAY;
			case NkSLTokenKind::NK_KW_UIMAGE2D_ARRAY:
				return NkSLBaseType::NK_UIMAGE2D_ARRAY;
			default:
	...
	NkSLNode *NkSLParser::ParseTopLevel() {
		...
		// @binding / @location / @push_constant : RATTACHER à la déclaration suivante.
		// Sinon l'annotation devient un nœud orphelin que les générateurs ignorent →
		// bindings auto-assignés séquentiellement (0,1,2…) au lieu des numéros déclarés
		// → mismatch avec le layout de descripteurs du consommateur (ex. NKRenderer).
		if (k == NkSLTokenKind::NK_AT_BINDING || k == NkSLTokenKind::NK_AT_LOCATION ||
			k == NkSLTokenKind::NK_AT_PUSH_CONSTANT) {
			NkSLAnnotationNode *ann = ParseAnnotation();
			NkSLNode *decl = ParseTopLevel(); // la déclaration cible (ou annotations chaînées)
			NkSLBinding *tgt = nullptr;
			if (decl) {
				switch (decl->kind) {
					case NkSLNodeKind::NK_DECL_UNIFORM_BLOCK:
					case NkSLNodeKind::NK_DECL_STORAGE_BLOCK:
					case NkSLNodeKind::NK_DECL_PUSH_CONSTANT:
						tgt = &static_cast<NkSLBlockDeclNode *>(decl)->binding;
						break;
					case NkSLNodeKind::NK_DECL_VAR:
					case NkSLNodeKind::NK_DECL_INPUT:
					case NkSLNodeKind::NK_DECL_OUTPUT:
						tgt = &static_cast<NkSLVarDeclNode *>(decl)->binding;
						break;
					default:
						break;
				}
			...
			if (tgt && ann) {
				if (ann->kind == NkSLNodeKind::NK_ANNOTATION_LOCATION) {
					if (ann->binding.HasLocation())
						tgt->location = ann->binding.location;
				} else { // BINDING / PUSH_CONSTANT
					if (ann->binding.HasSet())
						tgt->set = ann->binding.set;
					if (ann->binding.HasBinding())
						tgt->binding = ann->binding.binding;
					if (ann->binding.HasImageFormat())
						tgt->imageFormat = ann->binding.imageFormat;
				}
			}
			delete ann;
			return decl;
		}
		// @stage / @entry / @builtin : nœuds séparés (entry = fonction "main", stage
		// passé en externe à Compile). Inchangé.
		if (k == NkSLTokenKind::NK_AT_STAGE || k == NkSLTokenKind::NK_AT_ENTRY || k == NkSLTokenKind::NK_AT_BUILTIN) {
		...
		if (k == NkSLTokenKind::NK_KW_BUFFER)
			return ParseBlockDecl(NkSLStorageQual::NK_BUFFER);

		// in / out → variable globale d'interface
		if (k == NkSLTokenKind::NK_KW_IN || k == NkSLTokenKind::NK_KW_OUT)
			return ParseVarDecl(true);
		...
		if (k == NkSLTokenKind::NK_KW_LAYOUT) {
			...
			while (!Check(NkSLTokenKind::NK_RPAREN) && !IsAtEnd()) {
				...
				else if (qual.text == "local_size_x" || qual.text == "local_size_y" || qual.text == "local_size_z") {
					isLocalSize = true;
					if (Match(NkSLTokenKind::NK_OP_ASSIGN)) {
						NkSLToken val = Consume(NkSLTokenKind::NK_LIT_INT, "Expected integer for local_size");
						uint32 v = (uint32)val.intVal;
						if (qual.text == "local_size_x")
							mLocalSizeX = v;
						else if (qual.text == "local_size_y")
							mLocalSizeY = v;
						else
							mLocalSizeZ = v;
					}
				} else if (qual.kind == NkSLTokenKind::NK_OP_COMMA)
					continue;
				// On ignore les autres qualificateurs (binding=N, location=N, etc.)
				else if (qual.kind == NkSLTokenKind::NK_OP_ASSIGN || IsTypeToken(qual.kind))
					continue;
				else if (qual.kind == NkSLTokenKind::NK_LIT_INT)
					continue;
				else if (qual.kind == NkSLTokenKind::NK_IDENT)
					continue;
			}
		...
		if (k == NkSLTokenKind::NK_KW_SMOOTH || k == NkSLTokenKind::NK_KW_FLAT ||
			k == NkSLTokenKind::NK_KW_NOPERSPECTIVE)
			return ParseVarDecl(true);

		// Précision
		if (k == NkSLTokenKind::NK_KW_LOWP || k == NkSLTokenKind::NK_KW_MEDIUMP || k == NkSLTokenKind::NK_KW_HIGHP)
			return ParseVarDecl(true);
		...
		if (IsTypeToken(k)) {
			...
			if (name.kind == NkSLTokenKind::NK_IDENT) {
				...
				if (after.kind == NkSLTokenKind::NK_LPAREN)
	...
	NkSLAnnotationNode *NkSLParser::ParseAnnotation() {
		...
		if (ak == NkSLTokenKind::NK_AT_BINDING || ak == NkSLTokenKind::NK_AT_LOCATION) {
			ann->kind = (ak == NkSLTokenKind::NK_AT_BINDING) ? NkSLNodeKind::NK_ANNOTATION_BINDING
			...
			if (Match(NkSLTokenKind::NK_LPAREN)) {
				...
				if (stageToken.text == "vertex")
					ann->stage = NkSLStage::NK_VERTEX;
				else if (stageToken.text == "fragment")
					ann->stage = NkSLStage::NK_FRAGMENT;
				else if (stageToken.text == "compute")
					ann->stage = NkSLStage::NK_COMPUTE;
				else if (stageToken.text == "geometry")
					ann->stage = NkSLStage::NK_GEOMETRY;
			...
			if (name.kind == NkSLTokenKind::NK_IDENT) {
				...
				if (Match(NkSLTokenKind::NK_OP_ASSIGN)) {
					if (name.text == "format") {
						...
						if (name.text == "set")
							b.set = (int32)val.intVal;
						if (name.text == "binding")
							b.binding = (int32)val.intVal;
						if (name.text == "location")
							b.location = (int32)val.intVal;
						if (name.text == "offset")
							b.offset = (int32)val.intVal;
					}
				}
	...
	NkSLVarDeclNode *NkSLParser::ParseVarDecl(bool expectSemi) {
		...
		for (;;) {
			...
			if (k == NkSLTokenKind::NK_KW_CONST) {
				decl->isConst = true;
				Consume();
			} else if (k == NkSLTokenKind::NK_KW_INVARIANT) {
				decl->isInvariant = true;
				Consume();
			} else if (k == NkSLTokenKind::NK_KW_IN) {
				decl->storage = NkSLStorageQual::NK_IN;
				Consume();
				decl->kind = NkSLNodeKind::NK_DECL_INPUT;
			} else if (k == NkSLTokenKind::NK_KW_OUT) {
				decl->storage = NkSLStorageQual::NK_OUT;
				Consume();
				decl->kind = NkSLNodeKind::NK_DECL_OUTPUT;
			} else if (k == NkSLTokenKind::NK_KW_INOUT) {
				decl->storage = NkSLStorageQual::NK_INOUT;
				Consume();
			} else if (k == NkSLTokenKind::NK_KW_UNIFORM) {
				decl->storage = NkSLStorageQual::NK_UNIFORM;
				Consume();
			} else if (k == NkSLTokenKind::NK_KW_SHARED) {
				decl->storage = NkSLStorageQual::NK_SHARED;
				Consume();
			} else if (k == NkSLTokenKind::NK_KW_SMOOTH) {
				decl->interp = NkSLInterpolation::NK_SMOOTH;
				Consume();
			} else if (k == NkSLTokenKind::NK_KW_FLAT) {
				decl->interp = NkSLInterpolation::NK_FLAT;
				Consume();
			} else if (k == NkSLTokenKind::NK_KW_NOPERSPECTIVE) {
				decl->interp = NkSLInterpolation::NK_NOPERSPECTIVE;
				Consume();
			} else if (k == NkSLTokenKind::NK_KW_LOWP) {
				decl->precision = NkSLPrecision::NK_LOWP;
				Consume();
			} else if (k == NkSLTokenKind::NK_KW_MEDIUMP) {
				decl->precision = NkSLPrecision::NK_MEDIUMP;
				Consume();
			} else if (k == NkSLTokenKind::NK_KW_HIGHP) {
				decl->precision = NkSLPrecision::NK_HIGHP;
				Consume();
			} else
				break;
		}
		...
		// Array postfixé C-style : "type nom[N];" (ex. membres d'UBO `vec4 occ[32];`,
		// `Light2D lights[16];`). On applique la taille au type (le préfixé `type[N] nom`
		// est géré dans ParseType).
		if (decl->type && Match(NkSLTokenKind::NK_LBRACKET)) {
			if (Check(NkSLTokenKind::NK_RBRACKET)) {
				decl->type->arraySize = 0;
				decl->type->isUnsized = true;
			} else {
				decl->type->arraySize = (uint32)Consume().intVal;
			}
			Consume(NkSLTokenKind::NK_RBRACKET, "Expected ']' after array size");
			// On NE met PAS kind=NK_TYPE_ARRAY : ainsi GenVarDecl émet la forme
			// postfixée `type nom[N];` (standard GLSL/HLSL) au lieu de dépendre de
			// TypeToString pour un array préfixé.
		}
		...
		while (Check(NkSLTokenKind::NK_OP_COMMA)) {
			...
			if (decl->type) {
				...
				// arraySize NON copié : chaque déclarateur a SA propre dimension d'array
				// (ex. `vec4 a[32], b[32], c;` → a/b = [32], c = scalaire).
				ct->arraySize = 0;
				ct->isUnsized = false;
			...
			if (Check(NkSLTokenKind::NK_IDENT))
				d->name = Consume().text;
			// Array postfixé C-style pour CE déclarateur (ex. `vec4 a[32], b[32];`).
			if (d->type && Match(NkSLTokenKind::NK_LBRACKET)) {
				if (Check(NkSLTokenKind::NK_RBRACKET)) {
					d->type->arraySize = 0;
					d->type->isUnsized = true;
				} else
					d->type->arraySize = (uint32)Consume().intVal;
				Consume(NkSLTokenKind::NK_RBRACKET, "Expected ']' after array size");
			}
			if (Match(NkSLTokenKind::NK_OP_ASSIGN))
				d->initializer = ParseAssignment();
	...
	NkSLBlockDeclNode *NkSLParser::ParseBlockDecl(NkSLStorageQual storage) {
		...
		if (Peek().kind == NkSLTokenKind::NK_KW_UNIFORM || Peek().kind == NkSLTokenKind::NK_KW_BUFFER)
			Consume();

		// Nom du block
		if (Check(NkSLTokenKind::NK_IDENT))
			block->blockName = Consume().text;
		...
		while (!Check(NkSLTokenKind::NK_RBRACE) && !IsAtEnd()) {
			...
			for (auto *extra : mPendingDecls) {
				block->members.PushBack(extra);
				block->AddChild(extra);
			}
			mPendingDecls.Clear();
		}
		...
		if (Check(NkSLTokenKind::NK_IDENT))
			block->instanceName = Consume().text;
		...
		block->kind = storage == NkSLStorageQual::NK_BUFFER
						  ...
						  : (storage == NkSLStorageQual::NK_PUSH_CONSTANT ? NkSLNodeKind::NK_DECL_PUSH_CONSTANT
	...
	NkSLFunctionDeclNode *NkSLParser::ParseFunctionDecl(NkSLVarDeclNode *) {
		...
		while (!Check(NkSLTokenKind::NK_RPAREN) && !IsAtEnd()) {
			...
			if (Check(NkSLTokenKind::NK_KW_IN)) {
				param->storage = NkSLStorageQual::NK_IN;
				Consume();
			}
			if (Check(NkSLTokenKind::NK_KW_OUT)) {
				param->storage = NkSLStorageQual::NK_OUT;
				Consume();
			}
			if (Check(NkSLTokenKind::NK_KW_INOUT)) {
				param->storage = NkSLStorageQual::NK_INOUT;
				Consume();
			}
			...
			fn->AddChild(param);
			if (!Match(NkSLTokenKind::NK_OP_COMMA))
				break;
		}
	...
	NkSLNode *NkSLParser::ParseStatement() {
		...
		if (mDepth > kMaxParseDepth) {
			...
			if (!IsAtEnd())
				Consume(); // garantir la progression pour eviter une boucle
			return nullptr;
		}
		switch (Peek().kind) {
			case NkSLTokenKind::NK_LBRACE:
				return ParseBlock();
			case NkSLTokenKind::NK_KW_IF:
				return ParseIfStmt();
			case NkSLTokenKind::NK_KW_FOR:
				return ParseForStmt();
			case NkSLTokenKind::NK_KW_WHILE:
				return ParseWhileStmt();
			case NkSLTokenKind::NK_KW_DO:
				return ParseDoWhileStmt();
			case NkSLTokenKind::NK_KW_RETURN:
				return ParseReturnStmt();
			case NkSLTokenKind::NK_KW_SWITCH:
			...
			case NkSLTokenKind::NK_OP_SEMICOLON:
				...
				return nullptr;
			default: {
				// Déclaration locale vs expression. Une déclaration peut être préfixée de
				// qualificateurs (`const`, `highp`/`mediump`/`lowp`, `invariant`) AVANT le
				// type : `const float a = ...`. On les saute puis on teste `TYPE IDENT`.
				uint32 q = 0;
				for (;;) {
					NkSLTokenKind qk = PeekAt(q).kind;
					if (qk == NkSLTokenKind::NK_KW_CONST || qk == NkSLTokenKind::NK_KW_LOWP ||
						qk == NkSLTokenKind::NK_KW_MEDIUMP || qk == NkSLTokenKind::NK_KW_HIGHP ||
						qk == NkSLTokenKind::NK_KW_INVARIANT) {
						q++;
						continue;
					}
					break;
				}
				if (IsTypeToken(PeekAt(q).kind) && PeekAt(q + 1).kind == NkSLTokenKind::NK_IDENT)
					return ParseVarDecl(true);
	...
	NkSLNode *NkSLParser::ParseIfStmt() {
		...
		if (Match(NkSLTokenKind::NK_KW_ELSE))
			n->elseBranch = ParseStatement();
		n->AddChild(n->condition);
		n->AddChild(n->thenBranch);
		if (n->elseBranch)
			n->AddChild(n->elseBranch);
		return n;
	}

	NkSLNode *NkSLParser::ParseForStmt() {
		auto *n = new NkSLForNode();
		n->line = Peek().line;
		Consume(); // 'for'
		Consume(NkSLTokenKind::NK_LPAREN, "Expected '('");
		...
		if (!Check(NkSLTokenKind::NK_OP_SEMICOLON))
			n->condition = ParseExpression();
		Consume(NkSLTokenKind::NK_OP_SEMICOLON, "Expected ';'");
		if (!Check(NkSLTokenKind::NK_RPAREN))
			n->increment = ParseExpression();
		Consume(NkSLTokenKind::NK_RPAREN, "Expected ')'");
	...
	NkSLNode *NkSLParser::ParseAssignment() {
		...
		NkSLTokenKind k = Peek().kind;
		if (k == NkSLTokenKind::NK_OP_ASSIGN || k == NkSLTokenKind::NK_OP_PLUS_ASSIGN ||
			k == NkSLTokenKind::NK_OP_MINUS_ASSIGN || k == NkSLTokenKind::NK_OP_STAR_ASSIGN ||
			k == NkSLTokenKind::NK_OP_SLASH_ASSIGN || k == NkSLTokenKind::NK_OP_AND_ASSIGN ||
			k == NkSLTokenKind::NK_OP_OR_ASSIGN || k == NkSLTokenKind::NK_OP_XOR_ASSIGN ||
...
#define PARSE_BINARY(Name, Lower, ...)                                                                                 \
	...
	PARSE_BINARY(ParseRelational, ParseShift, NkSLTokenKind::NK_OP_LT, NkSLTokenKind::NK_OP_GT, NkSLTokenKind::NK_OP_LE,
				 NkSLTokenKind::NK_OP_GE)
...
#undef PARSE_BINARY
	...
	NkSLNode *NkSLParser::ParseUnary() {
		NkSLTokenKind k = Peek().kind;
		if (k == NkSLTokenKind::NK_OP_MINUS || k == NkSLTokenKind::NK_OP_LNOT || k == NkSLTokenKind::NK_OP_TILDE ||
			k == NkSLTokenKind::NK_OP_PLUS || k == NkSLTokenKind::NK_OP_INC || k == NkSLTokenKind::NK_OP_DEC) {
	...
	NkSLNode *NkSLParser::ParsePrimary() {
		...
		// Littéral
		if (tok.kind == NkSLTokenKind::NK_LIT_INT || tok.kind == NkSLTokenKind::NK_LIT_UINT ||
			tok.kind == NkSLTokenKind::NK_LIT_FLOAT || tok.kind == NkSLTokenKind::NK_LIT_DOUBLE ||
			tok.kind == NkSLTokenKind::NK_LIT_BOOL) {
			Consume();
			auto *lit = new NkSLLiteralNode();
			lit->line = tok.line;
			switch (tok.kind) {
				case NkSLTokenKind::NK_LIT_INT:
					lit->baseType = NkSLBaseType::NK_INT;
					lit->intVal = tok.intVal;
					break;
				case NkSLTokenKind::NK_LIT_UINT:
					lit->baseType = NkSLBaseType::NK_UINT;
					lit->uintVal = tok.uintVal;
					break;
				case NkSLTokenKind::NK_LIT_FLOAT:
					lit->baseType = NkSLBaseType::NK_FLOAT;
					lit->floatVal = tok.floatVal;
					break;
				case NkSLTokenKind::NK_LIT_DOUBLE:
					lit->baseType = NkSLBaseType::NK_DOUBLE;
					lit->floatVal = tok.floatVal;
					break;
				case NkSLTokenKind::NK_LIT_BOOL:
					lit->baseType = NkSLBaseType::NK_BOOL;
		...
		// Builtin
		if (tok.kind >= NkSLTokenKind::NK_KW_BUILTIN_POSITION && tok.kind <= NkSLTokenKind::NK_KW_BUILTIN_WORKINVID) {
			Consume();
		...
		if (IsTypeToken(tok.kind)) {
			...
			// Constructeur d'array GLSL : `Type[](e0, e1, ...)` ou `Type[N](...)`.
			// On encode la dimension dans le callee (ex. "vec2[]") → le générateur émet
			// `vec2[](args)`. Indispensable pour les const arrays initialisés (ex. poisson).
			if (Check(NkSLTokenKind::NK_LBRACKET)) {
				Consume(); // '['
				NkString suffix = "[";
				if (!Check(NkSLTokenKind::NK_RBRACKET))
					suffix += Consume().text; // taille (littéral)
				Consume(NkSLTokenKind::NK_RBRACKET, "Expected ']' in array constructor");
				suffix += "]";
				return ParseCallOrCast(typeName + suffix);
			}

...
namespace nkentseu {
	...
	void NkSLSemantic::AnalyzeVarDecl(NkSLVarDeclNode *v, bool isGlobal) {
		...
		if (isGlobal) {
			switch (v->storage) {
				...
				case NkSLStorageQual::NK_UNIFORM:
					...
					if (v->type && (NkSLTypeIsSampler(v->type->baseType) || NkSLTypeIsImage(v->type->baseType))) {
						if (!v->binding.HasBinding())
							Warning(v->line, "Sampler '" + v->name + "' has no explicit @binding — auto-assigned");
					}
					break;
				default:
					break;
		...
		if (isGlobal && !v->isConst && v->storage == NkSLStorageQual::NK_NONE && v->type &&
			!NkSLTypeIsSampler(v->type->baseType) && !NkSLTypeIsImage(v->type->baseType)) {
		...
		if (v->initializer) {
			...
			if (initType.base != NkSLBaseType::NK_UNKNOWN && declType.base != NkSLBaseType::NK_UNKNOWN) {
				if (!NkSLSymbolTable::IsAssignable(initType, declType)) {
					Error(v->line, "Type mismatch in initialization of '" + v->name + "': cannot assign '" +
									   TypeName(initType) + "' to '" + TypeName(declType) + "'");
				}
			}
		}
	}
		...
		if (n->value) {
			...
			if (!expected.IsVoid() && actual.base != NkSLBaseType::NK_UNKNOWN) {
				if (!NkSLSymbolTable::IsAssignable(actual, expected)) {
					Error(n->line, "Return type mismatch: expected '" + TypeName(expected) + "', got '" +
									   TypeName(actual) + "'");
				}
			}
		} else {
			if (!expected.IsVoid()) {
				Error(n->line, "Missing return value (expected '" + TypeName(expected) + "')");
			}
		}
	}
	...
	NkString NkSLSemantic::TypeName(const NkSLResolvedType &t) {
		if (t.base == NkSLBaseType::NK_STRUCT)
			return t.typeName;
		switch (t.base) {
	...
	NkSLResolvedType NkSLSemantic::TypeOf(NkSLNode *node) {
		...
		switch (node->kind) {
			case NkSLNodeKind::NK_EXPR_LITERAL:
				return TypeOfLiteral(static_cast<NkSLLiteralNode *>(node));
			case NkSLNodeKind::NK_EXPR_IDENT:
				return TypeOfIdent(static_cast<NkSLIdentNode *>(node));
			case NkSLNodeKind::NK_EXPR_UNARY:
				return TypeOfUnary(static_cast<NkSLUnaryNode *>(node));
			case NkSLNodeKind::NK_EXPR_BINARY:
				return TypeOfBinary(static_cast<NkSLBinaryNode *>(node));
			case NkSLNodeKind::NK_EXPR_TERNARY:
				return TypeOfTernary(node);
			case NkSLNodeKind::NK_EXPR_CALL:
				return TypeOfCall(static_cast<NkSLCallNode *>(node));
			case NkSLNodeKind::NK_EXPR_MEMBER:
				return TypeOfMember(static_cast<NkSLMemberNode *>(node));
			case NkSLNodeKind::NK_EXPR_INDEX:
				return TypeOfIndex(static_cast<NkSLIndexNode *>(node));
			case NkSLNodeKind::NK_EXPR_ASSIGN:
				return TypeOfAssign(static_cast<NkSLAssignNode *>(node));
			case NkSLNodeKind::NK_EXPR_CAST:
				return TypeOfCast(static_cast<NkSLCastNode *>(node));
			case NkSLNodeKind::NK_STMT_EXPR:
				return node->children.Empty() ? NkSLResolvedType::Void() : TypeOf(node->children[0]);
			default:
				return NkSLResolvedType{NkSLBaseType::NK_UNKNOWN};
		}
	}
	...
	NkSLResolvedType NkSLSemantic::TypeOfBinary(NkSLBinaryNode *b) {
		...
		if (!NkSLSymbolTable::IsAssignable(rhs, lhs) && !NkSLSymbolTable::IsAssignable(lhs, rhs) &&
			!AreCompatibleForOp(b->op, lhs, rhs)) {
			Error(b->line,
				  "Type mismatch in binary expression: '" + TypeName(lhs) + "' " + b->op + " '" + TypeName(rhs) + "'");
			return {NkSLBaseType::NK_UNKNOWN};
		}
	...
	bool NkSLSemantic::AreCompatibleForOp(const NkString &op, const NkSLResolvedType &lhs,
		...
		if (op == "*") {
			...
			if (lhs.base == NkSLBaseType::NK_MAT3 && rhs.base == NkSLBaseType::NK_VEC3)
				return true;
			if (lhs.base == NkSLBaseType::NK_MAT2 && rhs.base == NkSLBaseType::NK_VEC2)
				return true;
			// scalar * vector/matrix
			if (lhs.IsScalar() && (rhs.IsVector() || rhs.IsMatrix()))
				return true;
			if (rhs.IsScalar() && (lhs.IsVector() || lhs.IsMatrix()))
				return true;
		}
		if (op == "+" || op == "-") {
	...
	NkSLResolvedType NkSLSemantic::TypeOfCall(NkSLCallNode *call) {
		...
		if (sym && sym->kind == NkSLSymbolKind::NK_STRUCT_TYPE) {
			...
			if (s && (uint32)call->args.Size() != (uint32)s->members.Size())
				Warning(call->line, "Struct constructor argument count mismatch for '" + call->callee + "'");
			NkSLResolvedType rt;
			rt.base = NkSLBaseType::NK_STRUCT;
			rt.typeName = call->callee;
			return rt;
		}
		...
		static const struct {
				...
				NkSLBaseType t;
							 ...
							 {"uvec2", NkSLBaseType::NK_UVEC2},	  {"uvec3", NkSLBaseType::NK_UVEC3},
							 {"uvec4", NkSLBaseType::NK_UVEC4},	  {"mat2", NkSLBaseType::NK_MAT2},
							 {"mat3", NkSLBaseType::NK_MAT3},	  {"mat4", NkSLBaseType::NK_MAT4},
							 ...
							 {"mat4x3", NkSLBaseType::NK_MAT4X3}, {nullptr, NkSLBaseType::NK_UNKNOWN}};
		...
		if (!ov) {
			...
			Error(call->line, "No matching function for '" + call->callee + "(" + argStr + ")'");
			return {NkSLBaseType::NK_UNKNOWN};
		}
	...
	NkSLResolvedType NkSLSemantic::TypeOfMember(NkSLMemberNode *m) {
		...
		if (base.base == NkSLBaseType::NK_UNKNOWN)
			return base;
	...
	NkSLResolvedType NkSLSemantic::ValidateSwizzle(const NkSLResolvedType &base, const NkString &swizzle, uint32 line) {
		...
		for (uint32 i = 0; i < (uint32)swizzle.Size(); i++) {
			...
			if (c == 'x' || c == 'r' || c == 's')
				...
				Error(line, "Invalid swizzle component '" + NkString(1, c) + "'");
				break;
			}

			if (idx >= vecSize)
				Error(line, "Swizzle '" + NkString(1, c) + "' out of range for " + TypeName(base));
		}
		...
		if (n == 1) { /* scalar */
			...
			if (base.ScalarBase() == NkSLBaseType::NK_FLOAT)
				result.base = NkSLBaseType::NK_VEC2;
			if (base.ScalarBase() == NkSLBaseType::NK_INT)
				result.base = NkSLBaseType::NK_IVEC2;
			if (base.ScalarBase() == NkSLBaseType::NK_UINT)
				result.base = NkSLBaseType::NK_UVEC2;
		} else if (n == 3) {
			if (base.ScalarBase() == NkSLBaseType::NK_FLOAT)
				result.base = NkSLBaseType::NK_VEC3;
			if (base.ScalarBase() == NkSLBaseType::NK_INT)
				result.base = NkSLBaseType::NK_IVEC3;
			if (base.ScalarBase() == NkSLBaseType::NK_UINT)
				result.base = NkSLBaseType::NK_UVEC3;
		} else if (n == 4) {
			if (base.ScalarBase() == NkSLBaseType::NK_FLOAT)
				result.base = NkSLBaseType::NK_VEC4;
			if (base.ScalarBase() == NkSLBaseType::NK_INT)
				result.base = NkSLBaseType::NK_IVEC4;
			if (base.ScalarBase() == NkSLBaseType::NK_UINT)
				result.base = NkSLBaseType::NK_UVEC4;
		} else {
	...
	NkSLResolvedType NkSLSemantic::TypeOfIndex(NkSLIndexNode *idx) {
		...
		if (arrType.arraySize > 0 || arrType.isUnsized) {
			...
			elem.arraySize = 0;
			elem.isUnsized = false;
			return elem;
		}
		// vecteur[i] → scalaire
		if (arrType.IsVector())
			return NkSLResolvedType{arrType.ScalarBase()};
		// matrice[i] → vecteur colonne
		if (arrType.IsMatrix()) {
			switch (arrType.base) {
				case NkSLBaseType::NK_MAT2:
					return NkSLResolvedType::Vec2();
				case NkSLBaseType::NK_MAT3:
	...
	NkSLResolvedType NkSLSemantic::TypeOfAssign(NkSLAssignNode *a) {
		...
		if (!NkSLSymbolTable::IsAssignable(rhs, lhs) && lhs.base != NkSLBaseType::NK_UNKNOWN &&
			rhs.base != NkSLBaseType::NK_UNKNOWN)
			Error(a->line,
				  "Type mismatch in assignment: cannot assign '" + TypeName(rhs) + "' to '" + TypeName(lhs) + "'");
	...
	NkSLResolvedType NkSLSemantic::ValidateConstructorCall(NkSLBaseType targetType,
														   const NkVector<NkSLResolvedType> &argTypes, uint32 line) {
		NkSLResolvedType result;
		result.base = targetType;

		// Vérifications de base
		uint32 targetComponents = NkSLResolvedType{targetType}.ComponentCount();
		if (targetComponents == 0)
			return result; // type inconnu, on accepte

		uint32 providedComponents = 0;
		for (auto &t : argTypes)
			providedComponents += t.ComponentCount();
		...
		if (providedComponents < targetComponents && !(argTypes.Size() == 1)) {
			Warning(line, "Constructor '" + TypeName(result) + "' has too few components (" +
							  NkFormat("{0}", providedComponents) + " < " + NkFormat("{0}", targetComponents) + ")");
		}
		return result;
	}
	...
	void NkSLSemantic::CheckBindings() {
		...
		auto check = [&](int set, int binding, const NkString &name, uint32 line) {
			...
			if (existing) {
				Error(line, "Binding conflict: '" + name + "' and '" + *existing + "' share binding " +
								NkFormat("{0}", binding) + " on set " + NkFormat("{0}", set));
			} else {
	...
	// =============================================================================
	bool NkSLSemantic::CheckStageInterface(const NkSLSemantic &vertSem, const NkSLSemantic &fragSem,
		...
		for (auto *fragIn : fragIns) {
				...
				if (vertOut->name == fragIn->name) {
					...
					if (t1 != t2) {
						errors.PushBack({fragIn->line, 0, "",
										 "Stage interface mismatch for '" + fragIn->name + "': vertex outputs '" +
											 TypeName(t1) + "' but fragment expects '" + TypeName(t2) + "'",
										 true});
						ok = false;
					}
					...
					if (vertOut->binding.HasLocation() && fragIn->binding.HasLocation()) {
						if (vertOut->binding.location != fragIn->binding.location) {
							errors.PushBack({fragIn->line, 0, "",
											 "Location mismatch for '" + fragIn->name + "': vertex @location(" +
												 NkFormat("{0}", vertOut->binding.location) +
												 ") but fragment @location(" +
												 NkFormat("{0}", fragIn->binding.location) + ")",
											 true});
							ok = false;
						}
					}
			...
			if (!found) {
				errors.PushBack({fragIn->line, 0, "",
								 "Fragment input '" + fragIn->name + "' has no matching vertex output",
								 false}); // warning, pas erreur
			}
		}
		...
		for (auto *vertOut : vertOuts) {
			...
			if (!found) {
				errors.PushBack({vertOut->line, 0, "",
								 "Vertex output '" + vertOut->name + "' is not consumed by fragment shader", false});
			}
		}

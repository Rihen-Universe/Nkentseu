...
namespace nkentseu {
	...
	struct NkSLSemanticResult {
			...
			void AddError(uint32 l, const NkString &m) {
				errors.PushBack({l, 0, "", m, true});
				success = false;
			}

			void AddWarning(uint32 l, const NkString &m) {
				warnings.PushBack({l, 0, "", m, false});
			}
	};
	...
	class NkSLSemantic {
		public:
			...
			const NkVector<NkSLVarDeclNode *> &GetInputVars() const {
				return mInputVars;
			}

			const NkVector<NkSLVarDeclNode *> &GetOutputVars() const {
				return mOutputVars;
			}

			const NkVector<NkSLBlockDeclNode *> &GetBlocks() const {
				return mBlocks;
			}

			const NkVector<NkSLVarDeclNode *> &GetUniforms() const {
				return mUniforms;
			}

			const NkVector<NkSLFunctionDeclNode *> &GetEntryPoints() const {
				return mEntryPoints;
			}
			...
			static bool CheckStageInterface(const NkSLSemantic &vertexSem, const NkSLSemantic &fragmentSem,
		...
		private:
			...
			NkSLResolvedType ValidateBuiltinCall(const NkString &name, const NkVector<NkSLResolvedType> &argTypes,
												 uint32 line);
			NkSLResolvedType ValidateConstructorCall(NkSLBaseType targetType,
			...
			static bool AreCompatibleForOp(const NkString &op, const NkSLResolvedType &lhs,

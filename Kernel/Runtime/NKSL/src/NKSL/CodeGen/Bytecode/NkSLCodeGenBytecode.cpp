...
namespace nkentseu {
	...
	// ── Helpers bas niveau ───────────────────────────────────────────────────
	uint16 NkSLCodeGenBytecode::AllocReg() {
		return mNextReg++;
	}
	...
	static inline uint32 AlignUp(uint32 v, uint32 a) {
		return (v + (a - 1)) & ~(a - 1);
	}

	static inline uint8 CompOf(NkSLBaseType t) {
		return (uint8)NkSLBaseTypeComponents(t);
	}
	...
	void NkSLCodeGenBytecode::CollectSymbols(NkSLProgramNode *ast, NkSLStage stage) {
		...
		if (stage == NkSLStage::NK_VERTEX) {
			...
			s.type = NkSLBaseType::NK_VEC4;
			s.offset = 0;
			s.count = 4;
			mSyms["gl_Position"] = s;
			NkSLVMSymbol sym;
			sym.name = "gl_Position";
			sym.offset = 0;
			sym.count = 4;
			sym.location = 0;
		...
		for (auto *child : ast->children) {
			...
			if (child->kind == NkSLNodeKind::NK_DECL_VAR || child->kind == NkSLNodeKind::NK_DECL_INPUT ||
				...
				if (NkSLTypeIsSampler(bt)) {
					Sym s;
					s.kind = SymKind::SAMPLER;
					s.type = bt;
					s.offset = samplerIdx;
					s.isShadow =
						(bt == NkSLBaseType::NK_SAMPLER2D_SHADOW || bt == NkSLBaseType::NK_SAMPLER_CUBE_SHADOW ||
						 bt == NkSLBaseType::NK_SAMPLER2D_ARRAY_SHADOW);
					if (v->binding.HasBinding())
						s.offset = (uint32)v->binding.binding;
					mSyms[v->name] = s;
					NkSLVMSampler smp;
					smp.name = v->name;
					smp.index = s.offset;
					smp.isShadow = s.isShadow;
			...
			if (child->kind == NkSLNodeKind::NK_DECL_UNIFORM_BLOCK ||
				...
				if (!b->instanceName.Empty()) {
					...
					Sym inst;
					inst.kind = SymKind::UBO_INSTANCE;
					inst.block = blockIdx;
					mSyms[b->instanceName] = inst;
				}
	...
	NkSLOp NkSLCodeGenBytecode::BuiltinOp(const NkString &n) {
		if (n == "dot")
			return NkSLOp::OP_DOT;
		if (n == "any")
			return NkSLOp::OP_ANY;
		if (n == "all")
			return NkSLOp::OP_ALL;
		if (n == "lessThan")
			return NkSLOp::OP_CMP_LT;
		if (n == "greaterThan")
			return NkSLOp::OP_CMP_GT;
		if (n == "lessThanEqual")
			return NkSLOp::OP_CMP_LE;
		if (n == "greaterThanEqual")
			return NkSLOp::OP_CMP_GE;
		if (n == "equal")
			return NkSLOp::OP_CMP_EQ;
		if (n == "notEqual")
			return NkSLOp::OP_CMP_NE;
		if (n == "cross")
			return NkSLOp::OP_CROSS;
		if (n == "normalize")
			return NkSLOp::OP_NORMALIZE;
		if (n == "length")
			return NkSLOp::OP_LENGTH;
		if (n == "distance")
			return NkSLOp::OP_DISTANCE;
		if (n == "reflect")
			return NkSLOp::OP_REFLECT;
		if (n == "mix")
			return NkSLOp::OP_MIX;
		if (n == "lerp")
			return NkSLOp::OP_MIX;
		if (n == "clamp")
			return NkSLOp::OP_CLAMP;
		if (n == "saturate")
			return NkSLOp::OP_SATURATE;
		if (n == "step")
			return NkSLOp::OP_STEP;
		if (n == "smoothstep")
			return NkSLOp::OP_SMOOTHSTEP;
		if (n == "pow")
			return NkSLOp::OP_POW;
		if (n == "sqrt")
			return NkSLOp::OP_SQRT;
		if (n == "inversesqrt")
			return NkSLOp::OP_INVSQRT;
		if (n == "abs")
			return NkSLOp::OP_ABS;
		if (n == "sign")
			return NkSLOp::OP_SIGN;
		if (n == "floor")
			return NkSLOp::OP_FLOOR;
		if (n == "ceil")
			return NkSLOp::OP_CEIL;
		if (n == "fract")
			return NkSLOp::OP_FRACT;
		if (n == "min")
			return NkSLOp::OP_MIN;
		if (n == "max")
			return NkSLOp::OP_MAX;
		if (n == "exp")
			return NkSLOp::OP_EXP;
		if (n == "log")
			return NkSLOp::OP_LOG;
		if (n == "exp2")
			return NkSLOp::OP_EXP2;
		if (n == "log2")
			return NkSLOp::OP_LOG2;
		if (n == "sin")
			return NkSLOp::OP_SIN;
		if (n == "cos")
			return NkSLOp::OP_COS;
		if (n == "tan")
			return NkSLOp::OP_TAN;
		if (n == "asin")
			return NkSLOp::OP_ASIN;
		if (n == "acos")
			return NkSLOp::OP_ACOS;
		if (n == "atan")
			return NkSLOp::OP_ATAN;
		if (n == "radians")
			return NkSLOp::OP_RADIANS;
		if (n == "degrees")
			return NkSLOp::OP_DEGREES;
		if (n == "transpose")
			return NkSLOp::OP_TRANSPOSE;
		if (n == "inverse")
			return NkSLOp::OP_INVERSE;
		return NkSLOp::OP_NOP;
	}

	uint8 NkSLCodeGenBytecode::ConstructorCount(const NkString &n) {
		if (n == "float" || n == "int" || n == "uint" || n == "bool")
			return 1;
		if (n == "vec2" || n == "ivec2")
			return 2;
		if (n == "vec3" || n == "ivec3")
	...
	uint16 NkSLCodeGenBytecode::GenExpr(NkSLNode *n, uint8 *outCount) {
		...
		*outCount = 1;
		if (!n) {
			uint16 r = AllocReg();
			Emit(NkSLOp::OP_LOADK, r, 0, 0, AddConst(NkSLValue::Scalar(0.f)));
			return r;
		}
		...
		switch (n->kind) {
			case NkSLNodeKind::NK_EXPR_LITERAL: {
				...
				float val = (lit->baseType == NkSLBaseType::NK_BOOL) ? (lit->boolVal ? 1.f : 0.f)
							: (lit->baseType == NkSLBaseType::NK_INT || lit->baseType == NkSLBaseType::NK_UINT)
								...
								: (float)lit->floatVal;
				uint16 r = AllocReg();
				Emit(NkSLOp::OP_LOADK, r, 0, 0, AddConst(NkSLValue::Scalar(val)));
				*outCount = 1;
				return r;
			}
			case NkSLNodeKind::NK_EXPR_IDENT: {
				...
				if (id->name == "gl_VertexID") {
					uint16 r = AllocReg();
					Emit(NkSLOp::OP_LOAD_VID, r);
					*outCount = 1;
					return r;
				}
				if (id->name == "gl_InstanceID") {
					uint16 r = AllocReg();
					Emit(NkSLOp::OP_LOAD_IID, r);
					*outCount = 1;
					return r;
				}
				auto *s = mSyms.Find(id->name);
				if (!s) {
					Error(n->line, NkString("symbole inconnu: ") + id->name);
					return AllocReg();
				}
				if (s->kind == SymKind::LOCAL) {
					*outCount = s->count;
					return s->reg;
				}
				if (s->kind == SymKind::INPUT) {
					uint16 r = AllocReg();
					Emit(NkSLOp::OP_LOAD_IN, r, 0, 0, (int32)s->offset, s->count);
					*outCount = s->count;
					return r;
				}
				if (s->kind == SymKind::UNIFORM) {
					uint16 r = AllocReg();
					Emit(NkSLOp::OP_LOAD_UNI, r, 0, s->block, (int32)s->offset, s->count);
					*outCount = s->count;
					return r;
				}
			...
			case NkSLNodeKind::NK_EXPR_MEMBER: {
				...
				if (m->object && m->object->kind == NkSLNodeKind::NK_EXPR_IDENT) {
					...
					if (os && os->kind == SymKind::UBO_INSTANCE) {
						...
						if (!fs)
							fs = mSyms.Find(m->member);
						if (fs) {
							uint16 r = AllocReg();
							Emit(NkSLOp::OP_LOAD_UNI, r, 0, fs->block, (int32)fs->offset, fs->count);
							*outCount = fs->count;
							return r;
						}
					}
				...
				// sinon : swizzle
				uint8 oc;
				uint16 ro = GenExpr(m->object, &oc);
				int32 mask = ParseSwizzle(m->member);
				if (mask < 0) {
					Error(n->line, NkString("membre invalide: ") + m->member);
					*outCount = oc;
					return ro;
				}
				uint16 r = AllocReg();
				uint32 nc = NkSLSwizzleCount(mask);
			...
			case NkSLNodeKind::NK_EXPR_BINARY: {
				...
				uint8 lc, rc;
				uint16 lr = GenExpr(b->left, &lc);
				uint16 rr = GenExpr(b->right, &rc);
				uint16 r = AllocReg();
				NkSLOp op = NkSLOp::OP_ADD;
				uint8 oc = lc >= rc ? lc : rc;
			...
			case NkSLNodeKind::NK_EXPR_UNARY: {
				...
				uint8 oc;
				uint16 rr = GenExpr(u->operand, &oc);
				...
				uint16 r = AllocReg();
				if (u->op == "-")
					Emit(NkSLOp::OP_NEG, r, rr);
				else if (u->op == "!") {
					Emit(NkSLOp::OP_NOT, r, rr);
					oc = 1;
				} else
					Emit(NkSLOp::OP_MOV, r, rr);
				*outCount = oc;
				return r;
			}
			case NkSLNodeKind::NK_EXPR_TERNARY: {
				// cond ? a : b  -> via sauts
				uint8 cc;
				uint16 cr = GenExpr(n->children.Size() > 0 ? n->children[0] : nullptr, &cc);
				uint16 res = AllocReg();
				Emit(NkSLOp::OP_JZ, 0, cr, 0, 0);
				int32 jzPos = (int32)mProg->code.Size() - 1;
				uint8 ac;
				uint16 ar = GenExpr(n->children.Size() > 1 ? n->children[1] : nullptr, &ac);
				Emit(NkSLOp::OP_MOV, res, ar);
				Emit(NkSLOp::OP_JMP, 0, 0, 0, 0);
				int32 jmpPos = (int32)mProg->code.Size() - 1;
				mProg->code[jzPos].imm = (int32)mProg->code.Size();
				uint8 bc;
				uint16 br = GenExpr(n->children.Size() > 2 ? n->children[2] : nullptr, &bc);
			...
			case NkSLNodeKind::NK_EXPR_CALL: {
				...
				if (name == "texture" || name == "textureLod") {
					...
					uint8 uc;
					uint16 uvr = GenExpr(call->args.Size() > 1 ? call->args[1] : nullptr, &uc);
					uint16 r = AllocReg();
					if (shadow) {
						Emit(NkSLOp::OP_TEX_SAMPLE_SHADOW, r, uvr, 0, samp);
						*outCount = 1;
					} else {
						Emit(NkSLOp::OP_TEX_SAMPLE, r, uvr, 0, samp);
						*outCount = 4;
					}
					return r;
				}
				if (name == "textureSize") {
					...
					uint16 r = AllocReg();
					Emit(NkSLOp::OP_TEX_SIZE, r, 0, 0, samp);
					*outCount = 2;
					return r;
				}
				...
				if (ctorCount) {
					...
					for (auto *a : call->args) {
						uint8 ac;
						uint16 ar = GenExpr(a, &ac);
						(void)ac;
						argRegs.PushBack(ar);
					}
					// 2. Bloc de registres CONTIGUS : MOV chaque arg dedans.
					uint16 base = mNextReg;
					for (usize i = 0; i < argRegs.Size(); ++i) {
						uint16 dst = AllocReg();
						Emit(NkSLOp::OP_MOV, dst, argRegs[i]);
					}
					uint16 r = AllocReg();
				...
				if (bop != NkSLOp::OP_NOP) {
					uint8 c0 = 1, c1 = 1, c2 = 1;
					uint16 r0 = 0, r1 = 0, r2 = 0;
					if (call->args.Size() > 0)
						r0 = GenExpr(call->args[0], &c0);
					if (call->args.Size() > 1)
						r1 = GenExpr(call->args[1], &c1);
					if (call->args.Size() > 2)
						r2 = GenExpr(call->args[2], &c2);
					uint16 r = AllocReg();
					// 3 args : 3e opérande passé via imm (=registre)
					if (call->args.Size() > 2)
						Emit(bop, r, r0, r1, (int32)r2);
					else
						Emit(bop, r, r0, r1);
					...
					*outCount = (bop == NkSLOp::OP_DOT || bop == NkSLOp::OP_LENGTH || bop == NkSLOp::OP_DISTANCE ||
								 bop == NkSLOp::OP_ANY || bop == NkSLOp::OP_ALL)
									? 1
									: c0;
					return r;
				}
				...
				if (name == "dFdx" || name == "dFdy" || name == "fwidth" || name == "isnan" || name == "isinf") {
					uint8 ac = 1;
					if (call->args.Size() > 0) {
						uint16 tmp = GenExpr(call->args[0], &ac);
						(void)tmp;
					}
					NkSLValue z;
					z.count = ac ? ac : 1;
					for (uint8 i = 0; i < z.count; ++i)
						z.v[i] = (name == "fwidth") ? 1e-3f : 0.f; // isnan/isinf → false(0)
					int32 k = AddConst(z);
					uint16 r = AllocReg();
					Emit(NkSLOp::OP_LOADK, r, 0, 0, k);
					*outCount = z.count;
					return r;
				}
				...
				if (NkSLFunctionDeclNode *fn = FindFunc(name)) {
					NkVector<uint16> argRegs;
					NkVector<uint8> argCounts;
					for (auto *a : call->args) {
						uint8 ac;
						uint16 ar = GenExpr(a, &ac);
						argRegs.PushBack(ar);
						argCounts.PushBack(ac);
					}
					...
					GenStmt(fn->body);
					{
						...
						for (int32 j : fr.retJumps)
							mProg->code[(usize)j].imm = (int32)mProg->code.Size();
					}
					mInline.PopBack();
			...
			case NkSLNodeKind::NK_EXPR_INDEX: {
				...
				uint8 ac;
				uint16 ar = GenExpr(ix->array, &ac);
				uint8 ic;
				uint16 ir = GenExpr(ix->index, &ic);
				uint16 r = AllocReg();
				Emit(NkSLOp::OP_INDEX, r, ar, ir);
				*outCount = (ac == 16) ? 4 : (ac == 9 ? 3 : 1);
				return r;
			}
			case NkSLNodeKind::NK_EXPR_ASSIGN: {
				auto *as = static_cast<NkSLAssignNode *>(n);
				uint8 rc;
				uint16 rr = GenExpr(as->rhs, &rc);
				GenAssign(as->lhs, rr, rc);
				*outCount = rc;
				return rr;
			}
			default:
	...
	void NkSLCodeGenBytecode::GenAssign(NkSLNode *lhs, uint16 rhsReg, uint8 rhsCount) {
		...
		if (lhs->kind == NkSLNodeKind::NK_EXPR_IDENT) {
			...
			auto *s = mSyms.Find(id->name);
			if (!s) {
				Error(lhs->line, NkString("affectation symbole inconnu: ") + id->name);
				return;
			}
			if (s->kind == SymKind::OUTPUT) {
				Emit(NkSLOp::OP_STORE_OUT, 0, rhsReg, 0, (int32)s->offset, s->count);
				return;
			}
			if (s->kind == SymKind::LOCAL) {
				Emit(NkSLOp::OP_MOV, s->reg, rhsReg);
				return;
			}
			Error(lhs->line, NkString("affectation non supportée: ") + id->name);
			return;
		}
	...
	void NkSLCodeGenBytecode::GenStmt(NkSLNode *n) {
		...
		switch (n->kind) {
			...
			case NkSLNodeKind::NK_DECL_VAR: {
				...
				if (v->initializer) {
					uint8 ic;
					uint16 ir = GenExpr(v->initializer, &ic);
					Emit(NkSLOp::OP_MOV, reg, ir);
					s.count = ic;
				}
				mSyms[v->name] = s;
				break;
			...
			case NkSLNodeKind::NK_STMT_IF: {
				...
				uint8 cc;
				uint16 cr = GenExpr(f->condition, &cc);
				Emit(NkSLOp::OP_JZ, 0, cr, 0, 0);
				int32 jz = (int32)mProg->code.Size() - 1;
				GenStmt(f->thenBranch);
				if (f->elseBranch) {
					Emit(NkSLOp::OP_JMP, 0, 0, 0, 0);
					int32 jmp = (int32)mProg->code.Size() - 1;
			...
			case NkSLNodeKind::NK_STMT_FOR: {
				...
				if (fr->condition) {
					uint8 cc;
					uint16 cr = GenExpr(fr->condition, &cc);
					Emit(NkSLOp::OP_JZ, 0, cr, 0, 0);
					jz = (int32)mProg->code.Size() - 1;
				}
				GenStmt(fr->body);
				if (fr->increment) {
					uint8 c;
					GenExpr(fr->increment, &c);
				}
			...
			case NkSLNodeKind::NK_STMT_RETURN: {
				if (!mInline.Empty()) {
					...
					if (ret->value) {
						uint8 rc;
						uint16 rr = GenExpr(ret->value, &rc);
	...
	bool NkSLCodeGenBytecode::Generate(NkSLProgramNode *ast, NkSLStage stage, NkSLByteProgram &out,
		...
		if (!entry || !entry->body) {
			Error(0, "VM: fonction d'entrée introuvable");
			return false;
		}

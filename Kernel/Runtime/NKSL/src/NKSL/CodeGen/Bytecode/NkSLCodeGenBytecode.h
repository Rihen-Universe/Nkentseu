...
namespace nkentseu {
	...
	class NkSLCodeGenBytecode {
		public:
			...
			bool Generate(NkSLProgramNode *ast, NkSLStage stage, NkSLByteProgram &out,
		...
		private:
			...
			struct SymTable {
					...
					Sym &operator[](const NkString &n) {
						for (usize i = names.Size(); i-- > 0;)
							if (names[i] == n)
								return syms[i];
						names.PushBack(n);
						syms.PushBack(Sym{});
						return syms[syms.Size() - 1];
					}
					...
					void Push(const NkString &n, const Sym &s) {
						names.PushBack(n);
						syms.PushBack(s);
					} // toujours ajouter (scope)

					usize Size() const {
						return names.Size();
					}

					void Truncate(usize sz) {
						while (names.Size() > sz) {
							names.PopBack();
							syms.PopBack();
						}

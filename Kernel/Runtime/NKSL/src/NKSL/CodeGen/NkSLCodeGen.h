...
namespace nkentseu {
	...
	class NkSLCodeGenBase {
		...
		protected:
			...
			void Emit(const NkString &s) {
				mOutput += s;
			}

			void EmitLine(const NkString &s) {
				mOutput += Indent() + s + "\n";
			}
			...
			void AddError(uint32 l, const NkString &m) {
				mErrors.PushBack({l, 0, "", m, true});
			}

			void AddWarning(uint32 l, const NkString &m) {
				mWarnings.PushBack({l, 0, "", m, false});
			}
	...
	inline const char *NkSLImageFormatToHLSLElem(const NkString &fmt) {
		if (fmt == "r32f" || fmt == "r16f")
			return "float";
		if (fmt == "rg32f" || fmt == "rg16f")
			return "float2";
		if (fmt == "r32i" || fmt == "r16i" || fmt == "r8i")
			return "int";
		if (fmt == "r32ui" || fmt == "r16ui" || fmt == "r8ui")
			return "uint";
		if (fmt == "rgba32i" || fmt == "rgba16i" || fmt == "rgba8i")
			return "int4";
		if (fmt == "rgba32ui" || fmt == "rgba16ui" || fmt == "rgba8ui")
			return "uint4";
	...
	inline const char *NkSLImageFormatToMSLElem(const NkString &fmt) {
		if (fmt == "r32i" || fmt == "r16i" || fmt == "r8i" || fmt == "rgba32i" || fmt == "rgba16i" || fmt == "rgba8i")
			return "int";
		if (fmt == "r32ui" || fmt == "r16ui" || fmt == "r8ui" || fmt == "rgba32ui" || fmt == "rgba16ui" ||
			fmt == "rgba8ui")
			return "uint";
	...
	class NkSLCodeGenGLSL : public NkSLCodeGenBase {
		public:
			...
			NkSLCompileResult Generate(NkSLProgramNode *ast, NkSLStage stage, const NkSLCompileOptions &opts) override;
		...
		private:
			...
			// Réécrit `inst.membre` d'un push_constant en accès `_PushConstants[...]` (GL).
			NkString RewritePushMember(const NkString &name);
			...
			int mAutoBinding = 0;
			// Détection pour le Y-flip GL (réplique NkShaderConvert::flip_vert_y) :
			// on NE flippe PAS les VS depth-only (pas de varying out, ex. shadow) ni
			// 2D-pur-PC (push_constant sans UBO, ex. Render2D). Renseignés en pré-passe.
			bool mHasVaryingOut = false;
			bool mHasUBO = false;
			bool mHasPushConst = false;
			bool mHasInputs = false; // VS a des attributs in (sinon pas de flip)

			// Émulation push_constant GL : le device GL écrit les push constants via
			// glUniform4fv vers `uniform vec4 _PushConstants[N]` (NkOpenglCommandBuffer.h).
			// On émet ce tableau et on réécrit les accès `inst.membre` → _PushConstants[...].
			struct PushMember {
					NkString name;
					uint32 byteOffset = 0;
			...
			NkString mPushInstance; // nom de l'instance (ex "pc"), vide si aucun
			NkVector<PushMember> mPushMembers;
			uint32 mPushVec4Count = 0; // taille du tableau _PushConstants
	...
	class NkSLCodeGenGLSLVulkan : public NkSLCodeGenBase {
		...
		private:
			...
			NkString LayoutQualifier(const NkSLBinding &b, bool isBlock, bool isStorage, bool isPushConstant,
	...
	class NkSLCodeGenHLSL_DX11 : public NkSLCodeGenBase {
		...
		private:
			...
			NkString SemanticFor(NkSLVarDeclNode *v, NkSLStage stage, bool isInput, bool isFragOut, int autoIndex);
			...
			// Vrai pendant le corps de l'entry point quand main retourne une struct
			// (PS_Output/VS_Output) : un `return;` nu doit devenir `return output;`.
			bool mInEntryReturnsStruct = false;
	...
	class NkSLCodeGenHLSL_DX12 : public NkSLCodeGenBase {
		...
		private:
			...
			NkVector<NkString> mMatrixNames;
			bool mWritesDepth = false;
			// Vrai pendant le corps de l'entry point (main retourne une struct) → `return;` nu
			// devient `return output;`.
			bool mInEntryReturnsStruct = false;
	...
	class NkSLCodeGenMSLSpirvCross : public NkSLCodeGenBase {
		public:
			...
			NkSLCompileResult Generate(NkSLProgramNode *ast, NkSLStage stage, const NkSLCompileOptions &opts) override;
			NkSLCompileResult GenerateFromSPIRV(const NkVector<uint32> &spirvWords, NkSLStage stage,
												const NkSLCompileOptions &opts);

		private:
			NkSLCompileResult SPIRVToMSL(const NkVector<uint32> &spirvWords, NkSLStage stage,
										 const NkSLCompileOptions &opts);
	};
	...
	class NkSLReflector {
		public:
			...
			NkString GenerateLayoutCPP(const NkSLReflection &reflection, const NkString &varName = "layout");

...
void NkSLCodeGenGLSL::GenProgram(NkSLProgramNode* prog) {
    ...
    mPushInstance = ""; mPushMembers.Clear(); mPushVec4Count = 0;
        ...
        if (child->kind == NkSLNodeKind::NK_DECL_UNIFORM_BLOCK ||
            ...
            if (isPush) {
                ...
                for (auto* m : b->members) {
                    ...
                    uint32 al, sz; std140(m->type->baseType, al, sz);
            ...
            if (v->storage == NkSLStorageQual::NK_OUT)           mHasVaryingOut = true;
            if (v->storage == NkSLStorageQual::NK_IN)            mHasInputs     = true;
            if (v->storage == NkSLStorageQual::NK_PUSH_CONSTANT) mHasPushConst  = true;
        }
    }
...
void NkSLCodeGenGLSL::GenVarDecl(NkSLVarDeclNode* v, bool isGlobal) {
    ...
    NkString typeStr = (v->type->baseType == NkSLBaseType::NK_STRUCT)
                       ? v->type->typeName
                       : BaseTypeToString(v->type->baseType);
    line += typeStr + " " + v->name;
        ...
        switch (pm.type) {
            case NkSLBaseType::NK_MAT4:
                snprintf(buf, sizeof(buf),
                    "mat4(_PushConstants[%u], _PushConstants[%u], _PushConstants[%u], _PushConstants[%u])",
                    vi, vi+1, vi+2, vi+3);
                return NkString(buf);
            case NkSLBaseType::NK_MAT3:
                snprintf(buf, sizeof(buf),
                    "mat3(_PushConstants[%u].xyz, _PushConstants[%u].xyz, _PushConstants[%u].xyz)",
                    vi, vi+1, vi+2);
                return NkString(buf);
            case NkSLBaseType::NK_VEC4:
                snprintf(buf, sizeof(buf), "_PushConstants[%u]", vi); return NkString(buf);
            case NkSLBaseType::NK_VEC3:
                snprintf(buf, sizeof(buf), "_PushConstants[%u].xyz", vi); return NkString(buf);
            case NkSLBaseType::NK_VEC2:
                snprintf(buf, sizeof(buf), "_PushConstants[%u].xy", vi); return NkString(buf);
            default: {
                const char* sw = (comp==0)?"x":(comp==1)?"y":(comp==2)?"z":"w";
                snprintf(buf, sizeof(buf), "_PushConstants[%u].%s", vi, sw); return NkString(buf);
            }
        }
    }
    return "";
...
void NkSLCodeGenGLSL::GenBlock(NkSLBlockDeclNode* b) {
    ...
    if ((b->storage == NkSLStorageQual::NK_PUSH_CONSTANT ||
         b->kind == NkSLNodeKind::NK_DECL_PUSH_CONSTANT) && mPushVec4Count > 0) {
        char buf[64];
...
void NkSLCodeGenGLSL::GenFunction(NkSLFunctionDeclNode* fn) {
    ...
    const bool flipY = mOpts && mOpts->glFlipYPosition &&
                       mStage == NkSLStage::NK_VERTEX &&
                       (fn->isEntry || fn->name == "main") &&
                       fn->body && fn->body->kind == NkSLNodeKind::NK_STMT_BLOCK &&
                       mHasInputs && !purePC && !depthOnly;
    if (flipY) {
        EmitLine("{");
        IndentPush();
        for (auto* c : fn->body->children) GenStmt(c);
...
NkString NkSLCodeGenGLSL::GenExpr(NkSLNode* node) {
    ...
    switch (node->kind) {
        ...
        case NkSLNodeKind::NK_EXPR_MEMBER: {
            ...
            if (!mPushInstance.Empty() && m->object &&
                m->object->kind == NkSLNodeKind::NK_EXPR_IDENT &&

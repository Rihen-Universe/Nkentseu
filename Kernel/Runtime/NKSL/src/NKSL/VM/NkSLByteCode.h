...
namespace nkentseu {
	...
	struct NkSLValue {
			...
			static NkSLValue Scalar(float x) {
				NkSLValue r;
				r.count = 1;
				r.v[0] = x;
				return r;
	...
	enum class NkSLOp : uint16 {
		...
		OP_NOT,
		OP_ANY,
		OP_ALL, // réduction bvecN → scalaire (any/all)
		OP_CMP_LT,
		OP_CMP_GT,
		OP_CMP_LE,
		OP_CMP_GE,
		OP_CMP_EQ,
		OP_CMP_NE, // comparaisons composante (lessThan…)
		OP_LOAD_VID,
		OP_LOAD_IID, // gl_VertexID / gl_InstanceID (fournis par l'hôte)
	...
	inline uint32 NkSLSwizzleCount(int32 mask) {
		return (uint32)((mask >> 8) & 7);
	}

	inline uint8 NkSLSwizzleIdx(int32 mask, uint32 i) {
		return (uint8)((mask >> (i * 2)) & 3);
	}

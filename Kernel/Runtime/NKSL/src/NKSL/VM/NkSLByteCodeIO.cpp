...
namespace nkentseu {
	...
	struct BcReader {
			...
			NkString Str() {
				...
				if (!Need(len))
					return s;
				s = NkString((const char *)(p + off), (usize)len);
				off += len;
				return s;
			}
	...
	bool NkSLByteCodeSerialize(const NkSLByteProgram &prog, NkVector<uint8> &out) {
		...
		for (const auto &k : prog.constants) {
			w.U8(k.count);
			for (uint8 i = 0; i < k.count; ++i)
				w.F32(k.v[i]);
		}
		// inputs / outputs / uniforms
		w.U32((uint32)prog.inputs.Size());
		for (const auto &s : prog.inputs)
			w.Sym(s);
		w.U32((uint32)prog.outputs.Size());
		for (const auto &s : prog.outputs)
			w.Sym(s);
		w.U32((uint32)prog.uniforms.Size());
		for (const auto &s : prog.uniforms)
			w.Sym(s);
		// samplers
		...
		for (const auto &b : prog.uboBlocks) {
			w.U16(b.set);
			w.U16(b.binding);
		}
		return true;
	...
	bool NkSLByteCodeDeserialize(const uint8 *data, usize size, NkSLByteProgram &prog) {
		...
		for (uint32 i = 0; i < nk && r.ok; ++i) {
			NkSLValue v;
			v.count = r.U8();
			for (uint8 j = 0; j < v.count; ++j)
				v.v[j] = r.F32();
			prog.constants.PushBack(v);
		}
		uint32 ni = r.U32();
		for (uint32 i = 0; i < ni && r.ok; ++i)
			prog.inputs.PushBack(r.Sym());
		uint32 no = r.U32();
		for (uint32 i = 0; i < no && r.ok; ++i)
			prog.outputs.PushBack(r.Sym());
		uint32 nu = r.U32();
		for (uint32 i = 0; i < nu && r.ok; ++i)
			prog.uniforms.PushBack(r.Sym());
		uint32 ns = r.U32();
		for (uint32 i = 0; i < ns && r.ok; ++i) {
			NkSLVMSampler s;
			s.name = r.Str();
			s.index = r.U32();
			s.isShadow = r.U8() != 0;
			prog.samplers.PushBack(s);
		}
	...
	bool NkSLByteCodeSaveFile(const NkSLByteProgram &prog, const char *path) {
		NkVector<uint8> blob;
		if (!NkSLByteCodeSerialize(prog, blob))
			return false;
	...
	bool NkSLByteCodeLoadFile(const char *path, NkSLByteProgram &prog) {
		...
		if (sz <= 0)
			return false;
		NkVector<uint8> blob;
		blob.Resize((usize)sz);
		if (f.Read(blob.Data(), (usize)sz) != (usize)sz)
			return false;

...
NkString NkSLCodeGenHLSL::GenCall(NkSLCallNode* call) {
    ...
    // textureLod(s, uv, lod) → s_tex.SampleLevel(s_smp, uv, lod)  [IBL/HDR préfiltré]
    if (name == "textureLod" && call->args.Size() >= 3) {
        NkString t = GenExpr(call->args[0]);
        return t + "_tex.SampleLevel(" + t + "_smp, " + GenExpr(call->args[1]) +
               ", " + GenExpr(call->args[2]) + ")";
    }
    // textureGrad(s, uv, dPdx, dPdy) → s_tex.SampleGrad(s_smp, uv, dPdx, dPdy)
    if (name == "textureGrad" && call->args.Size() >= 4) {
        NkString t = GenExpr(call->args[0]);
        return t + "_tex.SampleGrad(" + t + "_smp, " + GenExpr(call->args[1]) +
               ", " + GenExpr(call->args[2]) + ", " + GenExpr(call->args[3]) + ")";
    }
    // texelFetch(s, coord, lod) → s_tex.Load(int3(coord, lod))  [2D]
    if (name == "texelFetch" && call->args.Size() >= 2) {
        NkString t   = GenExpr(call->args[0]);
        NkString lod = (call->args.Size() >= 3) ? GenExpr(call->args[2]) : NkString("0");
        return t + "_tex.Load(int3(" + GenExpr(call->args[1]) + ", " + lod + "))";
    }

...
namespace nkentseu {
	...
	static bool ResolveKind(const NkString &name, NkShaderAnnotationKind &out) {
		if (name == "param") {
			out = NkShaderAnnotationKind::NK_PARAM;
			return true;
		}
		if (name == "color") {
			out = NkShaderAnnotationKind::NK_COLOR;
			return true;
		}
		if (name == "range") {
			out = NkShaderAnnotationKind::NK_RANGE;
			return true;
		}
		if (name == "texture2D") {
			out = NkShaderAnnotationKind::NK_TEXTURE_2D;
			return true;
		}
		if (name == "cubemap") {
			out = NkShaderAnnotationKind::NK_TEXTURE_CUBE;
			return true;
		}
		if (name == "texture3D") {
			out = NkShaderAnnotationKind::NK_TEXTURE_3D;
			return true;
		}
		if (name == "textureArray") {
			out = NkShaderAnnotationKind::NK_TEXTURE_ARRAY;
			return true;
		}
		if (name == "enum") {
			out = NkShaderAnnotationKind::NK_ENUM;
			return true;
		}
		if (name == "group") {
			out = NkShaderAnnotationKind::NK_GROUP;
			return true;
		}
		if (name == "hidden") {
			out = NkShaderAnnotationKind::NK_HIDDEN;
			return true;
		}
		if (name == "material") {
			out = NkShaderAnnotationKind::NK_MATERIAL;
			return true;
		}
		if (name == "stage") {
			out = NkShaderAnnotationKind::NK_STAGE;
			return true;
		}
		if (name == "entry") {
			out = NkShaderAnnotationKind::NK_ENTRY;
			return true;
		}
		// Alias types natifs
		if (name == "float") {
			out = NkShaderAnnotationKind::NK_TYPE_FLOAT;
			return true;
		}
		if (name == "int") {
			out = NkShaderAnnotationKind::NK_TYPE_INT;
			return true;
		}
		if (name == "bool") {
			out = NkShaderAnnotationKind::NK_TYPE_BOOL;
			return true;
		}
		if (name == "vec2") {
			out = NkShaderAnnotationKind::NK_TYPE_VEC2;
			return true;
		}
		if (name == "vec3") {
			out = NkShaderAnnotationKind::NK_TYPE_VEC3;
			return true;
		}
		if (name == "vec4") {
			out = NkShaderAnnotationKind::NK_TYPE_VEC4;
			return true;
		}
		if (name == "ivec2") {
			out = NkShaderAnnotationKind::NK_TYPE_IVEC2;
			return true;
		}
		if (name == "ivec3") {
			out = NkShaderAnnotationKind::NK_TYPE_IVEC3;
			return true;
		}
		if (name == "ivec4") {
			out = NkShaderAnnotationKind::NK_TYPE_IVEC4;
			return true;
		}
		if (name == "mat3") {
			out = NkShaderAnnotationKind::NK_TYPE_MAT3;
			return true;
		}
		if (name == "mat4") {
			out = NkShaderAnnotationKind::NK_TYPE_MAT4;
			return true;
		}
		return false;
	}
	...
	static const char *GlslTypeForKind(NkShaderAnnotationKind k) {
		switch (k) {
			case NkShaderAnnotationKind::NK_TYPE_FLOAT:
				return "float";
			case NkShaderAnnotationKind::NK_TYPE_INT:
				return "int";
			case NkShaderAnnotationKind::NK_TYPE_BOOL:
				return "bool";
			case NkShaderAnnotationKind::NK_TYPE_VEC2:
				return "vec2";
			case NkShaderAnnotationKind::NK_TYPE_VEC3:
				return "vec3";
			case NkShaderAnnotationKind::NK_TYPE_VEC4:
				return "vec4";
			case NkShaderAnnotationKind::NK_TYPE_IVEC2:
				return "ivec2";
			case NkShaderAnnotationKind::NK_TYPE_IVEC3:
				return "ivec3";
			case NkShaderAnnotationKind::NK_TYPE_IVEC4:
				return "ivec4";
			case NkShaderAnnotationKind::NK_TYPE_MAT3:
				return "mat3";
			case NkShaderAnnotationKind::NK_TYPE_MAT4:
				return "mat4";
			default:
				return nullptr;
		}
	...
	static bool ParseValue(const char *s, uint32 len, uint32 &i, NkShaderAnnotationValue &out) {
		...
		if (s[i] == '(') {
			...
			out.f[3] = vals[3];
			if (count == 2)
				out.type = NkShaderAnnotationValue::Type::NK_VEC2;
			else if (count == 3)
				out.type = NkShaderAnnotationValue::Type::NK_VEC3;
			else if (count == 4)
				out.type = NkShaderAnnotationValue::Type::NK_VEC4;
			else {
		...
		if (IsIdentStart(s[i])) {
			...
			if (ident == "true") {
				out.type = NkShaderAnnotationValue::Type::NK_BOOL;
				out.b = true;
				return true;
			}
			if (ident == "false") {
	...
	static void ApplyArgs(const NkVector<ArgKV> &args, NkShaderAnnotation &a) {
		for (uint32 idx = 0; idx < args.Size(); ++idx) {
			...
			if (kv.key == "name") {
				...
				a.defaultValue = kv.val;
			} else if (kv.key == "slot") {
				if (kv.val.type == NkShaderAnnotationValue::Type::NK_INT)
					a.slot = (uint32)kv.val.i;
			} else if (kv.key == "srgb") {
				if (kv.val.type == NkShaderAnnotationValue::Type::NK_BOOL)
					a.srgb = kv.val.b;
			} else if (kv.key == "type") {
				if (kv.val.type == NkShaderAnnotationValue::Type::NK_STRING)
					a.glslType = kv.val.s;
	...
	// Parser principal
	// =========================================================================
	NkShaderAnnotationResult NkShaderAnnotationParser::Parse(const NkString &rawSource, NkSLStage hintStage) {
						...
						if (!ResolveKind(annotName, kind)) {
							...
							if (kind == NkShaderAnnotationKind::NK_MATERIAL) {
								...
								if (!a.name.Empty()) {
									NkString stg = a.name;
									if (stg == "vertex")
										res.metadata.stage = NkSLStage::NK_VERTEX;
									else if (stg == "fragment")
										res.metadata.stage = NkSLStage::NK_FRAGMENT;
									else if (stg == "compute")
										res.metadata.stage = NkSLStage::NK_COMPUTE;
									else if (stg == "geometry")
										res.metadata.stage = NkSLStage::NK_GEOMETRY;
									res.metadata.stageExplicit = true;
								}
							} else if (kind == NkShaderAnnotationKind::NK_ENTRY) {
								if (!a.name.Empty())
									res.metadata.entryPoint = a.name;
							} else {

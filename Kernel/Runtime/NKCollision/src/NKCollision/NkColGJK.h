...
namespace nkentseu {
	namespace collision {
		...
		inline NkVec3f NkSupportCore3D(const NkShape &s, const NkVec3f &d) noexcept {
			switch (s.type) {
				...
				case NkShapeType::NK_BOX3D: {
					...
					NkVec3f ls{ld.x < 0.f ? -s.p1.x : s.p1.x, ld.y < 0.f ? -s.p1.y : s.p1.y,
							   ld.z < 0.f ? -s.p1.z : s.p1.z};
				...
				case NkShapeType::NK_CYLINDER3D: {
					...
					if (rl2 > 1e-12f)
						cap = cap + radial * (s.radius / math::NkSqrt(rl2));
					return cap;
				}
				case NkShapeType::NK_CONE3D: {
					...
					NkVec3f rim = s.p0;
					if (rl2 > 1e-12f)
						rim = s.p0 + radial * (s.radius / math::NkSqrt(rl2));
		...
		inline NkVec2f NkSupportCore2D(const NkShape &s, const NkVec2f &d) noexcept {
			...
			switch (s.type) {
				...
				case NkShapeType::NK_BOX2D: {
					...
					return c + ux * (d.Dot(ux) < 0.f ? -s.p1.x : s.p1.x) + uy * (d.Dot(uy) < 0.f ? -s.p1.y : s.p1.y);
				}
		...
		inline NkVec3f NkShapeCenter3D(const NkShape &s) noexcept {
			if (s.type == NkShapeType::NK_CAPSULE3D)
				return (s.p0 + s.p1) * 0.5f;
			if (s.type == NkShapeType::NK_CONE3D)
				return s.p0 + s.p1 * (s.height * 0.5f);
			if ((s.type == NkShapeType::NK_TRIANGLE3D || s.type == NkShapeType::NK_CONVEX3D) && s.verts &&
				s.vertCount) {
				NkVec3f sum{};
				for (uint32 i = 0; i < s.vertCount; ++i)
					sum = sum + s.verts[i];
		...
		inline NkVec2f NkShapeCenter2D(const NkShape &s) noexcept {
			...
			if ((s.type == NkShapeType::NK_TRIANGLE2D || s.type == NkShapeType::NK_POLYGON2D) && s.verts &&
				s.vertCount) {
				NkVec2f sum{};
				for (uint32 i = 0; i < s.vertCount; ++i)
					sum = sum + NkVec2f{s.verts[i].x, s.verts[i].y};
				return sum * (1.f / (float32)s.vertCount);
			}
		...
		namespace detail {
			...
			struct NkSV3 {
					NkVec3f v;
					NkVec3f a;
			}; // point Minkowski cœur (A-B) + témoin sur A
			...
			inline NkVec3f NkClosestTri3D(const NkVec3f &a, const NkVec3f &b, const NkVec3f &c, float32 &u, float32 &v,
										  float32 &w) noexcept {
				...
				NkVec3f bp = b * -1.f;
				float32 d3 = ab.Dot(bp), d4 = ac.Dot(bp);
				if (d3 >= 0.f && d4 <= d3) {
					u = 0.f;
					v = 1.f;
					w = 0.f;
				...
				NkVec3f cp = c * -1.f;
				float32 d5 = ab.Dot(cp), d6 = ac.Dot(cp);
				if (d6 >= 0.f && d5 <= d6) {
					u = 0.f;
					v = 0.f;
					w = 1.f;
				...
				if (va <= 0.f && (d4 - d3) >= 0.f && (d5 - d6) >= 0.f) {
					float32 t = (d4 - d3) / ((d4 - d3) + (d5 - d6));
					u = 0.f;
					v = 1.f - t;
					w = t;
					return b + (c - b) * t;
				}
				float32 den = 1.f / (va + vb + vc);
				float32 vv = vb * den, ww = vc * den;
				u = 1.f - vv - ww;
				v = vv;
				w = ww;
			...
			inline NkVec3f NkClosestSimplex3D(NkSV3 *s, int32 &n, NkVec3f &wA, bool &intersect) noexcept {
				...
				if (n == 2) {
					...
					wA = s[0].a * (1.f - t) + s[1].a * t;
					return a + ab * t;
				}
				if (n == 3) {
					float32 u, v, w;
					NkVec3f c = NkClosestTri3D(s[0].v, s[1].v, s[2].v, u, v, w);
				...
				for (int32 f = 0; f < 4; ++f) {
					...
					if (refSide * oriSide < 0.f) { // origine du côté opposé à D -> dehors
						anyOutside = true;
						float32 u, v, w;
						NkVec3f c = NkClosestTri3D(A, B, C, u, v, w);
						float32 d2 = c.Dot(c);
						if (d2 < bestD2) {
							bestD2 = d2;
							bestC = c;
			...
			inline bool NkDoSimplex3D(NkSV3 *s, int32 &n, NkVec3f &dir) noexcept {
				...
				if (n == 2) {
					...
					if (ab.Dot(ao) > 0.f) {
						dir = NkTriple3(ab, ao, ab);
						if (dir.Dot(dir) < 1e-12f) {
							dir = NkVec3f{-ab.y, ab.x, 0.f};
							if (dir.Dot(dir) < 1e-12f)
								dir = NkVec3f{0.f, -ab.z, ab.y};
						}
					} else {
						s[0] = s[1];
						n = 1;
				...
				if (n == 3) {
					...
					if (abc.Cross(ac).Dot(ao) > 0.f) {
						if (ac.Dot(ao) > 0.f) {
							s[1] = s[2];
							n = 2;
							dir = NkTriple3(ac, ao, ac);
						} else {
							s[0] = s[1];
							s[1] = s[2];
							n = 2;
							return NkDoSimplex3D(s, n, dir);
						}
			...
			inline bool NkGJKBool3D(const NkShape &A, const NkShape &B, NkSV3 *s, int32 &n) noexcept {
				...
				if (dir.Dot(dir) < 1e-12f)
					dir = NkVec3f{1.f, 0.f, 0.f};
				s[0] = NkMinkCore3D(A, B, dir);
				n = 1;
				dir = s[0].v * -1.f;
			...
			inline NkGJKRes3 NkGJKDist3D(const NkShape &A, const NkShape &B) noexcept {
				...
				s[0] = NkMinkCore3D(A, B, dir);
				n = 1;
				NkVec3f closest = s[0].v;
				r.witnessA = s[0].a;
				for (int32 iter = 0; iter < 64; ++iter) {
					if (closest.Dot(closest) < 1e-10f) {
						r.intersect = true;
						break;
					}
					dir = closest * -1.f;
					...
					if (closest.Dot(closest) - closest.Dot(w.v) <= 1e-8f * closest.Dot(closest))
						break;
					s[n++] = w;
					bool inter = false;
					NkVec3f wA;
				...
				r.dist = math::NkSqrt(closest.Dot(closest));
				r.closest = closest;
				r.n = n;
				for (int32 i = 0; i < n; ++i)
					r.simplex[i] = s[i];
			...
			inline bool NkEPACore3D(const NkShape &A, const NkShape &B, NkSV3 *simplex, int32 n, NkVec3f &normalOut,
									float32 &depthOut, NkVec3f &witnessAOut) noexcept {
				if (n < 4)
					return false;
				NkVector<NkVec3f> verts;
				NkVector<NkVec3f> wits;
				NkVector<NkFace> faces;
				for (int32 i = 0; i < 4; ++i) {
					verts.PushBack(simplex[i].v);
					wits.PushBack(simplex[i].a);
				}
				...
				for (int32 iter = 0; iter < 48; ++iter) {
					...
					if (p.v.Dot(best.n) - best.dist < 1e-4f)
						break;
					const int32 ip = (int32)verts.Size();
					verts.PushBack(p.v);
					wits.PushBack(p.a);
					...
					for (uint32 i = 0; i < faces.Size();) {
						...
						if (f.n.Dot(p.v - verts[(uint32)f.a]) > 1e-6f) {
							...
							for (int32 k = 0; k < 3; ++k) {
								const int32 e0 = e[k * 2], e1 = e[k * 2 + 1];
								bool found = false;
								for (uint32 m = 0; m < edges.Size(); m += 2)
									if (edges[m] == e1 && edges[m + 1] == e0) {
										edges[m] = edges[edges.Size() - 2];
										edges[m + 1] = edges[edges.Size() - 1];
										edges.PopBack();
										edges.PopBack();
										found = true;
										break;
					...
					if (edges.Size() == 0)
						break;
					for (uint32 m = 0; m < edges.Size(); m += 2)
						addFace(edges[m], edges[m + 1], ip);
					if (faces.Size() == 0)
						break;
				}
				NkVec3f proj = best.n * best.dist;
				const NkVec3f &A0 = verts[(uint32)best.a];
				const NkVec3f &B0 = verts[(uint32)best.b];
				const NkVec3f &C0 = verts[(uint32)best.c];
		...
		inline bool NkGJKEPA3D(const NkShape &A, const NkShape &B, NkManifold3D &m) noexcept {
			...
			if (NkGJKBool3D(A, B, bsimplex, bn)) {
				...
				if (!NkEPACore3D(A, B, bsimplex, bn, normal, depth, witnessA)) {
					...
					float32 l = math::NkSqrt(cc.Dot(cc));
					normal = (l > 1e-6f) ? cc * (1.f / l) : NkVec3f{0, 1, 0};
					depth = 1e-4f;
					witnessA = NkShapeCenter3D(A);
				}
				depth += tot;
			} else {
			...
			NkVec3f cc = NkShapeCenter3D(B) - NkShapeCenter3D(A);
			if (normal.Dot(cc) < 0.f)
				normal = normal * -1.f; // cohérent A->B
			m.normal = normal;
			m.points[0].point = witnessA + normal * (mA - depth * 0.5f);
			m.points[0].depth = depth;
			m.count = 1;
			return true;
		}
		...
		namespace detail {
			...
			NK_FORCE_INLINE float32 NkCross2(const NkVec2f &a, const NkVec2f &b) noexcept {
			...
			inline NkVec2f NkClosestSimplex2D(NkSV2 *s, int32 &n, NkVec2f &wA, bool &intersect) noexcept {
				...
				if (n == 2) {
					NkVec2f a = s[0].v, b = s[1].v, ab = b - a;
					float32 den = ab.Dot(ab);
					float32 t = (den > 1e-12f) ? math::NkClamp((a * -1.f).Dot(ab) / den, 0.f, 1.f) : 0.f;
					if (t <= 0.f) {
						n = 1;
				...
				for (int32 e = 0; e < 3; ++e) {
					NkVec2f p = s[E[e][0]].v, q = s[E[e][1]].v, pq = q - p;
					float32 den = pq.Dot(pq);
					float32 t = (den > 1e-12f) ? math::NkClamp((p * -1.f).Dot(pq) / den, 0.f, 1.f) : 0.f;
					NkVec2f cpt = p + pq * t;
					float32 d2 = cpt.Dot(cpt);
					if (d2 < bestD2) {
						bestD2 = d2;
						bestC = cpt;
						i0 = E[e][0];
			...
			inline NkGJKRes2 NkGJKDist2D(const NkShape &A, const NkShape &B) noexcept {
				...
				for (int32 iter = 0; iter < 64; ++iter) {
					...
					bool inter = false;
					NkVec2f wA;
					closest = NkClosestSimplex2D(s, n, wA, inter);
					r.witnessA = wA;
					if (inter) {
						r.intersect = true;
						break;
			...
			inline bool NkEPACore2D(const NkShape &A, const NkShape &B, NkSV2 *simplex, int32 n, NkVec2f &normalOut,
									float32 &depthOut, NkVec2f &witnessAOut) noexcept {
				if (n < 3)
					return false;
				NkVector<NkVec2f> poly;
				NkVector<NkVec2f> wits;
				for (int32 i = 0; i < 3; ++i) {
					poly.PushBack(simplex[i].v);
					wits.PushBack(simplex[i].a);
				}
				float32 area = 0.f;
				for (uint32 i = 0; i < poly.Size(); ++i)
					area += NkCross2(poly[i], poly[(i + 1) % poly.Size()]);
				if (area < 0.f) {
					NkVector<NkVec2f> rp, rw;
					for (uint32 i = poly.Size(); i > 0; --i) {
						rp.PushBack(poly[i - 1]);
						rw.PushBack(wits[i - 1]);
					}
					poly = rp;
					wits = rw;
				}
				for (int32 iter = 0; iter < 48; ++iter) {
					...
					for (uint32 i = 0; i < poly.Size(); ++i) {
						...
						NkVec2f e = poly[j] - poly[i];
						NkVec2f nrm{e.y, -e.x};
						const float32 l = math::NkSqrt(nrm.Dot(nrm));
						if (l > 1e-12f)
							nrm = nrm * (1.f / l);
					...
					if (p.v.Dot(en) - edist < 1e-4f) {
						...
						NkVec2f e = poly[j] - poly[ei];
						const float32 el2 = e.Dot(e);
						float32 t =
							(el2 > 1e-12f) ? math::NkClamp((en * edist - poly[ei]).Dot(e) / el2, 0.f, 1.f) : 0.f;
					...
					for (uint32 i = 0; i <= ei; ++i) {
						np.PushBack(poly[i]);
						nw.PushBack(wits[i]);
					}
					np.PushBack(p.v);
					nw.PushBack(p.a);
					for (uint32 i = ei + 1; i < poly.Size(); ++i) {
						np.PushBack(poly[i]);
						nw.PushBack(wits[i]);
					}
					poly = np;
		...
		inline bool NkGJKEPA2D(const NkShape &A, const NkShape &B, NkManifold2D &m) noexcept {
			...
			if (NkGJKBool2D(A, B, bsimplex, bn)) {
				if (!NkEPACore2D(A, B, bsimplex, bn, normal, depth, witnessA)) {
					...
					float32 l = math::NkSqrt(cc.Dot(cc));
					normal = (l > 1e-6f) ? cc * (1.f / l) : NkVec2f{0, 1};
					depth = 1e-4f;
					witnessA = NkShapeCenter2D(A);
				}
				depth += tot;
			} else {

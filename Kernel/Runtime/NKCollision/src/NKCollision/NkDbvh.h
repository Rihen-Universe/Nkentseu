		...
		class NkDbvh {
			public:
				...
				int32 Insert(const NkAABB3D &tight, uint32 userId) {
					...
					mNodes[(uint32)leaf].child1 = kNull;
					mNodes[(uint32)leaf].child2 = kNull;
					InsertLeaf(leaf);
					return leaf;
				}
				...
				bool Update(int32 proxy, const NkAABB3D &tight) {
					if (proxy < 0)
						return false;
					if (NkAABBContains(mNodes[(uint32)proxy].aabb, tight))
						return false;
					RemoveLeaf(proxy);
				...
				uint32 UserId(int32 proxy) const noexcept {
					return mNodes[(uint32)proxy].userId;
				}
				...
				void Query(const NkAABB3D &box, NkVector<uint32> &out) const {
					...
					while (stack.Size() > 0) {
						const int32 i = stack[stack.Size() - 1];
						stack.PopBack();
						if (i == kNull)
							continue;
						const NkDbvhNode &nd = mNodes[(uint32)i];
						if (!nd.aabb.Overlaps(box))
							continue;
						if (nd.IsLeaf())
							out.PushBack(nd.userId);
						else {
							stack.PushBack(nd.child1);
							stack.PushBack(nd.child2);
						}
					}
				}
				...
				void RayCast(const NkRay3D &ray, NkVector<uint32> &out) const {
					...
					while (stack.Size() > 0) {
						...
						const NkDbvhNode &nd = mNodes[(uint32)i];
						NkRayHit3D h;
						if (!NkRayAABBSlab(ray, nd.aabb, h))
							continue;
						if (nd.IsLeaf())
							out.PushBack(nd.userId);
						else {
				...
				int32 Height() const noexcept {
					return mRoot == kNull ? 0 : mNodes[(uint32)mRoot].height;
				}
			...
			private:
				struct NkDbvhNode {
						...
						NK_FORCE_INLINE bool IsLeaf() const noexcept {
							return child1 == kNull;
						}
				};
				...
				static bool NkRayAABBSlab(const NkRay3D &ray, const NkAABB3D &bx, NkRayHit3D &hit) noexcept {
					...
					for (int32 a = 0; a < 3; ++a) {
						...
						const float32 inv = 1.f / dv;
						float32 t1 = (lo - o) * inv, t2 = (hi - o) * inv;
						if (t1 > t2) {
							float32 t = t1;
							t1 = t2;
							t2 = t;
				...
				void FreeNode(int32 i) {
					mNodes[(uint32)i].parent = mFree;
					mNodes[(uint32)i].height = -1;
					mFree = i;
				}
				...
				void InsertLeaf(int32 leaf) {
					if (mRoot == kNull) {
						mRoot = leaf;
						mNodes[(uint32)leaf].parent = kNull;
						return;
					}
					...
					while (!mNodes[(uint32)idx].IsLeaf()) {
						...
						auto descend = [&](int32 c) {
							float32 cc;
							if (mNodes[(uint32)c].IsLeaf())
								cc = NkAABBArea(NkAABBUnion(leafBox, mNodes[(uint32)c].aabb)) + inherit;
							else {
								const float32 old = NkAABBArea(mNodes[(uint32)c].aabb);
					...
					if (oldParent != kNull) {
						if (mNodes[(uint32)oldParent].child1 == sibling)
							mNodes[(uint32)oldParent].child1 = newParent;
						else
							mNodes[(uint32)oldParent].child2 = newParent;
					} else
						mRoot = newParent;
				...
				void RemoveLeaf(int32 leaf) {
					...
					const int32 sibling = (mNodes[(uint32)parent].child1 == leaf) ? mNodes[(uint32)parent].child2
																				  : mNodes[(uint32)parent].child1;
					if (grand != kNull) {
						if (mNodes[(uint32)grand].child1 == parent)
							mNodes[(uint32)grand].child1 = sibling;
						else
							mNodes[(uint32)grand].child2 = sibling;
						mNodes[(uint32)sibling].parent = grand;
						FreeNode(parent);
						RefitFrom(grand);
					} else {
						mRoot = sibling;
						mNodes[(uint32)sibling].parent = kNull;
						FreeNode(parent);
					}
				}
				...
				void RefitFrom(int32 i) {
					while (i != kNull) {
						...
						mNodes[(uint32)i].height =
							1 + math::NkMax(mNodes[(uint32)c1].height, mNodes[(uint32)c2].height);
				...
				int32 Balance(int32 iA) noexcept {
					...
					if (bal > 1) { // remonter C
						...
						mNodes[(uint32)iC].child1 = iA;
						mNodes[(uint32)iC].parent = A.parent;
						A.parent = iC;
						if (mNodes[(uint32)iC].parent != kNull) {
							if (mNodes[(uint32)mNodes[(uint32)iC].parent].child1 == iA)
								mNodes[(uint32)mNodes[(uint32)iC].parent].child1 = iC;
							else
								mNodes[(uint32)mNodes[(uint32)iC].parent].child2 = iC;
						} else
							mRoot = iC;
						if (mNodes[(uint32)iF].height > mNodes[(uint32)iG].height) {
							mNodes[(uint32)iC].child2 = iF;
							A.child2 = iG;
							mNodes[(uint32)iG].parent = iA;
							...
							mNodes[(uint32)iC].child2 = iG;
							A.child2 = iF;
							mNodes[(uint32)iF].parent = iA;
					...
					if (bal < -1) { // remonter B
						...
						mNodes[(uint32)iB].child1 = iA;
						mNodes[(uint32)iB].parent = A.parent;
						A.parent = iB;
						if (mNodes[(uint32)iB].parent != kNull) {
							if (mNodes[(uint32)mNodes[(uint32)iB].parent].child1 == iA)
								mNodes[(uint32)mNodes[(uint32)iB].parent].child1 = iB;
							else
								mNodes[(uint32)mNodes[(uint32)iB].parent].child2 = iB;
						} else
							mRoot = iB;
						if (mNodes[(uint32)iD].height > mNodes[(uint32)iE].height) {
							mNodes[(uint32)iB].child2 = iD;
							A.child1 = iE;
							mNodes[(uint32)iE].parent = iA;
							...
							mNodes[(uint32)iB].child2 = iE;
							A.child1 = iD;
							mNodes[(uint32)iD].parent = iA;

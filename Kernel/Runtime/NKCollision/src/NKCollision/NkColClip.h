	...
	namespace collision {
		...
		inline NkPoly2 NkBuildPoly2(const NkShape &s) noexcept {
			...
			if (s.type == NkShapeType::NK_BOX2D) {
				...
				for (int32 i = 0; i < n; ++i)
					p.v[i] = {s.verts[i].x, s.verts[i].y};
				p.count = n;
				// assurer CCW (aire signée > 0)
				float32 area = 0.f;
				for (int32 i = 0; i < n; ++i) {
					const NkVec2f &a = p.v[i];
					const NkVec2f &b = p.v[(i + 1) % n];
		...
		inline float32 NkMaxSeparation2(const NkPoly2 &A, const NkPoly2 &B, int32 &edgeOut) noexcept {
			...
			for (int32 i = 0; i < A.count; ++i) {
				...
				for (int32 j = 0; j < B.count; ++j)
					minDot = math::NkMin(minDot, n.Dot(B.v[j] - v));
				if (minDot > best) {
					best = minDot;
					edgeOut = i;
				}
		...
		inline int32 NkClipSeg2(NkVec2f *io, const NkVec2f &n, float32 offset) noexcept {
			...
			if (d1 <= 0.f)
				out[c++] = io[1];
			if (d0 * d1 < 0.f && c < 2) {
				const float32 t = d0 / (d0 - d1);
				out[c++] = io[0] + (io[1] - io[0]) * t;
			}
			io[0] = out[0];
			if (c > 1)
				io[1] = out[1];
		...
		inline bool NkCollidePolygons2D(const NkShape &sa, const NkShape &sb, NkManifold2D &m) noexcept {
			...
			for (int32 i = 0; i < inc->count; ++i) {
				const float32 d = refN.Dot(inc->n[i]);
				if (d < minDot) {
					minDot = d;
					incEdge = i;
				}
			...
			if (NkClipSeg2(seg, tangent * -1.f, (tangent * -1.f).Dot(rv1)) < 2)
				return false;
			if (NkClipSeg2(seg, tangent, tangent.Dot(rv2)) < 2)
				return false;
		...
		inline NkBoxFrame NkBoxFrameOf(const NkShape &s) noexcept {
			NkBoxFrame f;
			f.c = s.p0;
			f.u[0] = s.orientation * NkVec3f{1, 0, 0};
			f.u[1] = s.orientation * NkVec3f{0, 1, 0};
			f.u[2] = s.orientation * NkVec3f{0, 0, 1};
			f.e[0] = s.p1.x;
			f.e[1] = s.p1.y;
			f.e[2] = s.p1.z;
			return f;
		...
		inline int32 NkClipPolyPlane3(NkVec3f *poly, int32 count, const NkVec3f &n, float32 offset) noexcept {
			...
			for (int32 i = 0; i < count && c < 8; ++i) {
				const NkVec3f &A = poly[i];
				const NkVec3f &B = poly[(i + 1) % count];
				const float32 da = n.Dot(A) - offset, db = n.Dot(B) - offset;
				if (da <= 0.f)
					out[c++] = A;
				if (da * db < 0.f && c < 8) {
					const float32 t = da / (da - db);
					out[c++] = A + (B - A) * t;
				}
			}
			for (int32 i = 0; i < c; ++i)
		...
		inline bool NkCollideBoxBox3D(const NkShape &sa, const NkShape &sb, NkManifold3D &m) noexcept {
			NkManifold3D epa;
			if (!NkGJKEPA3D(sa, sb, epa))
				return false;			  // normale + 1 point (repli)
			...
			float32 bestA = -1.f;
			for (int32 i = 0; i < 3; ++i) {
				float32 d = math::NkAbs(A.u[i].Dot(n));
				if (d > bestA) {
					bestA = d;
					ai = i;
				}
			...
			float32 bestB = -1.f;
			for (int32 i = 0; i < 3; ++i) {
				float32 d = math::NkAbs(B.u[i].Dot(n));
				if (d > bestB) {
					bestB = d;
					bi = i;
				}
			...
			const NkBoxFrame *ref;
			const NkBoxFrame *inc;
			int32 refAxis;
			NkVec3f rn;
			if (bestA >= bestB) {
				ref = &A;
				inc = &B;
				refAxis = ai;
				rn = A.u[ai] * (A.u[ai].Dot(n) < 0.f ? -1.f : 1.f);
			} // rn ~ +n (A->B)
			else {
				ref = &B;
				inc = &A;
				refAxis = bi;
				rn = B.u[bi] * (B.u[bi].Dot(n) < 0.f ? 1.f : -1.f);
			} // rn ~ -n (B->A)

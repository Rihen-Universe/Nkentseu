...
namespace nkentseu {
	namespace collision {
		...
		NK_FORCE_INLINE float32 NkOBB2DProjRadius(const NkVec2f &h, const NkVec2f &u, const NkVec2f &v,
												  const NkVec2f &axis) noexcept {
			return h.x * math::NkAbs(u.Dot(axis)) + h.y * math::NkAbs(v.Dot(axis));
		}

		// OBB 2D vs OBB 2D (SAT, 4 axes). Manifold normale A->B + profondeur.
		inline bool NkOBB2DvsOBB2D(const NkVec2f &c0, const NkVec2f &h0, float32 r0, const NkVec2f &c1,
								   const NkVec2f &h1, float32 r1, NkManifold2D &m) noexcept {
			NkVec2f u0, v0, u1, v1;
			NkOBB2DAxes(r0, u0, v0);
			NkOBB2DAxes(r1, u1, v1);
			const NkVec2f d = c1 - c0;
			const NkVec2f axes[4] = {u0, v0, u1, v1};
			float32 minOverlap = 1e30f;
			NkVec2f minAxis{1.f, 0.f};
			for (int32 i = 0; i < 4; ++i) {
			...
			if (d.Dot(minAxis) < 0.f)
				minAxis = minAxis * -1.f;
			m.normal = minAxis;
			m.points[0].depth = minOverlap;
		...
		inline bool NkOBB2DvsCircle(const NkVec2f &bc, const NkVec2f &bh, float32 brot, const NkVec2f &sc, float32 sr,
									NkManifold2D &m) noexcept {
			NkVec2f u, v;
			NkOBB2DAxes(brot, u, v);
			NkVec2f rel = sc - bc;

...
namespace nkentseu {
	namespace collision {
		...
		struct NkShape {
				...
				const NkVec3f *verts =
					nullptr; // triangle/polygone/convexe (z=0 en 2D) · sommets trimesh · sommets chain2D
				uint32 vertCount = 0;
				...
				static NkShape Segment2D(const NkVec2f &a, const NkVec2f &b) noexcept {
					NkShape s;
					s.type = NkShapeType::NK_SEGMENT2D;
					s.p0 = {a.x, a.y, 0.f};
					s.p1 = {b.x, b.y, 0.f};
					s.radius = 0.f;
					return s;
				...
				static NkShape Circle2D(const NkVec2f &c, float32 r) noexcept {
					NkShape s;
					s.type = NkShapeType::NK_CIRCLE2D;
					s.p0 = {c.x, c.y, 0.f};
					s.radius = r;
					return s;
				}
				...
				static NkShape Box2D(const NkVec2f &center, const NkVec2f &halfExtents, float32 rot = 0.f) noexcept {
					NkShape s;
					s.type = NkShapeType::NK_BOX2D;
					s.p0 = {center.x, center.y, 0.f};
					s.p1 = {halfExtents.x, halfExtents.y, 0.f};
					s.rotation = rot;
					return s;
				}
				...
				static NkShape Triangle2D(const NkVec3f *verts3) noexcept {
					NkShape s;
					s.type = NkShapeType::NK_TRIANGLE2D;
					s.verts = verts3;
					s.vertCount = 3;
					s.radius = 0.f;
					return s;
				...
				static NkShape Polygon2D(const NkVec3f *verts, uint32 n) noexcept {
					NkShape s;
					s.type = NkShapeType::NK_POLYGON2D;
					s.verts = verts;
					s.vertCount = n;
					s.radius = 0.f;
					return s;
				...
				static NkShape OBB3D(const NkVec3f &center, const NkVec3f &halfExtents,
									 const nkentseu::math::NkQuatf &orient) noexcept {
					NkShape s;
					s.type = NkShapeType::NK_BOX3D;
					s.p0 = center;
					s.p1 = halfExtents;
				...
				static NkShape Capsule3D(const NkVec3f &a, const NkVec3f &b, float32 r) noexcept {
					NkShape s;
					s.type = NkShapeType::NK_CAPSULE3D;
					s.p0 = a;
					s.p1 = b;
					s.radius = r;
					return s;
				...
				static NkShape Cylinder3D(const NkVec3f &center, const NkVec3f &axisUnit, float32 halfHeight,
										  float32 r) noexcept {
					NkShape s;
					s.type = NkShapeType::NK_CYLINDER3D;
					s.p0 = center;
					s.p1 = axisUnit;
					s.height = halfHeight;
					s.radius = r;
				...
				static NkShape Cone3D(const NkVec3f &baseCenter, const NkVec3f &axisUnit, float32 fullHeight,
									  float32 baseRadius) noexcept {
					NkShape s;
					s.type = NkShapeType::NK_CONE3D;
					s.p0 = baseCenter;
					s.p1 = axisUnit;
					s.height = fullHeight;
					s.radius = baseRadius;
				...
				static NkShape Triangle3D(const NkVec3f *verts3) noexcept {
					NkShape s;
					s.type = NkShapeType::NK_TRIANGLE3D;
					s.verts = verts3;
					s.vertCount = 3;
					s.radius = 0.f;
					return s;
				...
				static NkShape Convex3D(const NkVec3f *verts, uint32 n) noexcept {
					NkShape s;
					s.type = NkShapeType::NK_CONVEX3D;
					s.verts = verts;
					s.vertCount = n;
					s.radius = 0.f;
					return s;
				...
				static NkShape Trimesh3D(const NkVec3f *verts, uint32 vertN, const uint32 *indices,
										 uint32 indexN) noexcept {
					NkShape s;
					s.type = NkShapeType::NK_TRIMESH3D;
					s.verts = verts;
					s.vertCount = vertN;
					s.indices = indices;
					s.indexCount = indexN;
				...
				static NkShape Heightfield3D(const NkVec3f &origin, float32 cellX, float32 cellZ,
					...
					NkShape s;
					s.type = NkShapeType::NK_HEIGHTFIELD3D;
					s.p0 = origin;
					s.p1 = {cellX, 0.f, cellZ};
					s.heights = heights;
					s.rows = rows;
					s.cols = cols;
					s.radius = 0.f;
				...
				static NkShape Compound(const NkShape *children, uint32 n, const NkVec3f &offset = {}) noexcept {
					NkShape s;
					s.type = NkShapeType::NK_COMPOUND;
					s.children = children;
					s.childCount = n;
					s.p0 = offset;
					s.radius = 0.f;
		...
		NK_FORCE_INLINE NkAABB3D NkComputeAABB3D(const NkShape &s) noexcept {
			...
			switch (s.type) {
				...
				case NkShapeType::NK_CYLINDER3D: {
					...
					NkVec3f e{s.height * math::NkAbs(a.x) + s.radius * math::NkSqrt(math::NkMax(0.f, 1.f - a.x * a.x)),
				...
				case NkShapeType::NK_CONE3D: {
					...
					const NkVec3f &a = s.p1;
					NkVec3f er{s.radius * math::NkSqrt(math::NkMax(0.f, 1.f - a.x * a.x)),
					...
					box.min = {math::NkMin(s.p0.x - er.x, apex.x), math::NkMin(s.p0.y - er.y, apex.y),
							   math::NkMin(s.p0.z - er.z, apex.z)};
					box.max = {math::NkMax(s.p0.x + er.x, apex.x), math::NkMax(s.p0.y + er.y, apex.y),
							   math::NkMax(s.p0.z + er.z, apex.z)};
					break;
				}
				...
				case NkShapeType::NK_HEIGHTFIELD3D: {
					...
					if (s.heights && s.rows * s.cols) {
						ymin = ymax = s.heights[0];
						for (uint32 i = 1; i < s.rows * s.cols; ++i) {
							ymin = math::NkMin(ymin, s.heights[i]);
							ymax = math::NkMax(ymax, s.heights[i]);
						}
					}
				...
				case NkShapeType::NK_COMPOUND: {
					if (s.children && s.childCount) {
						...
						for (uint32 i = 1; i < s.childCount; ++i) {
							...
							box.Expand(ci.min + s.p0);
							box.Expand(ci.max + s.p0);
						}
					} else {
						box.min = s.p0;
						box.max = s.p0;
			...
			switch (s.type) {
				...
				case NkShapeType::NK_CHAIN2D: {
					if (s.verts && s.vertCount) {
						...
						box.min = box.max = v0;
						for (uint32 i = 1; i < s.vertCount; ++i)
							box.Expand({s.verts[i].x, s.verts[i].y});
					} else {
						box.min = c;
						box.max = c;
					}

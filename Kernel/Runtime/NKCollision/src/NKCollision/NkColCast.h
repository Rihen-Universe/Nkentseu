...
namespace nkentseu {
	namespace collision {
		...
		namespace detail {
			...
			NK_FORCE_INLINE NkSV3 NkMinkOff3D(const NkShape &A, const NkVec3f &offA, const NkShape &B,
											  const NkVec3f &d) noexcept {
			...
			inline void NkGJKDistOff3D(const NkShape &A, const NkVec3f &offA, const NkShape &B, float32 &dist,
									   NkVec3f &closest, NkVec3f &witnessA, bool &intersect) noexcept {
				intersect = false;
				...
				for (int32 iter = 0; iter < 64; ++iter) {
					...
					if (c.Dot(c) - c.Dot(w.v) <= 1e-8f * c.Dot(c))
						break;
					s[n++] = w;
					bool inter = false;
					NkVec3f wA;
					c = NkClosestSimplex3D(s, n, wA, inter);
					witnessA = wA;
					if (inter) {
						intersect = true;
						break;
		...
		inline bool NkConvexCast3D(const NkShape &A, const NkVec3f &dir, float32 maxDist, const NkShape &B,
			...
			for (int32 iter = 0; iter < 32; ++iter) {
				float32 dist;
				NkVec3f closest, witnessA;
				bool intersect;
				NkGJKDistOff3D(A, dir * t, B, dist, closest, witnessA, intersect);
				if (intersect) {
					tOut = t;
					normalOut = (dir.Dot(dir) > 0.f) ? dir.Normalized() * -1.f : NkVec3f{0, 1, 0};
					pointOut = witnessA;
					return true;
				}
				...
				if (sep <= 1e-4f) {
					tOut = t;
					normalOut = n;
					pointOut = witnessA - n * NkShapeMargin(A);
					return true;
				}
				const float32 denom = -dir.Dot(n); // progression de A vers B le long de dir
				if (denom <= 1e-6f)
					return false; // ne s'approche pas -> rate
				t += sep / denom;
				if (t > maxDist)
					return false;
			}
		...
		inline bool NkRayConvex3D(const NkRay3D &ray, const NkShape &shape, NkRayHit3D &hit) noexcept {
			...
			float32 t;
			NkVec3f nrm, p;
			if (!NkConvexCast3D(pt, ray.dir, ray.maxT, shape, t, nrm, p))
				return false;
			hit.hit = true;
			hit.t = t;

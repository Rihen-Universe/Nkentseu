	...
	namespace collision {
		...
		class NkWorld {
			public:
				...
				uint32 AddBody(const NkShape &shape, uint32 layer = 0x1u, uint32 mask = 0xFFFFFFFFu,
							   void *user = nullptr);
				...
				const NkVector<NkCollisionPair> &Pairs() const noexcept {
					return mPairs;
				}

				// Événements calculés par Step() (comparaison avec la frame précédente).
				const NkVector<NkCollisionEvent> &EnterEvents() const noexcept {
					return mEnter;
				}

				const NkVector<NkCollisionEvent> &StayEvents() const noexcept {
					return mStay;
				}

				const NkVector<NkCollisionEvent> &ExitEvents() const noexcept {
					return mExit;
				}
				...
				void SetTrigger(uint32 id, bool t) {
				...
				bool ShapeCast(const NkShape &shape, const NkVec3f &dir, float32 maxDist, NkRayHit3D &hit,
				...
				bool SweepBody(uint32 id, const NkVec3f &translation, NkRayHit3D &hit, uint32 mask = 0xFFFFFFFFu) const;

				uint32 BodyCount() const noexcept {

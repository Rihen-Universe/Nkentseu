...
namespace nkentseu {
	namespace collision {
		...
		inline bool NkConvexVsTrimesh3D(const NkShape &mesh, const NkShape &convex, NkManifold3D &out) noexcept {
			if (!mesh.verts || !mesh.indices || mesh.indexCount < 3)
				return false;
			const NkAABB3D cb = NkComputeAABB3D(convex);
			bool any = false;
			float32 bestDepth = -1.f;
			for (uint32 i = 0; i + 2 < mesh.indexCount; i += 3) {
				NkVec3f tv[3] = {mesh.verts[mesh.indices[i]], mesh.verts[mesh.indices[i + 1]],
		...
		inline bool NkConvexVsHeightfield3D(const NkShape &hf, const NkShape &convex, NkManifold3D &out) noexcept {
			if (!hf.heights || hf.rows < 2 || hf.cols < 2)
				return false;
		...
		inline bool NkConvexVsChain2D(const NkShape &chain, const NkShape &convex, NkManifold2D &out) noexcept {
			if (!chain.verts || chain.vertCount < 2)
				return false;
			...
			for (uint32 i = 0; i + 1 < chain.vertCount; ++i) {
				...
				NkAABB2D sb;
				sb.min = {math::NkMin(a.x, b.x), math::NkMin(a.y, b.y)};
				sb.max = {math::NkMax(a.x, b.x), math::NkMax(a.y, b.y)};
				if (!cb.Overlaps(sb))
					continue;

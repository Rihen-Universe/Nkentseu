	...
	namespace collision {
		...
		static NkShape NkTranslateShape(const NkShape &s, const NkVec3f &off) noexcept {
			...
			if (s.type == NkShapeType::NK_CAPSULE3D || s.type == NkShapeType::NK_SEGMENT2D ||
				s.type == NkShapeType::NK_CAPSULE2D)
				r.p1 = s.p1 + off; // p1 = 2e extrémité
			return r;
		}
		...
		static bool NkCompoundNarrow(const NkShape &A, const NkShape &B, NkManifold3D &out) noexcept {
			...
			for (uint32 i = 0; i < comp.childCount; ++i) {
				...
				if (NkShapeIs2D(X.type) != NkShapeIs2D(Y.type))
					continue;
				NkManifold3D m;
				const bool hit = NkShapeIs2D(X.type) ? NkNarrow2D(X, Y, m) : NkNarrow3D(X, Y, m);
				if (hit && m.points[0].depth > best) {
					best = m.points[0].depth;
					out = m;
					any = true;
				}
			}
		...
		static bool NkNarrow3D(const NkShape &A, const NkShape &B, NkManifold3D &m) noexcept {
			...
			if (A.type == T::NK_TRIMESH3D && NkShapeIsConvex(B.type))
				return NkConvexVsTrimesh3D(A, B, m);
			if (B.type == T::NK_TRIMESH3D && NkShapeIsConvex(A.type)) {
				if (!NkConvexVsTrimesh3D(B, A, m))
					return false;
				m.normal = m.normal * -1.f;
				return true;
			}
			if (A.type == T::NK_HEIGHTFIELD3D && NkShapeIsConvex(B.type))
				return NkConvexVsHeightfield3D(A, B, m);
			if (B.type == T::NK_HEIGHTFIELD3D && NkShapeIsConvex(A.type)) {
				if (!NkConvexVsHeightfield3D(B, A, m))
					return false;
				m.normal = m.normal * -1.f;
				return true;
			}
			...
			if (ta == T::NK_SPHERE && tb == T::NK_SPHERE)
				...
				hit = NkSphereBox(a->p0, a->radius, b->p0, b->p1, m),
				m.normal = m.normal * -1.f; // box->sphere -> A(sphere)->B(box)
			...
			if (hit && swapped)
				m.normal = m.normal * -1.f; // rétablir A->B d'origine
			return hit;
		}
		...
		static bool NkNarrow2D(const NkShape &A, const NkShape &B, NkManifold3D &out) noexcept {
			...
			if (A.type == T::NK_CHAIN2D && NkShapeIsConvex(B.type)) {
				NkManifold2D m;
				if (!NkConvexVsChain2D(A, B, m))
					return false;
				NkFill3DFrom2D(m, out);
				return true;
			}
			if (B.type == T::NK_CHAIN2D && NkShapeIsConvex(A.type)) {
				NkManifold2D m;
				if (!NkConvexVsChain2D(B, A, m))
					return false;
				m.normal = m.normal * -1.f;
				NkFill3DFrom2D(m, out);
				return true;
			...
			if (NkShapeIsPoly2(A.type) && NkShapeIsPoly2(B.type)) {
				NkManifold2D pm;
				if (!NkCollidePolygons2D(A, B, pm))
					return false;
				NkFill3DFrom2D(pm, out);
				return true;
			}
			...
			if (ta == T::NK_CIRCLE2D && tb == T::NK_CIRCLE2D)
				...
				NkManifold2D gm;
				if (!NkGJKEPA2D(A, B, gm))
					return false; // A,B d'origine (pas de swap)
		...
		bool NkWorld::Narrow(const NkShape &A, const NkShape &B, NkManifold3D &out) noexcept {
			...
			if (a2 != b2)
				return false; // pas de mixte 2D/3D
		...
		void NkWorld::SetShape(uint32 id, const NkShape &s) {
			if (NkBody *b = GetBody(id)) {
				b->shape = s;
				mTree.Update(b->proxy, NkBodyAABB(s));
			} // garder le DBVH à jour
		}
		...
		void NkWorld::Step() {
			...
			for (uint32 i = 0; i < n; ++i) {
				...
				for (uint32 c = 0; c < (uint32)cand.Size(); ++c) {
					...
					if (candId <= A.id)
						continue; // self + dédup (traité côté plus petit id)
					NkBody *Bp = GetBody(candId);
					if (!Bp || !Bp->active)
						continue;
					const NkBody &B = *Bp;
					if (!((A.layer & B.mask) && (B.layer & A.mask)))
						continue; // filtre layers
					if (!aabbA.Overlaps(NkBodyAABB(B.shape)))
						continue; // AABB serrée
					NkManifold3D m;
			...
			for (uint32 k = 0; k < (uint32)mPrevKeys.Size(); ++k) {
				...
				bool still = false;
				for (uint32 c = 0; c < (uint32)curKeys.Size(); ++c)
					if (curKeys[c] == pk) {
						still = true;
						break;
					}
				if (!still) {
					NkCollisionEvent ev{(uint32)(pk >> 32), (uint32)(pk & 0xFFFFFFFFu)};
					mExit.PushBack(ev);
				}
			}
			mPrevKeys = curKeys;
		...
		bool NkWorld::Raycast3D(const NkRay3D &ray, NkRayHit3D &hit, uint32 mask) const {
			...
			best.t = ray.maxT;
			NkVector<uint32> cand;
			mTree.RayCast(ray, cand); // préfiltre DBVH (AABB traversées)
			for (uint32 i = 0; i < (uint32)cand.Size(); ++i) {
				const NkBody *bp = GetBody(cand[i]);
				if (!bp)
					continue;
				const NkBody &b = *bp;
				if (!b.active || NkShapeIs2D(b.shape.type) || !(b.layer & mask))
					continue;
				NkRayHit3D h;
				bool ok = false;
				...
				switch (s.type) {
					case NkShapeType::NK_SPHERE:
						ok = NkRaySphere(ray, s.p0, s.radius, h);
						break;
					case NkShapeType::NK_BOX3D:
						ok = NkRayOBB3D(ray, s.p0, s.p1, s.orientation, h);
						break; // OBB exact (identité = AABB)
					case NkShapeType::NK_PLANE3D:
						ok = NkRayPlane3D(ray, s.p0, s.p1, h);
						break;
					case NkShapeType::NK_TRIANGLE3D:
						if (s.verts && s.vertCount >= 3)
							ok = NkRayTriangle3D(ray, s.verts[0], s.verts[1], s.verts[2], h);
						break;
					case NkShapeType::NK_TRIMESH3D: { // triangle le plus proche
						if (s.verts && s.indices) {
							...
							for (uint32 t = 0; t + 2 < s.indexCount; t += 3)
								if (NkRayTriangle3D(ray, s.verts[s.indices[t]], s.verts[s.indices[t + 1]],
													s.verts[s.indices[t + 2]], th) &&
									th.t < bt) {
									bt = th.t;
									h = th;
									ok = true;
					...
					default: { // capsule/cylindre/cône/convexe -> ray-cast GJK exact
						if (NkShapeIsConvex(s.type))
							ok = NkRayConvex3D(ray, s, h);
						else {
							NkAABB3D a = NkComputeAABB3D(s);
							ok = NkRayAABB3D(ray, a.min, a.max, h);
						} // concave non géré -> AABB
						break;
					}
				}
				if (ok && h.t < best.t) {
		...
		bool NkWorld::Raycast2D(const NkRay2D &ray, NkRayHit2D &hit, uint32 mask) const {
			...
			for (uint32 i = 0; i < (uint32)mBodies.Size(); ++i) {
				...
				if (b.shape.type == NkShapeType::NK_CIRCLE2D) {
					if (NkRayCircle2D(ray, {b.shape.p0.x, b.shape.p0.y}, b.shape.radius, h) && h.t < best.t) {
						best = h;
						found = true;
					}
				} else {
					...
					NkRayHit3D h3;
					if (NkRayAABB3D(r3, {a.min.x, a.min.y, -0.01f}, {a.max.x, a.max.y, 0.01f}, h3) && h3.t < best.t) {
						best.hit = true;
						best.t = h3.t;
						best.point = {h3.point.x, h3.point.y};
						best.normal = {h3.normal.x, h3.normal.y};
						found = true;
					}
				}
			}
		...
		uint32 NkWorld::Overlap(const NkShape &s, NkVector<uint32> &out, uint32 mask) const {
			...
			NkVector<uint32> cand;
			mTree.Query(sab, cand); // broadphase DBVH O(log n)
			for (uint32 c = 0; c < (uint32)cand.Size(); ++c) {
				const NkBody *bp = GetBody(cand[c]);
				if (!bp || !bp->active || !(bp->layer & mask))
					continue;
				if (NkShapeIs2D(bp->shape.type) != s2)
					continue; // pas de mixte 2D/3D
				NkManifold3D m;
				if (Narrow(s, bp->shape, m))
					out.PushBack(bp->id); // narrowphase
			}
			return (uint32)out.Size();
		}

		// ── Shape cast (TOI) : forme convexe translatée le long de dir ────────
		bool NkWorld::ShapeCast(const NkShape &shape, const NkVec3f &dir, float32 maxDist, NkRayHit3D &hit,
								uint32 mask) const {
			const float32 dl = math::NkSqrt(dir.Dot(dir));
			if (dl < 1e-8f || !NkShapeIsConvex(shape.type))
				return false;
			...
			NkAABB3D swept = a0;
			swept.Expand(a0.min + d * maxDist);
			swept.Expand(a0.max + d * maxDist);
			bool found = false;
			NkRayHit3D best;
			best.t = maxDist;
			for (uint32 i = 0; i < (uint32)mBodies.Size(); ++i) {
				const NkBody &b = mBodies[i];
				if (!b.active || !(b.layer & mask) || NkShapeIs2D(b.shape.type) || !NkShapeIsConvex(b.shape.type))
					continue;
				if (!swept.Overlaps(NkBodyAABB(b.shape)))
					continue; // broadphase
				float32 t;
				NkVec3f nrm, p;
		...
		bool NkWorld::SweepBody(uint32 id, const NkVec3f &translation, NkRayHit3D &hit, uint32 mask) const {
			...
			if (!self || !NkShapeIsConvex(self->shape.type))
				return false;
			...
			NkAABB3D swept = a0;
			swept.Expand(a0.min + translation);
			swept.Expand(a0.max + translation);
			NkVector<uint32> cand;
			mTree.Query(swept, cand); // broadphase DBVH (volume balayé)
			bool found = false;
			NkRayHit3D best;
			best.t = dl;
			for (uint32 i = 0; i < (uint32)cand.Size(); ++i) {
				...
				const NkBody *bp = GetBody(cand[i]);
				if (!bp || !bp->active || !(bp->layer & mask) || NkShapeIs2D(bp->shape.type) ||
					!NkShapeIsConvex(bp->shape.type))
					continue;
				float32 t;
				NkVec3f nrm, p;

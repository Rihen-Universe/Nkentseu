...
int main() {
	...
	}
	{
		...
		CHECK(NkCapsuleCapsule({0, -1, 0}, {0, 1, 0}, 0.5f, {0.8f, -1, 0}, {0.8f, 1, 0}, 0.5f, m),
			  "capsule-capsule overlap");
		NkManifold3D m2;
		CHECK(!NkCapsuleCapsule({0, -1, 0}, {0, 1, 0}, 0.5f, {3, -1, 0}, {3, 1, 0}, 0.5f, m2),
			  "capsule-capsule separated");
	}
	...
	// ── GJK/EPA 3D : cohérence vs analytique + nouveaux types convexes ────────
	{
		...
		CHECK(Near(m.points[0].depth, 0.5f, 2e-2f) && m.normal.x > 0.9f,
			  "gjk3d sphere-sphere depth/normal ~ analytique");
		NkShape sc = NkShape::Sphere({3, 0, 0}, 1.f);
		NkManifold3D m2;
		...
		NkManifold3D mb;
		CHECK(NkGJKEPA3D(ba, bb, mb) && Near(mb.points[0].depth, 0.5f, 3e-2f) && mb.normal.x > 0.9f,
			  "gjk3d box-box depth/normal");

		// box-capsule (paire NON gérée analytiquement -> fallback GJK)
		NkShape box = NkShape::Box3D({0, 0, 0}, {1, 1, 1});
		NkShape cap =
			NkShape::Capsule3D({1.2f, -0.5f, 0}, {1.2f, 0.5f, 0}, 0.5f); // surface x=0.7 vs face x=1 -> depth 0.3
		NkManifold3D mc;
		...
		NkShape csf = NkShape::Sphere({3, 0, 0}, 1.f);
		NkManifold3D mcy2;
		...
		NkShape cos1 = NkShape::Sphere({1.1f, 0, 0}, 0.5f);
		NkManifold3D mco;
		CHECK(NkGJKEPA3D(cone, cos1, mco), "gjk3d cone-sphere overlap");
		NkShape cosf = NkShape::Sphere({3, 0, 0}, 0.5f);
		NkManifold3D mco2;
		...
		NkShape cvs = NkShape::Sphere({-0.3f, 0.3f, 0.3f}, 0.5f);
		NkManifold3D mcv;
		CHECK(NkGJKEPA3D(cv, cvs, mcv), "gjk3d convex(tetra)-sphere overlap");
		NkShape cvsf = NkShape::Sphere({5, 5, 5}, 0.5f);
		NkManifold3D mcv2;
		...
		NkShape t3s = NkShape::Sphere({0, 0.3f, 0}, 0.5f);
		NkManifold3D mt3;
	...
	// ── Plan / half-space (analytique) via le world ──────────────────────────
	{
		...
		if (pw.Pairs().Size() == 1) {
			...
			CHECK(pm.normal.y > 0.8f && Near(pm.points[0].depth, 0.5f, 6e-2f),
				  "world plane-sphere normale +Y depth 0.5");
		}
		NkWorld pw2;
	...
	// ── GJK/EPA 2D : polygone, triangle, fallback world ──────────────────────
	{
		...
		NkManifold2D m2d;
		CHECK(NkGJKEPA2D(p1, p2, m2d) && Near(m2d.points[0].depth, 0.5f, 6e-2f) && m2d.normal.x > 0.8f,
			  "gjk2d polygone-polygone depth 0.5");
		static const NkVec3f sq3[4] = {{5, -1, 0}, {7, -1, 0}, {7, 1, 0}, {5, 1, 0}};
		NkShape p3 = NkShape::Polygon2D(sq3, 4);
		NkManifold2D m2s;
		...
		NkShape circ = NkShape::Circle2D({-0.3f, 0.3f}, 0.5f);
		NkManifold2D mt2;
	...
	// ── Événements : enter / stay / exit ─────────────────────────────────────
	{
		...
		CHECK(w.EnterEvents().Size() == 1 && w.StayEvents().Size() == 0 && w.ExitEvents().Size() == 0,
			  "events: ENTER frame 1");
		w.Step(); // frame 2 : maintien
		CHECK(w.EnterEvents().Size() == 0 && w.StayEvents().Size() == 1 && w.ExitEvents().Size() == 0,
			  "events: STAY frame 2");
		w.SetShape(b, NkShape::Sphere({5, 0, 0}, 1.f)); // on écarte b
		w.Step();										// frame 3 : sortie
		CHECK(w.EnterEvents().Size() == 0 && w.StayEvents().Size() == 0 && w.ExitEvents().Size() == 1,
			  "events: EXIT frame 3");
	}
	...
	}
	{
		...
		NkWorld sw;
		for (int32 i = 0; i < 5; ++i)
			sw.AddBody(NkShape::Sphere({(float32)i, 0.f, 0.f}, 0.6f));
		sw.Step();
		...
		NkWorld sw2;
		for (int32 i = 0; i < 6; ++i)
			sw2.AddBody(NkShape::Sphere({(float32)i * 10.f, 0.f, 0.f}, 0.6f));
		sw2.Step();
	...
	// ── VAGUE 5 : requêtes de scène (raycast exact + overlap) ────────────────
	{
		...
		rx.maxT = 100.f;
		NkRayHit3D h;
		CHECK(NkRayOBB3D(rx, {0, 0, 0}, {1, 1, 1}, math::NkQuatf(), h) && Near(h.t, 4.f),
			  "ray-OBB(identite)==AABB t=4");
		...
		ry.maxT = 100.f;
		NkRayHit3D h2;
		CHECK(NkRayOBB3D(ry, {0, 0, 0}, {1, 1, 1}, math::NkQuatf(0.f, 0.f, s22, c22), h2) && Near(h2.t, 3.5858f, 1e-2f),
			  "ray-OBB 45deg t~3.586");
		// ray-plane
		NkRayHit3D hp;
	...
	// ── VAGUE 6 : ray-cast convexe générique (GJK/CA) + shape cast ───────────
	{
		...
		NkRayHit3D h;
		CHECK(NkRayConvex3D(ray, NkShape::Sphere({0, 0, 0}, 1.f), h) && Near(h.t, 4.f, 1e-2f),
			  "ray-convexe sphere t=4 (CA)");
		// ray vs capsule (côté à x=-0.5) -> t=4.5
		NkRayHit3D hc;
		CHECK(NkRayConvex3D(ray, NkShape::Capsule3D({0, -1, 0}, {0, 1, 0}, 0.5f), hc) && Near(hc.t, 4.5f, 2e-2f),
			  "ray-convexe capsule t=4.5 (CA)");
		// ray vs cône (booléen : touche)
		NkRayHit3D hco;
		CHECK(NkRayConvex3D(ray, NkShape::Cone3D({0, -1, 0}, {0, 1, 0}, 2.f, 1.f), hco),
			  "ray-convexe cone (CA) touche");
	...
	}
	{
		...
		w.AddBody(NkShape::Sphere({0, 0, 0}, 1.f));
		NkRayHit3D hit;
		CHECK(w.ShapeCast(NkShape::Sphere({-5, 0, 0}, 0.5f), {1, 0, 0}, 100.f, hit) && Near(hit.t, 3.5f, 2e-2f),
			  "shapecast sphere TOI=3.5");
		// Passe au-dessus -> rate
		NkRayHit3D hit2;
		CHECK(!w.ShapeCast(NkShape::Sphere({-5, 5, 0}, 0.5f), {1, 0, 0}, 100.f, hit2),
			  "shapecast sphere rate (passe au-dessus)");
	}
	...
	// ── VAGUE 7 : Dynamic AABB Tree (DBVH) — requêtes accélérées ─────────────
	{
		...
		for (int32 i = 0; i < 3; ++i)
			for (int32 j = 0; j < 3; ++j) {
				NkAABB3D b;
				NkVec3f c{(float32)i * 2.f, (float32)j * 2.f, 0.f};
				b.min = c - NkVec3f{0.4f, 0.4f, 0.4f};
				b.max = c + NkVec3f{0.4f, 0.4f, 0.4f};
				proxies[k] = tree.Insert(b, k);
				++k;
			}
		NkVector<uint32> res;
		// toute la grille
	...
	// ── VAGUE 9 : équilibrage DBVH (AVL) + queries world via l'arbre ─────────
	{
		...
		for (int32 i = 0; i < 64; ++i) {
			NkAABB3D b;
			NkVec3f c{(float32)i, 0.f, 0.f};
			b.min = c - NkVec3f{0.3f, 0.3f, 0.3f};
			b.max = c + NkVec3f{0.3f, 0.3f, 0.3f};
	...
	// ── VAGUE 10 : manifolds MULTI-POINTS 2D (clipping polygones) ────────────
	{
		...
		NkManifold2D m;
		CHECK(NkCollidePolygons2D(NkShape::Box2D({0, 0}, {1, 1}), NkShape::Box2D({1.5f, 0}, {1, 1}), m),
			  "clip box2D overlap");
		...
		CHECK(NkCollidePolygons2D(NkShape::Box2D({0, 0}, {1, 1}), NkShape::Box2D({0, 1.9f}, {1, 1}), ms) &&
				  ms.count == 2 && ms.normal.y > 0.8f,
			  "clip box2D empilement : 2 pts normale +Y");

		// séparées -> pas de collision
		NkManifold2D m2;
		CHECK(!NkCollidePolygons2D(NkShape::Box2D({0, 0}, {1, 1}), NkShape::Box2D({5, 0}, {1, 1}), m2),
			  "clip box2D separes");

		// triangle vs boîte (polygone général)
		static const NkVec3f tri[3] = {{0, 0, 0}, {2, 0, 0}, {0, 2, 0}};
		NkManifold2D mt;
		CHECK(NkCollidePolygons2D(NkShape::Triangle2D(tri), NkShape::Box2D({-0.3f, 0.3f}, {0.5f, 0.5f}), mt),
			  "clip triangle2D-box overlap");
	...
	// ── VAGUE 11 : manifolds MULTI-POINTS 3D (box-box, EPA + clipping de face) ─
	{
		...
		NkManifold3D m;
		CHECK(NkCollideBoxBox3D(NkShape::Box3D({0, 0, 0}, {1, 1, 1}), NkShape::Box3D({0, 1.9f, 0}, {1, 1, 1}), m),
			  "boxbox3D empilement overlap");
		...
		CHECK(NkCollideBoxBox3D(NkShape::Box3D({0, 0, 0}, {1, 1, 1}), NkShape::Box3D({1.8f, 0, 0}, {1, 1, 1}), mx) &&
				  mx.count == 4 && mx.normal.x > 0.8f,
			  "boxbox3D cote-a-cote X : 4 pts normale +X");

		// séparées -> pas de collision
		NkManifold3D m2;
		CHECK(!NkCollideBoxBox3D(NkShape::Box3D({0, 0, 0}, {1, 1, 1}), NkShape::Box3D({5, 0, 0}, {1, 1, 1}), m2),
			  "boxbox3D separes");
	...
	// ── VAGUE 12 : CCD (SweepBody, anti-tunneling) ───────────────────────────
	{
		...
		CHECK(w.SweepBody(ball, {10, 0, 0}, hit) && Near(hit.t, 4.7f, 3e-2f),
			  "CCD SweepBody : impact mur a 4.7 (anti-tunneling)");
	...
	// ── VAGUE 13 : contacts persistants (IDs de feature + warm-starting) ─────
	{
		...
		for (int32 i = 0; i < m1.count; ++i)
			sameIds = sameIds && (m1.points[i].id == m2.points[i].id);

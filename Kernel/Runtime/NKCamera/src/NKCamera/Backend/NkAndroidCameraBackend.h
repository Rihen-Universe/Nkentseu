...
namespace nkentseu {
	...
	class NkAndroidCameraBackend : public NKICameraBackend {
		public:
			...
			~NkAndroidCameraBackend() override {
				Shutdown();
			}
			...
			bool Init() override {
				...
				if (!mCameraManager) {
					mLastError = "ACameraManager_create failed";
					return false;
				}
			...
			NkVector<NkCameraDevice> EnumerateDevices() override {
				...
				for (int i = 0; i < idList->numCameras; ++i) {
					...
					for (uint32_t j = 0; j + 3 < configs.count; j += 4) {
						...
						if (configs.data.i32[j + 3] != 0)
							continue; // skip input configs
			...
			bool StartStreaming(const NkCameraConfig &config) override {
				...
				if (!HasCameraPermission()) {
					...
					mLastError = "android.permission.CAMERA is not granted. "
				...
				if (devices.Empty()) {
					mLastError = "No Camera2 YUV device exposed by platform. "
				...
				camera_status_t status =
					ACameraManager_openCamera(mCameraManager, mCameraId.CStr(), &mDeviceCallbacks, &mCameraDevice);
				if (status != ACAMERA_OK) {
					mLastError = NkString("ACameraManager_openCamera failed: ") + CameraStatusToString(status);
					return false;
				}

				media_status_t mstatus = AImageReader_new(static_cast<int32_t>(mWidth), static_cast<int32_t>(mHeight),
				...
				if (mstatus != AMEDIA_OK) {
					mLastError =
						"AImageReader_setImageListener failed: " + NkString::Fmtf("%d", static_cast<int>(mstatus));
					StopStreaming();
					return false;
				}
				...
				if (status != ACAMERA_OK || !mSessionOutputContainer) {
					mLastError =
						NkString("ACaptureSessionOutputContainer_create failed: ") + CameraStatusToString(status);
					StopStreaming();
					return false;
				}
				...
				status = ACameraDevice_createCaptureSession(mCameraDevice, mSessionOutputContainer, &mSessionCallbacks,
															&mCaptureSession);
				...
				if (status != ACAMERA_OK) {
					mLastError =
						NkString("ACameraCaptureSession_setRepeatingRequest failed: ") + CameraStatusToString(status);
					StopStreaming();
					return false;
				}
			...
			bool SetAutoFocus(bool e) override {
				if (!mCaptureRequest)
					return false;
				uint8_t mode = e ? ACAMERA_CONTROL_AF_MODE_CONTINUOUS_VIDEO : ACAMERA_CONTROL_AF_MODE_OFF;
			...
			NkPixelFormat GetFormat() const override {
				return NkPixelFormat::NK_PIXEL_YUV420;
			}
		...
		private:
			...
			void InitSensors() {
				...
				mAccel = ASensorManager_getDefaultSensor(mSensorManager, ASENSOR_TYPE_ACCELEROMETER);
				mGyro = ASensorManager_getDefaultSensor(mSensorManager, ASENSOR_TYPE_GYROSCOPE);

				mSensorLooper = ALooper_prepare(ALOOPER_PREPARE_ALLOW_NON_CALLBACKS);
				if (mAccel) {
					mAccelQueue =
						ASensorManager_createEventQueue(mSensorManager, mSensorLooper, 1, SensorCallback, this);
					ASensorEventQueue_enableSensor(mAccelQueue, mAccel);
					ASensorEventQueue_setEventRate(mAccelQueue, mAccel, 16667); // 60Hz
				}
				if (mGyro) {
					mGyroQueue =
						ASensorManager_createEventQueue(mSensorManager, mSensorLooper, 2, SensorCallback, this);
			...
			void ShutdownSensors() {
				...
				if (mGyroQueue) {
					ASensorEventQueue_disableSensor(mGyroQueue, mGyro);

...
namespace nkentseu {
	...
	class NkEmscriptenCameraBackend : public NKICameraBackend {
		public:
			NkEmscriptenCameraBackend() {
				sInstance = this;
			}

			~NkEmscriptenCameraBackend() override {
				Shutdown();
				if (sInstance == this)
					sInstance = nullptr;
			}
			...
			NkPixelFormat GetFormat() const override {
				return NkPixelFormat::NK_PIXEL_RGBA8;
			}
		...
		private:
			...
			static int32 EmJsStart(int32 deviceIdx, int32 w, int32 h, int32 fps, int32 bufPtr, int32 bufSize);
	...
	EM_JS(int, nkentseu_emcam_start, (int devIdx, int w, int h, int fps, int bufPtr, int bufSize), {
		if (!Module.nkcam)
			return 0;
		var nk = Module.nkcam;
		nk.bufPtr = bufPtr;
		...
		nk.canvas.width = w;
		nk.canvas.height = h;
		var constraints = {video : {width : {ideal : w}, height : {ideal : h}}};
		var devs = nk.devices || [];
		...
		return Asyncify.handleAsync(async() => {
			try {
				...
				nk.intervalId = setInterval(
					function() {
						if (!nk.video.videoWidth)
							return;
						...
						if (need <= nk.bufSize) {
							...
							Module._nkentseu_emcam_on_frame(nk.canvas.width, nk.canvas.height, tsUs >>> 0,
															(tsUs / 0x100000000) >>> 0);
						}
					},
					interval);
	...
	inline int32 NkEmscriptenCameraBackend::EmJsCheckSupport() {
		return nkentseu_emcam_supported();
	}

	inline void NkEmscriptenCameraBackend::EmJsSetup() {
		nkentseu_emcam_setup();
	}

	inline int32 NkEmscriptenCameraBackend::EmJsEnumerateCount() {
		return nkentseu_emcam_enum_count();
	}
	...
	inline int32 NkEmscriptenCameraBackend::EmJsStart(int32 d, int32 w, int32 h, int32 fps, int32 bufPtr,
													  int32 bufSize) {
		return nkentseu_emcam_start(d, w, h, fps, bufPtr, bufSize);
	}

	inline void NkEmscriptenCameraBackend::EmJsStop() {
		nkentseu_emcam_stop();
	}

	// Export C appelé par le pump setInterval JS. Doit être visible à l'édition
	// de lien via EMSCRIPTEN_KEEPALIVE.
	extern "C" EMSCRIPTEN_KEEPALIVE void nkentseu_emcam_on_frame(int w, int h, unsigned int tsLow,
																 unsigned int tsHigh) {
		if (!NkEmscriptenCameraBackend::sInstance)
			return;

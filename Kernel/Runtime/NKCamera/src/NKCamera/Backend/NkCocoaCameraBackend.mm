...
- (void)captureOutput:(AVCaptureOutput *)output
	...
	if (self.backend)
		self.backend->OnSampleBuffer((__bridge void *)sampleBuffer);
}
...
namespace nkentseu {
	...
	NkCocoaCameraBackend::~NkCocoaCameraBackend() {
		Shutdown();
	}

	bool NkCocoaCameraBackend::Init() {
		return true;
	}

	void NkCocoaCameraBackend::Shutdown() {
		StopStreaming();
	}
	...
	bool NkCocoaCameraBackend::StartStreaming(const NkCameraConfig &config) {
		...
		NSArray<AVCaptureDevice *> *devs = [AVCaptureDevice devicesWithMediaType:AVMediaTypeVideo];
		if (config.deviceIndex >= (uint32)devs.count) {
	...
	bool NkCocoaCameraBackend::StartVideoRecord(const NkVideoRecordConfig &config) {
		...
		NSDictionary *settings = @{
			...
			AVVideoCompressionPropertiesKey : @{AVVideoAverageBitRateKey : @(config.bitrateBps)}
		};
	...
	void NkCocoaCameraBackend::StopVideoRecord() {
		...
		mRecording = false;
		if (mAssetWriter) {
			AVAssetWriter *writer = (__bridge AVAssetWriter *)mAssetWriter;
			CFBridgingRelease(mAssetWriter);
			mAssetWriter = nullptr;
			AVAssetWriterInput *input = (__bridge AVAssetWriterInput *)mAssetInput;
			CFBridgingRelease(mAssetInput);
			mAssetInput = nullptr;
	...
	bool NkCocoaCameraBackend::IsRecording() const {
		return mRecording;
	}
	...
	bool NkCocoaCameraBackend::SetTorch(bool e) {
		(void)e;
		return false;
	}

	bool NkCocoaCameraBackend::SetZoom(float z) {
		(void)z;
		return false;
	}

...
namespace nkentseu {
	...
	void NkUIKitCameraBackend::Shutdown() {
		...
		auto *p = (UIKitPrivate *)mPriv;
		if (p->motionMgr) {
			[p->motionMgr stopDeviceMotionUpdates];
			p->motionMgr = nil;
		}
	}
	...
	NkVector<NkCameraDevice> NkUIKitCameraBackend::EnumerateDevices() {
		...
		for (AVCaptureDevice *d in ds.devices) {
			...
			dev.facing = (d.position == AVCaptureDevicePositionFront) ? NkCameraFacing::NK_CAMERA_FACING_FRONT
			...
			for (AVCaptureDeviceFormat *fmt in d.formats) {
				...
				m.width = (uint32)dim.width;
				m.height = (uint32)dim.height;
				m.fps = 30;
				m.format = NkPixelFormat::NK_PIXEL_BGRA8;
				if (m.width > 0 && m.height > 0)
					dev.modes.PushBack(m);
			}
			result.PushBack(dev);
		}
		return result;
	...
	bool NkUIKitCameraBackend::StartStreaming(const NkCameraConfig &config) {
		...
		if (!input) {
			mLastError = [[err localizedDescription] UTF8String];
			return false;
		}
		...
		if (config.height <= 480)
			p->session.sessionPreset = AVCaptureSessionPreset640x480;
		else if (config.height <= 720)
			p->session.sessionPreset = AVCaptureSessionPreset1280x720;
		else if (config.height <= 1080)
			p->session.sessionPreset = AVCaptureSessionPreset1920x1080;
		else
			p->session.sessionPreset = AVCaptureSessionPreset3840x2160;
		...
		p->vidDel = [[NkiOSVidDelegate alloc] init];
		p->vidDel.backend = this;
	...
	bool NkUIKitCameraBackend::CapturePhotoToFile(const NkString &path) {
		...
		if (!CapturePhoto(res) || res.frame.data.Empty())
			return false;
	...
	bool NkUIKitCameraBackend::StartVideoRecord(const NkVideoRecordConfig &config) {
		...
		p->wrInput = [AVAssetWriterInput assetWriterInputWithMediaType:AVMediaTypeVideo outputSettings:vs];
		...
		p->adaptor =
			[AVAssetWriterInputPixelBufferAdaptor assetWriterInputPixelBufferAdaptorWithAssetWriterInput:p->wrInput
	...
	void NkUIKitCameraBackend::StopVideoRecord() {
		...
		if (p->writer) {
			...
			[p->writer finishWritingWithCompletionHandler:^{
	...
	bool NkUIKitCameraBackend::IsRecording() const {
		return mRecording;
	}
	...
	bool NkUIKitCameraBackend::GetOrientation(NkCameraOrientation &out) {
		auto *p = (UIKitPrivate *)mPriv;
		if (!p->motionMgr || !p->motionMgr.isDeviceMotionActive)
			return false;
	...
	bool NkUIKitCameraBackend::SetAutoFocus(bool e) {
		...
		if (!p->device)
			return false;
		if (![p->device isFocusModeSupported:AVCaptureFocusModeContinuousAutoFocus])
			return false;
		NSError *err = nil;
		[p->device lockForConfiguration:&err];
		p->device.focusMode = e ? AVCaptureFocusModeContinuousAutoFocus : AVCaptureFocusModeLocked;
		[p->device unlockForConfiguration];
		return err == nil;
	}
	...
	bool NkUIKitCameraBackend::SetAutoWhiteBalance(bool e) {
		...
		NSError *err = nil;
		[p->device lockForConfiguration:&err];
		p->device.whiteBalanceMode =
			e ? AVCaptureWhiteBalanceModeContinuousAutoWhiteBalance : AVCaptureWhiteBalanceModeLocked;
		[p->device unlockForConfiguration];
		return err == nil;
	}
	...
	bool NkUIKitCameraBackend::SetFocusPoint(float x, float y) {
		...
		if (!p->device)
			return false;
		if (![p->device isFocusPointOfInterestSupported])
			return false;
		NSError *err = nil;
		[p->device lockForConfiguration:&err];

	...
	class NkLinuxCameraBackend : public NKICameraBackend {
		public:
					...
					while (ioctl(fd, VIDIOC_ENUM_FMT, &fd2) == 0) {
						...
						while (ioctl(fd, VIDIOC_ENUM_FRAMESIZES, &fs) == 0) {
							...
							if (fs.type == V4L2_FRMSIZE_TYPE_DISCRETE) {
								m.width = fs.discrete.width;
								m.height = fs.discrete.height;
							} else {
								m.width = fs.stepwise.max_width;
								m.height = fs.stepwise.max_height;
							}
							m.fps = 30;
							m.format = NkPixelFormat::NK_PIXEL_YUYV;
			...
			bool StartStreaming(const NkCameraConfig &config) override {
				...
				if (config.deviceIndex >= devs.Size()) {
					mLastError = NkString::Fmtf("Device index %u out of range", config.deviceIndex);
					return false;
				}
				...
				if (mFd < 0) {
					mLastError = "Cannot open " + path + ": " + strerror(errno);
					return false;
				}
				...
				for (const uint32 pixfmt :
					 {static_cast<uint32>(V4L2_PIX_FMT_MJPEG), static_cast<uint32>(V4L2_PIX_FMT_YUYV)}) {
				...
				mFPS = config.fps;
				mFormat = (fmt.fmt.pix.pixelformat == V4L2_PIX_FMT_YUYV) ? NkPixelFormat::NK_PIXEL_YUYV
																		 : NkPixelFormat::NK_PIXEL_MJPEG;

				// FPS
				struct v4l2_streamparm parm = {};
				parm.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
				parm.parm.capture.timeperframe = {1, mFPS};
				if (ioctl(mFd, VIDIOC_S_PARM, &parm) == 0 && parm.parm.capture.timeperframe.numerator != 0) {
					mFPS = parm.parm.capture.timeperframe.denominator / parm.parm.capture.timeperframe.numerator;
				}
			...
			bool CapturePhotoToFile(const NkString &path) override {
				...
				// Find last '.' to replace extension
				{
					...
					for (NkString::SizeType i = 0; i < out.Size(); ++i)
						if (out[i] == '.')
							dot = i;
					if (dot != NkString::SizeType(-1))
						out = out.SubStr(0, dot) + ".ppm";
					else
						out += ".ppm";
				}
			...
			bool StartVideoRecord(const NkVideoRecordConfig &config) override {
				...
				const bool forceImageSequence = (config.mode == NkVideoRecordConfig::Mode::IMAGE_SEQUENCE_ONLY) ||
												(requestedCodec == "images" || requestedCodec == "image-sequence" ||
												 requestedCodec == "sequence" || requestedCodec == "frames");
		...
		private:
			...
			bool WriteFrameAsPpm(const NkCameraFrame &frame, const NkString &outPath) const {
				...
				NkVector<uint8> rgb;
				rgb.Resize(static_cast<uint32>(w) * static_cast<uint32>(h) * 3u);
				if (frame.format == NkPixelFormat::NK_PIXEL_YUYV) {
					...
					for (uint32 i = 0; i < (w * h / 2u); ++i) {
						...
						auto cl = [](float v) -> uint8 {
							return static_cast<uint8>(v < 0.f ? 0.f : (v > 255.f ? 255.f : v));
						};
				...
				} else if (frame.format == NkPixelFormat::NK_PIXEL_RGBA8 ||

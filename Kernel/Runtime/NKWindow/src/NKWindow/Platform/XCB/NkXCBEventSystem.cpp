		...
		if (conn) {
			...
			if (mData->mXkbContext && !mData->mXkbKeymap) {
				...
				mData->mXkbKeymap = xkb_x11_keymap_new_from_device(
					mData->mXkbContext, conn, xkb_x11_get_core_keyboard_device_id(conn), XKB_KEYMAP_COMPILE_NO_FLAGS);
				if (mData->mXkbKeymap)
					mData->mXkbState = xkb_x11_state_new_from_device(mData->mXkbKeymap, conn,
	...
	void NkEventSystem::PumpOS() {
		...
		while ((ev = xcb_poll_for_event(conn)) != nullptr) {
			...
			if (sRandrAvailable && sRandrFirstEvent >= 0 &&
				...
				NkSystemDisplayEvent e(NkDisplayChange::NK_DISPLAY_RESOLUTION_CHANGED, info, 0);
			...
			switch (evType) {
				...
				case XCB_KEY_RELEASE:
					srcXid = ((xcb_key_press_event_t *)ev)->event;
					break;
				case XCB_BUTTON_PRESS:
				case XCB_BUTTON_RELEASE:
					srcXid = ((xcb_button_press_event_t *)ev)->event;
					break;
				case XCB_MOTION_NOTIFY:
					srcXid = ((xcb_motion_notify_event_t *)ev)->event;
					break;
				case XCB_FOCUS_IN:
				case XCB_FOCUS_OUT:
					srcXid = ((xcb_focus_in_event_t *)ev)->event;
					break;
				case XCB_EXPOSE:
					srcXid = ((xcb_expose_event_t *)ev)->window;
					break;
				case XCB_CONFIGURE_NOTIFY:
					srcXid = ((xcb_configure_notify_event_t *)ev)->window;
					break;
				case XCB_MAP_NOTIFY:
				case XCB_UNMAP_NOTIFY:
					srcXid = ((xcb_map_notify_event_t *)ev)->window;
					break;
				case XCB_DESTROY_NOTIFY:
					srcXid = ((xcb_destroy_notify_event_t *)ev)->window;
					break;
				case XCB_CLIENT_MESSAGE:
					srcXid = ((xcb_client_message_event_t *)ev)->window;
					break;
				case XCB_SELECTION_NOTIFY:
					srcXid = ((xcb_selection_notify_event_t *)ev)->requestor;
					break;
				case XCB_ENTER_NOTIFY:
				case XCB_LEAVE_NOTIFY:
					srcXid = ((xcb_enter_notify_event_t *)ev)->event;
					break;
				default:
					break;
			}
			...
			switch (evType) {
				...
				case XCB_KEY_RELEASE: {
					...
					if (key == NkKey::NK_UNKNOWN && mData->mXkbState) {
						xkb_keysym_t sym =
							xkb_state_key_get_one_sym(mData->mXkbState, static_cast<xkb_keycode_t>(ke->detail));
					...
					if (evType == XCB_KEY_PRESS && mData->mXkbState) {
						char buf[32] = {};
						xkb_state_key_get_utf8(mData->mXkbState, static_cast<xkb_keycode_t>(ke->detail), buf,
											   sizeof(buf));
				...
				case XCB_BUTTON_RELEASE: {
					...
					switch (be->detail) {
						case 1:
							if (pr) {
								NkMouseButtonPressEvent e(NkMouseButton::NK_MB_LEFT, x, y, sx, sy, 1, m);
								Enqueue(e, winId);
							} else {
								NkMouseButtonReleaseEvent e(NkMouseButton::NK_MB_LEFT, x, y, sx, sy, 1, m);
								Enqueue(e, winId);
							}
							break;
						case 2:
							if (pr) {
								NkMouseButtonPressEvent e(NkMouseButton::NK_MB_MIDDLE, x, y, sx, sy, 1, m);
								Enqueue(e, winId);
							} else {
								NkMouseButtonReleaseEvent e(NkMouseButton::NK_MB_MIDDLE, x, y, sx, sy, 1, m);
								Enqueue(e, winId);
							}
							break;
						case 3:
							if (pr) {
								NkMouseButtonPressEvent e(NkMouseButton::NK_MB_RIGHT, x, y, sx, sy, 1, m);
								Enqueue(e, winId);
							} else {
								NkMouseButtonReleaseEvent e(NkMouseButton::NK_MB_RIGHT, x, y, sx, sy, 1, m);
								Enqueue(e, winId);
							}
							break;
						case 4:
							if (pr) {
								NkMouseWheelVerticalEvent e(1.0, x, y, 0.0, false, m);
								Enqueue(e, winId);
							}
							break;
						case 5:
				...
				case XCB_MOTION_NOTIFY: {
					...
					NkMouseMoveEvent e(x, y, sx, sy, x - sPrevX, y - sPrevY, NkMouseButtons{}, XCBMods(me->state));
					sPrevX = x;
					sPrevY = y;
					Enqueue(e, winId);
					break;
				...
				case XCB_EXPOSE: {
					...
					if (xe->count == 0) {
						NkWindowPaintEvent e((int32)xe->x, (int32)xe->y, (uint32)xe->width, (uint32)xe->height);
						Enqueue(e, winId);
					}
					break;
				}
				...
				case XCB_CLIENT_MESSAGE: {
					...
					if (window) {
						...
						if (cm->type == wmProtocols && cm->format == 32 &&

...
namespace {
    ...
    void InitializeDpiAPIs() {
        ...
        if (shcore) {
            pGetDpiForMonitor = (GetDpiForMonitorProc)
...
namespace nkentseu {
    ...
    bool NkWindow::Create(const NkWindowConfig& config) {
        ...
        // No-op si appel multiple : on set qu'une fois par process.
        {
            ...
            if (!s_aumidSet) {
                ...
                wchar_t buf[256];
                DWORD len = GetEnvironmentVariableW(L"NkAppUserModelID",
                                                   buf, 256);
                if (len > 0 && len < 256) aumid = buf;
        ...
        HICON hExeIconSmall = static_cast<HICON>(LoadImageW(
            hExeInst, MAKEINTRESOURCEW(1), IMAGE_ICON, smCX, smCY,
            LR_DEFAULTCOLOR | LR_SHARED));
        HICON hExeIconBig = static_cast<HICON>(LoadImageW(
            hExeInst, MAKEINTRESOURCEW(1), IMAGE_ICON, bgCX, bgCY,
            LR_DEFAULTCOLOR | LR_SHARED));
        ...
        if (mData.mIconBig) {
            SendMessageW(mData.mHwnd, WM_SETICON, ICON_BIG,
                         reinterpret_cast<LPARAM>(mData.mIconBig));
        }
        if (mData.mIconSmall) {
    ...
    static NkDisplayInfo Win32FillDisplayInfo(HMONITOR hmon, uint32 index) {
        ...
        if (GetMonitorInfoW(hmon, &mi)) {
            ...
            usize n = dev.Size() < (sizeof(info.name) - 1) ? dev.Size()
                                                           : (sizeof(info.name) - 1);
            for (usize i = 0; i < n; ++i) info.name[i] = dev.CStr()[i];
            info.name[n] = '\0';
        }
        ...
        if (pGetDpiForMonitor &&
            pGetDpiForMonitor(hmon, 0 /* MDT_EFFECTIVE_DPI */, &dx, &dy) == S_OK) {
            // ok
        }
    ...
    NkVector<NkDisplayInfo> NkWindow::EnumerateMonitors() const {
        ...
        EnumDisplayMonitors(nullptr, nullptr, Win32MonitorEnumProc,
    ...
    NkDisplayInfo NkWindow::GetCurrentMonitor() const {
        HMONITOR hmon = mData.mHwnd
            ? MonitorFromWindow(mData.mHwnd, MONITOR_DEFAULTTONEAREST)
    ...
    void NkWindow::ClipMouseToClient(bool clip) {
        ...
        if (clip) {
            RECT cr;
            if (!::GetClientRect(mData.mHwnd, &cr)) return;

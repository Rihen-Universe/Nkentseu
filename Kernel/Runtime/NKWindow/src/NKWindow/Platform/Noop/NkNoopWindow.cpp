...
namespace nkentseu {
	...
	void NkWindow::SetBackgroundColor(uint32 rgba) {
		...
		NkWindowRefuserUneFois(NkWindowProp::BgColor, "Noop",
	...
	// COORDONNEES CLIENT : NON IMPLEMENTE sur Noop. On le DIT et on rend
	...
	bool NkWindow::SetMousePositionClient(int32 x, int32 y) {
		(void)x;
		(void)y;
		NkLog::Instance().Warnf("[NkWindow] SetMousePositionClient : NON IMPLEMENTE sur Noop.");
		return false;
	}

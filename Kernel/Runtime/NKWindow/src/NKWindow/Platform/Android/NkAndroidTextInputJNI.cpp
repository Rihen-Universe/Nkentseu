...
JNIEXPORT void JNICALL Java_com_nkentseu_window_NkNativeActivity_nkCommitText(JNIEnv *env, jclass, jstring text) {
	if (!text) {
		return;
	}
	...
	for (jsize i = 0; i < len; ++i) {
		...
		if (cp == '\n' || cp == '\r') {
			EmitKey(nkentseu::NkKey::NK_ENTER, nkentseu::NkScancode::NK_SC_ENTER, AKEY_EVENT_ACTION_DOWN, win);
			EmitKey(nkentseu::NkKey::NK_ENTER, nkentseu::NkScancode::NK_SC_ENTER, AKEY_EVENT_ACTION_UP, win);
			continue;
		}
		if (cp < 0x20 || cp == 0x7F) {
			continue;
...
JNIEXPORT void JNICALL Java_com_nkentseu_window_NkNativeActivity_nkKeyEvent(JNIEnv *, jclass, jint keyCode, jint action,
																			jint /*metaState*/) {
	const nkentseu::NkWindowId win = TargetWindowId();
	if (win == nkentseu::NK_INVALID_WINDOW_ID) {
		return;
	...
	switch (keyCode) {
		case AKEYCODE_DEL:
			key = nkentseu::NkKey::NK_BACK;
			sc = nkentseu::NkScancode::NK_SC_BACKSPACE;
			break;
		case AKEYCODE_FORWARD_DEL:
			key = nkentseu::NkKey::NK_DELETE;
			sc = nkentseu::NkScancode::NK_SC_DELETE;
			break;
		case AKEYCODE_ENTER:
		case AKEYCODE_NUMPAD_ENTER:
			key = nkentseu::NkKey::NK_ENTER;
			sc = nkentseu::NkScancode::NK_SC_ENTER;
			break;
		case AKEYCODE_TAB:
			key = nkentseu::NkKey::NK_TAB;
			sc = nkentseu::NkScancode::NK_SC_TAB;
			break;
		case AKEYCODE_DPAD_LEFT:
			key = nkentseu::NkKey::NK_LEFT;
			break;
		case AKEYCODE_DPAD_RIGHT:
			key = nkentseu::NkKey::NK_RIGHT;
			break;
		case AKEYCODE_DPAD_UP:
			key = nkentseu::NkKey::NK_UP;
			break;
		case AKEYCODE_DPAD_DOWN:
			key = nkentseu::NkKey::NK_DOWN;
			break;
		default:
			return;
	}

...
namespace nkentseu {
	...
	static NkWindow *ResolveWindowFromSeat(const NkWaylandSeatCtx *ctx, wl_surface *surface) {
		...
		if (ctx && ctx->focusedWindow != NK_INVALID_WINDOW_ID) {
			if (NkWindow *focused = NkWESystem::Instance().GetWindow(ctx->focusedWindow)) {
				return focused;
			}
		}
	...
	static void UpdateCursor(NkWaylandSeatCtx *ctx, NkWindow *window) {
		...
		if (window->mData.mMouseHidden) {
			...
			if (window->mData.mCursorTheme && window->mData.mCursorSurface) {
				::wl_cursor *cursor = wl_cursor_theme_get_cursor(window->mData.mCursorTheme, "left_ptr");
				if (!cursor) {
				...
				if (cursor && cursor->image_count > 0) {
					...
					if (buffer) {
						...
						wl_surface_damage(window->mData.mCursorSurface, 0, 0, static_cast<int32_t>(image->width),
										  static_cast<int32_t>(image->height));
						wl_surface_commit(window->mData.mCursorSurface);
						wl_pointer_set_cursor(ctx->pointer, ctx->pointerSerial, window->mData.mCursorSurface,
	...
	static void OnKeyboardKeymap(void *data, wl_keyboard *, uint32_t format, int fd, uint32_t size) {
		...
		if (!ctx || format != WL_KEYBOARD_KEYMAP_FORMAT_XKB_V1) {
			close(fd);
			return;
		}
		if (!ctx->xkbCtx) {
		...
		if (mapped != MAP_FAILED) {
			...
			if (keymapText) {
				while (keymapBytes < static_cast<size_t>(size)) {
					const ssize_t r = ::read(fd, keymapText + keymapBytes, static_cast<size_t>(size) - keymapBytes);
					if (r <= 0)
						break;
		...
		if (!keymapText || keymapBytes == 0u) {
			...
			ctx->xkbKeymap = xkb_keymap_new_from_names(ctx->xkbCtx, &fallbackNames, XKB_KEYMAP_COMPILE_NO_FLAGS);
			if (ctx->xkbKeymap) {
		...
		ctx->xkbKeymap =
			xkb_keymap_new_from_string(ctx->xkbCtx, keymapText, XKB_KEYMAP_FORMAT_TEXT_V1, XKB_KEYMAP_COMPILE_NO_FLAGS);
		memory::NkFree(keymapText);
	...
	static void OnKeyboardKey(void *data, wl_keyboard *, uint32_t, uint32_t, uint32_t key, uint32_t state) {
		auto *ctx = static_cast<NkWaylandSeatCtx *>(data);
		if (!ctx || !ctx->eventSystem)
			return;

		const xkb_keycode_t keycode = static_cast<xkb_keycode_t>(key + 8);
		const xkb_keysym_t sym = ctx->xkbState ? xkb_state_key_get_one_sym(ctx->xkbState, keycode) : XKB_KEY_NoSymbol;
	...
	static void OnKeyboardMods(void *data, wl_keyboard *, uint32_t, uint32_t depressed, uint32_t latched,
	...
	static const wl_keyboard_listener kKeyboardListener = {
		OnKeyboardKeymap, OnKeyboardEnter, OnKeyboardLeave, OnKeyboardKey, OnKeyboardMods, OnKeyboardRepeatInfo,
	};
	...
	static void OnPointerEnter(void *data, wl_pointer *, uint32_t serial, wl_surface *surface, wl_fixed_t sx,
							   wl_fixed_t sy) {
		auto *ctx = static_cast<NkWaylandSeatCtx *>(data);
		if (!ctx)
			return;
	...
	static void OnPointerButton(void *data, wl_pointer *, uint32_t serial, uint32_t, uint32_t button, uint32_t state) {
		auto *ctx = static_cast<NkWaylandSeatCtx *>(data);
		if (!ctx || !ctx->eventSystem)
			return;
		...
		switch (button) {
			case BTN_RIGHT:
				mapped = NkMouseButton::NK_MB_RIGHT;
				break;
			case BTN_MIDDLE:
				mapped = NkMouseButton::NK_MB_MIDDLE;
				break;
			case BTN_SIDE:
				mapped = NkMouseButton::NK_MB_BACK;
				break;
			case BTN_EXTRA:
				mapped = NkMouseButton::NK_MB_FORWARD;
				break;
			default:
				mapped = NkMouseButton::NK_MB_LEFT;
				break;
		}
	...
	static void OnPointerAxis(void *data, wl_pointer *, uint32_t, uint32_t axis, wl_fixed_t value) {
		auto *ctx = static_cast<NkWaylandSeatCtx *>(data);
		if (!ctx || !ctx->eventSystem)
			return;
	...
	static const wl_pointer_listener kPointerListener = {
		...
		OnPointerFrame, OnPointerAxisSource, OnPointerAxisStop, OnPointerAxisDiscrete,
	};
	...
	unsigned int NkWaylandLastInputSerial() {
		return static_cast<unsigned int>(sLastInputSerial);
	}
	...
	static void ResizeShmBuffer(NkWindow *window) {
		...
		if (!d.mShm || d.mWidth == 0 || d.mHeight == 0)
			return;
		...
		::wl_buffer *buf =
			wl_shm_pool_create_buffer(pool, 0, static_cast<int32_t>(d.mWidth), static_cast<int32_t>(d.mHeight),
	...
	void NkEventSystem::PumpOS() {
		...
		const uint32 windowCount = NkWESystem::Instance().GetWindowCount();

		for (uint32 i = 0; i < windowCount; ++i) {
			NkWindow *window = NkWESystem::Instance().GetWindowAt(i);
			if (!window)
				continue;
			...
			if (window->mData.mPendingResize) {
				...
				NkWindowResizeEvent e(window->mData.mWidth, window->mData.mHeight, window->mData.mPrevWidth,
			...
			if (window->mData.mPendingSizeRelease && window->mData.mConfigured) {
				...
				if (window->mData.mUsingLibdecor && window->mData.mLibdecorFrame) {
					...
					xdg_toplevel_set_min_size(window->mData.mXdgToplevel, minW, minH);
					xdg_toplevel_set_max_size(window->mData.mXdgToplevel, maxW, maxH);
					wl_surface_commit(window->mData.mSurface);
					wl_display_flush(display);
				}
			}

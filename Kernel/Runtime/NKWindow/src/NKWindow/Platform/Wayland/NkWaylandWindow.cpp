...
namespace nkentseu {
	...
	static const NkWaylandOutputInfo *PrimaryOutput() {
		...
		for (usize i = 0; i < outs.Size(); ++i) {
			if (outs[i]->posX == 0 && outs[i]->posY == 0)
				return outs[i];
		}
		return outs[0];
	}
	...
	static NkDisplayInfo WaylandFillDisplayInfo(const NkWaylandOutputInfo *o, uint32 index) {
		NkDisplayInfo info;
		info.index = index;
		if (!o)
			return info;
		...
		info.refreshRate = static_cast<uint32>(o->refreshMHz > 0 ? (o->refreshMHz + 500) / 1000 : 60);
	...
	static void EmitDisplayEvent(NkDisplayChange change, const NkWaylandOutputInfo *o, uint32 index) {
		if (!NkWESystem::Instance().IsInitialised())
			return;
	...
	static void OnOutputGeometry(void *data, ::wl_output * /*output*/, int32_t x, int32_t y, int32_t physWidth,
								 int32_t physHeight, int32_t /*subpixel*/, const char *make, const char *model,
								 int32_t /*transform*/) {
	...
	static void OnOutputDone(void *data, ::wl_output *) {
		...
		if (!info->announced) {
			...
			if (gOutputsBootstrapped) {
				...
				EmitDisplayEvent(NkDisplayChange::NK_DISPLAY_ADDED, info, idx >= 0 ? static_cast<uint32>(idx) : 0u);
			}
		}
	}
	...
	static void OnRegistryGlobal(void *data, ::wl_registry *registry, uint32_t name, const char *iface,
								 uint32_t version) {
		...
		if (::strcmp(iface, wl_compositor_interface.name) == 0) {
			...
			if (bindVersion > 0u) {
				window->mData.mWmBase =
					static_cast<::xdg_wm_base *>(wl_registry_bind(registry, name, &xdg_wm_base_interface, bindVersion));
			}
			...
			if (bindVersion > 0u) {
				window->mData.mShm =
					static_cast<::wl_shm *>(wl_registry_bind(registry, name, &wl_shm_interface, bindVersion));
			}
			...
			if (bindVersion > 0u) {
				window->mData.mSeat =
					static_cast<::wl_seat *>(wl_registry_bind(registry, name, &wl_seat_interface, bindVersion));
			}
			...
			if (!alreadyTracked && bindVersion > 0u) {
				...
				info->output =
					static_cast<::wl_output *>(wl_registry_bind(registry, name, &wl_output_interface, bindVersion));
				if (info->output) {
	...
	static void OnRegistryGlobalRemove(void *, ::wl_registry *, uint32_t name) {
		...
		for (usize i = 0; i < outs.Size(); ++i) {
			...
			if (info->registryName != name)
				continue;

			EmitDisplayEvent(NkDisplayChange::NK_DISPLAY_REMOVED, info, static_cast<uint32>(i));

			if (gCurrentOutput == info)
				gCurrentOutput = nullptr;
			if (info->output) {
	...
	static void OnToplevelConfigure(void *data, ::xdg_toplevel * /*toplevel*/, int32_t width, int32_t height,
									::wl_array *states) {
		NkWindow *window = static_cast<NkWindow *>(data);
		...
		if (states) {
			...
			for (uint32 i = 0; i < count; ++i) {
				if (s[i] == XDG_TOPLEVEL_STATE_FULLSCREEN)
					isFullscreen = true;
		...
		if (width > 0 && height > 0) {
			ApplyConfiguredSize(window, static_cast<uint32_t>(width), static_cast<uint32_t>(height));
		}
	...
	static void OnToplevelConfigureBounds(void * /*data*/, ::xdg_toplevel * /*toplevel*/, int32_t /*width*/,
										  int32_t /*height*/) {
...
#if __has_include("xdg-decoration-client-protocol.h") || __has_include(<xdg-decoration-client-protocol.h>)
	static void OnToplevelDecorationConfigure(void * /*data*/, ::zxdg_toplevel_decoration_v1 * /*deco*/,
											  uint32_t mode) {
	...
	static void OnLibdecorError(::libdecor * /*context*/, enum libdecor_error /*error*/, const char *message) {
		if (message && *message) {
			logger.Warn("[NkWindow] Wayland/libdecor: {}", message);
		}
	}

	static libdecor_interface kLibdecorInterface = {OnLibdecorError};

	static void OnLibdecorConfigure(::libdecor_frame *frame, ::libdecor_configuration *configuration, void *userData) {
	...
	static void OnLibdecorDismissPopup(::libdecor_frame * /*frame*/, const char * /*seatName*/, void * /*userData*/) {
	}

	static libdecor_frame_interface kLibdecorFrameInterface = {OnLibdecorConfigure, OnLibdecorClose, OnLibdecorCommit,
	...
	static ::wl_buffer *CreateShmBuffer(::wl_shm *shm, uint32_t width, uint32_t height, uint32_t stride,
										void **pixels) {
		if (pixels) {
			*pixels = nullptr;
		}
		...
		::wl_buffer *buf = wl_shm_pool_create_buffer(pool, 0, static_cast<int32_t>(width), static_cast<int32_t>(height),
	...
	static void DestroyWaylandResources(NkWindowData &d) {
		...
		// Curseur
		if (d.mCursorTheme) {
			wl_cursor_theme_destroy(d.mCursorTheme);
			d.mCursorTheme = nullptr;
		}
		if (d.mCursorSurface) {
			wl_surface_destroy(d.mCursorSurface);
			d.mCursorSurface = nullptr;
		}
		...
		if (d.mSurface) {
			wl_surface_destroy(d.mSurface);
			d.mSurface = nullptr;
		}
		if (d.mWmBase) {
			xdg_wm_base_destroy(d.mWmBase);
			d.mWmBase = nullptr;
		}
		...
		if (d.mShm) {
			wl_shm_destroy(d.mShm);
			d.mShm = nullptr;
		}
		if (d.mCompositor) {
			wl_compositor_destroy(d.mCompositor);
			d.mCompositor = nullptr;
		}
		...
		if (d.mDisplay) {
			...
			for (usize i = 0; i < outs.Size(); ++i) {
				if (outs[i]->output)
					wl_output_destroy(outs[i]->output);
		...
		if (d.mRegistry) {
			wl_registry_destroy(d.mRegistry);
			d.mRegistry = nullptr;
		}
		if (d.mDisplay) {
			wl_display_disconnect(d.mDisplay);
			d.mDisplay = nullptr;
		}
	}
	...
	bool NkWindow::Create(const NkWindowConfig &config) {
		...
		::xdg_toplevel *requestedParent = reinterpret_cast<::xdg_toplevel *>(config.native.parentWindowHandle);
		...
		if (!mData.mDisplay) {
			...
			mLastError = NkError(
				1, NkString::Fmtf(
					   ...
					   waylandDisplay ? waylandDisplay : "", runtimeDir ? runtimeDir : "", err, errMsg ? errMsg : ""));
			return false;
		}
		...
		// ------------------------------------------------------------------
		const bool enableCursorTheme = (::getenv("NK_WAYLAND_NO_CURSOR_THEME") == nullptr);
		...
		const bool enableLibdecor = (forceLibdecor || requestDecorations) && !disableLibdecor;
		if (enableLibdecor) {
			...
			if (mData.mLibdecor) {
				...
				if (mData.mLibdecorFrame) {
					...
					if (!config.resizable) {
						libdecor_frame_set_min_content_size(mData.mLibdecorFrame, static_cast<int32_t>(config.width),
															static_cast<int32_t>(config.height));
						libdecor_frame_set_max_content_size(mData.mLibdecorFrame, static_cast<int32_t>(config.width),
															static_cast<int32_t>(config.height));
					} else {
		...
		if (!usedLibdecor) {
			...
			if (!config.resizable) {
				xdg_toplevel_set_min_size(mData.mXdgToplevel, static_cast<int32_t>(config.width),
										  static_cast<int32_t>(config.height));
				xdg_toplevel_set_max_size(mData.mXdgToplevel, static_cast<int32_t>(config.width),
										  static_cast<int32_t>(config.height));
			} else {
				ApplySizeConstraints(mData, config);
			}
			...
			if (mData.mDecorationManager) {
				mData.mToplevelDecoration =
					zxdg_decoration_manager_v1_get_toplevel_decoration(mData.mDecorationManager, mData.mXdgToplevel);
				if (mData.mToplevelDecoration) {
					zxdg_toplevel_decoration_v1_add_listener(mData.mToplevelDecoration, &kToplevelDecorationListener,
															 this);
					zxdg_toplevel_decoration_v1_set_mode(mData.mToplevelDecoration,
														 config.frame ? ZXDG_TOPLEVEL_DECORATION_V1_MODE_SERVER_SIDE
		...
		// ------------------------------------------------------------------
		const bool enableShmBootstrap = (::getenv("NK_WAYLAND_DISABLE_SHM") == nullptr);
		if (enableShmBootstrap && mData.mShm) {
			mData.mStride = mData.mWidth * 4;
			mData.mBuffer = CreateShmBuffer(mData.mShm, mData.mWidth, mData.mHeight, mData.mStride, &mData.mPixels);

			if (mData.mBuffer && config.visible && mData.mConfigured) {
				memory::NkMemSet(mData.mPixels, 0, static_cast<size_t>(mData.mStride) * mData.mHeight);
				wl_surface_attach(mData.mSurface, mData.mBuffer, 0, 0);
				wl_surface_damage(mData.mSurface, 0, 0, static_cast<int32_t>(mData.mWidth),
		...
		if (config.dropEnabled) {
			mData.mDropTarget =
				memory::NkGetDefaultAllocator().New<NkWaylandDropTarget>(mData.mDisplay, mData.mSeat, mData.mSurface);
			if (mData.mDropTarget) {
				mData.mDropTarget->SetDataDeviceManager(mData.mDataDeviceManager);

				mData.mDropTarget->SetDropEnterCallback(
					NkWaylandDropTarget::DropEnterCallback([this](const NkDropEnterEvent &e) {
						NkDropEnterEvent copy(e);
						NkWESystem::Events().Enqueue_Public(copy, mId);
					}));
				mData.mDropTarget->SetDropLeaveCallback(
					NkWaylandDropTarget::DropLeaveCallback([this](const NkDropLeaveEvent &e) {
						NkDropLeaveEvent copy(e);
						NkWESystem::Events().Enqueue_Public(copy, mId);
					}));
				mData.mDropTarget->SetDropFileCallback(
					NkWaylandDropTarget::DropFileCallback([this](const NkDropFileEvent &e) {
						NkDropFileEvent copy(e);
						NkWESystem::Events().Enqueue_Public(copy, mId);
					}));
				mData.mDropTarget->SetDropTextCallback(
					NkWaylandDropTarget::DropTextCallback([this](const NkDropTextEvent &e) {
	...
	bool NkWindow::IsValid() const {
		return mIsOpen && mData.mSurface != nullptr;
	}
	...
	float NkWindow::GetDpiScale() const {
		...
		const NkWaylandOutputInfo *o = gCurrentOutput ? gCurrentOutput : PrimaryOutput();
		if (!o)
			return 1.0f;
		...
		if (o->physWidth > 0 && o->width > 0) {
			const float dpi = static_cast<float>(o->width) / (static_cast<float>(o->physWidth) / 25.4f);
	...
	void NkWindow::SetVisible(bool visible) {
		...
		if (!mData.mSurface || !mData.mDisplay)
			return;
		if (!mData.mConfigured)
			return;
	...
	void NkWindow::SetBackgroundColor(uint32 rgba) {
		...
		NkWindowRefuserUneFois(NkWindowProp::BgColor, "Wayland",
	...
	void NkWindow::BeginDragMove() {
		if (!mData.mXdgToplevel || !mData.mSeat)
			return;
	...
	void NkWindow::BeginResize(NkResizeEdge edge) {
		...
		switch (edge) {
			...
			case NkResizeEdge::TopLeft:
				e = XDG_TOPLEVEL_RESIZE_EDGE_TOP_LEFT;
				break;
			case NkResizeEdge::TopRight:
				e = XDG_TOPLEVEL_RESIZE_EDGE_TOP_RIGHT;
				break;
			case NkResizeEdge::BottomLeft:
				e = XDG_TOPLEVEL_RESIZE_EDGE_BOTTOM_LEFT;
				break;
			case NkResizeEdge::BottomRight:
				e = XDG_TOPLEVEL_RESIZE_EDGE_BOTTOM_RIGHT;
				break;
		}

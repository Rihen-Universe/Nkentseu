...
namespace nkentseu {
	...
	static void SyncConfigFromWindow(Display *display, ::Window xid, NkWindowConfig &config, const NkWindowData &data) {
		...
		if (wmState != None) {
			...
			if (XGetWindowProperty(display, xid, wmState, 0, 1024, False, XA_ATOM, &actualType, &actualFormat,
	...
	bool NkWindow::Create(const NkWindowConfig &config) {
		...
		int x =
			config.centered ? (DisplayWidth(sDisplay, mData.mScreen) - static_cast<int>(config.width)) / 2 : config.x;
		int y =
			config.centered ? (DisplayHeight(sDisplay, mData.mScreen) - static_cast<int>(config.height)) / 2 : config.y;
		...
		attrs.event_mask = ExposureMask | StructureNotifyMask | KeyPressMask | KeyReleaseMask | ButtonPressMask |
						   ButtonReleaseMask | PointerMotionMask | FocusChangeMask | EnterWindowMask | LeaveWindowMask;
		...
		if (config.surfaceHints.Has(NkSurfaceHintKey::NK_GLX_VISUAL_ID)) {
			VisualID vid = static_cast<VisualID>(config.surfaceHints.Get(NkSurfaceHintKey::NK_GLX_VISUAL_ID));
			...
			if (vi && n > 0) {
				...
				attrs.colormap = XCreateColormap(sDisplay, DefaultRootWindow(sDisplay), visual, AllocNone);
				vmask |= CWColormap;
		...
		mData.mXid = XCreateWindow(sDisplay, mData.mParentXid ? mData.mParentXid : DefaultRootWindow(sDisplay), x, y,
		...
		if (config.native.utilityWindow) {
			...
			XChangeProperty(sDisplay, mData.mXid, wmWindowType, XA_ATOM, 32, PropModeReplace,
		...
		if (config.transparent) {
			...
			XChangeProperty(sDisplay, mData.mXid, opacityAtom, XA_CARDINAL, 32, PropModeReplace,
		...
		if (mData.mDropTarget) {
			mData.mDropTarget->SetDropEnterCallback(
				NkXLibDropTarget::DropEnterCallback([this](const NkDropEnterEvent &ev) {
					NkDropEnterEvent copy(ev);
					NkWESystem::Events().Enqueue_Public(copy, mId);
				}));
			mData.mDropTarget->SetDropLeaveCallback(
				NkXLibDropTarget::DropLeaveCallback([this](const NkDropLeaveEvent &ev) {
					NkDropLeaveEvent copy(ev);
					NkWESystem::Events().Enqueue_Public(copy, mId);
				}));
			mData.mDropTarget->SetDropFileCallback(
				NkXLibDropTarget::DropFileCallback([this](const NkDropFileEvent &ev) {
					NkDropFileEvent copy(ev);
					NkWESystem::Events().Enqueue_Public(copy, mId);
				}));
			mData.mDropTarget->SetDropTextCallback(
				NkXLibDropTarget::DropTextCallback([this](const NkDropTextEvent &ev) {
	...
	NkVec2u NkWindow::GetPosition() const {
		...
		::Window child;
		int x = 0, y = 0;
		XTranslateCoordinates(mData.mDisplay, mData.mXid, DefaultRootWindow(mData.mDisplay), 0, 0, &x, &y, &child);
	...
	static bool XLibFillDisplayInfoFromCrtc(Display *display, XRRScreenResources *res, RRCrtc crtc,
		...
		for (int m = 0; m < res->nmode; ++m) {
			...
			if (mi.id == ci->mode) {
				if (mi.hTotal != 0 && mi.vTotal != 0) {
					double vrefresh = (double)mi.dotClock / ((double)mi.hTotal * (double)mi.vTotal);
		...
		if (oi) {
			...
			if (info.dpiScale < 0.5f)
				info.dpiScale = 1.f; // garde-fou valeurs aberrantes
	...
	NkDisplayInfo NkWindow::GetCurrentMonitor() const {
		...
		if (mons.Size() == 0) {
			return XLibFallbackDisplayInfo(mData.mDisplay ? mData.mDisplay : sDisplay, mData.mScreen);
		}
		...
		if (display && mData.mXid) {
			::Window child;
			int wx = 0, wy = 0;
			if (XTranslateCoordinates(display, mData.mXid, DefaultRootWindow(display), 0, 0, &wx, &wy, &child)) {
				XWindowAttributes a;
	...
	NkVec2u NkWindow::GetDisplaySize() const {
		...
		return {static_cast<uint32>(DisplayWidth(mData.mDisplay, mData.mScreen)),
	...
	void NkWindow::SetBackgroundColor(uint32 rgba) {
		...
		NkWindowRefuserUneFois(NkWindowProp::BgColor, "XLib",
	...
	void NkWindow::BeginDragMove() {
		NkXlibMoveResize(mData.mDisplay, mData.mXid, 8 /*_NET_WM_MOVERESIZE_MOVE*/);
	}
	...
	void NkWindow::SetMousePosition(uint32 x, uint32 y) {
		if (mData.mDisplay && mData.mXid) {
			XWarpPointer(mData.mDisplay, None, mData.mXid, 0, 0, 0, 0, static_cast<int>(x), static_cast<int>(y));
		}
	}
	...
	void NkWindow::ShowMouse(bool show) {
		...
		if (show) {
			...
			if (!blankCursor) {
				...
				blankCursor = XCreatePixmapCursor(mData.mDisplay, blank, blank, &dummy, &dummy, 0, 0);

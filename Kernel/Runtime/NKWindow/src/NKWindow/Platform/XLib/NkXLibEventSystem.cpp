...
namespace nkentseu {
	...
	void NkEventSystem::PumpOS() {
		...
		while (XPending(display) > 0) {
			...
			// ------------------------------------------------------------
			if (sRandrAvailable && sRandrEventBase >= 0 && xev.type == sRandrEventBase + RRScreenChangeNotify) {
			...
			switch (xev.type) {
				...
				case Expose: {
					if (xev.xexpose.count == 0) {
						NkWindowPaintEvent e(xev.xexpose.x, xev.xexpose.y, static_cast<uint32>(xev.xexpose.width),
				...
				case ConfigureNotify: {
					...
					}
					{
						NkWindowResizeEvent e(
							static_cast<uint32>(xev.xconfigure.width), static_cast<uint32>(xev.xconfigure.height),
							window ? window->GetConfig().width : 0u, window ? window->GetConfig().height : 0u);
						Enqueue(e, winId);
					}
					{

...
namespace nkentseu {
	...
	// ── Fenêtre discrète : CoreWindow UWP n'expose ni opacité, ni toujours-
	// devant, ni click-through (fenêtrage géré par le shell). Intention
	// mémorisée seulement, comme SetDecorated ci-dessus.
	...
	void NkWindow::SetBackgroundColor(uint32 rgba) {
		...
		NkWindowRefuserUneFois(NkWindowProp::BgColor, "UWP",
	...
	// COORDONNEES CLIENT : NON IMPLEMENTE sur UWP. On le DIT et on rend
	...
	bool NkWindow::SetMousePositionClient(int32 x, int32 y) {
		(void)x;
		(void)y;
		NkLog::Instance().Warnf("[NkWindow] SetMousePositionClient : NON IMPLEMENTE sur UWP.");
		return false;
	}

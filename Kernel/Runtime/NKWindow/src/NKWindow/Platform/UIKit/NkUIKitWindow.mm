...
- (void)nk_emitTextFromString:(NSString *)text {
	...
	for (NSUInteger i = 0; i < count; ++i) {
		...
		if (cp == '\n' || cp == '\r') {
			nkentseu::NkKeyPressEvent press(nkentseu::NkKey::NK_ENTER, nkentseu::NkScancode::NK_SC_ENTER, {}, 0, false,
											winId);
			nkentseu::NkWESystem::Events().Enqueue_Public(press, winId);
			nkentseu::NkKeyReleaseEvent release(nkentseu::NkKey::NK_ENTER, nkentseu::NkScancode::NK_SC_ENTER, {}, 0,
												false, winId);
			nkentseu::NkWESystem::Events().Enqueue_Public(release, winId);
			continue;
		}
		if (cp == '\t') {
			nkentseu::NkKeyPressEvent press(nkentseu::NkKey::NK_TAB, nkentseu::NkScancode::NK_SC_TAB, {}, 0, false,
											winId);
			nkentseu::NkWESystem::Events().Enqueue_Public(press, winId);
			nkentseu::NkKeyReleaseEvent release(nkentseu::NkKey::NK_TAB, nkentseu::NkScancode::NK_SC_TAB, {}, 0, false,
												winId);
			nkentseu::NkWESystem::Events().Enqueue_Public(release, winId);
			continue;
		}
...
- (void)deleteBackward {
	...
	const nkentseu::uint64 winId = mOwner->GetId();
	nkentseu::NkKeyPressEvent press(nkentseu::NkKey::NK_BACK, nkentseu::NkScancode::NK_SC_BACKSPACE, {}, 0, false,
									winId);
	nkentseu::NkWESystem::Events().Enqueue_Public(press, winId);
	nkentseu::NkKeyReleaseEvent release(nkentseu::NkKey::NK_BACK, nkentseu::NkScancode::NK_SC_BACKSPACE, {}, 0, false,
										winId);
	nkentseu::NkWESystem::Events().Enqueue_Public(release, winId);
}
...
namespace nkentseu {
	...
	NkVec2u NkWindow::GetDisplaySize() const {
		...
		return {static_cast<uint32>(screen.bounds.size.width * scale),
	...
	NkDisplayInfo NkWindow::GetCurrentMonitor() const {
		...
		UIScreen *screen = (mData.mUIWindow && mData.mUIWindow.screen) ? mData.mUIWindow.screen : [UIScreen mainScreen];
	...
	void NkWindow::SetBackgroundColor(uint32 rgba) {
		...
		NkWindowRefuserUneFois(NkWindowProp::BgColor, "UIKit",
	...
	static UIReturnKeyType UIKitMapReturnKey(NkWindow::NkSoftKeyboardReturnKey r) {
		switch (r) {
			case NkWindow::NkSoftKeyboardReturnKey::Done:
				return UIReturnKeyDone;
			case NkWindow::NkSoftKeyboardReturnKey::Go:
				return UIReturnKeyGo;
			case NkWindow::NkSoftKeyboardReturnKey::Next:
				return UIReturnKeyNext;
			case NkWindow::NkSoftKeyboardReturnKey::Search:
				return UIReturnKeySearch;
			case NkWindow::NkSoftKeyboardReturnKey::Send:
	...
	void NkWindow::ShowSoftKeyboard(const NkSoftKeyboardConfig &config) {
		...
		view.autocorrectionType = config.autocorrect ? UITextAutocorrectionTypeYes : UITextAutocorrectionTypeNo;
		view.autocapitalizationType =
			config.autocapitalize ? UITextAutocapitalizationTypeSentences : UITextAutocapitalizationTypeNone;

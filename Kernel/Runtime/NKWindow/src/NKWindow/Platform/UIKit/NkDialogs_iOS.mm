...
- (instancetype)initWithCallback:(const nkentseu::NkDialogs::Callback&)cb {
    if ((self = [super init])) { _cb = cb; self.selfRef = self; }
    return self;
}
- (void)finishWithPath:(NSString*)path confirmed:(bool)ok {
    nkentseu::NkDialogResult r;
    r.confirmed = ok;
    if (path) r.path = nkentseu::NkString([path UTF8String]);
    if (_cb) _cb(r);
...
static void NkPresentPicker(NSArray<NSString*>* types, UIDocumentPickerMode mode,
                            const NkDialogs::Callback& cb) {
    NkDialogs::Callback cbCopy = cb;
    dispatch_async(dispatch_get_main_queue(), ^{
        UIViewController* vc = NkTopViewController();
        if (!vc) { if (cbCopy) { NkDialogResult r; cbCopy(r); } return; }
        NkDocPickerDelegate* del = [[NkDocPickerDelegate alloc] initWithCallback:cbCopy];
        UIDocumentPickerViewController* p =
            [[UIDocumentPickerViewController alloc] initWithDocumentTypes:types inMode:mode];
        p.delegate = del;
...
void NkDialogs::OpenFileDialogAsync(const Callback& cb, const NkString& /*filter*/, const NkString& /*title*/) {
    NkPresentPicker(@[@"public.data", @"public.content", @"public.item"],
                    UIDocumentPickerModeImport, cb);
}
...
void NkDialogs::SaveFileDialogAsync(const Callback& cb, const NkString& defaultExt, const NkString& /*title*/) {
    ...
    if (dirs.count > 0) {
        ...
        if (defaultExt.CStr() && *defaultExt.CStr())
            full = [full stringByAppendingPathExtension:
                        [NSString stringWithUTF8String:defaultExt.CStr()]];
        r.confirmed = true;

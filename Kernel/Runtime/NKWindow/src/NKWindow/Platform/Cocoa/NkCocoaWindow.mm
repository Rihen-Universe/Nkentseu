...
namespace nkentseu {
	...
	static NkVec2u QueryContentSizePx(NSWindow *window) {
		...
		return {static_cast<uint32>(math::NkMax(0.0f, static_cast<float>(contentRect.size.width)) * scale),
				...
				mData.mScreenObserver = [[NSNotificationCenter defaultCenter]
					addObserverForName:NSApplicationDidChangeScreenParametersNotification
							...
							usingBlock:^(NSNotification *note) {
							  ...
							  NkSystemDisplayEvent evt(NkDisplayChange::NK_DISPLAY_RESOLUTION_CHANGED, info,
													   self->GetId());
	...
	NkVec2u NkWindow::GetPosition() const {
		...
		NkVec2u pos = {static_cast<uint32>(math::NkMax(0.0, frame.origin.x)),
	...
	NkVec2u NkWindow::GetDisplaySize() const {
		...
		return {static_cast<uint32>(math::NkMax(0.0, frame.size.width * scale)),
	...
	NkDisplayInfo NkWindow::GetCurrentMonitor() const {
		...
		NSScreen *screen = (mData.mNSWindow && mData.mNSWindow.screen) ? mData.mNSWindow.screen : [NSScreen mainScreen];
	...
	// COORDONNEES CLIENT : NON IMPLEMENTE sur macOS. On le DIT et on rend
	...
	bool NkWindow::SetMousePositionClient(int32 x, int32 y) {
		(void)x;
		(void)y;
		NkLog::Instance().Warnf("[NkWindow] SetMousePositionClient : NON IMPLEMENTE sur macOS.");
		return false;
	}

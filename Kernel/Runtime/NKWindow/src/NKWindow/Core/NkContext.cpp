...
namespace nkentseu {
	...
	namespace {
		...
		static bool NkIsSurfaceOnlyApi(graphics::NkGraphicsApi api) {
			return api == graphics::NkGraphicsApi::NK_GFX_API_VULKAN ||
				   api == graphics::NkGraphicsApi::NK_GFX_API_DX11 || api == graphics::NkGraphicsApi::NK_GFX_API_DX12 ||
		...
		static bool NkShouldRequestExplicitGL(const NkContextConfig &config) {
			return config.versionMajor >= 3u || config.versionMinor > 0u ||
				   config.profile != NkContextProfile::NK_CONTEXT_PROFILE_ANY || config.debug;
		}
	...
	(defined(NKENTSEU_WINDOWING_WAYLAND) && NKENTSEU_HAS_WAYLAND_EGL) || defined(NKENTSEU_PLATFORM_EMSCRIPTEN)
		...
		static bool NkEglChooseConfig(EGLDisplay display, const NkContextConfig &config, EGLConfig *outConfig,
									  bool esProfile) {
			if (!outConfig || display == EGL_NO_DISPLAY)
				return false;
			...
			const EGLint attrs[] = {EGL_SURFACE_TYPE,
									...
									renderableType,
									EGL_RED_SIZE,
									static_cast<EGLint>(config.redBits),
									EGL_GREEN_SIZE,
									static_cast<EGLint>(config.greenBits),
									EGL_BLUE_SIZE,
									static_cast<EGLint>(config.blueBits),
									EGL_ALPHA_SIZE,
									static_cast<EGLint>(config.alphaBits),
									EGL_DEPTH_SIZE,
									static_cast<EGLint>(config.depthBits),
									EGL_STENCIL_SIZE,
									static_cast<EGLint>(config.stencilBits),
									EGL_SAMPLES,
									(config.msaaSamples > 1u) ? static_cast<EGLint>(config.msaaSamples) : 0,
									EGL_NONE};
			EGLint count = 0;
			return eglChooseConfig(display, attrs, outConfig, 1, &count) == EGL_TRUE && count >= 1;
		}

		static EGLContext NkEglCreateContext(EGLDisplay display, EGLConfig config, const NkContextConfig &cfg) {
			const EGLint ctxAttrs[] = {EGL_CONTEXT_CLIENT_VERSION,
									   static_cast<EGLint>(NkClampU32Min(cfg.versionMajor, 2u)), EGL_NONE};
		...
		static BYTE NkToLayerByte(int32 layerType) {
			if (layerType > 0)
				return static_cast<BYTE>(PFD_OVERLAY_PLANE);
			if (layerType < 0)
				return static_cast<BYTE>(PFD_UNDERLAY_PLANE);
		...
		static void NkBuildWglContextAttribs(const NkContextConfig &config, int (&attrs)[16]) {
			...
			switch (config.profile) {
				...
				case NkContextProfile::NK_CONTEXT_PROFILE_COMPATIBILITY:
					profileMask = kWglContextCompatibilityBit;
					break;
		...
		static PIXELFORMATDESCRIPTOR NkBuildPixelFormatDescriptor(const NkContextConfig &config) {
			...
			if (custom.useCustomDescriptor) {
				DWORD flags = 0;
				if (custom.drawToWindow)
					flags |= PFD_DRAW_TO_WINDOW;
				if (custom.supportOpenGL)
					flags |= PFD_SUPPORT_OPENGL;
				if (custom.doubleBuffer)
					flags |= PFD_DOUBLEBUFFER;
				if (custom.stereo)
					flags |= PFD_STEREO;
				if (custom.supportGDI)
					flags |= PFD_SUPPORT_GDI;
				if (custom.genericFormat)
					flags |= PFD_GENERIC_FORMAT;
				if (custom.genericAccelerated)
			...
			const uint32 totalColor =
				NkClampU32Min(config.redBits + config.greenBits + config.blueBits + config.alphaBits, 24u);
		...
		static void *NkWglProc(const char *name) {
			...
			if (!sOgl)
				sOgl = LoadLibraryA("opengl32.dll");
		...
		static void BuildGlxContextAttribs(const NkContextConfig &config, int (&attrs)[16]) {
			...
			switch (config.profile) {
				case NkContextProfile::NK_CONTEXT_PROFILE_CORE:
					...
					break;
				case NkContextProfile::NK_CONTEXT_PROFILE_COMPATIBILITY:
					profileMask = kGlxContextCompatibilityBit;
					break;
				case NkContextProfile::NK_CONTEXT_PROFILE_ES:
		...
		static GLXFBConfig ChooseGlxFBConfig(Display *dpy, int screen, const NkContextConfig &config,
			...
			const int attrs[] = {GLX_X_RENDERABLE,
								 ...
								 GLX_TRUE_COLOR,
								 GLX_RED_SIZE,
								 static_cast<int>(NkClampU32Min(config.redBits, 1u)),
								 GLX_GREEN_SIZE,
								 static_cast<int>(NkClampU32Min(config.greenBits, 1u)),
								 GLX_BLUE_SIZE,
								 static_cast<int>(NkClampU32Min(config.blueBits, 1u)),
								 GLX_ALPHA_SIZE,
								 static_cast<int>(config.alphaBits),
								 GLX_DEPTH_SIZE,
								 static_cast<int>(config.depthBits),
								 GLX_STENCIL_SIZE,
								 static_cast<int>(config.stencilBits),
								 GLX_DOUBLEBUFFER,
								 config.doubleBuffer ? True : False,
								 GLX_STEREO,
								 config.stereo ? True : False,
								 GLX_ACCUM_RED_SIZE,
								 static_cast<int>(config.accumRedBits),
								 GLX_ACCUM_GREEN_SIZE,
								 static_cast<int>(config.accumGreenBits),
								 GLX_ACCUM_BLUE_SIZE,
								 static_cast<int>(config.accumBlueBits),
								 GLX_ACCUM_ALPHA_SIZE,
								 static_cast<int>(config.accumAlphaBits),
								 GLX_SAMPLE_BUFFERS,
								 hasMsaa,
								 GLX_SAMPLES,
								 msaaSamps,
		...
		static void ApplyGlxVisualHint(const NkContextConfig &hints, NkWindowConfig &cfg) {
			...
			const int attrs[] = {GLX_X_RENDERABLE,
								 ...
								 GLX_TRUE_COLOR,
								 GLX_RED_SIZE,
								 static_cast<int>(NkClampU32Min(hints.redBits, 1u)),
								 GLX_GREEN_SIZE,
								 static_cast<int>(NkClampU32Min(hints.greenBits, 1u)),
								 GLX_BLUE_SIZE,
								 static_cast<int>(NkClampU32Min(hints.blueBits, 1u)),
								 GLX_ALPHA_SIZE,
								 static_cast<int>(hints.alphaBits),
								 GLX_DEPTH_SIZE,
								 static_cast<int>(hints.depthBits),
								 GLX_STENCIL_SIZE,
								 static_cast<int>(hints.stencilBits),
								 GLX_DOUBLEBUFFER,
								 hints.doubleBuffer ? True : False,
								 GLX_STEREO,
								 hints.stereo ? True : False,
								 GLX_ACCUM_RED_SIZE,
								 static_cast<int>(hints.accumRedBits),
								 GLX_ACCUM_GREEN_SIZE,
								 static_cast<int>(hints.accumGreenBits),
								 GLX_ACCUM_BLUE_SIZE,
								 static_cast<int>(hints.accumBlueBits),
								 GLX_ACCUM_ALPHA_SIZE,
								 static_cast<int>(hints.accumAlphaBits),
								 GLX_SAMPLE_BUFFERS,
								 (hints.msaaSamples > 1u) ? 1 : 0,
								 GLX_SAMPLES,
								 (hints.msaaSamples > 1u) ? static_cast<int>(hints.msaaSamples) : 0,
								 0};
			int n = 0;
			GLXFBConfig *fbs = glXChooseFBConfig(dpy, screen, attrs, &n);
			if (fbs && n > 0) {
				...
				if (vi) {
					cfg.surfaceHints.Set(NkSurfaceHintKey::NK_GLX_VISUAL_ID, static_cast<uintptr>(vi->visualid));
					platform::NkX11Free(vi);
				}
		...
		static bool CreateOpenGLContext(NkContext &context) {
			...
			if (pixelFormat == 0) {
				...
				const uintptr shareHwndHint =
					surface.appliedHints.Get(NkSurfaceHintKey::NK_WGL_SHARE_PIXEL_FORMAT_HWND, 0);
				if (shareHwndHint != 0) {
					...
					spfd.nVersion = 1;
					const bool canDesc = (spf != 0) && (DescribePixelFormat(shareDC, spf, sizeof(spfd), &spfd) > 0);
				...
				if (!appliedShared) {
					...
					if (context.config.win32PixelFormat.forcedPixelFormatIndex > 0) {
						...
						described.nSize = sizeof(described);
			...
			if (NkShouldRequestExplicitGL(context.config)) {
				...
				auto createAttribs =
					reinterpret_cast<NkWglCreateContextAttribsProc>(wglGetProcAddress("wglCreateContextAttribsARB"));
				if (!createAttribs) {
			...
			const int screen = DefaultScreen(dpy);
			const VisualID hinted = static_cast<VisualID>(surface.appliedHints.Get(NkSurfaceHintKey::NK_GLX_VISUAL_ID));
			...
			if (NkShouldRequestExplicitGL(context.config)) {
				...
				if (!vi) {
					const int attrs[] = {GLX_RGBA,
										 GLX_RED_SIZE,
										 static_cast<int>(NkClampU32Min(context.config.redBits, 1u)),
										 GLX_GREEN_SIZE,
										 static_cast<int>(NkClampU32Min(context.config.greenBits, 1u)),
										 GLX_BLUE_SIZE,
										 static_cast<int>(NkClampU32Min(context.config.blueBits, 1u)),
										 GLX_ALPHA_SIZE,
										 static_cast<int>(context.config.alphaBits),
										 GLX_DOUBLEBUFFER,
										 context.config.doubleBuffer ? True : False,
										 GLX_DEPTH_SIZE,
										 static_cast<int>(context.config.depthBits),
										 GLX_STENCIL_SIZE,
										 static_cast<int>(context.config.stencilBits),
										 0};
			...
			EGLDisplay eglDisplay = externalDisplay ? reinterpret_cast<EGLDisplay>(eglDisplayHint)
			...
			EGLConfig eglConfig =
				reinterpret_cast<EGLConfig>(surface.appliedHints.Get(NkSurfaceHintKey::NK_EGL_CONFIG, 0));
			...
			EGLSurface eglSurface =
				eglCreateWindowSurface(eglDisplay, eglConfig, reinterpret_cast<EGLNativeWindowType>(ohWin), nullptr);
			if (eglSurface == EGL_NO_SURFACE) {
				eglTerminate(eglDisplay);
			...
			NSOpenGLPixelFormatAttribute pixelAttrs[] = {
				...
				static_cast<NSOpenGLPixelFormatAttribute>(context.config.depthBits),
				NSOpenGLPFAStencilSize,
				static_cast<NSOpenGLPixelFormatAttribute>(context.config.stencilBits),
				NSOpenGLPFAColorSize,
				static_cast<NSOpenGLPixelFormatAttribute>(context.config.redBits + context.config.greenBits +
														  context.config.blueBits + context.config.alphaBits),
				NSOpenGLPFAOpenGLProfile,
				context.config.versionMajor >= 4
					? NSOpenGLProfileVersion4_1Core
					: (context.config.versionMajor >= 3 ? NSOpenGLProfileVersion3_2Core : NSOpenGLProfileVersionLegacy),
				0};
			NSOpenGLPixelFormat *pixFmt = [[NSOpenGLPixelFormat alloc] initWithAttributes:pixelAttrs];
			if (!pixFmt) {
				NkSetContextError(context, 1400, "NSOpenGLPixelFormat creation failed (macOS).");
				return false;
			}
			NSOpenGLContext *glCtx = [[NSOpenGLContext alloc] initWithFormat:pixFmt shareContext:nil];
			[pixFmt release];
			if (!glCtx) {
			...
			context.getProcAddress = [](const char *name) -> void * {
				...
				if (!handle)
					handle = dlopen("/System/Library/Frameworks/OpenGL.framework/OpenGL", RTLD_LAZY);
	...
	defined(NKENTSEU_PLATFORM_VISIONOS)
			...
			EMSCRIPTEN_WEBGL_CONTEXT_HANDLE webglCtx = emscripten_webgl_create_context(canvasId, &attrs);
			if (webglCtx <= 0) {
	...
	void NkContextResetHints() {
		...
		gHints.accumRedBits = gHints.accumGreenBits = gHints.accumBlueBits = gHints.accumAlphaBits = 0;
		gHints.auxBuffers = 0;
	}
	...
	void NkContextWindowHint(NkContextHint hint, int32 value) {
		switch (hint) {
			...
			case NkContextHint::NK_CONTEXT_HINT_VERSION_MAJOR:
				if (value > 0)
					gHints.versionMajor = static_cast<uint32>(value);
				break;
			case NkContextHint::NK_CONTEXT_HINT_VERSION_MINOR:
				if (value >= 0)
					gHints.versionMinor = static_cast<uint32>(value);
				break;
	...
	void NkContextApplyWindowHints(NkWindowConfig &config) {
		if (gHints.api != graphics::NkGraphicsApi::NK_GFX_API_OPENGL)
			return;
		ApplyGlxVisualHint(gHints, config);
	}

	NkContextMode NkContextGetModeForApi(graphics::NkGraphicsApi api) {
		return NkIsSurfaceOnlyApi(api) ? NkContextMode::NK_CONTEXT_MODE_SURFACE_ONLY
...
#elif defined(NKENTSEU_PLATFORM_ANDROID) || (defined(NKENTSEU_PLATFORM_HARMONYOS) && NKENTSEU_HAS_HARMONY_EGL)
	...
	defined(__OBJC__)
			...
			EMSCRIPTEN_WEBGL_CONTEXT_HANDLE webglCtx =
				static_cast<EMSCRIPTEN_WEBGL_CONTEXT_HANDLE>(reinterpret_cast<intptr_t>(context.nativeContext));
			if (webglCtx > 0)
	...
	bool NkContextMakeCurrent(NkContext &context) {
		if (!context.valid)
			return false;
		if (context.mode == NkContextMode::NK_CONTEXT_MODE_SURFACE_ONLY)
			return true;
		...
		::wl_egl_window *eglWin = static_cast<::wl_egl_window *>(context.nativeWindow);
		if (eglWin) {
			wl_egl_window_resize(eglWin, static_cast<int>(NkClampU32Min(context.surface.width, 1u)),
								 static_cast<int>(NkClampU32Min(context.surface.height, 1u)), 0, 0);
		}
	...
	defined(__OBJC__)
		...
		EMSCRIPTEN_WEBGL_CONTEXT_HANDLE webglCtx =
			static_cast<EMSCRIPTEN_WEBGL_CONTEXT_HANDLE>(reinterpret_cast<intptr_t>(context.nativeContext));
		if (webglCtx <= 0 || emscripten_webgl_make_context_current(webglCtx) != EMSCRIPTEN_RESULT_SUCCESS) {
	...
	void *NkContextGetProcAddress(NkContext &context, const char *procName) {
		if (!context.valid || !procName || !context.getProcAddress)
			return nullptr;

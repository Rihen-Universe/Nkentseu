	...
	namespace renderer {
		...
		class NkMaterialLibrary {
			public:
				...
				// ── Cycle de vie ──────────────────────────────────────────────
				bool Init(NkIDevice *device, NkMaterialSystem *materialSystem, NkTextureLibrary *textureLibrary);
				void Shutdown();
				...
				bool Save(const NkMaterialAsset &asset, const NkString &outPath, NkAssetId *outGeneratedId = nullptr);

				// ── Introspection ─────────────────────────────────────────────
				uint32 CountLoaded() const noexcept {
					return (uint32)mLoaded.Size();
				}

	...
	namespace renderer {
		...
		struct NkMaterialAsset : public NkISerializable {
				...
				[[nodiscard]] nk_bool Deserialize(const NkArchive &archive) override {
					// Identite + type
					nk_uint32 rawType = 0;
					(void)archive.GetUInt32("type", rawType);
					type = static_cast<NkMaterialType>(rawType);
					(void)archive.GetString("name", name);
					nk_uint32 rawQ = 0, rawB = 0, rawC = 0, rawF = 0;
					(void)archive.GetUInt32("queue", rawQ);
					queue = static_cast<NkRenderQueue>(rawQ);
					(void)archive.GetUInt32("blendMode", rawB);
					blendMode = static_cast<NkBlendMode>(rawB);
					(void)archive.GetUInt32("cullMode", rawC);
					cullMode = static_cast<NkCullMode>(rawC);
					(void)archive.GetUInt32("fillMode", rawF);
					fillMode = static_cast<NkFillMode>(rawF);

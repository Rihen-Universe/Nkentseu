...
namespace nkentseu {
	namespace renderer {
		...
		// Init / Shutdown
		// =====================================================================
		bool NkMaterialLibrary::Init(NkIDevice *device, NkMaterialSystem *materialSystem,
		...
		uint32 NkMaterialLibrary::ScanDirectory(const NkString &assetRootDir) {
			...
			for (nk_size i = 0; i < files.Size(); ++i) {
				...
				if (meta.type != NkAssetType::Material)
					continue;
		...
		// =====================================================================
		bool NkMaterialLibrary::Save(const NkMaterialAsset &asset, const NkString &outPath, NkAssetId *outGeneratedId) {
			NkAssetMetadata meta;
			...
			NkString err;
			if (!NkAssetIO::Write(outPath.CStr(), meta, payload.Data(), payload.Size(), &err)) {
				logger.Errorf("[NkMaterialLibrary] Save: %s\n", err.CStr());
				return false;
			}
			if (outGeneratedId)
				*outGeneratedId = meta.id;
			return true;
		}
		...
		class MaterialLibraryWatcherCb : public NkFileWatcherCallback {
			public:
				...
				void OnFileChanged(const NkFileChangeEvent &ev) override {
					if (!library)
						return;
					if (ev.Type != NkFileChangeType::NK_MODIFIED && ev.Type != NkFileChangeType::NK_CREATED)
						return;
		...
		void NkMaterialLibrary::PollHotReload(float32 deltaTime) {
			...
			for (auto &[key, entry] : mLoaded) {
				NkAssetMetadata meta;
				if (!NkAssetIO::ReadMetadata(entry.diskPath.CStr(), meta))
					continue;
		...
		// =====================================================================
		NkMatInstHandle NkMaterialLibrary::CreateOrUpdateInstance(LoadedEntry &entry, bool isReload) {
			...
			if (!isReload || !handle.IsValid()) {
				...
				switch (entry.asset.type) {
					...
					case NkMaterialType::NK_PBR_SPECULAR:
						tmpl = mMatSys->DefaultPBR();
						break;
					case NkMaterialType::NK_TOON:
						tmpl = mMatSys->DefaultToon();
						break;
					case NkMaterialType::NK_UNLIT:
						tmpl = mMatSys->DefaultUnlit();
						break;
					case NkMaterialType::NK_SKIN:
						tmpl = mMatSys->DefaultSkin();
						break;
					case NkMaterialType::NK_HAIR:
						tmpl = mMatSys->DefaultHair();
						break;
					case NkMaterialType::NK_ANIME:
						tmpl = mMatSys->DefaultAnime();
						break;
					case NkMaterialType::NK_ARCHIVIZ:
						tmpl = mMatSys->DefaultArchviz();
						break;
					case NkMaterialType::NK_REFL_FLOOR:
						tmpl = mMatSys->DefaultReflFloor();
						break;
					default:
						tmpl = mMatSys->DefaultPBR();
						break;
			...
			const bool isToonLike =
				(entry.asset.type == NkMaterialType::NK_TOON || entry.asset.type == NkMaterialType::NK_TOON_INK ||
				 entry.asset.type == NkMaterialType::NK_ANIME);

			if (isToonLike) {
				const auto &t = entry.asset.toon;
				inst->SetAlbedo({t.albedoColor.x, t.albedoColor.y, t.albedoColor.z}, t.albedoColor.w);
				...
				inst->SetOutline(t.outlineWidth, {t.outlineColor.x, t.outlineColor.y, t.outlineColor.z});
				inst->SetRim(t.rimIntensity, {t.rimColor.x, t.rimColor.y, t.rimColor.z});
				...
				inst->SetEmissive({p.emissive.x, p.emissive.y, p.emissive.z}, p.emissiveStrength);
				if (p.subsurface > 0.f) {
					inst->SetSubsurface(p.subsurface, {p.subsurfaceColor.x, p.subsurfaceColor.y, p.subsurfaceColor.z});
				}
				if (p.clearcoat > 0.f) {
		...
		// =====================================================================
		bool NkMaterialLibrary::ReadAssetFromDisk(const NkString &diskPath, NkMaterialAsset &out,

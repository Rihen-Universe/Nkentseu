	...
	namespace renderer {
		...
		namespace {
			...
			bool ParseElement(const char *&p, const char *end, XmlNode &node) {
				...
				node.name.Clear();
				if (p > s)
					node.name.Append(s, (NkString::SizeType)(p - s));
				// attributs
				for (;;) {
					...
					XmlAttr at;
					if (p > as)
						at.name.Append(as, (NkString::SizeType)(p - as));
					while (p < end && (*p == ' ' || *p == '='))
						++p;
					if (p < end && (*p == '"' || *p == '\'')) {
						char q = *p;
						...
						while (p < end && *p != q)
							++p;
						if (p > vs)
							at.value.Append(vs, (NkString::SizeType)(p - vs));
						if (p < end)
							++p;
					}
					if (!at.name.Empty())
						node.attrs.PushBack(static_cast<XmlAttr &&>(at));
				}
				if (p < end && *p == '/') { // <name .../>
					p += 1;
					if (p < end && *p == '>')
			...
			void ExtractMesh(const XmlNode &mesh, NkGLTFMeshData &out) {
				...
				for (uint32 i = 0; i < (uint32)mesh.children.Size(); ++i) {
					...
					if (!(sn.name == NkString("source")))
						continue;
					Source src;
					if (auto *id = sn.Attr("id"))
						src.id = *id;
					if (const XmlNode *fa = sn.Child("float_array"))
						ParseFloats(fa->text, src.data);
					if (const XmlNode *tc = sn.Child("technique_common"))
						if (const XmlNode *ac = tc->Child("accessor"))
							if (auto *st = ac->Attr("stride")) {
								int32 v = 0;
								if (st->ToInt(v))
									src.stride = v;
							}
				...
				auto findSrc = [&](const NkString &id) -> const Source * {
					...
					for (uint32 i = 0; i < (uint32)sources.Size(); ++i)
						if (sources[(NkVector<Source>::SizeType)i].id == want)
							return &sources[(NkVector<Source>::SizeType)i];
					return nullptr;
				};
				// <vertices id=...> -> POSITION source
				NkString verticesId, posSrcFromVerts;
				if (const XmlNode *vn = mesh.Child("vertices")) {
					if (auto *id = vn->Attr("id"))
						verticesId = *id;
					for (uint32 i = 0; i < (uint32)vn->children.Size(); ++i) {
						...
						if (in.name == NkString("input"))
							if (auto *sem = in.Attr("semantic"))
								if (*sem == NkString("POSITION"))
									if (auto *src = in.Attr("source"))
										posSrcFromVerts = *src;
					}
				}
				...
				for (uint32 ci = 0; ci < (uint32)mesh.children.Size(); ++ci) {
					...
					// inputs
					const Source *posSrc = nullptr;
					int32 posOff = -1, posStride = 3;
					const Source *nrmSrc = nullptr;
					int32 nrmOff = -1, nrmStride = 3;
					const Source *uvSrc = nullptr;
					int32 uvOff = -1, uvStride = 2;
					int32 maxOff = 0;
					for (uint32 i = 0; i < (uint32)prim.children.Size(); ++i) {
						const XmlNode &in = prim.children[(NkVector<XmlNode>::SizeType)i];
						if (!(in.name == NkString("input")))
							continue;
						auto *sem = in.Attr("semantic");
						auto *src = in.Attr("source");
						...
						if (*sem == NkString("VERTEX")) {
							...
							nrmOff = o;
							if (nrmSrc)
								nrmStride = nrmSrc->stride;
						} else if (*sem == NkString("TEXCOORD")) {
							uvSrc = findSrc(*src);
							uvOff = o;
							if (uvSrc)
								uvStride = uvSrc->stride;
						}
					}
					int32 perCorner = maxOff + 1;
					if (!posSrc || perCorner <= 0)
						continue;

					NkVector<int64> pidx;
					if (const XmlNode *pn = prim.Child("p"))
						ParseInts(pn->text, pidx);
					if (pidx.Empty())
						continue;

					NkVector<int64> vcount;
					if (isPoly)
						if (const XmlNode *vc = prim.Child("vcount"))
							ParseInts(vc->text, vcount);
					...
					if (isPoly && !vcount.Empty()) {
						...
						for (uint32 pgi = 0; pgi < (uint32)vcount.Size(); ++pgi) {
							...
							if (corner + vc > numCorners)
								break;
							uint32 fan[256];
							int32 fn = 0;
							for (int32 k = 0; k < vc && fn < 256; ++k)
								fan[fn++] = emit(corner + k);
							for (int32 k = 1; k + 1 < fn; ++k) {
								out.indices.PushBack(fan[0]);
								out.indices.PushBack(fan[k]);
								out.indices.PushBack(fan[k + 1]);
							}
							corner += vc;
						}
						...
						for (int32 c = 0; c + 2 < numCorners; c += 3) {
							...
							out.indices.PushBack(a);
							out.indices.PushBack(b);
							out.indices.PushBack(cc);
						}
					}
					...
					if (!anyNormal) {
						...
						for (uint32 vi = baseV; vi < (uint32)out.vertices.Size(); ++vi) {
							...
							float32 l = std::sqrt(n.x * n.x + n.y * n.y + n.z * n.z);
							if (l > 1e-8f) {
								n.x /= l;
								n.y /= l;
								n.z /= l;
		...
		bool LoadDAE(const NkString &path, NkGLTFMeshData &out) {
			...
			if (const XmlNode *libGeo = root.Child("library_geometries")) {
				for (uint32 i = 0; i < (uint32)libGeo->children.Size(); ++i) {
					...
					if (!(g.name == NkString("geometry")))
						continue;
					if (const XmlNode *mesh = g.Child("mesh")) {
						ExtractMesh(*mesh, out);
						++meshCount;
					}
				}
			}
			...
			NkLog::Instance().Infof("[NkDAELoader] OK '%s' : %u meshes, %u verts, %u indices, %u sous-meshes%s",
									path.CStr(), meshCount, (uint32)out.vertices.Size(), (uint32)out.indices.Size(),
									(uint32)out.subMeshes.Size(), zUp ? " (Z_UP->Y)" : "");
			return true;
		}

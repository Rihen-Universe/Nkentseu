...
namespace nkentseu {
	namespace renderer {
		...
		namespace {
			...
			void EmitTri(NkGLTFMeshData &out, const NkVec3f &a, const NkVec3f &b, const NkVec3f &c, NkVec3f n) {
			...
			bool LoadBinary(const NkVector<nk_uint8> &buf, NkGLTFMeshData &out) {
				...
				if ((uint64)84 + (uint64)50 * n != (uint64)buf.Size())
					return false; // pas binaire
				const uint8 *t = d + 84;
			...
			bool LoadAscii(const char *p, const char *end, NkGLTFMeshData &out) {
				...
				while (p < end) {
					...
					if (tok == NkString("facet")) {
						...
						if (vcount == 3)
							EmitTri(out, vtx[0], vtx[1], vtx[2], n);
					} else {
		...
		bool LoadSTL(const NkString &path, NkGLTFMeshData &out) {
			...
			bool ok = false;
			if (isBinary)
				ok = LoadBinary(buf, out);
			else
				ok = LoadAscii((const char *)buf.Data(), (const char *)buf.Data() + buf.Size(), out);
			...
			ComputeBounds(out);
			out.debugName = path;
			NkLog::Instance().Infof("[NkSTLLoader] OK '%s' (%s) : %u triangles, %u verts", path.CStr(),
									isBinary ? "binaire" : "ascii", (uint32)out.indices.Size() / 3,
									(uint32)out.vertices.Size());
			return true;
		}

...
namespace nkentseu {
	namespace renderer {
		...
		NkMeshHandle NkMeshSystem::Create(const NkMeshDesc &desc) {
			...
			NkBufferDesc vbd;
			vbd.sizeBytes = desc.vertexCount * desc.layout.stride;
			vbd.usage = desc.dynamic ? NkResourceUsage::NK_UPLOAD : NkResourceUsage::NK_IMMUTABLE;
			...
			if (desc.indices && desc.indexCount > 0) {
				NkBufferDesc ibd;
				ibd.sizeBytes = desc.indexCount * sizeof(uint32);
				ibd.usage = desc.dynamic ? NkResourceUsage::NK_UPLOAD : NkResourceUsage::NK_IMMUTABLE;
		...
		bool NkMeshSystem::UpdateVerticesRange(NkMeshHandle h, const void *data, uint32 firstVertex, uint32 count) {
			auto *e = mMeshes.Find(h.id);
			if (!e || !e->dynamic)
				return false;
			if (firstVertex + count > e->vertexCount)
				return false;
		...
		bool NkMeshSystem::UpdateIndices(NkMeshHandle h, const uint32 *data, uint32 count) {
			...
			if (!e || !e->dynamic)
				return false;
			return mDevice->WriteBuffer(e->ibo, data, count * sizeof(uint32));
		}
		...
		const NkSubMesh &NkMeshSystem::GetSubMesh(NkMeshHandle h, uint32 idx) const {
			...
			auto *e = mMeshes.Find(h.id);
			if (!e || idx >= (uint32)e->subMeshes.Size())
				return sNull;
		...
		NkBufferHandle NkMeshSystem::GetVBO(NkMeshHandle h) const {
			auto *e = mMeshes.Find(h.id);
			return e ? e->vbo : NkBufferHandle{};
		}

		NkBufferHandle NkMeshSystem::GetIBO(NkMeshHandle h) const {
			auto *e = mMeshes.Find(h.id);
			return e ? e->ibo : NkBufferHandle{};
		}
		...
		void NkMeshSystem::DrawSubMesh(NkICommandBuffer *cmd, NkMeshHandle h, uint32 subIdx, uint32 instances) {
			auto *e = mMeshes.Find(h.id);
			if (!e || subIdx >= (uint32)e->subMeshes.Size())
				return;
		...
		void NkMeshSystem::BuildPrimitives() {
			...
			// ── CUBE ──────────────────────────────────────────────────────────────
			{
				...
				// different par face.
				static const char *kFaceNames[6] = {"+Z front", "-Z back", "-X left",
			...
			// ── SPHERE ────────────────────────────────────────────────────────────
			{
				...
				NkMeshDesc d = NkMeshDesc::Simple(NkVertexLayout::Default3D(), v.Data(), (uint32)v.Size(), i.Data(),
												  (uint32)i.Size());
				d.debugName = "Sphere";
				d.bounds = NkAABB::Unit();
				mSphere = Create(d);
			...
			// ── CYLINDER ─────────────────────────────────────────────────────────
			{
				...
				for (uint32 s = 0; s <= segs; s++) {
					...
					top.normal = {c, 0, sn};
					top.tangent = {-sn, 0, c};
					top.uv = {(float32)s / segs, 0};
					top.color = white;
					bot.pos = {c * 0.5f, -0.5f, sn * 0.5f};
					bot.normal = {c, 0, sn};
					bot.tangent = {-sn, 0, c};
					bot.uv = {(float32)s / segs, 1};
					bot.color = white;
					v.PushBack(top);
					v.PushBack(bot);
				}
				...
				NkMeshDesc d = NkMeshDesc::Simple(NkVertexLayout::Default3D(), v.Data(), (uint32)v.Size(), idx.Data(),
												  (uint32)idx.Size());
		...
		void NkMeshSystem::BuildSphereData(uint32 stacks, uint32 slices, NkVector<NkVertex3D> &v,
										   NkVector<uint32> &idx) {
			uint32 white = PackColor(255, 255, 255);
		...
		void NkMeshSystem::BuildIcosphereData(uint32 subs, NkVector<NkVertex3D> &v, NkVector<uint32> &idx) {
			// Icosahedron base
			float32 phi = (1.f + sqrtf(5.f)) * 0.5f;
			float32 d = 1.f / sqrtf(1.f + phi * phi);
			float32 p = phi * d;
			NkVec3f base[12] = {{-d, p, 0},	 {d, p, 0},	 {-d, -p, 0}, {d, -p, 0}, {0, -d, p},  {0, d, p},
								{0, -d, -p}, {0, d, -p}, {p, 0, -d},  {p, 0, d},  {-p, 0, -d}, {-p, 0, d}};
			uint32 baseFaces[20][3] = {{0, 11, 5}, {0, 5, 1},  {0, 1, 7},	{0, 7, 10}, {0, 10, 11},
		...
		NkMeshDesc NkMeshDesc::Simple(const NkVertexLayout &l, const void *verts, uint32 vc, const uint32 *inds,
									  uint32 ic) {
			NkMeshDesc d;
			d.layout = l;
			d.vertices = verts;
		...
		NkMeshHandle NkMeshSystem::GetSphere(uint32, uint32) {
			BuildPrimitives();
			return mSphere;
		}

		NkMeshHandle NkMeshSystem::GetIcosphere(uint32) {
			BuildPrimitives();
			return mIcosphere;
		}

		NkMeshHandle NkMeshSystem::GetPlane(uint32, uint32) {
			BuildPrimitives();
			return mPlane;
		}

		NkMeshHandle NkMeshSystem::GetQuad() {
			BuildPrimitives();
			return mQuad;
		}

		NkMeshHandle NkMeshSystem::GetCylinder(uint32) {
			BuildPrimitives();
			return mCylinder;
		}

		NkMeshHandle NkMeshSystem::GetCone(uint32) {
			BuildPrimitives();
			return mCone;
		}

	...
	namespace renderer {
		...
		namespace {
			...
			PlyType ParseType(const NkString &s) {
				if (s == NkString("char") || s == NkString("int8"))
					return PT_CHAR;
				if (s == NkString("uchar") || s == NkString("uint8"))
					return PT_UCHAR;
				if (s == NkString("short") || s == NkString("int16"))
					return PT_SHORT;
				if (s == NkString("ushort") || s == NkString("uint16"))
					return PT_USHORT;
				if (s == NkString("int") || s == NkString("int32"))
					return PT_INT;
				if (s == NkString("uint") || s == NkString("uint32"))
					return PT_UINT;
				if (s == NkString("float") || s == NkString("float32"))
					return PT_FLOAT;
				if (s == NkString("double") || s == NkString("float64"))
					return PT_DOUBLE;
				return PT_NONE;
			}
			...
			bool IsFloatType(PlyType t) {
				return t == PT_FLOAT || t == PT_DOUBLE;
			}
			...
			float64 ReadBinF64(const uint8 *&d, const uint8 *end, PlyType t) {
				...
				switch (t) {
					...
					case PT_UCHAR:
						...
						break;
					case PT_SHORT:
						r = (float64)(int16)ReadU16LE(d);
						break;
					case PT_USHORT:
						r = (float64)ReadU16LE(d);
						break;
					case PT_INT:
						r = (float64)(int32)ReadU32LE(d);
						break;
					case PT_UINT:
						r = (float64)ReadU32LE(d);
						break;
					case PT_FLOAT:
						r = (float64)ReadF32LE(d);
						break;
					case PT_DOUBLE:
						r = ReadF64LE(d);
						break;
			...
			struct VtxMap {
					int32 x = -1, y = -1, z = -1, nx = -1, ny = -1, nz = -1, r = -1, g = -1, b = -1, a = -1, s = -1,
						  t = -1;
			};
			...
			void BuildVtxMap(const PlyElem &e, VtxMap &vm) {
				for (int32 i = 0; i < (int32)e.props.Size(); ++i) {
					...
					if (n == NkString("x"))
						vm.x = i;
					else if (n == NkString("y"))
						vm.y = i;
					else if (n == NkString("z"))
						vm.z = i;
					else if (n == NkString("nx"))
						vm.nx = i;
					else if (n == NkString("ny"))
						vm.ny = i;
					else if (n == NkString("nz"))
						vm.nz = i;
					else if (n == NkString("red"))
						vm.r = i;
					else if (n == NkString("green"))
						vm.g = i;
					else if (n == NkString("blue"))
						vm.b = i;
					else if (n == NkString("alpha"))
						vm.a = i;
					else if (n == NkString("s") || n == NkString("u") || n == NkString("texture_u"))
						vm.s = i;
					else if (n == NkString("t") || n == NkString("v") || n == NkString("texture_v"))
						vm.t = i;
				}
			}

			void StoreVertex(NkGLTFMeshData &out, const VtxMap &vm, const NkVector<PlyProp> &props,
							 const float64 *vals) {
				NkVertex3D v{};
				v.pos = {vm.x >= 0 ? (float32)vals[vm.x] : 0.f, vm.y >= 0 ? (float32)vals[vm.y] : 0.f,
						 vm.z >= 0 ? (float32)vals[vm.z] : 0.f};
				v.normal = {vm.nx >= 0 ? (float32)vals[vm.nx] : 0.f, vm.ny >= 0 ? (float32)vals[vm.ny] : 0.f,
							vm.nz >= 0 ? (float32)vals[vm.nz] : 0.f};
				v.tangent = {0.f, 0.f, 0.f};
				v.uv = {vm.s >= 0 ? (float32)vals[vm.s] : 0.f, vm.t >= 0 ? (float32)vals[vm.t] : 0.f};
				v.uv2 = v.uv;
				auto chan = [&](int32 idx) -> uint8 {
					if (idx < 0)
						return 255;
					float64 x = vals[idx];
					if (IsFloatType(props[(NkVector<PlyProp>::SizeType)idx].type))
						x *= 255.0;
					if (x < 0)
						x = 0;
					if (x > 255)
		...
		bool LoadPLY(const NkString &path, NkGLTFMeshData &out) {
			...
			while (hp < end) {
				...
				if (tok == NkString("format")) {
					ReadToken(hp, end, tok);
					if (tok == NkString("ascii"))
						ascii = true;
					else {
						ascii = false;
						bigEndian = (tok == NkString("binary_big_endian"));
					}
				} else if (tok == NkString("element")) {
					PlyElem e;
					ReadToken(hp, end, e.name);
					...
					if (tok == NkString("list")) {
						...
						ReadToken(hp, end, ct);
						ReadToken(hp, end, et);
						pr.countType = ParseType(ct);
						pr.type = ParseType(et);
					} else {
			...
			for (uint32 ei = 0; ei < (uint32)elems.Size(); ++ei) {
				...
				for (uint32 r = 0; r < e.count; ++r) {
					if (isVertex) {
						float64 vals[64];
						int32 np = (int32)e.props.Size();
						if (np > 64)
							np = 64;
						for (int32 pi = 0; pi < np; ++pi) {
							const PlyProp &pr = e.props[(NkVector<PlyProp>::SizeType)pi];
							if (pr.isList) { // rare sur vertex : on saute
								int64 cnt = ascii ? ParseInt64WS(ap, end)
												  : (int64)ReadBinF64(bp, (const uint8 *)end, pr.countType);
								for (int64 k = 0; k < cnt; ++k) {
									if (ascii)
										ParseDoubleWS(ap, end);
									else
										ReadBinF64(bp, (const uint8 *)end, pr.type);
								}
								vals[pi] = 0.0;
							} else {
						...
						for (int32 pi = 0; pi < (int32)e.props.Size(); ++pi) {
							...
							if (pr.isList) {
								...
								int32 nn = 0;
								for (int64 k = 0; k < cnt; ++k) {
									int32 vi = ascii ? (int32)ParseInt64WS(ap, end)
													 : (int32)ReadBinF64(bp, (const uint8 *)end, pr.type);
									if (nn < 64)
										idx[nn++] = vi;
								}
			...
			ComputeBounds(out);
			out.debugName = path;
			NkLog::Instance().Infof("[NkPLYLoader] OK '%s' (%s) : %u verts, %u indices", path.CStr(),
									ascii ? "ascii" : "binaire", (uint32)out.vertices.Size(),
									(uint32)out.indices.Size());
			return true;
		}

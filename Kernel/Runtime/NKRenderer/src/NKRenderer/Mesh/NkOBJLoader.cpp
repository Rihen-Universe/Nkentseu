		...
		namespace {
			...
			NkString DirOf(const NkString &path) {
				...
				for (int32 i = 0; i < cut; ++i)
					d.Append(path[(NkString::SizeType)i]);
				return d;
			}
			...
			int32 ResolveIndex(int32 raw, int32 count) {
				if (raw > 0)
					return raw - 1;
				if (raw < 0)
					return count + raw; // -1 = dernier
				return -1;				// 0 = absent
			}

			// ── Parse le .mtl : remplit out.materials + out.images, et la table
			//    name->index. Charge les textures relativement a `dir`. ──────────
			void ParseMTL(const NkString &mtlPath, const NkString &dir, NkGLTFMeshData &out,
						  NkVector<NkString> &matNames) {
				...
				while (p < end) {
					...
					if (LineIs(p, end, "newmtl")) {
						...
						if (cur->roughnessFactor > 1.f)
							cur->roughnessFactor = 1.f;
					} else if (cur && (LineIs(p, end, "d"))) {
						p += 1;
						cur->baseColorFactor.w = ParseFloat(p, end);
					} else if (cur && LineIs(p, end, "Tr")) {
						p += 2;
						cur->baseColorFactor.w = 1.f - ParseFloat(p, end);
						...
						cur->emissiveImage = loadTex(nm);
							   ...
							   (LineIs(p, end, "map_Bump") || LineIs(p, end, "bump") || LineIs(p, end, "norm"))) {
		...
		bool LoadOBJ(const NkString &path, NkGLTFMeshData &out) {
			...
			while (p < end) {
				...
				if (c0 == 'v' && (p + 1 < end) && (p[1] == ' ' || p[1] == '\t')) {
					p += 1;
					NkVec3f v;
					v.x = ParseFloat(p, end);
					v.y = ParseFloat(p, end);
					v.z = ParseFloat(p, end);
					positions.PushBack(v);
				} else if (c0 == 'v' && (p + 1 < end) && p[1] == 't') {
					p += 2;
					NkVec2f t;
					t.x = ParseFloat(p, end);
					t.y = ParseFloat(p, end);
					texcoords.PushBack(t);
				} else if (c0 == 'v' && (p + 1 < end) && p[1] == 'n') {
					p += 2;
					NkVec3f n;
					n.x = ParseFloat(p, end);
					n.y = ParseFloat(p, end);
					n.z = ParseFloat(p, end);
					normals.PushBack(n);
					...
					while (faceN < 64) {
						...
						if (ni >= 0)
							anyNormalRef = true;
						if (vi < 0 || vi >= (int32)positions.Size())
							continue;
						...
						if (uint32 *found = vcache.Find(key)) {
							...
							vert.normal = (ni >= 0 && ni < (int32)normals.Size())
											  ? normals[(NkVector<NkVec3f>::SizeType)ni]
											  : NkVec3f{0.f, 0.f, 0.f};
			...
			NkLog::Instance().Infof(
				"[NkOBJLoader] OK '%s' : %u verts, %u indices, %u sous-meshes, %u materiaux, %u images", path.CStr(),
				(uint32)out.vertices.Size(), (uint32)out.indices.Size(), (uint32)out.subMeshes.Size(),

...
namespace nkentseu {
	namespace renderer {
		...
		namespace {
			...
			bool FindArray(const char *bs, const char *be, const char *name, const char *&s, const char *&e) {
				usize nl = 0;
				while (name[nl])
					++nl;
			...
			void ExtractMeshBlock(const char *bs, const char *be, NkGLTFMeshData &out) {
				...
				if (FindArray(bs, be, "points", s, e))
					NumsF(s, e, points);
				if (FindArray(bs, be, "faceVertexIndices", s, e))
					NumsI(s, e, fvi);
				if (FindArray(bs, be, "faceVertexCounts", s, e))
					NumsI(s, e, fvc);
				if (FindArray(bs, be, "normals", s, e))
					NumsF(s, e, normals);
				if (FindArray(bs, be, "primvars:st", s, e))
					NumsF(s, e, st);

				if (points.Empty() || fvi.Empty())
					return;
		...
		bool LoadUSDA(const NkString &path, NkGLTFMeshData &out) {
			...
			if (!(text.Size() >= 5 && p[0] == '#' && p[1] == 'u' && p[2] == 's' && p[3] == 'd' && p[4] == 'a')) {
				NkLog::Instance().Warnf("[NkUSDALoader] entete '#usda' absent (USDC binaire non supporte) : %s",
										path.CStr());
				return false;
			}
			...
			NkLog::Instance().Infof("[NkUSDALoader] OK '%s' : %u meshes, %u verts, %u indices, %u sous-meshes%s",
									...
									(uint32)out.subMeshes.Size(), zUp ? " (Z->Y)" : "");
			return true;
		}

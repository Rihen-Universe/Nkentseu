	...
	namespace renderer {
		namespace meshutil {
			...
			inline void ReadToken(const char *&p, const char *end, NkString &out) {
				...
				out.Clear();
				if (p > s)
					out.Append(s, (NkString::SizeType)(p - s));
			}

			// ── Lecture binaire little-endian ────────────────────────────────
			inline uint16 ReadU16LE(const uint8 *d) {
				return (uint16)(d[0] | (d[1] << 8));
			}
			...
			inline void GenerateSmoothNormals(NkGLTFMeshData &m) {
				...
				for (uint32 i = 0; i + 2 < (uint32)m.indices.Size(); i += 3) {
					...
					m.vertices[i0].normal = {m.vertices[i0].normal.x + n.x, m.vertices[i0].normal.y + n.y,
											 m.vertices[i0].normal.z + n.z};
					m.vertices[i1].normal = {m.vertices[i1].normal.x + n.x, m.vertices[i1].normal.y + n.y,
											 m.vertices[i1].normal.z + n.z};
					m.vertices[i2].normal = {m.vertices[i2].normal.x + n.x, m.vertices[i2].normal.y + n.y,
											 m.vertices[i2].normal.z + n.z};
				}
				for (uint32 i = 0; i < (uint32)m.vertices.Size(); ++i) {

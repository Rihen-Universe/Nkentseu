...
namespace nkentseu {
	namespace renderer {
		...
		namespace {
			...
			NkTexHandle UploadImage(NkTextureLibrary *texLib, const NkGLTFImage &img, bool srgb, const char *dbg) {
				...
				// requise : la moyenne d'un atlas non-noir est franchement >0.
				{
					static int diag = -1;
					if (diag == -1) {
						const char *v = getenv("NK_GLTF_TEXDIAG");
						diag = (v && v[0] && v[0] != '0') ? 1 : 0;
					}
					if (diag) {
						...
						if (px && W && H) {
							uint64 sr = 0, sg = 0, sb = 0, sa = 0;
							const uint64 n = (uint64)W * H;
							...
							auto P = [&](uint32 x, uint32 y, const char *tag) {
								...
								logger.Info("[GLTF_TEXDIAG]   px[{0}]=({1},{2},{3},{4})\n", tag, (uint32)px[o],
											(uint32)px[o + 1], (uint32)px[o + 2], (uint32)px[o + 3]);
							};
							logger.Info("[GLTF_TEXDIAG] '{0}' {1}x{2} srgb={3} moyenne RGBA=({4},{5},{6},{7})\n",
										dbg ? dbg : "?", W, H, srgb ? 1 : 0, (uint32)(sr / n), (uint32)(sg / n),
										(uint32)(sb / n), (uint32)(sa / n));
							P(0, 0, "haut-gauche");
							P(W / 2, H / 2, "centre");
							P(W - 1, H - 1, "bas-droite");
						}
					}
				}
		...
		bool BuildGLTFMaterials(const NkGLTFMeshData &data, NkMaterialSystem *matSys, NkTextureLibrary *texLib,
			...
			auto MarkSrgb = [&](int32 idx, bool srgb) {
				if (idx < 0 || (uint32)idx >= imgCount)
					return;
				// sRGB l'emporte si au moins un usage couleur (baseColor/emissive).
				if (srgb)
					imgSrgb[(uint32)idx] = 1;
				else if (imgSrgb[(uint32)idx] == -1)
					imgSrgb[(uint32)idx] = 0;
			};
			...
			for (uint32 i = 0; i < imgCount; ++i) {
				...
				out.textures[i] = UploadImage(texLib, data.images[i], srgb, "gltf_image");
			}
			...
			for (uint32 m = 0; m < (uint32)data.materials.Size(); ++m) {
				...
				// Facteurs PBR.
				NkVec3f albedo = {gm.baseColorFactor.x, gm.baseColorFactor.y, gm.baseColorFactor.z};
				...
				if (gm.emissiveFactor.x > 0.f || gm.emissiveFactor.y > 0.f || gm.emissiveFactor.z > 0.f) {
				...
				if (baseColor.IsValid())
					inst->SetAlbedoMap(baseColor);

				NkTexHandle normal = TexFor(gm.normalImage);
				if (normal.IsValid())
					inst->SetNormalMap(normal, gm.normalScale);
				...
				if (emissiveMap.IsValid())
					inst->SetEmissiveMap(emissiveMap);

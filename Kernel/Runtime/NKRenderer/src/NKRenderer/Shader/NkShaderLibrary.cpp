...
namespace nkentseu {
	namespace renderer {
		...
		NkShaderLibrary::~NkShaderLibrary() {
			Shutdown();
		}
		...
		static void TestCrossApiConversion() {
			...
			auto reportAndSave = [&](const char *target, const char *outName, const NkShaderConvertResult &r) {
				if (r.success) {
					logger.Info("[CrossAPITest] {0}: OK ({1} chars) -> {2}\n", NkString(target),
								(uint32)r.source.Size(), NkString(outName));
...
// system() est indisponible sur iOS/tvOS/watchOS -> no-op mobile.
			...
			reportAndSave("VK->GL", (name + ".gl.glsl").CStr(), NkShaderConverter::GlslToGlsl(src, st, name));
			...
			reportAndSave("VK->MSL", (name + ".msl").CStr(), NkShaderConverter::GlslToMsl(src, st, name));
		...
		void NkShaderLibrary::Shutdown() {
			ReleaseAll();
			memory::NkGetDefaultAllocator().Delete(mBackend);
			mBackend = nullptr;
			memory::NkGetDefaultAllocator().Delete(mNkSLBackend);
			mNkSLBackend = nullptr;
		}
		...
		static bool IsTextTarget(NkGraphicsApi api) {
			return api == NkGraphicsApi::NK_GFX_API_OPENGL || api == NkGraphicsApi::NK_GFX_API_OPENGLES ||
				   api == NkGraphicsApi::NK_GFX_API_WEBGL || api == NkGraphicsApi::NK_GFX_API_DX11 ||
				   api == NkGraphicsApi::NK_GFX_API_DX12 || api == NkGraphicsApi::NK_GFX_API_METAL;
		}

		static void SaveToShaderCache(uint64 key, NkGraphicsApi api, const NkShaderCompileResult &res,
		...
		static bool LoadFromShaderCache(uint64 key, NkGraphicsApi api, NkVector<uint8> &outBytecode,
										NkString &outGlsl) {
		...
		bool NkShaderLibrary::Recompile(NkShaderProgram &prog) {
			...
			const NkString fmtKey =
				NkString(ApiCacheTag(mApi)) + "." +
				kShaderGenVersion; // clé par backend (anti-pollution) + version codegen (auto-invalidation)
			...
			if (!vertSrc.Empty()) {
				...
				if (!LoadFromShaderCache(key, mApi, prog.vertBytecode, vertGlsl)) {
					...
					if (!res.success) {
						...
						if (!res.preprocessed.Empty())
							vertGlsl = res.preprocessed;
		...
		// ── Chargement ───────────────────────────────────────────────────────────
		NkShaderHandle NkShaderLibrary::LoadVF(const NkString &vPath, const NkString &fPath, const NkString &name) {
		...
		NkShaderHandle NkShaderLibrary::LoadVGF(const NkString &v, const NkString &g, const NkString &f,
												const NkString &name) {
			NkShaderProgram prog;
		...
		NkShaderHandle NkShaderLibrary::LoadCompute(const NkString &cPath, const NkString &name) {
			NkShaderProgram prog;
		...
		NkShaderHandle NkShaderLibrary::CompileVF(const NkString &vSrc, const NkString &fSrc, const NkString &name,
			...
			bool vsOk = true;
			{
				...
				if (!LoadFromShaderCache(key, mApi, prog.vertBytecode, vsGlslStr)) {
					...
					if (vr.success) {
						...
						if (!vr.preprocessed.Empty())
							vsGlslStr = vr.preprocessed;
			...
			bool fsOk = true;
			{
				...
				if (!LoadFromShaderCache(key, mApi, prog.fragBytecode, fsGlslStr)) {
					...
					if (fr.success) {
						...
						if (!fr.preprocessed.Empty())
							fsGlslStr = fr.preprocessed;
			...
			logger.Info("[CompileVF] '{0}' vsGlsl={1} fsGlsl={2} hlsl(vs={3},fs={4})\n", prog.name,
						(uint32)(vsGlsl ? strlen(vsGlsl) : 0), (uint32)(fsGlsl ? strlen(fsGlsl) : 0),
			...
			if (isDX) {
				...
				if (dn && prog.name == dn) {
					...
					if (!vsHlslStr.Empty()) {
						...
						if (f) {
							fwrite(vsHlslStr.CStr(), 1, vsHlslStr.Size(), f);
							fclose(f);
						}
					}
					if (!fsHlslStr.Empty()) {
						...
						if (f) {
							fwrite(fsHlslStr.CStr(), 1, fsHlslStr.Size(), f);
							fclose(f);
						}
					}
				}
			...
			// DIAG temporaire : dump du GLSL genere (opt-in NK_DUMP_GLSL=<name>).
			{
				...
				if (dn && prog.name == dn && vsGlsl && vsGlsl[0]) {
					...
					if (f) {
						fwrite(vsGlsl, 1, strlen(vsGlsl), f);
						fclose(f);
					}
					if (fsGlsl && fsGlsl[0]) {
						FILE *ff = fopen("Build/hlsl_dump/dump_fs.glsl", "wb");
						if (ff) {
							fwrite(fsGlsl, 1, strlen(fsGlsl), ff);
							fclose(ff);
						}
					}
				}
			...
			if (mNkSLBackend) {
				...
				if (NkFile::Exists(nkVS.CStr()) && NkFile::Exists(nkFS.CStr())) {
					...
					if (h.IsValid()) {
						...
						if (prog) {
							prog->vertPath = nkVS;
							prog->vertMtime = GetFileMtime(nkVS);
							prog->fragPath = nkFS;
							prog->fragMtime = GetFileMtime(nkFS);
						}
						return h;
					}
		...
		void NkShaderLibrary::Release(NkShaderHandle &h) {
			...
			if (!p)
				return;
			if (p->valid && p->rhiHandle.IsValid())
				mDevice->DestroyShader(p->rhiHandle);
			if (!p->name.Empty())
				mByName.Remove(p->name);

...
namespace nkentseu {
	namespace renderer {
		...
		static NkString ParseIncludeArg(const NkString &line) {
			...
			uint32 start = i;
			while (i < n && s[i] != closeQ)
				++i;
			if (i >= n)
				return ""; // pas de quote fermant
			NkString arg;
			arg.Resize(i - start);
			for (uint32 k = 0; k < i - start; ++k)
				arg[k] = s[start + k];
			return arg;
		}
		...
		// ── Public API ────────────────────────────────────────────────────────
		NkString NkShaderIncludeResolver::Resolve(const NkString &source, const NkString &currentFilePath) {
		...
		NkString NkShaderIncludeResolver::ResolveRecursive(const NkString &source, const NkString &currentFilePath,
			...
			for (uint32 i = 0; i <= n; ++i) {
				...
				if (!arg.Empty()) {
					...
					if (resolved.Empty()) {
						...
						NkString resolvedSub = ResolveRecursive(rawSubFile, resolved, visited, depth + 1);
						out += "// [NkShaderInclude] begin: ";
						out += resolved;
						out += "\n";
						out += resolvedSub;
						if (!resolvedSub.Empty() && resolvedSub[resolvedSub.Size() - 1] != '\n') {
							out += "\n";
						}

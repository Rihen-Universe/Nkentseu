	...
	namespace renderer {
		...
		class NkSceneNode : public NkITransformable {
			public:
				...
				// ── NkITransformable ───────────────────────────────────────────
				const NkTransform &GetLocalTransform() const noexcept override {
					return mLocal;
				}
				...
				NkVec3f GetLocalPosition() const noexcept override {
					return mLocal.translation;
				}

				NkQuatf GetLocalRotation() const noexcept override {
					return mLocal.rotation;
				}

				NkVec3f GetLocalScale() const noexcept override {
					return mLocal.scale;
				}
				...
				NkITransformable *GetParent() const noexcept override {
					return mParent;
				}
				...
				const NkVector<NkSceneNode *> &GetChildren() const noexcept {
					return mChildren;
				}

...
namespace nkentseu {
	namespace renderer {
		...
		bool NkRenderTarget::Init(NkIDevice *dev, NkTextureLibrary *texLib, const NkRenderTargetDesc &desc) {
			NkOffscreenDesc od;
			od.width = desc.width;
		...
		NkRenderPassHandle NkRenderTarget::BeginRender(NkICommandBuffer *cmd, NkVec4f clearColor, bool clearDepth) {
		...
		NkMat4f NkRenderTarget::ObliqueProjection(NkMat4f proj, const NkMat4f &view, NkVec4f worldPlane) {
			...
			if (fabsf(c_dot_q) < 1e-7f)
				return proj; // plan parallèle au near — pas d'oblique

...
namespace nkentseu {
	namespace renderer {
		...
		class NkRenderTarget {
			public:
				bool Init(NkIDevice *dev, NkTextureLibrary *texLib, const NkRenderTargetDesc &desc);
				void Shutdown();

				bool IsValid() const {
					return mOffscreen.IsValid();
				}
				...
				NkRenderPassHandle BeginRender(NkICommandBuffer *cmd, NkVec4f clearColor = {0.f, 0.f, 0.f, 1.f},
				...
				NkTexHandle GetColorHandle() const {
					return mOffscreen.GetColorResult();
				}

				NkTexHandle GetDepthHandle() const {
					return mOffscreen.GetDepthResult();
				}

				NkRenderPassHandle GetRenderPass() const {
					return mOffscreen.GetRP();
				}

				uint32 GetWidth() const {
					return mOffscreen.GetWidth();
				}

				uint32 GetHeight() const {
					return mOffscreen.GetHeight();
				}
				...
				static NkMat4f ObliqueProjection(NkMat4f proj, const NkMat4f &view, NkVec4f worldPlane);

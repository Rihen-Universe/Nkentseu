...
namespace nkentseu {
	namespace renderer {
		...
		struct NkPassBuilder {
				...
				// ── Inputs ────────────────────────────────────────────────────────
				NkPassBuilder &Reads(NkGraphResId r) {
					reads.PushBack(r);
					return *this;
				}
				...
				// ── Color attachments ─────────────────────────────────────────────
				NkPassBuilder &SetColor(uint32 slot, NkGraphResId res, NkLoadOp load = NkLoadOp::NK_LOAD,
				...
				// ── Depth attachment ──────────────────────────────────────────────
				NkPassBuilder &SetDepth(NkGraphResId res, NkLoadOp load = NkLoadOp::NK_CLEAR, float32 clearD = 1.f) {
					hasDepth = true;
					depth.resId = res;
					depth.loadOp = load;
				...
				NkPassBuilder &Writes(NkGraphResId r) {
					if (type == NkPassType::NK_COMPUTE)
						return WritesStorage(r);
				...
				// ── Callback ──────────────────────────────────────────────────────
				NkPassBuilder &Execute(NkPassCallback cb) {
					execute = cb;
					return *this;
				}

				// ── Flags ─────────────────────────────────────────────────────────
				NkPassBuilder &SetEnabled(bool v) {
					enabled = v;
					return *this;
				}

				NkPassBuilder &SetAsync(bool v) {
					asyncCompute = v;
					return *this;
				}

				NkPassBuilder &SetAlwaysExecute(bool v) {
					alwaysExecute = v;
					return *this;
				}
		...
		class NkRenderGraph {
			public:
				...
				NkGraphResId ImportTexture(const NkString &name, NkTextureHandle tex, NkResourceState initialState,
										   const NkTextureDesc &desc = {});
				NkGraphResId ImportBuffer(const NkString &name, NkBufferHandle buf, NkResourceState initialState);
				...
				uint32 GetPassCount() const noexcept {
					return (uint32)mPasses.Size();
				}

				uint32 GetActivePassCount() const noexcept {
					return (uint32)mSorted.Size();
				}

				uint32 GetResourceCount() const noexcept {
			...
			private:
				...
				// ── Helpers internes ────────────────────────────────────────────
				NkGraphResId NextResId() noexcept {
					return mNextResId++;
				}

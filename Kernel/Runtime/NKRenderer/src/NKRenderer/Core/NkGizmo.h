...
namespace nkentseu {
    namespace renderer {
        ...
        struct NkGizmoTarget { NkMat4f base; NkVec3f localHalf; float32 pickRadius; };
        ...
        class NkGizmo3D {
            public:
                ...
                NkGizmo3D() { for (int32 i=0;i<kMax;i++) mRot[i] = NkMat4f::Identity(); }

                // ── Config ────────────────────────────────────────────────────────
                void SetCamera(NkVec3f camPos, NkVec3f camTarget, float32 fovYDeg,
                ...
                void CycleMode()        { mMode   = (mMode+1) % 4; }
                void CycleOrientation() { mOrient = (mOrient+1) % 3; }
                // Choix EXPLICITE du mode/orientation par valeur (enum/entier) — pas de cycle.
                void SetMode(int32 m)        { mMode = m & 3; }
                void SetOrientation(int32 o) { mOrient = ((o%3)+3)%3; }
                ...
                void SetModeByName(const char* name) {
                    char c = Lower0(name); if (!c) return;
                    if      (c=='t') mMode=MODE_TRANSLATE;
                    else if (c=='r') mMode=MODE_ROTATE;
                    else if (c=='s') mMode=MODE_SCALE;
                    else if (c=='c') mMode=MODE_COMBINE;
                }
                void SetOrientationByName(const char* name) {
                    char c = Lower0(name); if (!c) return;
                    if      (c=='g') mOrient=ORIENT_GLOBAL;
                    else if (c=='l') mOrient=ORIENT_LOCAL;
                    else if (c=='n') mOrient=ORIENT_NORMAL;
                }
                ...
                int32 ActiveIndex()  const { return mSelId; }
                ...
                void Update(const NkGizmoTarget* targets, int32 count, const NkGizmoInput& in) {
                    ...
                    for (int32 i=0;i<count;i++) if (mSel[i]) { selCount++; pivot=pivot+Ctr(i); }
                    ...
                    if (mOrient!=ORIENT_GLOBAL && mSelId>=0 && mSelId<count) {
                        ...
                        mGB[0]=Norm((M*NkVec3f{1,0,0})-o); mGB[1]=Norm((M*NkVec3f{0,1,0})-o); mGB[2]=Norm((M*NkVec3f{0,0,1})-o);
                    }

                    // Interaction.
                    if (in.leftPressed) DoPick(in);
                    if (mDragging) {
                        if (!in.leftDown || !mHaveSel) mDragging=false;
                        else DoDrag(in);
                    }
                }
                ...
                void Draw(DrawLine drawLine) const {
                    if (!mHaveSel) return;
                    GH hs[32]; int32 nh=BuildHandles(hs);
                    for (int32 hi=0; hi<nh; hi++) {
                        const NkVec4f col=HandleColor(hs[hi]);
                        const bool active = mDragging && mGOp==hs[hi].op && mGMask==hs[hi].mask && mGKind==hs[hi].kind;
                        const int32 tPx = active?7:4;
                        ForEachSeg(hs[hi], mPivot, mGL, [&](NkVec3f A, NkVec3f B){ ThickLine(A,B,col,tPx,drawLine); });
                    }
                    // Marqueurs OBB (par objet sélectionné).
                    for (int32 i=0;i<mCount;i++) if (mSel[i]) {
                        const NkMat4f& M=mComposed[i]; NkVec3f hh=mHalf[i];
                        const NkVec4f Y = (i==mSelId)? NkVec4f{1.f,0.6f,0.05f,1.f} : NkVec4f{1.f,0.85f,0.1f,1.f};
                        NkVec3f c000=Corner(M,hh,-1,-1,-1), c100=Corner(M,hh,+1,-1,-1), c010=Corner(M,hh,-1,+1,-1), c110=Corner(M,hh,+1,+1,-1);
                        NkVec3f c001=Corner(M,hh,-1,-1,+1), c101=Corner(M,hh,+1,-1,+1), c011=Corner(M,hh,-1,+1,+1), c111=Corner(M,hh,+1,+1,+1);
                        ThickLine(c000,c100,Y,2,drawLine); ThickLine(c010,c110,Y,2,drawLine); ThickLine(c001,c101,Y,2,drawLine); ThickLine(c011,c111,Y,2,drawLine);
                        ThickLine(c000,c010,Y,2,drawLine); ThickLine(c100,c110,Y,2,drawLine); ThickLine(c001,c011,Y,2,drawLine); ThickLine(c101,c111,Y,2,drawLine);
                        ThickLine(c000,c001,Y,2,drawLine); ThickLine(c100,c101,Y,2,drawLine); ThickLine(c010,c011,Y,2,drawLine); ThickLine(c110,c111,Y,2,drawLine);
                    }
                }
            ...
            private:
                ...
                // ── Maths de base ─────────────────────────────────────────────────
                static float32 Dot(NkVec3f a, NkVec3f b){ return a.x*b.x+a.y*b.y+a.z*b.z; }
                static NkVec3f Cross(NkVec3f a, NkVec3f b){ return {a.y*b.z-a.z*b.y, a.z*b.x-a.x*b.z, a.x*b.y-a.y*b.x}; }
                static float32 Len(NkVec3f v){ return sqrtf(Dot(v,v)); }
                static NkVec3f Norm(NkVec3f v){ float32 l=Len(v); return (l>1e-6f)? NkVec3f{v.x/l,v.y/l,v.z/l} : v; }
                static float32 NkGMax(float32 a, float32 b){ return a>b?a:b; }
                static char Lower0(const char* s){ if(!s||!s[0]) return 0; char c=s[0]; return (c>='A'&&c<='Z')?(char)(c+32):c; }
                static void AddComp(NkVec3f& v, int32 a, float32 x){ if(a==0)v.x+=x; else if(a==1)v.y+=x; else v.z+=x; }
                static int32  AxisOf(int32 m){ return (m&1)?0:(m&2)?1:2; }
                static void   PlaneAx(int32 m, int32& i, int32& j){ int32 f=-1; i=0;j=0; for(int32 k=0;k<3;k++) if(m&(1<<k)){ if(f<0){i=k;f=k;} else j=k; } }
                static float32 CenterR(int32 op, float32 L){ return (op==2)?0.16f*L:0.13f*L; }
                static NkVec3f Corner(const NkMat4f& M, NkVec3f hh, float32 sx, float32 sy, float32 sz){ return M*NkVec3f{sx*hh.x, sy*hh.y, sz*hh.z}; }
                NkVec3f Ctr(int32 i) const { return mComposed[i]*NkVec3f{0.f,0.f,0.f}; }

                bool Project(NkVec3f P, float32& px, float32& py) const {
                    NkVec3f v={P.x-mCamPos.x,P.y-mCamPos.y,P.z-mCamPos.z};
                    float32 zc=Dot(v,mFwd); if (zc<=1e-3f) return false;
                    float32 nx=Dot(v,mRgt)/(zc*mThX), ny=Dot(v,mUp)/(zc*mThY);
                    px=(nx*0.5f+0.5f)*mVpW; py=(0.5f-ny*0.5f)*mVpH; return true;
                }
                static float32 SegDist(float32 px,float32 py, float32 ax,float32 ay, float32 bx,float32 by){
                    ...
                    float32 t=(l2>1e-6f)?((px-ax)*dx+(py-ay)*dy)/l2:0.f; t=t<0.f?0.f:(t>1.f?1.f:t);
                    float32 cx=ax+t*dx, cy=ay+t*dy; return sqrtf((px-cx)*(px-cx)+(py-cy)*(py-cy));
                }

                int32 BuildHandles(GH hs[32]) const {
                    int32 nh=0;
                    const bool cT=(mMode==0||mMode==3), cR=(mMode==1||mMode==3), cS=(mMode==2||mMode==3);
                    if (cT){ hs[nh++]={0,1,0};hs[nh++]={0,2,0};hs[nh++]={0,4,0}; hs[nh++]={0,3,1};hs[nh++]={0,6,1};hs[nh++]={0,5,1}; hs[nh++]={0,7,2}; }
                    if (cR){ hs[nh++]={1,1,3};hs[nh++]={1,2,3};hs[nh++]={1,4,3}; }
                    if (cS){ hs[nh++]={2,1,0};hs[nh++]={2,2,0};hs[nh++]={2,4,0}; hs[nh++]={2,3,1};hs[nh++]={2,6,1};hs[nh++]={2,5,1}; hs[nh++]={2,7,2}; }
                    return nh;
                }

                // Segments monde d'une poignée (hit-test ET dessin partagent la géométrie).
                template<class CB>
                void ForEachSeg(const GH& h, NkVec3f C, float32 L, CB cb) const {
                    const bool combine=(mMode==3);
                    const float32 Lt=L, Ls=combine?0.55f*L:L, Lr=combine?0.90f*L:L;
                    if (h.kind==0) {
                        int32 a=AxisOf(h.mask); NkVec3f dir=mGB[a], u=mGB[(a+1)%3], w=mGB[(a+2)%3];
                        float32 len=(h.op==0)?Lt:Ls; NkVec3f E=C+dir*len; cb(C,E);
                        if (h.op==0) { float32 hl=0.20f*L, hw=0.07f*L; NkVec3f b=E-dir*hl;
                        ...
                        } else { float32 hb=0.06f*L; NkVec3f cc[8];
                        ...
                        int32 i,j; PlaneAx(h.mask,i,j); NkVec3f di=mGB[i], dj=mGB[j];
                        ...
                        float32 rU=CenterR(h.op,L); NkVec3f prev{};
                        for(int32 k=0;k<=28;k++){ float32 t=(float32)k/28.f*6.2831853f; NkVec3f P=C+(mRgt*cosf(t)+mUp*sinf(t))*rU; if(k>0) cb(prev,P); prev=P; }
                    } else {
                        int32 a=AxisOf(h.mask); NkVec3f u=mGB[(a+1)%3], w=mGB[(a+2)%3]; NkVec3f prev{};
                ...
                NkVec4f HandleColor(const GH& h) const {
                    const NkVec4f ACOL[3]={{1.00f,0.15f,0.20f,1.f},{0.35f,0.95f,0.10f,1.f},{0.15f,0.45f,1.00f,1.f}};
                    if (mDragging && mGOp==h.op && mGMask==h.mask && mGKind==h.kind) return {1.f,0.92f,0.15f,1.f};
                    if (h.kind==2) return {0.95f,0.95f,0.95f,1.f};
                    if (h.kind==1) { int32 nrm=(h.mask==3)?2:(h.mask==6)?0:1; return ACOL[nrm]; }
                ...
                void ThickLine(NkVec3f A, NkVec3f B, NkVec4f col, int32 tPx, DrawLine& dl) const {
                    ...
                    float32 wpp=(2.f*mThY*dist)/mVpH; NkVec3f perp=mRgt;
                    if (oa&&ob){ float32 sdx=bx-ax, sdy=by-ay, sl=sqrtf(sdx*sdx+sdy*sdy);
                        if (sl>1e-3f){ float32 pxn=-sdy/sl, pyn=sdx/sl; perp=mRgt*pxn - mUp*pyn; } }
                    if (tPx<1) tPx=1;
                    for (int32 i=0;i<tPx;i++){ float32 off=((float32)i-(tPx-1)*0.5f)*wpp; NkVec3f o=perp*off; dl(A+o,B+o,col); }
                }
                ...
                void DoPick(const NkGizmoInput& in) {
                    ...
                    if (mHaveSel) {
                        ...
                        float32 best=13.f; GH hs[32]; int32 nh=BuildHandles(hs);
                        for (int32 hi=0; hi<nh; hi++) {
                            float32 d=1e30f;
                            if (hs[hi].kind==2) { if (cok){ float32 dc=sqrtf((cur-cpx)*(cur-cpx)+(curY-cpy)*(curY-cpy));
                                float32 rpx=CenterR(hs[hi].op,mGL)*pxPerW; if (dc<rpx) d=dc*0.4f; } }
                            else ForEachSeg(hs[hi], mPivot, mGL, [&](NkVec3f P0, NkVec3f P1){
                                float32 ax,ay,bx,by; if (Project(P0,ax,ay)&&Project(P1,bx,by)){ float32 dd=SegDist(cur,curY,ax,ay,bx,by); if(dd<d)d=dd; } });
                    ...
                    for (int32 i=0;i<mCount;i++){ NkVec3f c=Ctr(i); NkVec3f oc={mCamPos.x-c.x,mCamPos.y-c.y,mCamPos.z-c.z};
                    ...
                    if (bestId>=0) {
                        if (!in.shiftDown) { for (int32 i=0;i<mCount;i++) mSel[i]=false; mSel[bestId]=true; mSelId=bestId; }
                        else { mSel[bestId]=!mSel[bestId]; if (mSel[bestId]) mSelId=bestId;
                               else { mSelId=-1; for (int32 i=0;i<mCount;i++) if (mSel[i]){ mSelId=i; break; } } }
                ...
                void DoDrag(const NkGizmoInput& in) {
                    ...
                    float32 cpx,cpy; bool cok=Project(mPivot,cpx,cpy);
                    if (op==0) {
                        if (kind==2) { float32 wpp=(2.f*mThY*mPivDist)/mVpH; NkVec3f wd=mRgt*(mdx*wpp)+mUp*(-mdy*wpp);
                            for (int32 i=0;i<mCount;i++) if (mSel[i]) mTr[i]=mTr[i]+wd; }
                        ...
                        for (int32 i=0;i<mCount;i++) if (mSel[i]) { NkVec3f B[3],Pi; BasisPivot(i,localOri,B,Pi); NkVec3f rel=Ctr(i)-Pi;
                            ...
                            if (mScale[i].x<-0.9f) mScale[i].x=-0.9f; if (mScale[i].y<-0.9f) mScale[i].y=-0.9f; if (mScale[i].z<-0.9f) mScale[i].z=-0.9f; }
                    } else {
                        if (cok) { const int32 a=AxisOf(mask);
                ...
                void BasisPivot(int32 i, bool localOri, NkVec3f B[3], NkVec3f& Pi) const {
                    if (!localOri) { B[0]={1,0,0}; B[1]={0,1,0}; B[2]={0,0,1}; Pi=mPivot; }
                    else { const NkMat4f& M=mComposed[i]; NkVec3f o=M*NkVec3f{0.f,0.f,0.f};
                           B[0]=Norm((M*NkVec3f{1,0,0})-o); B[1]=Norm((M*NkVec3f{0,1,0})-o); B[2]=Norm((M*NkVec3f{0,0,1})-o); Pi=o; }
                }
                ...
                int32   mCount=0;
                NkMat4f mComposed[kMax]; NkVec3f mHalf[kMax]={}; float32 mPickR[kMax]={};
                NkVec3f mPivot={0,0,0}, mGB[3]={{1,0,0},{0,1,0},{0,0,1}};
                float32 mPivDist=1.f, mGL=1.f; bool mHaveSel=false;
        };

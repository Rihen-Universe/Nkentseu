    ...
    namespace renderer {
        ...
        void NkRenderGraph::Execute(NkICommandBuffer* cmd) {
            ...
            for (NkPassBuilder* pass : mSorted) {
                ...
                if (needsRenderPass) {
                    ...
                    if (pass->clearColor) {
                        // NkAttachmentDesc colorAttach;
                        // colorAttach.clearValue.color = NkClearColor(pass->clearColorValue.x, 
                        //                                             pass->clearColorValue.y, 
                        //                                             pass->clearColorValue.z, 
                        //                                             pass->clearColorAlpha);
                        // rpDesc.AddColor(colorAttach);
                        cmd->SetClearColor(NkColorF(pass->clearColorValue.x, 
                                                    pass->clearColorValue.y, 
                                                    pass->clearColorValue.z, 
                                                    pass->clearColorAlpha));
                    }

                    if (pass->clearDepth || pass->clearStencil) {
                        // NkAttachmentDesc depthAttach;

                        // if (pass->clearDepth) colorAttach.depthAttach.depthStencil.depth = pass->clearDepth;
                        // if (pass->clearStencil) colorAttach.depthAttach.depthStencil.stencil = pass->clearStencilValue;

                        // rpDesc.SetDepth(depthAttach);

                        float32 depth = 1.0f;
                        float32 stencil = 0.0f;
                        if (pass->clearDepth) depth = pass->clearDepth;
                        if (pass->clearStencil) stencil = pass->clearStencilValue;
                        cmd->SetClearDepth(depth, stencil);
                    }
        ...
        void NkRenderGraph::InsertBarriers(NkICommandBuffer* cmd, NkPassBuilder* pass) {
            ...
            for (NkGraphResId resId : pass->reads) {
                ...
                NkResourceState needed = NkResourceState::NK_SHADER_READ;
                if (res.currentState != needed) {
                    if (res.texture.IsValid())
                        cmd->TextureBarrier(res.texture, res.currentState, needed);
            ...
            for (NkGraphResId resId : pass->writes) {
                ...
                if (idx >= (uint32)mResources.Size()) continue;
                auto& res = mResources[idx];
                NkResourceState needed = (pass->type == NkPassType::NK_SHADOW) ? NkResourceState::NK_DEPTH_WRITE : NkResourceState::NK_RENDER_TARGET;
                if (res.currentState != needed) {
                    if (res.texture.IsValid())
                        cmd->TextureBarrier(res.texture, res.currentState, needed);
                    res.currentState = needed;

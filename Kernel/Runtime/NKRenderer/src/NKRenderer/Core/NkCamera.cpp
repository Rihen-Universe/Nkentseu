...
namespace nkentseu {
	namespace renderer {
		...
		// NkCamera3D
		// =====================================================================
		NkCamera3D::NkCamera3D(const NkCamera3DData &data) : mData(data) {
			mDirty = true;
		}

		void NkCamera3D::SetPosition(NkVec3f p) noexcept {
			mData.position = p;
			mDirty = true;
		}

		void NkCamera3D::SetTarget(NkVec3f t) noexcept {
			mData.target = t;
			mDirty = true;
		}

		void NkCamera3D::SetUp(NkVec3f u) noexcept {
			mData.up = u;
			mDirty = true;
		}
		...
		void NkCamera3D::RebuildImpl() const noexcept {
			// ── View matrix (LookAt) ─────────────────────────────────────────
			NkVec3f fwd = {mData.target.x - mData.position.x, mData.target.y - mData.position.y,
			...
			NkVec3f up = mData.up;
			NkVec3f right = {fwd.y * up.z - fwd.z * up.y, fwd.z * up.x - fwd.x * up.z, fwd.x * up.y - fwd.y * up.x};
			...
			NkVec3f u = {right.y * fwd.z - right.z * fwd.y, right.z * fwd.x - right.x * fwd.z,
		...
		void NkCamera3D::BuildFrustum() const noexcept {
			...
			for (int p = 0; p < 6; p++) {
				float32 len = math::NkSqrt(mFrustumPlanes[p][0] * mFrustumPlanes[p][0] +
		...
		NkVec3f NkCamera3D::GetForward() const noexcept {
			NkVec3f f = {mData.target.x - mData.position.x, mData.target.y - mData.position.y,
						 mData.target.z - mData.position.z};
		...
		NkVec3f NkCamera3D::GetRight() const noexcept {
			...
			NkVec3f up = mData.up;
			NkVec3f r = {fwd.y * up.z - fwd.z * up.y, fwd.z * up.x - fwd.x * up.z, fwd.x * up.y - fwd.y * up.x};
		...
		bool NkCamera3D::IsAABBVisible(const NkAABB &b) const noexcept {
			...
			for (int p = 0; p < 6; p++) {
				...
				if (mFrustumPlanes[p][0] * px + mFrustumPlanes[p][1] * py + mFrustumPlanes[p][2] * pz +
						mFrustumPlanes[p][3] <
					0.f)
					return false;
			}
		...
		bool NkCamera3D::IsSphereVisible(NkVec3f c, float32 r) const noexcept {
			Rebuild();
			for (int p = 0; p < 6; p++) {
				float32 d = mFrustumPlanes[p][0] * c.x + mFrustumPlanes[p][1] * c.y + mFrustumPlanes[p][2] * c.z +
							mFrustumPlanes[p][3];
				if (d < -r)
					return false;
			}
		...
		NkCameraUBO NkCamera3D::BuildUBO(float32 time, float32 dt) const noexcept {
			...
			ubo.viewport = {mData.aspect, 1.f, 1.f / (mData.aspect > 0 ? mData.aspect : 1.f), 1.f};
			float32 n = mData.nearPlane, f = mData.farPlane;
			ubo.depthParams = {n, f, (n - f) / (n * f), 1.f / n};
			for (int p = 0; p < 6; p++) {
				ubo.frustumPlanes[p] = {mFrustumPlanes[p][0], mFrustumPlanes[p][1], mFrustumPlanes[p][2],
										mFrustumPlanes[p][3]};
			}
			return ubo;
		}
		...
		// NkCamera2D
		// =====================================================================
		NkCamera2D::NkCamera2D(const NkCamera2DData &data) : mData(data) {
			mDirty = true;
		}

		void NkCamera2D::SetCenter(NkVec2f c) noexcept {
			mData.center = c;
			mDirty = true;
		}

		void NkCamera2D::SetZoom(float32 z) noexcept {
			mData.zoom = z;
			mDirty = true;
		}

		void NkCamera2D::SetRotation(float32 d) noexcept {
			mData.rotation = d;
			mDirty = true;
		}
		...
		NkCameraUBO NkCamera2D::BuildUBO(float32 time, float32 dt) const noexcept {
			...
			ubo.viewport = {(float32)mData.width, (float32)mData.height, mData.width > 0 ? 1.f / mData.width : 0.f,

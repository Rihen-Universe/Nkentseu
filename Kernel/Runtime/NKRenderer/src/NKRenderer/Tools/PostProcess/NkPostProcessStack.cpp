...
namespace renderer {
    ...
    bool NkPostProcessStack::Init(NkIDevice* d, NkTextureLibrary* t,
        ...
        // Compile pipelines
        NkPipelineDesc pd; 
        pd.type=NkPipelineType::NK_POSTPROCESS;
        pd.name="PP_SSAO";   
        mPipeSSAO =mDevice->CreatePipeline(pd);
        pd.name="PP_Bloom"; 
        mPipeBloom=mDevice->CreatePipeline(pd);
        pd.name="PP_Tone";   
        mPipeTone =mDevice->CreatePipeline(pd);
        pd.name="PP_FXAA";   
        mPipeFXAA =mDevice->CreatePipeline(pd);
        return true;
    }
    ...
    void NkPostProcessStack::CreateTextures() {
        ...
        for(int i=0;i<6;i++){
            uint32 s=1<<i;
            mBloomTex[i]=mTex->CreateRenderTarget(mW/s,mH/s,NkGPUFormat::NK_RGBA16_FLOAT,false,true,"Bloom");
        }

...
namespace nkentseu {
	namespace renderer {
		...
		NkAIRenderingTarget::NkAIRenderingTarget(const NkAITargetDesc &desc) : mDesc(desc) {
		...
		void NkAIRenderingTarget::Capture(NkICommandBuffer *cmd, NkTexHandle colorTex, NkTexHandle depthTex,
										  NkTexHandle normalTex, NkTexHandle albedoTex, NkTexHandle motionTex,
										  NkTexHandle instanceTex) {
			if (!mEnabled || mRing.Empty())
				return;
		...
		void NkAIRenderingTarget::IssueCopy(NkICommandBuffer * /*cmd*/, NkTexHandle /*src*/, RingSlot & /*slot*/,
											uint32 /*chanIdx*/) {

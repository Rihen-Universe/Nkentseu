	...
	namespace renderer {
		...
		class NkVoxelVolume {
			public:
				...
				[[nodiscard]] uint32 DimX() const noexcept {
					return mCfg.dimX;
				}

				[[nodiscard]] uint32 DimY() const noexcept {
					return mCfg.dimY;
				}

				[[nodiscard]] uint32 DimZ() const noexcept {
					return mCfg.dimZ;
				}

				// Handles RHI (storage images 3D, NK_UNORDERED_ACCESS).
				[[nodiscard]] NkTextureHandle Density() const noexcept {
					return mDensity;
				}
				...
				[[nodiscard]] uint32 ResDensity() const noexcept {
					return mResDensity;
				}

				[[nodiscard]] uint32 ResMaterial() const noexcept {
					return mResMaterial;
				}

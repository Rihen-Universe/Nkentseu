...
namespace nkentseu {
	namespace renderer {
		...
		NkVoxelSystem::~NkVoxelSystem() noexcept {
			Shutdown();
		}

		bool NkVoxelSystem::Init(NkIDevice *device, NkRenderGraph *graph, NkTextureLibrary *texLib,
								 NkShaderLibrary *shaderLib, const NkVoxelConfig &cfg) noexcept {
			mDevice = device;
			mGraph = graph;
			mTexLib = texLib;
			mShaders = shaderLib;
			...
			if (!mCompute.Init(device))
				return false;
			if (!mPipelines.Init(&mCompute))
				return false;
		...
		NkVec3f NkVoxelSystem::WorldToVoxel(const NkVec3f &w) const noexcept {
			...
			return NkVec3f{(w.x - mCfg.originWorld.x) * inv, (w.y - mCfg.originWorld.y) * inv,
						   (w.z - mCfg.originWorld.z) * inv};
		}

		NkVec3f NkVoxelSystem::VoxelToWorld(const NkVec3f &v) const noexcept {
			return NkVec3f{mCfg.originWorld.x + v.x * mCfg.voxelSize, mCfg.originWorld.y + v.y * mCfg.voxelSize,
		...
		void NkVoxelSystem::SetBrush(const NkVoxelBrush &brush) noexcept {
			mBrush = brush;
		}
		...
		void NkVoxelSystem::EndStroke() noexcept {
			mStroke.End();
		}
		...
		void NkVoxelSystem::RecordEditPass(NkICommandBuffer *cmd) noexcept {
			...
			if (!mReady || !cmd)
				return;
			if (mStroke.PendingDabs().Empty())
				return;

			// Clampe la dirty box aux dimensions de la grille.
			NkVoxelBox box = mStroke.DirtyBox();
			if (box.maxX > (int32)mCfg.dimX)
				box.maxX = (int32)mCfg.dimX;
			if (box.maxY > (int32)mCfg.dimY)
				box.maxY = (int32)mCfg.dimY;
			if (box.maxZ > (int32)mCfg.dimZ)
				box.maxZ = (int32)mCfg.dimZ;
			if (box.IsEmpty()) {
			...
			NkComputePassDesc pass;
			pass.debugName = "Voxel_Edit";
			...
			for (uint32 i = 0; i < (uint32)dabs.Size(); ++i) {
				NkVoxelBrushGPU pc = MakeVoxelGPU(mStroke.Brush(), dabs[i], box.minX, box.minY, box.minZ);

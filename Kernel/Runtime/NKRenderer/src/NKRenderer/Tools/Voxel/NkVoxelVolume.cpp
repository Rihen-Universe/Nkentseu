...
namespace nkentseu {
	namespace renderer {
		...
		NkVoxelVolume::~NkVoxelVolume() noexcept {
			Shutdown();
		}
		...
		bool NkVoxelVolume::CreateTargets() noexcept {
			if (!mDevice || mCfg.dimX == 0 || mCfg.dimY == 0 || mCfg.dimZ == 0)
				return false;
		...
		void NkVoxelVolume::DestroyTargets() noexcept {
			if (mDevice) {
				if (mDensity.IsValid())
					mDevice->DestroyTexture(mDensity);
		...
		void NkVoxelVolume::ImportToGraph(NkRenderGraph *graph) noexcept {
			if (!graph)
				return;
			mResDensity = graph->ImportTexture("Voxel_Density", mDensity, NkResourceState::NK_UNORDERED_ACCESS);
			mResMaterial = graph->ImportTexture("Voxel_Material", mMaterial, NkResourceState::NK_UNORDERED_ACCESS);
			if (mColor.IsValid())
				mResColor = graph->ImportTexture("Voxel_Color", mColor, NkResourceState::NK_UNORDERED_ACCESS);
		}

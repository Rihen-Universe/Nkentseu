...
namespace nkentseu {
	namespace renderer {
		...
		void NkVoxelStroke::AddSample(const NkVec3f &centerVox, float32 pressure) noexcept {
			...
			const float32 spacing = (mBrush.radiusVox * 0.35f > 1.f) ? mBrush.radiusVox * 0.35f : 1.f;
			...
			if (dist < spacing)
				return; // trop proche : on attend le prochain echantillon

			const float32 inv = 1.f / dist;
			for (float32 t = spacing; t <= dist; t += spacing) {
				emit(mLastSample.x + dx * inv * t, mLastSample.y + dy * inv * t, mLastSample.z + dz * inv * t,
					 pressure);
			}
			mLastSample = centerVox;
		}

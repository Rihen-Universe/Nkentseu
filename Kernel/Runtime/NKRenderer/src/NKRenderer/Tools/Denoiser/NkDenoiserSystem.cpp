...
namespace nkentseu {
	namespace renderer {
		...
		bool NkDenoiserSystem::Init(NkIDevice *device, NkDenoiserBackend preferredBackend,
		...
		void NkDenoiserSystem::Denoise(NkICommandBuffer *cmd, const NkDenoiseRequest &req) {
			if (!mReady || !req.colorIn.IsValid() || !req.colorOut.IsValid())
				return;
		...
		void NkDenoiserSystem::DenoiseFast(NkICommandBuffer *cmd, NkTexHandle colorIn, NkTexHandle colorOut,
										   bool resetHistory) {
		...
		bool NkDenoiserSystem::SupportsBackend(NkDenoiserBackend b) const {
			...
			return b == NkDenoiserBackend::NK_DENOISE_SPATIAL || b == NkDenoiserBackend::NK_DENOISE_TAA;
		}
		...
		void NkDenoiserSystem::RunSpatialFilter(NkICommandBuffer * /*cmd*/, const NkDenoiseRequest & /*req*/) {
			// Dispatch a fullscreen bilateral filter compute shader
			// Reads req.colorIn, writes req.colorOut
			// Stub: pass-through for now
		}

		void NkDenoiserSystem::RunTemporalAccum(NkICommandBuffer * /*cmd*/, const NkDenoiseRequest &req) {

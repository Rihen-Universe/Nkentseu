	...
	namespace renderer {
		...
		class NkDenoiserSystem {
			public:
				...
				// ── Cycle de vie ──────────────────────────────────────────────────────
				bool Init(NkIDevice *device, NkDenoiserBackend preferredBackend = NkDenoiserBackend::NK_DENOISE_AUTO,
				...
				void DenoiseFast(NkICommandBuffer *cmd, NkTexHandle colorIn, NkTexHandle colorOut,
				...
				const NkDenoiserConfig &GetConfig() const {
					return mCfg;
				}
				...
				NkDenoiserBackend GetActiveBackend() const {
					return mActiveBackend;
				}

				const NkString &GetBackendName() const {
					return mBackendName;
				}

...
namespace nkentseu {
	namespace renderer {
		...
		void NkTextRenderer::Shutdown() {
			for (auto *e : mFonts) {
				if (e) {
					if (e->nkfAtlas) {
						e->nkfAtlas->Clear();
						memory::NkGetDefaultAllocator().Delete(e->nkfAtlas);
					}
		...
		NkFontHandle NkTextRenderer::LoadFont(const char *path, float32 sizePx, bool sdf) {
			...
			if (!entry->nkfFont) {
				memory::NkGetDefaultAllocator().Delete(entry->nkfAtlas);
		...
		NkFontHandle NkTextRenderer::LoadFontFromMemory(const uint8 *data, uint32 size, float32 sizePx, bool sdf) {
			auto *entry = memory::NkGetDefaultAllocator().New<NkFontEntry>();
			entry->handle = AllocHandle();
			entry->sizePixels = sizePx;
			entry->isSDF = sdf;
		...
		NkFontHandle NkTextRenderer::LoadFontCustom(const char *path, float32 sizePx,
			...
			for (uint32 cp = 32; cp < 127; cp++) {
				int32 gw = 0, gh = 0;
				if (!desc.GetGlyph(entry->mHandle, cp, atlasPx.Data(), atlasW, cx, cy, 0, 0, desc.userData, nullptr,
		...
		void NkTextRenderer::UnloadFont(NkFontHandle &handle) {
			for (uint32 i = 0; i < (uint32)mFonts.Size(); i++) {
				if (mFonts[i] && mFonts[i]->handle == handle) {
					...
					if (e->isCustom && e->mCustom.Destroy)
						e->mCustom.Destroy(e->mHandle, e->mCustom.userData);
					if (e->atlasTexture.IsValid())
						mTexLib->Release(e->atlasTexture);
					memory::NkGetDefaultAllocator().Delete(e);
					mFonts.RemoveAt(i);
					break;
				}
		...
		void NkTextRenderer::DrawText(NkVec2f pos, const char *text, NkFontHandle font, float32 size,
			...
			while (*p) {
				...
				if (!g)
					continue;
				if (g->visible) {
					NkRectF dst = {x + g->x0 * scale, y + g->y0 * scale, (g->x1 - g->x0) * scale,
								   (g->y1 - g->y0) * scale};
		...
		void NkTextRenderer::DrawTextCentered(NkRectF bounds, const char *text, NkFontHandle font, float32 size,
											  uint32 colorRGBA) {
			NkVec2f sz = CalcTextSize(text, font, size);
			NkVec2f pos = {bounds.x + (bounds.w - sz.x) * 0.5f, bounds.y + (bounds.h - sz.y) * 0.5f};
			DrawText(pos, text, font, size, colorRGBA);
		}

		void NkTextRenderer::DrawTextWorld(NkVec3f worldPos, const NkMat4f &viewProj, uint32 vpW, uint32 vpH,
										   const char *text, NkFontHandle font, float32 size, uint32 colorRGBA) {
		...
		// ── Extrusion 3D ─────────────────────────────────────────────────────────
		NkMeshHandle NkTextRenderer::ExtrudeText3D(const char *text, NkFontHandle font, float32 scale, float32 depth,
												   NkMeshSystem *meshSys, const NkMat4f &transform) {
			NkFontEntry *e = FindEntry(font);
			if (!e || !e->nkfFont || !text || !meshSys)
				return NkMeshHandle::Null();

			// Use NKFont 3D extrusion
			NkFontGlyphMesh3D mesh3D = e->nkfFont->GenerateTextMesh3D(text, scale, depth, transform, {1, 1, 1, 1});

			if (mesh3D.vertices.Empty())
				return NkMeshHandle::Null();
			...
			for (auto idx : mesh3D.indices)
				idxBuf.PushBack((uint32)idx);

			NkMeshDesc desc = NkMeshDesc::Simple(NkVertexLayout::Default3D(), verts.Data(), (uint32)verts.Size(),

...
namespace nkentseu {
	namespace renderer {
		...
		bool NkCullingSystem::Init(const NkCullingConfig &cfg) {
			mCfg = cfg;
			mOctree = memory::NkGetDefaultAllocator().New<OctreeType>(cfg.worldX, cfg.worldY, cfg.worldZ, cfg.worldW,
																	  cfg.worldH, cfg.worldD);
			mReady = true;
			return true;
		}
		...
		void NkCullingSystem::Register(uint64 id, const NkAABB &aabb, float32 maxDist, bool alwaysVisible) {
			NkCullable obj;
			obj.id = id;
			obj.aabb = aabb;
		...
		NkCullResult NkCullingSystem::EvaluateOne(const NkCullable &obj) const {
			...
			if (mCfg.distanceCulling && obj.maxDrawDist > 0.f) {
				...
				NkVec3f diff = {c.x - mCamPos.x, c.y - mCamPos.y, c.z - mCamPos.z};
				float32 dist = sqrtf(diff.x * diff.x + diff.y * diff.y + diff.z * diff.z);
				if (dist > obj.maxDrawDist)
					return NkCullResult::NK_CULLED_DISTANCE;
				if (dist < obj.minDrawDist)
					return NkCullResult::NK_CULLED_DISTANCE;
			}

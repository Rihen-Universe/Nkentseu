...
namespace nkentseu {
	namespace renderer {
		...
		NkSimulationRenderer::~NkSimulationRenderer() {
			Shutdown();
		}

		bool NkSimulationRenderer::Init(NkIDevice *device, NkRender3D *r3d, NkVFXSystem *vfx) {
			mDevice = device;
			mR3D = r3d;
			mVFX = vfx;
			return true;
		...
		// ── Personnage PV3DE ─────────────────────────────────────────────────────
		void NkSimulationRenderer::SubmitCharacter(NkMeshHandle mesh, NkMatInstHandle skinMat, const NkMat4f &transform,
												   const NkMat4f *bones, uint32 boneCount, const NkBlendShapeState &bs,
			...
			if (mVFX) {
				...
				if ((em.sadness > 0.5f || em.pain > 0.7f) && bs.tears > 0.1f) {
					...
					NkMat4f eyeL = transform;
					eyeL[3][0] -= 0.03f;
					eyeL[3][1] += 0.08f;
					NkMat4f eyeR = transform;
					eyeR[3][0] += 0.03f;
					eyeR[3][1] += 0.08f;
					NkEmitterDesc ed;
		...
		void NkSimulationRenderer::UpdateSkinParams(NkMatInstHandle skinMat, const NkBlendShapeState &bs,
													const NkEmotionState &em) {
			...
			float32 redness = bs.blush + em.anger * 0.4f + em.joy * 0.2f + em.pain * 0.3f;

...
namespace nkentseu {
	namespace renderer {
		...
		// ---------------------------------------------------------------------
		bool NkVirtualShadowMaps::Init(NkIDevice *d, NkMeshSystem *m, NkMaterialSystem *mat,
									   const NkVirtualShadowMapsConfig &cfg, uint32 framesInFlight) {
			mDevice = d;
			mMesh = m;
			mMat = mat;
			mCfg = cfg;
			mFramesInFlight = framesInFlight > 0 ? framesInFlight : 3;
			if (mFramesInFlight > 16)
				mFramesInFlight = 16;
			if (!mDevice)
				return false;
			...
			auto td = NkTextureDesc::DepthStencil(atlasSz, atlasSz, NkGPUFormat::NK_D32_FLOAT);
			...
			for (uint32 i = 0; i < mFramesInFlight; i++) {
				mUBOSlotsRing[i] = mDevice->CreateBuffer(NkBufferDesc::Uniform(sizeof(ShadowSlotsUBOBlock)));
			}
			mCurFrameSlot = 0;
		...
		void NkVirtualShadowMaps::Shutdown() {
			if (mShadowFB.IsValid()) {
				mDevice->DestroyFramebuffer(mShadowFB);
				mShadowFB = {};
			}
			for (auto &ubo : mUBOSlotsRing) {
				if (ubo.IsValid())
					mDevice->DestroyBuffer(ubo);
			}
			mUBOSlotsRing.Clear();
			if (mShadowSampler.IsValid()) {
				mDevice->DestroySampler(mShadowSampler);
				mShadowSampler = {};
			}
			if (mShadowRawSampler.IsValid()) {
				mDevice->DestroySampler(mShadowRawSampler);
				mShadowRawSampler = {};
			}
			if (mAtlasRhi.IsValid()) {
				mDevice->DestroyTexture(mAtlasRhi);
				mAtlasRhi = {};
			}
		}
		...
		// ---------------------------------------------------------------------
		void NkVirtualShadowMaps::ComputeCascadeSplits(float32 nearP, float32 farP, float32 lambda, uint32 N,
		...
		// ---------------------------------------------------------------------
		static void ComputeSubFrustumCornersWS(const NkCamera3D &cam, float32 zNear, float32 zFar,
		...
		// ---------------------------------------------------------------------
		void NkVirtualShadowMaps::ComputeDirectionalCascade(const NkCamera3D &mainCam, const NkLightDesc &light,
															uint32 cascadeIdx, float32 splitNear, float32 splitFar,
															uint32 tilePx, NkMat4f &outView, NkMat4f &outProj) const {
			(void)splitNear;
			(void)splitFar;
			...
			if (mCfg.autoFitDirectional && mRender3D) {
				...
				center = b.Center();
				NkVec3f ex = {(b.max.x - b.min.x) * 0.5f, (b.max.y - b.min.y) * 0.5f, (b.max.z - b.min.z) * 0.5f};
				...
				center = mCfg.useFixedCascadeCenter ? mCfg.cascadeWorldCenter : mainCam.GetPosition();
			...
			outProj = NkMat4f::Orthogonal(radius * 2.f, radius * 2.f, 0.1f, radius * 8.f);
		}
		...
		// ---------------------------------------------------------------------
		void NkVirtualShadowMaps::AllocSlotsDirectional(const NkCamera3D &mainCam, const NkLightDesc &light,
														uint32 lightIdx) {
			...
			for (uint32 c = 0; c < N; c++) {
				...
				ComputeDirectionalCascade(mainCam, light, c, subNear, subFar, tilePx, lightView, lightProj);
				...
				s.lightPosOrDir = NkVec4f{light.direction.x, light.direction.y, light.direction.z, subFar};
		...
		void NkVirtualShadowMaps::UploadSlotsUBO() {
			...
			for (uint32 i = 0; i < kMaxLightsShadow / 4; i++) {
				b.firstSlotPerLight[i] =
					NkVec4f{float32(mFirstSlotPerLight[i * 4 + 0]), float32(mFirstSlotPerLight[i * 4 + 1]),
							float32(mFirstSlotPerLight[i * 4 + 2]), float32(mFirstSlotPerLight[i * 4 + 3])};
				b.slotCountPerLight[i] =
			...
			float32 shadowYFlip = (_api == ::nkentseu::NkGraphicsApi::NK_GFX_API_DX11 ||
								   _api == ::nkentseu::NkGraphicsApi::NK_GFX_API_DX12)
									  ? 1.f
									  : 0.f;
			...
			if (mCurFrameSlot < mUBOSlotsRing.Size() && mUBOSlotsRing[mCurFrameSlot].IsValid()) {
		...
		void NkVirtualShadowMaps::RenderAllShadows(NkICommandBuffer *cmd) {
			...
			for (uint32 i = 0; i < mActiveSlotCount; i++) {
				...
				NkViewport vp{float32(s.tileRect.x),
							  float32(s.tileRect.y),
							  float32(s.tileRect.w),
							  float32(s.tileRect.h),
							  0.f,
							  1.f,
							  /*flipY=*/false};
			...
			if (mAtlasRhi.IsValid()) {
				cmd->TextureBarrier(mAtlasRhi, NkResourceState::NK_DEPTH_WRITE, NkResourceState::NK_SHADER_READ,

...
namespace nkentseu {
	namespace renderer {
		...
		class NkShadowSystem {
			public:
				bool Init(NkIDevice *d, NkMeshSystem *m, NkMaterialSystem *mat, const NkShadowSystemConfig &cfg = {});
				void Shutdown();
				...
				void SetConfig(const NkShadowSystemConfig &c) {
					mCfg = c;
				}

				NkShadowSystemConfig &GetConfig() noexcept {
					return mCfg;
				}

				const NkShadowSystemConfig &GetConfig() const noexcept {
					return mCfg;
				}

				void BeginShadowPass(NkICommandBuffer *cmd, NkVec3f lightDir, const NkCamera3D &mainCam);
				...
				NkBufferHandle GetShadowUBO() const {
					return mUBOShadow;
				}
				...
				const NkMat4f *GetCascadeMats(uint32 *n) const {
					*n = mActiveCascades;
					return mCascadeMats;
				}

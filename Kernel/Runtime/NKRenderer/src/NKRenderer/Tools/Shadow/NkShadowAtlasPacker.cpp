	...
	namespace renderer {
		...
		uint32 NkShadowAtlasPacker::ComputeFitY(uint32 startIdx, uint32 tileW) const {
			...
			while (i < N && width < tileW) {
				if (mSegments[i].y > maxY)
					maxY = mSegments[i].y;
				width += mSegments[i].w;
				i++;
			}
			if (width < tileW)
				return UINT_MAX; // depasse la fin de l'atlas
			return maxY;
		}
		...
		// ---------------------------------------------------------------------
		bool NkShadowAtlasPacker::Allocate(uint32 tileW, uint32 tileH, NkShadowTileRect &outRect) {
			outRect.valid = false;
			if (tileW == 0 || tileH == 0)
				return false;
			if (tileW > mAtlasW || tileH > mAtlasH)
				return false;
			...
			for (uint32 i = 0; i < N; i++) {
				...
				if (fitY == UINT_MAX)
					continue;
				if (fitY + tileH > mAtlasH)
					continue; // depasse haut
				if (fitY < bestY) {
					bestY = fitY;
					bestIdx = i;
					bestX = mSegments[i].x;
		...
		// ---------------------------------------------------------------------
		void NkShadowAtlasPacker::InsertSegment(uint32 startIdx, uint32 tileW, uint32 tileH, uint32 fitY) {
			...
			if (curIdx > 0 && mSegments[curIdx - 1].y == mSegments[curIdx].y) {
				mSegments[curIdx - 1].w += mSegments[curIdx].w;
				mSegments.RemoveAt(curIdx);
				curIdx -= 1;
			}
			if (curIdx + 1 < mSegments.Size() && mSegments[curIdx + 1].y == mSegments[curIdx].y) {

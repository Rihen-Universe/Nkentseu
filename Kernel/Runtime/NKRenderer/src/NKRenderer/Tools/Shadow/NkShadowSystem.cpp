...
namespace nkentseu {
	namespace renderer {
		...
		bool NkShadowSystem::Init(NkIDevice *d, NkMeshSystem *m, NkMaterialSystem *mat,
		...
		void NkShadowSystem::Shutdown() {
			...
			if (mUBOShadow.IsValid()) {
				mDevice->DestroyBuffer(mUBOShadow);
				mUBOShadow = {};
			}
		...
		void NkShadowSystem::UploadShadowUBO() {
			...
			int32 softMode = 0;
			if (mCfg.pcfMode == NkPCFMode::PCSS)
				softMode = 2;
			else if (mCfg.pcfMode != NkPCFMode::NONE)
				softMode = 1;
		...
		void NkShadowSystem::BeginShadowPass(NkICommandBuffer *cmd, NkVec3f lightDir, const NkCamera3D &cam) {
			(void)cmd;
			(void)lightDir;
			(void)cam;
		...
		static void ComputeSubFrustumCornersWS(const NkCamera3D &cam, float zNear, float zFar, NkVec3f outCorners[8]) {
		...
		void NkShadowSystem::RenderShadowPasses(NkICommandBuffer *cmd) {
			...
			for (uint32 c = 0; c < N; c++) {
				...
				NkMat4f lightProj = NkMat4f::Orthogonal(radius * 2.f, radius * 2.f, 0.1f, radius * 8.f);
			...
			for (uint32 c = 0; c < N; c++) {
				...
				uint32 tileY = c / mCellsX;
				NkViewport vp{
					float(tileX * mTileSize), float(tileY * mTileSize), float(mTileSize), float(mTileSize), 0.f, 1.f,
					/*flipY=*/false};
				NkRect2D scissor{(int32)(tileX * mTileSize), (int32)(tileY * mTileSize), (int32)mTileSize,
								 (int32)mTileSize};
				cmd->SetViewport(vp);

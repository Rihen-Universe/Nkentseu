...
namespace nkentseu {
	namespace renderer {
		...
		NkPixolBuffer::~NkPixolBuffer() noexcept {
			Shutdown();
		}
		...
		void NkPixolBuffer::DestroyTargets() noexcept {
			if (mDevice) {
				if (mDepth.IsValid())
					mDevice->DestroyTexture(mDepth);
				if (mNormal.IsValid())
					mDevice->DestroyTexture(mNormal);
				if (mMaterial.IsValid())
					mDevice->DestroyTexture(mMaterial);
				if (mColor.IsValid())
					mDevice->DestroyTexture(mColor);
				if (mMask.IsValid())
					mDevice->DestroyTexture(mMask);
			}
		...
		void NkPixolBuffer::ImportToGraph(NkRenderGraph *graph) noexcept {
			if (!graph)
				return;
			mResDepth = graph->ImportTexture("Pixol_Depth", mDepth, NkResourceState::NK_UNORDERED_ACCESS);
			mResNormal = graph->ImportTexture("Pixol_Normal", mNormal, NkResourceState::NK_UNORDERED_ACCESS);
			if (mColor.IsValid())
				mResColor = graph->ImportTexture("Pixol_Color", mColor, NkResourceState::NK_UNORDERED_ACCESS);
			if (mMask.IsValid())
				mResMask = graph->ImportTexture("Pixol_Mask", mMask, NkResourceState::NK_UNORDERED_ACCESS);
		}

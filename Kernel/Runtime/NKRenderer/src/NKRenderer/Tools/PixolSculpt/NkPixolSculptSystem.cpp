...
namespace nkentseu {
	namespace renderer {
		...
		NkPixolSculptSystem::~NkPixolSculptSystem() noexcept {
			Shutdown();
		}

		bool NkPixolSculptSystem::Init(NkIDevice *device, NkRenderGraph *graph, NkTextureLibrary *texLib,
									   NkShaderLibrary *shaderLib, uint32 width, uint32 height,
		...
		void NkPixolSculptSystem::EndStroke() noexcept {
			mStroke.End();
		}
		...
		void NkPixolSculptSystem::RecordBrushPass(NkICommandBuffer *cmd) noexcept {
			...
			if (dirty.x + dirty.w > (int32)mWidth)
				dirty.w = (int32)mWidth - dirty.x;
			if (dirty.y + dirty.h > (int32)mHeight)
				dirty.h = (int32)mHeight - dirty.y;
			if (dirty.IsEmpty()) {
			...
			pass.debugName = "PixolSculpt_Brush";
		...
		void NkPixolSculptSystem::RecordResolvePass(NkICommandBuffer *cmd) noexcept {
			...
			NkComputePassDesc pass;
			pass.debugName = "PixolSculpt_Resolve";
			mCompute.BeginPass(cmd, pass);
			mCompute.SetPipeline(pipe);
			mCompute.BindStorageTexture(0, mPixol.Depth());
			mCompute.BindStorageTexture(1, mPixol.Normal());

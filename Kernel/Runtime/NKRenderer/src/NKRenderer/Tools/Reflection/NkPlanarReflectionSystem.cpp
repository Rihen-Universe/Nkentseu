...
namespace nkentseu {
	namespace renderer {
		...
		bool NkPlanarReflectionSystem::Init(NkIDevice *dev, NkTextureLibrary *texLib, NkMaterialSystem *matSys) {
			mDevice = dev;
			mTexLib = texLib;
			mMatSys = matSys;
		...
		NkPlanarReflectionHandle NkPlanarReflectionSystem::Register(const NkPlanarReflectionDesc &desc) {
			...
			logger.Info("[NkPlanarReflectionSystem] Registered plane idx={0} ({1}x{2} hdr={3} twoSided={4})\n", h.idx,
						desc.rtWidth, desc.rtHeight, desc.hdr ? 1 : 0, needsBack ? 1 : 0);
			return h;
		}

		void NkPlanarReflectionSystem::Unregister(NkPlanarReflectionHandle handle) {
			if (!handle.IsValid() || handle.idx >= mPlanes.Size())
				return;

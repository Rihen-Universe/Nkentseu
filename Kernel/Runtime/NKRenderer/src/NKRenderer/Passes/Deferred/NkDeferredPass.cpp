...
namespace nkentseu {
    namespace renderer {
        ...
        void NkDeferredPass::BeginLighting(NkICommandBuffer* cmd) {
            if (!mReady) return;
            BuildLightGrid(cmd);
            cmd->TextureBarrier(mTexLib->GetRHIHandle(mGBuffer.lightAccum), NkResourceState::NK_SHADER_READ, NkResourceState::NK_RENDER_TARGET);
        }

        void NkDeferredPass::EndLighting(NkICommandBuffer* cmd) {
            if (!mReady) return;
            cmd->TextureBarrier(mTexLib->GetRHIHandle(mGBuffer.lightAccum), NkResourceState::NK_RENDER_TARGET, NkResourceState::NK_SHADER_READ);
        }

	...
	namespace renderer {
		...
		struct NkGBuffer {
				...
				bool IsValid() const noexcept {
					return albedoMetallic.IsValid() && normalRoughness.IsValid() && emissiveAO.IsValid() &&
						   depth.IsValid() && lightAccum.IsValid();
				}
		};
		...
		class NkDeferredPass {
			public:
				...
				bool Init(NkIDevice *device, NkRenderGraph *graph, NkTextureLibrary *texLib, uint32 width,
				...
				// ── Accès ─────────────────────────────────────────────────────────────
				const NkGBuffer &GetGBuffer() const {
					return mGBuffer;
				}

				NkTexHandle GetLightAccum() const {
					return mGBuffer.lightAccum;
				}

				const NkDeferredConfig &GetConfig() const {
					return mCfg;
				}
				...
				uint32 GetSubmittedLights() const {
			...
			private:
				...
				NkTextureDesc MakeGBufDesc(uint32 w, uint32 h, NkGPUFormat fmt, bool isDepth = false) const;
		};

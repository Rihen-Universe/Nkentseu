...
2. **HEVC — features de bord** (refusées proprement aujourd'hui, pas des bugs) : tuiles, 4:2:2/4:4:4,
   10-bit inter, `ref_pic_lists_modification`, `scaling_list_data`, règle CU 8×8 `log2ParallelMergeLevel>2`.
3. **Muxer WebM** (seul conteneur d'écriture manquant — brique 5).
4. **Nouveaux codecs (optionnels, chacun un chantier dédié)** : AV1, MPEG-2 vidéo, Theora/OGV, AMR-NB/WB.
5. **Expansion encodeur (optionnel)** : profils H.264 avancés au-delà du baseline déjà livré.

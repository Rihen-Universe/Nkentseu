	...
	namespace ecs {
		...
		class NkEntityBuilder {
			public:
				...
				template <typename T> NkEntityBuilder &With(const T &value = T{}) noexcept;

				template <typename T> NkEntityBuilder &With(T &&value) noexcept;
		...
		class NkBatchBuilder {
			public:
				...
				template <typename T> NkBatchBuilder &With(const T &value = T{}) noexcept;
		...
		class NkWorld {
			public:
				 ...
				 */
				template <typename T> T &Add(NkEntityId id, const T &value = T{}) noexcept;

				template <typename T> T &Add(NkEntityId id, T &&value) noexcept;
				 ...
				 */
				template <typename T> void Remove(NkEntityId id) noexcept;

				// ── Gestion des composants (opérations différées) ───────────
				template <typename T> void AddDeferred(NkEntityId id, const T &value = T{}) noexcept;

				template <typename T> void RemoveDeferred(NkEntityId id) noexcept;

				// ── Accès aux composants (lecture/écriture) ─────────────────
				template <typename T> [[nodiscard]] T *Get(NkEntityId id) noexcept;

				template <typename T> [[nodiscard]] const T *Get(NkEntityId id) const noexcept;

				/**
				 * @brief Récupère une référence garantie vers un composant.
				 * @warning Asserte si le composant n'existe pas.
				 */
				template <typename T> [[nodiscard]] T &GetRef(NkEntityId id) noexcept;

				template <typename T> [[nodiscard]] bool Has(NkEntityId id) const noexcept;
				 ...
				 * @endcode
				 */
				template <typename... Ts> [[nodiscard]] NkQueryResult<Ts...> Query() noexcept;

				// ── Système d'événements gameplay ───────────────────────────
				template <typename T, typename Fn> NkGameplayEventBus::SubscriptionId Subscribe(Fn &&fn) noexcept;

				template <typename T> void Unsubscribe(NkGameplayEventBus::SubscriptionId id) noexcept;

				template <typename T> void Emit(const T &event) noexcept;

				template <typename T> void Emit(T &&event) noexcept;

				template <typename T> void QueueEvent(const T &event) noexcept;
		...
		// ── NkEntityBuilder ─────────────────────────────────────────────────
		inline NkEntityBuilder::NkEntityBuilder(NkWorld &world) noexcept : mWorld(world), mId(world.CreateEntity()) {
		}

		template <typename T> NkEntityBuilder &NkEntityBuilder::With(const T &value) noexcept {
			mWorld.Add<T>(mId, value);
			return *this;
		}

		template <typename T> NkEntityBuilder &NkEntityBuilder::With(T &&value) noexcept {
		...
		// ── NkBatchBuilder ──────────────────────────────────────────────────
		inline NkBatchBuilder::NkBatchBuilder(NkWorld &world, NkEntityId *out, uint32 count) noexcept
			: mWorld(world), mOut(out), mCount(count) {
		}

		template <typename T> NkBatchBuilder &NkBatchBuilder::With(const T &value) noexcept {
			mMask.Set(NkIdOf<T>());
			mPending.PushBack({NkIdOf<T>(), NkFunction<void(NkWorld &, NkEntityId)>(
		...
		template <typename T> T &NkWorld::Add(NkEntityId id, const T &value) noexcept {
			return AddImpl<T>(id, value);
		}

		template <typename T> T &NkWorld::Add(NkEntityId id, T &&value) noexcept {
			return AddImpl<T>(id, traits::NkMove(value));
		}

		template <typename T> void NkWorld::Remove(NkEntityId id) noexcept {
			RemoveImpl(id, NkIdOf<T>());
		}

		template <typename T> void NkWorld::AddDeferred(NkEntityId id, const T &value) noexcept {
			mDeferredOps.PushBack({DeferredOp::Kind::Add, id, NkIdOf<T>(),
								   NkFunction<void(NkWorld &, NkEntityId)>(
									   [value](NkWorld &w, NkEntityId eid) { w.Add<T>(eid, value); })});
		}

		template <typename T> void NkWorld::RemoveDeferred(NkEntityId id) noexcept {
			mDeferredOps.PushBack(
				{DeferredOp::Kind::Remove, id, NkIdOf<T>(),
				 NkFunction<void(NkWorld &, NkEntityId)>([](NkWorld &w, NkEntityId eid) { w.Remove<T>(eid); })});
		}

		template <typename T> T *NkWorld::Get(NkEntityId id) noexcept {
		...
		template <typename T> const T *NkWorld::Get(NkEntityId id) const noexcept {
			const NkEntityRecord *rec = mEntityIndex.GetRecord(id);
			if (!rec || rec->archetypeId == kInvalidArchetypeId) {
				return nullptr;
			}
		...
		template <typename T> T &NkWorld::GetRef(NkEntityId id) noexcept {
			T *ptr = Get<T>(id);
			NKECS_ASSERT(ptr != nullptr);
			return *ptr;
		}

		template <typename T> bool NkWorld::Has(NkEntityId id) const noexcept {
			const NkEntityRecord *rec = mEntityIndex.GetRecord(id);
			if (!rec || rec->archetypeId == kInvalidArchetypeId) {
				return false;
			}
		...
		template <typename T> void NkWorld::Set(NkEntityId id, const T &value) noexcept {
			if (T *ptr = Get<T>(id)) {
				*ptr = value;
			}
		}

		template <typename... Ts> NkQueryResult<Ts...> NkWorld::Query() noexcept {
			const NkComponentMask required = detail::MakeMask<std::remove_const_t<Ts>...>();
			return NkQueryResult<Ts...>(mGraph, mEntityIndex, required, NkComponentMask{}, NkComponentMask{});
		}

		template <typename T, typename Fn> NkGameplayEventBus::SubscriptionId NkWorld::Subscribe(Fn &&fn) noexcept {
			return mEventBus.Subscribe<T>(traits::NkForward<Fn>(fn));
		}

		template <typename T> void NkWorld::Unsubscribe(NkGameplayEventBus::SubscriptionId id) noexcept {
			mEventBus.Unsubscribe<T>(id);
		}

		template <typename T> void NkWorld::Emit(const T &event) noexcept {
			mEventBus.Emit(event);
		}

		template <typename T> void NkWorld::Emit(T &&event) noexcept {
			mEventBus.Emit(traits::NkMove(event));
		}

		template <typename T> void NkWorld::QueueEvent(const T &event) noexcept {
		...
		// =====================================================================
		template <typename T, typename TVal> T &NkWorld::AddImpl(NkEntityId id, TVal &&value) noexcept {
			...
			const NkArchetypeId newArchId = (oldArchId == kInvalidArchetypeId) ? mGraph.GetOrCreate(SingleMask(cid))

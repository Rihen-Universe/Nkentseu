...
namespace nkentseu {
	namespace ecs {
		...
		class NkScheduler {
			public:
				explicit NkScheduler(uint32 jobThreads = 0) noexcept : mJobPool(jobThreads) {
				}

				// ── Enregistrement ────────────────────────────────────────────────────────

				// Enregistre un système typé (instanciation automatique)
				template <typename T, typename... Args> T &AddSystem(Args &&...args) noexcept {
					static_assert(std::is_base_of_v<NkSystem, T>, "T must derive from NkSystem");
				...
				NkLambdaSystem &AddLambda(NkSystemDesc desc, std::function<void(NkWorld &, float32)> fn) noexcept {
				...
				template <typename T> void SetEnabled(bool enabled) noexcept {
				...
				void RebuildDAG() noexcept {
					...
					for (uint32 i = 0; i < mSystems.size(); ++i) {
						for (uint32 j = 0; j < i; ++j) {
							if (mSystems[i].desc.group != mSystems[j].desc.group)
								continue;
					...
					while (remaining > 0) {
						...
						for (uint32 idx : sorted) {
							...
							for (uint32 dep : mSystems[idx].dependsOn) {
								if (std::find(sorted.begin(), sorted.end(), dep) != sorted.end() && !done[dep]) {
									ready = false;
									break;
								}
							}
						...
						if (wave.empty())
							break; // cycle de dépendances → abort
						...
						if (wave.size() == 1 || !mSystems[wave[0]].desc.runInParallel) {
							...
							for (uint32 idx : wave) {
								mJobPool.Submit([&, idx] { mSystems[idx].system->Execute(world, dt); });
							}
							mJobPool.WaitAll();
							for (uint32 idx : wave) {
								done[idx] = true;

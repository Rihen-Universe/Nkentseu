...
#define NK_SYSTEM(Type) /* no-op */
		...
		class NkLambdaSystem final : public NkSystem {
			public:
				...
				NkLambdaSystem(NkSystemDesc desc, ExecFn fn) noexcept : mDesc(std::move(desc)), mFn(std::move(fn)) {
				}

...
#define NK_EVENT(Type) /* no-op */
		 ...
		 * @endcode
		 */
		template <typename T> class NkEventChannel final : public IEventQueue {
			public:
		...
		class NkGameplayEventBus {
			public:
				 ...
				 */
				template <typename T, typename Fn> SubscriptionId Subscribe(Fn &&fn) noexcept;
				 ...
				 */
				template <typename T> void Unsubscribe(SubscriptionId id) noexcept;
				 ...
				 */
				template <typename T> void Queue(const T &event) noexcept;
				 ...
				 * @param event Événement à queue (déplacé pour éviter la copie).
				 */
				template <typename T> void Queue(T &&event) noexcept;
			...
			private:
				 ...
				 */
				template <typename T> NkEventChannel<T> &GetOrCreate() noexcept;
				 ...
				 */
				template <typename T> NkEventChannel<T> *Find() noexcept;
		};
		...
		template <typename T>
		typename NkEventChannel<T>::HandleId NkEventChannel<T>::Subscribe(Handler handler) noexcept {
		...
		template <typename T> void NkEventChannel<T>::Unsubscribe(HandleId id) noexcept {
			threading::NkScopedLockMutex lock(mMutex);
		...
		template <typename T> void NkEventChannel<T>::Emit(const T &event) noexcept {
			threading::NkScopedLockMutex lock(mMutex);
			for (nk_usize i = 0; i < mHandlers.Size(); ++i) {
				auto &e = mHandlers[i];
				if (e.active && e.fn) {
		...
		template <typename T> void NkEventChannel<T>::Queue(const T &event) noexcept {
			threading::NkScopedLockMutex lock(mMutex);
			mQueue.PushBack(event);
		}

		template <typename T> void NkEventChannel<T>::Queue(T &&event) noexcept {
			threading::NkScopedLockMutex lock(mMutex);
			mQueue.PushBack(static_cast<T &&>(event));
		}

		// ── NkEventChannel<T> : Drain (exécution différée + purge) ────────
		template <typename T> void NkEventChannel<T>::Drain() noexcept {
			threading::NkScopedLockMutex lock(mMutex);
		...
		template <typename T> void NkEventChannel<T>::Clear() noexcept {
			threading::NkScopedLockMutex lock(mMutex);
			mQueue.Clear();
		}

		template <typename T> bool NkEventChannel<T>::Empty() const noexcept {
			threading::NkScopedLockMutex lock(mMutex);
			return mQueue.IsEmpty();
		}

		template <typename T> uint32 NkEventChannel<T>::HandlerCount() const noexcept {
			threading::NkScopedLockMutex lock(mMutex);
		...
		typename NkGameplayEventBus::SubscriptionId NkGameplayEventBus::Subscribe(Fn &&fn) noexcept {
			threading::NkScopedLockMutex lock(mMutex);
			...
			const uint64 tid = static_cast<uint64>(NkTypeRegistry::Global().IdOf<T>());
			return (tid << 32u) | static_cast<uint64>(hid);
		}

		template <typename T> void NkGameplayEventBus::Unsubscribe(SubscriptionId id) noexcept {
			threading::NkScopedLockMutex lock(mMutex);
		...
		template <typename T> void NkGameplayEventBus::Emit(const T &event) noexcept {
			threading::NkScopedLockMutex lock(mMutex);
		...
		template <typename T> void NkGameplayEventBus::Emit(T &&event) noexcept {
			threading::NkScopedLockMutex lock(mMutex);
			if (NkEventChannel<T> *ch = Find<T>()) {
		...
		template <typename T> void NkGameplayEventBus::Queue(const T &event) noexcept {
			threading::NkScopedLockMutex lock(mMutex);
			GetOrCreate<T>().Queue(event);
		}

		template <typename T> void NkGameplayEventBus::Queue(T &&event) noexcept {
			threading::NkScopedLockMutex lock(mMutex);
			GetOrCreate<T>().Queue(traits::NkForward<T>(event));
		}

		// ── NkGameplayEventBus : GetOrCreate / Find (privé) ──────────────
		template <typename T> NkEventChannel<T> &NkGameplayEventBus::GetOrCreate() noexcept {
			...
			static_assert(sizeof(NkEventChannel<T>) <= sizeof(e.storage),
		...
		template <typename T> NkEventChannel<T> *NkGameplayEventBus::Find() noexcept {
			const NkComponentId tid = NkTypeRegistry::Global().IdOf<T>();
			for (uint32 i = 0; i < mCount; ++i) {
				if (mEntries[i].typeId == tid) {
					return static_cast<NkEventChannel<T> *>(mEntries[i].queue);

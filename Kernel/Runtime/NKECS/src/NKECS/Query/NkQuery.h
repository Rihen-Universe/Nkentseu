...
namespace nkentseu {
	namespace ecs {
		...
		namespace detail {
			...
			template <typename... Ts> NkComponentMask MakeMask() noexcept {
			...
			template <typename T> T *GetPoolPtr(NkArchetype *arch, uint32 /*row*/) noexcept {
		...
		template <typename... Ts> class NkQueryResult {
			public:
				// -----------------------------------------------------------------
				...
				NkQueryResult(NkArchetypeGraph &graph, NkEntityIndex &index, NkComponentMask required,
							  NkComponentMask withFilter, NkComponentMask withoutFilter) noexcept
					: mGraph(graph), mIndex(index), mRequired(required), mWithFilter(withFilter),
				...
				template <typename T> NkQueryResult &With() noexcept {
				...
				template <typename T> NkQueryResult &Without() noexcept {
				...
				template <typename Fn> void ForEach(Fn &&fn) noexcept {
					mGraph.ForEachMatching(mFinalInclude, mWithoutFilter, [&](NkArchetype *arch) noexcept {
						...
						auto ptrs = std::make_tuple(detail::GetPoolPtr<Ts>(arch, 0)...);
						...
						for (uint32 row = 0; row < count; ++row) {
							...
							CallFn(fn, entities[row], ptrs, row, std::index_sequence_for<Ts...>{});
						}
					});
				}
				...
				template <typename Fn> void ForEachBatch(Fn &&fn) noexcept {
					mGraph.ForEachMatching(mFinalInclude, mWithoutFilter, [&](NkArchetype *arch) noexcept {
						const uint32 count = arch->Count();
						if (count == 0) {
							return;
				...
				[[nodiscard]] uint32 Count() const noexcept {
					...
					const_cast<NkArchetypeGraph &>(mGraph).ForEachMatching(
						mFinalInclude, mWithoutFilter, [&](NkArchetype *arch) noexcept { total += arch->Count(); });
					return total;
				}
				...
				[[nodiscard]] NkEntityId First() const noexcept {
					...
					const_cast<NkArchetypeGraph &>(mGraph).ForEachMatching(
						mFinalInclude, mWithoutFilter, [&](NkArchetype *arch) noexcept {
			...
			private:
				...
				static void CallFn(Fn &fn, NkEntityId id, Ptrs &ptrs, uint32 row, std::index_sequence<Is...>) noexcept {
				...
				static void CallBatch(Fn &fn, NkArchetype *arch, uint32 count, std::index_sequence<Is...>) noexcept {
					...
					fn(count, arch->Entities().data, detail::GetPoolPtr<std::tuple_element_t<Is, TPack>>(arch, 0)...);
				}

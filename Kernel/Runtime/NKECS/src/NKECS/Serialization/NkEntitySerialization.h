	...
	namespace ecs {
		namespace serialization {
			...
			NKECS_INLINE void *AddComponentByIdRaw(NkWorld &world, NkEntityId id, NkComponentId cid) noexcept {
				...
				const NkArchetypeId newArchId =
					(oldArchId == kInvalidArchetypeId) ? graph.GetOrCreate(single) : graph.AddComponent(oldArchId, cid);
			 ...
			 */
			NKECS_INLINE nk_bool DeserializeEntity(NkWorld &world, NkEntityId id,

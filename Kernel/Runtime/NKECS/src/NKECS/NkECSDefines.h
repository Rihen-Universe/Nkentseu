...
#else
#define NKECS_UNREACHABLE()                                                                                            \
	do {                                                                                                               \
	} while (0) // Fallback (aucun effet)
#endif
...
#define NKECS_ASSERT(cond)                                                                                             \
	do {                                                                                                               \
		...
		template <typename T> struct NkSpan {
				...
				constexpr NkSpan(T *d, usize s) noexcept : data(d), size(s) {
				}

				// Constructeur à partir d'un tableau C-style (déduction de la taille).
				template <usize N> constexpr NkSpan(T (&arr)[N]) noexcept : data(arr), size(N) {
				}

	...
	namespace ecs {
		...
		class NkArchetype {
			public:
				...
				explicit NkArchetype(NkArchetypeId id, const NkComponentMask &mask) noexcept : mId(id), mMask(mask) {
				...
				// -----------------------------------------------------------------
				[[nodiscard]] NkArchetypeId Id() const noexcept {
					return mId;
				}

				[[nodiscard]] const NkComponentMask &Mask() const noexcept {
					return mMask;
				}
				...
				template <typename Fn> void ForEachRow(Fn &&fn) const noexcept {
				...
				template <typename T> [[nodiscard]] T *GetComponent(uint32 row) noexcept {
				...
				// Version const du précédent.
				template <typename T> [[nodiscard]] const T *GetComponent(uint32 row) const noexcept {
					const NkComponentId cid = NkIdOf<T>();
				...
				template <typename T> [[nodiscard]] T *ComponentData() noexcept {
					const NkComponentId cid = NkIdOf<T>();
					NkComponentPool *pool = GetPool(cid);
					if (pool == nullptr) {
						return nullptr;
				...
				template <typename T> void SetComponent(uint32 row, const T &value) noexcept {
			...
			private:
				...
				void GrowEntityArray() noexcept {
					const uint32 newCap = (mEntityCapacity < 8u) ? 8u : mEntityCapacity + mEntityCapacity / 2u;
		...
		class NkEntityIndex {
			public:
				...
				NkEntityIndex(NkEntityIndex &&other) noexcept
					: mEntries(other.mEntries), mNumEntries(other.mNumEntries), mCapacity(other.mCapacity),
			...
			private:
				...
				void GrowIfNeeded() noexcept {
					...
					const uint32 newCap = (mCapacity < 64u) ? 64u : mCapacity + mCapacity / 2u;

	...
	namespace ecs {
		...
		class NkComponentPool {
			public:
				...
				explicit NkComponentPool(const ComponentMeta *meta) noexcept : mMeta(meta) {
				...
				NkComponentPool(NkComponentPool &&o) noexcept
					: mMeta(o.mMeta), mData(o.mData), mSize(o.mSize), mCapacity(o.mCapacity) {
				...
				template <typename T> [[nodiscard]] T *AtTyped(uint32 index) const noexcept {
				...
				template <typename T> [[nodiscard]] T *Data() const noexcept {
				...
				void Shrink() noexcept {
					if (!mMeta || mMeta->isZeroSize || mSize == mCapacity || mData == nullptr)
						return;
					if (mSize == 0) {
						nkentseu::memory::NkFree(mData);
						mData = nullptr;
					...
					void *newData = nkentseu::memory::NkAlloc(static_cast<nk_size>(bytes), nullptr,
															  static_cast<nk_size>(mMeta->align));

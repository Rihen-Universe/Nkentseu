...
namespace nkentseu {
	...
	namespace ecs {
		...
		namespace detail {
			...
			template <typename T> void DefaultConstruct(void *ptr, uint32 count) noexcept {
			...
			template <typename T> void Destruct(void *ptr, uint32 count) noexcept {
			...
			template <typename T> void MoveConstruct(void *dst, void *src, uint32 count) noexcept {
				if constexpr (!std::is_empty_v<T>) {
			...
			template <typename T> void CopyConstruct(void *dst, const void *src, uint32 count) noexcept {
				if constexpr (!std::is_empty_v<T>) {
					T *d = static_cast<T *>(dst);
			...
			template <typename T> void SwapAt(void *a, void *b) noexcept {
				if constexpr (!std::is_empty_v<T>) {
					using std::swap;
			...
			template <typename T> struct ComponentIdStorage {
		...
		class NkTypeRegistry {
			public:
				...
				// ---------------------------------------------------------------------
				template <typename T> NkComponentId Register() noexcept {
				...
				// ---------------------------------------------------------------------
				void SetReflection(NkComponentId id, const ::nkentseu::reflection::NkClass *reflectClass,
				...
				// ---------------------------------------------------------------------
				template <typename T> [[nodiscard]] NkComponentId IdOf() noexcept {
					return Register<T>();
				}
				...
				// ---------------------------------------------------------------------
				template <typename T> [[nodiscard]] const char *TypeName() noexcept {
				...
				// ---------------------------------------------------------------------
				template <typename T> [[nodiscard]] NkComponentId IdOfConst() const noexcept {
					using TRaw = std::remove_const_t<std::remove_reference_t<T>>;
		...
		template <typename T> [[nodiscard]] NKECS_INLINE NkComponentId NkIdOf() noexcept {
			return NkTypeRegistry::Global().IdOf<T>();
		}

		// Retourne un pointeur vers les métadonnées associées au type T.
		template <typename T> [[nodiscard]] NKECS_INLINE const ComponentMeta *NkMetaOf() noexcept {
		...
		struct NkComponentMask {
				...
				[[nodiscard]] uint64 Hash() const noexcept {
					return detail::FNV1aBytes(reinterpret_cast<const uint8 *>(words), sizeof(words));
				}
				...
				template <typename Fn> void ForEach(Fn &&fn) const noexcept {
	...
	namespace {                                                                                                        \
		...
		struct _NkAutoReg_##Type {                                                                                     \
				_NkAutoReg_##Type() noexcept {                                                                         \
					...
					::nkentseu::ecs::NkTypeRegistry::Global().SetName(::nkentseu::ecs::NkIdOf<Type>(), #Type);         \
				}                                                                                                      \
		};                                                                                                             \

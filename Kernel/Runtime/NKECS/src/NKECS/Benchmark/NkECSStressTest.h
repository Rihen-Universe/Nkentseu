...
namespace nkentseu {
	namespace ecs {
		namespace benchmark {
			...
			// ============================================================================
			struct BenchmarkCompA {
					float32 v1 = 0.f, v2 = 0.f;
			};

			struct BenchmarkCompB {
					int32 v = 0;
			};

			struct BenchmarkCompC {
					bool flag = true;
			};

			struct BenchmarkCompD {
					float64 v = 0.0;
			};

			struct BenchmarkCompE {
					NkVec3 pos = NkVec3::Zero();
			};
			...
			inline std::vector<std::unique_ptr<NkSystem>> GenerateTestSystems(uint32 count) {
				...
				for (uint32 i = 0; i < count; ++i) {
					...
					if (i % 4 == 0)
						desc.Writes<BenchmarkCompA>().Reads<BenchmarkCompB>();
					else if (i % 4 == 1)
						desc.Reads<BenchmarkCompA>().Writes<BenchmarkCompC>();
					else if (i % 4 == 2)
						desc.Reads<BenchmarkCompB>().Writes<BenchmarkCompD>();
					else
						desc.Reads<BenchmarkCompC>().Reads<BenchmarkCompD>();
			...
			// ============================================================================
			[[nodiscard]] inline StressTestResult RunStressTest(uint32 entityCount = 10000,
				...
				uint32 processed = 0;
				world.Query<NkTransform, NkTag>().ForEach(
					[&processed](NkEntityId, NkTransform &, NkTag &) { ++processed; });
				...
				world.Query<NkTransform, NkTag, BenchmarkCompA>().ForEach(
					[&](NkEntityId id, const NkTransform &t, const NkTag &tag, const BenchmarkCompA &a) {
						if (sampleCount-- == 0)
							return;
				...
				t0 = std::chrono::high_resolution_clock::now();
				serialization::NkSerializeArchive(archive, serialization::NkSerializationFormat::NK_JSON, jsonOutput);
				t1 = std::chrono::high_resolution_clock::now();
				...
				t0 = std::chrono::high_resolution_clock::now();
				serialization::native::NkNativeWriter::WriteArchiveCompressed(archive, nativeOutput, true);
				t1 = std::chrono::high_resolution_clock::now();
				res.nativeSizeBytes = static_cast<double>(nativeOutput.Size());
				res.compressionRatio = res.jsonSizeBytes > 0 ? res.nativeSizeBytes / res.jsonSizeBytes : 0.0;

				// ── 4. Benchmark de désérialisation ──────────────────────────────────
				// JSON
				NkArchive jsonArchive;
				t0 = std::chrono::high_resolution_clock::now();
				serialization::NkDeserializeArchive(jsonOutput.c_str(), serialization::NkSerializationFormat::NK_JSON,
													jsonArchive);
				t1 = std::chrono::high_resolution_clock::now();
				...
				t0 = std::chrono::high_resolution_clock::now();
				serialization::native::NkNativeReader::ReadArchive(nativeOutput.Data(), nativeOutput.Size(),
																   nativeArchive);
				t1 = std::chrono::high_resolution_clock::now();
				// On ne met pas à jour deserializeTimeMs car on garde la valeur JSON

				// ── 5. Validation des résultats ──────────────────────────────────────
				res.success = (res.entityCount == entityCount) && (res.queryTimeMs > 0) && (res.serializeTimeMs > 0) &&
				...
				printf("⏱ Sérialisation JSON (ms) : %.3f (%.2f MB)\n", res.serializeTimeMs,
					   res.jsonSizeBytes / 1048576.0);
				printf("⏱ Sérialisation NkNative (ms) : %.3f (%.2f MB)\n", res.serializeTimeMs,
					   res.nativeSizeBytes / 1048576.0);
			...
			// ============================================================================
			[[nodiscard]] inline bool ExportBenchmarkResultsJSON(const StressTestResult &res,

		...
		namespace benchmark {
			...
			inline std::vector<std::unique_ptr<NkSystem>> GenerateTestSystems(uint32 count) {
				...
				for (uint32 i = 0; i < count; ++i) {
					...
					if (i >= 5)
						desc.After<NkLambdaSystem>(); // Force l'ordre partiel
			...
			inline BenchmarkResult RunProductionTest() {
				...
				for (uint32 i = 0; i < kEntityCount; ++i) {
					...
					if (i % 2 == 0)
						world.Add<BenchmarkCompB>(entities[i]);
					if (i % 3 == 0)
						world.Add<BenchmarkCompC>(entities[i]);
					if (i % 5 == 0)
						world.Add<BenchmarkCompD>(entities[i]);
				}
				...
				uint32 processed = 0;
				world.Query<BenchmarkCompA, BenchmarkCompC>().ForEach(
					[&processed](NkEntityId, BenchmarkCompA &, BenchmarkCompC &) { ++processed; });

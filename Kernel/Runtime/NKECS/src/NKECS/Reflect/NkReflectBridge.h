...
namespace nkentseu {
	namespace ecs {
		namespace reflect {
			 ...
			 */
			NKECS_INLINE ::nkentseu::reflection::NkTypeCategory FieldTypeToCategory(NkFieldType t) noexcept {
				using C = ::nkentseu::reflection::NkTypeCategory;
				switch (t) {
					case NkFieldType::Bool:
						return C::NK_BOOL;
					case NkFieldType::Int8:
						return C::NK_INT8;
					case NkFieldType::Int16:
						return C::NK_INT16;
					case NkFieldType::Int32:
						return C::NK_INT32;
					case NkFieldType::Int64:
						return C::NK_INT64;
					case NkFieldType::UInt8:
						return C::NK_UINT8;
					case NkFieldType::UInt16:
						return C::NK_UINT16;
					case NkFieldType::UInt32:
						return C::NK_UINT32;
					case NkFieldType::UInt64:
						return C::NK_UINT64;
					case NkFieldType::Float32:
						return C::NK_FLOAT32;
					case NkFieldType::Float64:
						return C::NK_FLOAT64;
					case NkFieldType::String:
						return C::NK_STRING;

					// Identifiants ECS : entiers non signes 64/32 bits.
					case NkFieldType::EntityId:
						return C::NK_UINT64;
					case NkFieldType::ComponentId:
						return C::NK_UINT32;
					case NkFieldType::ArchetypeId:
						return C::NK_UINT32;

					// Enum : traite comme entier (le serializer ecrit la valeur).
					case NkFieldType::Enum:
					case NkFieldType::Flags:
						return C::NK_ENUM;
					...
					case NkFieldType::Object:
						return C::NK_STRUCT;

					case NkFieldType::Array:
						return C::NK_ARRAY;
			...
			NKECS_INLINE bool FieldTypeIsScalar(NkFieldType t) noexcept {
				...
				return (c >= C::NK_BOOL && c <= C::NK_FLOAT64) || c == C::NK_STRING || c == C::NK_ENUM;
			}
			...
			template <typename T> struct NkComponentReflectionStore {
					...
					alignas(
						::nkentseu::reflection::NkType) unsigned char typeBuf[kMaxFields *
																			  sizeof(::nkentseu::reflection::NkType)];
					alignas(::nkentseu::reflection::NkProperty) unsigned char propBuf
						[kMaxFields * sizeof(::nkentseu::reflection::NkProperty)];
					alignas(::nkentseu::reflection::NkClass) unsigned char classBuf[sizeof(
						::nkentseu::reflection::NkClass)];
					...
					::nkentseu::reflection::NkType *TypeAt(nk_usize i) {
						return reinterpret_cast<::nkentseu::reflection::NkType *>(typeBuf) + i;
					}

					::nkentseu::reflection::NkProperty *PropAt(nk_usize i) {
						return reinterpret_cast<::nkentseu::reflection::NkProperty *>(propBuf) + i;
					}
			};

			template <typename T> NKECS_INLINE NkComponentReflectionStore<T> &NkComponentStore() {
			...
			template <typename T>
			NKECS_INLINE void NkComponentSerializeHook(const void *comp, ::nkentseu::NkArchive &ar) {
				const ::nkentseu::reflection::NkClass *cls = NkComponentStore<T>().cls;
				if (cls) {
			...
			template <typename T>
			NKECS_INLINE void NkComponentDeserializeHook(void *comp, const ::nkentseu::NkArchive &ar) {
				const ::nkentseu::reflection::NkClass *cls = NkComponentStore<T>().cls;
				if (cls) {
			 ...
			 */
			template <typename T> const ::nkentseu::reflection::NkClass *NkRegisterComponentReflection() {
				...
				NkClass *cls = new (store.classBuf) NkClass(info->name, sizeof(T), selfType);
				selfType.SetClass(cls); // auto-link pour la recursion d'objet

				const nk_usize n = (info->fieldCount < NkComponentReflectionStore<T>::kMaxFields)
									   ? info->fieldCount
				...
				for (nk_usize i = 0; i < n; ++i) {
					...
					NkType *fieldType = new (store.TypeAt(i)) NkType(f.name, // nom du champ comme nom de type proxy
					...
					NkProperty *prop =
						new (store.PropAt(i)) NkProperty(f.name, *fieldType, static_cast<nk_usize>(f.offset), 0u);
					...
					if (f.tooltip) {
						prop->SetTooltip(f.tooltip);
					}
					if (f.category) {
						prop->SetCategory(f.category);
					}
					if (f.displayName) {
						prop->SetDisplayName(f.displayName);
					}
				...
				NkTypeRegistry::Global().SetReflection(id, cls, &NkComponentSerializeHook<T>,

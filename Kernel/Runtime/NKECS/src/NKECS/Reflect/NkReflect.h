...
#define NK_FIELD(FieldName)                                                                                            \
	{#FieldName,                                                                                                       \
	 ...
	 1,                                                                                                                \
	 NkMeta_Visible | NkMeta_EditAnywhere | NkMeta_Serialize,                                                          \
	 nullptr,                                                                                                          \
	 nullptr,                                                                                                          \
	 nullptr,                                                                                                          \
	 0.f,                                                                                                              \
...
#define NK_ARRAY(FieldName, Count)                                                                                     \
	{#FieldName,                                                                                                       \
	 ...
	 Count,                                                                                                            \
	 NkMeta_Visible | NkMeta_EditFixedSize | NkMeta_Serialize,                                                         \
	 nullptr,                                                                                                          \
	 nullptr,                                                                                                          \
	 nullptr,                                                                                                          \
	 0.f,                                                                                                              \
...
#define NK_REFLECT_END(Type)                                                                                           \
	...
	info.fields = _fields;                                                                                             \
	info.fieldCount = static_cast<uint32>(sizeof(_fields) / sizeof(_fields[0]));                                       \
...
#define NK_PROPERTY(FieldName, ...)                                                                                    \
	NK_FIELD_META(FieldName, NK_META_PACK_FLAGS(__VA_ARGS__), NK_META_EXTRACT_CATEGORY(__VA_ARGS__),                   \
				  NK_META_EXTRACT_TOOLTIP(__VA_ARGS__), NK_META_EXTRACT_MIN(__VA_ARGS__),                              \
...
#define NK_META_PACK_FLAGS(...)                                                                                        \
	(NK_META_FLAG_IF(EditAnywhere, __VA_ARGS__) | NK_META_FLAG_IF(BlueprintReadWrite, __VA_ARGS__) |                   \
	 NK_META_FLAG_IF(Replicated, __VA_ARGS__) | NK_META_FLAG_IF(Range, __VA_ARGS__) | 0)

// Helper pour conditionnellement ajouter un flag
#define NK_META_FLAG_IF(Flag, ...) (NK_META_HAS_ARG(Flag, __VA_ARGS__) ? static_cast<uint64>(NkMeta_##Flag) : 0ULL)

// Vérifie si un argument nommé est présent (simplifié)
#define NK_META_HAS_ARG(Arg, ...) NK_META_CHECK_HELPER(NK_META_CAT(NK_META_ARG_, Arg), __VA_ARGS__, 0, 0)

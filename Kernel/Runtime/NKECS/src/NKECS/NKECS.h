...
// Les projets utilisant l'ECS peuvent simplement inclure ce fichier pour accéder à l'ensemble des fonctionnalités de
// l'ECS.
// =============================================================================
// Note : pour les projets qui souhaitent une compilation plus rapide, il est possible d'inclure uniquement les
...
static_assert(sizeof(nkentseu::ecs::NkEntityId) == 8,
			  "NkEntityId doit occuper 8 octets (64 bits) pour assurer l'unicité et les flags.");

// Vérifie que le masque de composants est assez grand pour représenter tous les types possibles
static_assert(sizeof(nkentseu::ecs::NkComponentMask) == nkentseu::ecs::kMaxComponentTypes / 8,
			  "La taille du masque de composants ne correspond pas à kMaxComponentTypes.");
...
namespace nkentseu {
	...
	template <typename T> NkComponentId NkIdOf() noexcept {

...
class MainMenuScript : public NkSceneScript {
	public:
		...
		void OnBeginPlay() noexcept override {
			...
			if (auto *scene = GetScene()) {
				...
				ConfigureButton(mButtonStart, "JOUER", [this]() { OnStartGameClicked(); });

				// Bouton "Options" — ouvre le sous-menu des paramètres
				mButtonOptions = scene->SpawnNode("UI_Button_Options");
				ConfigureButton(mButtonOptions, "OPTIONS", [this]() { OnOptionsClicked(); });

				// Bouton "Quitter" — ferme l'application
				mButtonQuit = scene->SpawnNode("UI_Button_Quit");
				ConfigureButton(mButtonQuit, "QUITTER", [this]() { OnQuitClicked(); });
		...
		void OnTick(float dt) noexcept override {
			...
			if (mNavigationTimer >= NAV_COOLDOWN) {
				...
				if (InputSystem::IsPressed(NkKey::ArrowUp) || InputSystem::IsPressed(NkKey::Gamepad_DPadUp)) {
					mSelectedIndex = (mSelectedIndex - 1 + mButtonCount) % mButtonCount;
					UpdateSelectionHighlight();
					mNavigationTimer = 0.f;
				}
				// Flèche bas / D-pad bas : descendre dans la liste
				else if (InputSystem::IsPressed(NkKey::ArrowDown) || InputSystem::IsPressed(NkKey::Gamepad_DPadDown)) {
					mSelectedIndex = (mSelectedIndex + 1) % mButtonCount;
					UpdateSelectionHighlight();
					mNavigationTimer = 0.f;
				}
				// Entrée / Bouton A : valider la sélection
				else if (InputSystem::IsPressed(NkKey::Enter) || InputSystem::IsPressed(NkKey::Gamepad_A)) {
	...
	private:
		 ...
		 */
		void ConfigureButton(NkEntityId buttonId, const char *text, NkFunction<void()> onClick) {
...
class GameLevelScript : public NkSceneScript {
	public:
		...
		explicit GameLevelScript(nk_int32 difficulty = 1, nk_int32 levelIndex = 0) noexcept
			: mDifficulty(difficulty), mLevelIndex(levelIndex) {
		}
	...
	private:
		...
		[[nodiscard]] NkGameObject SpawnPlayerAtCheckpoint(const char *checkpointName) {
			if (auto *scene = GetScene()) {
				...
				if (spawnPoint.IsValid()) {
					...
					return scene->SpawnPrefab(GetPlayerPrefab(), spawnPoint.GetPosition(), "Player");
				}
			}
		...
		void SpawnEnemyWaves(nk_int32 count) {
			if (auto *scene = GetScene()) {
				for (nk_int32 i = 0; i < count; ++i) {
					...
					auto enemy =
						scene->SpawnPrefab(GetEnemyPrefabForLevel(), spawnPos, NkString::Format("Enemy_%d", i).CStr());
					if (enemy.IsValid()) {
		...
		void HandlePlayerInput(float dt) {
			...
			NkVec2f inputAxis = {InputSystem::GetAxis(NkAxis::Horizontal), InputSystem::GetAxis(NkAxis::Vertical)};

			if (inputAxis.LengthSquared() > 0.01f) {
				// Déplacement du joueur selon l'input
				NkVec3f moveDir = {inputAxis.x, 0.f, inputAxis.y};
				mPlayer.SetPosition(mPlayer.GetPosition() + moveDir * 5.f * dt // 5 units/sec
				);
			}
		...
		void UpdateProjectilesPhysics(float fixedDt) {
			...
			if (auto *world = GetWorld()) {
				world->Query<ProjectileComponent, NkTransform>().ForEach(
					[=](NkEntityId id, ProjectileComponent &proj, NkTransform &transform) {
		...
		void CheckPlayerEnemyCollisions() {
			...
			if (auto *world = GetWorld()) {
				world->Query<EnemyComponent, HitboxComponent, NkTransform>().ForEach(
					[this](NkEntityId, EnemyComponent &, HitboxComponent &hitbox, NkTransform &enemyTransform) {
		...
		void CheckVictoryConditions() {
			...
			if (auto *world = GetWorld()) {
				...
				world->Query<EnemyComponent, HealthComponent>().ForEach(
					[&](NkEntityId, EnemyComponent &, HealthComponent &health) {
						if (health.value > 0) {
							++aliveEnemies;
						}
					});
		...
		[[nodiscard]] NkVec3f GetRandomEnemySpawnPosition() const noexcept {
			...
			return {NkRandom::Range(-10.f, 10.f), 0.f, NkRandom::Range(-10.f, 10.f)};
		}
...
void Example_Transitions(NkSceneManager &mgr) {
	 ...
	 */
	mgr.LoadScene("Level_3", NkSceneTransition::WithLoading("LoadingScreen", 0.3f));
...
class EditorLayer /* : public Layer */ {
	public:
		...
		EditorLayer(NkWorld &world, NkScheduler &scheduler)
			: mWorld(world), mScheduler(scheduler), mSceneManager(world), mPlaying(false),
		...
		void OnPlay() {
			...
			if (mSceneManager.LoadScene(mCurrentSceneName, NkSceneTransition::Instant())) {
				mPlaying = true;
...
*@endcode * / void Example_SafePointerChecks() {
...
*@endcode * / void Example_ResourceCleanup() {
	// Implémentation illustrative — voir commentaire ci-dessus
}

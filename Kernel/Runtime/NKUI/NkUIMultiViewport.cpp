...
namespace nkentseu {
	namespace nkui {
		...
		static void AddUtf8ToInput(NkUIInputState &inp, const char *utf8) {
			...
			for (int32 i = 0; utf8[i];) {
				...
				if (c0 < 0x80u) {
					...
					cp = ((c0 & 0x1F) << 6) | (uint8(utf8[i + 1]) & 0x3F);
					n = 2;
				} else if ((c0 & 0xF0u) == 0xE0u && utf8[i + 1] && utf8[i + 2]) {
					cp = ((c0 & 0xF) << 12) | ((uint8(utf8[i + 1]) & 0x3F) << 6) | (uint8(utf8[i + 2]) & 0x3F);
					n = 3;
				}
				inp.AddInputChar(cp);
				i += n;
		...
		bool NkUIMultiViewportManager::Init(NkIDevice *device, NkRenderPassHandle mainRenderPass, NkGraphicsApi api,
											NkWindow *mainWindow, int32 mainViewW, int32 mainViewH) noexcept {
			mDevice = device;
			mApi = api;
		...
		void NkUIMultiViewportManager::BeginFrame(NkUIContext &ctx, NkUIWindowManager &wm, float32 dt, int32 mainViewW,
				...
				for (int32 i = 0; i < mCount; ++i) {
					if (mViewports[i].alive) {
						...
						if (ws) {
							...
							if (ws->pos.y < 0.f)
								ws->pos.y = 20.f;
							if (ws->pos.x > mainViewW)
								ws->pos.x = static_cast<float32>(mainViewW - 100);
							if (ws->pos.y > mainViewH)
								ws->pos.y = static_cast<float32>(mainViewH - 100);
						}
						DestroyViewport(i);
					}
				}
			...
			for (int32 i = 0; i < mCount; ++i) {
				...
				if (mViewports[i].alive && !mViewports[i].osWindow.IsOpen()) {
					shouldDestroy = true;
					if (ws)
						ws->isOpen = false; // propage la fermeture
				}
			...
			for (int32 i = 0; i < wm.numWindows; ++i) {
				...
				if (!ws.isOpen || ws.isDocked)
					continue;
				if (FindViewport(ws.id) >= 0)
					continue; // déjà géré
			...
			for (int32 i = 0; i < mCount; ++i) {
				...
				events.AddEventCallback<NkMouseMoveEvent>(
					[&vp](NkMouseMoveEvent *e) {
						vp.localInput.SetMousePos(static_cast<float32>(e->GetX()), static_cast<float32>(e->GetY()));
					},
					osWinId);

				// Boutons souris
				events.AddEventCallback<NkMouseButtonPressEvent>(
					[&vp](NkMouseButtonPressEvent *e) {
						vp.localInput.SetMousePos(static_cast<float32>(e->GetX()), static_cast<float32>(e->GetY()));
						const int32 btn = ToUiBtn(e->GetButton());
						if (btn >= 0 && btn < 5)
							vp.localInput.SetMouseButton(btn, true);
					},
					osWinId);

				events.AddEventCallback<NkMouseButtonReleaseEvent>(
					[&vp](NkMouseButtonReleaseEvent *e) {
						vp.localInput.SetMousePos(static_cast<float32>(e->GetX()), static_cast<float32>(e->GetY()));
						const int32 btn = ToUiBtn(e->GetButton());
						if (btn >= 0 && btn < 5)
							vp.localInput.SetMouseButton(btn, false);
					},
					osWinId);

				// Molette
				events.AddEventCallback<NkMouseWheelVerticalEvent>(
					[&vp](NkMouseWheelVerticalEvent *e) {
						vp.localInput.AddMouseWheel(static_cast<float32>(e->GetOffsetY()),
				...
				events.AddEventCallback<NkKeyPressEvent>(
					[&vp](NkKeyPressEvent *e) {
						if (e->GetKey() != NkKey::NK_UNKNOWN)
							vp.localInput.SetKey(e->GetKey(), true);
					},
					osWinId);

				events.AddEventCallback<NkKeyReleaseEvent>(
					[&vp](NkKeyReleaseEvent *e) {
						if (e->GetKey() != NkKey::NK_UNKNOWN)
				...
				events.AddEventCallback<NkTextInputEvent>(
					[&vp](NkTextInputEvent *e) { AddUtf8ToInput(vp.localInput, e->GetUtf8()); }, osWinId);
		...
		void NkUIMultiViewportManager::Submit(NkUIContext &ctx, NkUIWindowManager &wm) noexcept {
			...
			for (int32 i = 0; i < mCount; ++i) {
				NkUIViewportState &vp = mViewports[i];
				if (!vp.alive || !vp.osWindow.IsOpen())
					continue;
				if (!vp.cmd)
					continue;
		...
		bool NkUIMultiViewportManager::CreateViewport(NkUIID windowId, NkUIWindowManager &wm) noexcept {
			...
			if (mApi == NkGraphicsApi::NK_GFX_API_OPENGL && mMainWindow) {
				cfg.native.win32PixelFormatShareWindowHandle = reinterpret_cast<uintptr>(mMainWindow->mData.mHwnd);
			}
#endif
		...
		bool NkUIMultiViewportManager::IsOutsideMainViewport(const NkUIWindowState &ws) const noexcept {
			...
			const bool outsideX = cx < -kOutsideMargin || cx > static_cast<float32>(mMainViewW) + kOutsideMargin;
			const bool outsideY = cy < -kOutsideMargin || cy > static_cast<float32>(mMainViewH) + kOutsideMargin;

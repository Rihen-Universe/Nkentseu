...
namespace nkentseu {
	namespace nkui {
		...
		static uint8 ComputeResizeEdge(const NkRect &r, NkVec2 mp, float32 borderSize, bool canInteract) noexcept {
			if (!canInteract)
				return 0;
			...
			if (inY && mp.x >= r.x - margin && mp.x <= r.x + margin)
				edge |= 4; // left
			if (inY && mp.x >= r.x + r.w - margin && mp.x <= r.x + r.w + margin)
				edge |= 8; // right
			if (inX && mp.y >= r.y - margin && mp.y <= r.y + margin)
				edge |= 1; // top
			if (inX && mp.y >= r.y + r.h - margin && mp.y <= r.y + r.h + margin)
				edge |= 2; // bottom
		...
		NkUIWindowState *NkUIWindowManager::FindOrCreate(const char *name, NkVec2 defaultPos, NkVec2 defaultSize,
		...
		void NkUIWindowManager::BeginFrame(NkUIContext &ctx) noexcept {
			...
			if (movingId != NKUI_ID_NONE) {
				...
				if (ws && !ws->isDocked && !HasFlag(ws->flags, NkUIWindowFlags::NK_NO_MOVE)) {
					...
					if (ws->pos.y < minY)
						ws->pos.y = minY;
					if (ws->pos.y + titleH > ctx.viewH)
						ws->pos.y = static_cast<float32>(ctx.viewH) - titleH;
					if (ws->pos.x + ws->size.x < 40)
						ws->pos.x = 40 - ws->size.x;
					if (ws->pos.x > ctx.viewW - 40)
						ws->pos.x = static_cast<float32>(ctx.viewW) - 40;
				}
		...
		void NkUIWindow::DrawShadow(NkUIDrawList &dl, const NkUIWindowState &ws, bool focused) noexcept {
			if (!focused)
				return; // ombre seulement pour la fenêtre active
			...
			for (int32 i = 2; i >= 0; --i) {
				...
				dl.AddRectFilled({r.x + 1.5f - expand, r.y + 1.5f - expand, r.w + expand * 2, r.h + expand * 2}, sc,
								 10.f);
			}
		}
		...
		void NkUIWindow::DrawTitleBar(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font,
			...
			if (!HasFlag(ws.flags, NkUIWindowFlags::NK_NO_CLOSE)) {
				...
				if (hov)
					dl.AddRectFilled(closeR, {200, 50, 50, 200}, rx);
				// X
			...
			if (!HasFlag(ws.flags, NkUIWindowFlags::NK_NO_COLLAPSE)) {
				...
				if (hov)
					dl.AddRectFilled(collapseR, c.titleBarBtnHov, 0);
				dl.AddArrow({collapseR.x + btnSz * 0.5f, collapseR.y + btnSz * 0.5f}, 7.f, ws.isCollapsed ? 0 : 1,
							c.titleBarText);
				if (hov && ctx.ConsumeMouseClick(0)) {
			...
			// if (canInteract && NkRectContains(dragR, ctx.input.mousePos) && ctx.IsMouseClicked(0) &&
			// !HasFlag(ws.flags, NkUIWindowFlags::NK_NO_MOVE)) {
			if (canInteract && !ws.isDocked && NkRectContains(dragR, ctx.input.mousePos) && ctx.IsMouseClicked(0) &&
				!HasFlag(ws.flags, NkUIWindowFlags::NK_NO_MOVE)) {
				const float32 rb = ctx.theme.metrics.resizeBorder;
				const uint8 edge =
					ComputeResizeEdge({ws.pos.x, ws.pos.y, ws.size.x, ws.size.y}, ctx.input.mousePos, rb, true);
		...
		void NkUIWindow::HandleResize(NkUIContext &ctx, NkUIDrawList & /*dl*/, NkUIWindowState &ws) noexcept {
			if (HasFlag(ws.flags, NkUIWindowFlags::NK_NO_RESIZE) || ws.isCollapsed)
				return;
			const bool canInteract = WindowCanCapturePointer(ctx, ws.id);
			const float32 rb = ctx.theme.metrics.resizeBorder;
			const NkRect r = {ws.pos.x, ws.pos.y, ws.size.x, ws.size.y};
		...
		void NkUIWindow::DrawResizeBorders(NkUIDrawList &dl, const NkUIContext &ctx,
										   const NkUIWindowState &ws) noexcept {
			if (HasFlag(ws.flags, NkUIWindowFlags::NK_NO_RESIZE) || ws.isCollapsed)
				return;
			const NkRect r = {ws.pos.x, ws.pos.y, ws.size.x, ws.size.y};
			const float32 sz = 5.f; // taille de la poignée de coin
			dl.AddResizeGrip({r.x + r.w - sz - 2, r.y + r.h - sz - 2}, sz, ctx.theme.colors.separator);
		}
		...
		bool NkUIWindow::Begin(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDrawList &dl, NkUIFont &font,
							   NkUILayoutStack &ls, const char *name, bool *pOpen, NkUIWindowFlags flags) noexcept {
			if (sDepth >= 8)
				return false;
			ctx.wm = &wm;
			...
			const float32 titleH =
				HasFlag(ws->flags, NkUIWindowFlags::NK_NO_TITLE_BAR) ? 0.f : ctx.theme.metrics.titleBarHeight;
			const float32 rx = ctx.theme.metrics.cornerRadius;
			...
			if (!ws->isCollapsed) {
				dl.AddRectFilled({ws->pos.x, ws->pos.y + titleH, ws->size.x, contentH}, ctx.theme.colors.bgWindow, rx);
				// Bordure
			...
			if (!HasFlag(ws->flags, NkUIWindowFlags::NK_NO_SCROLLBAR)) {
				...
				if (showV && clientW > ws->size.x - sbW)
					clientW -= sbW;
				if (showH && clientH > contentH - sbW)
					clientH -= sbW;
			}
			if (clientW < 1.f)
				clientW = 1.f;
		...
		void NkUIWindow::End(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDrawList &dl, NkUILayoutStack &ls) noexcept {
			if (sDepth <= 0)
				return;
			...
			if (cur.savedDL)
				ctx.dl = cur.savedDL; // toujours restaurer ctx.dl
			...
			const float32 cursorContentH =
				ctx.cursor.y - cur.startCursor.y + ctx.theme.metrics.windowPadY + ws->scrollY;
			...
			const bool allowWheelForWindow = ws->isDocked ? (pointerInWheelRect && dockedNotBlockedByFloating)
														  : (pointerInWheelRect && floatingHoverMatch);

			if (!HasFlag(ws->flags, NkUIWindowFlags::NK_NO_SCROLL_WITH_MOUSE) && allowWheelForWindow) {
			...
			if (ws->scrollX < 0.f)
				ws->scrollX = 0.f;
			if (ws->scrollX > maxScrollX)
				ws->scrollX = maxScrollX;
			if (ws->scrollY < 0.f)
				ws->scrollY = 0.f;
			if (ws->scrollY > maxScrollY)
				ws->scrollY = maxScrollY;

			// Barre de défilement verticale
			if (showV) {
				const NkRect track = {ws->pos.x + fullViewW - sbW, ws->pos.y + titleH, sbW, viewH > 0.f ? viewH : 1.f};
			...
			if (showH) {
				const NkRect track = {ws->pos.x, ws->pos.y + titleH + fullViewH - sbW, viewW > 0.f ? viewW : 1.f, sbW};
		...
		void NkUIWindow::RenderAll(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDrawList &dl, NkUIFont &font) noexcept {
			...
			for (int32 i = 0; i < wm.numZOrder; ++i) {
				...
				if (ws && ws->isOpen && HasFlag(ws->flags, NkUIWindowFlags::NK_MODAL)) {
					dl.AddRectFilled({0, 0, static_cast<float32>(ctx.viewW), static_cast<float32>(ctx.viewH)},

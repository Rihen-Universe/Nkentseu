...
namespace nkentseu {
	namespace nkui {
		...
		bool NkUIMenu::DrawMenuButton(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, const char *label, NkRect r,
			...
			if (bg.a > 0)
				dl.AddRectFilled(r, bg, t.metrics.cornerRadiusSm);
			const NkColor tc =
				!enabled ? t.colors.textDisabled : (open || hov ? t.colors.textOnAccent : t.colors.textPrimary);
			font.RenderText(dl, {r.x + t.metrics.paddingX, BaselineY(r, font)}, label, tc);
			return hov && enabled;
		}

		NkRect NkUIMenu::DrawMenuPanel(NkUIContext &ctx, NkUIDrawList &dl, const NkUIMenuState &ms) noexcept {
			const auto &t = ctx.theme;
		...
		void NkUIMenu::EndMenuBar(NkUIContext &ctx) noexcept {
			sInMenuBar = false;
			(void)ctx;
		}
		...
		bool NkUIMenu::BeginMenu(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, const char *label,
		...
		bool NkUIMenu::MenuItem(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, const char *label,
								const char *shortcut, bool *selected, bool enabled) noexcept {
			if (sMenuDepth <= 0)
				return false;
			...
			if (hov)
				pdl.AddRectFilled(itemR, ctx.theme.colors.accent, ctx.theme.metrics.cornerRadiusSm);
			...
			if (shortcut && *shortcut) {
				...
				font.RenderText(pdl, {itemR.x + itemR.w - sw - 8, y}, shortcut,
								!enabled ? ctx.theme.colors.textDisabled : ctx.theme.colors.textSecondary);
			}
		...
		bool NkUIMenu::BeginContextMenu(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, const char *id) noexcept {
			...
			if (ctx.ConsumeMouseClick(1))
				OpenContextMenu(ctx, id);
			if (!sContextMenuOpen || ctx.GetID(id) != sContextMenuId)
				return false;
			...
			if (ctx.IsMouseClicked(0)) {
				...
				if (!NkRectContains(mr, ctx.input.mousePos)) {
					sContextMenuOpen = false;
					return false;
				}
			}
			// Crée un faux menu state pour réutiliser MenuItem
			NkUIMenuState &ms = sMenus[0];
			ms.id = sContextMenuId;
			ms.depth = 0;
			ms.open = true;
			ms.itemCount = 0;
		...
		void NkUIMenu::OpenPopup(NkUIContext &ctx, const char *id) noexcept {
			...
			for (int32 i = 0; i < MAX_POPUPS; ++i) {
				...
				if (!sPopups[i].id) {
					sPopups[i].id = pid;
					sPopups[i].open = true;
					sPopups[i].pos = ctx.input.mousePos;
					return;
				}
			}
		}
		...
		void NkUIMenu::ClosePopup(NkUIContext &ctx) noexcept {
			if (sPopupDepth > 0) {
				sPopups[sPopupDepth - 1].open = false;
				--sPopupDepth;
			}
			(void)ctx;
		}

		bool NkUIMenu::BeginPopup(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, const char *id,
		...
		void NkUIMenu::OpenModal(NkUIContext &, const char *title) noexcept {
			::strncpy(sModalTitle, title, 63);
			sModalOpen = true;
		}

		bool NkUIMenu::BeginModal(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, const char *title, bool *open,
								  NkVec2 size) noexcept {
			if (!sModalOpen && (!open || !*open))
				return false;
			if (open)
				sModalOpen = *open;
			sModalSize = size;
			...
			font.RenderText(
				pdl,
				{pr.x + ctx.theme.metrics.titleBarPadX, pr.y + (tbH - font.size) * 0.5f + font.metrics.ascender * 0.5f},

	...
	namespace nkui {
		...
		static void CollapseEmptyLeaf(NkUIDockManager &dm, NkUIWindowManager &wm, int32 leafIdx) noexcept {
			...
			while (current >= 0 && current < dm.numNodes) {
				...
				const int32 sibIdx = (parent.children[0] == current) ? parent.children[1] : parent.children[0];
		...
		void NkUIDockManager::Destroy() noexcept {
			numNodes = 0;
			rootIdx = -1;
		}
		...
		void NkUIDockManager::SetViewport(NkRect r, NkUIWindowManager *wm) noexcept {
			if (rootIdx >= 0)
				nodes[rootIdx].rect = r;
			RecalcRectsAll();
			if (wm)
				SyncDockedWindowRects(*wm); // sync immédiat
		}
		...
		void NkUIDockManager::RecalcRectsAll() noexcept {
			RecalcRects(rootIdx);
		}
		...
		bool NkUIDockManager::NodeAllowsTabs(const NkUIWindowManager &wm, int32 nodeIdx,
			...
			for (int32 i = 0; i < node.numWindows; ++i) {
				const NkUIWindowState *ws = const_cast<NkUIWindowManager &>(wm).Find(node.windows[i]);
				if (ws && HasFlag(ws->flags, NkUIWindowFlags::NK_NO_TABS))
					return false;
			}
			if (draggedWinId != NKUI_ID_NONE) {
				const NkUIWindowState *dws = const_cast<NkUIWindowManager &>(wm).Find(draggedWinId);
		...
		bool NkUIDockManager::DockWindow(NkUIWindowManager &wm, NkUIID winId, int32 nodeIdx,
			...
			if (!ws)
				return false;
			if (HasFlag(ws->flags, NkUIWindowFlags::NK_NO_DOCK))
				return false;
			...
			if (drop == NkUIDockDrop::NK_CENTER || drop == NkUIDockDrop::NK_TAB) {
				...
				if (node.numWindows >= NkUIDockNode::MAX_WINDOWS)
					return false;
			...
			const int32 c0 = AllocNode();
			if (c0 < 0)
				return false;
			const int32 c1 = AllocNode();
			if (c1 < 0)
				return false;
			...
			for (int32 i = 0; i < oldN; ++i)
				oldWins[i] = target.windows[i];

			const bool horiz = (drop == NkUIDockDrop::NK_LEFT || drop == NkUIDockDrop::NK_RIGHT);
			...
			const int32 newChildIdx = (drop == NkUIDockDrop::NK_LEFT || drop == NkUIDockDrop::NK_TOP) ? 0 : 1;
			...
			existNode.activeTab = (oldN > 0) ? std::clamp(oldA, 0, existNode.numWindows - 1) : 0;
		...
		void NkUIDockManager::UndockWindow(NkUIWindowManager &wm, NkUIID winId) noexcept {
			...
			if (wm.movingId == winId)
				wm.movingId = NKUI_ID_NONE;
			if (wm.resizingId == winId)
		...
		int32 NkUIDockManager::FindNodeAt(NkVec2 pos) const noexcept {
			...
			for (int32 i = 0; i < numNodes; ++i) {
				if (nodes[i].type == NkUIDockNodeType::NK_LEAF && NkRectContains(nodes[i].rect, pos))
					return i;
			}
		...
		void NkUIDockManager::DrawDirectionalHighlight(NkUIDrawList &dl, int32 nodeIdx, NkUIDockDrop drop,
			...
			switch (drop) {
				case NkUIDockDrop::NK_LEFT:
					dl.AddRectFilled({r.x, r.y, th, r.h}, color);
					break;
				case NkUIDockDrop::NK_RIGHT:
					dl.AddRectFilled({r.x + r.w - th, r.y, th, r.h}, color);
					break;
				case NkUIDockDrop::NK_TOP:
					dl.AddRectFilled({r.x, r.y, r.w, th}, color);
					break;
				case NkUIDockDrop::NK_BOTTOM:
					dl.AddRectFilled({r.x, r.y + r.h - th, r.w, th}, color);
					break;
				default:
					break;
			}
		...
		NkUIDockDrop NkUIDockManager::DrawDropZones(NkUIContext &ctx, NkUIDrawList &dl, int32 nodeIdx) noexcept {
			if (nodeIdx < 0 || nodeIdx >= numNodes)
				return NkUIDockDrop::NK_NONE;
			const NkRect &r = nodes[nodeIdx].rect;
			...
			for (const auto &z : zones)
				if (z.enabled && NkRectContains(z.rect, mp)) {
					hovered = z.drop;
					break;
				}
			...
			if (hovered != NkUIDockDrop::NK_NONE) {
				...
				switch (hovered) {
					case NkUIDockDrop::NK_LEFT:
						prev = {r.x, r.y, r.w * .5f, r.h};
						break;
					case NkUIDockDrop::NK_RIGHT:
						prev = {r.x + r.w * .5f, r.y, r.w * .5f, r.h};
						break;
					case NkUIDockDrop::NK_TOP:
						prev = {r.x, r.y, r.w, r.h * .5f};
						break;
					case NkUIDockDrop::NK_BOTTOM:
						prev = {r.x, r.y + r.h * .5f, r.w, r.h * .5f};
						break;
			...
			for (const auto &z : zones) {
				...
				switch (z.drop) {
					case NkUIDockDrop::NK_LEFT:
						dl.AddArrow(c, bs * .3f, 2, ic);
						break;
					case NkUIDockDrop::NK_RIGHT:
						dl.AddArrow(c, bs * .3f, 0, ic);
						break;
					case NkUIDockDrop::NK_TOP:
						dl.AddArrow(c, bs * .3f, 3, ic);
						break;
					case NkUIDockDrop::NK_BOTTOM:
						dl.AddArrow(c, bs * .3f, 1, ic);
						break;
					case NkUIDockDrop::NK_CENTER: {
		...
		// =============================================================================
		bool NkUIDockManager::AddTabDecoration(NkTabDecoFn fn, void *userData, float32 width, NkUIID id) noexcept {
			if (!fn || numTabDecos >= MAX_TAB_DECOS)
				return false;
		...
		void NkUIDockManager::SyncDockedWindowRects(NkUIWindowManager &wm) noexcept {
			for (int32 i = 0; i < numNodes; ++i) {
				...
				if (node.type != NkUIDockNodeType::NK_LEAF && node.type != NkUIDockNodeType::NK_ROOT)
					continue;
				if (node.numWindows <= 0)
					continue;
		...
		void NkUIDockManager::RenderNode(NkUIContext &ctx, NkUIDrawList & /*dl_unused*/, NkUIFont &font,
			...
			if (node.type == NkUIDockNodeType::NK_SPLIT_H || node.type == NkUIDockNodeType::NK_SPLIT_V) {
				...
				if (node.children[0] >= 0) {
					...
					if (vert)
						bgDl.AddLine({a.x + a.w, node.rect.y}, {a.x + a.w, node.rect.y + node.rect.h}, lc, 1.f);
					else
						bgDl.AddLine({node.rect.x, a.y + a.h}, {node.rect.x + node.rect.w, a.y + a.h}, lc, 1.f);
				}
			...
			for (int32 t = 0; t < node.numWindows; ++t) {
				...
				const float32 tw = (ws ? font.MeasureWidth(ws->name) : 40.f) + kPadX * 2.f + kCloseW;
				natW[t] = tw;
				totalNat += tw;
			}
			...
			// if (isDragging && dragWindowId != NKUI_ID_NONE && NkRectContains(tabBarR, mp) && !(dragIsFromThisNode &&
			// dragWinAlreadyHere)) {
			...
			if (!tabDropActive) {
				...
				for (int32 t = 0; t < node.numWindows; ++t) {
					...
					const NkRect clR = {tx + tw - kCloseW, r.y + (tabH - 14.f) * 0.5f, 14.f, 14.f};
					...
					if (dockCanReceiveInput && NkRectContains(tabR, mp) && !NkRectContains(clR, mp) &&
						ctx.ConsumeMouseClick(0)) {
						node.activeTab = t;
						wm.activeId = node.windows[t];
						isDraggingTab = true;
...

			{
				...
				for (int32 t = 0; t < node.numWindows; ++t) {
					...
					// Fond
					const NkColor bg = act ? col.tabActive : hov ? col.buttonHover : col.tabBg;
					if (act) {
						dl.AddRectFilledCorners(tabR, bg, met.cornerRadiusSm, 0x3);
					} else {
						dl.AddRectFilled(tabR, bg);
						dl.AddLine({tx, r.y + tabH - 1.f}, {tx + tw, r.y + tabH - 1.f}, sep, 1.f);
					}
					...
					if (ws) {
						...
						font.RenderText(dl, {tx + kPadX, r.y + (tabH - lh) * 0.5f + asc}, ws->name,
										act ? col.tabActiveText : col.tabText, maxTW, /*ellipsis=*/true);
					}
					...
					if (tabDropActive) {
						...
						if (insertBefore == t) {
							...
							dl.AddLine({ix, r.y + 2.f}, {ix, r.y + tabH - 2.f}, {80, 180, 255, 255}, 3.f);
							// Petite flèche vers le bas
							dl.AddTriangleFilled({ix - 5.f, r.y + 2.f}, {ix + 5.f, r.y + 2.f}, {ix, r.y + 9.f},
												 {80, 180, 255, 220});
						}
				...
				if (totalFin < avail)
					dl.AddLine({r.x + totalFin, r.y + tabH - 1.f}, {r.x + avail, r.y + tabH - 1.f}, sep, 1.f);
			}
			...
			// =========================================================================
			{
				...
				if (dockOverflowButton) {
					...
					dl.AddArrow({ovR.x + ovR.w * 0.5f, ovR.y + ovR.h * 0.5f}, 6.f, 1, col.textSecondary);
					...
					if (open) {
						...
						for (int32 t = 0; t < node.numWindows; ++t) {
							...
							if (iSel || iHov)
								popDl.AddRectFilled(iR, iSel ? col.accent : col.buttonHover, met.cornerRadiusSm);
							...
							const float32 lh = font.metrics.lineHeight > 0.f
												   ...
												   : asc + (dsc > 0.f ? dsc : font.size * 0.2f);
							font.RenderText(popDl, {iR.x + 8.f, iR.y + (iH - lh) * 0.5f + asc}, ws->name,
											(iSel || iHov) ? col.textOnAccent : col.textPrimary, iR.w - 12.f,
											/*ellipsis=*/true);
						...
						if (ctx.IsMouseClicked(0) && !NkRectContains(menuR, mp) && !NkRectContains(ovR, mp)) {
							sOpenNode = NKUI_ID_NONE;
						}
					}
				}
			...
			// =========================================================================
			{
				const NkRect emptyR = {r.x + totalFin, r.y, avail - totalFin, tabH};
				if (emptyR.w > 8.f && NkRectContains(emptyR, mp) && ctx.input.IsMouseDown(0) &&
					wm.movingId == NKUI_ID_NONE && node.activeTab < node.numWindows) {
			...
			for (int32 t = 0; t < node.numWindows; ++t) {
				...
				if (t == node.activeTab && ws->isOpen) {
					...
					dl.AddLine({r.x, r.y + tabH}, {r.x + r.w, r.y + tabH}, col.separator.WithAlpha(235), 1.f);
					ws->pos = {r.x, r.y + tabH};
		...
		//  BeginFrame
		// ─────────────────────────────────────────────────────────────────────────────
		void NkUIDockManager::BeginFrame(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDrawList &dl,
										 NkUIFont &) noexcept {
			...
			if (isDragging && dragWindowId != NKUI_ID_NONE) {
				...
				if (nodeUnder >= 0) {
					...
					const float32 tabH = ctx.theme.metrics.dockTabHeight;
					const NkRect tabBarR = {nodes[nodeUnder].rect.x, nodes[nodeUnder].rect.y, nodes[nodeUnder].rect.w,
											tabH};
		...
		void NkUIDockManager::Render(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkUIWindowManager &wm,
									 NkUILayoutStack &ls, int32 rootOverride) noexcept {
			const int32 root = (rootOverride >= 0 && rootOverride < numNodes) ? rootOverride : rootIdx;
			if (root >= 0)
		...
		bool NkUIDockManager::BeginDocked(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDrawList &dl,
										  const char *name) noexcept {
			NkUIWindowState *ws = wm.Find(name);
			if (!ws || !ws->isDocked || !ws->isOpen)
				return false;
			const int32 nodeIdx = static_cast<int32>(ws->dockNodeId);
			if (nodeIdx < 0 || nodeIdx >= numNodes)
				return false;
			...
			if (node.activeTab < 0 || node.activeTab >= node.numWindows)
				return false;
			if (node.windows[node.activeTab] != ws->id)
				return false;

			const NkRect clientR = {ws->pos.x, ws->pos.y, ws->size.x, ws->size.y};
			const NkVec2 start = {clientR.x + ctx.theme.metrics.windowPadX, clientR.y + ctx.theme.metrics.windowPadY};
			ctx.SetCursor(start);
		...
		bool NkUIDockManager::BeginChildDock(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDrawList &dl, NkUIFont &font,
											 NkUILayoutStack &ls, NkUIID parentWinId, NkRect childViewport) noexcept {
		...
		void NkUIDockManager::SaveLayout(const char *path) const noexcept {
			...
			for (int32 i = 0; i < numNodes; ++i) {
				...
				::fprintf(f,
						  ...
						  "\"nw\":%d,\"at\":%d}%s\n",
						  (int32)n.type, n.rect.x, n.rect.y, n.rect.w, n.rect.h, n.splitRatio, n.parent, n.children[0],
						  n.children[1], n.numWindows, n.activeTab, i < numNodes - 1 ? "," : "");
			}
			::fprintf(f, "]}\n");
			::fclose(f);
		}
...
//                 if (pi < 0 || pi >= dm.numNodes) { c.type = NkUIDockNodeType::NK_ROOT;
//                 c.children[0]=c.children[1]=-1; break; } NkUIDockNode& parent = dm.nodes[pi]; const int32 sib =
//                 (parent.children[0]==cur) ? parent.children[1] : parent.children[0]; if (sib < 0 || sib >=
//                 dm.numNodes) { parent.type=NkUIDockNodeType::NK_LEAF; parent.numWindows=0; parent.activeTab=0;
//                 parent.children[0]=parent.children[1]=-1; cur=pi; continue; } NkUIDockNode sc = dm.nodes[sib];
//                 sc.parent=parent.parent; sc.rect=parent.rect; dm.nodes[pi] = sc; if (dm.nodes[pi].children[0]>=0)
//                 dm.nodes[dm.nodes[pi].children[0]].parent=pi; if (dm.nodes[pi].children[1]>=0)
//                 dm.nodes[dm.nodes[pi].children[1]].parent=pi; dm.nodes[cur]=NkUIDockNode{};
//                 dm.nodes[sib]=NkUIDockNode{}; if (cur==dm.rootIdx||sib==dm.rootIdx) dm.rootIdx=pi;
...
//         void NkUIDockManager::SetViewport(NkRect r) noexcept { if(rootIdx>=0) nodes[rootIdx].rect=r;
//         RecalcRectsAll(); }
...
//             if(n.type==NkUIDockNodeType::NK_SPLIT_H){float32 w0=n.rect.w*n.splitRatio;
//             nodes[c0].rect={n.rect.x,n.rect.y,w0,n.rect.h};
//             nodes[c1].rect={n.rect.x+w0,n.rect.y,n.rect.w-w0,n.rect.h};} else{float32 h0=n.rect.h*n.splitRatio;
//             nodes[c0].rect={n.rect.x,n.rect.y,n.rect.w,h0};
//             nodes[c1].rect={n.rect.x,n.rect.y+h0,n.rect.w,n.rect.h-h0};} RecalcRects(c0); RecalcRects(c1);
//         }
...
//             for(int32 i=0;i<node.numWindows;++i){const NkUIWindowState*
//             ws=const_cast<NkUIWindowManager&>(wm).Find(node.windows[i]);
//             if(ws&&HasFlag(ws->flags,NkUIWindowFlags::NK_NO_TABS)) return false;} if(di!=NKUI_ID_NONE){const
//             NkUIWindowState* dws=const_cast<NkUIWindowManager&>(wm).Find(di);
//             if(dws&&HasFlag(dws->flags,NkUIWindowFlags::NK_NO_TABS)) return false;} return true;
//         }
...
//             if((drop==NkUIDockDrop::NK_CENTER||drop==NkUIDockDrop::NK_TAB)&&!NodeAllowsTabs(wm,ni,winId))
//             drop=NkUIDockDrop::NK_BOTTOM; if(drop==NkUIDockDrop::NK_CENTER||drop==NkUIDockDrop::NK_TAB){
...
//             target.type=horiz?NkUIDockNodeType::NK_SPLIT_H:NkUIDockNodeType::NK_SPLIT_V;
//             target.children[0]=c0; target.children[1]=c1; target.splitRatio=0.5f; target.numWindows=0;
//             target.activeTab=0;
//             ::memset(target.windows,0,sizeof(target.windows));
//             nodes[c0]=NkUIDockNode{}; nodes[c0].type=NkUIDockNodeType::NK_LEAF; nodes[c0].parent=ni;
//             nodes[c0].splitRatio=0.5f; nodes[c1]=NkUIDockNode{}; nodes[c1].type=NkUIDockNodeType::NK_LEAF;
//             nodes[c1].parent=ni; nodes[c1].splitRatio=0.5f; int32
//             nc=(drop==NkUIDockDrop::NK_LEFT||drop==NkUIDockDrop::NK_TOP)?0:1, ec=1-nc; NkUIDockNode&
//             exN=nodes[ec==0?c0:c1]; NkUIDockNode& newN=nodes[nc==0?c0:c1]; for(int32
//             i=0;i<oldN&&exN.numWindows<NkUIDockNode::MAX_WINDOWS;++i){exN.windows[exN.numWindows++]=ow[i];
//             NkUIWindowState* m=wm.Find(ow[i]);
//             if(m){m->isDocked=true;m->dockNodeId=(NkUIID)(ec==0?c0:c1);m->zOrder=0;}}
...
//         int32 NkUIDockManager::FindNodeAt(NkVec2 pos) const noexcept {
//             for(int32 i=0;i<numNodes;++i)
//             if(nodes[i].type==NkUIDockNodeType::NK_LEAF&&NkRectContains(nodes[i].rect,pos)) return i;
//             if(rootIdx>=0&&NkRectContains(nodes[rootIdx].rect,pos)) return rootIdx;
//             return -1;
//         }

//         void NkUIDockManager::DrawDirectionalHighlight(NkUIDrawList& dl, int32 ni, NkUIDockDrop drop, NkColor col)
//         const noexcept {
...
//                 if(dR<dMin){dMin=dR;hov=NkUIDockDrop::NK_RIGHT;} if(dT<dMin){dMin=dT;hov=NkUIDockDrop::NK_TOP;}
//                 if(dB<dMin){dMin=dB;hov=NkUIDockDrop::NK_BOTTOM;} if(dMin>band) hov=NkUIDockDrop::NK_NONE;
//             }
//             if(hov!=NkUIDockDrop::NK_NONE){
//                 NkRect prev={};
//                 switch(hov){case NkUIDockDrop::NK_LEFT:prev={r.x,r.y,r.w*.4f,r.h};break;case
//                 NkUIDockDrop::NK_RIGHT:prev={r.x+r.w*.6f,r.y,r.w*.4f,r.h};break;case
//                 NkUIDockDrop::NK_TOP:prev={r.x,r.y,r.w,r.h*.4f};break;case
//                 NkUIDockDrop::NK_BOTTOM:prev={r.x,r.y+r.h*.6f,r.w,r.h*.4f};break;case
//                 NkUIDockDrop::NK_CENTER:prev={r.x+r.w*.2f,r.y+r.h*.2f,r.w*.6f,r.h*.6f};break;default:break;}
...
//                 const NkVec2 c={z.rect.x+z.rect.w*.5f,z.rect.y+z.rect.h*.5f};
//                 switch(z.drop){case NkUIDockDrop::NK_LEFT:dl.AddArrow(c,sz*.3f,2,zBd);break;case
//                 NkUIDockDrop::NK_RIGHT:dl.AddArrow(c,sz*.3f,0,zBd);break;case
//                 NkUIDockDrop::NK_TOP:dl.AddArrow(c,sz*.3f,3,zBd);break;case
//                 NkUIDockDrop::NK_BOTTOM:dl.AddArrow(c,sz*.3f,1,zBd);break;case
//                 NkUIDockDrop::NK_CENTER:dl.AddRectFilled({c.x-sz*.2f,c.y-sz*.2f,sz*.4f,sz*.4f},zBd,3.f);break;default:break;}
//             }
//             return hov;
//         }
...
//         }
//         void NkUIDockManager::RemoveTabDecoration(NkUIID id) noexcept {
//             for(int32 i=0;i<numTabDecos;++i) if(tabDecos[i].id==id){for(int32 j=i;j<numTabDecos-1;++j)
//             tabDecos[j]=tabDecos[j+1];--numTabDecos;return;}
//         }
...
//                 const NkColor lc=ctx.theme.colors.separator.WithAlpha(200);
//                 if(node.children[0]>=0){const NkRect& a=nodes[node.children[0]].rect; if(vert)
//                 bgDl.AddLine({a.x+a.w,node.rect.y},{a.x+a.w,node.rect.y+node.rect.h},lc,1.f); else
//                 bgDl.AddLine({node.rect.x,a.y+a.h},{node.rect.x+node.rect.w,a.y+a.h},lc,1.f);}
//                 RenderNode(ctx,bgDl,font,wm,node.children[0],ls); RenderNode(ctx,bgDl,font,wm,node.children[1],ls);
//                 return;
//             }
//             if((node.type!=NkUIDockNodeType::NK_LEAF&&node.type!=NkUIDockNodeType::NK_ROOT)||node.numWindows<=0)
//             return;
...
//             for(int32 t=0;t<node.numWindows;++t){const NkUIWindowState* ws=wm.Find(node.windows[t]);
//             natW[t]=(ws?font.MeasureWidth(ws->name):40.f)+kPX*2.f+kCW; totNat+=natW[t];} if(totNat<=avail){for(int32
//             t=0;t<node.numWindows;++t) finW[t]=natW[t];} else{float32 ratio=avail/totNat; for(int32
//             t=0;t<node.numWindows;++t) finW[t]=(natW[t]*ratio<kMin)?kMin:natW[t]*ratio;} float32 totFin=0.f;
//             for(int32 t=0;t<node.numWindows;++t) totFin+=finW[t];
...
//                 for(int32 t=0;t<node.numWindows;++t){if(mp.x<tx+finW[t]*.5f){ins=t;break;}
//                 if(mp.x<tx+finW[t]){ins=t+1;break;} tx+=finW[t];}
//             }
...
//                     if(NkRectContains(clR,mp)&&ctx.ConsumeMouseClick(0)){NkUIWindowState*
//                     ws=wm.Find(node.windows[t]); UndockWindow(wm,node.windows[t]); if(ws)ws->isOpen=false; break;}
//                     if(NkRectContains(tabR,mp)&&!NkRectContains(clR,mp)&&ctx.ConsumeMouseClick(0)){node.activeTab=t;
//                     wm.activeId=node.windows[t]; isDraggingTab=true; dragWindowId=node.windows[t];} tx+=finW[t];
//                 }
//             }
...
//                     const NkRect clR={tx+tw-kCW,r.y+(tabH-14.f)*.5f,14.f,14.f}; const bool
//                     clH=hov&&NkRectContains(clR,mp); const NkUIWindowState* ws=wm.Find(node.windows[t]);
//                     // Fond tab
//                     if(act) dl.AddRectFilledCorners(tabR,col.tabActive,met.cornerRadiusSm,0x3);
//                     else{dl.AddRectFilled(tabR,hov?col.buttonHover:col.tabBg);
//                     dl.AddLine({tx,r.y+tabH-1.f},{tx+tw,r.y+tabH-1.f},sep,1.f);} if(t>0)
//                     dl.AddLine({tx,r.y+4.f},{tx,r.y+tabH-4.f},sep,1.f);
//                     // Texte
//                     if(ws)
//                     font.RenderText(dl,{tx+kPX,DockBaseY(tabR,font)},ws->name,act?col.tabActiveText:col.tabText,tw-kPX*2.f-kCW,true);
//                     // X
//                     if(hov||act){const NkVec2 cx={clR.x+clR.w*.5f,clR.y+clR.h*.5f}; const float32 d=3.5f; const
//                     NkColor xc=clH?NkColor{220,60,60,255}:col.tabText;
//                     if(clH)dl.AddCircleFilled(cx,7.f,{200,50,50,180});
//                     dl.AddLine({cx.x-d,cx.y-d},{cx.x+d,cx.y+d},xc,1.5f);
//                     dl.AddLine({cx.x+d,cx.y-d},{cx.x-d,cx.y+d},xc,1.5f);}
//                     // Indicateur drop
//                     if(tabDrop&&ins==t){dl.AddLine({tx-1.5f,r.y+2.f},{tx-1.5f,r.y+tabH-2.f},{80,180,255,255},3.f);
//                     dl.AddTriangleFilled({tx-5.f,r.y+2.f},{tx+5.f,r.y+2.f},{tx,r.y+9.f},{80,180,255,220});}
//                     if(tabDrop&&t==node.numWindows-1&&ins==node.numWindows){float32 ix=tx+tw-1.5f;
//                     dl.AddLine({ix,r.y+2.f},{ix,r.y+tabH-2.f},{80,180,255,255},3.f);
//                     dl.AddTriangleFilled({ix-5.f,r.y+2.f},{ix+5.f,r.y+2.f},{ix,r.y+9.f},{80,180,255,220});}
//                     tx+=finW[t];
//                 }
...
//                 if(DockWindow(wm,dragWindowId,idx,NkUIDockDrop::NK_CENTER)){
//                     if(node.numWindows>prevN&&ins<prevN){NkUIID nw=node.windows[node.numWindows-1]; for(int32
//                     i=node.numWindows-1;i>ins;--i)node.windows[i]=node.windows[i-1]; node.windows[ins]=nw;
//                     node.activeTab=ins;}
//                 }
...
//                         const NkRect menuR={mX,r.y+tabH,mW,mH};
//                         popDl.AddRectFilled(menuR,col.bgPopup,met.cornerRadius);
//                         popDl.AddRect(menuR,col.border,1.f,met.cornerRadius); popDl.PushClipRect(menuR); for(int32
//                         t=0;t<node.numWindows;++t){
...
//                             const float32
//                             dsc=font.metrics.descender>=0.f?font.metrics.descender:-font.metrics.descender; const
//                             float32
//                             lh=font.metrics.lineHeight>0.f?font.metrics.lineHeight:asc+(dsc>0.f?dsc:font.size*.2f);
...
//                     }
//                 }
//                 for(int32 i=numTabDecos-1;i>=0;--i){NkTabDecoration& d=tabDecos[i]; if(!d.enabled||!d.fn)continue;
//                 decoX-=d.width; const NkRect dR={decoX,r.y,d.width,tabH};
//                 dl.AddLine({decoX,r.y+2.f},{decoX,r.y+tabH-2.f},sep,1.f); d.fn(ctx,dl,font,idx,dR,d.userData);}
//             }
...
//                     if(dx*dx+dy*dy>25.f){NkUIWindowState* ws=wm.Find(node.windows[node.activeTab]); if(ws){float32
//                     ww=ws->size.x>10.f?ws->size.x:300.f; UndockWindow(wm,ws->id); ws->isOpen=true;
//                     ws->pos={mp.x-ww*.5f,mp.y-10.f};
//                     wm.activeId=ws->id;wm.movingId=ws->id;wm.moveOffset={ww*.5f,10.f};wm.BringToFront(ws->id);}}
//                 }
//             }

//             // Drag tab → undock
//             if(isDraggingTab&&dragWindowId!=NKUI_ID_NONE){
//                 if(ctx.input.IsMouseDown(0)&&wm.movingId==NKUI_ID_NONE){float32
//                 dx=ctx.input.mouseDelta.x,dy=ctx.input.mouseDelta.y; if(dx*dx+dy*dy>25.f){NkUIWindowState*
//                 ws=wm.Find(dragWindowId); if(ws){float32 ww=ws->size.x>10.f?ws->size.x:300.f;
//                 UndockWindow(wm,ws->id); ws->isOpen=true; ws->pos={mp.x-ww*.5f,mp.y-10.f};
//                 wm.activeId=ws->id;wm.movingId=ws->id;wm.moveOffset={ww*.5f,10.f};wm.BringToFront(ws->id);isDraggingTab=false;dragWindowId=NKUI_ID_NONE;}}}
...
//                 for(int32 t=0;t<node.numWindows;++t){NkUIWindowState* ws=wm.Find(node.windows[t]);
//                 if(ws){ws->pos={r.x,r.y+tabH};ws->size={r.w,r.h-tabH};}}
...
//         void NkUIDockManager::BeginFrame(NkUIContext& ctx, NkUIWindowManager& wm, NkUIDrawList& dl, NkUIFont&)
//         noexcept {
//             if(!isDragging&&wm.movingId!=NKUI_ID_NONE){NkUIWindowState* dws=wm.Find(wm.movingId);
//             if(dws&&!HasFlag(dws->flags,NkUIWindowFlags::NK_NO_DOCK)){isDragging=true;dragWindowId=wm.movingId;}}
//             if(isDragging&&dragWindowId!=NKUI_ID_NONE){
//                 int32 nu=FindNodeAt(ctx.input.mousePos);
//                 if(nu>=0){
//                     dragTargetNode=nu;
//                     const NkRect
//                     tbR={nodes[nu].rect.x,nodes[nu].rect.y,nodes[nu].rect.w,ctx.theme.metrics.dockTabHeight};
...
//         bool NkUIDockManager::BeginDocked(NkUIContext& ctx, NkUIWindowManager& wm, NkUIDrawList& dl, const char*
//         name) noexcept {
...
//         bool NkUIDockManager::BeginChildDock(NkUIContext& ctx, NkUIWindowManager& wm, NkUIDrawList& dl, NkUIFont&
//         font, NkUILayoutStack& ls, NkUIID pwId, NkRect cv) noexcept {
//             NkUIWindowState* pw=wm.Find(pwId); if(!pw) return false;
//             if(!HasFlag(pw->flags,NkUIWindowFlags::NK_ALLOW_DOCK_CHILD)) return false;
//             if(pw->childDockRoot<0||pw->childDockRoot>=numNodes){pw->childDockRoot=AllocNode();
//             if(pw->childDockRoot<0)return false;
//             nodes[pw->childDockRoot].type=NkUIDockNodeType::NK_ROOT;nodes[pw->childDockRoot].rect=cv;nodes[pw->childDockRoot].parent=-1;nodes[pw->childDockRoot].parentWindowId=pwId;}
...
//         }
//         void NkUIDockManager::EndChildDock(NkUIContext&, NkUIDrawList& dl) noexcept { childWindowId=NKUI_ID_NONE;
//         dl.PopClipRect(); }

//         void NkUIDockManager::SaveLayout(const char* path) const noexcept {
//             FILE* f=::fopen(path,"wb"); if(!f)return;
//             ::fprintf(f,"{\"numNodes\":%d,\"rootIdx\":%d,\"nodes\":[\n",numNodes,rootIdx);
//             for(int32 i=0;i<numNodes;++i){const NkUIDockNode& n=nodes[i];
//             ::fprintf(f,"{\"type\":%d,\"rect\":[%.1f,%.1f,%.1f,%.1f],\"ratio\":%.4f,\"parent\":%d,\"c0\":%d,\"c1\":%d,\"nw\":%d,\"at\":%d}%s\n",(int32)n.type,n.rect.x,n.rect.y,n.rect.w,n.rect.h,n.splitRatio,n.parent,n.children[0],n.children[1],n.numWindows,n.activeTab,i<numNodes-1?",":"");}
//             ::fprintf(f,"]\n}\n"); ::fclose(f);
//         }
//         bool NkUIDockManager::LoadLayout(const char* path) noexcept {
...
//                 nodes[i].type=(NkUIDockNodeType)tp;nodes[i].rect={rx,ry,rw,rh};nodes[i].splitRatio=ratio;nodes[i].parent=par;nodes[i].children[0]=c0;nodes[i].children[1]=c1;nodes[i].numWindows=0;nodes[i].activeTab=at;
//                 ++p;
//             }
//             return true;
//         }

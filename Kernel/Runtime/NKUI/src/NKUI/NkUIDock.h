...
namespace nkentseu {
	namespace nkui {
		...
		enum class NkUIDockNodeType : uint8 { NK_ROOT = 0, NK_LEAF, NK_SPLIT_H, NK_SPLIT_V };

		enum class NkUIDockDrop : uint8 { NK_NONE = 0, NK_LEFT, NK_RIGHT, NK_TOP, NK_BOTTOM, NK_CENTER, NK_TAB };
		...
		using NkTabDecoFn = float32 (*)(NkUIContext &, NkUIDrawList &, NkUIFont &, int32 nodeIdx, NkRect decoRect,
										void *userData);
		...
		struct NKUI_API NkUIDockManager {
				...
				bool DockWindow(NkUIWindowManager &wm, NkUIID windowId, int32 nodeIdx, NkUIDockDrop drop) noexcept;
				...
				bool NodeAllowsTabs(const NkUIWindowManager &wm, int32 nodeIdx,
									NkUIID draggedWindowId = NKUI_ID_NONE) const noexcept;

				/// Dessine une ligne de surbrillance sur le bord cible pendant le drag.
				void DrawDirectionalHighlight(NkUIDrawList &dl, int32 nodeIdx, NkUIDockDrop drop,
				...
				/// Noeud racine
				NkUIDockNode *Root() noexcept {
					return rootIdx >= 0 ? &nodes[rootIdx] : nullptr;
				}

				/// Dessine les dropzones et retourne la zone survolee.
				/// Filtre NK_CENTER si les tabs sont interdits pour ce node/fenetre.
				NkUIDockDrop DrawDropZones(NkUIContext &ctx, NkUIDrawList &dl, int32 nodeIdx) noexcept;

				/// Traitement des inputs dock (appele chaque frame avant Begin)
				void BeginFrame(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDrawList &dl, NkUIFont &font) noexcept;

				/// Dessine tout l'arbre de dock (tabs + contenu)
				void Render(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkUIWindowManager &wm,
							NkUILayoutStack &ls, int32 rootOverride = -1) noexcept;
				...
				bool BeginChildDock(NkUIContext &ctx, NkUIWindowManager &wm, NkUIDrawList &dl, NkUIFont &font,
									NkUILayoutStack &ls, NkUIID parentWindowId, NkRect childViewport) noexcept;
			...
			private:
				void RenderNode(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkUIWindowManager &wm, int32 idx,

...
namespace nkentseu {
	namespace nkui {
		...
		struct NKUI_API NkUILayoutSerializer {
				 ...
				 * }
				 */
				static bool Save(const NkUIWindowManager &wm, const NkUIDockManager &dm, const char *path) noexcept;

				static bool Load(NkUIWindowManager &wm, NkUIDockManager &dm, const char *path) noexcept;

				// En mémoire (buffer à libérer avec free())
				static bool SaveToMemory(const NkUIWindowManager &wm, const NkUIDockManager &dm, char *&outJson,
										 usize &outLen) noexcept;

				static bool LoadFromMemory(NkUIWindowManager &wm, NkUIDockManager &dm, const char *json,
										   usize len) noexcept;
		};
		...
		struct NKUI_API NkUIColorPickerFull {
				 ...
				 */
				static bool Draw(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkUILayoutStack &ls,
								 const char *id, NkColor &color) noexcept;

				// Conversions HSV ↔ RGB
				static void RGBtoHSV(uint8 r, uint8 g, uint8 b, float32 &h, float32 &s, float32 &v) noexcept;
				static void HSVtoRGB(float32 h, float32 s, float32 v, uint8 &r, uint8 &g, uint8 &b) noexcept;
			...
			private:
				...
				static void DrawSVSquare(NkUIContext &ctx, NkUIDrawList &dl, NkRect r, float32 h, float32 &s,
										 float32 &v) noexcept;
				static void DrawHueBar(NkUIContext &ctx, NkUIDrawList &dl, NkRect r, float32 &h) noexcept;
				static void DrawAlphaBar(NkUIContext &ctx, NkUIDrawList &dl, NkRect r, float32 h, float32 s, float32 v,
										 float32 &a) noexcept;
				static void DrawPreview(NkUIDrawList &dl, NkRect r, NkColor oldCol, NkColor newCol) noexcept;
		};
		...
		struct NKUI_API NkUIFontNKFont {
				 ...
				 */
				static bool Load(const uint8 *fontData, usize dataSize, float32 pixelSize, NkUIFontAtlas &atlas,
								 NkUIFont &outFont) noexcept;

				/// Charge depuis un fichier .ttf/.otf/.woff/.woff2
				static bool LoadFile(const char *path, float32 pixelSize, NkUIFontAtlas &atlas,
									 NkUIFont &outFont) noexcept;

				/// Ajoute une plage de glyphes à l'atlas (ex: ASCII 32-127, Latin 128-255)
				static bool AddGlyphRange(NkUIFont &font, NkUIFontAtlas &atlas, uint32 first, uint32 last) noexcept;
		};
		...
		struct NKUI_API NkUIOpenGLRenderer {
				...
				uint32 UploadTexture(const uint8 *pixels, int32 w, int32 h, int32 channels) noexcept;

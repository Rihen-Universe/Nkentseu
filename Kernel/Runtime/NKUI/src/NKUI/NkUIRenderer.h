	...
	namespace nkui {
		...
		class NKUI_API NkUIRenderer {
			public:
				...
				virtual uint32 UploadTexture(const uint8 *pixels, int32 w, int32 h, int32 channels) noexcept {
					return 0;
				}
		...
		class NKUI_API NkUICPURenderer final : public NkUIRenderer {
			...
			private:
				...
				void DrawTriangle(const NkUIVertex &v0, const NkUIVertex &v1, const NkUIVertex &v2,
								  NkRect clipRect) noexcept;

				/// Mélange source-over dans le tampon
				void BlendPixel(int32 x, int32 y, uint8 r, uint8 g, uint8 b, uint8 a) noexcept;

				/// Échantillonnage bilinéaire de texture (simplifié)
				NkColor SampleTexture(uint32 texId, float32 u, float32 v) const noexcept;
		};

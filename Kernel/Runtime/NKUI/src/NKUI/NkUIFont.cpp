...
namespace nkentseu {
	namespace nkui {
		...
		bool NkUIFontAtlas::AddGlyph(uint32 codepoint, int32 x, int32 y, int32 w, int32 h, float32 advanceX,
									 float32 bearingX, float32 bearingY) noexcept {
		...
		uint32 NkUIFontManager::AddFromAtlas(const char *name, float32 size, NkUIFontAtlas *atlas,
		...
		int32 NkUIFontManager::AllocFontSlot(NkUIFontManager &mgr) {
			if (mgr.numFonts >= NkUIFontManager::MAX_FONTS)
				return -1;
			if (mgr.numAtlases >= NkUIFontManager::MAX_ATLAS)
				return -1;
			if (mgr.mNumBridges >= NkUIFontManager::MAX_BRIDGES)
				return -1;
		...
		int32 NkUIFontManager::CommitFontSlot(NkUIFontManager &mgr, int32 idx, const char *name, float32 sizePx) {
		...
		int32 NkUIFontManager::LoadFromFile(const char *path, float32 sizePx, const char *name,
			...
			if (!bridge->InitFromFile(&atlases[numAtlases], &fonts[numFonts], path, sizePx, ranges)) {
		...
		int32 NkUIFontManager::LoadFromMemory(const nkft_uint8 *data, nkft_size dataSize, float32 sizePx,
											  const char *name, const nkft_uint32 *ranges) noexcept {
			const int32 idx = AllocFontSlot(*this);
			if (idx < 0)
				return -1;
			...
			if (!bridge->InitFromMemory(&atlases[numAtlases], &fonts[numFonts], data, dataSize, sizePx, ranges)) {
		...
		int32 NkUIFontManager::LoadCustom(const char *path, float32 sizePx, const NkUIFontLoaderDesc &desc,
										  const char *name, const nkft_uint32 *ranges) noexcept {
			if (!desc.IsValid()) {
			...
			if (!bridge->InitCustom(&atlases[numAtlases], &fonts[numFonts], path, sizePx, desc, ranges)) {
				bridge->~NkUIFontBridge();
				return -1;
			}
		...
		// ============================================================================
		void NkUIFont::RenderCharAtlas(NkUIDrawList &dl, NkVec2 pos, const NkUIGlyph *g, NkColor col) const noexcept {
			if (!g || !atlas)
				return;
			if (atlas->texId == 0) {
			...
			dl.AddImage(atlas->texId, {glyphLeft, glyphTop, glyphW, glyphH}, {g->u0, g->v0}, {g->u1, g->v1}, col);
		}
		...
		static inline uint8 Reverse6Bits(uint8 x) noexcept {
			...
			return ((x & 0x20) >> 5) | ((x & 0x10) >> 3) | ((x & 0x08) >> 1) | ((x & 0x04) << 1) | ((x & 0x02) << 3) |
				   ((x & 0x01) << 5);
		}
			...
			if (!config.yAxisUp) {
				...
				for (int32 row = 0; row < kBitmapFontH; ++row) {
					...
					for (int32 col2 = 0; col2 < kBitmapFontW; ++col2) {
						if ((line >> col2) & 1) {
							dl.AddRectFilled({pos.x + col2 * pw, yPos, pw, ph}, col);
						}
					}
				}
			} else {
		...
		// ============================================================================
		void NkUIFont::RenderChar(NkUIDrawList &dl, NkVec2 pos, uint32 cp, NkColor col) const noexcept {
			// pos.y = baseline
		...
		void NkUIFont::RenderText(NkUIDrawList &dl, NkVec2 pos, const char *text, NkColor col, float32 maxWidth,
								  bool ellipsis) const noexcept {
			if (!text || !*text)
				return;
		...
		void NkUIFont::RenderTextWrapped(NkUIDrawList &dl, NkRect bounds, const char *text, NkColor col,
										 float32 lineSpacing) const noexcept {
			if (!text || !*text)
				return;
		...
		int32 NkUIFontManager::LoadEmbedded(NkEmbeddedFontId id, float32 sizePx, const char *name,
											const nkft_uint32 *ranges) noexcept {

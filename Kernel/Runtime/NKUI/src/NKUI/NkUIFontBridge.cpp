...
namespace nkentseu {
	namespace nkui {
		...
		const nkft_uint32 *NkUIFontBridge::RangesASCII() {
			return sRangesASCII;
		}

		const nkft_uint32 *NkUIFontBridge::RangesLatinExtended() {
			return sRangesLatinExt;
		}

		const nkft_uint32 *NkUIFontBridge::RangesDefault() {
			return sRangesLatinExt;
		}
		...
		bool NkUIFontBridge::InitFromFile(NkUIFontAtlas *atlasIn, NkUIFont *fontOut, const char *path,
										  nkft_float32 sizePx, const nkft_uint32 *ranges) {
		...
		bool NkUIFontBridge::InitFromMemory(NkUIFontAtlas *atlasIn, NkUIFont *fontOut, const nkft_uint8 *data,
											nkft_size dataSize, nkft_float32 sizePx, const nkft_uint32 *ranges) {
			if (!atlasIn || !fontOut || !data || dataSize == 0)
				return false;
		...
		bool NkUIFontBridge::BuildAtlas(const nkft_uint32 *ranges, nkft_float32 sizePx) {
			...
			for (nkft_uint32 ri = 0; ranges[ri] && ranges[ri + 1]; ri += 2) {
				for (nkft_uint32 cp = ranges[ri]; cp <= ranges[ri + 1]; ++cp) {
					...
					nkfont::NkGetGlyphBitmapBox(&nkfontFace, gid, scale, scale, 0.f, 0.f, &bx0, &by0, &bx1, &by1);
					...
					if (gw > 0 && gh > 0) {
						...
						nkfont::NkMakeGlyphBitmap(&nkfontFace, dst, gw, gh, NkUIFontAtlas::ATLAS_W, scale, scale, 0.f,
												  0.f, gid);

						// Enregistre le glyphe dans l'atlas
						atlas->AddGlyph(cp, atlasX, atlasY, gw, gh, advanceX, bearingX,
			...
			logger.Info("[NkUIFontBridge] Build: %u glyphes chargés, %u ignorés, asc=%.1f desc=%.1f\n", numLoaded,
						numSkipped, font->metrics.ascender, font->metrics.descender);
		...
		bool NkUIFontBridge::InitCustom(NkUIFontAtlas *atlasIn, NkUIFont *fontOut, const char *path,
										nkft_float32 sizePx, const NkUIFontLoaderDesc &desc,
		...
		bool NkUIFontBridge::BuildAtlasCustom(const nkft_uint32 *ranges, nkft_float32 sizePx) {
			if (!atlas || !font || !ranges || !fontHandle)
				return false;
			...
			for (nkft_uint32 ri = 0; ranges[ri] && ranges[ri + 1]; ri += 2) {
				for (nkft_uint32 cp = ranges[ri]; cp <= ranges[ri + 1]; ++cp) {
					...
					if (gw > 0 && gh > 0) {
						...
						if (!customDesc.GetGlyph(fontHandle, cp, atlas->pixels, NkUIFontAtlas::ATLAS_W, atlasX, atlasY,
												 gw, gh, customDesc.userData, &glyph)) {
							continue;
						}

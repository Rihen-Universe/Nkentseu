...
namespace nkentseu {
	namespace nkui {
		...
		struct NkUIConstraint {
				...
				static NkUIConstraint Auto_() noexcept {
					return {NK_AUTO, 0};
				}

				static NkUIConstraint Fixed_(float32 px) noexcept {
					return {NK_FIXED, px};
				}

				static NkUIConstraint Percent_(float32 p) noexcept {
					return {NK_PERCENT, p};
				}

				static NkUIConstraint Grow_(float32 flex = 1) noexcept {
					return {NK_GROW, flex};
				}
		};
		...
		struct NKUI_API NkUILayoutStack {
				...
				NkUILayoutNode *Top() noexcept {
					return depth > 0 ? &nodes[depth - 1] : nullptr;
				}

				NkUILayoutNode *Push() noexcept {
					return depth < MAX_DEPTH ? &nodes[depth++] : nullptr;
				}
		...
		struct NKUI_API NkUILayout {
				 ...
				 */
				static NkRect NextItemRect(NkUIContext &ctx, NkUILayoutStack &stack, float32 wantW,
										   float32 wantH) noexcept;

				/// Signale que le dernier widget a occupé ce rect (avance le curseur)
				static void AdvanceItem(NkUIContext &ctx, NkUILayoutStack &stack, NkRect placed) noexcept;

				/// Résout la largeur d'un item selon sa contrainte et le layout parent
				static float32 ResolveWidth(const NkUILayoutNode &node, const NkUIConstraint &c, float32 want) noexcept;
				static float32 ResolveHeight(const NkUILayoutNode &node, const NkUIConstraint &c,
											 float32 want) noexcept;

				/// Dessine une scrollbar verticale/horizontale, retourne true si cliquée
				static bool DrawScrollbar(NkUIContext &ctx, NkUIDrawList &dl, bool vertical, NkRect track,
										  float32 contentSize, float32 viewSize, float32 &scroll, NkUIID id) noexcept;

				/// Dessine un splitter draggable, met à jour ratio
				static bool DrawSplitter(NkUIContext &ctx, NkUIDrawList &dl, NkRect rect, bool vertical, float32 &ratio,
										 NkUIID id) noexcept;
		};
	} // namespace nkui
} // namespace nkentseu

...
namespace nkentseu {
	namespace nkui {
		...
		struct NKUI_API NkUIFontAtlas {
				static constexpr int32 ATLAS_W =
					1024; ///< Largeur de l'atlas (512->1024 : marge pour grandes tailles + accents ; atlas par police)
				 ...
				 */
				bool AddGlyph(uint32 codepoint, int32 x, int32 y, int32 w, int32 h, float32 advanceX, float32 bearingX,
							  float32 bearingY) noexcept;
		...
		struct NKUI_API NkUIFont {
				 ...
				 */
				void RenderText(NkUIDrawList &dl, NkVec2 pos, const char *text, NkColor col, float32 maxWidth = -1.f,
								bool ellipsis = false) const noexcept;
				 ...
				 */
				void RenderTextWrapped(NkUIDrawList &dl, NkRect bounds, const char *text, NkColor col,
									   float32 lineSpacing = 1.f) const noexcept;
				 ...
				 */
				static const int32 GetBFH() noexcept {
					return kBitmapFontH;
				}
		...
		struct NKUI_API NkUIFontManager {
				 ...
				 * @return Index de la police
				 */
				uint32 AddFromAtlas(const char *name, float32 size, NkUIFontAtlas *atlas,
				...
				NkUIFont *Get(uint32 idx) noexcept {
					return (idx < static_cast<uint32>(numFonts)) ? &fonts[idx] : (numFonts > 0 ? &fonts[0] : nullptr);
				}

				/**
				 * @brief Récupère la police par défaut
				 */
				NkUIFont *Default() noexcept {
					return numFonts > 0 ? &fonts[0] : nullptr;
				}
			...
			public:
				 ...
				 */
				int32 LoadFromFile(const char *path, float32 sizePx, const char *name = nullptr,
								   const uint32 *ranges = nullptr) noexcept;

				/**
				 * @brief Charge une police depuis un buffer mémoire via NKFont.
				 */
				int32 LoadFromMemory(const uint8 *data, usize dataSize, float32 sizePx, const char *name = nullptr,
									 const uint32 *ranges = nullptr) noexcept;
				 ...
				 */
				int32 LoadCustom(const char *path, float32 sizePx, const NkUIFontLoaderDesc &desc,
								 const char *name = nullptr, const uint32 *ranges = nullptr) noexcept;
				 ...
				 * @return Index de la police dans fonts[], ou -1 si échec.
				 */
				int32 LoadEmbedded(NkEmbeddedFontId id, float32 sizePx, const char *name = nullptr,
								   const uint32 *ranges = nullptr) noexcept;
		};
	} // namespace nkui
} // namespace nkentseu
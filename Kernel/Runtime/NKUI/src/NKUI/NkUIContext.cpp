...
namespace nkentseu {
	namespace nkui {
		...
		bool NkUIContext::Init(int32 vW, int32 vH, const NkUIFontConfig &fontConfig) noexcept {
			if (mInitialized)
				return true;
			if (!fontManager.Init(fontConfig))
				return false;
			...
			for (int32 i = 0; i < LAYER_COUNT; ++i) {
				if (!layers[i].Init(32768, 98304, 256))
					return false;
			}
		...
		void NkUIContext::PushID(const char *str) noexcept {
			if (idDepth < 64)
				idStack[idDepth++] = GetID(str);
		}

		void NkUIContext::PushID(int32 n) noexcept {
			if (idDepth < 64)
				idStack[idDepth++] = GetID(n);
		}
		...
		void NkUIContext::StepAnimations() noexcept {
			...
			for (int32 i = 0; i < numAnims; ++i) {
				...
				// Snap si très proche
				if (::fabsf(a.value - a.target) < 0.001f)
					a.value = a.target;
			}
		...
		void NkUIContext::SetShapeOverride(NkUIWidgetType type, NkUIShapeOverrideFn fn, const void *ud) noexcept {
			...
			if (idx <= 0 || idx >= static_cast<int32>(NkUIWidgetType::NK_COUNT))
				return;
			overrides[idx] = fn;
			overrideUD[idx] = ud;
		}
		...
		void NkUIContext::ClearShapeOverride(NkUIWidgetType type) noexcept {
			const int32 idx = static_cast<int32>(type);
			if (idx > 0 && idx < static_cast<int32>(NkUIWidgetType::NK_COUNT))
				overrides[idx] = nullptr;
		}

		bool NkUIContext::CallShapeOverride(NkUIWidgetType type, NkRect rect, NkUIWidgetState state, const char *label,
											float32 val) noexcept {
			const int32 idx = static_cast<int32>(type);
			if (idx <= 0 || idx >= static_cast<int32>(NkUIWidgetType::NK_COUNT) || !overrides[idx])
				return false;
		...
		void NkUIContext::SameLine(float32 offsetX, float32 spacing) noexcept {
			...
			if (offsetX > 0)
				cursor.x = cursorStart.x + offsetX;
			else
				cursor.x += sp;
		...
		NkColor NkUIContext::GetColor(NkUIWidgetType type, NkUIWidgetState state) const noexcept {
			...
			switch (type) {
				...
				case NkUIWidgetType::NK_INPUT_FLOAT:
					if (HasState(state, NkUIWidgetState::NK_FOCUSED))
						return c.inputBg;
					return c.inputBg;
			...
			while (*p && *p != '}') {
				...
				if (*p == '-' || (*p >= '0' && *p <= '9')) {
					...
					if (::strcmp(key, "corner_radius") == 0)
						theme.metrics.cornerRadius = v;
					else if (::strcmp(key, "item_height") == 0)
						theme.metrics.itemHeight = v;
					else if (::strcmp(key, "padding_x") == 0)
						theme.metrics.paddingX = v;
					else if (::strcmp(key, "padding_y") == 0)
						theme.metrics.paddingY = v;
					else if (::strcmp(key, "spacing") == 0)
						theme.metrics.itemSpacing = v;
					else if (::strcmp(key, "border_width") == 0)
						theme.metrics.borderWidth = v;
					else if (::strcmp(key, "title_height") == 0)
						theme.metrics.titleBarHeight = v;
				}
				// Valeur booléenne
				...
				// Tableau (couleur ou autre)
					...
					if (::strcmp(key, "accent") == 0)
						theme.colors.accent = col;
					else if (::strcmp(key, "bg_primary") == 0)
						theme.colors.bgPrimary = col;
					else if (::strcmp(key, "bg_window") == 0)
						theme.colors.bgWindow = col;
					else if (::strcmp(key, "text") == 0)
						theme.colors.textPrimary = col;
					else if (::strcmp(key, "button_bg") == 0)
						theme.colors.buttonBg = col;
					else if (::strcmp(key, "border") == 0)
						theme.colors.border = col;
					else if (::strcmp(key, "title_bg") == 0)
						theme.colors.titleBarBg = col;
				} else {
					++p;
				}
			}
		...
		void NkUIContext::SaveThemeJSON(const char *path) const noexcept {
			...
			wc(f, "accent", c.accent);
			::fprintf(f, ",\n");
			wc(f, "bg_primary", c.bgPrimary);
			::fprintf(f, ",\n");
			wc(f, "bg_window", c.bgWindow);
			::fprintf(f, ",\n");
			wc(f, "text", c.textPrimary);
			::fprintf(f, ",\n");
			wc(f, "button_bg", c.buttonBg);
			::fprintf(f, ",\n");
			wc(f, "border", c.border);
			::fprintf(f, ",\n");
			wc(f, "title_bg", c.titleBarBg);
			::fprintf(f, "\n");

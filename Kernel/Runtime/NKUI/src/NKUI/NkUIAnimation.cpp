...
namespace nkentseu {
	namespace nkui {
		...
		float32 NkUIEasing::Apply(NkEase ease, float32 t) noexcept {
			...
			switch (ease) {
				case NkEase::NK_LINEAR:
					return Linear(t);
				case NkEase::NK_IN_QUAD:
					return InQuad(t);
				case NkEase::NK_OUT_QUAD:
					return OutQuad(t);
				case NkEase::NK_IN_OUT_QUAD:
					return InOutQuad(t);
				case NkEase::NK_IN_CUBIC:
					return InCubic(t);
				case NkEase::NK_OUT_CUBIC:
					return OutCubic(t);
				case NkEase::NK_IN_OUT_CUBIC:
					return InOutCubic(t);
				case NkEase::NK_IN_QUART:
					return t * t * t * t;
				...
				case NkEase::NK_IN_SINE:
					return 1 - ::cosf(t * NKUI_PI * 0.5f);
				case NkEase::NK_OUT_SINE:
					return ::sinf(t * NKUI_PI * 0.5f);
				case NkEase::NK_IN_OUT_SINE:
					return -(::cosf(NKUI_PI * t) - 1) * 0.5f;
				...
				case NkEase::NK_IN_BOUNCE:
					return 1 - OutBounce(1 - t);
				case NkEase::NK_OUT_BOUNCE:
					return OutBounce(t);
				case NkEase::NK_IN_OUT_BOUNCE:
					return t < 0.5f ? (1 - OutBounce(1 - 2 * t)) * 0.5f : (1 + OutBounce(2 * t - 1)) * 0.5f;
				case NkEase::NK_IN_BACK: {
					const float32 c = 1.70158f;
					const float32 c2 = c + 1;
					return c2 * t * t * t - c * t * t;
				}
				case NkEase::NK_OUT_BACK:
					return OutBack(t);

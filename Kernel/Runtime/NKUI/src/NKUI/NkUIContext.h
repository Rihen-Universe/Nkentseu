...
namespace nkentseu {
	namespace nkui {
		...
		struct NKUI_API NkUIContext {
				...
				bool Init(int32 viewportW = 1280, int32 viewportH = 720,
						  const NkUIFontConfig &fontConfig = {}) noexcept;
				...
				bool IsHot(NkUIID id) const noexcept {
					return hotId == id;
				}

				bool IsActive(NkUIID id) const noexcept {
					return activeId == id;
				}

				bool IsFocused(NkUIID id) const noexcept {
					return focusId == id;
				}

				void SetHot(NkUIID id) noexcept {
					hotId = id;
				}

				void SetActive(NkUIID id) noexcept {
					activeId = id;
				}

				void SetFocus(NkUIID id) noexcept {
					focusId = id;
				}

				void ClearHot() noexcept {
					hotId = NKUI_ID_NONE;
				}
				...
				void SetCursor(NkVec2 pos) noexcept {
					cursor = pos;
				}
				...
				NkVec2 GetCursor() const noexcept {
					return cursor;
				}

				NkVec2 GetCursorStart() const noexcept {
					return cursorStart;
				}

				bool IsMouseClicked(int32 button = 0) const noexcept {
					return button >= 0 && button < 5 && input.IsMouseClicked(button) && !mouseClickConsumed[button];
				}

				bool IsMouseReleased(int32 button = 0) const noexcept {
					return button >= 0 && button < 5 && input.IsMouseReleased(button) && !mouseReleaseConsumed[button];
				}
				...
				void SetMouseCursor(NkUIMouseCursor cursor_) noexcept {
					mouseCursor = cursor_;
				}

				NkUIMouseCursor GetMouseCursor() const noexcept {
					return mouseCursor;
				}

				void SetActiveFont(NkUIFont *font) noexcept {
					activeFont = font;
				}

				NkUIFont *GetActiveFont() const noexcept {
					return activeFont;
				}
				...
				void SetTheme(const NkUITheme &t) noexcept {
					theme = t;
				}

				const NkUITheme &GetTheme() const noexcept {
					return theme;
				}
				...
				float32 GetCornerRadius() const noexcept {
					return theme.metrics.cornerRadius;
				}

				float32 GetItemHeight() const noexcept {
					return theme.metrics.itemHeight;
				}

				float32 GetPaddingX() const noexcept {
					return theme.metrics.paddingX;
				}

				float32 GetPaddingY() const noexcept {
					return theme.metrics.paddingY;
				}

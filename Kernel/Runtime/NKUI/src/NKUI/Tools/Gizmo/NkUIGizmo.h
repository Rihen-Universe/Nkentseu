...
namespace nkentseu {
	namespace nkui {
		...
		enum class NkUIGizmoMode : uint8 { NK_TRANSLATE = 0, NK_ROTATE, NK_SCALE };
		...
		struct NKUI_API NkUIGizmo {
				/// Grille 2D (plan XY)
				static void DrawGrid2D(const NkUIGridConfig &cfg, NkUIDrawList &dl, NkRect viewport,
									   NkVec2 originPx) noexcept;

				/// Grille 3D projetée — infinie si cfg.infinite == true
				static void DrawGrid3D(const NkUIGridConfig &cfg, NkUIDrawList &dl, const NkUIGizmo3DDesc &desc,
									   const NkUIGizmoConfig &gizmoCfg) noexcept;

				static NkUIGizmoResult Manipulate2D(NkUIContext &ctx, NkUIDrawList &dl, const NkUIGizmo2DDesc &desc,
													const NkUIGizmoConfig &cfg, NkUIGizmoState &state,
													NkUIGizmoTransform &transform) noexcept;

				static NkUIGizmoResult Manipulate3D(NkUIContext &ctx, NkUIDrawList &dl, const NkUIGizmo3DDesc &desc,
													const NkUIGizmoConfig &cfg, NkUIGizmoState &state,
													NkUIGizmoTransform &transform) noexcept;
		};

...
namespace nkentseu {
	namespace nkui {
		...
		void NkUITreeState::SetSelected(NkUITreeNodeID id, bool sel, bool clearOthers) noexcept {
			if (clearOthers) {
				numSelected = 0;
			}
			if (sel) {
		...
		static void SafeGetLabel(const NkUITreeCallbacks &cb, NkUITreeNodeID node, char *buf, int32 len) noexcept {
			if (cb.getLabel) {
		...
		static void SafeGetIcon(const NkUITreeCallbacks &cb, NkUITreeNodeID node, char *buf, int32 len) noexcept {
			if (cb.getIcon) {
		...
		void NkUITree::DrawNodeRow(DrawCtx &dc, NkUITreeNodeID node, int32 depth, bool isLastChild,
			...
			const NkRect rowR = {dc.clipRect.x, dc.curY, dc.clipRect.w - kScrollbarW, rowH};
			...
			if (cfg.showConnectors && depth > 0) {
				...
				dl.AddLine({lx, dc.curY + rowH * 0.5f}, {hx1, dc.curY + rowH * 0.5f}, cfg.connectorColor, 1.f);
			}
			...
			if (hasChildren) {
				...
				if (open) {
					// down arrow: ▼
					dl.AddTriangleFilled({arrowX + 2.f, arrowCY - 4.f}, {arrowX + kArrowW - 2.f, arrowCY - 4.f},
										 {arrowX + kArrowW * 0.5f, arrowCY + 3.f}, {200, 200, 200, 200});
				} else {
					// right arrow: ▶
					dl.AddTriangleFilled({arrowX + 2.f, arrowCY - 5.f}, {arrowX + 2.f, arrowCY + 5.f},
										 {arrowX + kArrowW - 2.f, arrowCY}, {200, 200, 200, 200});
				}
			}
			...
			if (state.renaming && state.renamingNode == node) {
				...
				const NkRect inputR = {textX, dc.curY + 1.f, rowR.x + rowR.w - textX - 4.f, rowH - 2.f};
				dl.AddRectFilled(inputR, {30, 30, 40, 240});
				dl.AddRect(inputR, {100, 140, 220, 200}, 1.f);
				dc.font->RenderText(dl, {inputR.x + 2.f, inputR.y + 2.f}, state.renameBuffer, {230, 230, 230, 255});
			...
			const bool inRow = mp.x >= rowR.x && mp.x < rowR.x + rowR.w && mp.y >= rowR.y && mp.y < rowR.y + rowR.h;

			if (inRow) {
				state.hoveredNode = node;

				// Arrow click
				const bool inArrow = hasChildren && mp.x >= arrowX && mp.x < arrowX + kArrowW;
				...
				if (ctx.input.mouseClicked[0]) {
					if (inArrow) {
						...
						dc.result->node = node;
						if (dc.cb->onSelect)
							dc.cb->onSelect(state.selected, state.numSelected, dc.cb->userData);
					}

					// DnD start
					if (cfg.allowDnD && !inArrow && (!dc.cb->canDrag || dc.cb->canDrag(node, dc.cb->userData))) {
				...
				if (state.dndActive && state.dndSource != node) {
					if (!dc.cb->canDrop || dc.cb->canDrop(state.dndSource, node, dc.cb->userData)) {
			...
			if (state.dndActive && !ctx.input.mouseDown[0]) {
				if (state.dndTarget != NKUI_TREE_NODE_NONE && state.dndTarget != state.dndSource) {
					if (dc.cb->onMove)
						dc.cb->onMove(state.dndSource, state.dndTarget, dc.cb->userData);
		...
		void NkUITree::DrawNodeRecursive(DrawCtx &dc, NkUITreeNodeID node, int32 depth, bool isLastChild) noexcept {
			...
			if (dc.curY + rowH < dc.clipRect.y) {
				...
				int32 childCount = dc.cb->getChildCount ? dc.cb->getChildCount(node, dc.cb->userData) : 0;
		...
		static void DrawDnDGhost(NkUIDrawList &dl, NkUIFont &font, NkVec2 mousePos, const char *label) noexcept {
			...
			font.RenderText(dl, {ghost.x + 4.f, ghost.y + 3.f}, label, {220, 220, 220, 200});
		}
		...
		NkUITreeResult NkUITree::Draw(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkUIID id, NkRect rect,
									  NkUITreeNodeID root, const NkUITreeCallbacks &cb, const NkUITreeConfig &cfg,
									  NkUITreeState &state) noexcept {
			const NkUITreeNodeID roots[1] = {root};
			return DrawMultiRoot(ctx, dl, font, id, rect, roots, 1, cb, cfg, state);
		}
		...
		NkUITreeResult NkUITree::DrawMultiRoot(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkUIID id,
											   NkRect rect, const NkUITreeNodeID *roots, int32 rootCount,
											   const NkUITreeCallbacks &cb, const NkUITreeConfig &cfg,
											   NkUITreeState &state) noexcept {
			...
			// Mouse wheel scroll
			const NkVec2 mp = ctx.input.mousePos;
			const bool inRect = mp.x >= rect.x && mp.x < rect.x + rect.w && mp.y >= rect.y && mp.y < rect.y + rect.h;
			...
			if (state.scrollY > maxScroll)
				state.scrollY = maxScroll > 0.f ? maxScroll : 0.f;
			...
			if (contH > visH) {
				...
				const float32 thumbY = rect.y + (state.scrollY / maxScroll) * (rect.h - thumbH);
				const NkRect thumb = {sbX, thumbY, kScrollbarW, thumbH};
				dl.AddRectFilled(thumb, {100, 100, 130, 220});
			}

			// ── Clip rect adjusted for scroll ─────────────────────────────────────
			const NkRect clipR = {rect.x, rect.y, rect.w - kScrollbarW, rect.h};
		...
		bool NkUITree::ExpandToNodeRec(NkUITreeNodeID cur, NkUITreeNodeID target, const NkUITreeCallbacks &cb,
									   NkUITreeState &state) noexcept {
			if (cur == target)
				return true;

			int32 count = cb.getChildCount ? cb.getChildCount(cur, cb.userData) : 0;
		...
		bool NkUITree::ExpandToNode(NkUITreeNodeID root, NkUITreeNodeID target, const NkUITreeCallbacks &cb,
									NkUITreeState &state) noexcept {

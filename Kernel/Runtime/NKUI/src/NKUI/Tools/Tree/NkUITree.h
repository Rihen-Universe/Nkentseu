...
namespace nkentseu {
	namespace nkui {
		...
		using NkUITreeGetChildFn = NkUITreeNodeID (*)(NkUITreeNodeID node, int32 childIndex, void *userData);

		/// Libellé à afficher (écrit dans outLabel, max maxLen bytes).
		using NkUITreeGetLabelFn = void (*)(NkUITreeNodeID node, char *outLabel, int32 maxLen, void *userData);

		/// Icône Unicode/ASCII (ex: "D" pour dossier, "F" pour fichier).
		/// Peut écrire "" pour pas d'icône.
		using NkUITreeGetIconFn = void (*)(NkUITreeNodeID node, char *outIcon, int32 maxLen, void *userData);
		...
		using NkUITreeCanDropFn = bool (*)(NkUITreeNodeID dragged, NkUITreeNodeID target, void *userData);

		/// Appelé quand l'utilisateur valide un renommage.
		using NkUITreeOnRenameFn = void (*)(NkUITreeNodeID node, const char *newName, void *userData);

		/// Appelé quand un noeud est déposé sur un autre (DnD reparent).
		using NkUITreeOnMoveFn = void (*)(NkUITreeNodeID dragged, NkUITreeNodeID newParent, void *userData);

		/// Appelé quand la sélection change. ids = tableau des noeuds sélectionnés.
		using NkUITreeOnSelectFn = void (*)(const NkUITreeNodeID *ids, int32 count, void *userData);
		...
		struct NKUI_API NkUITreeState {
				...
				void SetSelected(NkUITreeNodeID id, bool sel, bool clearOthers = true) noexcept;
		...
		struct NKUI_API NkUITree {
				...
				static NkUITreeResult Draw(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkUIID id, NkRect rect,
				...
				static NkUITreeResult DrawMultiRoot(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkUIID id,
				...
				static bool ExpandToNode(NkUITreeNodeID root, NkUITreeNodeID target, const NkUITreeCallbacks &cb,
										 NkUITreeState &state) noexcept;
			...
			private:
				...
				static void DrawNodeRecursive(DrawCtx &dc, NkUITreeNodeID node, int32 depth, bool isLastChild) noexcept;

				static void DrawNodeRow(DrawCtx &dc, NkUITreeNodeID node, int32 depth, bool isLastChild,
										bool hasChildren) noexcept;

				static bool ExpandToNodeRec(NkUITreeNodeID cur, NkUITreeNodeID target, const NkUITreeCallbacks &cb,
											NkUITreeState &state) noexcept;
		};

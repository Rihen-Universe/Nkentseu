		...
		struct NKUI_API NkUIInputState {
				...
				void BeginFrame() noexcept {
					...
					for (int32 i = 0; i < 5; ++i) {
						...
						mouseDblClick[i] = false;
					}
					for (int32 i = 0; i < (int32)NkKey::NK_KEY_MAX; ++i) {
						keyPressed[i] = false;
						keyReleased[i] = false;
					}
					numInputChars = 0;
				...
				void AddMouseWheel(float32 dy, float32 dx = 0) noexcept {
					mouseWheel += dy;
					mouseWheelH += dx;
				}
				...
				void SetKey(NkKey key, bool down) noexcept {
					...
					if (!down && keyDown[k])
						keyReleased[k] = true;
					keyDown[k] = down;
					if (key == NkKey::NK_LCTRL || key == NkKey::NK_RCTRL)
						ctrl = down;
					if (key == NkKey::NK_LSHIFT || key == NkKey::NK_RSHIFT)
						shift = down;
					if (key == NkKey::NK_LALT || key == NkKey::NK_RALT)
						alt = down;
				}

				void AddInputChar(uint32 c) noexcept {
					if (numInputChars < 32)
						inputChars[numInputChars++] = c;
				}

				bool IsKeyDown(NkKey k) const noexcept {
					return keyDown[static_cast<int32>(k)];
				}

				bool IsKeyPressed(NkKey k) const noexcept {
					return keyPressed[static_cast<int32>(k)];
				}

				bool IsMouseDown(int32 b = 0) const noexcept {
					return b < 5 && mouseDown[b];
				}

				bool IsMouseClicked(int32 b = 0) const noexcept {
					return b < 5 && mouseClicked[b];
				}

				bool IsMouseReleased(int32 b = 0) const noexcept {
					return b < 5 && mouseReleased[b];
				}
		};
	} // namespace nkui
} // namespace nkentseu

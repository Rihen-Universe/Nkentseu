...
namespace nkentseu {
	namespace nkui {
		...
		void NkUIDrawList::Destroy() noexcept {
			...
			idx = nullptr;
			cmds = nullptr;
			vtxCount = idxCount = cmdCount = 0;
			vtxCap = idxCap = cmdCap = 0;
		}
		...
		void NkUIDrawList::ReserveVtx(uint32 n) noexcept {
			if (vtxCount + n > vtxCap) {
				...
				vtx = static_cast<NkUIVertex *>(
					memory::NkRealloc(vtx, vtxCap * sizeof(NkUIVertex), newCap * sizeof(NkUIVertex)));
				vtxCap = newCap;
			}
		}
		...
		void NkUIDrawList::EnsureCmd() noexcept {
			...
			const bool needNew = (cmdCount == 0) || (cmds[cmdCount - 1].type != NkUIDrawCmdType::NK_TRIANGLES) ||
								 (cmds[cmdCount - 1].clipRect.x != clip.x) ||
								 (cmds[cmdCount - 1].clipRect.y != clip.y) ||
								 (cmds[cmdCount - 1].clipRect.w != clip.w) || (cmds[cmdCount - 1].clipRect.h != clip.h);
		...
		void NkUIDrawList::PushClipRect(NkRect r, bool intersect) noexcept {
			if (clipDepth > 0 && intersect)
				r = NkRectIntersect(clipStack[clipDepth - 1], r);
			if (clipDepth < 32)
				clipStack[clipDepth++] = r;
		...
		void NkUIDrawList::AddPolyline(const NkVec2 *pts, int32 n, NkColor col, float32 thickness,
									   bool closed) noexcept {
			if (n < 2)
				return;
			for (int32 i = 0; i < n - 1; ++i)
				AddLine(pts[i], pts[i + 1], col, thickness);
			if (closed)
				AddLine(pts[n - 1], pts[0], col, thickness);
		}
		...
		void NkUIDrawList::AddConvexPolyFilled(const NkVec2 *pts, int32 n, NkColor col) noexcept {
			...
			for (int32 i = 0; i < n; ++i)
				VtxWrite(pts[i], uv, col);
			for (int32 i = 2; i < n; ++i)
				IdxWrite(base, base + i - 1, base + i);
		}
		...
		void NkUIDrawList::AddRectFilled(NkRect r, NkColor col, float32 rx, float32 ry) noexcept {
			...
			if (clipped.w <= 0 || clipped.h <= 0)
				return;
			if (rx <= 0 && ry <= 0) {
				if (IsOccluded(clipped))
					return; // occlusion cull
				EnsureCmd();
				ReserveVtx(4);
				ReserveIdx(6);
				const NkVec2 uv = {-1.f, -1.f}; // sentinel: solid primitive (no texture sample)
		...
		void NkUIDrawList::AddRectFilledMultiColor(NkRect r, NkColor cTL, NkColor cTR, NkColor cBR,
												   NkColor cBL) noexcept {
			if (r.w <= 0 || r.h <= 0)
				return;
			if (cTL.a <= 1 && cTR.a <= 1 && cBR.a <= 1 && cBL.a <= 1)
				return; // alpha threshold
			// Clip rect cull
			NkRect clipped = NkRectIntersect(r, GetClipRect());
			if (clipped.w <= 0 || clipped.h <= 0)
		...
		void NkUIDrawList::AddRectFilledCorners(NkRect r, NkColor col, float32 radius, uint8 cornerMask) noexcept {
			if (r.w <= 0 || r.h <= 0)
				return;
			if (col.a <= 1)
				return; // alpha threshold
			...
			// TR
			if (cornerMask & 2)
				PathArcTo({r.x + r.w - rr, r.y + rrh}, rr, -NKUI_PI * 0.5f, 0);
			else
				PathLineTo({r.x + r.w, r.y});
			// BR
			if (cornerMask & 4)
				PathArcTo({r.x + r.w - rr, r.y + r.h - rrh}, rr, 0, NKUI_PI * 0.5f);
			else
		...
		void NkUIDrawList::AddRect(NkRect r, NkColor col, float32 thickness, float32 rx, float32 ry) noexcept {
			if (r.w <= 0 || r.h <= 0)
				return;
			if (col.a <= 1)
				return; // alpha threshold
		...
		void NkUIDrawList::AddCircleFilled(NkVec2 c, float32 r, NkColor col, int32 segs) noexcept {
			...
			if (col.a <= 1)
				return; // alpha threshold
			{
				NkRect bb = {c.x - r, c.y - r, r * 2, r * 2};
				NkRect clipped = NkRectIntersect(bb, GetClipRect());
				if (clipped.w <= 0 || clipped.h <= 0)
					return;
			}
			if (segs <= 0)
				segs = CalcCircleSegs(r);
			PathClear();
		...
		void NkUIDrawList::AddCircle(NkVec2 c, float32 r, NkColor col, float32 thickness, int32 segs) noexcept {
			if (r <= 0)
				return;
			if (col.a <= 1)
				return; // alpha threshold
		...
		void NkUIDrawList::AddEllipseFilled(NkVec2 c, float32 rx, float32 ry, NkColor col, int32 segs) noexcept {
			if (rx <= 0 || ry <= 0)
				return;
			if (col.a <= 1)
				return; // alpha threshold
			{
				NkRect bb = {c.x - rx, c.y - ry, rx * 2, ry * 2};
				NkRect clipped = NkRectIntersect(bb, GetClipRect());
				if (clipped.w <= 0 || clipped.h <= 0)
					return;
			}
			if (segs <= 0)
				segs = CalcCircleSegs((rx + ry) * 0.5f);
			PathClear();
			for (int32 i = 0; i < segs; ++i) {
				const float32 a = 2.f * NKUI_PI * i / segs;
		...
		void NkUIDrawList::AddEllipse(NkVec2 center, float32 rx, float32 ry, NkColor col, float32 thickness,
									  int32 segs) noexcept {
			if (rx <= 0 || ry <= 0)
				return;
			if (col.a <= 1)
		...
		void NkUIDrawList::AddArc(NkVec2 c, float32 r, float32 a0, float32 a1, NkColor col, float32 thickness,
								  int32 segs) noexcept {
			PathArcTo(c, r, a0, a1, segs);
			PathStroke(col, thickness, false);
		}

		void NkUIDrawList::AddArcFilled(NkVec2 c, float32 r, float32 a0, float32 a1, NkColor col, int32 segs) noexcept {
			pathCount = 0;
			PathLineTo(c);
			PathArcTo(c, r, a0, a1, segs);
			PathFill(col);
		...
		void NkUIDrawList::AddBezierCubic(NkVec2 p0, NkVec2 p1, NkVec2 p2, NkVec2 p3, NkColor col, float32 thickness,
										  int32 segs) noexcept {
			PathClear();
			PathMoveTo(p0);
		...
		void NkUIDrawList::AddBezierQuadratic(NkVec2 p0, NkVec2 p1, NkVec2 p2, NkColor col, float32 thickness,
											  int32 segs) noexcept {
			PathClear();
			PathMoveTo(p0);
		...
		void NkUIDrawList::AddImage(uint32 texId, NkRect dst, NkVec2 uvMin, NkVec2 uvMax, NkColor tint) noexcept {
			if (dst.w <= 0 || dst.h <= 0)
				return;
			if (tint.a <= 1)
				return; // alpha threshold
			...
			if (cmdCount > 0) {
				...
				if (prev.type == NkUIDrawCmdType::NK_TEXTURED_TRIS && prev.texId == texId &&
					prev.clipRect.x == clip.x && prev.clipRect.y == clip.y && prev.clipRect.w == clip.w &&
		...
		void NkUIDrawList::AddImageRounded(uint32 texId, NkRect dst, float32 radius, NkColor tint) noexcept {
		...
		void NkUIDrawList::AddText(NkVec2 pos, const char *text, NkColor col, float32 size, uint32 fId) noexcept {
		...
		void NkUIDrawList::AddTextWrapped(NkRect bounds, const char *text, NkColor col, float32 size,
										  uint32 fId) noexcept {
			(void)bounds;
			(void)text;
			(void)col;
		...
		void NkUIDrawList::PathClear() noexcept {
			pathCount = 0;
		}
		...
		void NkUIDrawList::AddCheckMark(NkVec2 pos, float32 sz, NkColor col) noexcept {
			...
			const NkVec2 pts[3] = {{pos.x, pos.y + h * 0.5f}, {pos.x + sz * 0.35f, pos.y + h}, {pos.x + sz, pos.y}};
		...
		void NkUIDrawList::AddArrow(NkVec2 center, float32 sz, int32 dir, NkColor col) noexcept {
			...
			switch (dir) {
				case 0:
					pts[0] = {center.x - h, center.y - h};
					pts[1] = {center.x + h, center.y};
					pts[2] = {center.x - h, center.y + h};
					break;
				case 1:
					pts[0] = {center.x - h, center.y - h};
					pts[1] = {center.x, center.y + h};
					pts[2] = {center.x + h, center.y - h};
					break;
				case 2:
					pts[0] = {center.x + h, center.y - h};
		...
		void NkUIDrawList::AddShadow(NkRect r, float32 radius, NkColor col, NkVec2 offset) noexcept {
			...
			for (int32 i = 0; i < steps; ++i) {
				...
				AddRectFilled({r.x + offset.x - expand, r.y + offset.y - expand, r.w + expand * 2, r.h + expand * 2},
							  sc, radius);
			}
		}
		...
		void NkUIDrawList::AddColorWheel(NkRect r, float32 hue, float32 sat, float32 val) noexcept {
			...
			for (int32 i = 0; i < segs; ++i) {
				...
				auto hsv = [](float32 H, float32 S, float32 V) -> NkColor {
					if (S <= 0)
						return {uint8(V * 255), uint8(V * 255), uint8(V * 255), 255};
					H *= 6;
					int32 i2 = static_cast<int32>(H);
					float32 f = H - i2;

...
namespace nkentseu {
	namespace nkui {
		...
		struct NKUI_API NkUIEasing {
				...
				static float32 Linear(float32 t) noexcept {
					return t;
				}

				static float32 InQuad(float32 t) noexcept {
					return t * t;
				}

				static float32 OutQuad(float32 t) noexcept {
					return t * (2 - t);
				}
				...
				static float32 InCubic(float32 t) noexcept {
					return t * t * t;
				}
				...
				static float32 OutBounce(float32 t) noexcept {
					...
					if (t < 1 / d)
						return n * t * t;
					if (t < 2 / d) {
						const float32 t2 = t - 1.5f / d;
						return n * t2 * t2 + 0.75f;
					}
					if (t < 2.5f / d) {
						const float32 t2 = t - 2.25f / d;
						return n * t2 * t2 + 0.9375f;
					}
		...
		struct NkUITween {
				...
				bool IsDone() const noexcept {
					return !active && !looping && !pingPong;
				}
		};
		...
		struct NKUI_API NkUIAnimator {
				...
				float32 Play(NkUIID id, float32 from, float32 to, float32 duration, NkEase ease = NkEase::NK_OUT_QUAD,
							 bool loop = false, bool pingPong = false) noexcept {
					NkUITween *t = Find(id);
					if (!t) {
				...
				NkVec2 Shake(NkUIID id, float32 intensity = 5.f, float32 duration = 0.4f) noexcept {
					if (!IsPlaying(id))
						Play(id, 0.f, 1.f, duration, NkEase::NK_OUT_QUAD);
				...
				float32 Pulse(NkUIID id, float32 amplitude = 0.05f, float32 period = 0.8f) noexcept {
					if (!IsPlaying(id))
						Play(id, 0.f, 1.f, period, NkEase::NK_LINEAR, true);
					const float32 t = Get(id);
				...
				float32 FadeOut(NkUIID id, float32 duration = 0.2f) noexcept {
					if (!IsPlaying(id))
						Play(id, 1.f, 0.f, duration, NkEase::NK_IN_QUAD);
					return Get(id, 1.f);
				}
				...
				float32 SlideInFromTop(NkUIID id, float32 h, float32 duration = 0.25f) noexcept {
					if (!IsPlaying(id))
						Play(id, -h, 0.f, duration, NkEase::NK_OUT_BACK);
					return Get(id);
				}
				...
				float32 Bounce(NkUIID id, float32 h = 20.f, float32 duration = 0.4f) noexcept {
					if (!IsPlaying(id))
						Play(id, -h, 0.f, duration, NkEase::NK_OUT_BOUNCE);
					return Get(id);
				}

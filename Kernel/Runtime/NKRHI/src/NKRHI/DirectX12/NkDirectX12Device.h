...
namespace nkentseu {
	...
	namespace NkDX12RootLayout {
		...
		constexpr uint32 NUM_CBV = 32;
		constexpr uint32 OFF_CBV = 0; // b0..b31 → bloc[0..31]
		constexpr uint32 NUM_SRV = 32;
		constexpr uint32 OFF_SRV = OFF_CBV + NUM_CBV; // t0..t31 → bloc[32..63]
		constexpr uint32 NUM_UAV = 8;
		constexpr uint32 OFF_UAV = OFF_SRV + NUM_SRV;			// u0..u7  → bloc[64..71]
		...
		constexpr uint32 NUM_SAMP = 32;
		constexpr uint32 BLOCK_SAMPLER = NUM_SAMP; // 32 samplers / draw
	} // namespace NkDX12RootLayout
	...
	class NkDirectX12Device final : public NkIDevice {
		public:
			...
			NkFramebufferHandle GetSwapchainFramebuffer() const override {
				return mSwapchainFBs[mBackBufferIdx];
			}
			...
			ID3D12CommandAllocator *FrameAlloc() const {
				return mFrameData[mFrameIndex].allocator.Get();
			}
			...
			ID3D12PipelineState *ResolvePipelineForRenderPass(uint64 pipelineId, NkRenderPassHandle rp);

			NkDX12DescHeap &RtvHeap() {
				return mRtvHeap;
			}

			NkDX12DescHeap &DsvHeap() {
				return mDsvHeap;
			}

			NkDX12DescHeap &CbvSrvUavHeap() {
				return mCbvSrvUavHeap;
			} // STAGING (CPU) — source des copies

			NkDX12DescHeap &SamplerHeap() {
				return mSamplerHeap;
			} // STAGING (CPU)

			NkDX12DescHeap &CbvSrvUavRing() {
				return mCbvSrvUavRing;
			} // shader-visible — destination/draw

			NkDX12DescHeap &SamplerRing() {
				return mSamplerRing;
			} // shader-visible
			...
			void TransitionResource(ID3D12GraphicsCommandList *cmd, ID3D12Resource *res, D3D12_RESOURCE_STATES from,
									D3D12_RESOURCE_STATES to);
			void TransitionTextureState(ID3D12GraphicsCommandList *cmd, uint64 textureId, D3D12_RESOURCE_STATES to);
			...
			void ReadbackHDRGridDiag(ID3D12Resource *res, D3D12_RESOURCE_STATES prev, uint32 tw, uint32 th);
			// DIAGNOSTIC (NK_DX12_READBACK>=2) : grille de profondeur sur l'atlas d'ombre.
			void ReadbackDepthGridDiag(ID3D12Resource *res, D3D12_RESOURCE_STATES prev, uint32 tw, uint32 th,
									   uint64 tid);
		...
		private:
			...
			ComPtr<ID3D12PipelineState> BuildGraphicsPSO(const NkGraphicsPipelineDesc &d, ID3D12RootSignature *rootSig,
														 UINT numRT, const DXGI_FORMAT *rtvFormats,
			...
			void RenderPassFormats(NkRenderPassHandle rp, UINT &numRT, DXGI_FORMAT rtvFormats[8],
								   DXGI_FORMAT &dsvFormat) const;
			// FmtSignature : hash compact des formats (pour clé de cache des variantes).
			static uint64 FmtSignature(UINT numRT, const DXGI_FORMAT *rtvFormats, DXGI_FORMAT dsvFormat);
			...
			UINT mNullSrvIdx = UINT_MAX, mNullUavIdx = UINT_MAX, mDefaultCbvIdx = UINT_MAX,
				 mDefaultSamplerIdx = UINT_MAX;

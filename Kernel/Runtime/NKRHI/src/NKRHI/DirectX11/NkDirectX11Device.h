...
namespace nkentseu {
	...
	struct NkDX11Texture {
			...
			ID3D11Resource *Resource() const {
				return tex3d ? static_cast<ID3D11Resource *>(tex3d) : static_cast<ID3D11Resource *>(tex);
			}
	};

	struct NkDX11Sampler {
	...
	class NkDirectX11Device final : public NkIDevice {
		public:
			...
			// Accès interne
			ID3D11Device1 *D3D() const {
				return mDevice;
			}

			ID3D11DeviceContext1 *Ctx() const {
				return mContext;
			}
			...
			ID3D11SamplerState *const *PoolSamplers() const {
				return mPoolSamplers;
			}

			// Cbuffer dynamique d'émulation des push constants (registre b13).
			ID3D11Buffer *PushConstantCB() const {
				return mPushConstantCB;
			}

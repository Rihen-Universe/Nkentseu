...
namespace nkentseu {
	...
	void NkDirectX11CommandBuffer::End() {
		if (mDev)
			mDev->DrainInfoQueue(); // DIAG : log validation AVANT FinishCommandList (qui crashe)
		if (mDeferred)
			mDeferred->FinishCommandList(FALSE, &mCmdList);
		if (mDev)
			mDev->DrainInfoQueue(); // DIAG : log validation générée par FinishCommandList
	}
	...
	// Render Pass
	// =============================================================================
	bool NkDirectX11CommandBuffer::BeginRenderPass(NkRenderPassHandle /*rp*/, NkFramebufferHandle fb,
												   const NkRect2D &area) {
		if (!mDeferred || !fb.IsValid() || area.width <= 0 || area.height <= 0)
			return false;
		...
		if (mPendingClearColor)
			for (uint32 i = 0; i < fbo->rtvCount; i++)
				if (fbo->rtvs[i])
					mDeferred->ClearRenderTargetView(fbo->rtvs[i], mClearColor);
		if (mPendingClearDepth && fbo->dsv)
			mDeferred->ClearDepthStencilView(fbo->dsv, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, mClearDepth,
											 (UINT8)mClearStencil);
		mPendingClearColor = false;
		mPendingClearDepth = false;

		D3D11_VIEWPORT vp{(float)area.x, (float)area.y, (float)area.width, (float)area.height, 0.f, 1.f};
	...
	void NkDirectX11CommandBuffer::SetScissors(const NkRect2D *rects, uint32 n) {
		...
		n = math::NkMin(n, (uint32)D3D11_VIEWPORT_AND_SCISSORRECT_OBJECT_COUNT_PER_PIPELINE);
		for (uint32 i = 0; i < n; i++)
			d[i] = {rects[i].x, rects[i].y, (LONG)(rects[i].x + rects[i].width), (LONG)(rects[i].y + rects[i].height)};
	...
	void NkDirectX11CommandBuffer::BindGraphicsPipeline(NkPipelineHandle p) {
		...
		if (pipe->il)
			mDeferred->IASetInputLayout(pipe->il);
		if (pipe->rs)
			mDeferred->RSSetState(pipe->rs);
		if (pipe->dss)
			mDeferred->OMSetDepthStencilState(pipe->dss, 0);
		if (pipe->bs) {
	...
	// Descriptor Set
	// =============================================================================
	void NkDirectX11CommandBuffer::BindDescriptorSet(NkDescSetHandle set, uint32 /*idx*/, uint32 * /*off*/,
													 uint32 /*cnt*/) {
	...
	// =============================================================================
	void NkDirectX11CommandBuffer::PushConstants(NkShaderStage /*stages*/, uint32 offset, uint32 size,
												 const void *data) {
		if (!data || size == 0)
			return;
	...
	// =============================================================================
	void NkDirectX11CommandBuffer::BindVertexBuffer(uint32 binding, NkBufferHandle buf, uint64 offset) {
		ID3D11Buffer *b = mDev->GetDXBuffer(buf.id);
		UINT stride = (binding < D3D11_IA_VERTEX_INPUT_RESOURCE_SLOT_COUNT) ? mVertexStrides[binding] : 0;
		UINT off = (UINT)offset;
		mDeferred->IASetVertexBuffers(binding, 1, &b, &stride, &off);
	}

	void NkDirectX11CommandBuffer::BindVertexBuffers(uint32 first, const NkBufferHandle *bufs, const uint64 *offsets,
													 uint32 n) {
	...
	void NkDirectX11CommandBuffer::BindIndexBuffer(NkBufferHandle buf, NkIndexFormat fmt, uint64 offset) {
		DXGI_FORMAT dxFmt = fmt == NkIndexFormat::NK_UINT16 ? DXGI_FORMAT_R16_UINT : DXGI_FORMAT_R32_UINT;
	...
	// Copies
	// =============================================================================
	void NkDirectX11CommandBuffer::CopyBuffer(NkBufferHandle src, NkBufferHandle dst, const NkBufferCopyRegion &r) {
	...
	void NkDirectX11CommandBuffer::BeginDebugGroup(const char *name, float, float, float) {
		...
		if (SUCCEEDED(mDeferred->QueryInterface(__uuidof(ID3DUserDefinedAnnotation), (void **)&ann))) {
			wchar_t wname[256] = {};

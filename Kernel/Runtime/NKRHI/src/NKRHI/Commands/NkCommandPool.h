...
namespace nkentseu {
	...
	class NkCommandPool {
		public:
			...
			explicit NkCommandPool(NkIDevice *device, NkCommandBufferType type = NkCommandBufferType::NK_GRAPHICS)
			...
			NkCommandPool(NkCommandPool &&o) noexcept
				: mDevice(o.mDevice), mType(o.mType), mFree(traits::NkMove(o.mFree)),
				  mInFlight(traits::NkMove(o.mInFlight)) {
				o.mDevice = nullptr;
			}

			// ── Init tardive (si construit par défaut) ─────────────────────────────
			void Init(NkIDevice *device, NkCommandBufferType type = NkCommandBufferType::NK_GRAPHICS) {
				mDevice = device;
				mType = type;
			}
			...
			void Reset() {
				...
				threading::NkScopedLockMutex lock(mMutex);
				for (usize i = 0; i < mInFlight.Size(); i++)
					mDevice->DestroyCommandBuffer(mInFlight[i]);
				for (usize i = 0; i < mFree.Size(); i++)
					mDevice->DestroyCommandBuffer(mFree[i]);
				mInFlight.Clear();
				mFree.Clear();
			}
			...
			uint32 FreeCount() const {
				return (uint32)mFree.Size();
			}

			uint32 InFlightCount() const {
				return (uint32)mInFlight.Size();
			}

			uint32 TotalCount() const {
				return FreeCount() + InFlightCount();
			}

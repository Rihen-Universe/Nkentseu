...
namespace nkentseu {
	namespace swbridge {
		...
		// =============================================================================
		static AttrSemantic AutoSemantic(uint32 location, NkSLBaseType type, bool hasPos, bool hasCol, bool hasUV,
										 bool hasNorm) {
			if (location == 0)
				return AttrSemantic::POSITION;

			if (location == 1) {
				if (type == NkSLBaseType::NK_VEC2)
					return AttrSemantic::UV;
				if (type == NkSLBaseType::NK_VEC3 || type == NkSLBaseType::NK_VEC4 || type == NkSLBaseType::NK_UVEC4)
					return AttrSemantic::COLOR;
			}

			if (location == 2) {
				if (!hasUV && type == NkSLBaseType::NK_VEC2)
					return AttrSemantic::UV;
				if (!hasCol && (type == NkSLBaseType::NK_VEC3 || type == NkSLBaseType::NK_VEC4))
					return AttrSemantic::COLOR;
				if (!hasNorm && type == NkSLBaseType::NK_VEC3)
					return AttrSemantic::NORMAL;
			}
		...
		static NkVector<AttrDesc> BuildLayout(const NkSLReflection &vertRefl, uint32 &outStride,
			...
			for (uint32 i = 0; i < (uint32)sorted.Size(); ++i) {
				...
				if (mapping && mapping->isSet[a.location] && mapping->semantics[a.location] != AttrSemantic::AUTO) {
				...
				if (a.semantic == AttrSemantic::POSITION)
					hasPos = true;
				if (a.semantic == AttrSemantic::COLOR)
					hasCol = true;
				if (a.semantic == AttrSemantic::UV)
					hasUV = true;
				if (a.semantic == AttrSemantic::NORMAL)
					hasNorm = true;
		...
		static inline void ReadAttr(const uint8 *src, const AttrDesc &a, float &x, float &y, float &z, float &w) {
			x = y = z = 0.f;
			w = 1.f;
			switch (a.type) {
				case NkSLBaseType::NK_FLOAT:
				...
				case NkSLBaseType::NK_VEC2:
					x = RdF(src, a.offset);
					y = RdF(src, a.offset + 4);
					break;
				case NkSLBaseType::NK_VEC3:
					x = RdF(src, a.offset);
					y = RdF(src, a.offset + 4);
					z = RdF(src, a.offset + 8);
					break;
				case NkSLBaseType::NK_VEC4:
					x = RdF(src, a.offset);
					y = RdF(src, a.offset + 4);
					z = RdF(src, a.offset + 8);
					w = RdF(src, a.offset + 12);
					break;
				case NkSLBaseType::NK_UVEC4: {
		...
		// =============================================================================
		static void ProjMV(const float *m, float vx, float vy, float vz, float &ox, float &oy, float &oz, float &ow) {
		...
		// =============================================================================
		static void SampleTexNN(const NkSWTexture *tex, float u, float v, float &r, float &g, float &b, float &a) {
			r = g = b = a = 1.f;
			if (!tex || tex->mips.Empty())
				return;
			...
			float uf = u - floorf(u);
			float vf = v - floorf(v);
			const int tx = (int)(uf * w);
			const int ty = (int)(vf * h);
		...
		// =============================================================================
		static NkVertexShaderSoftware MakeVertexFn(const NkVector<AttrDesc> &attrs, uint32 stride, bool hasProj) {
			// Log de débogage (une seule fois)
			NKSW_LOG("MakeVertexFn: stride=%u, %u attrs, proj=%s", stride, (uint32)attrs.Size(),
					 hasProj ? "yes" : "no");
			for (uint32 i = 0; i < (uint32)attrs.Size(); ++i) {
				...
				NKSW_LOG("  [%u] loc=%u off=%u bytes=%u -> %s", i, attrs[i].location, attrs[i].offset, attrs[i].bytes,
						 semStr);
			}

			return [attrs, stride, hasProj](const void *vdata, uint32 idx, const void *udata) -> NkVertexSoftware {
				...
				for (uint32 i = 0; i < (uint32)attrs.Size(); ++i) {
					...
					switch (attrs[i].semantic) {
						...
						case AttrSemantic::COLOR:
							...
							cg = y;
							cb = z;
							ca = (attrs[i].type == NkSLBaseType::NK_VEC4 || attrs[i].type == NkSLBaseType::NK_UVEC4)
									 ? w
									 : 1.f;
							break;
						case AttrSemantic::UV:
						...
						case AttrSemantic::GENERIC: {
							...
							if (slot + 3 < 16) {
								...
								out.attrs[slot + 2] = z;
								out.attrs[slot + 3] = w;
								if (out.attrCount < slot + 4)
									out.attrCount = slot + 4;
							}
							break;
						}
						default:
			...
			if (texCount == 1) {
				...
				return [](const NkVertexSoftware &f, const void *, const void *tp) -> math::NkVec4 {
					...
					if (tp) {
						...
						tex = batch->tex[0];
						if (!tex)
							tex = static_cast<const NkSWTexture *>(tp); // fallback
					}
					float r, g, b, a;
		...
		// =============================================================================
		static NkSWBridgeResult NkCompileSources(const NkString &vertSrc, const NkString &fragSrc,
			...
			// ── Vertex ────────────────────────────────────────────────────────────────
			NkSLCompileOptions vo;
			vo.entryPoint = "vert_main";
			auto vr =
				compiler.CompileWithReflection(vertSrc, NkSLStage::NK_VERTEX, NkSLTarget::NK_CPLUSPLUS, vo, vertName);
			...
			// ── Fragment ──────────────────────────────────────────────────────────────
			NkSLCompileOptions fo;
			fo.entryPoint = "frag_main";
			auto fr =
				compiler.CompileWithReflection(fragSrc, NkSLStage::NK_FRAGMENT, NkSLTarget::NK_CPLUSPLUS, fo, fragName);
			...
			NKSW_LOG("  stride=%u attrs=%u proj=%s textures=%u ubos=%u", res.stride, (uint32)attrs.Size(),
		...
		static NkSWBridgeResult NkCompileFiles(const char *vertPath, const char *fragPath,
			...
			if (!LoadFile(vertPath, vs)) {
				NkSWBridgeResult r;
				r.error = NkString("Cannot open: ") + vertPath;
				NKSW_ERR("%s", r.error.CStr());
				return r;
			}
			if (!LoadFile(fragPath, fs)) {
				NkSWBridgeResult r;
				r.error = NkString("Cannot open: ") + fragPath;
				NKSW_ERR("%s", r.error.CStr());
				return r;
			}
		...
		// =============================================================================
		static NkSWBridgeResult NkCompileFile(const char *path, const NkSWVertexMapping *mapping = nullptr) {
			NkString src;
			if (!LoadFile(path, src)) {
				NkSWBridgeResult r;
				r.error = NkString("Cannot open: ") + path;
				NKSW_ERR("%s", r.error.CStr());
				return r;
			}
		...
		static NkSWBridgeResult NkCompile(const NkString &vertSrc, const NkString &fragSrc,
										  ...
										  const NkSWVertexMapping *mapping = nullptr) {
			return NkCompileSources(vertSrc, fragSrc, name + ".vert", name + ".frag", mapping);
		}

		// Surcharge fichier unique (compatibilité)
		static NkSWBridgeResult NkCompile(const NkString &src, const NkString &name = "inline.sw.sksl",
										  uint32 /*hint*/ = 0) {

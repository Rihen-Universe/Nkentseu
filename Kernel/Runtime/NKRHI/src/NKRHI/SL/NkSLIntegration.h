...
namespace nkentseu {
	namespace nksl {
		...
		//   NK_GFX_API_DX11 → NK_HLSL_DX11 (SM5, fxc)
		//   NK_GFX_API_DX12 → NK_HLSL_DX12 (SM6, dxc)
		...
		inline NkSLTarget ApiToGLSLTarget(NkGraphicsApi api) {
			switch (api) {
				case NkGraphicsApi::NK_GFX_API_VULKAN:
					return NkSLTarget::NK_GLSL_VULKAN;
				default:
					return NkSLTarget::NK_GLSL;
			}
		}
		...
		inline NkSLTarget ApiToHLSLTarget(NkGraphicsApi api) {
			if (api == NkGraphicsApi::NK_GFX_API_DX12)
				return NkSLTarget::NK_HLSL_DX12;
			return NkSLTarget::NK_HLSL_DX11;
		}
		...
		NkShaderHandle CreateShaderFromSource(NkIDevice *device, const NkString &source,
											  const NkVector<NkSLStage> &stages, const NkString &name = "shader",
		...
		// Reflection seule
		// =============================================================================
		NkSLReflection GetReflection(const NkString &source, NkSLStage stage, const NkString &filename = "shader");

		// =============================================================================
		// NkShaderDesc
		// =============================================================================
		bool BuildShaderDesc(NkGraphicsApi api, const NkString &source, const NkVector<NkSLStage> &stages,
							 NkShaderDesc &outDesc, const NkString &name = "shader",
							 const NkSLCompileOptions &opts = {});

		bool BuildShaderDescWithReflection(NkGraphicsApi api, const NkString &source, const NkVector<NkSLStage> &stages,
										   NkShaderDesc &outDesc, NkSLReflection &outReflection,
										   const NkString &name = "shader", const NkSLCompileOptions &opts = {});
		...
		// =============================================================================
		NkString GenerateLayoutCPP(const NkString &source, NkSLStage stage, const NkString &varName = "layout",
								   const NkString &filename = "shader");

		NkString GenerateLayoutJSON(const NkString &source, NkSLStage stage, const NkString &filename = "shader");

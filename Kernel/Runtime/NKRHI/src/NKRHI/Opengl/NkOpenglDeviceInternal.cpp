...
namespace nkentseu {
	...
	void NkOpenglApplyRenderState(NkOpenGLDevice *dev, uint64 pipelineId) {
		...
		if (bl.attachments.Size() > 0) {
			...
			if (a.blendEnable) {
				glEnable(GL_BLEND);
				glBlendFuncSeparate(toGLBF(a.srcColor), toGLBF(a.dstColor), toGLBF(a.srcAlpha), toGLBF(a.dstAlpha));
				glBlendEquationSeparate(toGLBO(a.colorOp), toGLBO(a.alphaOp));
			} else {
				glDisable(GL_BLEND);
			}
			glColorMask((a.colorWriteMask & 1) ? GL_TRUE : GL_FALSE, (a.colorWriteMask & 2) ? GL_TRUE : GL_FALSE,
						(a.colorWriteMask & 4) ? GL_TRUE : GL_FALSE, (a.colorWriteMask & 8) ? GL_TRUE : GL_FALSE);
		}
#else
		for (uint32 i = 0; i < bl.attachments.Size(); i++) {
			...
			if (a.blendEnable) {
				...
				glBlendFuncSeparatei(i, toGLBF(a.srcColor), toGLBF(a.dstColor), toGLBF(a.srcAlpha), toGLBF(a.dstAlpha));
				glBlendEquationSeparatei(i, toGLBO(a.colorOp), toGLBO(a.alphaOp));
			} else {
				glDisablei(GL_BLEND, i);
			}
			glColorMaski(i, (a.colorWriteMask & 1) ? GL_TRUE : GL_FALSE, (a.colorWriteMask & 2) ? GL_TRUE : GL_FALSE,
						 (a.colorWriteMask & 4) ? GL_TRUE : GL_FALSE, (a.colorWriteMask & 8) ? GL_TRUE : GL_FALSE);
		}
#endif
		glBlendColor(bl.blendConstants[0], bl.blendConstants[1], bl.blendConstants[2], bl.blendConstants[3]);

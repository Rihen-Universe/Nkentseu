...
namespace nkentseu {
	...
	class NkVulkanDevice final : public NkIDevice {
		public:
			...
			NkFramebufferHandle GetSwapchainFramebuffer() const override {
				if (mSwapchainFBs.Empty())
					return {};
				if (mCurrentImageIdx >= mSwapchainFBs.Size())
					return {};
		...
		private:
			...
			void TransitionImage(VkCommandBuffer cmd, VkImage img, VkImageLayout from, VkImageLayout to,
								 VkImageAspectFlags aspect = VK_IMAGE_ASPECT_COLOR_BIT, uint32 baseMip = 0,
								 uint32 mips = 1, uint32 baseLayer = 0, uint32 layers = 1);
	...
	class NkVulkanDevice final : public NkIDevice {
		public:
			bool Initialize(const NkDeviceInitInfo &) override {
				return false;
			}
			...
			NkRenderPassHandle GetFramebufferRenderPass(NkFramebufferHandle) const override {
				return {};
			}
			...
			NkDescSetHandle AllocateDescriptorSet(NkDescSetHandle) override {
				return {};
			}
			...
			NkFenceHandle CreateFence(bool) override {
				return {};
			}
			...
			bool BeginFrame(NkFrameContext &) override {
				return false;
			}

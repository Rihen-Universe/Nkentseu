...
namespace nkentseu {
	...
	inline bool NkSwapchainFormatIsSrgb(NkSwapchainFormat f) {
		return f == NkSwapchainFormat::NK_SWAPCHAIN_BGRA8_SRGB || f == NkSwapchainFormat::NK_SWAPCHAIN_RGBA8_SRGB;
	}
	...
	enum class NkPFDPixelType : uint8 { NK_PFD_PIXEL_RGBA = 0, NK_PFD_PIXEL_COLOR_INDEX = 1 };
	...
	enum class NkGLProfile : uint32 { NK_GL_PROFILE_CORE = 0, NK_GL_PROFILE_COMPATIBILITY = 1, NK_GL_PROFILE_ES = 2 };
	...
	struct NkOpenGLDesc {
			...
			static NkOpenGLDesc Desktop46(bool dbg = false) {
				...
				d.contextFlags =
					NkGLContextFlags::NK_GL_CONTEXT_FLAG_FORWARD_COMPAT |

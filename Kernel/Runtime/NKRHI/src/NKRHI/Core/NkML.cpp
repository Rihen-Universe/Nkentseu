...
namespace nkentseu {
	...
	// =============================================================================
	NkTensor NkMLContext::CreateTensor(const NkTensorShape &shape, bool requiresGrad, const char *name) {
		NkTensor t;
		t.shape = shape;
	...
	void NkMLContext::DestroyTensor(NkTensor &t) {
		if (t.buffer.IsValid())
			mDevice->DestroyBuffer(t.buffer);
		if (t.grad.IsValid())
			mDevice->DestroyBuffer(t.grad);
		t = NkTensor{};
	}
	...
	void NkMLContext::FillZero(NkTensor &t) {
		auto *cmd = mDevice->CreateCommandBuffer();
		cmd->Begin();
		cmd->ClearBuffer(t.buffer, 0);
		cmd->End();
		mDevice->Submit(&cmd, 1);
		mDevice->DestroyCommandBuffer(cmd);
	}

	void NkMLContext::FillOnes(NkTensor &t) {
		FillConst(t, 1.f);
	}

	void NkMLContext::FillConst(NkTensor &t, float32 val) {
		NkVector<float32> v;
		v.Resize(t.NumElements());
	...
	// =============================================================================
	NkPipelineHandle NkMLContext::GetOrCompilePipeline(const char *key, const char *glslSrc) {
	...
	// =============================================================================
	void NkMLContext::Dispatch1D(NkICommandBuffer *cmd, NkPipelineHandle pipe, uint32 n, uint32 groupSize) {
	...
	// MatMul A×B → C
	// =============================================================================
	void NkMLContext::MatMul(const NkTensor &A, const NkTensor &B, NkTensor &C, bool transA, bool transB) {
		...
		struct PC {
				...
				float32 alpha;
				int transA, transB;
		} pc{(int)M, (int)N, (int)K, 1.f, (int)transA, (int)transB};
	...
	// =============================================================================
	void NkMLContext::MatMulAdd(const NkTensor &A, const NkTensor &B, const NkTensor &bias, NkTensor &C) {
...
void main(){
	...
	// =============================================================================
	static void RunUnary(NkMLContext *ml, NkIDevice *dev, NkDescSetHandle layout, const char *key, const char *src,
...
void main(){uint i=gl_GlobalInvocationID.x;if(i<n)y[i]=x[i]>0?x[i]:alpha*x[i];}
	...
	// LayerNorm
	// =============================================================================
	void NkMLContext::LayerNorm(const NkTensor &in, const NkTensor &weight, const NkTensor &bias2, NkTensor &out,
								float32 eps) {
		...
		cmd->BindDescriptorSet(set, 0);
		uint32 cols = in.Cols();
		uint32 rows = in.NumElements() / cols;
	...
	void NkMLContext::AdamWStep(NkTensor &param, const NkTensor &grad, AdamState &state, const AdamConfig &cfg) {
		state.t++;
		...
		struct {
				uint32 n;
				float32 lr, beta1, beta2, eps, wd, beta1t, beta2t;
		} pc{param.NumElements(), cfg.lr, cfg.beta1, cfg.beta2, cfg.eps, cfg.weightDecay, beta1t, beta2t};
	...
	float32 NkMLContext::MSELoss(const NkTensor &pred, const NkTensor &target) {
		...
		cmd->PushConstants(NkShaderStage::NK_COMPUTE, 0, sizeof(n), &n);
		cmd->Dispatch((n + 255) / 256, 1, 1);
		cmd->UAVBarrier(mScratch.buffer);
		cmd->End();
		mDevice->Submit(&cmd, 1);
		mDevice->FreeDescriptorSet(set);
		mDevice->DestroyCommandBuffer(cmd);
	...
	// =============================================================================
	void NkMLContext::SoftmaxCrossEntropyBackward(const NkTensor &logits, const NkTensor &labels,
												  NkTensor &gradLogits) {
		...
		cmd->PushConstants(NkShaderStage::NK_COMPUTE, 0, sizeof(pc), &pc);
		cmd->Dispatch((n + 255) / 256, 1, 1);
		cmd->UAVBarrier(gradLogits.buffer);
		cmd->End();
		mDevice->Submit(&cmd, 1);
		mDevice->FreeDescriptorSet(set);
		mDevice->DestroyCommandBuffer(cmd);
	...
	void NkMLContext::PrintTensor(const NkTensor &t, uint32 maxElems, const char *prefix) {
		...
		if (t.NumElements() > maxElems)
			logger_src.Infof(" ...");
		logger_src.Infof("]\n");
	}

	bool NkMLContext::AllClose(const NkTensor &A, const NkTensor &B, float32 atol, float32 rtol) {
		if (A.shape != B.shape)
			return false;
		NkVector<float32> va;
		va.Resize(A.NumElements());
		NkVector<float32> vb;
		vb.Resize(B.NumElements());
		Download(A, va.Data());
		Download(B, vb.Data());
		for (uint32 i = 0; i < va.Size(); i++) {
			float32 diff = fabsf(va[i] - vb[i]);
			if (diff > atol + rtol * fabsf(vb[i]))
				return false;
		}
		return true;
	}
	...
	void NkMLContext::Copy(const NkTensor &src, NkTensor &dst) {
		...
		auto *cmd = mDevice->CreateCommandBuffer();
		cmd->Begin();
		cmd->CopyBuffer(src.buffer, dst.buffer, r);
		cmd->End();
		mDevice->Submit(&cmd, 1);
		mDevice->DestroyCommandBuffer(cmd);
	}
	...
	// MatMulBackward
	void NkMLContext::MatMulBackward(const NkTensor &A, const NkTensor &B, const NkTensor &gradC, NkTensor &gradA,
									 NkTensor &gradB) {
		// gradA = gradC × B^T
	...
	// ReLUBackward
	void NkMLContext::ReLUBackward(const NkTensor &out, const NkTensor &gradOut, NkTensor &gradIn) {

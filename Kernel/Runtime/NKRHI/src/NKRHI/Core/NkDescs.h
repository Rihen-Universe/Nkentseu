...
namespace nkentseu {
	...
	struct NkBufferDesc {
			...
			static NkBufferDesc Vertex(uint64 sz, const void *data = nullptr) {
				return {sz, NkBufferType::NK_VERTEX, NkResourceUsage::NK_IMMUTABLE, NkBindFlags::NK_VERTEX_BUFFER,
						data};
			}
	...
	struct NkTextureDesc {
			...
			// ── Helpers ──
			static NkTextureDesc Tex2D(uint32 w, uint32 h, NkGPUFormat fmt = NkGPUFormat::NK_RGBA8_SRGB,
									   uint32 mips = 1) {
				return {NkTextureType::NK_TEX2D, fmt, w, h, 1, 1, mips};
			}

			static NkTextureDesc RenderTarget(uint32 w, uint32 h, NkGPUFormat fmt = NkGPUFormat::NK_RGBA8_SRGB,
			...
			static NkTextureDesc DepthStencil(uint32 w, uint32 h, NkGPUFormat fmt = NkGPUFormat::NK_D32_FLOAT,
											  NkSampleCount msaa = NkSampleCount::NK_S1) {
				NkTextureDesc d;
				d.type = NkTextureType::NK_TEX2D;
				d.format = fmt;
			...
			static NkTextureDesc Cubemap(uint32 sz, NkGPUFormat fmt = NkGPUFormat::NK_RGBA16_FLOAT, uint32 mips = 0) {
				return {NkTextureType::NK_CUBE,			fmt, sz, sz, 1, 6, mips, NkSampleCount::NK_S1,
						NkBindFlags::NK_SHADER_RESOURCE};
			}

			static NkTextureDesc Tex3D(uint32 w, uint32 h, uint32 d2, NkGPUFormat fmt = NkGPUFormat::NK_R32_FLOAT) {
				return {NkTextureType::NK_TEX3D,
						fmt,
						w,
						h,
						d2,
	...
	struct NkSamplerDesc {
			...
			// Presets
			static NkSamplerDesc Linear() {
				return {};
			}

			static NkSamplerDesc Nearest() {
				NkSamplerDesc d;
				d.magFilter = NkFilter::NK_NEAREST;
				d.minFilter = NkFilter::NK_NEAREST;
				d.mipFilter = NkMipFilter::NK_NEAREST;
				return d;
			}
			...
			static NkSamplerDesc Shadow() {
				...
				d.compareOp = NkCompareOp::NK_LESS_EQUAL;
	...
	struct NkVertexLayout {
			...
			// Builder fluide
			NkVertexLayout &AddAttribute(uint32 loc, uint32 bind, NkVertexFormat fmt, uint32 off,
	...
	struct NkVertexSoftware {
			...
			math::NkVec4 GetAttrVec4(uint32 base) const {
				return (base + 3 < 16) ? math::NkVec4{attrs[base], attrs[base + 1], attrs[base + 2], attrs[base + 3]}
									   : math::NkVec4{};
			}
	};

	// Signature des shaders logiciels
	using NkVertexShaderSoftware =
		NkFunction<NkVertexSoftware(const void *vertexData, uint32 vertexIndex, const void *uniformData)>;

	using NkPixelShaderSoftware = NkFunction<math::NkVec4f(const NkVertexSoftware &interpolated,
														   const void *uniformData, const void *texSampler)>;

	using NkComputeShaderSoftware =
		NkFunction<void(uint32 groupCountX, uint32 groupCountY, uint32 groupCountZ, const void *uniformData)>;
	...
	struct NkRasterizerDesc {
			...
			static NkRasterizerDesc Default() {
				return {};
			}

			static NkRasterizerDesc NoCull() {
				NkRasterizerDesc d;
				d.cullMode = NkCullMode::NK_NONE;
				return d;
			}

			static NkRasterizerDesc Wireframe() {
				NkRasterizerDesc d;
			...
			static NkRasterizerDesc ShadowMap() {
				...
				d.cullMode = NkCullMode::NK_FRONT;
				return d;
			}
	};
	...
	struct NkDepthStencilDesc {
			...
			static NkDepthStencilDesc Default() {
				return {};
			}

			static NkDepthStencilDesc NoDepth() {
				NkDepthStencilDesc d;
				d.depthTestEnable = false;
				d.depthWriteEnable = false;
				return d;
			}

			static NkDepthStencilDesc ReadOnly() {
				NkDepthStencilDesc d;
				d.depthWriteEnable = false;
				return d;
			}

			static NkDepthStencilDesc DepthEqual() {
				NkDepthStencilDesc d;
				d.depthCompareOp = NkCompareOp::NK_EQUAL;
				d.depthWriteEnable = false;
				return d;
			}

			static NkDepthStencilDesc ReverseZ() {
				NkDepthStencilDesc d;
				d.depthCompareOp = NkCompareOp::NK_GREATER;
				return d;
			}
	};
	...
	struct NkBlendAttachment {
			...
			static NkBlendAttachment Opaque() {
				return {};
			}
	...
	struct NkBlendDesc {
			...
			NkBlendDesc() {
				attachments.PushBack(NkBlendAttachment::Opaque());
			}

			static NkBlendDesc Opaque() {
				return NkBlendDesc{};
			}

			static NkBlendDesc Alpha() {
				NkBlendDesc d;
				d.attachments[0] = NkBlendAttachment::Alpha();
				return d;
			}

			static NkBlendDesc Additive() {
				NkBlendDesc d;
				d.attachments[0] = NkBlendAttachment::Additive();
				return d;
			}
	};
	...
	struct NkShaderStageDesc {
			...
			const void *cpuFn =
				nullptr; // NkSWVertexShader* / NkPixelShaderSoftware* / NkSWComputeShader* (software rasterizer)
	...
	struct NkShaderDesc {
			...
			NkShaderDesc &AddSWVertex(const NkVertexShaderSoftware &fn) {
				...
				s.cpuFn = nkentseu::memory::NkGetDefaultAllocator().New<NkVertexShaderSoftware>(
					fn); // libéré dans CreateShader
				stages.PushBack(s);
				return AddStage(s);
			}
	...
	struct NkComputePipelineDesc {
			...
			NkComputePipelineDesc &AddPushConstant(NkShaderStage stages, uint32 offset, uint32 size) {
				pushConstants.PushBack({stages, offset, size});
				return *this;
			}
	};
	...
	struct NkDescriptorSetLayoutDesc {
			...
			NkDescriptorSetLayoutDesc &Add(uint32 binding, NkDescriptorType type,
										   NkShaderStage stages = NkShaderStage::NK_ALL_GRAPHICS, uint32 count = 1) {
				bindings.PushBack({binding, type, count, stages});
				return *this;
			}

			// Binding d'un tableau de N ressources — pour bindless descriptor indexing
			NkDescriptorSetLayoutDesc &AddBindless(uint32 binding, NkDescriptorType type, NkShaderStage stages,
												   uint32 maxCount) {
				isBindless = true;
				return Add(binding, type, stages, maxCount);
			}

			uint32 TotalBindingCount() const {

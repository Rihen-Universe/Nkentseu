...
namespace nkentseu {
	...
	bool NkComputeContext::Init(NkIDevice *device) {
		...
		logger_src.Infof("[NkComputeContext] Init OK | async=%s | maxGroupX=%u | sharedMem=%uKB\n",
						 SupportsAsyncCompute() ? "oui" : "non", caps.maxComputeGroupSizeX,
	...
	NkPipelineHandle NkComputeContext::GetOrCreatePipeline(const char *key, NkShaderHandle shader,
	...
	NkPipelineHandle NkComputeContext::GetOrCompileGLSL(const char *key, const char *glslSrc, const char *debugName) {
		NkString k(key);
		auto *entry = mPipelineCache.Find(k);
		if (entry != nullptr) {
			return entry->pipeline;
	...
	void NkComputeContext::ClearPipelineCache() {
		for (auto &pair : mPipelineCache) {
			if (pair.Second.pipeline.IsValid())
				mDevice->DestroyPipeline(pair.Second.pipeline);
			if (pair.Second.shader.IsValid())
				mDevice->DestroyShader(pair.Second.shader);
		}
	...
	void NkComputeContext::SetPipeline(NkPipelineHandle pipeline) {
		if (!mInPass || !mCmd)
			return;
		if (mBoundPipeline == pipeline && mPipelineBound)
			return; // déjà lié
	...
	void NkComputeContext::BindBuffer(uint32 binding, NkBufferHandle buf, uint64 offset, uint64 range) {
		NkBoundResource r;
		r.binding = binding;
	...
	void NkComputeContext::BindUniformBuffer(uint32 binding, NkBufferHandle buf, uint64 offset, uint64 range) {
		NkBoundResource r;
		r.binding = binding;
	...
	void NkComputeContext::BindSampledTexture(uint32 binding, NkTextureHandle tex, NkSamplerHandle sampler) {
		NkBoundResource r;
		r.binding = binding;
	...
	void NkComputeContext::BindDescriptorSet(NkDescSetHandle set, uint32 setIndex, uint32 *dynamicOffsets,
	...
	void NkComputeContext::Dispatch2D(uint32 width, uint32 height, uint32 tileX, uint32 tileY) {
		Dispatch(NkComputeGroups(width, tileX), NkComputeGroups(height, tileY), 1);
	}

	void NkComputeContext::Dispatch3D(uint32 width, uint32 height, uint32 depth, uint32 tileX, uint32 tileY,
									  uint32 tileZ) {
		Dispatch(NkComputeGroups(width, tileX), NkComputeGroups(height, tileY), NkComputeGroups(depth, tileZ));
	}
	...
	void NkComputeContext::UAVBarrier(NkTextureHandle tex) {
		if (!mCmd)
			return;
		NkTextureBarrier b{tex, NkResourceState::NK_UNORDERED_ACCESS, NkResourceState::NK_UNORDERED_ACCESS,
						   NkPipelineStage::NK_COMPUTE_SHADER, NkPipelineStage::NK_COMPUTE_SHADER};
		mCmd->Barrier(nullptr, 0, &b, 1);
	}

	void NkComputeContext::TransitionForCompute(NkTextureHandle tex, NkResourceState before) {
		if (!mCmd)
			return;
		mCmd->TextureBarrier(tex, before, NkResourceState::NK_UNORDERED_ACCESS, NkPipelineStage::NK_ALL_COMMANDS,
							 NkPipelineStage::NK_COMPUTE_SHADER);
	}

	void NkComputeContext::TransitionForGraphics(NkTextureHandle tex, NkResourceState after) {
		if (!mCmd)
			return;
		mCmd->TextureBarrier(tex, NkResourceState::NK_UNORDERED_ACCESS, after, NkPipelineStage::NK_COMPUTE_SHADER,
	...
	NkFenceHandle NkComputeContext::SubmitAsync(NkICommandBuffer *cmd, NkSemaphoreHandle waitSemaphore,
	...
	void NkComputeContext::WaitForFence(NkFenceHandle fence, uint64 timeoutNanos) {
		if (!mDevice || !fence.IsValid())
			return;
		mDevice->WaitFence(fence, timeoutNanos);
		mDevice->DestroyFence(fence);
	}

	void NkComputeContext::SyncComputeToGraphics(NkSemaphoreHandle computeSignal, NkICommandBuffer *graphicsCmd) {

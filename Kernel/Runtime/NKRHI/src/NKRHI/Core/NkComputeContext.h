...
namespace nkentseu {
	...
	class NkComputeContext {
		public:
			...
			void BeginPass(NkICommandBuffer *cmd, const NkComputePassDesc &desc = {});
			...
			NkICommandBuffer *GetCurrentCmd() const {
				return mCmd;
			}
			...
			NkPipelineHandle GetOrCreatePipeline(const char *key, NkShaderHandle shader,
												 const char *debugName = nullptr);

			// Variante : compile le shader GLSL inline et met en cache.
			// Compatible OpenGL 4.3+, Vulkan (via SPIRV-Cross si disponible).
			NkPipelineHandle GetOrCompileGLSL(const char *key, const char *glslSrc, const char *debugName = nullptr);
			...
			void BindBuffer(uint32 binding, NkBufferHandle buf, uint64 offset = 0, uint64 range = 0);

			// Uniform Buffer — lecture seule, taille ≤ 64 Ko sur toutes les APIs.
			void BindUniformBuffer(uint32 binding, NkBufferHandle buf, uint64 offset = 0, uint64 range = 0);
			...
			void BindSampledTexture(uint32 binding, NkTextureHandle tex, NkSamplerHandle sampler);

			// Bind un descriptor set déjà construit (chemin avancé).
			// Annule et remplace les bindings accumulés par les méthodes ci-dessus.
			void BindDescriptorSet(NkDescSetHandle set, uint32 setIndex = 0, uint32 *dynamicOffsets = nullptr,
			...
			template <typename T> void PushConstants(const T &data, uint32 offset = 0) {
			...
			void Dispatch2D(uint32 width, uint32 height, uint32 tileX = 16, uint32 tileY = 16);

			// 3D — couvre un volume W×H×D avec des tuiles 3D.
			// Idéal : simulation de fluides, voxels, navigation 3D
			void Dispatch3D(uint32 width, uint32 height, uint32 depth, uint32 tileX = 8, uint32 tileY = 8,
							uint32 tileZ = 8);
			...
			void TransitionForCompute(NkTextureHandle tex, NkResourceState before = NkResourceState::NK_SHADER_READ);

			// Transition d'une texture vers SHADER_READ (après écriture compute).
			// Typiquement : résultat d'un compute → lu en fragment shader.
			void TransitionForGraphics(NkTextureHandle tex, NkResourceState after = NkResourceState::NK_SHADER_READ);
			...
			NkFenceHandle SubmitAsync(NkICommandBuffer *cmd, NkSemaphoreHandle waitSemaphore = {},
									  NkSemaphoreHandle signalSemaphore = {});

			// Attend côté CPU que la fence soit signalée.
			// timeoutNanos = UINT64_MAX → attente infinie.
			void WaitForFence(NkFenceHandle fence, uint64 timeoutNanos = UINT64_MAX);
			...
			void SyncComputeToGraphics(NkSemaphoreHandle computeSignal, NkICommandBuffer *graphicsCmd);
			...
			const NkComputePassStats &GetLastPassStats() const {
				return mLastStats;
			}
	...
	class NkComputeBuilder {
		public:
			...
			NkComputeBuilder &Buffer(uint32 binding, NkBufferHandle buf, uint64 offset = 0, uint64 range = 0) {
				mCtx.BindBuffer(binding, buf, offset, range);
				return *this;
			}

			NkComputeBuilder &Uniform(uint32 binding, NkBufferHandle buf, uint64 offset = 0, uint64 range = 0) {
			...
			NkComputeBuilder &SampledTex(uint32 binding, NkTextureHandle tex, NkSamplerHandle sampler) {
				mCtx.BindSampledTexture(binding, tex, sampler);
				return *this;
			}

			template <typename T> NkComputeBuilder &Push(const T &data, uint32 offset = 0) {
			...
			NkComputeBuilder &Dispatch2D(uint32 w, uint32 h, uint32 tx = 16, uint32 ty = 16) {
				mCtx.Dispatch2D(w, h, tx, ty);
				return *this;
			}

			NkComputeBuilder &Dispatch3D(uint32 w, uint32 h, uint32 d, uint32 tx = 8, uint32 ty = 8, uint32 tz = 8) {

...
#if (defined(NKENTSEU_WINDOWING_XLIB) || defined(NKENTSEU_FORCE_WINDOWING_XLIB_ONLY)) &&                               \
...
#endif
#if (defined(NKENTSEU_WINDOWING_WAYLAND) || defined(NKENTSEU_FORCE_WINDOWING_WAYLAND_ONLY)) &&                         \
...
namespace nkentseu {
	...
	inline NkGraphicsApi NkDeviceInitApi(const NkDeviceInitInfo &init) {
		if (init.api != NkGraphicsApi::NK_GFX_API_NONE)
			return init.api;

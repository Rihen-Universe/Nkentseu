...
namespace nkentseu {
	...
	class NkGrid3D {
		public:
			...
			void SetConfig(const NkGrid3DConfig &cfg) {
				mConfig = cfg;
			}

			const NkGrid3DConfig &GetConfig() const {
				return mConfig;
			}

			void Draw(NkICommandBuffer *cmd, const math::NkMat4 &viewMatrix, const math::NkMat4 &projMatrix,
					  uint32 width, uint32 height);

		private:
			bool CreatePipeline(NkRenderPassHandle renderPass);
			void UpdatePushConstants(NkICommandBuffer *cmd, const math::NkMat4 &invView, const math::NkMat4 &invProj);

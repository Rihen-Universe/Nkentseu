...
namespace nkentseu {
	...
	void NkGrid3D::Shutdown() {
		if (mDevice) {
			...
			if (mDescriptorSet.IsValid())
				mDevice->FreeDescriptorSet(mDescriptorSet);
			if (mDescriptorSetLayout.IsValid())
				mDevice->DestroyDescriptorSetLayout(mDescriptorSetLayout);
			if (mDummySampler.IsValid())
				mDevice->DestroySampler(mDummySampler);
		}
		mDevice = nullptr;
		mInitialized = false;
	}
	...
	bool NkGrid3D::CreatePipeline(NkRenderPassHandle renderPass) {
		...
		pipelineDesc.AddPushConstant(NkShaderStage::NK_VERTEX | NkShaderStage::NK_FRAGMENT, 0,
									 sizeof(float) * 4 + sizeof(int) * 2 + sizeof(float) * 6 + sizeof(uint32) +
										 sizeof(float) * 4 * 5);
	...
	void NkGrid3D::UpdatePushConstants(NkICommandBuffer *cmd, const math::NkMat4 &invView,
	...
	void NkGrid3D::Draw(NkICommandBuffer *cmd, const math::NkMat4 &viewMatrix, const math::NkMat4 &projMatrix,

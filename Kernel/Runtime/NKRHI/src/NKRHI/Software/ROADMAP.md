...
- **2026-07-06** — Ombres portées VSM software (3 parties : support viewport dans le rasterizer sans régression, passe Shadow depth-only `lightVP·model·pos`, échantillonnage atlas + PCF 3×3 dans le fragment géométrie ; cohérence écriture/lecture par construction) **+ instancing software** (`gl_InstanceID` via `SwCurInstance`, re-gen des sommets par instance → ombres des cubes instanciés `ShadowInstanced` lisant l'InstanceUBO). Limites : 1 directionnelle, biais fixe ; coût interne ~130→280 ms/image (mesuré : le PCF n'est PAS le goulot, c'est le fragment d'éclairage par pixel ; toggle `NK_SW_PCF=2`). Publication + article : `D:\Rihen\Rodolf\Publications\09_2026-07-06_software-shadows\`. Fiche : `BugReports/Backends-Rendu/software-ombres-vsm-viewport.md`.
- **2026-07-06** — HDR + bloom software. Cibles de rendu couleur HDR en **RGBA32F float** (chemin `colorFloat` sans clamp : état `colorFloat`/`colorBpp`, `OutputPixelF`, strides `W*colorBpp`, Read/Write/clear float, propagation `ResolveResources`→`RasterizeList`). Le tonemap ACES lit enfin le vrai HDR (plus d'écrêtage [0,1] avant tonemap). Chemin 8-bit **inchangé** (validé byte-identique). + **bloom mono-passe** dans le tonemap (bright-pass 2 anneaux 16 taps, seuil ~2.2, toggle `NK_SW_NOBLOOM`) → halo sur les hautes lumières. Limites : approximation 1-passe (vs Dual-Kawase), anneaux, ~60 ms, IBL manquant (cubemaps absents). Publication + article + **vidéo** : `D:\Rihen\Rodolf\Publications\10_2026-07-06_software-hdr-bloom\`.

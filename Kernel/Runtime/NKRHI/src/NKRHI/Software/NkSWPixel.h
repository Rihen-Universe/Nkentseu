...
namespace nkentseu {
	namespace sw_detail {
		...
		NK_FORCE_INLINE void BlendPixel(uint8 *__restrict p, uint8 r, uint8 g, uint8 b, uint8 a) noexcept {
			if (a == 0u)
				return;
			if (a == 255u) {
				StorePixel(p, r, g, b, 255u);
				return;
			}
		...
		// =============================================================================
		NK_FORCE_INLINE void FillSpanOpaque(uint8 *__restrict row, int32 xStart, int32 xEnd, uint8 r, uint8 g,
											uint8 b) noexcept {
			if (xStart >= xEnd)
				return;
		...
		// =============================================================================
		NK_FORCE_INLINE void BlendSpanAdd(uint8 *__restrict row, int32 xStart, int32 xEnd, uint8 r, uint8 g, uint8 b,
										  uint8 a) noexcept {
			if (xStart >= xEnd || a == 0u)
				return;
		...
		// =============================================================================
		NK_FORCE_INLINE void BlendSpanAlpha(uint8 *__restrict row, int32 xStart, int32 xEnd, uint8 r, uint8 g, uint8 b,
											uint8 a) noexcept {
			if (xStart >= xEnd || a == 0u)
				return;
			if (a == 255u) {
				FillSpanOpaque(row, xStart, xEnd, r, g, b);
				return;
			}
...
#if NK_SW_PIXEL_BGRA
				__m128i src16 =
					_mm_set_epi16(0, (int16_t)r, (int16_t)g, (int16_t)b, 0, (int16_t)r, (int16_t)g, (int16_t)b);
#else
				__m128i src16 =
					_mm_set_epi16(0, (int16_t)b, (int16_t)g, (int16_t)r, 0, (int16_t)b, (int16_t)g, (int16_t)r);
#endif
				__m128i blended = _mm_srli_epi16(
					_mm_add_epi16(_mm_add_epi16(_mm_mullo_epi16(src16, vAlpha), _mm_mullo_epi16(dst16, vInvAlpha)),
								  _mm_set1_epi16(127)),
					8);

				const __m128i vAlpha255 = _mm_set_epi16(255, 0, 0, 0, 255, 0, 0, 0);
				blended = _mm_or_si128(_mm_andnot_si128(_mm_set_epi16(-1, 0, 0, 0, -1, 0, 0, 0), blended), vAlpha255);
			...
			while (count >= 1) {
				...
				uint16x8_t vBlend =
					vaddq_u16(vmull_u8(vSrc, vdup_n_u8((uint8_t)sa)), vmull_u8(vDst, vdup_n_u8((uint8_t)inv)));
				// >> 8 (approx /255)
		...
		// =============================================================================
		NK_FORCE_INLINE void ClearRow(uint8 *__restrict row, uint32 width, uint8 r, uint8 g, uint8 b,
									  uint8 a = 255u) noexcept {

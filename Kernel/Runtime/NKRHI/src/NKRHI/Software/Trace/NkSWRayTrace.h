...
namespace nkentseu {
	namespace swtrace {
		...
		static inline V3 operator*(const V3 &a, float s) {
		...
		static inline float AxisVal(const V3 &v, int a) {
		...
		static inline float RayAABB(const V3 &o, const V3 &invD, const V3 &lo, const V3 &hi, float tMax) {
			...
			tmin = fmaxf(tmin, fminf(t1, t2));
			tmax = fminf(tmax, fmaxf(t1, t2));
			if (tmax >= fmaxf(tmin, 0.f) && tmin < tMax)
				return tmin < 0.f ? 0.f : tmin;
			return -1.f;
		}
		...
		struct BVH {
				...
				V3 Centroid(int t) const {
					const Tri &tr = tris[t];
					return V3((tr.v0.x + tr.v1.x + tr.v2.x) / 3.f, (tr.v0.y + tr.v1.y + tr.v2.y) / 3.f,
				...
				int Subdivide(int start, int count) {
					...
					while (i <= j) {
						if (AxisVal(Centroid(idx[i]), axis) < split)
							++i;
						else {
							const int tmp = idx[i];
							idx[i] = idx[j];
					...
					if (leftCount == 0 || leftCount == count)
						leftCount = count / 2; // fallback médian
				...
				int Closest(const Ray &r, float tMin, float &tBest) const {
					...
					while (sp > 0) {
						...
						if (RayAABB(r.o, invD, nd.lo, nd.hi, tBest) < 0.f)
							continue;
						if (nd.count > 0) {
							for (int i = 0; i < nd.count; ++i) {
								const int ti = idx[nd.start + i];
								float t;
								if (IntersectMT(r, tris[ti], tMin, t) && t < tBest) {
									tBest = t;
									hit = ti;
								}
							}
				...
				bool Any(const Ray &r, float tMin, float maxT) const {
					...
					while (sp > 0) {
						const BVHNode &nd = nodes[stack[--sp]];
						if (RayAABB(r.o, invD, nd.lo, nd.hi, maxT) < 0.f)
							continue;
						if (nd.count > 0) {
							for (int i = 0; i < nd.count; ++i) {
								const int ti = idx[nd.start + i];
								float t;
								if (IntersectMT(r, tris[ti], tMin, t) && t < maxT)
									return true;
							}
						} else if (sp < 62) {
							stack[sp++] = nd.left;
		...
		static inline void Basis(const V3 &n, V3 &t, V3 &b) {
			if (fabsf(n.x) > fabsf(n.z))
				t = Normalize(V3(-n.y, n.x, 0.f));
			else
				t = Normalize(V3(0.f, -n.z, n.y));
			b = Cross(n, t);
		}
		...
		static inline float AmbientOcclusion(const V3 &p, const V3 &nrm, const BVH &bvh, const Quality &q) {
			...
			for (int k = 0; k < q.ao; ++k) {
				...
				ar.o = p + nrm * 1e-3f;
				ar.d = dir;
				if (bvh.Any(ar, 1e-3f, q.aoRadius))
					++occ;
			}
		...
		static inline V3 Shade(const V3 &hitP, const V3 &nrm, const V3 &albedo, const V3 &lightDir, const BVH &bvh,
							   const Quality &q) {
		...
		static V3 TraceRay(const Ray &r, const BVH &bvh, const Tri *tris, const V3 &lightDir, const V3 &sky, int depth,
						   const Quality &q) {
			float t;
		...
		static inline void RenderScene(uint8 *out, int W, int H, const math::NkMat4f &view, const math::NkMat4f &proj,
									   const Tri *tris, int n, const V3 &lightDir, const V3 &skyColor,
									   const Quality &q = Quality{},
									   bool bgra = false) // bgra: backbuffer natif Windows
		{
			...
			auto renderRow = [&](int y) {
				for (int x = 0; x < W; ++x) {
					...
					const uint8 R = (uint8)(Clamp01(c.x) * 255.f + .5f), G = (uint8)(Clamp01(c.y) * 255.f + .5f),
								B = (uint8)(Clamp01(c.z) * 255.f + .5f);
					if (bgra) {
						o[0] = B;
						o[1] = G;
						o[2] = R;
			...
			static const bool s_rtNoMT = []() {
				const char *e = std::getenv("NK_SW_RT_NOMT");
				return e && e[0] == '1';
			}();
		...
		static inline int BuildDemoScene(Tri *tris, int cap) {
			...
			const float m = -0.7f, p = 0.7f, y0 = 0.3f, y1 = 1.7f;
			const V3 rc(0.85f, 0.22f, 0.18f);
			...
			const V3 sc(2.3f, 1.0f, -0.2f);
			const float rad = 1.0f;
			const V3 sb(0.20f, 0.45f, 0.85f);
			const int STK = 24, SLI = 32;
			auto sp = [&](float th, float ph) {
				return V3(sc.x + rad * sinf(th) * cosf(ph), sc.y + rad * cosf(th), sc.z + rad * sinf(th) * sinf(ph));
			};
			for (int i = 0; i < STK; ++i) {
				const float t0 = PI * i / STK, t1 = PI * (i + 1) / STK;
				for (int j = 0; j < SLI; ++j) {
					const float p0 = 2 * PI * j / SLI, p1 = 2 * PI * (j + 1) / SLI;
		...
		static inline void RenderLive(uint8 *backbuf, int W, int H, uint32 frame, bool bgra) {
			...
			const math::NkMat4f view =
				math::NkMat4f::LookAt(eye, math::NkVec3f(0.9f, 0.8f, -0.1f), math::NkVec3f(0.f, 1.f, 0.f));
			const math::NkMat4f proj =
				math::NkMat4f::Perspective(math::NkAngle(45.f), (float)W / (float)H, 0.1f, 100.f);

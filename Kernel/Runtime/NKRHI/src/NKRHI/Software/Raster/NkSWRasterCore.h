	...
	namespace swraster {
		...
		struct ClipVert {
				...
				static ClipVert From(const NkVertexSoftware &v) {
					...
					for (uint32 i = 0; i < c.attrCount; ++i)
						c.attrs[i] = v.attrs[i];
					return c;
				}
		...
		static inline ClipVert LerpClip(const ClipVert &a, const ClipVert &b, float32 t) {
			...
			for (uint32 i = 0; i < r.attrCount; ++i)
		...
		// =============================================================================
		static inline float32 NearDist(const ClipVert &v) {
		...
		// =============================================================================
		static inline void BlendFactor(NkBlendFactor f, float32 sr, float32 sg, float32 sb, float32 sa, float32 dr,
									   float32 dg, float32 db, float32 da, float32 &fr, float32 &fg, float32 &fb,
									   float32 &fa) {
			switch (f) {
		...
		static inline float32 Clamp01(float32 v) {
			return v < 0.f ? 0.f : (v > 1.f ? 1.f : v);
		}

		// Écrit un pixel (src en 0..1) selon l'état de blend
		static inline void OutputPixel(uint8 *p, const NkSWRenderState &st, float32 sr, float32 sg, float32 sb,
									   float32 sa) {
			if (st.blendEnable) {
				...
				if (a8 == 255u)
					sw_detail::StorePixel(p, r8, g8, b8, a8);
				else if (a8 > 0u)
					sw_detail::BlendPixel(p, r8, g8, b8, a8);
			}
		}
		...
		static inline void OutputPixelF(float32 *fp, const NkSWRenderState &st, float32 sr, float32 sg, float32 sb,
										float32 sa) {
			if (st.blendEnable) {
				...
				fp[1] = sg * sfg + dg * dfg;
				fp[2] = sb * sfb + db * dfb;
				fp[3] = sa * sfa + da * dfa; // pas de Clamp01 (HDR)
			} else {
				if (sa >= 1.f) {
					fp[0] = sr;
					fp[1] = sg;
		...
		static inline ScreenVert Project(const ClipVert &v, float32 W, float32 H, const NkSWViewport *vp = nullptr) {
			...
			const float32 invW = (aw > 1e-6f) ? (1.f / v.pos.w) : ((v.pos.w >= 0.f) ? 1e6f : -1e6f);
			...
			if (vp && vp->w > 0.f) { // viewport sous-rect (NDC -> rect du viewport)
				...
				s.sy = vp->flipY ? (vp->y + (1.f - ndcy) * 0.5f * vp->h) : (vp->y + (ndcy * 0.5f + 0.5f) * vp->h);
			...
			for (uint32 i = 0; i < s.attrCount; ++i)
				s.attrs[i] = v.attrs[i];
			return s;
		}
		...
		// =============================================================================
		static inline void ReadTexel(const uint8 *d, int32 texW, int32 texH, uint32 bpp, int32 x, int32 y, float32 &r,
									 float32 &g, float32 &b, float32 &a) {
		...
		static inline void SampleNearest(const uint8 *d, int32 texW, int32 texH, uint32 bpp, float32 u, float32 v,
		...
		static inline void SampleBilinear(const uint8 *d, int32 texW, int32 texH, uint32 bpp, float32 u, float32 v,
										  float32 &r, float32 &g, float32 &b, float32 &a) {
		...
		// =============================================================================
		static void RasterScreenTriangle(const ScreenVert &A, const ScreenVert &B, const ScreenVert &C, uint8 *colorBuf,
										 float32 *depthBuf, uint32 W, uint32 H, int32 clipX0, int32 clipY0,
										 int32 clipX1, int32 clipY1, const NkSWShader *shader, const void *uniformData,
										 const void *texPtr, const NkSWTexBatch *texBatch, const NkSWRenderState &st) {
			...
			if (st.cullMode != NkCullMode::NK_NONE) {
				...
				if (st.cullMode == NkCullMode::NK_BACK && !isFront)
					return;
				if (st.cullMode == NkCullMode::NK_FRONT && isFront)
					return;
			}

			// Normaliser pour area>0 afin d'unifier le test barycentrique
			const ScreenVert *v0 = &A;
			const ScreenVert *v1 = &B;
			const ScreenVert *v2 = &C;
			if (area < 0.f) {
				const ScreenVert *t = v1;
				v1 = v2;
				v2 = t;
				area = -area;
			}
			...
			int32 xMin = (int32)floorf(fMinX);
			int32 xMax = (int32)ceilf(fMaxX);
			int32 yMin = (int32)floorf(fMinY);
			int32 yMax = (int32)ceilf(fMaxY);
			if (xMin < clipX0)
				xMin = clipX0;
			if (xMax > clipX1 - 1)
				xMax = clipX1 - 1;
			...
			uint32 texBpp = 0;
			if (!hasShader && !depthOnly && texBatch && texBatch->count > 0 && texBatch->tex[0] &&
				!texBatch->tex[0]->mips.Empty()) {
				tex = texBatch->tex[0];
				texW = (int32)tex->Width(0);
				texH = (int32)tex->Height(0);
				texData = tex->mips[0].Data();
				texBpp = tex->Bpp();
			}
			...
			const ScreenVert &c0 = *v0;
			const ScreenVert &c1 = *v1;
			const ScreenVert &c2 = *v2;
			...
			for (int32 y = yMin; y <= yMax; ++y) {
				...
				for (int32 x = xMin; x <= xMax; ++x) {
					...
					e0 += A0;
					e1 += A1;
					e2 += A2; // avance (sûr même sur 'continue')
					if (ce0 < 0.f || ce1 < 0.f || ce2 < 0.f)
						continue;
					...
					if (depthRow && st.depthTest && !fragWritesDepth) {
						if (!DepthPass(st.depthOp, z, depthRow[x]))
							continue;
					}

					if (depthOnly) {
						if (depthRow && st.depthWrite)
							depthRow[x] = z;
						continue;
					}
					...
					if (hasShader) {
						...
						const uint32 ac = c0.attrCount > c1.attrCount
											  ? (c0.attrCount > c2.attrCount ? c0.attrCount : c2.attrCount)
						...
						if (fragWritesDepth && tl_fragDepthSet) {
							z = tl_fragDepth;
							if (depthRow && st.depthTest && !DepthPass(st.depthOp, z, depthRow[x]))
								continue;
						...
						if (tex) {
							...
							float32 tr, tg, tb, ta;
							if (texLinear)
								SampleBilinear(texData, texW, texH, texBpp, u, v, tr, tg, tb, ta);
							else
								SampleNearest(texData, texW, texH, texBpp, u, v, tr, tg, tb, ta);
							or_ *= tr;
							og *= tg;
							ob *= tb;
							oa *= ta;
					...
					if (depthRow && st.depthWrite)
						depthRow[x] = z;
					if (st.colorFloat)
						OutputPixelF((float32 *)p, st, or_, og, ob, oa);
					else
		...
		static inline uint32 ClipProject(const NkVertexSoftware &v0, const NkVertexSoftware &v1,
										 const NkVertexSoftware &v2, float32 W, float32 H, ReadyTri out[2],
										 const NkSWViewport *vp = nullptr) {
			ClipVert in[3] = {ClipVert::From(v0), ClipVert::From(v1), ClipVert::From(v2)};
			if (NearDist(in[0]) < 0.f && NearDist(in[1]) < 0.f && NearDist(in[2]) < 0.f)
				return 0;
		...
		static void DrawTriangle(const NkVertexSoftware &v0, const NkVertexSoftware &v1, const NkVertexSoftware &v2,
								 uint8 *colorBuf, float32 *depthBuf, uint32 W, uint32 H, int32 clipX0, int32 clipY0,
								 int32 clipX1, int32 clipY1, const NkSWShader *shader, const void *uniformData,
								 const void *texPtr, const NkSWTexBatch *texBatch, const NkSWRenderState &st) {
			if (W == 0 || H == 0)
				return;
			...
			for (uint32 i = 0; i < k; ++i)
				RasterScreenTriangle(rt[i].s[0], rt[i].s[1], rt[i].s[2], colorBuf, depthBuf, W, H, clipX0, clipY0,
									 clipX1, clipY1, shader, uniformData, texPtr, texBatch, st);
		}

...
namespace nkentseu {
	...
	// -------------------------------------------------------------------------
	NkTextGamepadMappingPersistence::NkTextGamepadMappingPersistence(NkString baseDirectory, NkString fileExtension)
		: mBaseDirectory(traits::NkMove(baseDirectory)), mExtension(traits::NkMove(fileExtension)) {
	...
	NkString NkTextGamepadMappingPersistence::SanitizeUserId(const NkString &raw) {
		...
		for (char c : raw) {
			if (encoding::ascii::NkIsAlphaNumeric(c) || c == '_' || c == '-' || c == '.') {
				out.PushBack(c);
			} else {
				out.PushBack('_');
			}
	...
	// -------------------------------------------------------------------------
	bool NkTextGamepadMappingPersistence::Save(const NkString &userId, const NkGamepadMappingProfileData &profile,
											   NkString *outError) {
		...
		for (const NkGamepadMappingSlotData &slot : profile.slots) {
			if (::fprintf(out, "slot %u %d\n", static_cast<unsigned>(slot.slotIndex), slot.active ? 1 : 0) < 0) {
				::fclose(out);
				if (outError) {
					*outError = "Write failed for mapping file: " + filePath;
				}
			...
			for (const NkGamepadAxisMapEntry &a : slot.axes) {
				if (::fprintf(out, "axis %u %u %.9g %d\n", static_cast<unsigned>(a.physicalAxis),
							  static_cast<unsigned>(a.logicalAxis), static_cast<double>(a.scale),
							  a.invert ? 1 : 0) < 0) {
					::fclose(out);
					if (outError) {
						*outError = "Write failed for mapping file: " + filePath;
	...
	// -------------------------------------------------------------------------
	bool NkTextGamepadMappingPersistence::Load(const NkString &userId, NkGamepadMappingProfileData &outProfile,
											   NkString *outError) {
		const NkString filePath = BuildUserFilePath(userId);
		...
		while (ReadTextLine(in, line)) {
			...
			if (token == "backend") {
				...
				int32 active = 0;
				if (!ParseUIntToken(tokens[1], slot.slotIndex) || !ParseIntToken(tokens[2], active)) {
					::fclose(in);
					if (outError) {
						*outError = "Invalid slot entry in mapping file: " + filePath;
					}
				...
				if (!ParseUIntToken(tokens[1], e.physicalButton) || !ParseUIntToken(tokens[2], e.logicalButton)) {
					::fclose(in);
					if (outError) {
						*outError = "Invalid button entry in mapping file: " + filePath;
					}
				...
				int32 invert = 0;
				if (!ParseUIntToken(tokens[1], e.physicalAxis) || !ParseUIntToken(tokens[2], e.logicalAxis) ||
					!ParseFloatToken(tokens[3], e.scale) || !ParseIntToken(tokens[4], invert)) {
					::fclose(in);
					if (outError) {
						*outError = "Invalid axis entry in mapping file: " + filePath;
					}

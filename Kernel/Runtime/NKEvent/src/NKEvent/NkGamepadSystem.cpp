...
// Tout dispatch passe par NkWESystem::Events() (NkEventSystem possÃ©dÃ© par NkSystem).
// =============================================================================
...
namespace nkentseu {
	...
	// NkEventSystem::Instance() n'existe plus â†’ NkWESystem::Events().
	// -------------------------------------------------------------------------
	...
	float32 NkGamepadSystem::ClampAxisForTarget(uint32 logicalAxisIndex, float32 value) noexcept {
		if (logicalAxisIndex >= AXIS_COUNT)
			return 0.f;
	...
	const NkGamepadSnapshot &NkGamepadSystem::ApplyRemap(uint32 idx, const NkGamepadSnapshot &raw) noexcept {
		...
		for (uint32 a = 0; a < AXIS_COUNT; ++a) {
			...
			if (remap.logicalAxis >= AXIS_COUNT)
				continue;
			...
			if (math::NkFabs(v) > math::NkFabs(dst))
				dst = v;
		}
		...
		if (!out.buttons[left] && dpadXValue < -0.5f)
			out.buttons[left] = true;
		if (!out.buttons[right] && dpadXValue > 0.5f)
			out.buttons[right] = true;
		if (!out.buttons[up] && dpadYValue > 0.5f)
			out.buttons[up] = true;
		if (!out.buttons[down] && dpadYValue < -0.5f)
			out.buttons[down] = true;
		...
		if (!out.buttons[ltB])
			out.buttons[ltB] = ltValue > 0.5f;
		if (!out.buttons[rtB])
			out.buttons[rtB] = rtValue > 0.5f;
	...
	void NkGamepadSystem::SetButtonMapByIndex(uint32 idx, uint32 physicalButtonIndex,
											  NkGamepadButton logicalButton) noexcept {
		if (idx >= NK_MAX_GAMEPADS || physicalButtonIndex >= BUTTON_COUNT)
			return;
		...
		profile.buttonMap[physicalButtonIndex] = (logicalIndex < BUTTON_COUNT) ? logicalIndex : NK_GAMEPAD_UNMAPPED;
		profile.active = true;

		SyncMappedSnapshot(idx);
	}

	void NkGamepadSystem::SetAxisMapByIndex(uint32 idx, uint32 physicalAxisIndex, NkGamepadAxis logicalAxis,
											bool invert, float32 scale) noexcept {
		if (idx >= NK_MAX_GAMEPADS || physicalAxisIndex >= AXIS_COUNT)
			return;

		NkRemapProfile &profile = mMappings[idx];
		uint32 logicalIndex = static_cast<uint32>(logicalAxis);
		if (logicalIndex >= AXIS_COUNT)
			logicalIndex = NK_GAMEPAD_UNMAPPED;
	...
	bool NkGamepadSystem::IsButtonDownByIndex(uint32 idx, uint32 btnIndex) const noexcept {
		if (idx >= NK_MAX_GAMEPADS || btnIndex >= BUTTON_COUNT)
			return false;
	...
	float32 NkGamepadSystem::GetAxisByIndex(uint32 idx, uint32 axisIndex) const noexcept {
		if (idx >= NK_MAX_GAMEPADS || axisIndex >= AXIS_COUNT)
			return 0.f;
	...
	void NkGamepadSystem::Rumble(uint32 idx, float32 motorLow, float32 motorHigh, float32 triggerLeft,
								 float32 triggerRight, uint32 durationMs) {
		if (!mReady || !mBackend)
			return;
		mBackend->Rumble(idx, motorLow, motorHigh, triggerLeft, triggerRight, durationMs);

		// Dispatch de l'Ã©vÃ©nement rumble via NkWESystem::Events()
		NkGamepadRumbleEvent event(idx, motorLow, motorHigh, triggerLeft, triggerRight, durationMs);
		if (mEventSystem)
			mEventSystem->DispatchEvent(event);
	}
	...
	bool NkGamepadSystem::ImportMappingProfile(const NkGamepadMappingProfileData &profile, bool clearExisting,
											   NkString *outError) {
		if (clearExisting) {
			ClearAllMappings();
		}
		...
		for (const NkGamepadMappingSlotData &slot : profile.slots) {
			if (slot.slotIndex >= NK_MAX_GAMEPADS)
				continue;
			...
			for (const NkGamepadButtonMapEntry &b : slot.buttons) {
				if (b.physicalButton >= BUTTON_COUNT)
					continue;
				dst.buttonMap[b.physicalButton] =
					(b.logicalButton < BUTTON_COUNT) ? b.logicalButton : NK_GAMEPAD_UNMAPPED;
			}

			for (const NkGamepadAxisMapEntry &a : slot.axes) {
				if (a.physicalAxis >= AXIS_COUNT)
					continue;
	...
	bool NkGamepadSystem::SaveMappingProfile(const NkString &userId, NkString *outError) const {
		if (!mMappingPersistence) {
			if (outError)
				*outError = "No mapping persistence backend configured.";
			return false;
		}
	...
	bool NkGamepadSystem::LoadMappingProfile(const NkString &userId, bool clearExisting, NkString *outError) {
		if (!mMappingPersistence) {
			if (outError)
				*outError = "No mapping persistence backend configured.";
			return false;
	...
	// -------------------------------------------------------------------------
	// Fire helpers â€” tout le dispatch passe par *mEventSystem = NkWESystem::Events()
	// -------------------------------------------------------------------------
	...
	void NkGamepadSystem::FireConnect(const NkGamepadInfo &info, bool connected) {
		...
		if (connected) {
			NkGamepadConnectEvent ev(info);
			mEventSystem->DispatchEvent(ev);
		} else {
			NkGamepadDisconnectEvent ev(info.index);
			mEventSystem->DispatchEvent(ev);
		}
		...
		if (connected)
			gpState.OnConnect(info.index, info);
		else
			gpState.OnDisconnect(info.index);
	...
	void NkGamepadSystem::FireButton(uint32 idx, NkGamepadButton btn, NkButtonState st) {
		...
		if (st == NkButtonState::NK_PRESSED) {
			NkGamepadButtonPressEvent ev(idx, btn);
			mEventSystem->DispatchEvent(ev);
		} else {
			NkGamepadButtonReleaseEvent ev(idx, btn);
			mEventSystem->DispatchEvent(ev);
		}
		...
		if (slot) {
			if (st == NkButtonState::NK_PRESSED)
				slot->OnButtonPress(btn);
			else
				slot->OnButtonRelease(btn);
		}
	...
	void NkGamepadSystem::FireAxis(uint32 idx, NkGamepadAxis ax, float32 value, float32 prevValue) {
		if (!mEventSystem) {
			if (mAxisCb)

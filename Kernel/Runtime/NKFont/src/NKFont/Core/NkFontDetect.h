...
namespace nkentseu {
	...
	class NkFontDetector {
		public:
			 ...
			 */
			static NkFontProfile Analyze(const nkfont::NkFontFaceInfo *info, const NkFontCodepoint *testCps = nullptr,
										 nkft_int32 testCount = 0);

			/**
			 * @brief Analyse directement depuis un buffer TTF (initialise en interne).
			 */
			static NkFontProfile AnalyzeBuffer(const nkft_uint8 *data, nkft_size size, nkft_int32 faceIndex = 0);

			/**
			 * @brief Analyse directement depuis un fichier.
			 */
			static NkFontProfile AnalyzeFile(const char *path, nkft_int32 faceIndex = 0);
			 ...
			 */
			static void ApplyOptimalConfig(const NkFontProfile &profile, nkft_float32 sizePx, NkFontConfig *outConfig);
			 ...
			 */
			static nkft_float32 SnapSize(const NkFontProfile &profile, nkft_float32 sizePx);
		...
		private:
			...
			static bool GlyphHasCurves(const nkfont::NkFontFaceInfo *info, NkGlyphId glyph);

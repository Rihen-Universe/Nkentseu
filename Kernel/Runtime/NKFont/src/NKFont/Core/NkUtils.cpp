...
namespace nkentseu {
	...
	void NkRasterizer::RasterizeCoverage(const NkShape &shape, uint8_t *out, int width, int height, int stride,
										 int subSample) {
	...
	void NkMSDFGenerator::GenerateMSDF(const NkShape &shape, NkMSDFPixel *out, int width, int height, float range) {
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				float fx = x + 0.5f;
				float fy = y + 0.5f;
	...
	void NkDilation::Apply(uint8_t *pixels, int width, int height, int stride, int iterations) {
	...
	namespace nkfont {
		...
		static void NkFlattenQuadratic(const math::NkVec2f &p0, const math::NkVec2f &p1, const math::NkVec2f &p2,
									   float tolerance, NkShape &shape) {
			float dx = p2.x - p0.x;
			float dy = p2.y - p0.y;
		...
		static void NkFlattenCubic(const math::NkVec2f &p0, const math::NkVec2f &p1, const math::NkVec2f &p2,
								   const math::NkVec2f &p3, float tolerance, NkShape &shape) {
			float dx = p3.x - p0.x;
			float dy = p3.y - p0.y;
		...
		bool NkBuildShapeFromGlyph(const NkFontFaceInfo &face, NkGlyphId glyph, NkShape &outShape, float scale) {
			...
			for (nkft_uint32 i = 0; i < buf.count; ++i) {
				...
				math::NkVec2f p = {v.x * scale, v.y * scale};
				...
				switch (v.type) {
					...
					case NK_FONT_VERTEX_CURVE: {
						math::NkVec2f c = {v.cx * scale, v.cy * scale};
					...
					case NK_FONT_VERTEX_CUBIC: {
						math::NkVec2f c0 = {v.cx * scale, v.cy * scale};

						math::NkVec2f c1 = {v.cx1 * scale, v.cy1 * scale};

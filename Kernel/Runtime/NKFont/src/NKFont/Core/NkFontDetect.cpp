...
namespace nkentseu {
	...
	bool NkFontDetector::GlyphHasCurves(const nkfont::NkFontFaceInfo *info, NkGlyphId glyph) {
		nkfont::NkFontVertexBuffer vbuf;
		if (!nkfont::NkGetGlyphShape(info, glyph, &vbuf))
			return false;

		for (nkft_uint32 i = 0; i < vbuf.count; ++i) {
			const auto &v = vbuf.verts[i];
			if (v.type == nkfont::NK_FONT_VERTEX_CURVE || v.type == nkfont::NK_FONT_VERTEX_CUBIC) {
				return true;
			}
		}
		return false;
	...
	NkFontProfile NkFontDetector::Analyze(const nkfont::NkFontFaceInfo *info, const NkFontCodepoint *testCps,
	...
	NkFontProfile NkFontDetector::AnalyzeBuffer(const nkft_uint8 *data, nkft_size size, nkft_int32 faceIndex) {
	...
	NkFontProfile NkFontDetector::AnalyzeFile(const char *path, nkft_int32 faceIndex) {
	...
	void NkFontDetector::ApplyOptimalConfig(const NkFontProfile &profile, nkft_float32 sizePx,
	...
	nkft_float32 NkFontDetector::SnapSize(const NkFontProfile &profile, nkft_float32 sizePx) {

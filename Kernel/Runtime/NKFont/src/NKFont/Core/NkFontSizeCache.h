...
namespace nkentseu {
	...
	class NkFontSizeCache {
		public:
			 ...
			 */
			void Init(NkFontAtlas *atlas, const char *fontPath, const nkft_float32 *preloadSizes = nullptr,
	...
	struct NkFontScaleRenderer {
			 ...
			 */
			static nkft_float32 ComputeScale(nkft_float32 fontRefSizePx, nkft_float32 targetSizePx) {
				if (fontRefSizePx <= 0.f)
					return 1.f;
			 ...
			 */
			static nkft_float32 CalcTextSizeX(const NkFont *font, const char *text, nkft_float32 scale) {
				if (!font || !text)
					return 0.f;
			 ...
			 */
			static void GetScaledGlyphQuad(const NkFontGlyph *glyph, nkft_float32 curX, nkft_float32 curY,
										   nkft_float32 scale, nkft_float32 *outX0, nkft_float32 *outY0,
										   nkft_float32 *outX1, nkft_float32 *outY1, nkft_float32 *outAdvanceX) {
				if (!glyph)
					return;
				if (outX0)
					*outX0 = curX + glyph->x0 * scale;
				if (outY0)
					*outY0 = curY + glyph->y0 * scale;
				if (outX1)
					*outX1 = curX + glyph->x1 * scale;
				if (outY1)
					*outY1 = curY + glyph->y1 * scale;
				if (outAdvanceX)
					*outAdvanceX = glyph->advanceX * scale;
			}
	};

...
namespace nkentseu {
	...
	class NkRasterizer {
		public:
			...
			static void RasterizeCoverage(const NkShape &shape, uint8_t *out, int width, int height, int stride,
	...
	class NkMSDFGenerator {
		public:
			static void GenerateMSDF(const NkShape &shape, NkMSDFPixel *out, int width, int height, float range);
	...
	class NkDilation {
		public:
			static void Apply(uint8_t *pixels, int width, int height, int stride, int iterations);
	};
	...
	namespace nkfont {
		 ...
		 */
		bool NkBuildShapeFromGlyph(const NkFontFaceInfo &face, NkGlyphId glyph, NkShape &outShape, float scale = 1.0f);

...
namespace nkentseu {
	...
	struct NkFontAtlas {
			...
			NkFont *AddFontFromMemory(const nkft_uint8 *data, nkft_size dataSize, nkft_float32 sizePixels,
									  const NkFontConfig *cfg = nullptr);
			NkFont *AddFontFromMemoryOwned(nkft_uint8 *data, nkft_size dataSize, nkft_float32 sizePixels,
										   const NkFontConfig *cfg = nullptr);
	...
	struct NkFont {
		...
		public:
			...
			NkFontGlyphMesh3D GenerateGlyphMesh3D(NkFontCodepoint cp, nkft_float32 scale, nkft_float32 extrusionDepth,
												  const math::NkMat4f &worldMatrix, const math::NkVec4f &color) const;
			 ...
			 */
			void ForEachGlyph3DVertex(NkFontCodepoint cp, nkft_float32 scale, nkft_float32 extrusionDepth,
									  const math::NkMat4f &worldMatrix, const math::NkVec4f &color,
									  NkFont3DVertexCallback callback, void *userData) const;

			/**
			 * @brief Version callback pour texte complet.
			 */
			void ForEachText3DVertex(const char *text, nkft_float32 scale, nkft_float32 extrusionDepth,
									 const math::NkMat4f &worldMatrix, const math::NkVec4f &color,
									 NkFont3DVertexCallback callback, void *userData) const;
	};

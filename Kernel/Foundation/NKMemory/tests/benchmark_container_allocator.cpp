...
TEST_CASE(NKMemoryBenchmark, ContainerAllocatorVsMallocSmallObjects) {
	...
	const double containerNs =
		std::chrono::duration<double, std::nano>(containerEnd - containerBegin).count() / static_cast<double>(kSamples);
	const double mallocNs =
		std::chrono::duration<double, std::nano>(mallocEnd - mallocBegin).count() / static_cast<double>(kSamples);

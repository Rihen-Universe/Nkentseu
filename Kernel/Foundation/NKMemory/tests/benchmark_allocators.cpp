...
namespace {
	...
	template <nk_size N> static void InsertionSort(nk_int64 (&values)[N], nk_size count) {
		if (count <= 1u) {
			return;
		}
	...
	static BenchResult BenchSimpleAllocateFree(const char *name, AllocatorT &allocator, nk_size minSize,
											   nk_size maxSize, nk_size alignment) {
...
TEST_CASE(NKMemoryBenchmark, AllocatorP99) {
	...
	for (const BenchResult &result : results) {
		NK_FOUNDATION_LOG_INFO("  - %-20s avg=%10.2f ns  p99=%10.2f ns  failures=%llu/%llu", result.name, result.avgNs,
							   result.p99Ns, static_cast<unsigned long long>(result.failures),

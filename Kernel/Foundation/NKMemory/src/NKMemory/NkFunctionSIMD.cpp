...
namespace {
	 ...
	 */
	void *SearchPatternPortable(const void *haystack, nkentseu::nk_size haystackSize, const void *needle,
								nkentseu::nk_size needleSize) noexcept {
		if (!haystack || !needle) {
			return nullptr;
		}
		if (needleSize == 0u) {
...
// =================================================================
			...
			if (bytes >= 64u && NkIsAlignedSIMD(dst) && NkIsAlignedSIMD(src)) {
				...
				if (i < bytes) {
					FallbackCopy(static_cast<nk_uint8 *>(dst) + i, static_cast<const nk_uint8 *>(src) + i, bytes - i);
				}
				return;
			}
...
// =================================================================
			...
			if (bytes >= 64u && NkIsAlignedSIMD(dst)) {
				...
				if (i < bytes) {
					FallbackSet(static_cast<nk_uint8 *>(dst) + i, value, bytes - i);
				}
				return;
			}

...
namespace nkentseu {
	namespace memory {
		...
		NkGcTracer::NkGcTracer(NkGarbageCollector &gc) noexcept : mGc(gc) {
		}
		...
		NkGcObject::NkGcObject() noexcept
			: mGcNext(nullptr), mGcPrev(nullptr), mGcMarked(false), mGcOwned(false), mGcAllocator(nullptr),
			  mGcDestroy(nullptr) {
		}
		...
		NkGcRoot::NkGcRoot(NkGcObject **slot) noexcept : mSlot(slot), mNext(nullptr) {
		}
		...
		NkGarbageCollector::NkGarbageCollector(NkAllocator *allocator) noexcept
			: mObjects(nullptr), mRoots(nullptr), mObjectCount(0u), mObjectIndex(nullptr), mObjectIndexCapacity(0u),
			  mObjectIndexSize(0u), mObjectIndexTombstones(0u),
			  mAllocator(allocator ? allocator : &NkGetDefaultAllocator()), mLock() {
		}
		...
		nk_bool NkGarbageCollector::InsertIndexLocked(NkGcObject *object) noexcept {
			...
			for (nk_size probes = 0u; probes < mObjectIndexCapacity; ++probes) {
				...
				if (slot == nullptr) {
					...
					const nk_size target = (tombstoneIdx != static_cast<nk_size>(-1)) ? tombstoneIdx : idx;
...
/*
	...
	✓ Les objets NkGcObject eux-mêmes ne sont PAS thread-safe : synchronisation externe requise pour accès concurrent
   aux données membres ✓ New/Delete acquièrent le lock pour registration/unregistration : safe pour usage multi-thread

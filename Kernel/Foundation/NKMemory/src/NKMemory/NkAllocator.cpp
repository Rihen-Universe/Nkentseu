...
namespace nkentseu {
	namespace memory {
		...
		NkNewAllocator::NkNewAllocator() noexcept : NkAllocator("NkNewAllocator") {
		}
		...
		NkMallocAllocator::NkMallocAllocator() noexcept : NkAllocator("NkMallocAllocator") {
		}
...
// Optimisation Windows : _aligned_realloc préserve l'alignement
		...
		NkVirtualAllocator::NkVirtualAllocator() noexcept
			: NkAllocator("NkVirtualAllocator"),
			  flags(NK_MEMORY_FLAG_READ | NK_MEMORY_FLAG_WRITE | NK_MEMORY_FLAG_RESERVE | NK_MEMORY_FLAG_COMMIT |
		...
		NkAllocator::Pointer NkVirtualAllocator::AllocateVirtual(SizeType size, const nk_char * /*tag*/) noexcept {
			...
			if (flags & NK_MEMORY_FLAG_READ)
				protection |= PROT_READ;
			if (flags & NK_MEMORY_FLAG_WRITE)
				protection |= PROT_WRITE;
			if (flags & NK_MEMORY_FLAG_EXECUTE)
		...
		NkLinearAllocator::NkLinearAllocator(SizeType capacityBytes, NkAllocator *backingAllocator)
			: NkAllocator("NkLinearAllocator"),
			  mBacking(backingAllocator ? backingAllocator : &NkGetDefaultAllocator()), mBuffer(nullptr),
			  mCapacity(capacityBytes), mOffset(0u), mOwnsBuffer(true) {
			if (capacityBytes == 0u) {
				mOwnsBuffer = false;
				return;
			}
		...
		NkArenaAllocator::NkArenaAllocator(SizeType capacityBytes, NkAllocator *backingAllocator)
			: NkAllocator("NkArenaAllocator"), mBacking(backingAllocator ? backingAllocator : &NkGetDefaultAllocator()),
			  mBuffer(nullptr), mCapacity(capacityBytes), mOffset(0u), mOwnsBuffer(true) {
			if (capacityBytes == 0u) {
				mOwnsBuffer = false;
				return;
			}
		...
		NkStackAllocator::NkStackAllocator(SizeType capacityBytes, NkAllocator *backingAllocator)
			: NkAllocator("NkStackAllocator"), mBacking(backingAllocator ? backingAllocator : &NkGetDefaultAllocator()),
			  mBuffer(nullptr), mCapacity(capacityBytes), mOffset(0u), mLastAllocation(nullptr), mOwnsBuffer(true) {
			if (capacityBytes == 0u) {
				mOwnsBuffer = false;
				return;
			}
		...
		void NkStackAllocator::Deallocate(Pointer ptr) {
			...
			auto *header =
				reinterpret_cast<AllocationHeader *>(static_cast<nk_uint8 *>(ptr) - sizeof(AllocationHeader));
		...
		NkPoolAllocator::NkPoolAllocator(SizeType blockSize, SizeType blockCount, NkAllocator *backingAllocator)
			: NkAllocator("NkPoolAllocator"), mBacking(backingAllocator ? backingAllocator : &NkGetDefaultAllocator()),
			  mBuffer(nullptr), mFreeList(nullptr),
			  mBlockSize(MaxSize(blockSize, static_cast<SizeType>(sizeof(FreeNode)))), mBlockCount(blockCount),
			  mOwnsBuffer(true) {
		...
		NkFreeListAllocator::NkFreeListAllocator(SizeType capacityBytes, NkAllocator *backingAllocator)
			: NkAllocator("NkFreeListAllocator"),
			  mBacking(backingAllocator ? backingAllocator : &NkGetDefaultAllocator()), mBuffer(nullptr),
			  mCapacity(capacityBytes), mHead(nullptr), mOwnsBuffer(true) {
		...
		NkAllocator::Pointer NkFreeListAllocator::Allocate(SizeType size, AlignType alignment) {
			...
			while (block) {
				...
				const nk_size userStart =
					NK_ALIGN_UP(blockStart + sizeof(BlockHeader) + sizeof(AllocationHeader), alignment);
		...
		NkBuddyAllocator::NkBuddyAllocator(SizeType capacityBytes, SizeType minBlockSize, NkAllocator *backingAllocator)
			: NkAllocator("NkBuddyAllocator"), mMinBlockSize(MaxSize(minBlockSize, static_cast<SizeType>(16u))),

...
// ========================================================================
	...
	void MemCopyAvx2Regular(nkentseu::nk_uint8 *NKENTSEU_RESTRICT destination,
							const nkentseu::nk_uint8 *NKENTSEU_RESTRICT source, nkentseu::nk_size size) noexcept {
		...
		for (; offset + AVX2_COPY_CHUNK <= size; offset += AVX2_COPY_CHUNK) {
			...
			if (offset + AVX2_PREFETCH_T0_DIST < size) {
				_mm_prefetch(reinterpret_cast<const char *>(source + offset + AVX2_PREFETCH_T0_DIST), _MM_HINT_T0);
			}

			// Chargement et stockage non-alignés (flexibilité maximale)
			const __m256i data = _mm256_loadu_si256(reinterpret_cast<const __m256i *>(source + offset));
			_mm256_storeu_si256(reinterpret_cast<__m256i *>(destination + offset), data);
		}
	...
	void MemCopyAvx2Streaming(nkentseu::nk_uint8 *NKENTSEU_RESTRICT destination,
		...
		for (; offset + AVX2_COPY_CHUNK <= size; offset += AVX2_COPY_CHUNK) {
			...
			if (offset + AVX2_PREFETCH_NTA_DIST < size) {
				_mm_prefetch(reinterpret_cast<const char *>(source + offset + AVX2_PREFETCH_NTA_DIST), _MM_HINT_NTA);
			}

			const __m256i data = _mm256_loadu_si256(reinterpret_cast<const __m256i *>(source + offset));
			// Store non-temporel : écriture directe en mémoire, bypass cache
			_mm256_stream_si256(reinterpret_cast<__m256i *>(destination + offset), data);
		}
...
// =================================================================
			...
			if (size >= AVX2_COPY_CHUNK) {
				...
				if (size >= AVX2_STREAMING_THRESH && NkIsAlignedPtr(destination, AVX2_COPY_CHUNK)) {

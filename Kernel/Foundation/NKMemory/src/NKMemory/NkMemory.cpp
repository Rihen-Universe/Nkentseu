...
namespace nkentseu {
	namespace memory {
		...
		NkMemorySystem::NkMemorySystem() noexcept
			: mAllocator(&NkGetDefaultAllocator()), mHead(nullptr), mLiveBytes(0u), mPeakBytes(0u),
			  mLiveAllocations(0u), mTotalAllocations(0u), mLogCallback(nullptr), mInitialized(false), mLock(),
			  mAllocIndex(&NkGetMallocAllocator()) // Hash table allouée via malloc pour éviter récursion
			  ,
			  mTracker(), mGc(&NkGetDefaultAllocator()) // GC par défaut utilise l'allocateur défaut
			  ,
			  mDefaultGcNode(), mGcHead(nullptr), mGcCount(0u), mNextGcId(1u) {
		...
		void NkMemorySystem::Shutdown(nk_bool reportLeaks) noexcept {
			...
			while (leakedHead) {
				...
				if (reportLeaks) {
					nk_char line[512];
					NK_FOUNDATION_SPRINT(
						line, sizeof(line), "[NKMemory] leak ptr=%p size=%llu count=%llu tag=%s at %s:%d (%s)",
						leakedHead->userPtr, static_cast<unsigned long long>(leakedHead->size),
						static_cast<unsigned long long>(leakedHead->count),
						leakedHead->tag ? leakedHead->tag : "<none>", leakedHead->file ? leakedHead->file : "<unknown>",
						leakedHead->line, leakedHead->function ? leakedHead->function : "<unknown>");
					LogLine(line);
				}
		...
		void *NkMemorySystem::Allocate(nk_size size, nk_size alignment, const nk_char *file, nk_int32 line,
			...
			RegisterAllocation(memory, memory, size, 1u, alignment, &NkMemorySystem::DestroyRaw, file, line, function,
							   tag);
			return memory;
		}
		...
		void NkMemorySystem::DestroyRaw(void * /*userPtr*/, void *basePtr, NkAllocator *allocator, nk_size /*count*/) {
		...
		void NkMemorySystem::RegisterAllocation(void *userPtr, void *basePtr, nk_size size, nk_size count,
												nk_size alignment, NkDestroyFn destroy, const nk_char *file,
												nk_int32 line, const nk_char *function, const nk_char *tag) {
		...
		const NkMemorySystem::NkGcNode *
		NkMemorySystem::FindGcNodeLocked(const NkGarbageCollector *collector) const noexcept {
		...
		void NkMemorySystem::CopyGcName(nk_char *destination, nk_size destinationSize, const nk_char *source) noexcept {

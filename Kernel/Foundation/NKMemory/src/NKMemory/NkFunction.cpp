...
namespace {
	 ...
	 */
	[[nodiscard]] inline bool IsValidSlice(nkentseu::nk_size totalSize, nkentseu::nk_size offset,
										   nkentseu::nk_size requestedSize, nkentseu::nk_size *clampedSize) noexcept {
...
namespace nkentseu {
	namespace memory {
		...
		void *NkCopy(void *dst, usize dstSize, usize dstOffset, const void *src, usize srcSize, usize srcOffset,
			...
			return NkMemCopy(static_cast<uint8 *>(dst) + dstOffset, static_cast<const uint8 *>(src) + srcOffset,
							 copySize);
		}
		...
		void *NkMove(void *dst, usize dstSize, usize dstOffset, const void *src, usize srcSize, usize srcOffset,
			...
			return NkMemMove(static_cast<uint8 *>(dst) + dstOffset, static_cast<const uint8 *>(src) + srcOffset,
							 moveSize);
		}
		...
		int32 NkCompare(const void *a, usize aSize, usize offsetA, const void *b, usize bSize, usize offsetB,
			...
			return NkMemCompare(static_cast<const uint8 *>(a) + offsetA, static_cast<const uint8 *>(b) + offsetB,
								compareSize);
		}

...
namespace {
	 ...
	 */
	constexpr nkentseu::nk_size
		CONTAINER_CLASS_SIZES[nkentseu::memory::NkContainerAllocator::NK_CONTAINER_CLASS_COUNT] = {
			16u, 32u, 64u, 96u, 128u, 192u, 256u, 384u, 512u, 768u, 1024u, 1536u, 2048u};
	...
	inline void AtomicSaturatingDecrement(nkentseu::NkAtomicSize &value) noexcept {
		...
		while (expected > 0u) {
			if (value.CompareExchangeWeak(expected, expected - 1u, nkentseu::NkMemoryOrder::NK_RELAXED)) {
				return;
			}
		}
	}
...
namespace nkentseu {
	namespace memory {
		...
		NkContainerAllocator::NkContainerAllocator(NkAllocator *backingAllocator, nk_size pageSize, const nk_char *name)
			: NkAllocator(name ? name : "NkContainerAllocator"),
			  ...
			  ,
			  mLargeHead(nullptr), mLargeAllocationCount(0u), mLargeAllocationBytes(0u),
			  mGeneration(1u) // Génération initiale = 1
			  ,
			  mRegistryNext(nullptr), mClassLocks{} // Zero-initialization des locks
			  ,
			  mLargeLock() {
			...
			for (nk_uint32 i = 0u; i < NK_CONTAINER_CLASS_COUNT; ++i) {
				...
				mClasses[i].SlotSize = AlignUpPow2(mHeaderOffset + mClasses[i].UserSize, NK_CONTAINER_SMALL_ALIGNMENT);
		...
		void NkContainerAllocator::Deallocate(Pointer ptr) {
			...
			auto *header = reinterpret_cast<NkContainerAllocHeader *>(static_cast<nk_uint8 *>(ptr) -
																	  sizeof(NkContainerAllocHeader));
		...
		void NkContainerAllocator::TryDeallocateSmallToTls(Pointer ptr, NkContainerAllocHeader *header) noexcept {
			if (!ptr || !header) {
				return;
			}
		...
		NkAllocator::Pointer NkContainerAllocator::Reallocate(Pointer ptr, SizeType oldSize, SizeType newSize,
															  SizeType alignment) {

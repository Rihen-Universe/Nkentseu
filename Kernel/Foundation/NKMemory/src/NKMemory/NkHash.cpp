...
namespace nkentseu {
	namespace memory {
		...
		NkPointerHashSet::NkPointerHashSet(NkAllocator *allocator) noexcept
			: mKeys(nullptr), mCapacity(0u), mSize(0u), mTombstones(0u), mAllocator(allocator) {
		}
		...
		nk_bool NkPointerHashSet::Insert(const void *key) noexcept {
			...
			for (nk_size probes = 0u; probes < mCapacity; ++probes) {
				...
				if (slot == nullptr) {
					...
					const nk_size target = (firstTombstone != detail::InvalidIndex()) ? firstTombstone : index;
...
namespace nkentseu {
	namespace memory {
		...
		NkPointerHashMap::NkPointerHashMap(NkAllocator *allocator) noexcept
			: mEntries(nullptr), mCapacity(0u), mSize(0u), mTombstones(0u), mAllocator(allocator) {
		}

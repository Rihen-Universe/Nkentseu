...
namespace nkentseu {
	namespace memory {
		...
		NkMultiLevelAllocator::Pointer NkMultiLevelAllocator::Allocate(SizeType size, SizeType alignment) {
			...
			const nk_uintptr alignedUser =
				static_cast<nk_uintptr>(NK_ALIGN_UP(static_cast<nk_size>(firstUser), safeAlignment));
...
// Corruption détectée : logger en debug et ignorer
		...
		NkMultiLevelAllocator::Pointer NkMultiLevelAllocator::Reallocate(Pointer ptr, SizeType oldSize,
																		 SizeType newSize, SizeType alignment) {
			...
			auto *header =
				reinterpret_cast<NkDispatchHeader *>(static_cast<nk_uint8 *>(ptr) - sizeof(NkDispatchHeader));
			if (header->magic == ::MULTI_LEVEL_BLOCK_MAGIC) {
		...
		NkMultiLevelAllocatorStats NkMultiLevelAllocator::GetStats() const noexcept {
			...
			if (mTinyPool) {
				...
				stats.tiny.usage = (stats.tiny.capacity > 0u) ? static_cast<float32>(stats.tiny.allocated) /
																	static_cast<float32>(stats.tiny.capacity)
															  : 0.0f;
			}
			...
			if (mSmallPool) {
				...
				stats.small.usage = (stats.small.capacity > 0u) ? static_cast<float32>(stats.small.allocated) /
																	  static_cast<float32>(stats.small.capacity)
																: 0.0f;
			}
		...
		void NkMultiLevelAllocator::DumpStats() const noexcept {
			...
			NK_FOUNDATION_LOG_INFO(
				"MEDIUM (1KB-1MB): %llu bytes live (%llu active allocs)",
			...
			NK_FOUNDATION_LOG_INFO("Estimated fragmentation: %.2f",
								   static_cast<double>(stats.GetFragmentation() * 100.0f));
			NK_FOUNDATION_LOG_INFO("==========================================");
		}

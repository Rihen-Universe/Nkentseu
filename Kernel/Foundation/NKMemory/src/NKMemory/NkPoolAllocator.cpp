...
namespace nkentseu {
	namespace memory {
		...
		NkFixedPoolAllocator<BlockSize, NumBlocks>::NkFixedPoolAllocator(const nk_char *name)
			: NkAllocator(name), mBlocks(nullptr), mFreeList(nullptr), mNumFree(NumBlocks) {
			...
			for (nk_size i = 0u; i < NumBlocks; ++i) {
				...
				*ptrLoc =
					(i + 1u < NumBlocks) ? mBlocks + ((i + 1u) * BlockSize) : nullptr; // Dernier bloc : fin de liste
			}
...
// Corruption détectée : ignorer silencieusement en release, logger en debug
		...
		void NkFixedPoolAllocator<BlockSize, NumBlocks>::Reset() noexcept {
			...
			for (nk_size i = 0u; i < NumBlocks; ++i) {
				nk_uint8 *block = mBlocks + (i * BlockSize);
				auto **ptrLoc = reinterpret_cast<nk_uint8 **>(block);
				*ptrLoc = (i + 1u < NumBlocks) ? mBlocks + ((i + 1u) * BlockSize) : nullptr;
			}
...
namespace nkentseu {
	namespace memory {
		...
		NkVariablePoolAllocator::NkVariablePoolAllocator(const nk_char *name)
			: NkAllocator(name), mHead(nullptr), mLiveBytes(0u), mLiveAllocations(0u), mLock() {
		...
		NkVariablePoolAllocator::Pointer NkVariablePoolAllocator::Allocate(SizeType size, SizeType alignment) {
			...
			const nk_uintptr alignedUser = NK_ALIGN_UP(static_cast<nk_size>(firstUser), safeAlignment);
		...
		void NkVariablePoolAllocator::Deallocate(Pointer ptr) {
			...
			auto *header = reinterpret_cast<Header *>(static_cast<nk_uint8 *>(ptr) - sizeof(Header));
...
/*
	...
	2. NkVariablePoolAllocator avec size classes : maintenir plusieurs sous-pools pour tailles courantes (32, 64,
   128...)

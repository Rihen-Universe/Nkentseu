...
namespace nkentseu {
	...
	namespace platform {
		...
		namespace {
			 ...
			 */
			NKENTSEU_API_INLINE void CopyStringTruncated(char *dst, size_t dstCount, const char *src) {
				if (!dst || dstCount == 0u) {
					return;
				}
			 ...
			 */
			NKENTSEU_API_INLINE void TrimStringToBuffer(const char *text, char *dst, size_t dstCount) {
				if (!dst || dstCount == 0u) {
					return;
				}
				...
				const size_t copyLen =
					(trimmedLen < static_cast<size_t>(dstCount - 1u)) ? trimmedLen : static_cast<size_t>(dstCount - 1u);
			 ...
			 * @ingroup CPUFeaturesInternals
			 */
			NKENTSEU_API_INLINE bool ContainsSubString(const char *text, const char *pattern) {
		...
		NKENTSEU_API_INLINE CacheInfo::CacheInfo()
			: lineSize(64), l1DataSize(32), l1InstructionSize(32), l2Size(256), l3Size(8192) {
		...
		NKENTSEU_API_INLINE CPUTopology::CPUTopology()
			: numPhysicalCores(1), numLogicalCores(1), numSockets(1), hasHyperThreading(false) {
		...
		NKENTSEU_API_INLINE SIMDFeatures::SIMDFeatures()
			: hasMMX(false), hasSSE(false), hasSSE2(false), hasSSE3(false), hasSSSE3(false), hasSSE41(false),
			  hasSSE42(false), hasAVX(false), hasAVX2(false), hasAVX512F(false), hasAVX512DQ(false), hasAVX512BW(false),
			  hasAVX512VL(false), hasFMA(false), hasFMA4(false), hasNEON(false), hasSVE(false), hasSVE2(false) {
		 ...
		 */
		NKENTSEU_API_INLINE void CPUFeatures::CPUID(int32_t function, int32_t subfunction, int32_t *eax, int32_t *ebx,
													int32_t *ecx, int32_t *edx) {
			int32_t regs[4] = {0};
		...
		NKENTSEU_NO_INLINE void CPUFeatures::DetectTopology() {
			...
			if (length > 0) {
				auto *buffer =
					new SYSTEM_LOGICAL_PROCESSOR_INFORMATION[length / sizeof(SYSTEM_LOGICAL_PROCESSOR_INFORMATION)];
		...
		NKENTSEU_NO_INLINE void CPUFeatures::DetectFrequency() {
			...
			if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, processorKey, 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
				DWORD mhz = 0;
		...
		NKENTSEU_API_INLINE void CPUFeatures::ToString(char *outBuffer, size_t outBufferSize) const {
			...
			// En-tête général
			offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "CPU Information:\n");
			offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "  Vendor: %s\n", vendor);
			offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "  Brand: %s\n", brand);
			...
			// Section Topologie
			offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "\nTopology:\n");
			offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "  Physical Cores: %d\n",
							   topology.numPhysicalCores);
			offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "  Logical Cores: %d\n",
							   topology.numLogicalCores);
			offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "  Hyper-Threading: %s\n",
							   topology.hasHyperThreading ? "Yes" : "No");

			// Section Cache
			offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "\nCache:\n");
			offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "  Line Size: %d bytes\n",
							   cache.lineSize);
			offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset),
			...
			offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "\nSIMD Features:\n");
			if (simd.hasSSE) {
				offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "  SSE ");
			}
			if (simd.hasSSE2) {
				offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "SSE2 ");
			}
			if (simd.hasSSE3) {
				offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "SSE3 ");
			}
			if (simd.hasSSE41) {
				offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "SSE4.1 ");
			}
			if (simd.hasSSE42) {
				offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "SSE4.2 ");
			}
			if (simd.hasAVX) {
				offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "AVX ");
			}
			if (simd.hasAVX2) {
				offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "AVX2 ");
			}
			if (simd.hasAVX512F) {
				offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "AVX-512F ");
			}
			if (simd.hasFMA) {
				offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "FMA ");
			}
			if (simd.hasNEON) {
				offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "NEON ");
			}
			offset += snprintf(buffer + offset, sizeof(buffer) - static_cast<size_t>(offset), "\n");

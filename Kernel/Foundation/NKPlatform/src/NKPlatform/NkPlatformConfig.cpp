	...
	namespace platform {
		...
		PlatformConfig::PlatformConfig()
			: platformName(GetPlatformName()), archName(GetArchName()), compilerName(GetCompilerName()),
			  ...
			  hasUnicode(NKENTSEU_HAS_UNICODE), hasThreading(NKENTSEU_HAS_THREADING),
			  hasFilesystem(NKENTSEU_HAS_FILESYSTEM), hasNetwork(NKENTSEU_HAS_NETWORK),
			  maxPathLength(NKENTSEU_MAX_PATH), cacheLineSize(NKENTSEU_CACHE_LINE_SIZE) {
		...
		PlatformCapabilities::PlatformCapabilities()
			: totalPhysicalMemory(0), availablePhysicalMemory(0), pageSize(0), processorCount(0),
			  logicalProcessorCount(0), hasDisplay(false), primaryScreenWidth(0), primaryScreenHeight(0), hasSSE(false),
			  hasSSE2(false), hasAVX(false), hasAVX2(false), hasNEON(false) {
			// -----------------------------------------------------------------

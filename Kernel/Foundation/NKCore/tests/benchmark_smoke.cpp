...
TEST_CASE(NKCoreBenchmark, OptionalToggleVsStdOptional) {
	...
	auto nkResult = RUN_BENCHMARK_WITH_OPTIONS(
		"NKCore.NkOptional.Toggle",
		[&]() {
			nkValue.Emplace(42);
	...
	auto stdResult = RUN_BENCHMARK_WITH_OPTIONS(
		"NKCore.std::optional.Toggle",
		[&]() {
			stdValue.emplace(42);
	...
	NK_FOUNDATION_LOG_INFO("[NKCore Benchmark] Optional Toggle NK=%.6f ms STL=%.6f ms ratio=%.3f", nkResult.mMeanTimeMs,
						   stdResult.mMeanTimeMs, ratio);
}
...
TEST_CASE(NKCoreBenchmark, AtomicFetchAddNkVsStdAtomic) {
	...
	auto nkResult = RUN_BENCHMARK_WITH_OPTIONS(
		"NKCore.NkAtomic.FetchAdd", [&]() { nkSink = nkSink + static_cast<int>(nkCounter.FetchAdd(1)); }, options);

	auto stdResult = RUN_BENCHMARK_WITH_OPTIONS(
		"NKCore.std::atomic.fetch_add",
		[&]() { stdSink = stdSink + stdCounter.fetch_add(1, std::memory_order_seq_cst); }, options);
	...
	NK_FOUNDATION_LOG_INFO("[NKCore Benchmark] Atomic FetchAdd NK=%.6f ms STL=%.6f ms ratio=%.3f", nkResult.mMeanTimeMs,
						   stdResult.mMeanTimeMs, ratio);
}

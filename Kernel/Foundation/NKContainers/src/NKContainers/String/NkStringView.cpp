...
namespace nkentseu {
	 ...
	 */
	NkStringView::NkStringView(const NkString &str) noexcept : mData(str.Data()), mLength(str.Length()) {
	}
	 ...
	 */
	NkStringView::SizeType NkStringView::Find(NkStringView str, SizeType pos) const noexcept {
		if (str.mLength == 0) {
			return pos;
		}
		if (pos > mLength) {
	 ...
	 */
	NkStringView::SizeType NkStringView::FindIgnoreCase(NkStringView str, SizeType pos) const noexcept {
	 ...
	 */
	NkStringView::SizeType NkStringView::RFind(NkStringView str, SizeType pos) const noexcept {
		if (str.mLength == 0) {
		...
		SizeType searchPos = (pos == npos || pos >= mLength - str.mLength) ? mLength - str.mLength : pos;
	 ...
	 */
	NkStringView::SizeType NkStringView::RFindIgnoreCase(NkStringView str, SizeType pos) const noexcept {
	 ...
	 */
	NkStringView::SizeType NkStringView::FindFirstOf(NkStringView chars, SizeType pos) const noexcept {
		for (SizeType i = pos; i < mLength; ++i) {
	 ...
	 * @return Index du premier match, ou npos si aucun
	 */
	NkStringView::SizeType NkStringView::FindFirstOf(const char *chars, SizeType pos) const noexcept {
		for (SizeType i = pos; i < mLength; ++i) {
			for (SizeType j = 0; chars[j] != '\0'; ++j) {
				if (mData[i] == chars[j]) {
					return i;
	 ...
	 */
	NkStringView::SizeType NkStringView::FindLastOf(NkStringView chars, SizeType pos) const noexcept {
		SizeType searchPos = (pos == npos || pos >= mLength) ? mLength - 1 : pos;
		for (SizeType i = searchPos + 1; i > 0; --i) {
			SizeType idx = i - 1;
			for (SizeType j = 0; j < chars.mLength; ++j) {
	 ...
	 * @return Index du dernier match, ou npos si aucun
	 */
	NkStringView::SizeType NkStringView::FindLastOf(const char *chars, SizeType pos) const noexcept {
		SizeType searchPos = (pos == npos || pos >= mLength) ? mLength - 1 : pos;
		for (SizeType i = searchPos + 1; i > 0; --i) {
			SizeType idx = i - 1;
			for (SizeType j = 0; chars[j] != '\0'; ++j) {
	 ...
	 */
	NkStringView::SizeType NkStringView::FindFirstNotOf(NkStringView chars, SizeType pos) const noexcept {
		for (SizeType i = pos; i < mLength; ++i) {
			bool found = false;
			for (SizeType j = 0; j < chars.mLength; ++j) {
				if (mData[i] == chars.mData[j]) {
	 ...
	 * @return Index du premier caractère "extérieur", ou npos
	 */
	NkStringView::SizeType NkStringView::FindFirstNotOf(const char *chars, SizeType pos) const noexcept {
		for (SizeType i = pos; i < mLength; ++i) {
			bool found = false;
			for (SizeType j = 0; chars[j] != '\0'; ++j) {
				if (mData[i] == chars[j]) {
	 ...
	 */
	NkStringView::SizeType NkStringView::FindLastNotOf(NkStringView chars, SizeType pos) const noexcept {
		SizeType searchPos = (pos == npos || pos >= mLength) ? mLength - 1 : pos;
		for (SizeType i = searchPos + 1; i > 0; --i) {
			SizeType idx = i - 1;
			bool found = false;
	 ...
	 * @return Index du dernier caractère "extérieur", ou npos
	 */
	NkStringView::SizeType NkStringView::FindLastNotOf(const char *chars, SizeType pos) const noexcept {
		SizeType searchPos = (pos == npos || pos >= mLength) ? mLength - 1 : pos;
		for (SizeType i = searchPos + 1; i > 0; --i) {
			SizeType idx = i - 1;
			bool found = false;
	...
	bool NkStringView::IsBoolean() const noexcept {
		...
		return trimmed == "true" || trimmed == "false" || trimmed == "TRUE" || trimmed == "FALSE" ||
			   trimmed == "True" || trimmed == "False" || trimmed == "1" || trimmed == "0";
	}
	 ...
	 */
	NkStringView::SizeType NkStringView::LevenshteinDistance(NkStringView other) const noexcept {

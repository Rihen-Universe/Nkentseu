...
namespace nkentseu {
	...
	namespace encoding {
		...
		namespace utf32 {
			...
			NkConversionResult NkToUTF8(const uint32 *src, usize srcLen, char *dst, usize dstLen, usize &charsRead,
										usize &bytesWritten) {
			...
			NkConversionResult NkToUTF16(const uint32 *src, usize srcLen, uint16 *dst, usize dstLen, usize &charsRead,
										 usize &unitsWritten) {
				// Validation des pointeurs d'entrée
				if (!src || !dst) {
					return NkConversionResult::NK_SOURCE_ILLEGAL;

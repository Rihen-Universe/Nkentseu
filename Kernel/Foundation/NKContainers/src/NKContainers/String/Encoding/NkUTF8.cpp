		...
		namespace utf8 {
			...
			usize NkDecodeChar(const char *str, uint32 &outCodepoint) {
				...
				if ((byte1 & 0xE0) == 0xC0) {
					...
					outCodepoint = ((static_cast<uint32>(byte1) & 0x1F) << 6) | (static_cast<uint32>(str[1]) & 0x3F);
				...
				if ((byte1 & 0xF0) == 0xE0) {
					...
					outCodepoint = ((static_cast<uint32>(byte1) & 0x0F) << 12) |
								   ((static_cast<uint32>(str[1]) & 0x3F) << 6) | (static_cast<uint32>(str[2]) & 0x3F);
				...
				if ((byte1 & 0xF8) == 0xF0) {
					...
					outCodepoint = ((static_cast<uint32>(byte1) & 0x07) << 18) |
								   ...
								   ((static_cast<uint32>(str[2]) & 0x3F) << 6) | (static_cast<uint32>(str[3]) & 0x3F);
			...
			NkConversionResult NkToUTF16(const char *src, usize srcLen, uint16 *dst, usize dstLen, usize &bytesRead,
										 usize &charsWritten) {
			...
			NkConversionResult NkToUTF32(const char *src, usize srcLen, uint32 *dst, usize dstLen, usize &bytesRead,
										 usize &charsWritten) {
				// Validation des pointeurs d'entrée
				if (!src || !dst) {
					return NkConversionResult::NK_SOURCE_ILLEGAL;
			...
			NkConversionResult NkFromUTF16(const uint16 *src, usize srcLen, char *dst, usize dstLen, usize &charsRead,
										   usize &bytesWritten) {
			...
			NkConversionResult NkFromUTF32(const uint32 *src, usize srcLen, char *dst, usize dstLen, usize &charsRead,
										   usize &bytesWritten) {
				// Validation des pointeurs
				if (!src || !dst) {
					return NkConversionResult::NK_SOURCE_ILLEGAL;

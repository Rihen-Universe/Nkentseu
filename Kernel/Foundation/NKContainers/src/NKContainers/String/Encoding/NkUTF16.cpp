		...
		namespace utf16 {
			...
			usize NkDecodeChar(const uint16 *str, uint32 &outCodepoint) {
				...
				if (NkIsHighSurrogate(unit1)) {
					...
					outCodepoint =
						0x10000 + ((static_cast<uint32>(unit1 & 0x3FF)) << 10) + static_cast<uint32>(unit2 & 0x3FF);
			...
			NkConversionResult NkToUTF8(const uint16 *src, usize srcLen, char *dst, usize dstLen, usize &charsRead,
										usize &bytesWritten) {
			...
			NkConversionResult NkToUTF32(const uint16 *src, usize srcLen, uint32 *dst, usize dstLen, usize &charsRead,
										 usize &unitsWritten) {
				// Validation des pointeurs
				if (!src || !dst) {
					return NkConversionResult::NK_SOURCE_ILLEGAL;

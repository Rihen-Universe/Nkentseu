...
namespace nkentseu {
	namespace ai {
		namespace data {
			...
			// NkDataset
			// =================================================================
			NkDataset::NkDataset(const NkTensor &features, const NkVector<int32> &labels, uint32 numClasses)
			...
			// NkDataLoader
			// =================================================================
			NkDataLoader::NkDataLoader(const NkDataset &ds, uint32 batchSize, bool shuffle, uint32 seed)
				: mDs(ds), mBatchSize(batchSize ? batchSize : 1), mShuffle(shuffle), mSeed(seed ? seed : 1u) {
				const uint32 n = mDs.Size();
				mOrder.Reserve(n);
				for (uint32 i = 0; i < n; ++i)
					mOrder.PushBack((int32)i);
			...
			NkBatch NkDataLoader::GetBatch(uint32 index) const {
				...
				if (start >= n)
					return batch; // hors bornes -> lot vide
				uint32 B = mBatchSize;
				if (start + B > n)
					B = n - start; // dernier lot partiel
			...
			NkDataset LoadMnist(const char *imagesPath, const char *labelsPath) {
				...
				bool ok = ReadU32BE(fi, magicI) && ReadU32BE(fi, nI) && ReadU32BE(fi, rows) && ReadU32BE(fi, cols) &&
				...
				pixels.Resize(total);
				if (fread(pixels.Data(), 1, total, fi) != total) {
					fclose(fi);
					fclose(fl);
					return invalid;
				}

				NkVector<uint8> lbytes;
				lbytes.Resize(nL);
				if (fread(lbytes.Data(), 1, nL, fl) != nL) {
					fclose(fi);
					fclose(fl);
					return invalid;
				}
				...
				for (nk_size i = 0; i < total; ++i)
					xp[i] = (float)pixels[(uint32)i] / 255.0f;

				NkVector<int32> Y;
				Y.Reserve(nL);
				for (uint32 i = 0; i < nL; ++i)
					Y.PushBack((int32)lbytes[i]);

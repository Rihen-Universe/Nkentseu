...
namespace nkentseu {
    namespace ai {
        namespace data {
            ...
            public:
                NkDataset() = default;
                NkDataset(const NkTensor& features, const NkVector<int32>& labels,
                          uint32 numClasses);

                uint32 Size()       const { return mSize; }
                uint32 FeatureDim() const { return mDim; }
                uint32 NumClasses() const { return mNumClasses; }

                const NkTensor&        Features() const { return mX; }
                const NkVector<int32>& Labels()   const { return mY; }

                bool IsValid() const { return mSize > 0 && mDim > 0; }
            ...
            public:
                ...
                NkDataLoader(const NkDataset& ds, uint32 batchSize,
                             bool shuffle = true, uint32 seed = 1u);

                uint32  NumBatches() const { return mNumBatches; }
                uint32  BatchSize()  const { return mBatchSize; }

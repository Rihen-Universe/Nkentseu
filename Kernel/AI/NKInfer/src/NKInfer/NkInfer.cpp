	...
	namespace ai {
		namespace infer {
			...
			bool LoadParams(const char *path, NkVector<NkVar> &params) {
				...
				bool ok = fread(magic, 1, 4, f) == 4 && magic[0] == kMagic[0] && magic[1] == kMagic[1] &&
						  magic[2] == kMagic[2] && magic[3] == kMagic[3] &&
						  fread(&version, sizeof(uint32), 1, f) == 1 && fread(&count, sizeof(uint32), 1, f) == 1 &&

...
namespace nkentseu {
	namespace ai {
		namespace gen {
			...
			void BuildInterleavedPN(const NkMesh &mesh, NkVector<float> &outVertices, NkVector<uint32> &outIndices);

			// Maillage surfacique CUBIQUE (proxy) d'une grille G×G×G. `iso` = seuil
			// d'occupation ; `cell` = taille d'un voxel dans l'espace monde.
			NkMesh VoxelsToMesh(const float *voxels, uint32 G, float iso = 0.5f, float cell = 1.0f);
			...
			NkMesh SurfaceNets(const float *field, uint32 nx, uint32 ny, uint32 nz, float iso = 0.5f,
							   float cell = 1.0f);
			...
			// suivante (cf ROADMAP).
			NkMesh SurfaceNetsQuads(const float *field, uint32 nx, uint32 ny, uint32 nz, float iso = 0.5f,
									float cell = 1.0f);

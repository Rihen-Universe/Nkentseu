    ...
    namespace ai {
        namespace gen {
            ...
            static void AddQuad(NkMesh& m,
                ...
                for (int i = 0; i < 12; ++i) m.positions.PushBack(p[i]);
                // (a,b,c) et (a,c,d)
                m.triangles.PushBack(base + 0); m.triangles.PushBack(base + 1); m.triangles.PushBack(base + 2);
                m.triangles.PushBack(base + 0); m.triangles.PushBack(base + 2); m.triangles.PushBack(base + 3);
            }

            NkMesh VoxelsToMesh(const float* v, uint32 G, float iso, float cell) {
                NkMesh mesh;
                auto occ = [&](int x, int y, int z) -> bool {
                    if (x < 0 || y < 0 || z < 0 || x >= (int)G || y >= (int)G || z >= (int)G) return false;
                ...
                for (int x = 0; x < (int)G; ++x) {
                    ...
                    const float x1 = (float)(x + 1) * cell, y1 = (float)(y + 1) * cell, z1 = (float)(z + 1) * cell;

                    // Une face n'est émise que si le voisin dans cette direction est vide.
                    // -z (face avant)
                    if (!occ(x, y, z - 1)) AddQuad(mesh, x0,y0,z0, x0,y1,z0, x1,y1,z0, x1,y0,z0);
                    // +z (arrière)
                    if (!occ(x, y, z + 1)) AddQuad(mesh, x0,y0,z1, x1,y0,z1, x1,y1,z1, x0,y1,z1);
                    // -y (bas)
                    if (!occ(x, y - 1, z)) AddQuad(mesh, x0,y0,z0, x1,y0,z0, x1,y0,z1, x0,y0,z1);
                    // +y (haut)
                    if (!occ(x, y + 1, z)) AddQuad(mesh, x0,y1,z0, x0,y1,z1, x1,y1,z1, x1,y1,z0);
                    // -x (gauche)
                    if (!occ(x - 1, y, z)) AddQuad(mesh, x0,y0,z0, x0,y0,z1, x0,y1,z1, x0,y1,z0);
                    // +x (droite)
                    if (!occ(x + 1, y, z)) AddQuad(mesh, x1,y0,z0, x1,y1,z0, x1,y1,z1, x1,y0,z1);
                }
                return mesh;
            }
            ...
            NkMesh SurfaceNets(const float* f, uint32 nx, uint32 ny, uint32 nz,
                               float iso, float cell) {
                NkMesh mesh;
                ...
                auto solid = [&](uint32 x, uint32 y, uint32 z) -> bool {
                    return sample(x, y, z) > iso;
                };
                // Index de sommet par cellule (-1 = pas de surface).
                NkVector<int32> buffer; buffer.Resize(Cx * Cy * Cz);
                for (uint32 i = 0; i < buffer.Size(); ++i) buffer[i] = -1;
                auto cellIdx = [&](uint32 x, uint32 y, uint32 z) -> uint32 {
                    return x + Cx * (y + Cy * z);
                };
                ...
                for (uint32 x = 0; x < Cx; ++x) {
                    float g[8]; int mask = 0;
                    for (int i = 0; i < 8; ++i) {
                        g[i] = sample(x + kCorner[i][0], y + kCorner[i][1], z + kCorner[i][2]);
                        if (g[i] > iso) mask |= (1 << i);
                    }
                    if (mask == 0 || mask == 255) continue;   // cellule entièrement in/out

                    // Position = moyenne des points d'intersection des arêtes.
                    float vx = 0.f, vy = 0.f, vz = 0.f; int ec = 0;
                    for (int e = 0; e < 12; ++e) {
                ...
                for (uint32 x = 0; x < Cx; ++x) {
                    ...
                    for (int d = 0; d < 3; ++d) {
                        ...
                        if (s0 == sd) continue;   // pas de changement de signe
                        ...
                        int32 q[4]; bool ok = true;
                        for (int c = 0; c < 4; ++c) {
                            int cx = (int)x, cy = (int)y, cz = (int)z;
                            int du = off[c][0], dv = off[c][1];
                            if (u == 0) cx += du; else if (u == 1) cy += du; else cz += du;
                            if (w == 0) cx += dv; else if (w == 1) cy += dv; else cz += dv;
                            if (cx < 0 || cy < 0 || cz < 0) { ok = false; break; }
                            q[c] = buffer[cellIdx((uint32)cx, (uint32)cy, (uint32)cz)];
                            if (q[c] < 0) { ok = false; break; }
                        }
                        if (!ok) continue;

                        // Deux triangles ; winding selon le côté solide (normales cohérentes).
                        if (!s0) {
                            mesh.triangles.PushBack(q[0]); mesh.triangles.PushBack(q[1]); mesh.triangles.PushBack(q[2]);
                            mesh.triangles.PushBack(q[0]); mesh.triangles.PushBack(q[2]); mesh.triangles.PushBack(q[3]);
                        } else {
                            mesh.triangles.PushBack(q[0]); mesh.triangles.PushBack(q[2]); mesh.triangles.PushBack(q[1]);
                            mesh.triangles.PushBack(q[0]); mesh.triangles.PushBack(q[3]); mesh.triangles.PushBack(q[2]);
                        }
                    }
                }
                return mesh;
            ...
            NkMesh SurfaceNetsQuads(const float* f, uint32 nx, uint32 ny, uint32 nz,
                                    float iso, float cell) {
                NkMesh mesh;
                if (nx < 2 || ny < 2 || nz < 2) return mesh;
                ...
                for (uint32 x = 0; x < Cx; ++x) {
                    ...
                    for (int d = 0; d < 3; ++d) {
                        ...
                        if (!ok) continue;
                        if (!s0) { mesh.quads.PushBack(q[0]); mesh.quads.PushBack(q[1]); mesh.quads.PushBack(q[2]); mesh.quads.PushBack(q[3]); }
                        else     { mesh.quads.PushBack(q[0]); mesh.quads.PushBack(q[3]); mesh.quads.PushBack(q[2]); mesh.quads.PushBack(q[1]); }
                    }
                }
                return mesh;
            }
            ...
            NkMesh DecimateClustering(const NkMesh& src, uint32 gridResolution) {
                ...
                float ext[3];
                for (int k = 0; k < 3; ++k) { ext[k] = mx[k] - mn[k]; if (ext[k] < 1e-6f) ext[k] = 1e-6f; }

                // Accumulateurs par cellule (grille dense G³).
                const uint32 NC = G * G * G;
                NkVector<float>  acc;  acc.Resize(NC * 3);
                NkVector<uint32> cnt;  cnt.Resize(NC);
                NkVector<int32>  vidx; vidx.Resize(NC);   // index du sommet de sortie par cellule
                for (uint32 i = 0; i < NC; ++i) { acc[i*3]=acc[i*3+1]=acc[i*3+2]=0.f; cnt[i]=0; vidx[i]=-1; }
                ...
                auto cellOf = [&](uint32 v) -> uint32 {
                    ...
                    for (int k = 0; k < 3; ++k) {
                        ...
                        if (ci < 0) ci = 0; if (ci >= (int)G) ci = (int)G - 1;
                        c[k] = (uint32)ci;
                    }
                ...
                NkVector<uint32> remap; remap.Resize(vc);
                ...
                for (uint32 t = 0; t < tc; ++t) {
                    ...
                    if (a == b || b == c || a == c) continue;    // dégénéré -> supprimé
                    out.triangles.PushBack(a); out.triangles.PushBack(b); out.triangles.PushBack(c);
                }
                return out;
            }
            ...
            void ComputeNormals(NkMesh& mesh) {
                ...
                for (uint32 i = 0; i < vc * 3; ++i) mesh.normals[i] = 0.f;

                auto accum = [&](uint32 a, uint32 b, uint32 c) {
                    const float* pa=&mesh.positions[a*3]; const float* pb=&mesh.positions[b*3]; const float* pc=&mesh.positions[c*3];
                    ...
                    uint32 idx[3]={a,b,c};
                    for (int k=0;k<3;++k){ mesh.normals[idx[k]*3+0]+=nx; mesh.normals[idx[k]*3+1]+=ny; mesh.normals[idx[k]*3+2]+=nz; }
                };
                const uint32 tc = mesh.TriangleCount();
                for (uint32 t=0;t<tc;++t) accum(mesh.triangles[t*3+0], mesh.triangles[t*3+1], mesh.triangles[t*3+2]);
                const uint32 qc = mesh.QuadCount();
                for (uint32 q=0;q<qc;++q){ // quad -> 2 triangles pour l'accumulation
                    uint32 a=mesh.quads[q*4+0],b=mesh.quads[q*4+1],c=mesh.quads[q*4+2],d=mesh.quads[q*4+3];
                    accum(a,b,c); accum(a,c,d);
                }
                for (uint32 i=0;i<vc;++i){
                    float* n=&mesh.normals[i*3]; float L=sqrtf(n[0]*n[0]+n[1]*n[1]+n[2]*n[2]);
                    if (L>1e-8f){ n[0]/=L; n[1]/=L; n[2]/=L; } else { n[0]=0.f; n[1]=1.f; n[2]=0.f; }
                }
            }

            void BuildInterleavedPN(const NkMesh& mesh, NkVector<float>& outV, NkVector<uint32>& outI) {
                NkMesh tmp = mesh;
                if (!tmp.HasNormals()) ComputeNormals(tmp);
                const uint32 vc = tmp.VertexCount();
                outV.Resize(vc * 6);
                for (uint32 i=0;i<vc;++i){
                    outV[i*6+0]=tmp.positions[i*3+0]; outV[i*6+1]=tmp.positions[i*3+1]; outV[i*6+2]=tmp.positions[i*3+2];
                    outV[i*6+3]=tmp.normals[i*3+0];   outV[i*6+4]=tmp.normals[i*3+1];   outV[i*6+5]=tmp.normals[i*3+2];
                }
                outI.Resize(0);
                for (uint32 i=0;i<tmp.triangles.Size();++i) outI.PushBack(tmp.triangles[i]);
                // Quads -> triangles pour un index buffer standard.
                const uint32 qc = tmp.QuadCount();
                for (uint32 q=0;q<qc;++q){
                    uint32 a=tmp.quads[q*4+0],b=tmp.quads[q*4+1],c=tmp.quads[q*4+2],d=tmp.quads[q*4+3];
                    outI.PushBack(a);outI.PushBack(b);outI.PushBack(c);
                    outI.PushBack(a);outI.PushBack(c);outI.PushBack(d);
                }
            }
            ...
            bool SaveMeshObj(const char* path, const NkMesh& mesh) {
                ...
                for (uint32 i = 0; i < tc; ++i) {
                    uint32 a=mesh.triangles[i*3+0]+1, b=mesh.triangles[i*3+1]+1, c=mesh.triangles[i*3+2]+1;
                    if (hasN) fprintf(f, "f %u//%u %u//%u %u//%u\n", a,a, b,b, c,c);
                    else      fprintf(f, "f %u %u %u\n", a, b, c);
                }
                const uint32 qc = mesh.QuadCount();          // faces quad (f a b c d)
                for (uint32 i = 0; i < qc; ++i) {
                    uint32 a=mesh.quads[i*4+0]+1, b=mesh.quads[i*4+1]+1, c=mesh.quads[i*4+2]+1, d=mesh.quads[i*4+3]+1;
                    if (hasN) fprintf(f, "f %u//%u %u//%u %u//%u %u//%u\n", a,a, b,b, c,c, d,d);
                    else      fprintf(f, "f %u %u %u %u\n", a, b, c, d);
                }
                fclose(f);
                return true;
            }

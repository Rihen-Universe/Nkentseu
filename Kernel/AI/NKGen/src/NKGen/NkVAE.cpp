    ...
    namespace ai {
        namespace gen {
            ...
            NkVAE::NkVAE(uint32 inDim, uint32 hidden, uint32 latent, uint32 seed)
                : mEnc1     (inDim,  hidden, seed +  1u),
                  mEncMu    (hidden, latent, seed + 13u),
                  mEncLogvar(hidden, latent, seed + 29u),
                  mDec1     (latent, hidden, seed + 41u),
                  mDec2     (hidden, inDim,  seed + 57u),
                  mInDim(inDim), mLatent(latent) {}
            ...
            NkVar KLDivergence(const NkVar& mu, const NkVar& logvar) {
                ...
                NkVar inner = autograd::AddScalar(
                                  autograd::Sub(autograd::Sub(logvar, muSq), expLV), 1.0);

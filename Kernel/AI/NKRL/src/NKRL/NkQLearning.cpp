...
namespace nkentseu {
    namespace ai {
        namespace rl {
            ...
            NkQLearning::NkQLearning(uint32 numStates, uint32 numActions,
                                     float alpha, float gamma, float epsilon, uint32 seed)
                : mNumStates(numStates), mNumActions(numActions),
                  mAlpha(alpha), mGamma(gamma), mEpsilon(epsilon), mRng(seed ? seed : 1u) {
                mQ.Resize(numStates * numActions);
                for (uint32 i = 0; i < mQ.Size(); ++i) mQ[i] = 0.0f;
            }
            ...
            uint32 NkQLearning::SelectAction(uint32 state) {
                if (Rand01() < mEpsilon) {
                    ...
                    if (a >= mNumActions) a = mNumActions - 1;
                    return a;
                }
            ...
            void NkQLearning::Update(uint32 s, uint32 a, float r, uint32 sNext, bool done) {
                ...
                if (!done) {
                    ...
                    float best = row[0];
                    for (uint32 k = 1; k < mNumActions; ++k) if (row[k] > best) best = row[k];
                    target += mGamma * best;
                }

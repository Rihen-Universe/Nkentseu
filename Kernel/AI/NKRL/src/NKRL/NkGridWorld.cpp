...
namespace nkentseu {
	namespace ai {
		namespace rl {
			...
			NkGridWorld::NkGridWorld(uint32 size, uint32 startState, uint32 goalState, const NkVector<uint32> &holes,
									 float stepCost)
				: mSize(size ? size : 1), mStart(startState), mGoal(goalState), mCur(startState), mStepCost(stepCost),
				  mHoles(holes) {
			}

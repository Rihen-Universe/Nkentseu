...
namespace nkentseu {
    namespace ai {
        namespace nn {
            ...
            public:
                NkConv2D() = default;
                NkConv2D(uint32 inChannels, uint32 outChannels, uint32 kernel,
                         uint32 stride = 1, uint32 pad = 0, uint32 seed = 1u);
            ...
            public:
                ...
                NkConvTranspose2D(uint32 inChannels, uint32 outChannels, uint32 kernel,
                                  uint32 stride = 1, uint32 pad = 0, uint32 seed = 1u);
                NkVar Forward(const NkVar& x) const;
                void  Parameters(NkVector<NkVar>& out) const;
                const NkVar& Weight() const { return mW; }
            ...
            public:
                NkConv3D() = default;
                NkConv3D(uint32 inChannels, uint32 outChannels, uint32 kernel,
                         uint32 stride = 1, uint32 pad = 0, uint32 seed = 1u);
                NkVar Forward(const NkVar& x) const;
                void  Parameters(NkVector<NkVar>& out) const;
            private:
                NkVar mW, mB; uint32 mStride = 1, mPad = 0;
            };
            ...
            public:
                ...
                NkConvTranspose3D(uint32 inChannels, uint32 outChannels, uint32 kernel,
                                  uint32 stride = 1, uint32 pad = 0, uint32 seed = 1u);
                NkVar Forward(const NkVar& x) const;
                void  Parameters(NkVector<NkVar>& out) const;
            private:
            ...
            inline NkVar Flatten(const NkVar& x) {
                ...
                for (uint32 i = 1; i < s.Size(); ++i) rest *= s[i];

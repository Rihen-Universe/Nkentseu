...
namespace nkentseu {
    namespace ai {
        namespace nn {
            ...
            NkConv2D::NkConv2D(uint32 inChannels, uint32 outChannels, uint32 kernel,
                               uint32 stride, uint32 pad, uint32 seed)
                : mStride(stride), mPad(pad) {
                NkTensor W = NkTensor::Zeros(NkShape{ (int64)outChannels, (int64)inChannels,
                                                      (int64)kernel, (int64)kernel });
            ...
            NkConvTranspose2D::NkConvTranspose2D(uint32 inChannels, uint32 outChannels, uint32 kernel,
                                                 uint32 stride, uint32 pad, uint32 seed)
                : mStride(stride), mPad(pad) {
                // Poids [Cin, Cout, k, k] (convention conv transposée).
                NkTensor W = NkTensor::Zeros(NkShape{ (int64)inChannels, (int64)outChannels,
                                                      (int64)kernel, (int64)kernel });
                NkTensor b = NkTensor::Zeros(NkShape{ (int64)1, (int64)outChannels, (int64)1, (int64)1 });
            ...
            static NkVar MakeConvWeight3D(uint32 d0, uint32 d1, uint32 k, uint32 fanIn, uint32 seed) {
                ...
                double r = (fanIn > 0) ? std::sqrt(6.0 / (double)fanIn) : 0.1;
                uint32 s = seed ? seed : 1u; float* wp = W.DataAs<float>();
                const int64 n = NkShapeNumel(W.Shape());
                for (int64 i = 0; i < n; ++i) {
                    s = s * 1664525u + 1013904223u;
                    double u = (double)((s >> 8) & 0xFFFFu) / 65535.0 * 2.0 - 1.0;
            ...
            void NkConv3D::Parameters(NkVector<NkVar>& out) const { out.PushBack(mW); out.PushBack(mB); }

            NkConvTranspose3D::NkConvTranspose3D(uint32 inC, uint32 outC, uint32 k, uint32 stride, uint32 pad, uint32 seed)
                : mStride(stride), mPad(pad) {
            ...
            void NkConvTranspose3D::Parameters(NkVector<NkVar>& out) const { out.PushBack(mW); out.PushBack(mB); }

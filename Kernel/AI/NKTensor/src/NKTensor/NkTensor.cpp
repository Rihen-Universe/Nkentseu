...
namespace nkentseu {
	namespace ai {
		...
		void NkTensorStorage::Release(NkTensorStorage *s) {
			...
			if (--s->refcount == 0) {
				if (s->data)
					Alloc().Deallocate(s->data);
				if (s->gpuBuffer)
					NkTensorGpu::Get().DestroyBuffer(s->gpuBuffer);
				Alloc().Delete(s);
			}
		}
		...
		NkTensor::NkTensor(const NkTensor &other) {
			ShareFrom(other);
		}
		...
		NkTensor::NkTensor(NkTensor &&o) noexcept
			: mStorage(o.mStorage), mOffset(o.mOffset), mShape(o.mShape), mStrides(o.mStrides), mDType(o.mDType),
			  mDevice(o.mDevice) {
			o.mStorage = nullptr;
		}
		...
		NkTensor NkTensor::Arange(double start, double stop, double step, NkDType dtype, NkDevice device) {
			...
			if (step != 0.0) {
				...
				if (start + (double)n * step < stop)
					++n;
			}
			NkTensor t = Empty(NkShape{n}, dtype, device);
			for (int64 i = 0; i < n; i++)
				t.SetItem(NkShape{i}, start + (double)i * step);
			return t;
		}
		...
		// Infos
		// =====================================================================
		int64 NkTensor::Numel() const {
			return NkShapeNumel(mShape);
		}

		bool NkTensor::IsContiguous() const {
			NkShape c = NkContiguousStrides(mShape);
			if (c.Size() != mStrides.Size())
				return false;
		...
		char *NkTensor::BytePtr() const {
			if (!mStorage || !mStorage->data)
				return nullptr;
			return (char *)mStorage->data + (nk_size)mOffset * NkDTypeSize(mDType);
		}

		void *NkTensor::RawData() {
			return BytePtr();
		}

		const void *NkTensor::RawData() const {
			return BytePtr();
		}
		...
		NkTensor NkTensor::Reshape(const NkShape &newShape) const {
			...
			NkTensor v;
			v.mStorage = mStorage;
			NkTensorStorage::Retain(v.mStorage);
			v.mOffset = mOffset;
			v.mShape = resolved;
		...
		NkTensor NkTensor::Transpose(uint32 dim0, uint32 dim1) const {
			...
			if (dim0 < v.mShape.Size() && dim1 < v.mShape.Size()) {
				int64 s = v.mShape[dim0];
				v.mShape[dim0] = v.mShape[dim1];
				v.mShape[dim1] = s;
				int64 t = v.mStrides[dim0];
				v.mStrides[dim0] = v.mStrides[dim1];
				v.mStrides[dim1] = t;
			}
			return v;
		}
		...
		void NkTensor::Print(const char *name) const {
			...
			printf("] %s (%s, %s)\n", NkDTypeName(mDType), mDevice == NkDevice::NK_CPU ? "cpu" : "gpu",

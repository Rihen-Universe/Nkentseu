    ...
    namespace ai {
        namespace ops {
            ...
            static void ZeroIndex(NkShape& idx, uint32 rank) {
                idx.Resize(rank);
                for (uint32 i = 0; i < rank; i++) idx[i] = 0;
            }
            ...
            static NkShape BroadcastShape(const NkShape& a, const NkShape& b, bool& ok) {
                ...
                for (uint32 i = 0; i < r; i++) {
                    ...
                    else { ok = false; out[i] = 0; }
                }
                return out;
            }
            // Strides de `in` alignés sur `outShape` (0 = dimension diffusée).
            static NkShape AlignedStrides(const NkShape& inShape, const NkShape& inStrides,
                                          const NkShape& outShape) {
                const uint32 ri = (uint32)inShape.Size(), ro = (uint32)outShape.Size();
                NkShape s; s.Resize(ro);
            ...
            template<typename T> static inline T ApplyBin(BinOp op, T x, T y) {
                switch (op) {
                    case BinOp::Add: return (T)(x + y);
                    case BinOp::Sub: return (T)(x - y);
                    case BinOp::Mul: return (T)(x * y);
                    case BinOp::Div: return (y != (T)0) ? (T)(x / y) : (T)0;
                }
                return x;
            }
            ...
            static NkTensor Binary(const NkTensor& a, const NkTensor& b, BinOp op) {
                if (a.DType() != b.DType()) {
                    fprintf(stderr, "[NkTensorOps] binaire : dtype incompatibles (%s vs %s)\n",
                            NkDTypeName(a.DType()), NkDTypeName(b.DType()));
                    return NkTensor();
                }
                bool ok = false;
                NkShape os = BroadcastShape(a.Shape(), b.Shape(), ok);
                if (!ok) { fprintf(stderr, "[NkTensorOps] formes non-broadcastables\n"); return NkTensor(); }
                ...
                NK_DTYPE_DISPATCH(a.DType(), T, {
                    NkShape idx; ZeroIndex(idx, (uint32)os.Size());
                    int64 k = 0;
                    do {
                        int64 oa = 0, ob = 0;
                        for (uint32 d = 0; d < os.Size(); d++) { oa += idx[d] * sa[d]; ob += idx[d] * sb[d]; }
            ...
            static NkTensor Unary(const NkTensor& a, UnOp op, bool floatOnly) {
                ...
                NK_DTYPE_DISPATCH(a.DType(), T, {
                    if (contig) {
                        ...
                        for (int64 i = 0; i < n; i++) d[i] = ApplyUn<T>(op, s[i]);
                    } else {
                        NkShape idx; ZeroIndex(idx, (uint32)sh.Size());
                        int64 k = 0;
                        do {
                            int64 off = 0;
                            for (uint32 d = 0; d < sh.Size(); d++) off += idx[d] * st[d];
            ...
            NkTensor AddScalar(const NkTensor& a, double s) {
                ...
                NK_DTYPE_DISPATCH(a.DType(), T, {
                    const T* src = ac.DataAs<T>(); T* d = out.DataAs<T>();
                    const T sv = (T)s;
                    for (int64 i = 0; i < n; i++) d[i] = (T)(src[i] + sv);
                });
                return out;
            }
                ...
                NK_DTYPE_DISPATCH_FLOAT(a.DType(), T, {
                    ...
                    for (int64 i = 0; i < M; i++) {
                        for (int64 k = 0; k < K; k++) {
                            ...
                            const T* Brow = &B[k * N];
                            T* Crow = &C[i * N];
                            for (int64 j = 0; j < N; j++) Crow[j] = (T)(Crow[j] + aik * Brow[j]);
                        }
                    }
                });
                return out;
            ...
            static NkTensor ReduceAxis(const NkTensor& a, uint32 axis, RedKind kind) {
                ...
                if (r == 1) {
                    if (kind == RedKind::Sum)  return Sum(a);
                    if (kind == RedKind::Mean) return Mean(a);
                    if (kind == RedKind::Max)  return Max(a);
                    // Argmax global
                    const int64 n = a.Shape()[0];
                    double best = 0.0; int64 bi = 0; bool first = true;
                    for (int64 j = 0; j < n; j++) {
                        const double v = a.GetItem(NkShape{ j });
                        if (first || v > best) { best = v; bi = j; first = false; }
                    }
                ...
                NkShape outShape; outShape.Resize(r - 1);
                for (uint32 i = 0, oi = 0; i < r; i++) if (i != axis) outShape[oi++] = a.Shape()[i];

                const NkDType outDt = (kind == RedKind::Argmax) ? NkDType::NK_I64 : a.DType();
                NkTensor out = NkTensor::Empty(outShape, outDt);

                NkShape outIdx; ZeroIndex(outIdx, (uint32)outShape.Size());
                NkShape inIdx;  inIdx.Resize(r);
                do {
                    // Reconstruit l'index d'entrée (l'axe réduit sera balayé).
                    for (uint32 i = 0, oi = 0; i < r; i++) inIdx[i] = (i == axis) ? 0 : outIdx[oi++];

                    double acc = 0.0, best = 0.0; int64 bi = 0; bool first = true;
                    for (int64 j = 0; j < axisSize; j++) {
                        inIdx[axis] = j;
                        const double v = a.GetItem(inIdx);
                        switch (kind) {
                            case RedKind::Sum:
                            case RedKind::Mean:   acc += v; break;
                            case RedKind::Max:    if (first || v > best) { best = v; first = false; } break;
                            case RedKind::Argmax: if (first || v > best) { best = v; bi = j; first = false; } break;
                        }
                    }
                    double res = 0.0;
                    switch (kind) {
                        case RedKind::Sum:    res = acc; break;
                        case RedKind::Mean:   res = acc / (axisSize > 0 ? (double)axisSize : 1.0); break;
                        case RedKind::Max:    res = best; break;
                        case RedKind::Argmax: res = (double)bi; break;
                    }
            ...
            NkTensor Sum   (const NkTensor& a, uint32 axis) { return ReduceAxis(a, axis, RedKind::Sum);    }
            NkTensor Mean  (const NkTensor& a, uint32 axis) { return ReduceAxis(a, axis, RedKind::Mean);   }
            NkTensor Max   (const NkTensor& a, uint32 axis) { return ReduceAxis(a, axis, RedKind::Max);    }
            NkTensor Argmax(const NkTensor& a, uint32 axis) { return ReduceAxis(a, axis, RedKind::Argmax); }

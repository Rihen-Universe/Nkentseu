		...
		class NkTensor {
			public:
				...
				static NkTensor Ones(const NkShape &shape, NkDType dtype = NkDType::NK_F32,
									 NkDevice device = NkDevice::NK_CPU);
				static NkTensor Full(const NkShape &shape, double value, NkDType dtype = NkDType::NK_F32,
									 NkDevice device = NkDevice::NK_CPU);
				...
				static NkTensor Arange(double start, double stop, double step = 1.0, NkDType dtype = NkDType::NK_F32,
									   NkDevice device = NkDevice::NK_CPU);
				// Matrice identité n x n.
				static NkTensor Eye(int64 n, NkDType dtype = NkDType::NK_F32, NkDevice device = NkDevice::NK_CPU);
				...
				NkDevice Device() const {
					return mDevice;
				}

				const NkShape &Shape() const {
					return mShape;
				}

				const NkShape &Strides() const {
					return mStrides;
				}

				nk_size ItemSize() const {
					return NkDTypeSize(mDType);
				}
